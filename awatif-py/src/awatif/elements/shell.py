...
def shell_q4_stiffness(coords_xy: np.ndarray, E: float, nu: float, t: float,
    ...
    # ── Mindlin-Reissner bending (Uz, Rx, Ry) ──
    if include_bending:
        D_b = (E * t**3 / (12 * (1 - nu**2))) * np.array([
            [1, nu, 0], [nu, 1, 0], [0, 0, (1 - nu) / 2],
        ])
        ks = 5/6  # shear correction
        D_s = ks * G * t * np.eye(2)
        K_b = np.zeros((12, 12))
        for xi, eta, w in GAUSS_2x2:
            N, N_xi, N_eta = _shape_q4(xi, eta)
            _, detJ, dN = _jacobian_2d(coords_xy, N_xi, N_eta)
            # B_bending: curvatures κx, κy, κxy
            # DOFs per node: [Uz, Rx, Ry] @ [3i, 3i+1, 3i+2]
            # φx = Ry (rot around y → causes tilt in x direction)
            # φy = Rx (rot around x → causes tilt in y)
            # Wait: standard convention κx = ∂φx/∂x, κy = ∂φy/∂y, κxy = ∂φx/∂y + ∂φy/∂x
            # With φx = -Ry (sign convention for positive bending), φy = Rx
            Bb = np.zeros((3, 12))
            Bs = np.zeros((2, 12))
            for i in range(4):
                Bb[0, 3*i+2] = -dN[0, i]   # κx = -∂Ry/∂x
                Bb[1, 3*i+1] =  dN[1, i]   # κy = ∂Rx/∂y
                Bb[2, 3*i+1] =  dN[0, i]   # κxy term
                Bb[2, 3*i+2] = -dN[1, i]
                # Shear strains: γxz = ∂Uz/∂x - Ry, γyz = ∂Uz/∂y + Rx
                Bs[0, 3*i]   = dN[0, i]
                Bs[0, 3*i+2] = -N[i]
                Bs[1, 3*i]   = dN[1, i]
                Bs[1, 3*i+1] = N[i]
            K_b += w * (Bb.T @ D_b @ Bb + Bs.T @ D_s @ Bs) * detJ
