"""
SAFE API — Losa de Cimentación 6×8×0.50m, 6 columnas grilla 2×3.

Replica el caso Hekatan cli_losa.mjs. Mesh 12×16 = 192 áreas Q4
explícitas (paridad exacta con Hekatan). Springs Winkler vía
DatabaseTables `SubModulus` (workaround del bug SetAreaSpringProp U3
ignorado, ver benchmarks/safe/README.md gotcha #8).

Uso:
  python safe_api_losa.py [--json=out.json] [--keep-open]
"""
...
SAVE_PATH = str(HERE / "Losa_Hekatan_via_API.fdb")

P = {
    "Lz": 6.0, "Bz": 8.0, "tz": 0.50,
    "E_kNm2": 24855e3, "nu": 0.20, "rho_kNm3": 24.0,
    "ks_kNm3": 19613.0,
    "P_kN": 20 * 9.80665,         # 20 tonf por columna
    "nx": 12, "ny": 16,           # mesh 12×16 (paridad Hekatan, dx=dy=0.5m)
}
# 6 columnas grilla 2×3
COL_XS = [1.5, 4.5]
COL_YS = [2.0, 4.0, 6.0]
COLUMNS = [(x, y) for x in COL_XS for y in COL_YS]
...
sample_targets = [
    ...
    ("col_1 (1.5, 2.0)",       1.5, 2.0),
    ("col_2 (4.5, 2.0)",       4.5, 2.0),
    ("col_3 (1.5, 4.0)",       1.5, 4.0),
    ("col_4 (4.5, 4.0)",       4.5, 4.0),
    ("col_5 (1.5, 6.0)",       1.5, 6.0),
    ("col_6 (4.5, 6.0)",       4.5, 6.0),
    ("entre col 1-2 (3.0, 2.0)", 3.0, 2.0),
    ("entre col 3-4 (3.0, 4.0)", 3.0, 4.0),
    ("entre col 5-6 (3.0, 6.0)", 3.0, 6.0),
    ("entre col 1-3 (1.5, 3.0)", 1.5, 3.0),
    ("entre col 3-5 (1.5, 5.0)", 1.5, 5.0),
    ("centro losa (3.0, 4.0)",   3.0, 4.0),
    ("mid-borde (3.0, 0)",       3.0, 0.0),
]
...
out = {
    "solver": "SAFE 20 via API (construido desde cero, mesh 12×16 + SubModulus override)",
    "case": "Losa de Cimentación 6×8×0.50m, 6 columnas",
    "saved_at": SAVE_PATH,
    "params": P,
...
print("\n" + "="*58)
print(f"  SAFE API - Losa {P['Lz']}×{P['Bz']}×{P['tz']}m, {len(COLUMNS)} col × {P['P_kN']:.1f} kN")
print("="*58)
