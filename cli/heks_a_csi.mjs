#!/usr/bin/env node
/**
 * Un .heks a los textos de CSI (.e2k y .s2k) por el MISMO camino que la app (cliModeler +
...
export function desdeHeks(texto, cftAs) {
  ...
  return { e2k: exportE2k({ ...comun, weightMode: "manual" }), s2k: exportS2k({ ...comun, selfWtMult: 0, cftAs }), n: states.nodes.val.length, e: states.elements.val.length };
}`, "heks-a-csi");
...
writeFileSync(salida + ".e2k", e2k, "utf-8"); writeFileSync(salida + ".s2k", r.s2k, "utf-8");
console.log(`${heks}: ${r.n} nudos, ${r.e} elementos -> ${salida}.e2k + .s2k${opt.meshtype ? " (OBJMESHTYPE " + opt.meshtype + ")" : ""}`);
