...
export function exportar(id, over) {
  ...
  return { e2k: exportE2k({ ...arg, diaphragm: "none", weightMode: "manual" }),
           ...
           modelo: { nodes: st.nodes.val, elements: st.elements.val,
...
const res = {};
for (const [id, over] of CASOS) {
  const r = m.exportar(id, over);
  writeFileSync(`${DIR}/${id}.e2k`, r.e2k, "utf8");
  writeFileSync(`${DIR}/${id}.s2k`, r.s2k, "utf8");
  res[id] = { nodos: r.nodos, d: r.d, modelo: r.modelo };
  const nPt = (r.e2k.match(/^  POINT "/gm)||[]).length;
  const nSt = (r.e2k.match(/^  STORY "/gm)||[]).length;
  console.log(`${id}: ${r.nodos.length} nudos → e2k con ${nPt} POINT y ${nSt} STORY`);
}
