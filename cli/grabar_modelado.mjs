...
await pag.goto(`http://localhost:4712${BASE}workspace/?t=new-blank`,
...
// ── Apoyos en los dos pies ──────────────────────────────────────────────────
await decir("3 · Apoyo: sin apoyos la matriz es singular y no hay solucion");
await boton("Apoyo");
await enLienzo(...P.pieIzq);
await enLienzo(...P.pieDer);
await esperar(900);

// ── Carga en el dintel ──────────────────────────────────────────────────────
await decir("4 · Carga: -10 kN en el nudo que se toque");
await boton("Carga");
await enLienzo(...P.topIzq);
await enLienzo(...P.topDer);
await esperar(1200);

// ── Y a mirarlo en 3D ───────────────────────────────────────────────────────
await decir("5 · A 3D a mirar el resultado");
await boton("3D");
await esperar(1300);
...
try {
  execFileSync("ffmpeg", ["-y", "-framerate", "11", "-i", join(FR, "f_%04d.png"),
    ...
    join(OUT, "modelado.gif")], { stdio: "pipe" });
