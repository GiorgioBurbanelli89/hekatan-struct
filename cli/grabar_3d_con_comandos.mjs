...
const FR = join(OUT, "tresd_comandos");
...
await pag.goto(`http://localhost:4716${BASE}workspace/?t=new-blank`,
...
// ═══ EN VISTA 3D, CON COMANDOS ═════════════════════════════════════════════
// Sin tocar un solo boton de herramienta: la letra elige, y las coordenadas
// `x,y,z` ponen el punto exacto. La Z va escrita, asi que no hace falta
// cambiar de plano de trabajo para subir.
await decir("Con comandos · 1 · A 3D: se dibuja SIN cambiar de vista");
await foto(); await foto();
await boton("3D");
await esperar(1300);

await decir("2 · Tecla L, y las coordenadas a mano: 0,0,0 → 6,0,0 → …");
await pag.keyboard.press("l");
await esperar(600);
for (const c of ["0,0,0", "6,0,0", "6,4,0", "0,4,0", "0,0,0"]) {
  await decir(`Escribiendo  ${c}`);
  await teclearVisible(c);
}
await pag.keyboard.press("Escape");
await esperar(900);
...
// La Z va en la coordenada: se sube sin cambiar de plano ni de vista.
await decir("3 · La Z va escrita: 0,0,3 sube 3 m sin cambiar de plano");
await pag.keyboard.press("l");
await esperar(500);
for (const c of ["0,0,0", "0,0,3", "6,0,3", "6,0,0"]) {
  await decir(`Escribiendo  ${c}`);
  await teclearVisible(c);
}
await pag.keyboard.press("Escape");
