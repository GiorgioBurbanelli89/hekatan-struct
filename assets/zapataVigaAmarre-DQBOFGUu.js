...
let __tla = Promise.all([
  ...
  $a = {
    ...
    build(e, s) {
      var _a;
      const r = e.Lz1, u = e.Bz1, f = e.Lv, M = e.Bv, h = e.Hv, x = e.Lz2, z = e.Bz2, A = e.tz, y = e.bc;
      e.Hp;
      const k = 9.80665, I = (e.useDead ?? 1) >= 0.5 ? 1 : 0, B = (e.useLive ?? 1) >= 0.5 ? 1 : 0, ne = (I * (e.P1 ?? 0) + B * (e.P1_L ?? 0)) * k, te = (I * (e.P2 ?? 0) + B * (e.P2_L ?? 0)) * k, G = e.ks, se = (I * (e.M1x ?? 0) + B * (e.M1x_L ?? 0)) * k, re = (I * (e.M1y ?? 0) + B * (e.M1y_L ?? 0)) * k, de = (I * (e.M2x ?? 0) + B * (e.M2x_L ?? 0)) * k, ce = (I * (e.M2y ?? 0) + B * (e.M2y_L ?? 0)) * k, T = Math.round(e.nSubX), _ = Math.round(e.nSubY), w = (u - z) / 2, Z = y / 2, O = u / 2, L = r + f + x / 2, H = z / 2 + w, le = O;
      function Y(a, o, n, t) {
        const d = [
          a,
          ...n.filter((l) => l > a && l < o),
      ...
      const Se = 0.5, We = W * 1e3, Ue = Ye * 1e3, Je = y * y * 100, De = y ** 4 / 12 * 100, Qe = 0.14 * y ** 4 * 100;
      ...
      const ea = Ee(Ie, Z, O, R, X, p), aa = Ee(we, L, H, j, q, N);
      ...
      const Fe = r / T, Ae = u / _, oa = x / T, na = z / _, Be = 0.5, S = [], ae = [];
      ...
      const ta = new Set(ae), Ce = Xe * 12, ie = (_a = document.querySelector("#viewer")) == null ? void 0 : _a.__settings, Ve = (a, o, n = 1) => {
        const t = a ? o : 0, c = -(be * Math.max(t, 1) + ka), b = n > 0 ? n : n < 0 ? -1 / n : 1, l = Ia * b, m = wa * b, D = [];
        for (const C of ae) {
          if (!ta.has(C)) continue;
          const v = s.nodes.rawVal[C];
          if (!v) continue;
          const i = v[0], E = v[1], g = Ge == null ? void 0 : Ge.get(C), Pe = (P) => Number.isFinite(P) ? P : 0, la = g ? Pe(g[0]) : 0, ia = g ? Pe(g[1]) : 0, ra = g ? Pe(g[2]) : 0, $e = i + la * t, Oe = E + ia * t, Re = 0 + ra * t, da = Re - c, ze = (P) => [
            i + ($e - i) * P,
            E + (Oe - E) * P,
            c + da * P
          ...
          for (let P = 0; P <= Ce; P++) {
            const Ma = 0.05 + 0.9 * (P / Ce), [ha, xa, _a2] = ze(Ma), je = 2 * Math.PI * Xe * (P / Ce);
    ...
    async exportF2k(e) {
      ...
      }, []), r = 9.80665, u = e.Lz1, f = e.Bz1, M = e.Lv, h = e.Bv, x = e.Hv, z = e.Lz2, A = e.Bz2, y = e.tz, k = e.bc, I = e.ks, B = (f - A) / 2, ne = k / 2, te = f / 2, G = u + M + z / 2, se = A / 2 + B, re = u / 2, de = f / 2, ce = u + M + z / 2, T = B + A / 2, _ = (e.useDead ?? 1) >= 0.5 ? 1 : 0, w = (e.useLive ?? 1) >= 0.5 ? 1 : 0, Z = [
        {
          xC: re,
          yC: de,
          xCol: ne,
...
\u2022 Z2 ${z}\xD7${A} m \u2014 PD=${e.P2} + PL=${e.P2_L ?? 0} = ${H} tonf | Pu=${Y.toFixed(1)} tonf
...
Abrilo en SAFE 20.x: File \u2192 Import \u2192 SAFE Text File (.f2k)`), console.log(`[F2K Zapata+Viga] exportado: Z1=${u}\xD7${f}, Z2=${z}\xD7${A}, ks=${I} kN/m\xB3`);
      } catch (L) {
