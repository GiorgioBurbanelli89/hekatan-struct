...
function ft(e, t, c, o) {
  ...
  function l(v, M, p, i) {
    ...
    const Y = new N();
    Y.setAttribute("position", new $(new Float32Array(T), 3));
    const B = new Float32Array([0, -s, -h, 0, s, -h, 0, s, -b, 0, y, -b, 0, y, b, 0, s, b, 0, s, h, 0, -s, h, 0, -s, b, 0, -y, b, 0, -y, -b, 0, -s, -b, 0, -s, -h]), R = new N();
    return R.setAttribute("position", new $(B, 3)), { fill: Y, outline: R };
  }
  function u(v, M, p) {
    const i = v / 2, d = M / 2, m = i - p, s = d - p, h = [];
    function b(Y, B, R, X) {
      h.push(0, Y, B, 0, R, B, 0, R, X, 0, Y, B, 0, R, X, 0, Y, X);
    }
  ...
  function x(v, M, p) {
    ...
    const Y = new Float32Array([0, -i, -d, 0, i, -d, 0, i, -d, 0, i, d, 0, i, d, 0, -i, d, 0, -i, d, 0, -i, -d, 0, -m, -s, 0, m, -s, 0, m, -s, 0, m, s, 0, m, s, 0, -m, s, 0, -m, s, 0, -m, -s]), B = new N();
    return B.setAttribute("position", new $(Y, 3)), { concFill: h, steelFillGeom: f, outline: B };
  }
  function w(v, M, p) {
  ...
  function C(v, M, p, i) {
    ...
    for (const Y of b) m.push(...s[Y]);
    for (const Y of b) m.push(...h[Y]);
    const y = new N();
    y.setAttribute("position", new $(new Float32Array(m), 3));
    const T = [];
    for (const Y of [s, h]) for (let B = 0; B < Y.length; B++) {
      const R = (B + 1) % Y.length;
      T.push(...Y[B], ...Y[R]);
    }
    const f = new N();
  ...
  function z(v, M, p, i, d) {
    ...
    const B = new N();
    return B.setAttribute("position", new $(new Float32Array(Y), 3)), { fill: f, outline: B };
  }
  function F(v, M, p, i) {
    ...
    const Y = new N();
    return Y.setAttribute("position", new $(new Float32Array(f), 3)), { fill: T, outline: Y };
  }
  function L(v, M, p = 24) {
    const i = v / 2, d = i - M, m = [];
    for (let y = 0; y < p; y++) {
      const T = y / p * Math.PI * 2, f = (y + 1) / p * Math.PI * 2, Y = Math.cos(T), B = Math.sin(T), R = Math.cos(f), X = Math.sin(f);
      m.push(0, i * Y, i * B, 0, i * R, i * X, 0, d * R, d * X), m.push(0, i * Y, i * B, 0, d * R, d * X, 0, d * Y, d * B);
    }
    const s = new N();
  ...
  const g = new Q({ color: 52479, transparent: true, opacity: 0.35, side: J, depthWrite: false }), I = new te({ color: 52479 }), E = new Q({ color: 16750848, transparent: true, opacity: 0.4, side: J, depthWrite: false }), A = new te({ color: 16750848 });
  function V(v, M) {
  ...
  return S.derive(() => {
    ...
    p.forEach((s, h) => {
      ...
      if (!f) return;
      const Y = [(b[0] + y[0]) / 2, (b[1] + y[1]) / 2, (b[2] + y[2]) / 2], B = Se(b, y);
      if (f.type === "CFT") {
        const X = x(f.b, f.h, f.tw ?? f.b * 0.05), k = new K(X.concFill, g);
        k.position.set(...Y), k.rotation.setFromRotationMatrix(B), n.add(k);
        const _ = new K(X.steelFillGeom, E);
        _.position.set(...Y), _.rotation.setFromRotationMatrix(B), n.add(_);
        const W = new ne(X.outline, A);
        W.position.set(...Y), W.rotation.setFromRotationMatrix(B), n.add(W);
      } else {
        let X, k, _;
        switch (f.type) {
          case "rect":
          ...
          case "circ":
            ...
            break;
          case "I":
            X = l(f.b, f.h, f.tf, f.tw), k = E, _ = A;
            break;
          case "HSS":
            X = u(f.b, f.h, f.tw ?? f.b * 0.05), k = E, _ = A;
            break;
          case "CFT":
            X = x(f.b, f.h, f.tw ?? f.b * 0.05), k = E, _ = A;
            break;
          case "L":
            X = w(f.b ?? f.h, f.h, f.t ?? f.tw ?? 3e-3), k = E, _ = A;
            break;
          case "2L":
            X = C(f.b ?? f.h, f.h, f.t ?? f.tw ?? 3e-3, f.dis ?? 0.01), k = E, _ = A;
            break;
          case "C":
          case "coldC":
            X = P(f.b, f.h, f.tf ?? f.t ?? 3e-3, f.tw ?? f.t ?? 3e-3), k = E, _ = A;
            break;
          case "2C":
            X = z(f.b, f.h, f.tf ?? 5e-3, f.tw ?? 5e-3, f.dis ?? 0.01), k = E, _ = A;
            break;
          case "T":
            X = F(f.b, f.h, f.tf ?? 0.01, f.tw ?? 6e-3), k = E, _ = A;
            break;
          case "pipe":
            X = L(f.d, f.tw ?? f.d * 0.05), k = E, _ = A;
            break;
          default:
            return;
        }
...
class Te extends q {
  constructor(t, c, o, n, a, r, l) {
    ...
    if (this.text = new H(`${a[0].toFixed(4)}`), this.text2 = new H(`${(a[1] * -1).toFixed(4)}`), this.normalizedResult = r, this.textPosition = Fe(t, c), this.text2Position = Fe(c, t), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(n), this.text2.rotation.setFromRotationMatrix(n), this.add(this.text, this.text2), x) {
      ...
      const I = new he(w), E = new he(C), A = new Q({ color: r[0] > 0 ? 24435 : 11411474, side: J }), V = new Q({ color: -r[1] > 0 ? 24435 : 11411474, side: J });
      this.mesh = new K(I, A), this.mesh2 = new K(E, V), this.mesh.position.set(...t), this.mesh2.position.set(...t), this.mesh.rotation.setFromRotationMatrix(n), this.mesh2.rotation.setFromRotationMatrix(n), l && this.mesh.rotateX(Math.PI / 2), l && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
...
function bt(e, t, c, o) {
  ...
  return S.derive(() => {
    ...
    n.children.forEach((l) => l.dispose()), n.clear();
    const a = Ae[t.nodeResults.rawVal], r = 0.05 * t.gridSize.val;
...
function Mt({ drawingObj: e, gridObj: t, scene: c, camera: o, controls: n, gridSize: a, derivedDisplayScale: r, rendererElm: l, viewerRender: u }) {
  ...
  c.add(F), P.geometry.setAttribute("position", new U(e.points.rawVal.flat(), 3)), P.geometry.computeBoundingSphere(), P.frustumCulled = false, z.frustumCulled = false, c.add(z), C.position.set(0.5 * a, 0.5 * a, 0), C.rotateX(Math.PI / 2), C.geometry.rotateX(Math.PI / 2), C.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), S.derive(() => {
    ...
    const A = 0.05 * a * 0.5 * r.val;
    z.material.size = A, x.params.Points.threshold = 0.4 * A;
  }), S.derive(() => {
    var _a;
    const A = e.points.val ?? [], v = (((_a = e.polylines) == null ? void 0 : _a.val) ?? []).at(-1) ?? [], M = [];
    for (const i of v) {
      const [d, m, s] = A[i];
      M.push(d, m, s);
    }
  ...
  l.addEventListener("pointerdown", () => {
    ...
    g = 0, w.x = A.clientX / window.innerWidth * 2 - 1, w.y = -(A.clientY / window.innerHeight) * 2 + 1, x.setFromCamera(w, o);
    const V = x.intersectObject(C);
    if (V.length) {
      let v = V[0].point;
      (A.ctrlKey || A.metaKey) && (v = new Z(Math.round(V[0].point.x), Math.round(V[0].point.y), Math.round(V[0].point.z))), e.points.val = [...e.points.rawVal, v.toArray()], e.polylines && (e.polylines.val = [...e.polylines.rawVal.slice(0, -1), [...e.polylines.rawVal.length ? e.polylines.rawVal.pop() : [], e.points.rawVal.length - 1]]);
    }
  }), l.addEventListener("contextmenu", () => {
    !e.polylines || e.polylines.rawVal[e.polylines.rawVal.length - 1].length === 0 || (e.polylines.val = [...e.polylines.rawVal, []]);
  }), l.addEventListener("pointermove", (A) => {
    w.x = A.clientX / window.innerWidth * 2 - 1, w.y = -(A.clientY / window.innerHeight) * 2 + 1, x.setFromCamera(w, o);
    const V = x.intersectObject(C);
    if (z.geometry.deleteAttribute("position"), V.length) {
      let v = V[0].point;
      (A.ctrlKey || A.metaKey) && (v = new Z(Math.round(V[0].point.x), Math.round(V[0].point.y), Math.round(V[0].point.z))), z.geometry.setAttribute("position", new U(v.toArray(), 3));
    }
    u();
  }), l.addEventListener("pointermove", (A) => {
    var _a;
  ...
  l.addEventListener("pointermove", (A) => {
    ...
    if (E !== void 0) {
      let i = M[0].point;
      (A.ctrlKey || A.metaKey) && (i = new Z(Math.round(i.x), Math.round(i.y), Math.round(i.z))), p[E] = i.toArray();
    }
    e.points.val = p;
  }), l.addEventListener("pointerup", () => {
    n.enabled = true, I = false;
  }), l.addEventListener("contextmenu", (A) => {
    var _a;
    w.x = A.clientX / window.innerWidth * 2 - 1, w.y = -(A.clientY / window.innerHeight) * 2 + 1, x.setFromCamera(w, o);
    let V = false;
    const v = x.intersectObject(P), M = x.intersectObject(C);
...
const ye = { rainbow: [[0, 255], [0.2, 65535], [0.5, 65280], [0.8, 16776960], [1, 16711680]], cooltowarm: [[0, 3952322], [0.2, 10206463], [0.5, 14474460], [0.8, 16163717], [1, 11797542]], blackbody: [[0, 0], [0.2, 7864320], [0.5, 15086080], [0.8, 16776960], [1, 16777215]], grayscale: [[0, 0], [0.2, 4210752], [0.5, 8355712], [0.8, 12566463], [1, 16777215]] }, gt = [[0, 16711935], [0.077, 16711860], [0.154, 16711680], [0.231, 16732160], [0.308, 16747520], [0.385, 16760320], [0.462, 16776960], [0.538, 11861760], [0.615, 65280], [0.692, 65460], [0.769, 65535], [0.846, 46335], [0.923, 255], [1, 180]];
function Ft(e, t, c) {
  const o = new ke(), n = new D(), a = new K(new N(), new Q({ side: J, vertexColors: true }));
  return o.addColorMap("sap2000", gt), o.setColorMap("sap2000"), a.renderOrder = -1, a.frustumCulled = false, S.derive(() => {
...
function Ct(e, t, c, o) {
  const n = Ft(c, e.elements, o);
  return S.derive(() => {
    n.visible = t.shellResults.val != "none";
  }), n;
}
const St = 6, ge = 10, At = 0.012;
function Yt(e) {
...
function Xt(e, t, c, o) {
  ...
  return S.derive(() => {
    ...
    const V = Math.max(E[0] - I[0], E[1] - I[1], E[2] - I[2], 1) * At, v = [], M = [], p = [];
    let i = 0;
    for (const { idx: h, vals: b } of F) {
      const y = x[h], T = u[y[0]], f = u[y[1]];
      if (!T || !f) continue;
      const Y = new Z(f[0] - T[0], f[1] - T[1], f[2] - T[2]), B = Y.length();
      if (B < 1e-10) continue;
      Y.normalize();
      const R = Math.abs(Y.y) < 0.99 ? new Z(0, 1, 0) : new Z(1, 0, 0), X = new Z().crossVectors(Y, R).normalize(), k = new Z().crossVectors(Y, X).normalize(), _ = ge + 1, W = St;
      for (let G = 0; G < _; G++) {
        const O = G / ge, se = T[0] + Y.x * B * O, ie = T[1] + Y.y * B * O, me = T[2] + Y.z * B * O, fe = b[0] + (b[1] - b[0]) * O, ae = a.getColor(fe) ?? new D(0, 0, 0);
...
function Ie(e, t = 8) {
  ...
  return c.querySelectorAll("p").forEach((u) => l.push(u)), setTimeout(() => {
    S.derive(() => {
      n.forEach((u, x) => {
        const w = l[x];
        w && (w.innerText = zt(e.val, u).toString());
      });
    });
  }), c;
}
...
function Nt({ mesh: e, settingsObj: t, drawingObj: c, objects3D: o, solids: n }) {
  ...
  const g = nt(t), I = S.derive(() => g.displayScale.val === 0 ? 1 : g.displayScale.val > 0 ? g.displayScale.val : -1 / g.displayScale.val), E = Pt(e, g);
  let A = Pe(g.gridSize.rawVal);
  ...
  }, C.update(), r.add(A, ut(g.gridSize.rawVal, g.flipAxes.rawVal)), new ResizeObserver((s) => {
    var _a, _b;
    for (const h of s) {
  ...
  if (e) {
    ...
    const s = Rt(e, g), h = Ct(e, g, E, s), b = Ie(s);
    r.add(h), a.appendChild(b);
    const y = Xt(e, g, E);
    r.add(y);
    const T = y.__colorMapValues, f = Ie(T);
    f.id = "frame-legend", a.appendChild(f), S.derive(() => {
      const Y = g.shellResults.val != "none", B = g.frameResults.val.startsWith("contour:");
      b.hidden = !Y, h.visible = Y, f.hidden = !B;
    });
  }
  if (n) {
  ...
  c && Mt({ drawingObj: c, gridObj: A, scene: r, camera: l, controls: C, gridSize: v, derivedDisplayScale: I, rendererElm: w.domElement, viewerRender: i }), pe((s, h) => {
    w.setClearColor(h.background, 1), r.remove(A), A.geometry.dispose(), A.material.dispose(), A = Pe(g.gridSize.rawVal), r.add(A), a.style.setProperty("--awatif-legend-color", h.legendMarker), i();
  });
...
const Ye = S.state(null), Ce = S.state(""), Tt = S.state("kN"), It = S.state("mm"), Bt = S.state("kN/m\xB2"), Et = { kN: 1, tonf: 9.80665, kip: 4.4482216 }, Be = { mm: 1e3, cm: 100, m: 1, in: 39.3700787402 }, Zt = { "kN/m\xB2": 1, kPa: 1, MPa: 1 / 1e3, GPa: 1 / 1e6, "kgf/cm\xB2": 1 / 98.0665, "tonf/m\xB2": 1 / 9.80665, psi: 1 / 6.89476, ksi: 1 / 6894.76 };
function Rt(e, t) {
  const c = S.state([]);
  let o;
  return ((n) => {
    ...
    const E = { bendingXX: [n, 0], bendingYY: [a, 0], bendingXY: [r, 0], membraneXX: [l, 0], membraneYY: [u, 0], membraneXY: [x, 0], tranverseShearX: [w, 0], tranverseShearY: [C, 0], vonMises: [P, 0], pressure: [z, 0], displacementX: [(_x = (_w = e.deformOutputs) == null ? void 0 : _w.val) == null ? void 0 : _x.deformations, 0], displacementY: [(_z = (_y = e.deformOutputs) == null ? void 0 : _y.val) == null ? void 0 : _z.deformations, 1], displacementZ: [(_B = (_A = e.deformOutputs) == null ? void 0 : _A.val) == null ? void 0 : _B.deformations, 2] }, A = t.shellResults.val, V = Tt.val, v = It.val, M = A === "displacementX" || A === "displacementY" || A === "displacementZ", p = A === "bendingXX" || A === "bendingYY" || A === "bendingXY", i = A === "membraneXX" || A === "membraneYY" || A === "membraneXY", d = A === "vonMises" || A === "pressure", m = A === "tranverseShearX" || A === "tranverseShearY", s = (_C = t.solidResults) == null ? void 0 : _C.val, h = s === "vonMises" || s === "sigmaXX" || s === "sigmaYY" || s === "sigmaZZ" || s === "tauXY" || s === "tauYZ" || s === "tauXZ", b = s === "ux" || s === "uy" || s === "uz", y = Bt.val, T = h ? Zt[y] : b || M ? Be[v] : p || i || d || m ? 1 / Et[V] : 1, f = h ? y : b || M ? v : p ? `${V}\xB7m/m` : i ? `${V}/m\xB2` : d ? `${V}/m\xB2` : m ? `${V}/m` : "";
    Ce.val = f;
    const Y = [];
