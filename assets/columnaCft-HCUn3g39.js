import { d as O, __tla as __tla_0 } from "./didacticCpp-pa3uOomD.js";
import { d as G, a as J, s as V, c as Z } from "./fiberSectionCft-DmBufNsV.js";
let Q;
let __tla = Promise.all([
  ...
  function H(e) {
    const f = e.shape ?? "circular", n = e.t, P = e.L, o = e.fc, s = e.Fy, c = e.Es ?? 2e8, a = e.rhoSR ?? 0, r = e.K ?? 1;
    let l, d, u, F, M, _, $;
    if (f === "circular") {
      const i = e.D, m = i - 2 * n;
      u = Math.PI * i * i / 4;
      const b = Math.PI * m * m / 4;
      l = u - b, d = b, F = Math.PI / 64 * (Math.pow(i, 4) - Math.pow(m, 4)), M = Math.PI / 64 * Math.pow(m, 4), _ = i / n, $ = 0.95;
    } else {
      const i = e.D, m = e.H ?? i;
      u = i * m;
      const b = i - 2 * n, p = m - 2 * n, K = b * p;
      l = u - K, d = K, F = (i * Math.pow(m, 3) - b * Math.pow(p, 3)) / 12, M = b * Math.pow(p, 3) / 12, _ = Math.max(i, m) / n, $ = 0.85;
    }
    const C = j(o), I = a * u, g = Math.min(0.7, 0.25 + 3 * (l + I) / u), E = 0.5, h = s * l + $ * o * (d + I * c / C), A = c * F + E * c * 0 + g * C * M, L = c * l + c * I + C * d;
    let D, x, t;
    f === "circular" ? (D = 0.15 * c / s, x = 0.19 * c / s) : (D = 2.26 * Math.sqrt(c / s), x = 3 * Math.sqrt(c / s)), _ <= D ? t = "compact" : _ <= x ? t = "noncompact" : t = "slender";
    const S = r * P, N = Math.PI * Math.PI * A / (S * S), v = h / N;
    let w;
    ...
    if (f === "circular") {
      const i = e.D, m = i - 2 * n;
      y = (Math.pow(i, 3) - Math.pow(m, 3)) / 6;
    } else {
      const i = e.D, m = e.H ?? i, b = i - 2 * n, p = m - 2 * n;
      y = (i * m * m - b * p * p) / 4;
    }
    const z = s * y + 0.85 * o * M / (e.D / 2);
    let k;
    if (f === "circular") k = 2 * n * (e.D - n);
    else {
      const i = e.H ?? e.D;
      k = 2 * n * (i - n);
    }
  ...
  function T(e, f, n) {
    const P = H(e), o = 0.75, s = 0.9, c = o * P.Pn, a = s * P.Mno, r = f / c;
    let l, d;
    return r >= 0.2 ? (l = r + 8 / 9 * (n / a), d = "Eq. H1-1a (P/Pc \u2265 0.2)") : (l = r / 2 + n / a, d = "Eq. H1-1b (P/Pc < 0.2)"), {
      ratio: l,
      passes: l <= 1,
      Pc: c,
      Mc: a,
  ...
  Q = {
    ...
    build(e, f) {
      ...
      const u = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), M = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map(), $ = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), h = 2e8, R = r.EA_eff / h, A = r.EI_eff / h, L = 2 * A, D = h / (2 * (1 + 0.3));
      for (let t = 0; t < s.length; t++) u.set(t, R), F.set(t, A), M.set(t, A), _.set(t, L), $.set(t, h), C.set(t, D), I.set(t, 24), g.set(t, e.t_tube), E.set(t, 0.2);
      f.elementInputs.val = {
        areas: u,
        momentsOfInertiaZ: M,
        momentsOfInertiaY: F,
  ...
  Demand/Capacity ratio = ${x.ratio.toFixed(3)} ${x.passes ? "\u2713 OK" : "\u2717 FAIL"} (${x.governing})`);
    ...
    computedLabels(e, f) {
      const n = e.shape < 0.5 ? "circular" : "rectangular", P = {
        shape: n,
        D: e.Dout,
        H: e.Hrect,
        t: e.t_tube,
