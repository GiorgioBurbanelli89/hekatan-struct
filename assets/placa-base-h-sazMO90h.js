...
import { v as n, i as ht, L as xt, e as Wt, d as ft, E as Zt, h as Qt, V as Ut, a as te, B as ee, g as ke, D as Ee } from "./theme-dnnHIQ7U.js";
import { a as Te } from "./analyze-DZ5QYzyM.js";
import { d as je, __tla as __tla_0 } from "./didacticCpp-C2di29sC.js";
import { g as Ge } from "./getViewer-f7b0_qaE.js";
import { g as He } from "./getParameters-BanUaban.js";
import { g as Se } from "./styles-CClJeH1b.js";
...
Promise.all([
  ...
  n.derive(() => {
    const s = r.B.value.val, v = r.H.value.val, m = r.t_plate.value.val, P = r.d_col.value.val, B = r.bf_col.value.val, E = r.tf_col.value.val, ue = r.tw_col.value.val, D = r.L_col.value.val, p = r.d_bolt.value.val, X = r.L_bolt.value.val, A = r.L_proj.value.val, _t = r.B_ped.value.val, gt = r.H_ped.value.val, V = r.h_ped.value.val, yt = r.fc.value.val, J = r.sx.value.val, wt = r.sy.value.val, Pt = Math.max(2, Math.round(r.nBoltsX.value.val)), R = Math.max(2, Math.round(r.nBoltsY.value.val)), F = r.Pu.value.val, K = r.Mu.value.val, T = Math.round(r.nx.value.val), j = Math.round(r.ny.value.val), b = [], u = [], W = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), tt = /* @__PURE__ */ new Map(), et = /* @__PURE__ */ new Map(), ot = /* @__PURE__ */ new Map(), st = /* @__PURE__ */ new Map(), at = /* @__PURE__ */ new Map();
    function _(t, e, a) {
      return b.push([
        t,
        e,
    ...
    for (let t = 0; t < j; t++) for (let e = 0; e < T; e++) G(g[t][e], g[t][e + 1], g[t + 1][e + 1], g[t + 1][e], m);
    function H(t, e) {
      let a = -1, o = 1 / 0;
      for (let l = 0; l <= j; l++) for (let c = 0; c <= T; c++) {
        const z = g[l][c], [C, ut, O] = b[z], L = Math.hypot(C - t, ut - e);
        L < o && (o = L, a = z);
      }
      return a;
    }
    ...
    for (let t = 0; t < i; t++) for (let e = 0; e < I; e++) G(y[t][e], y[t][e + 1], y[t + 1][e + 1], y[t + 1][e], ue);
    ...
    for (const [t, e] of ct) {
      ...
      const z = new Wt(Ct, Ct, rt, 6), C = new ft(z, ge);
      C.position.set(t, e, A - rt / 2), C.rotation.x = Math.PI / 2, f.push(C);
      const ut = new Zt(z), O = new Qt(ut, ye);
    ...
    const kt = new ke(_t, gt, V), we = new ht({
      color: 9079434,
      transparent: true,
      opacity: 0.35,
      side: Ee
    ...
    for (let t = 1; t < I; t++) N.push(y[i][t]);
    ...
    const Fe = 0.65, $ = s * v, qt = _t * gt, Ot = Math.min(2, Math.sqrt(qt / $)), Ie = 0.85 * yt * $ * Ot, Ne = Math.min(Ie, 1.7 * yt * $), Yt = Fe * Ne, $e = F / Math.max(1, Yt), Dt = Math.max(0, (s - 0.95 * P) / 2), Xt = F / $, Vt = Dt * Math.sqrt(2 * Math.max(0, Xt) / (0.9 * qe)), ze = Vt / Math.max(1e-6, m), Ce = Math.max(0.05, s - 2 * J), Jt = Math.max(0, K / Ce - F / 2) / Math.max(1, R), Rt = 0.75 * (0.75 * Math.PI / 4 * p * p) * Oe, Le = Jt / Math.max(1, Rt);
    pe.val = {
      vmMax: mt,
      A1: $,
      A2: qt,
