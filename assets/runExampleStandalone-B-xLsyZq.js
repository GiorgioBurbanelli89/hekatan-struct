...
import { f as nt, d as it, e as It, g as Ft, h as St, i as st, j as Dt, s as lt, k as ct, l as dt, m as pt, n as Nt, o as Ct, p as At, q as Bt, t as Ot, r as jt } from "./units-Be4w7gIT.js";
let Kt;
let __tla = (async () => {
      ...
      if (l == null ? void 0 : l.addFolder) {
        ...
        y.on("click", async () => {
          ...
          try {
            ...
            n("\u23F3 calculando\u2026");
            const t = new Worker(new URL("/hekatan-struct-lineal/assets/modal.worker-BIevPaE3.js", import.meta.url), {
              type: "module"
            }), i = (D) => {
              const N = {};
              ...
              if (!c) {
                ...
                try {
                  const { modalAnalysis: N } = await ot(async () => {
                    const { modalAnalysis: F } = await import("./index-CSWEE5gX.js").then(async (m2) => {
                      await m2.__tla;
                      return m2;
                    });
                    return {
        ...
        p.addBinding(h, "limite", {
          ...
          for (let r = 0; r < x.length; r++) {
            ...
            for (let T = 0; T < 2; T++) {
              let F = i[T];
              for (; ; ) {
                const V = (z.get(F) || []).filter((J) => !d.has(J)).find((J) => {
                  const G = y(x[J]);
            ...
            for (const T of t) for (const F of x[T]) {
              const V = k[F], H = m < 1e-9 ? 0 : ((V[0] - c[0]) * (o[0] - c[0]) + (V[1] - c[1]) * (o[1] - c[1]) + (V[2] - c[2]) * (o[2] - c[2])) / (m * m), J = L + (I - L) * H, G = Math.abs((b.get(F) || [
                0,
                0,
                0
              ])[2] - J) * 1e3;
