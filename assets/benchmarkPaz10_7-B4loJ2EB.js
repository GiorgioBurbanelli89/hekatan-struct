import { a as W } from "./analyze-DQyLT4zB.js";
import { m as X, d as j, __tla as __tla_0 } from "./didacticCpp-C2di29sC.js";
import { P as i, d as J, e as Q, a as K, b as ee, f as U, c as te, __tla as __tla_1 } from "./pazFrameE2k-BCTNzm-z.js";
import { n as ae, g as Y } from "./newmarkBeta-CfFoMJcW.js";
let de;
  ...
  de = {
    ...
    onParamChange(a, s) {
      if (a === "exportE2k" && s.exportE2k > 0.5) {
        const { filename: n, content: o } = se(s);
        J(n, o), console.log(`[Paz 10.7] e2k exportado: ${n}`), s.exportE2k = 0;
      }
      a === "showTH" && s.showTH < 0.5 && Y().hide();
    },
    build(a, s) {
      const n = Math.max(2, Math.round(a.nElem)), o = a.L_in, m = o / n, e = i.in_to_m(m), _ = i.psi_to_kNm2(a.E_psi), c = i.in4_to_m4(a.I_in4), h = Math.pow(12 * c, 0.25), T = h * h, H = a.mbar * 386.088 * 175.13 / 9.80665 / T, I = [];
      for (let t = 0; t <= n; t++) I.push([
        t * e,
        0,
        0
      ]);
      const u = [];
      for (let t = 0; t < n; t++) u.push([
        t,
        t + 1
      ]);
      ...
      const f = /* @__PURE__ */ new Map(), z = Math.floor(n / 2), B = i.lb_to_kN(a.F_lb);
      f.set(z, [
        0,
        0,
        -B,
      ...
      s.objects3D.val = [];
      const d = 192 * a.E_psi * a.I_in4 / Math.pow(o, 3), M = 0.5 * a.mbar * o, v = Math.sqrt(d / M), w = 2 * Math.PI / v, G = 1 / w, Z = 2 * a.xi * Math.sqrt(d * M), y = (t) => t <= 0 ? [
        0
      ] : t <= a.t1_s ? [
        a.F_lb
      ] : t <= a.t2_s ? [
      ...
      let p = `[Paz 10.7] Fixed-fixed beam, ${n} elementos
`;
...
`, a.showTH > 0.5) {
    ...
    runModal(a, s, n) {
      if (s.nodes.val.length) try {
        const o = X(s.nodes.val, s.elements.val, s.nodeInputs.val, s.elementInputs.val, 4);
        console.log(`[Paz 10.7 \u2014 Modal] frecuencias:
` + o.frequencies.slice(0, 4).map((m, e) => `  Modo ${e + 1}: f = ${m.toFixed(3)} Hz   T = ${(1 / m).toFixed(4)} s`).join(`
`)), (n == null ? void 0 : n.render) && n.render(o, {
        ...
        console.error("[Paz 10.7 Modal]", o.message);
      }
    }
  };
  function se(a) {
    const s = i.in_to_m(a.L_in), n = i.psi_to_kNm2(a.E_psi) / 9.80665, o = i.in4_to_m4(a.I_in4), m = Math.pow(12 * o, 0.25), e = [];
    e.push(...Q({
      units: "TONF M",
      title1: "Paz 10.7 Fixed-Fixed Beam",
      title2: "Validation"
    })), e.push("$ STORIES - IN SEQUENCE FROM TOP"), e.push('  STORY "Story1"  HEIGHT 0.001 MASTERSTORY "Yes"  '), e.push('  STORY "Base"  ELEV -0.001 '), e.push(""), e.push("$ GRIDS"), e.push('  GRIDSYSTEM "G1"  TYPE "CARTESIAN"  BUBBLESIZE 1.25 '), e.push(""), e.push("$ DIAPHRAGM NAMES"), e.push('  DIAPHRAGM "D1"    TYPE RIGID'), e.push(""), e.push("$ MATERIAL PROPERTIES"), e.push(...K("A992Fy50", n, 7.849)), e.push(""), e.push("$ FRAME SECTIONS"), e.push(...ee({
      name: "BEAM_PAZ10_7",
      material: "A992Fy50",
      shape: "Steel I/Wide Flange",
      D: m,
