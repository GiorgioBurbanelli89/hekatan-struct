...
import { v as n } from "./theme-dnnHIQ7U.js";
import { a as H } from "./analyze-DZ5QYzyM.js";
import { d as k, __tla as __tla_0 } from "./didacticCpp-C2di29sC.js";
import { g as A } from "./getViewer-f7b0_qaE.js";
import { g as B } from "./getParameters-BanUaban.js";
import { g as G } from "./styles-CClJeH1b.js";
...
Promise.all([
  ...
  n.derive(() => {
    const l = Math.round(p.nFloors.value.val), F = p.H_floor.value.val, s = Math.round(p.nCols.value.val), v = p.baseR.value.val, S = p.windFactor.value.val, i = [], o = [];
    for (let t = 0; t <= l; t++) {
      const e = t / l, m = t * F;
      let r = v * (0.6 + 0.4 * Math.sin(Math.PI * e));
      e > 0.9 && (r = v * (0.6 + 0.4 * Math.sin(Math.PI * 0.9)) * (1 - (e - 0.9) * 8), r = Math.max(r, 1));
      for (let a = 0; a < s; a++) {
