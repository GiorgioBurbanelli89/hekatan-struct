...
import { c as C } from "./chartPanel-DKe9UCQ8.js";
import { I as E, T as N, c as v } from "./espectroNec-CMQq_yyp.js";
...
function a() {
  ...
  e.pisos.forEach((i) => l.push([i.Fx, i.hi]));
  const c = [];
  e.pisos.forEach((i) => c.push([i.Vi, i.hi])), c.push([0, e.pisos[e.pisos.length - 1].hi + t.he]), g.setSeries([{ label: "Fx (kN)", data: l, color: "#f0883e", type: "scatter", width: 6 }, { label: "Vi cortante (kN)", data: c, color: "#58a6ff", width: 2 }]), g.setAxes({ xLabel: "Fuerza / Cortante (kN)", yLabel: "Altura (m)", xMin: 0, yMin: 0, grid: true });
  const f = t.norma === "NEC15" ? "NEC-15 (2014) \xB7 vigente" : "Borrador 2023", u = e.pisos.slice().reverse().map((i) => `
    <tr>
      <td style="text-align:center">${i.piso}</td>
      <td style="text-align:right">${o(i.hi, 1)}</td>
      <td style="text-align:right">${o(i.Cvx, 3)}</td>
      <td style="text-align:right;color:#f0883e">${o(i.Fx, 1)}</td>
      <td style="text-align:right;color:#58a6ff">${o(i.Vi, 1)}</td>
    </tr>`).join("");
  h.innerHTML = `
   ...
   <ul style="margin:4px 0 0 0;padding-left:15px;font-size:10.5px;color:#8b949e">
     ${[e.refs.V, e.refs.W, e.refs.dist, e.refs.k].map((i) => `<li>${i}</li>`).join("")}
   </ul>`;
}
const n = new y({ title: "\u2699\uFE0F Cortante basal NEC-SE-DS", container: b });
n.addBinding(t, "norma", { label: "Norma", options: { "NEC-15 (2014)": "NEC15", "Borrador 2023": "BORRADOR2023" } }).on("change", a);
n.addBinding(t, "region", { label: "Regi\xF3n", options: { Costa: "Costa", Sierra: "Sierra", Oriente: "Oriente", Esmeraldas: "Esmeraldas", Gal\u00E1pagos: "Galapagos" } }).on("change", a);
n.addBinding(t, "suelo", { label: "Suelo", options: { A: "A", B: "B", C: "C", D: "D", E: "E" } }).on("change", a);
n.addBinding(t, "Z", { label: "Factor Z (g)", min: 0.1, max: 0.6, step: 0.01 }).on("change", a);
const r = n.addFolder({ title: "Reducci\xF3n", expanded: true });
r.addBinding(t, "R", { label: "R", min: 1, max: 8, step: 0.5 }).on("change", a);
r.addBinding(t, "I", { label: "Importancia I", options: E }).on("change", a);
r.addBinding(t, "phiP", { label: "\u03A6P", min: 0.9, max: 1, step: 0.05 }).on("change", a);
r.addBinding(t, "phiE", { label: "\u03A6E", min: 0.9, max: 1, step: 0.05 }).on("change", a);
const p = n.addFolder({ title: "Edificio", expanded: true });
p.addBinding(t, "N", { label: "N\xB0 pisos", min: 1, max: 40, step: 1 }).on("change", a);
p.addBinding(t, "he", { label: "Altura entrepiso (m)", min: 2.4, max: 5, step: 0.1 }).on("change", a);
p.addBinding(t, "wPiso", { label: "Peso/piso W (kN)", min: 100, max: 1e4, step: 50 }).on("change", a);
p.addBinding(t, "tipoTa", { label: "Tipo (Ta)", options: Object.fromEntries(Object.keys(N).map((e) => [e, e])) }).on("change", a);
a();
