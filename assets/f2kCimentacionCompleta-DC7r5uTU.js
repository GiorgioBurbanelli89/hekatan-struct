...
function _(c) {
  ...
  for (let o = 0; o < p; o++) {
    const i = h[o];
    e.push(`   Name=Footing${o + 1}   "Modeling Type"=Shell-Thin   "Property Type"=Footing   Material=4000Psi   "Slab Thickness"=${t(i.tz)}   "Notional Size Type"=Auto   "Notional Auto Factor"=1   "f11 Modifier"=1   "f22 Modifier"=1   "f12 Modifier"=1   "m11 Modifier"=1   "m22 Modifier"=1   "m12 Modifier"=1   "v13 Modifier"=1   "v23 Modifier"=1   "Mass Modifier"=1   "Weight Modifier"=1   Color=Blue   GUID=${u()}   Orthotropic?=No`), e.push(`   Name=Stiff${o + 1}     "Modeling Type"=Shell-Thin   "Property Type"=Stiff     Material=4000Psi   "Slab Thickness"=${t(i.tz)}   "Notional Size Type"=Auto   "Notional Auto Factor"=1   "f11 Modifier"=1   "f22 Modifier"=1   "f12 Modifier"=1   "m11 Modifier"=100   "m22 Modifier"=100   "m12 Modifier"=100   "v13 Modifier"=1   "v23 Modifier"=1   "Mass Modifier"=0   "Weight Modifier"=0   Color=Cyan   GUID=${u()}   Orthotropic?=No`);
  }
  e.push(" "), e.push('TABLE:  "SPRING PROPERTY DEFINITIONS - AREA SPRINGS"'), e.push(`   Name=ASpr1   "Subgrade Modulus"=${t(P)}   "Nonlinear Option"="Compression Only"   Color=Green   GUID=${u()}`), e.push(" "), e.push('TABLE:  "LOAD PATTERN DEFINITIONS"'), e.push(`   Name=Dead   "Is Auto Load"=No   Type=Dead   "Self Weight Multiplier"=1   GUID=${u()}`), e.push(`   Name=Live   "Is Auto Load"=No   Type=Live   "Self Weight Multiplier"=0   GUID=${u()}`), e.push(" "), e.push('TABLE:  "LOAD CASE DEFINITIONS - SUMMARY"'), e.push(`   Name=Dead   Type="Linear Static"   GUID=${u()}`), e.push(`   Name=Live   Type="Linear Static"   GUID=${u()}`), e.push(" "), e.push('TABLE:  "LOAD CASE DEFINITIONS - LINEAR STATIC"'), e.push(`   Name=Dead   "Exclude Group"=None   "Mass Source"=MsSrc1   "Initial Condition"=Unstressed   "Load Type"=Load   "Load Name"=Dead   "Load SF"=1   "Design Type"="Program Determined"   GUID=${u()}`), e.push(`   Name=Live   "Exclude Group"=None   "Mass Source"=MsSrc1   "Initial Condition"=Unstressed   "Load Type"=Load   "Load Name"=Live   "Load SF"=1   "Design Type"="Program Determined"   GUID=${u()}`), e.push(" "), e.push('TABLE:  "MASS SOURCE DEFINITION"'), e.push(`   Name=MsSrc1   "Is Default"=Yes   "Include Lateral Mass?"=No   "Include Vertical Mass?"=Yes   "Lump Mass?"=Yes   "Source Self Mass?"=Yes   "Source Added Mass?"=Yes   "Source Load Patterns?"=No   "Move Mass Centroid?"=No   GUID=${u()}`), e.push(" "), e.push('TABLE:  "COMBINATION DEFINITIONS"'), e.push(`   Name=Pu_1.4D+1.7L   Type="Linear Add"   "Is Auto"=No   "Load Name"=Dead   "Scale Factor"=1.4   GUID=${u()}`), e.push('   Name=Pu_1.4D+1.7L   "Load Name"=Live   "Scale Factor"=1.7'), e.push(" ");
  ...
  for (let o = 0; o < p; o++) {
    ...
    for (const S of A) e.push(`   UniqueName=${S.uid}   "Is Auto Point"=No   IsSpecial=${S.special ? "Yes" : "No"}   X=${t(S.x)}   Y=${t(S.y)}   Z=${t(d)}   GUID=${u()}`);
  }
  const f = [];
  let x = p * 9 + 1;
  const $ = (o, i, s) => {
    ...
    const n = x++;
    return e.push(`   UniqueName=${n}   "Is Auto Point"=No   IsSpecial=No   X=${t(o)}   Y=${t(i)}   Z=${t(s)}   GUID=${u()}`), n;
  };
  ...
  for (let o = 0; o < p; o++) {
    ...
    e.push(`   "Unique Name"=${o + 1}        UniquePt1=${s + 1}   UniquePt2=${s + 2}   UniquePt3=${s + 3}   UniquePt4=${s + 4}   Perimeter=${t(a)}   Area=${t(n)}   GUID=${u()}`), e.push(`   "Unique Name"=LOAD${o + 1}    UniquePt1=${s + 5}   UniquePt2=${s + 6}   UniquePt3=${s + 7}   UniquePt4=${s + 8}   Perimeter=${t(A)}   Area=${t(r)}   GUID=${u()}`);
  }
  ...
  for (let o = 0; o < p; o++) {
    ...
    (n !== 0 || a !== 0 || r !== 0) && e.push(`   UniqueName=${s + 9}   "Load Pattern"=Dead   FX=0   FY=0   FZ=${t(-n)}   MX=${t(a)}   MY=${t(r)}   MZ=0   "X Dimension"=${t(i.bc)}   "Y Dimension"=${t(i.bc)}   GUID=${u()}`);
    const A = (i.P_live_kN ?? 0) * l, S = (i.Mx_live_kNm ?? 0) * l, I = (i.My_live_kNm ?? 0) * l;
    (A !== 0 || S !== 0 || I !== 0) && e.push(`   UniqueName=${s + 9}   "Load Pattern"=Live   FX=0   FY=0   FZ=${t(-A)}   MX=${t(S)}   MY=${t(I)}   MZ=0   "X Dimension"=${t(i.bc)}   "Y Dimension"=${t(i.bc)}   GUID=${u()}`);
  }
...
function v(c, N = "cimentacion_edificio.f2k") {
  const M = _(c), d = new Blob([M], { type: "text/plain;charset=utf-8" }), h = URL.createObjectURL(d), p = document.createElement("a");
  p.href = h, p.download = N, document.body.appendChild(p), p.click(), document.body.removeChild(p), URL.revokeObjectURL(h);
}
export {
