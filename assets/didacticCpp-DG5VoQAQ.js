import { M as ws, __tla as __tla_0 } from "./deform-Uxr1_sxQ.js";
...
let __tla = Promise.all([
  ...
  et = function(t, r, e, s, a = 10, o = 0, Q = 0, $ = 1, N, G) {
    ...
    n.push(os);
    const X = s.drillingTypes, I = X ? Array.from(X.keys()) : [], C = X ? Array.from(X.values()) : [], b = ts(I, Uint32Array, y.HEAPU32);
    n.push(b);
    const es = ts(C, Uint32Array, y.HEAPU32);
    n.push(es);
    const J = s.drillingPenaltyScales, ss = J ? Array.from(J.keys()) : [], x = J ? Array.from(J.values()) : [], j = ts(ss, Uint32Array, y.HEAPU32);
    n.push(j);
  ...
  at = function(t, r, e, s, a = 10) {
    ...
    const os = f.HEAPU32[T / 4], X = f.HEAPU32[Y / 4], I = f.HEAPU32[L / 4], C = f.HEAPU32[B / 4], b = f.HEAPU32[D / 4], es = f.HEAPU32[W / 4], J = f.HEAPU32[rs / 4], ss = f.HEAPU32[M / 4];
    let x = [], j = [], ls = [];
    if (X > 0 && os) {
    ...
    if (C > 0 && b > 0 && I) {
      const F = new Float64Array(f.HEAPF64.buffer, I, C * b);
      for (let l = 0; l < C; l++) j.push(Array.from(F.slice(l * b, (l + 1) * b)));
      o.push(I);
    }
    if (J > 0 && ss > 0 && es) {
      const F = new Float64Array(f.HEAPF64.buffer, es, J * ss);
      for (let l = 0; l < J; l++) ls.push(Array.from(F.slice(l * ss, (l + 1) * ss)));
      o.push(es);
    }
  ...
  nt = function(t) {
    ...
    for (let l = 0; l < os; l++) {
      ...
      I.push(d), Math.abs(d.w) > Math.abs(C) && (C = d.w);
    }
    const b = new Float64Array(R.HEAPF64.buffer, W, rs), es = [];
    let J = 0, ss = 0, x = 0, j = 0, ls = 0;
    for (let l = 0; l < X; l++) {
      const _ = l * 9, d = {
        nodes: [
          b[_],
      ...
      es.push(d), Math.abs(d.Mxx) > Math.abs(J) && (J = d.Mxx), Math.abs(d.Myy) > Math.abs(ss) && (ss = d.Myy), Math.abs(d.Mxy) > Math.abs(x) && (x = d.Mxy), Math.abs(d.Qx) > Math.abs(j) && (j = d.Qx), Math.abs(d.Qy) > Math.abs(ls) && (ls = d.Qy);
    }
    let F;
    if (P > 0 && A > 0) {
      const l = P / 2, _ = A / 2;
  ...
  ct = function(t, r, e, s) {
    ...
    const M = p.HEAPU32[Y / 4], os = p.HEAPU32[L / 4], X = p.HEAPU32[B / 4], I = p.HEAPU32[D / 4], C = p.HEAPU32[W / 4], b = p.HEAPU32[rs / 4], es = M && os > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, M, os)) : [], J = X && I > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, X, I)) : [], ss = C && b > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, C, b)) : [];
    return M && a.push(M), X && a.push(X), C && a.push(C), a.forEach((x) => p._free(x)), tt(es, J, ss, t.length, r.length);
  };
