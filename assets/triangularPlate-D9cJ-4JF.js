import { __tla as __tla_0 } from "./didacticCpp-BeLq_Lcs.js";
  ...
  j = {
    ...
    build(e, r) {
      ...
      for (let n = 0; n < i; n++) for (let s = 0; s < o; s++) {
        const l = n * (o + 1) + s, M = l + 1, y = l + (o + 1), x = y + 1;
        m.push([
          l,
          M,
          x
      ...
      try {
        ...
        const L = /* @__PURE__ */ new Map(), $ = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map();
        ...
        w = Math.abs(n.nodeResults[R].w);
        const T = e.E * Math.pow(e.t, 3) / (12 * (1 - e.nu * e.nu)), k = Math.min(c, d), F = 126e-5 * Math.abs(e.q) * Math.pow(k, 4) / T;
        console.log(`[Triangular Plate MITC3] ${c}\xD7${d}m, t=${e.t}m, q=${e.q}kN/m\xB2 \u2192 w_center FEM = ${(w * 1e3).toFixed(3)} mm | te\xF3rico Timoshenko \xA742 (clamped a\xB2) = ${(F * 1e3).toFixed(3)} mm | ratio = ${(w / F).toFixed(3)}`);
      } catch (n) {
