import "./modulepreload-polyfill-B5Qt9EMX.js";
import { v as N, P as Ce } from "./theme-2eEBQPmF.js";
import { c as Ee, a as Be, g as Fe } from "./getViewer-DaIPsgnU.js";
import { g as ze } from "./styles-Cjdl64P4.js";
import { c as Te } from "./renderModalTable-H-0rlxOz.js";
import { z as Ae, __tla as __tla_0 } from "./zapataVigaAmarre-CyeuhrCg.js";
import { z as Ie, __tla as __tla_1 } from "./zapataAislada-CQ8saScZ.js";
import { z as qe, __tla as __tla_2 } from "./zapataAisladaValidacion-DH6R_dpi.js";
import { e as Le, __tla as __tla_3 } from "./edificioConLosa-DCsNggd6.js";
import { e as De, __tla as __tla_4 } from "./edificioConMuros-_sYWpyDv.js";
import { p as He, __tla as __tla_5 } from "./plane-Ds4tgtrN.js";
import { m as $e, __tla as __tla_6 } from "./membranaCSI-QfwdK1v1.js";
import { p as Oe, __tla as __tla_7 } from "./plateThin-QR4GrYBb.js";
import { p as Re, __tla as __tla_8 } from "./plateThick-fLoNWNRH.js";
import { m as je, __tla as __tla_9 } from "./membrana-BuR0U3Qy.js";
import { s as Ke, __tla as __tla_10 } from "./shellThin-COhxL8-p.js";
import { s as Ne, __tla as __tla_11 } from "./shellThick-iD5e9G2X.js";
import { e as Ve, __tla as __tla_12 } from "./edificioAporticado-O5n1vp5J.js";
import { e as Ue, __tla as __tla_13 } from "./edificioComparativaFem-BOdC8ee2.js";
import { e as Xe, __tla as __tla_14 } from "./edificioHormigon-B7s9FPp0.js";
import { e as Qe, __tla as __tla_15 } from "./edificioAceroV2-faIMvRe9.js";
import { e as Ye, __tla as __tla_16 } from "./edificioMixto-j0Q_K_VJ.js";
import { e as Ze, __tla as __tla_17 } from "./edificioMuros-MFxqjFch.js";
import { e as Ge, __tla as __tla_18 } from "./edificioDual-D9DVf_yi.js";
import { c as We, __tla as __tla_19 } from "./columnaCft-B6pvcVQ8.js";
import { t as Je, __tla as __tla_20 } from "./triangularPlate-Rbe5mPYL.js";
import { c as et, __tla as __tla_21 } from "./conexionRbs-rPKnChPa.js";
import { c as tt, __tla as __tla_22 } from "./conexionBfp-DKg-K603.js";
import { c as at, __tla as __tla_23 } from "./conexionEndPlate-CNsgQwej.js";
import { p as nt, __tla as __tla_24 } from "./placaBase-l-kzVdu2.js";
import { t as ot, __tla as __tla_25 } from "./trussGen-nJV5pXwJ.js";
import { b as it, __tla as __tla_26 } from "./barraAxial-B4Ty5S6V.js";
import { p as st, __tla as __tla_27 } from "./portico2D-BoGrb60e.js";
import { t as rt, __tla as __tla_28 } from "./tower3D-DsOWpEwZ.js";
import { g as lt, __tla as __tla_29 } from "./galpon-CTYJ0c7d.js";
import { e as ct, __tla as __tla_30 } from "./edifAcero-Dvy5ina8.js";
import { m as dt, __tla as __tla_31 } from "./mezanine-BVeFIxuS.js";
import { downloadZapataF2k as mt } from "./f2kExporter-COIU6xYe.js";
import { parseZapataF2k as ft } from "./f2kImporter-Cp9ARV1y.js";
import { f as ie, d as fe, a as he, b as be, s as ye, c as ut, m as pt, e as yt, t as gt, g as xt } from "./units-d0zQYQ9A.js";
import { a as ge } from "./exampleVersion-D1A_5i59.js";
import "./Text-QYN3x2IP.js";
import "./analyze-BydHtRcI.js";
import "./pureFunctionsAny.generated-DeJSBP3k.js";
import { __tla as __tla_32 } from "./didacticCpp-B5f-GyHC.js";
import { __tla as __tla_33 } from "./deform-CGyu4ATa.js";
import "./planeQ4-DsCzHfbV.js";
import "./fiberSectionCft-DmBufNsV.js";
import "./mitc3-2FJr2z_r.js";
import "./menegottoPinto-B-C2cxus.js";
import { __tla as __tla_34 } from "./secantPlasticity-F1hoh1EI.js";
import "./colorMapPercentile-OnF3uP-w.js";
Promise.all([
  (() => {
    try {
      return __tla_0;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_1;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_2;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_3;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_4;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_5;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_6;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_7;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_8;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_9;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_10;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_11;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_12;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_13;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_14;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_15;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_16;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_17;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_18;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_19;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_20;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_21;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_22;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_23;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_24;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_25;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_26;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_27;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_28;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_29;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_30;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_31;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_32;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_33;
    } catch {
    }
  })(),
  (() => {
    try {
      return __tla_34;
    } catch {
    }
  })()
]).then(async () => {
  function ve(a) {
    const { mesh: p, viewerElm: r, onStatusChange: i } = a, y = a.scalePercent ?? 5, [b, v] = a.visFrequencyRange ?? [
      0.5,
      3
    ];
    let s = null, u = 0, h = 0, M = [], w = [];
    function S() {
      i == null ? void 0 : i();
    }
    function z() {
      var _a;
      if (!s || !s.frequencies || s.frequencies.length === 0) return {
        mode: "Sin resultados",
        frequency: "\u2014",
        period: "\u2014",
        dominant: "\u2014",
        state: "\u23F8 Detenido"
      };
      const x = s.frequencies[u] ?? 0, E = x > 0 ? 1 / x : 0, n = [
        "Ux",
        "Uy",
        "Uz",
        "Rx",
        "Ry",
        "Rz"
      ], k = (_a = s.massParticipation) == null ? void 0 : _a[u];
      let _ = "\u2014";
      if (k) {
        let j = 0, H = 0;
        for (let D = 0; D < 6; D++) Math.abs(k[D]) > j && (j = Math.abs(k[D]), H = D);
        _ = `${n[H]} (${(j * 100).toFixed(0)}%)`;
      }
      return {
        mode: `Modo ${u + 1} / ${s.frequencies.length}`,
        frequency: `${x.toFixed(4)} Hz`,
        period: `${E.toFixed(4)} s`,
        dominant: _,
        state: h !== 0 ? "\u25B6 Reproduciendo" : "\u23F8 Pausado"
      };
    }
    function B() {
      return r.__ctx;
    }
    function q(x) {
      var _a;
      if (h && (cancelAnimationFrame(h), h = 0), x) {
        const E = M.length > 0 ? M : w;
        E.length > 0 && (p.nodes.val = E.map((n) => [
          ...n
        ]), (_a = B()) == null ? void 0 : _a.render());
      }
    }
    function I() {
      var _a, _b;
      if (!s || !s.modeShapes || s.modeShapes.length === 0 || !s.modeShapes[u]) return;
      q(false);
      const x = s.modeShapes[u], E = ((_a = s.frequencies) == null ? void 0 : _a[u]) || 1, n = ((_b = s.frequencies) == null ? void 0 : _b[0]) || 1, k = Math.max(b, Math.min(v, E / n));
      w = p.nodes.rawVal.map((g) => [
        ...g
      ]);
      const _ = w.length;
      let j = 1 / 0, H = 1 / 0, D = 1 / 0, J = -1 / 0, Y = -1 / 0, t = -1 / 0;
      for (const g of w) g[0] < j && (j = g[0]), g[0] > J && (J = g[0]), g[1] < H && (H = g[1]), g[1] > Y && (Y = g[1]), g[2] < D && (D = g[2]), g[2] > t && (t = g[2]);
      const e = Math.sqrt((J - j) ** 2 + (Y - H) ** 2 + (t - D) ** 2) || 1;
      let l = 0;
      for (let g = 0; g < _; g++) {
        const T = x[g * 6] || 0, A = x[g * 6 + 1] || 0, C = x[g * 6 + 2] || 0, $ = Math.sqrt(T * T + A * A + C * C);
        $ > l && (l = $);
      }
      const P = l > 1e-12 ? e * y / 100 / l : 1, m = performance.now(), o = () => {
        var _a2;
        const g = (performance.now() - m) / 1e3, T = Math.sin(2 * Math.PI * k * g) * P, A = new Array(_);
        for (let C = 0; C < _; C++) {
          const $ = w[C];
          A[C] = [
            $[0] + (x[C * 6] || 0) * T,
            $[1] + (x[C * 6 + 1] || 0) * T,
            $[2] + (x[C * 6 + 2] || 0) * T
          ];
        }
        p.nodes.val = A, (_a2 = B()) == null ? void 0 : _a2.render(), h = requestAnimationFrame(o);
      };
      h = requestAnimationFrame(o), S();
    }
    return {
      setResults(x) {
        var _a;
        s = x, u >= (((_a = x == null ? void 0 : x.frequencies) == null ? void 0 : _a.length) ?? 0) && (u = 0), M = p.nodes.rawVal.map((E) => [
          ...E
        ]), S();
      },
      setMode(x) {
        var _a;
        if (!s) return;
        const E = ((_a = s.frequencies) == null ? void 0 : _a.length) ?? 0;
        u = Math.max(0, Math.min(E - 1, x)), h !== 0 ? I() : S();
      },
      play() {
        s && h === 0 && I();
      },
      stop() {
        q(true), S();
      },
      isPlaying() {
        return h !== 0;
      },
      modeCount() {
        var _a;
        return ((_a = s == null ? void 0 : s.frequencies) == null ? void 0 : _a.length) ?? 0;
      },
      currentMode() {
        return u;
      },
      currentFreq() {
        var _a;
        return ((_a = s == null ? void 0 : s.frequencies) == null ? void 0 : _a[u]) ?? 0;
      },
      getStatus() {
        return z();
      },
      dispose() {
        q(true), s = null;
      }
    };
  }
  function c(a, p, r, i = false) {
    return {
      id: a,
      name: p,
      category: r,
      benchmark: i,
      standaloneUrl: `../${a}/`
    };
  }
  const ht = c("1d-mesh", "Hekatan \u2013 1D Mesh", "Legacy \xB7 FEM b\xE1sico"), bt = c("2d-mesh", "Hekatan \u2013 2D Mesh", "Legacy \xB7 FEM b\xE1sico"), vt = c("3d-structure", "Hekatan \u2013 3D Structure", "Legacy \xB7 FEM b\xE1sico"), Mt = c("axial-bar", "Hekatan \u2013 Axial Bar", "Legacy \xB7 Frames"), kt = c("truss", "Hekatan \u2013 Truss", "Legacy \xB7 Frames"), _t = c("advanced-truss", "Hekatan \u2013 Advanced Truss", "Legacy \xB7 Frames"), wt = c("beams", "Paz 6.3 Space Frame (validaci\xF3n 4 solvers)", "\u{1F3C1} Benchmarks", true), St = c("building", "Hekatan \u2013 Building (upstream)", "Legacy \xB7 Edificios"), Pt = c("plate", "Hekatan \u2013 Plate (legacy)", "Legacy \xB7 Placas"), Ct = c("plate-q4", "Hekatan \u2013 Plate Q4 Studio", "Legacy \xB7 Placas"), Et = c("color-map", "Hekatan \u2013 Color Map demo", "Legacy \xB7 Visualizaci\xF3n"), Bt = c("curves", "Hekatan \u2013 Curves demo", "Legacy \xB7 Visualizaci\xF3n"), Ft = c("drawing", "Hekatan \u2013 Drawing canvas", "Legacy \xB7 Visualizaci\xF3n"), zt = c("tables", "Hekatan \u2013 Tables demo", "Legacy \xB7 Visualizaci\xF3n"), Tt = c("cad-editor", "Hekatan \u2013 CAD Editor", "Legacy \xB7 Editores"), At = c("calc-editor", "Hekatan \u2013 Calc Editor", "Legacy \xB7 Editores"), It = c("slab-designer", "Hekatan \u2013 Slab Designer", "Legacy \xB7 Editores"), qt = c("fem-explained", "Hekatan \u2013 FEM Explained", "Legacy \xB7 Educativo"), Lt = c("report", "Hekatan \u2013 Report (Calcpad)", "Legacy \xB7 Educativo"), Dt = c("gateway-arch", "Gateway Arch", "Estructuras emblem\xE1ticas"), Ht = c("cable-stayed-bridge", "Puente Atirantado", "Estructuras emblem\xE1ticas"), $t = c("twisted-tower", "Torre Retorcida", "Estructuras emblem\xE1ticas"), Ot = c("burj-khalifa", "Burj Khalifa style", "Estructuras emblem\xE1ticas"), Rt = c("sydney-opera", "Sydney Opera House", "Estructuras emblem\xE1ticas"), jt = c("diagrid", "Diagrid (Gherkin) style", "Estructuras emblem\xE1ticas"), Kt = c("pergola", "P\xE9rgola de acero", "Estructuras emblem\xE1ticas"), Nt = c("shear-wall-q4", "Muro de Corte Q4", "Demos FEM Q4"), Vt = c("cantilever-beam-q4", "Viga Cantilever Q4", "Demos FEM Q4"), Ut = c("placa-cantilever-q4", "Placa Cantilever XY Q4", "Demos FEM Q4"), Xt = c("slope-stability", "Estabilidad de Talud (SRM)", "Geot\xE9cnico"), Qt = c("placa-base-h", "Placa Base + Columna H (CBFEM)", "Conexiones"), Yt = c("bolt-hole-detail", "Detalle Perno + Orificio (Kirsch)", "Conexiones"), Zt = c("conexion-diafragma-cft", "Conexi\xF3n Viga-Columna CFT con Diafragma (Cervantes)", "Conexiones"), Gt = c("placa-base-hueca", "Placa Base + Columna HSS Hueca (acero)", "Conexiones"), Wt = c("placa-base-cft", "Placa Base + Columna CFT (rellena de concreto)", "Conexiones"), Jt = c("columna-cft-h8", "Columna CFT con s\xF3lidos H8", "Columnas FEM 3D"), ea = c("viga-doble-t", "Viga Doble-T (perfil W)", "Vigas / Perfiles"), ta = c("tablero-puente", "Tablero Puente (3 vigas+losa, test Solar)", "Puentes"), aa = c("solid-cube-fem", "Cubo S\xF3lido H8 (validaci\xF3n CalculiX)", "\u{1F3C1} Benchmarks", true), na = c("bulbo-presiones-suelo", "Bulbo de Presiones \u2014 Serquen SF-70", "\u{1F3C1} Benchmarks", true), oa = [
    ht,
    bt,
    vt,
    Mt,
    kt,
    _t,
    wt,
    St,
    Pt,
    Ct,
    Et,
    Bt,
    Ft,
    zt,
    Tt,
    At,
    It,
    qt,
    Lt,
    Dt,
    Ht,
    $t,
    Ot,
    Rt,
    jt,
    Kt,
    Nt,
    Vt,
    Ut,
    Xt,
    Qt,
    Yt,
    Zt,
    Gt,
    Wt,
    Jt,
    ea,
    ta,
    aa,
    na
  ], U = [
    qe,
    Ie,
    Ae,
    it,
    ot,
    st,
    rt,
    lt,
    We,
    et,
    tt,
    at,
    nt,
    Ve,
    Ue,
    Xe,
    Qe,
    Ye,
    Ze,
    Ge,
    Le,
    De,
    ct,
    dt,
    Oe,
    Re,
    Je,
    je,
    $e,
    He,
    Ke,
    Ne,
    ...oa
  ];
  N.derive(() => {
    Ee.val = ie.val;
  });
  N.derive(() => {
    Be.val = fe.val;
  });
  const re = N.state([]), le = N.state([]), Me = N.state({}), ke = N.state({}), ce = N.state({}), de = N.state({}), _e = N.state([]), F = {
    nodes: re,
    elements: le,
    nodeInputs: Me,
    elementInputs: ke,
    deformOutputs: ce,
    analyzeOutputs: de,
    objects3D: _e
  };
  let f = null, d = {}, V = null;
  const xe = {
    modeIdx: 1
  };
  let L, X = null, G = null, ia = [];
  const Q = Te();
  Q.div.style.display = "none";
  function ue() {
    F.objects3D.val = [], F.nodes.val = [], F.elements.val = [], F.nodeInputs.val = {}, F.elementInputs.val = {}, F.deformOutputs.val = {}, F.analyzeOutputs.val = {};
  }
  function se(a) {
    var _a, _b, _c, _d;
    if (f = a, a.standaloneUrl) {
      const i = [
        "__rbsK3Anim",
        "__bfpK3Anim",
        "__endPlateK3Anim"
      ];
      for (const y of i) {
        const b = window[y];
        b && (clearInterval(b), window[y] = null);
      }
      try {
        (_a = L == null ? void 0 : L.stop) == null ? void 0 : _a.call(L);
      } catch {
      }
      ge.v++, ue(), d = {}, W();
      return;
    }
    d = Object.fromEntries(Object.entries(a.params ?? {}).map(([i, y]) => {
      const b = y.default;
      return y.unitType === "force" ? [
        i,
        he(b)
      ] : y.unitType === "moment" ? [
        i,
        be(b)
      ] : [
        i,
        b
      ];
    }));
    const p = [
      "__rbsK3Anim",
      "__bfpK3Anim",
      "__endPlateK3Anim"
    ];
    for (const i of p) {
      const y = window[i];
      y && (clearInterval(y), window[i] = null);
    }
    try {
      (_b = L == null ? void 0 : L.stop) == null ? void 0 : _b.call(L);
    } catch {
    }
    if (ge.v++, ue(), (_c = a.build) == null ? void 0 : _c.call(a, pe(), F, Q), ((_d = a.id) == null ? void 0 : _d.startsWith("conexion-")) || a.id === "placa-base") {
      const i = O.__settings;
      (i == null ? void 0 : i.displayScale) && (i.displayScale.val = -6);
    }
    if (a.defaultShellResult) {
      const i = O.__settings;
      (i == null ? void 0 : i.shellResults) && (i.shellResults.val = a.defaultShellResult), (i == null ? void 0 : i.loads) && (i.loads.val = true), (i == null ? void 0 : i.supports) && (i.supports.val = true);
    }
    ra(a.availableShellResults), sa(), we(), W();
  }
  function sa() {
    var _a, _b;
    const a = O.__settings;
    if (!(a == null ? void 0 : a.deformScale)) return;
    const p = F.nodes.rawVal, r = (_a = F.deformOutputs.rawVal) == null ? void 0 : _a.deformations;
    if (!(p == null ? void 0 : p.length) || !r) {
      a.deformScale.val = 1;
      return;
    }
    let i = 1 / 0, y = 1 / 0, b = 1 / 0, v = -1 / 0, s = -1 / 0, u = -1 / 0;
    for (const n of p) n[0] < i && (i = n[0]), n[0] > v && (v = n[0]), n[1] < y && (y = n[1]), n[1] > s && (s = n[1]), n[2] < b && (b = n[2]), n[2] > u && (u = n[2]);
    const h = Math.sqrt((v - i) ** 2 + (s - y) ** 2 + (u - b) ** 2) || 1;
    let M = 0, w = 0;
    r.forEach((n) => {
      const k = Math.sqrt((n[0] || 0) ** 2 + (n[1] || 0) ** 2), _ = Math.abs(n[2] || 0);
      k > M && (M = k), _ > w && (w = _);
    });
    const S = v - i, z = s - y, q = u - b > 1.1 * Math.max(S, z);
    let I, x;
    if (q) M > 1e-9 ? I = Math.min(5e3, Math.max(1, 0.1 * h / M)) : I = 10, x = 0.15;
    else {
      const n = Math.max(M, w);
      if (n < 1e-30) {
        a.deformScale.val = 1;
        return;
      }
      I = Math.min(5e4, Math.max(1, 0.15 * h / n)), x = 1;
    }
    a.deformScale.val = Math.max(1, I), a.deformScaleZ && (a.deformScaleZ.val = x);
    const E = ((_b = f == null ? void 0 : f.id) == null ? void 0 : _b.startsWith("conexion-")) || (f == null ? void 0 : f.id) === "placa-base";
    a.displayScale && (a.displayScale.val = E ? -6 : -1.5);
  }
  function we() {
    const a = O.__ctx, p = F.nodes.rawVal;
    if (!a || !(p == null ? void 0 : p.length)) return;
    const { camera: r, controls: i, render: y } = a;
    let b = 1 / 0, v = 1 / 0, s = 1 / 0, u = -1 / 0, h = -1 / 0, M = -1 / 0;
    for (const _ of p) _[0] < b && (b = _[0]), _[0] > u && (u = _[0]), _[1] < v && (v = _[1]), _[1] > h && (h = _[1]), _[2] < s && (s = _[2]), _[2] > M && (M = _[2]);
    const w = (b + u) / 2, S = (v + h) / 2, z = (s + M) / 2, B = u - b, q = h - v, I = M - s, x = Math.max(Math.sqrt(B * B + q * q + I * I), 1), E = 2.2 * x;
    i.target.set(w, S, z);
    const n = E / Math.sqrt(3);
    r.position.set(w + n, S - n, z + n), r.up.set(0, 0, 1), r.near = x * 1e-3, r.far = x * 50, r.updateProjectionMatrix(), r.lookAt(w, S, z), i.update(), y == null ? void 0 : y();
    const k = O.__settings;
    (k == null ? void 0 : k.gridSize) && (k.gridSize.val = Math.max(Math.ceil(Math.max(B, q) * 1.2), 2));
  }
  function ra(a) {
    const p = O.querySelectorAll("select"), r = Array.from(p).find((y) => Array.from(y.options).some((b) => b.value === "bendingXX"));
    if (!r) return;
    for (const y of Array.from(r.options)) {
      const b = y.value === "none" || !a || a.includes(y.value);
      y.hidden = !b, y.disabled = !b;
    }
    const i = O.__settings;
    (i == null ? void 0 : i.shellResults) && (r.value = i.shellResults.val, r.dispatchEvent(new Event("change", {
      bubbles: true
    })));
  }
  function pe() {
    if (!f) return {};
    const a = {
      ...d
    };
    for (const [p, r] of Object.entries(f.params)) r.unitType === "force" && (a[p] = gt(d[p])), r.unitType === "moment" && (a[p] = xt(d[p]));
    return a;
  }
  function ee() {
    if (f) {
      if (ue(), f.build(pe(), F, Q), we(), f.computedLabels && X) {
        const a = f.computedLabels(d, F);
        for (const p of Object.keys(X)) p in a && (X[p] = a[p]);
      }
      if (f.inlineComputed && G) for (const a of f.inlineComputed) {
        const p = `__inline_${a.after}_${a.label}`;
        G[p] = a.compute(d, F);
      }
      V == null ? void 0 : V.refresh();
    }
  }
  const oe = document.createElement("div"), Se = "hk_paneHostPos", me = (() => {
    try {
      const a = localStorage.getItem(Se);
      if (a) return JSON.parse(a);
    } catch {
    }
    return null;
  })();
  oe.style.cssText = "position:fixed;" + (me ? `left:${me.left}px;top:${me.top}px;right:auto;` : "top:96px;right:16px;") + "width:min(320px,calc(100vw - 32px));max-width:90vw;z-index:100;max-height:calc(100vh - 112px);overflow-y:auto;font-size:12px;box-shadow:0 6px 24px rgba(0,0,0,0.35);border:1px solid rgba(255,255,255,0.08);";
  document.body.appendChild(oe);
  function Pe(a) {
    let r = a.querySelector(".tp-rotv_b, .tp-fldv_b");
    if (!r) {
      setTimeout(() => Pe(a), 200);
      return;
    }
    r.style.cursor = "move", r.style.userSelect = "none";
    let i = false, y = 0, b = 0, v = 0, s = 0;
    r.addEventListener("mousedown", (u) => {
      i = true, y = u.clientX, b = u.clientY;
      const h = a.getBoundingClientRect();
      v = h.left, s = h.top, a.style.right = "auto", a.style.left = `${v}px`, a.style.top = `${s}px`, u.preventDefault();
    }), window.addEventListener("mousemove", (u) => {
      if (!i) return;
      const h = u.clientX - y, M = u.clientY - b, w = Math.max(0, Math.min(window.innerWidth - 40, v + h)), S = Math.max(0, Math.min(window.innerHeight - 40, s + M));
      a.style.left = `${w}px`, a.style.top = `${S}px`;
    }), window.addEventListener("mouseup", () => {
      if (i) {
        i = false;
        try {
          localStorage.setItem(Se, JSON.stringify({
            left: parseFloat(a.style.left),
            top: parseFloat(a.style.top)
          }));
        } catch {
        }
      }
    });
  }
  function ae(a) {
    const p = O.__ctx;
    if (!p) return;
    const { camera: r, controls: i, render: y } = p, b = F.nodes.rawVal ?? [];
    let v = 1 / 0, s = 1 / 0, u = 1 / 0, h = -1 / 0, M = -1 / 0, w = -1 / 0;
    for (const n of b) n[0] < v && (v = n[0]), n[0] > h && (h = n[0]), n[1] < s && (s = n[1]), n[1] > M && (M = n[1]), n[2] < u && (u = n[2]), n[2] > w && (w = n[2]);
    const S = (v + h) / 2, z = (s + M) / 2, B = (u + w) / 2, q = h - v || 1, I = M - s || 1, x = w - u || 1, E = Math.sqrt(q * q + I * I + x * x) || 5;
    if (i.target.set(S, z, B), a === "iso") {
      r.fov = 45;
      const n = E * 1.2;
      r.position.set(S + n * 0.6, z - n * 0.6, B + n * 0.6);
    } else {
      r.fov = 5;
      const k = E / 2 * 25;
      switch (a) {
        case "plan":
          r.position.set(S, z, B + k);
          break;
        case "elevX":
          r.position.set(S + k, z, B);
          break;
        case "elevY":
          r.position.set(S, z + k, B);
          break;
      }
    }
    r.up.set(0, 0, 1), r.updateProjectionMatrix(), r.lookAt(S, z, B), i.update(), y == null ? void 0 : y();
  }
  function W() {
    var _a;
    if (V && (V.dispose(), V = null), oe.innerHTML = "", !f) return;
    const a = new Ce({
      container: oe,
      title: f.name
    });
    setTimeout(() => Pe(oe), 0);
    const p = Array.from(new Set(U.map((t) => t.category))), r = [
      "\u{1F3C1} Benchmarks",
      "Cimentaciones",
      "Frames 1D",
      "P\xF3rticos 2D",
      "P\xF3rticos 3D",
      "Edificios",
      "Placas",
      "C\xE1scaras",
      "S\xF3lidos",
      "Conexiones",
      "Columnas"
    ], i = [
      ...r.filter((t) => p.includes(t)),
      ...p.filter((t) => !r.includes(t)).sort()
    ], y = "Todas", b = {
      [y]: y
    };
    for (const t of i) b[t] = t;
    const v = {
      category: f.category,
      id: f.id
    }, s = a.addBinding(v, "category", {
      label: "Categor\xEDa",
      options: b
    }), u = (t) => Object.fromEntries(U.filter((e) => t === y || e.category === t).map((e) => [
      `${e.benchmark ? "\u{1F3C1} " : ""}${e.name}`,
      e.id
    ]));
    let h = a.addBinding(v, "id", {
      label: "Ejemplo",
      options: u(v.category)
    });
    if (h.on("change", (t) => {
      const e = U.find((l) => l.id === t.value);
      e && setTimeout(() => se(e), 0);
    }), s.on("change", (t) => {
      const e = u(t.value), l = Object.values(e);
      if (l.length === 0) return;
      l.includes(v.id) || (v.id = l[0]);
      try {
        h.dispose();
      } catch {
      }
      h = a.addBinding(v, "id", {
        label: "Ejemplo",
        options: e,
        index: 2
      }), h.on("change", (m) => {
        const o = U.find((g) => g.id === m.value);
        o && setTimeout(() => se(o), 0);
      });
      const P = U.find((m) => m.id === v.id);
      P && P.id !== (f == null ? void 0 : f.id) && setTimeout(() => se(P), 0);
    }), f.standaloneUrl) {
      const t = f.standaloneUrl, e = a.addFolder({
        title: "\u2139 Ejemplo legacy",
        expanded: true
      });
      e.addButton({
        title: "\u{1F517} Abrir ejemplo \u2192"
      }).on("click", () => {
        window.location.href = t;
      }), e.addButton({
        title: "(usa toolbar VanJS propio)"
      }).on("click", () => {
      }), V = a;
      return;
    }
    const M = a.addFolder({
      title: "Vista",
      expanded: false
    });
    if (M.addButton({
      title: "\u{1F3D7} Isom\xE9trica"
    }).on("click", () => ae("iso")), M.addButton({
      title: "\u2B07 Planta (X-Y)"
    }).on("click", () => ae("plan")), M.addButton({
      title: "\u2192 Elevaci\xF3n X (frente)"
    }).on("click", () => ae("elevX")), M.addButton({
      title: "\u2191 Elevaci\xF3n Y (lado)"
    }).on("click", () => ae("elevY")), f && f.id.startsWith("zapata")) {
      const t = a.addFolder({
        title: "\u{1F4C4} SAFE F2K",
        expanded: false
      });
      t.addButton({
        title: "\u{1F4E4} Exportar a SAFE (.f2k)"
      }).on("click", () => {
        try {
          const e = d, l = e.ks_factor ?? 10.5, P = e.q_adm ?? 20, m = l * P * 9.80665, o = (e.useSimple ?? 1) >= 0.5, g = o ? (e.P_simple ?? 0) * 9.80665 : (e.P_D ?? 10) * 9.80665, T = o ? 0 : (e.P_L ?? 5) * 9.80665, A = o ? (e.Mx_simple ?? 0) * 9.80665 : (e.Mx_D ?? 0) * 9.80665, C = o ? (e.My_simple ?? 0) * 9.80665 : (e.My_D ?? 0) * 9.80665, $ = mt({
            Lz: e.Lz ?? 1.5,
            Bz: e.Bz ?? 1.5,
            tz: e.tz ?? 0.3,
            bc: e.bc ?? 0.4,
            ks_kNm3: m,
            P_dead_kN: g,
            P_live_kN: T,
            Mx_dead_kNm: A,
            My_dead_kNm: C
          }, `Zapata_Hekatan_${Date.now()}.f2k`);
          console.log(`\u2705 F2K exportado: ${$} bytes con ks=${m.toFixed(0)} kN/m\xB3, P_D=${g.toFixed(1)} kN`), alert(`F2K descargado correctamente.

ks=${m.toFixed(0)} kN/m\xB3
P_dead=${g.toFixed(1)} kN

Abrilo en SAFE 20.x: File \u2192 Import \u2192 SAFE Text File (.f2k)`);
        } catch (e) {
          alert(`Error exportando F2K: ${(e == null ? void 0 : e.message) ?? e}`), console.error(e);
        }
      }), t.addButton({
        title: "\u{1F4E5} Importar F2K\u2026"
      }).on("click", () => {
        const e = document.createElement("input");
        e.type = "file", e.accept = ".f2k,.txt", e.onchange = async (l) => {
          var _a2, _b, _c;
          const P = (_a2 = l.target.files) == null ? void 0 : _a2[0];
          if (P) try {
            const m = await P.text(), o = ft(m);
            o.Lz != null && (d.Lz = o.Lz), o.Bz != null && (d.Bz = o.Bz), o.tz != null && (d.tz = o.tz), o.bc != null && (d.bc = o.bc), o.q_adm != null && (d.q_adm = o.q_adm), o.ks_factor != null && (d.ks_factor = o.ks_factor), o.ks_kNm3 != null && (d.ks = o.ks_kNm3), o.P_dead_tonf != null && (d.useSimple = 1, d.P_simple = o.P_dead_tonf, d.useD = 0, d.useL = 0, d.useS = 0), o.Mx_dead_tonfm != null && (d.Mx_simple = o.Mx_dead_tonfm), o.My_dead_tonfm != null && (d.My_simple = o.My_dead_tonfm), o.q_adm != null && o.ks_factor != null && (d.soilType = 0), W(), ee(), alert(`F2K importado: ${P.name}
Lz=${o.Lz}, Bz=${o.Bz}, tz=${o.tz}
ks=${(_b = o.ks_kNm3) == null ? void 0 : _b.toFixed(0)} kN/m\xB3
P_dead=${(_c = o.P_dead_tonf) == null ? void 0 : _c.toFixed(2)} tonf

Los sliders del Tweakpane se actualizaron a estos valores.`);
          } catch (m) {
            alert(`Error importando F2K: ${(m == null ? void 0 : m.message) ?? m}`), console.error(m);
          }
        }, e.click();
      });
    }
    const w = a.addFolder({
      title: "Unidades",
      expanded: false
    }), S = {
      force: ie.val,
      disp: fe.val
    };
    w.addBinding(S, "force", {
      label: "Fuerza",
      options: {
        kN: "kN",
        tonf: "tonf",
        kip: "kip"
      }
    }).on("change", (t) => {
      const e = ie.val, l = t.value;
      if (f && e !== l) {
        const P = e === "kN" ? 1 : e === "tonf" ? 9.80665 : 4.4482216, m = l === "kN" ? 1 : l === "tonf" ? 9.80665 : 4.4482216;
        for (const [o, g] of Object.entries(f.params)) (g.unitType === "force" || g.unitType === "moment") && (d[o] = d[o] * P / m);
      }
      ie.val = l, W(), ee();
    }), w.addBinding(S, "disp", {
      label: "Desplazamiento",
      options: {
        mm: "mm",
        cm: "cm",
        m: "m",
        in: "in"
      }
    }).on("change", (t) => {
      fe.val = t.value, W(), ee();
    });
    const z = "Par\xE1metros", B = /* @__PURE__ */ new Map(), q = (t) => t === z || /\bmodo\b/i.test(t) || /activar/i.test(t) || /combinaci/i.test(t), I = (t) => (B.has(t) || B.set(t, a.addFolder({
      title: t,
      expanded: q(t)
    })), B.get(t));
    let x = null;
    const E = () => {
      x !== null && clearTimeout(x), x = window.setTimeout(() => {
        x = null, ee();
      }, 120);
    }, n = {}, k = {}, _ = {}, j = (t) => t.rangeAdjustable === true || t.rangeAdjustable !== false && (t.unitType === "force" || t.unitType === "moment"), H = /* @__PURE__ */ new Map();
    if (G = {}, f.inlineComputed) for (const t of f.inlineComputed) {
      const e = `__inline_${t.after}_${t.label}`;
      G[e] = t.compute(d, F), H.has(t.after) || H.set(t.after, []), H.get(t.after).push({
        label: t.label,
        key: e,
        compute: t.compute
      });
    }
    const D = f.dynamicParams ? f.dynamicParams(d) : {};
    for (const [t, e] of Object.entries(D)) if (!(t in d)) {
      const l = e.default;
      d[t] = e.unitType === "force" ? he(l) : e.unitType === "moment" ? be(l) : l;
    }
    const J = {
      ...f.params,
      ...D
    };
    for (const [t, e] of Object.entries(J)) {
      const l = e.folder ?? z, P = I(l);
      if (e.boolean) {
        n[t] = d[t] >= 0.5;
        const R = P.addBinding(n, t, {
          label: e.label ?? t
        });
        R.on("change", (Z) => {
          d[t] = Z.value ? 1 : 0, (f == null ? void 0 : f.onParamChange) && (f.onParamChange(t, d), a.refresh()), applyHiddenBindings(), E();
        }), e.hiddenIf && ia.push({
          binding: R,
          hiddenIf: e.hiddenIf
        });
        continue;
      }
      const m = ye(e.label ?? t), o = e.unitType === "force" ? ` ${ut()}` : e.unitType === "moment" ? ` ${pt()}` : e.unitType === "disp" ? ` ${yt()}` : "", T = {
        label: m + o
      };
      e.options !== void 0 ? T.options = e.options : (e.min !== void 0 && (T.min = e.min), e.max !== void 0 && (T.max = e.max), e.step !== void 0 && (T.step = e.step));
      let A = null;
      const C = (R, Z) => {
        var _a2;
        if (A) try {
          (_a2 = A.dispose) == null ? void 0 : _a2.call(A);
        } catch {
        }
        const K = {
          ...T
        };
        R !== void 0 && (K.min = R), Z !== void 0 && (K.max = Z), K.min !== void 0 && d[t] < K.min && (d[t] = K.min), K.max !== void 0 && d[t] > K.max && (d[t] = K.max), A = P.addBinding(d, t, K), A.on("change", () => {
          (f == null ? void 0 : f.onParamChange) && (f.onParamChange(t, d), a.refresh()), e.regenOnChange ? window.setTimeout(() => {
            W(), ee();
          }, 80) : E();
        });
      };
      C(e.min, e.max), j(e) && e.min !== void 0 && e.max !== void 0 && (k[t] = {
        min: e.min,
        max: e.max
      }, _[t] = {
        rebuild: (R, Z) => C(R, Z)
      });
      const $ = H.get(t);
      if ($ && G) for (const R of $) P.addBinding(G, R.key, {
        readonly: true,
        label: R.label,
        view: "text"
      });
    }
    const Y = Object.keys(k);
    if (Y.length > 0) {
      const t = a.addFolder({
        title: "\u{1F4CF} Rangos",
        expanded: false
      });
      for (const e of Y) {
        const l = f.params[e], P = ye(l.label ?? e), m = l.step ?? 1, o = Math.abs(l.max - l.min), g = l.min - o * 5, T = l.max + o * 5;
        t.addBinding(k[e], "min", {
          label: `${P} min`,
          min: g,
          max: l.max,
          step: m
        }).on("change", (A) => {
          const C = Math.min(A.value, k[e].max - m);
          k[e].min = C, _[e].rebuild(C, k[e].max);
        }), t.addBinding(k[e], "max", {
          label: `${P} max`,
          min: l.min,
          max: T,
          step: m
        }).on("change", (A) => {
          const C = Math.max(A.value, k[e].min + m);
          k[e].max = C, _[e].rebuild(k[e].min, C);
        });
      }
    }
    if (f.computedLabels) {
      const t = a.addFolder({
        title: "\u{1F4CA} Calculados",
        expanded: true
      }), e = f.computedLabels(d, F);
      X = {
        ...e
      }, console.log("[Calculados]", X);
      for (const l of Object.keys(e)) t.addBinding(X, l, {
        readonly: true,
        view: "text",
        interval: 0
      });
    } else X = null;
    if (f.hasModal) {
      const t = a.addFolder({
        title: "\u26A1 Modal + Animaci\xF3n",
        expanded: true
      }), e = {
        mode: "\u2014",
        frequency: "\u2014",
        period: "\u2014",
        dominant: "\u2014",
        state: "\u23F8 Detenido"
      };
      (_a = L.dispose) == null ? void 0 : _a.call(L), L = ve({
        mesh: {
          nodes: re,
          elements: le,
          deformOutputs: ce,
          analyzeOutputs: de
        },
        viewerElm: O,
        scalePercent: 5,
        onStatusChange: () => {
          const m = L.getStatus();
          e.mode = m.mode, e.frequency = m.frequency, e.period = m.period, e.dominant = m.dominant, e.state = m.state, V == null ? void 0 : V.refresh();
        }
      });
      let l = null;
      const P = {
        div: Q.div,
        render: (m, o) => {
          var _a2;
          l = m, Q.render(m, o), ((_a2 = m == null ? void 0 : m.frequencies) == null ? void 0 : _a2.length) && (L.setResults(m), L.setMode(0), L.play(), xe.modeIdx = 1, V == null ? void 0 : V.refresh());
        }
      };
      t.addButton({
        title: "\u25B6 Correr modal + animar"
      }).on("click", () => {
        L.stop(), Q.div.style.display = "block", f.runModal && f.runModal(pe(), F, P);
      }), t.addBinding(xe, "modeIdx", {
        label: "Modo #",
        min: 1,
        max: 30,
        step: 1
      }).on("change", (m) => {
        l && L.setMode(Math.round(m.value) - 1);
      }), t.addBinding(e, "mode", {
        readonly: true,
        view: "text",
        interval: 0,
        label: "Modo"
      }), t.addBinding(e, "frequency", {
        readonly: true,
        view: "text",
        interval: 0,
        label: "Frecuencia"
      }), t.addBinding(e, "period", {
        readonly: true,
        view: "text",
        interval: 0,
        label: "Per\xEDodo"
      }), t.addBinding(e, "dominant", {
        readonly: true,
        view: "text",
        interval: 0,
        label: "Dominante"
      }), t.addBinding(e, "state", {
        readonly: true,
        view: "text",
        interval: 0,
        label: "Estado"
      }), t.addButton({
        title: "\u23F9 Detener y restaurar"
      }).on("click", () => {
        L.stop();
      }), t.addButton({
        title: "\u25B6 Reanudar"
      }).on("click", () => {
        l && L.play();
      });
    }
    V = a;
  }
  const la = {
    deformedShape: true,
    displayScale: -1.5,
    shellResults: "pressure",
    gridSize: 10,
    showCotas: true
  }, O = Fe({
    mesh: {
      nodes: re,
      elements: le,
      nodeInputs: Me,
      elementInputs: ke,
      deformOutputs: ce,
      analyzeOutputs: de
    },
    objects3D: _e,
    settingsObj: la
  });
  document.body.append(O, ze({
    sourceCode: "https://github.com/GiorgioBurbanelli89/hekatan-struct",
    author: "https://www.linkedin.com/in/jorge-burbano-213741138/"
  }));
  document.body.appendChild(Q.div);
  L = ve({
    mesh: {
      nodes: re,
      elements: le,
      deformOutputs: ce,
      analyzeOutputs: de
    },
    viewerElm: O,
    scalePercent: 5
  });
  let ne = new URLSearchParams(window.location.search).get("t");
  if (!ne || ne === "zapata-aislada") {
    ne = "zapata-aislada-validacion";
    try {
      const a = new URL(window.location.href);
      a.searchParams.set("t", ne), window.history.replaceState(null, "", a.toString());
    } catch {
    }
  }
  const te = U.find((a) => a.id === ne) || U.find((a) => a.id === "zapata-aislada-validacion") || U[0];
  te && (se(te), (te.id === "zapata-aislada" || te.id === "zapata-aislada-validacion" || te.id === "zapata-viga-amarre") && setTimeout(() => ae("iso"), 200));
});
