import { b as M, g as $, d as z, P as c, __tla as __tla_0 } from "./pazFrameE2k-CFRjUIcU.js";
import { p as D, r as F, n as w, g as p } from "./newmarkBeta-CZ1uRmPu.js";
let W;
  ...
  W = {
    ...
    computedLabels(e) {
      ...
      return {
        ...
        "u_max esperado libro": `${m.u_max_in} in (DLF=${m.DLF_max})`
      };
    },
    build(e, o) {
      const i = h(e);
      ...
      let d = `[Paz 4.1] 1-DOF rectangular impulse
`;
      d += `  k=${a.toFixed(2)} lb/in, m=${s.toFixed(4)} lb\xB7s\xB2/in
`, d += `  T=${b.toFixed(4)} s  (libro 0.2446)
`, d += `  td/T=${(e.td_s / b).toFixed(3)}
`;
      const g = [
        [
          s
        ...
        dt: e.dt_s,
        nSteps: y
      }), u = Math.max(...n.u.map((t) => Math.abs(t[0]))), E = e.F0_lb / a, H = u / E, S = (u - m.u_max_in) / m.u_max_in * 100;
      d += `  Newmark-\u03B2: u_max = ${u.toFixed(4)} in  (libro ${m.u_max_in}, \u0394 ${S.toFixed(2)}%)
`, d += `  DLF calculado = ${H.toFixed(3)} (libro ${m.DLF_max})
`;
      const L = a * u * l / 2 / e.S_in3;
      if (d += `  \u03C3_max columnas = ${L.toFixed(0)} psi  (libro ${m.sigma_max_psi})
`, e.showTH > 0.5) {
