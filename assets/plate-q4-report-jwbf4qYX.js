...
import { p as w, __tla as __tla_0 } from "./didacticCpp-BkpWNA4I.js";
import { __tla as __tla_1 } from "./deform-BSBmQer1.js";
...
Promise.all([
  ...
  function B(r, e, p, l, u, $, b = 80) {
    let c = 0;
    for (let i = 1; i <= b; i += 2) for (let s = 1; s <= b; s += 2) {
      const d = (i * Math.PI / r) ** 2 + (s * Math.PI / e) ** 2, n = 16 * p / (Math.PI ** 2 * i * s);
      c += n / (l * d ** 2) * Math.sin(i * Math.PI * u / r) * Math.sin(s * Math.PI * $ / e);
    }
    return c;
  }
  function O(r, e, p, l, u, $, b, c = 80) {
    let i = 0;
    for (let s = 1; s <= c; s += 2) for (let d = 1; d <= c; d += 2) {
      const n = s * Math.PI / r, o = d * Math.PI / e, h = n * n + o * o, m = 16 * p / (Math.PI ** 2 * s * d) / (l * h ** 2);
      i += -l * (n * n + u * o * o) * m * Math.sin(s * Math.PI * $ / r) * Math.sin(d * Math.PI * b / e);
    }
    return i;
  }
  function z(r, e, p, l, u, $, b, c = 80) {
    let i = 0;
    for (let s = 1; s <= c; s += 2) for (let d = 1; d <= c; d += 2) {
      const n = s * Math.PI / r, o = d * Math.PI / e, h = n * n + o * o, m = 16 * p / (Math.PI ** 2 * s * d) / (l * h ** 2);
      i += -l * (u * n * n + o * o) * m * Math.sin(s * Math.PI * $ / r) * Math.sin(d * Math.PI * b / e);
    }
    return i;
  }
  function N(r, e, p, l, u, $, b, c = 80) {
    let i = 0;
    for (let s = 1; s <= c; s += 2) for (let d = 1; d <= c; d += 2) {
      const n = s * Math.PI / r, o = d * Math.PI / e, h = n * n + o * o, m = 16 * p / (Math.PI ** 2 * s * d) / (l * h ** 2);
      i += -l * (1 - u) * n * o * m * Math.cos(s * Math.PI * $ / r) * Math.cos(d * Math.PI * b / e);
    }
    return i;
  }
  function a(r, e = 6) {
  ...
  function C() {
    const r = performance.now(), e = F(), p = performance.now() - r, l = t.E * t.t ** 3 / (12 * (1 - t.nu ** 2)), u = t.E / (2 * (1 + t.nu)), b = 5 / 6 * u * t.t, c = (t.nx + 1) * (t.ny + 1), i = t.nx * t.ny, s = c * 3, d = t.Lx / t.nx, n = t.Ly / t.ny, o = B(t.Lx, t.Ly, Math.abs(t.q), l, t.Lx / 2, t.Ly / 2), h = O(t.Lx, t.Ly, Math.abs(t.q), l, t.nu, t.Lx / 2, t.Ly / 2), f = z(t.Lx, t.Ly, Math.abs(t.q), l, t.nu, t.Lx / 2, t.Ly / 2), m = N(t.Lx, t.Ly, Math.abs(t.q), l, t.nu, 0, 0), q = N(t.Lx, t.Ly, Math.abs(t.q), l, t.nu, d / 2, n / 2), y = Math.abs(e.centerW ?? e.maxW), g = e.maxMxx, v = e.maxMyy, _ = e.maxMxy, S = e.maxQx, E = e.maxQy, I = Math.floor(t.ny / 2), L = Math.floor(t.nx / 2), D = I * t.nx + L, M = e.elementResults[D] || e.elementResults[Math.floor(i / 2)], P = e.elementResults[0];
    return `
<style>
...
<div class="two-col">
  ...
  <div>
    ...
    <table>
      ...
      <tr><td>G (cortante)</td><td class="num">${u.toExponential(4)}</td><td>kN/m\xB2</td></tr>
...
<table>
  ...
  <tr><td>Total elementos</td><td>${i}</td></tr>
...
<div class="eq eq-center">
  $$D_{s0} = \\kappa_s \\cdot G \\cdot t = \\frac{5}{6} \\cdot \\frac{E}{2(1+\\nu)} \\cdot t = \\frac{5}{6} \\cdot ${u.toExponential(4)} \\cdot ${t.t} = ${a(b, 4)}$$
</div>
<div class="eq eq-center">
  $$\\mathbf{D}_s = D_{s0} \\begin{bmatrix} 1 & 0 \\\\ 0 & 1 \\end{bmatrix} = ${a(b, 2)} \\, \\mathbf{I}_2$$
</div>
<div class="note">
...
<p>Para cada elemento e = 1..${i}, se ensambla su K<sub>e</sub> (12\xD712 = 4 nodos \xD7 3 GDL) y f<sub>e</sub> (12\xD71) en las matrices globales. Los \xEDndices <em>p, q</em> recorren los 12 GDL locales del elemento:</p>
...
<p>Mapeo DOF (dofMap): el GDL local <em>p</em> = 3\xB7i + d \u2014 con nodo local i \u2208 {0,1,2,3} y componente d \u2208 {0=w, 1=\u03B2<sub>x</sub>, 2=\u03B2<sub>y</sub>} \u2014 se mapea al GDL global <strong>dofMap[p] = 3\xB7G + d</strong>, donde G es el nodo global del nodo local i (no confundir con el punto de Gauss g del vector de carga).</p>

<table>
  <tr><th>Propiedad</th><th>Valor</th></tr>
  <tr><td>Tama\xF1o K global</td><td>${s} \xD7 ${s}</td></tr>
  <tr><td>Formato</td><td>Sparse (Eigen::SparseMatrix, triplet assembly)</td></tr>
  <tr><td>Entradas no-cero estimadas</td><td>~${i * 144} (antes de compresi\xF3n)</td></tr>
</table>
...
<div class="${Math.abs((y - o) / o) < 0.05 ? "pass" : "note"}">
  <strong>w<sub>center</sub>:</strong> FEM = ${a(y)} vs Navier = ${a(o)} \u2192 Error = ${x(y, o)} \u2713
</div>
<div class="pass">
