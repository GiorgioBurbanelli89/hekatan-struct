...
import { v as n, L as ta, M as pe, e as ue, d as he, V as ea, B as aa, a as oa } from "./theme-dnnHIQ7U.js";
import { P as sa } from "./tweakpane-BXg6ZhiP.js";
import { a as na } from "./analyze-DZ5QYzyM.js";
import { d as la, __tla as __tla_0 } from "./didacticCpp-C2di29sC.js";
import { g as ca, a as ve, b as be } from "./getViewer-f7b0_qaE.js";
import { e as ra } from "./makeDraggable-zx2br6Yh.js";
import { g as ia } from "./getParameters-BanUaban.js";
import { g as ma } from "./styles-CClJeH1b.js";
import "./pureFunctionsAny.generated-DeJSBP3k.js";
import { __tla as __tla_1 } from "./deform-D-nmLsId.js";
...
Promise.all([
  ...
  const kt = 2e8, St = 0.3, fe = kt / (2 * (1 + St)), xe = 78, da = 25e4, pa = 6e5, i = {
    B: {
      value: n.state(0.5),
      min: 0.3,
      max: 1.2,
  ...
  n.derive(() => {
    ...
    for (let t = 0; t <= R; t++) {
      const e = [];
      for (let a = 0; a <= $; a++) e.push(A(-o / 2 + a * qt, -p / 2 + t * Ht, u));
      F.push(e);
    }
    ...
    const f = Math.max(2, Math.round(m / qt)), x = Math.max(2, Math.round(d / Ht)), nt = m / f, lt = d / x, q = it / g, P = [];
    for (let t = 0; t <= g; t++) {
      const e = [];
      for (let a = 0; a <= f; a++) {
        const s = -m / 2 + a * nt;
        t === 0 ? e.push(j(s, -d / 2)) : e.push(A(s, -d / 2, u + t * q));
      }
      P.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -d / 2 + a * lt;
        t === 0 ? e.push(j(-m / 2, s)) : a === 0 ? e.push(P[t][0]) : a === x ? e.push(N[t][0]) : e.push(A(-m / 2, s, u + t * q));
      }
      z.push(e);
    }
    ...
    for (let t = 0; t <= g; t++) {
      ...
      for (let a = 0; a <= x; a++) {
        const s = -d / 2 + a * lt;
        t === 0 ? e.push(j(m / 2, s)) : a === 0 ? e.push(P[t][f]) : a === x ? e.push(N[t][f]) : e.push(A(m / 2, s, u + t * q));
      }
      k.push(e);
    }
    for (let t = 0; t < g; t++) for (let e = 0; e < x; e++) S(k[t][e], k[t][e + 1], k[t + 1][e + 1], k[t + 1][e], O);
    const ke = Math.min(0.2, it * 0.4), Gt = Math.min(0.1, (o - m) / 2 * 0.7), Se = Math.max(1, Math.round(ke / q));
    function C(t, e, a, s) {
      const [c, l] = t, [_, T] = e, Ct = a[0][s], zt = j(c + _ * Gt, l + T * Gt), de = a[Math.min(Se, a.length - 1)][s];
      S(Ct, zt, de, de, O);
    }
    ...
    const $t = Math.PI * y * y / 4, _t = Math.PI * y ** 4 / 64, Rt = 2 * _t, H = [], Fe = (o - 2 * dt) / Math.max(1, Et - 1), je = (p - 2 * Lt) / Math.max(1, mt - 1);
    for (let t = 0; t < Et; t++) for (let e = 0; e < mt; e++) {
      const a = -o / 2 + dt + t * Fe, s = -p / 2 + Lt + e * je;
      Math.abs(a) < m / 2 + 5e-3 && Math.abs(s) < d / 2 + 5e-3 || H.push([
        a,
        s
      ]);
    }
    ...
    for (let t = 0; t <= M; t++) {
      ...
      for (let a = 0; a <= b; a++) {
        const s = [];
        for (let c = 0; c <= v; c++) s.push(A(-U / 2 + c * yt, -V / 2 + a * Qt, -pt + t * Ee));
        e.push(s);
      }
      r.push(e);
    }
    ...
    for (const [t, e] of Ie) {
      const a = A(t, e, u + J), s = j(t, e), c = A(t, e, u - D);
    ...
    function G(t, e) {
      const s = [];
      for (let _ = 0; _ <= 5 * 2; _++) {
        const T = _ / 10, Ct = u * (1 - T), zt = _ % 2 === 0 ? 0 : 8e-3;
        s.push(new ea(t + zt, e, Ct));
      }
  ...
  document.body.append(ia(i), Ae, ma({
