(async ()=>{
    ...
    async function Te(R = {}) {
        ...
        function he(e) {
            Object.getOwnPropertyDescriptor(d, e) || Object.defineProperty(d, e, {
                ...
                get () {
                    ...
                    De(e) && (r += ". Alternatively, forcing filesystem support (-sFORCE_FILESYSTEM) can export this for you"), S(r);
                }
            });
        }
        var pe, _e, I, G, de, _, M, re = !1;
        function ce() {
            var e = cr.buffer;
            I = new Int8Array(e), d.HEAPU8 = G = new Uint8Array(e), de = new Int32Array(e), d.HEAPU32 = _ = new Uint32Array(e), d.HEAPF64 = new Float64Array(e), M = new BigInt64Array(e), new BigUint64Array(e);
        }
        ...
        var Ee;
        function fr() {
            return d.locateFile ? Z("deform.wasm") : new URL("/hekatan-struct-lineal/assets/deform-DpFV3VkJ.wasm", import.meta.url).href;
        }
        function rr(e) {
...
`));
                ...
                class a {
                    ...
                    cacheLength() {
                        ...
                        var W = (q, Ae)=>{
                            ...
                            return D.open("GET", t, !1), E !== L && D.setRequestHeader("Range", "bytes=" + q + "-" + Ae), D.responseType = "arraybuffer", D.overrideMimeType && D.overrideMimeType("text/plain; charset=x-user-defined"), D.send(null), D.status >= 200 && D.status < 300 || D.status === 304 || S("Couldn't load " + t + ". Status: " + D.status), D.response !== void 0 ? new Uint8Array(D.response || []) : hr(D.responseText || "");
                        }, Ze = this;
                        Ze.setDataGetter((q)=>{
                            var Ae = q * L, D = (q + 1) * L - 1;
                            return D = Math.min(D, E - 1), typeof Ze.chunks[q] > "u" && (Ze.chunks[q] = W(Ae, D)), typeof Ze.chunks[q] > "u" && S("doXHR failed!"), Ze.chunks[q];
            ...
            writeStat (e, r) {
                _[e >> 2] = r.dev, _[e + 4 >> 2] = r.mode, _[e + 8 >> 2] = r.nlink, _[e + 12 >> 2] = r.uid, _[e + 16 >> 2] = r.gid, _[e + 20 >> 2] = r.rdev, M[e + 24 >> 3] = BigInt(r.size), de[e + 32 >> 2] = 4096, de[e + 36 >> 2] = r.blocks;
                var t = r.atime.getTime(), o = r.mtime.getTime(), i = r.ctime.getTime();
                return M[e + 40 >> 3] = BigInt(Math.floor(t / 1e3)), _[e + 48 >> 2] = t % 1e3 * 1e3 * 1e3, M[e + 56 >> 3] = BigInt(Math.floor(o / 1e3)), _[e + 64 >> 2] = o % 1e3 * 1e3 * 1e3, M[e + 72 >> 3] = BigInt(Math.floor(i / 1e3)), _[e + 80 >> 2] = i % 1e3 * 1e3 * 1e3, M[e + 88 >> 3] = BigInt(r.ino), 0;
            },
            writeStatFs (e, r) {
    ...
    function tt(R, O, d, v, x = 10, H = 0, j = 0, K = 1, J, ve) {
        ...
        }, Re = b(v.elasticities), le = b(v.areas), ee = b(v.momentsOfInertiaZ), k = b(v.momentsOfInertiaY), Oe = b(v.shearModuli), De = b(v.torsionalConstants), Me = b(v.densities), he = b(v.thicknesses), pe = b(v.poissonsRatios), _e = b(v.membraneModifiers), I = b(v.bendingModifiers), G = v.plateFormulations, de = G ? Array.from(G.keys()) : [], _ = G ? Array.from(G.values()) : [], M = C(de, Uint32Array, h.HEAPU32);
        g.push(M);
        ...
        g.push(ye), h._modal(Z, R.length, Q, oe.length, ie, O.length, se, c, N.length, Re.keysPtr, Re.valuesPtr, Re.size, le.keysPtr, le.valuesPtr, le.size, ee.keysPtr, ee.valuesPtr, ee.size, k.keysPtr, k.valuesPtr, k.size, Oe.keysPtr, Oe.valuesPtr, Oe.size, De.keysPtr, De.valuesPtr, De.size, Me.keysPtr, Me.valuesPtr, Me.size, he.keysPtr, he.valuesPtr, he.size, pe.keysPtr, pe.valuesPtr, pe.size, _e.keysPtr, _e.valuesPtr, _e.size, I.keysPtr, I.valuesPtr, I.size, M, re, de.length, er, S, Qe.length, rr, tr, Ee.length, Ce.keysPtr, Ce.valuesPtr, Ce.size, Ie.keysPtr, Ie.valuesPtr, Ie.size, Ue.keysPtr, Ue.valuesPtr, Ue.size, Le, ir, nr.length, ge.keysPtr, ge.valuesPtr, ge.size, K, Be.keysPtr, Be.valuesPtr, Be.size, ze, $ ? $.length : 0, x, H, j, He, V, Y, xe, We, je, Ge, ye);
