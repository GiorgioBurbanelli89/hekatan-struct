...
import { m as Dt, n as bt, o as Gt, p as Rt, L as jt, B as Zt, V as Pt, a as qt, S as Jt, e as Kt, c as Ut } from "./Text-BA03d7vw.js";
let fo;
let __tla = Promise.all([
  ...
  fo = {
    ...
    build(t, n) {
          ...
          for (let P = 0; P < r.length - 1; P++) for (let m = 0; m < f.length - 1; m++) {
            ...
            for (let T = 0; T <= ct; T++) {
              ...
              for (let lt = 0; lt <= at; lt++) {
                const xt = S + lt / at * (B - S), $t = $ + T / ct * (w - $), _t = e(xt, $t, a), pt = o.get(_t);
            ...
            for (let T = 0; T < ct; T++) for (let k = 0; k < at; k++) K.add(z.length), z.push([
              Z[T][k],
              Z[T][k + 1],
              Z[T + 1][k + 1],
              Z[T + 1][k]
      ...
      n.deformOutputs.val = vt, n.analyzeOutputs.val = kt(_, z, n.elementInputs.val, vt);
      const et = Wt(r, f, O), At = dt[0], Ht = ut[0], Yt = gt[0], Tt = mt[0];
      et.push(q(`Col ${(At * 100).toFixed(0)}\xD7${(Ht * 100).toFixed(0)} cm`, r[0] + 0.3, f[0] + 0.3, O[1] * 0.5, "#ffaa00")), et.push(q(`Viga ${(Yt * 100).toFixed(0)}\xD7${(Tt * 100).toFixed(0)} cm`, (r[0] + r[1]) / 2, f[0], O[1] + 0.2, "#ffaa00"));
      try {
