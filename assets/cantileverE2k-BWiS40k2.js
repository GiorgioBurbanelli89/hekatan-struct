import { a as j } from "./analyze-DZ5QYzyM.js";
import { d as W, __tla as __tla_0 } from "./didacticCpp-C2di29sC.js";
let k, Q;
let __tla = Promise.all([
  ...
  function w(t, a) {
    ...
    if (a === 0) {
      const n = s - 2 * i, u = s * s - n * n, A = (s ** 4 - n ** 4) / 12, T = 2 * A, _ = u, e = A, I = T, M = t.E_s, h = M / 2.6, c = t.gamma_s * u, r = `HSS${(s * 1e3).toFixed(0)}x${(s * 1e3).toFixed(0)}x${(i * 1e3).toFixed(0)}`;
      return {
        matKey: a,
        A_eq: _,
        I_eq: e,
      ...
      const n = s * s, u = s * s * s * s / 12, A = 0.141 * s * s * s * s, T = t.E_c, _ = T / 2.4, e = t.gamma_c * n, I = `C${(s * 1e3).toFixed(0)}x${(s * 1e3).toFixed(0)}`;
      return {
        matKey: a,
        A_eq: n,
        I_eq: u,
      ...
      const n = s - 2 * i, u = s * s - n * n, A = n * n, T = (s ** 4 - n ** 4) / 12, _ = n ** 4 / 12, e = 2 * T, I = t.E_s / t.E_c, M = u + A / I, h = T + _ / I, c = e, r = t.E_s, l = r / 2.6, O = t.gamma_s * u + t.gamma_c * A, m = `CR${(s * 1e3).toFixed(0)}X${(s * 1e3).toFixed(0)}X${(i * 1e3).toFixed(0)}1mm`;
      return {
        matKey: a,
        A_eq: M,
        I_eq: h,
  ...
  k = function(t, a, s, i = 1) {
    const n = w(t, s), u = t.L, A = Math.max(1, Math.round(t.nSegments)), T = u / A, _ = [];
    ...
    for (let o = 0; o <= A; o++) {
      const B = o === 0 || o === A, J = -n.q * T * (B ? 0.5 : 1) * i;
      M.set(o, [
        0,
        0,
        J,
    ...
    const h = t.P_lat_y ?? 0, c = t.M_top_x ?? 0, r = t.M_top_y ?? 0, l = t.M_top_z ?? 0;
    if (Math.abs(t.P_lat) > 1e-9 || Math.abs(h) > 1e-9 || Math.abs(c) > 1e-9 || Math.abs(r) > 1e-9 || Math.abs(l) > 1e-9) {
      const o = M.get(A) ?? [
        0,
        0,
        0,
      ...
      o[0] += t.P_lat, o[1] += h, o[3] += c, o[4] += r, o[5] += l, M.set(A, o);
    }
  ...
  function X(t, a) {
    ...
    const l = [];
    Math.abs(t.A_mod - 1) > 1e-6 && l.push(`AREAMODIFIER ${E(t.A_mod)}`), Math.abs(t.As2_mod - 1) > 1e-6 && l.push(`AS2MODIFIER ${E(t.As2_mod)}`), Math.abs(t.As3_mod - 1) > 1e-6 && l.push(`AS3MODIFIER ${E(t.As3_mod)}`), Math.abs(t.J_mod - 1) > 1e-6 && l.push(`JMODIFIER ${E(t.J_mod)}`), Math.abs(t.I22_mod - 1) > 1e-6 && l.push(`I22MODIFIER ${E(t.I22_mod)}`), Math.abs(t.I33_mod - 1) > 1e-6 && l.push(`I33MODIFIER ${E(t.I33_mod)}`), l.length > 0 && e.push(`  FRAMESECTION  "${c.name}"  ${l.join("  ")}  `), e.push(""), e.push("$ POINT COORDINATES"), e.push('  POINT "1"  0 0 '), e.push(""), e.push("$ LINE CONNECTIVITIES"), e.push('  LINE  "C1"  COLUMN  "1"  "1"  1'), e.push(""), e.push("$ POINT ASSIGNS"), e.push('  POINTASSIGN  "1"  "Story1"  DIAPH "D1"  '), e.push('  POINTASSIGN  "1"  "Base"  RESTRAINT "UX UY UZ RX RY RZ"  DIAPH "DISCONNECTED"  '), e.push(""), e.push("$ LINE ASSIGNS"), e.push(`  LINEASSIGN  "C1"  "Story1"  SECTION "${c.name}"  RIGIDZONE 0.5 MINNUMSTA ${Math.max(2, t.nSegments)} AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `), e.push("");
