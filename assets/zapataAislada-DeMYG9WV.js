import { L as Ve, v as ct, V as k, a as Te, B as $e } from "./Text-CSfbf1xJ.js";
import { a as rt } from "./analyze-BydHtRcI.js";
  ...
  Lt = {
    ...
    inlineComputed: [
      ...
      },
      {
        ...
        label: "k_r Biot",
        compute: (e) => {
          const t = e.tz ?? 0.15, l = e.Lz ?? 2.5, s = e.ks ?? 2059, r = A * t ** 3 / (12 * (1 - B ** 2)) / (s * l ** 4);
          return r.toFixed(3) + (r < 1 ? " FLEX" : " R\xCDG");
        }
      }
    ],
    computedLabels(e, t) {
      var _a;
      const l = (e.q_adm ?? 20) * w, s = e.ks ?? l * (e.ks_factor ?? 10.5), L = e.tz ?? 0.15, r = e.Lz ?? 2.5, C = A * L ** 3 / (12 * (1 - B ** 2)), x = C / (s * r ** 4), X = (e.useSimple ?? 0) >= 0.5, ee = (e.useD ?? 0) >= 0.5, P = (e.useL ?? 0) >= 0.5, Z = (e.useS ?? 0) >= 0.5, z = (e.useFactors ?? 1) >= 0.5, te = z ? e.fD ?? 1.2 : 1, ae = z ? e.fL ?? 1.6 : 1, D = z ? e.fS ?? 0 : 1, O = ee ? 1 : 0, R = P ? 1 : 0, G = Z ? 1 : 0;
      let y = 0;
    ...
    build(e, t) {
      var _a;
      const { Lz: l, Bz: s, tz: L, bc: r, Hp: C } = e, x = e.q_adm, X = e.ks_factor, ee = x * w, P = e.ks ?? ee * X, Z = (e.useSimple ?? 0) >= 0.5, z = (e.useD ?? 0) >= 0.5, te = (e.useL ?? 0) >= 0.5, ae = (e.useS ?? 0) >= 0.5, D = (e.useFactors ?? 1) >= 0.5, O = D ? e.fD ?? 1.2 : 1, R = D ? e.fL ?? 1.6 : 1, G = D ? e.fS ?? 0 : 1, y = z ? 1 : 0, m = te ? 1 : 0, Y = ae ? 1 : 0, V = O, F = R, H = G;
      let I = 0, v = 0, N = 0;
      ...
      const ie = [], J = [], ce = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), ke = /* @__PURE__ */ new Map(), Le = /* @__PURE__ */ new Map(), Pe = /* @__PURE__ */ new Map(), De = /* @__PURE__ */ new Map(), ve = /* @__PURE__ */ new Map(), Ne = /* @__PURE__ */ new Map(), me = /* @__PURE__ */ new Map(), qe = /* @__PURE__ */ new Map(), fe = /* @__PURE__ */ new Map(), de = (a, o, n) => {
      ...
      for (let a = 0; a < M.length - 1; a++) for (let o = 0; o < b.length - 1; o++) {
        ...
        J.push([
          ...
          u[a + 1][o + 1],
          u[a + 1][o]
        ]), Le.set(n, L), ce.set(n, A), re.set(n, B), me.set(n, Be);
      }
      const Ue = de(oe, se, 0), Ee = de(oe, se, C), g = J.length;
      J.push([
        Ue,
        Ee
      ]), ce.set(g, A), re.set(g, B), Ne.set(g, dt), ke.set(g, r * r), Pe.set(g, r ** 4 / 12), De.set(g, r ** 4 / 12), ve.set(g, 0.14 * r ** 4), me.set(g, Be), qe.set(g, {
        type: "rect",
        b: r,
        h: r
      });
      ...
      try {
        ...
        const a = rt(t.nodes.val, t.elements.val, t.elementInputs.val, t.deformOutputs.val), o = t.deformOutputs.rawVal.deformations, n = /* @__PURE__ */ new Map();
        let _ = 0;
        ...
        const j = i / e.q_adm, f = A * L ** 3 / (12 * (1 - B ** 2)) / (P * l ** 4), c = [];
        Z ? c.push("Simple") : (z && c.push(`D${D ? "\xD7" + V : ""}`), te && c.push(`L${D ? "\xD7" + F : ""}`), ae && c.push(`S${D ? "\xD7" + H : ""}`), c.length || c.push("\u26A0 NINGUNO activo"));
        const q = c.join(" + ");
  ...
  k_r\xEDgidez = ${f.toFixed(2)} (${f < 1 ? "FLEXIBLE" : "R\xCDGIDA"} \u2014 flexible muestra concentraci\xF3n, r\xEDgida uniforme)`);
      ...
      const Se = Ge * 12, Ze = new Set(ne), xe = (_a = document.querySelector("#viewer")) == null ? void 0 : _a.__settings, Fe = (a, o) => {
        ...
        for (const j of ne) {
          ...
          if (!le) continue;
          const f = le[0], c = le[1], q = ye == null ? void 0 : ye.get(j), E = (d) => Number.isFinite(d) ? d : 0, W = q ? E(q[0]) : 0, $ = q ? E(q[1]) : 0, he = q ? E(q[2]) : 0, Q = f + W * n, be = c + $ * n, U = 0 + he * n, Ke = U - i, Me = (d) => [
            f + (Q - f) * d,
            c + (be - c) * d,
            i + Ke * d
    ...
    runModal(e, t, l) {
      var _a, _b;
      const s = t.nodes.val, L = t.elements.val, r = t.nodeInputs.val, C = t.elementInputs.val;
      if (!(!s.length || !L.length || !((_a = C.densities) == null ? void 0 : _a.size))) try {
        const x = mt(s, L, r, C, 12);
        l.render(x, {
