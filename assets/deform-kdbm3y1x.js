...
let __tla = (async () => {
  On = async function(we = {}) {
    ...
    var Pe;
    function br() {
      return d.locateFile ? pr("deform.wasm") : new URL("/hekatan-struct-lineal/assets/deform-Cwcbau8L.wasm", import.meta.url).href;
    }
    function Tr(e) {
    ...
    var pe = (e) => sr(e), Xe = (e) => {
      ...
      var m = (h) => {
        var S = h >= 0 ? "-" : "+", P = Math.abs(h), N = String(Math.floor(P / 60)).padStart(2, "0"), D = String(P % 60).padStart(2, "0");
        return `UTC${S}${N}${D}`;
      }, v = m(c), _ = m(l);
...
`));
      ...
      createLazyFile(e, r, t, n, o) {
        class a {
          ...
          cacheLength() {
            ...
            var h = Number(_.getResponseHeader("Content-length")), S, P = (S = _.getResponseHeader("Accept-Ranges")) && S === "bytes", N = (S = _.getResponseHeader("Content-Encoding")) && S === "gzip", D = 1024 * 1024;
            P || (D = h);
            var C = (L, X) => {
              L > X && T("invalid range (" + L + ", " + X + ") or no bytes requested!"), X > h - 1 && T("only " + h + " bytes available! programmer error!");
              var O = new XMLHttpRequest();
              return O.open("GET", t, false), h !== D && O.setRequestHeader("Range", "bytes=" + L + "-" + X), O.responseType = "arraybuffer", O.overrideMimeType && O.overrideMimeType("text/plain; charset=x-user-defined"), O.send(null), O.status >= 200 && O.status < 300 || O.status === 304 || T("Couldn't load " + t + ". Status: " + O.status), O.response !== void 0 ? new Uint8Array(O.response || []) : Me(O.responseText || "");
            }, W = this;
            W.setDataGetter((L) => {
              var X = L * D, O = (L + 1) * D - 1;
              return O = Math.min(O, h - 1), typeof W.chunks[L] > "u" && (W.chunks[L] = C(X, O)), typeof W.chunks[L] > "u" && T("doXHR failed!"), W.chunks[L];
            }), (N || !h) && (D = h = 1, h = this.getter(0).length, D = h, ue("LazyFiles on gzip forces download of the whole file when length is accessed")), this._length = h, this._chunkSize = D, this.lengthKnown = true;
          }
          get length() {
        ...
        function m(v, _, h, S, P) {
          ...
          var D = Math.min(N.length - P, S);
          if (u(D >= 0), N.slice) for (var C = 0; C < D; C++) _[h + C] = N[P + C];
          else for (var C = 0; C < D; C++) _[h + C] = N.get(P + C);
          return D;
        }
    ...
    function _n(e, r, t, n, o, a, s, c, l, f, m, v, _, h, S, P, N, D) {
      var C = E();
      try {
        w(e)(r, t, n, o, a, s, c, l, f, m, v, _, h, S, P, N, D);
      } catch (W) {
