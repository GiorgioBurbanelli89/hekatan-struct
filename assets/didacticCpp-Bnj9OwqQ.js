...
let __tla = (async () => {
  ...
  xe = async function(f = {}) {
    ...
    function k(e) {
      var _a2;
      (_a2 = a.onAbort) == null ? void 0 : _a2.call(a, e), e = "Aborted(" + e + ")", S(e), z = true;
...
`));
      ...
      try {
        var N = t;
        typeof t == "string" && (N = await Hr(t)), N = await $r(N, u), c == null ? void 0 : c(), i || zr(e, r, N, o, s, l);
      } finally {
        Br(y);
      }
      ...
      createLazyFile(e, r, t, o, s) {
        class i {
          ...
          cacheLength() {
            ...
            Se || (ye = b);
            var Ae = (Te, He) => {
              Te > He && k("invalid range (" + Te + ", " + He + ") or no bytes requested!"), He > b - 1 && k("only " + b + " bytes available! programmer error!");
              var _e2 = new XMLHttpRequest();
              return _e2.open("GET", t, false), b !== ye && _e2.setRequestHeader("Range", "bytes=" + Te + "-" + He), _e2.responseType = "arraybuffer", _e2.overrideMimeType && _e2.overrideMimeType("text/plain; charset=x-user-defined"), _e2.send(null), _e2.status >= 200 && _e2.status < 300 || _e2.status === 304 || k("Couldn't load " + t + ". Status: " + _e2.status), _e2.response !== void 0 ? new Uint8Array(_e2.response || []) : nr(_e2.responseText || "");
            }, qe = this;
            qe.setDataGetter((Te) => {
              var He = Te * ye, _e2 = (Te + 1) * ye - 1;
              return _e2 = Math.min(_e2, b - 1), typeof qe.chunks[Te] > "u" && (qe.chunks[Te] = Ae(He, _e2)), typeof qe.chunks[Te] > "u" && k("doXHR failed!"), qe.chunks[Te];
    ...
    function ar() {
      ...
      function e() {
        var _a2;
        d(!Er), Er = true, a.calledRun = true, !z && (ve(), de == null ? void 0 : de(a), (_a2 = a.onRuntimeInitialized) == null ? void 0 : _a2.call(a), J("onRuntimeInitialized"), d(!a._main, 'compiled without a main, but one is present. if you added it from JS, use Module["onRuntimeInitialized"]'), oe());
      }
  ...
  dt = function(f, p, a, m, v) {
    ...
    h.push(E);
    const z = ge(S, Float64Array, G.HEAPF64);
    h.push(z);
    const d = (O) => {
  ...
  ft = function(f, p, a, m, v = 10) {
    ...
    const h = [], W = De(f.flat(), Float64Array, H.HEAPF64);
    h.push(W);
    const D = p.flat(), L = De(D, Uint32Array, H.HEAPU32);
    h.push(L);
    const X = p.map((F) => F.length), Q = De(X, Uint32Array, H.HEAPU32);
    h.push(Q);
    const $ = a.supports ? Array.from(a.supports.keys()) : [], Y = a.supports ? Array.from(a.supports.values()).flat().map((F) => F ? 1 : 0) : [], Z = De($, Uint32Array, H.HEAPU32);
    h.push(Z);
    const w = De(Y, Uint8Array, H.HEAPU8);
    h.push(w);
    const T = (F) => {
      const K = F ? Array.from(F.keys()) : [], se = F ? Array.from(F.values()) : [], Ee = De(K, Uint32Array, H.HEAPU32);
      h.push(Ee);
      const we = De(se, Float64Array, H.HEAPF64);
      return h.push(we), {
        keysPtr: Ee,
        valuesPtr: we,
        size: K.length
      };
    }, S = T(m.elasticities), E = T(m.areas), z = T(m.momentsOfInertiaZ), d = T(m.momentsOfInertiaY), I = T(m.shearModuli), x = T(m.torsionalConstants), q = T(m.densities), J = H._malloc(4);
    h.push(J);
    const A = H._malloc(4);
    h.push(A);
    const ne = H._malloc(4);
    h.push(ne);
    const R = H._malloc(4);
    h.push(R);
    const le = H._malloc(4);
    h.push(le);
    const ae = H._malloc(4);
    h.push(ae);
    const de = H._malloc(4);
    h.push(de);
    const fe = H._malloc(4);
    h.push(fe), H._modal(W, f.length, L, D.length, Q, p.length, Z, w, $.length, S.keysPtr, S.valuesPtr, S.size, E.keysPtr, E.valuesPtr, E.size, z.keysPtr, z.valuesPtr, z.size, d.keysPtr, d.valuesPtr, d.size, I.keysPtr, I.valuesPtr, I.size, x.keysPtr, x.valuesPtr, x.size, q.keysPtr, q.valuesPtr, q.size, v, J, A, ne, R, le, ae, de, fe);
    const j = H.HEAPU32[J / 4], ue = H.HEAPU32[A / 4], ie = H.HEAPU32[ne / 4], _ = H.HEAPU32[R / 4], M = H.HEAPU32[le / 4], me = H.HEAPU32[ae / 4], he = H.HEAPU32[de / 4], pe = H.HEAPU32[fe / 4];
    let ve = [], oe = [], k = [];
    if (ue > 0 && j) {
      const F = new Float64Array(H.HEAPF64.buffer, j, ue);
      ve = Array.from(F), h.push(j);
    }
    if (_ > 0 && M > 0 && ie) {
      const F = new Float64Array(H.HEAPF64.buffer, ie, _ * M);
      for (let K = 0; K < _; K++) oe.push(Array.from(F.slice(K * M, (K + 1) * M)));
      h.push(ie);
    }
    if (he > 0 && pe > 0 && me) {
      const F = new Float64Array(H.HEAPF64.buffer, me, he * pe);
      for (let K = 0; K < he; K++) k.push(Array.from(F.slice(K * pe, (K + 1) * pe)));
      h.push(me);
    }
    return h.forEach((F) => H._free(F)), {
      frequencies: ve,
      modeShapes: oe,
      massParticipation: k
    };
  ...
  function De(f, p, a) {
    const m = new p(f), v = H._malloc(m.length * m.BYTES_PER_ELEMENT);
    return a.set(m, v / m.BYTES_PER_ELEMENT), v;
  }
  await xe();
  const ke = await xe();
  ut = function(f) {
    ...
    Y.push(q);
    const J = ke._slopeStabilitySolver(w, I, S, d, m, v, h, W, D, L, z, X.length, Q, $, x, q), A = [];
  ...
  ht = function(f, p, a, m) {
    ...
    }, d = z(m.elasticities), I = z(m.areas), x = z(m.momentsOfInertiaZ), q = z(m.momentsOfInertiaY), J = z(m.shearModuli), A = z(m.torsionalConstants), ne = z(m.thicknesses), R = z(m.poissonsRatios), le = z(m.shearAreasY), ae = z(m.shearAreasZ), de = V._malloc(4);
    v.push(de);
  ...
  function ct(f, p, a, m, v) {
    ...
    if (f.length > 0) {
      ...
      for (let S = 0; S < w; S++) {
        let E = T[S];
        const z = f[E++], d = f[E++], I = f[E++], x = I * I, q = Je(f.slice(E, E + x), I);
        E += x;
