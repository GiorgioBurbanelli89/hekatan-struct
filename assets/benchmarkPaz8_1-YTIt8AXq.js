import { b as W, g as B, d as C, P as r, __tla as __tla_0 } from "./pazFrameE2k-BN1k77mc.js";
import { s as A, a as N, b as D, n as $, g } from "./newmarkBeta-CZ1uRmPu.js";
let O;
  ...
  O = {
    ...
    onParamChange(e, i) {
      if (e === "exportE2k" && i.exportE2k > 0.5) {
        const b = h(i), { filename: n, content: c } = B(b, "Paz_8_1");
        C(n, c), console.log(`[Paz 8.1] e2k exportado: ${n}`), i.exportE2k = 0;
      }
    ...
    build(e, i) {
      const b = h(e);
      W(b, i);
      const n = e.E_psi, c = e.I1_in4, T = e.I2_in4, F = e.H1_ft * 12, H = e.H2_ft * 12, y = 12 * n * c * 2 / Math.pow(F, 3), k = 12 * n * T * 2 / Math.pow(H, 3), E = e.W1_lb / 386.088, S = e.W2_lb / 386.088, { K: f, M: _ } = A([
        E,
        S
      ], [
        y,
...
`;
      try {
        const s = N(f, _), d = Math.sqrt(s.omega2[0]), z = Math.sqrt(s.omega2[1]), w = D(_, f, d, z, e.xi), I = Math.floor(e.tEnd_s / e.dt_s), o = $({
          M: _,
          K: f,
          C: w,
          loadFunc: u,
          ...
          gamma: 0.5,
          beta: 0.25
        }), v = Math.max(...o.u.map((a) => Math.abs(a[0]))), L = Math.max(...o.u.map((a) => Math.abs(a[1])));
        if (m += `  Modal: f1=${s.freqs[0].toFixed(3)}, f2=${s.freqs[1].toFixed(3)} Hz
`, m += `  Newmark-\u03B2: u1_max=${v.toFixed(4)} in (libro \u22480.70)
`, m += `             u2_max=${L.toFixed(4)} in (libro \u22480.92)
`, e.showTH > 0.5) {
