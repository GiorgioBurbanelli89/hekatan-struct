function ht() {
  ...
  function T(M, C) {
    ...
    const v = ["Ux", "Uy", "Uz", "Rx", "Ry", "Rz"], U = [0, 0, 0, 0, 0, 0], E = M.frequencies.length;
    ...
    for (let s = 0; s < E; s++) {
      ...
      F < 0 && r[0] > h && (F = s + 1), B < 0 && r[1] > h && (B = s + 1), G < 0 && r[5] > h && (G = s + 1);
    }
...
<tr style="color:#ff0; border-bottom:1px solid #ff03">
  ...
  <th style="padding:2px 6px">\u03C9 (rad/s)</th>`;
    for (const s of v) t += `<th style="padding:2px 5px">${s}</th>`;
  ...
  <th style="padding:2px 5px; color:#fff">Tipo</th></tr>`;
    ...
    if (M.frequencies.forEach((s, r) => {
      ...
      const p = a < 0.05 ? "\u2014" : `${v[R]} (${(a * 100).toFixed(0)} %)`, m = R === 0 || R === 1 ? "#0f0" : R === 5 ? "#0ff" : R === 2 ? "#fa0" : "#888", P = r + 1 === n, Y = r + 1 === x, L = r + 1 === z;
...
<div style="margin-top:6px; font-size:10px; color:#888;">
  \u2605 = primer modo donde \u03A3Ux y \u03A3Uy \u2265 90 %  \xB7  Tipos: <span style="color:#0f0">Ux/Uy</span>=lateral \xB7 <span style="color:#0ff">Rz</span>=torsional \xB7 <span style="color:#fa0">Uz</span>=vertical (no relevante para sismo)
    ...
    (_c = g.querySelector("#modal-minimize")) == null ? void 0 : _c.addEventListener("click", () => {
      ...
      const r = `Mode  Freq(Hz)  Period(s)  \u03C9(rad/s)  ${v.join("     ")}  \u03A3Ux    \u03A3Uy    \u03A3Rz   Tipo`;
      ...
      M.frequencies.forEach((y, R) => {
        ...
        const L = Y < 0.05 ? "\u2014" : `${v[P]} (${(Y * 100).toFixed(0)}%)`, X = m.map((b) => ((b * 100).toFixed(1) + "%").padStart(6)).join(" ");
...
function St(g) {
  var _a;
  const N = g.split(/\r?\n/), u = { force: "TONF", length: "M" }, T = [], M = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), U = [], E = [], n = /* @__PURE__ */ new Map(), x = /* @__PURE__ */ new Map(), z = [], w = [];
  let H = "", F = "";
  ...
  for (const d of N) {
    ...
    if (F === "POINT COORDINATES") {
      ...
      i && v.set(i[1], [parseFloat(i[2]), parseFloat(i[3])]);
    }
  ...
  if (T.length > 0) {
    const d = T.length - 1;
    G.set(T[d].name, T[d].elev);
    for (let A = d - 1; A >= 0; A--) {
      const I = G.get(T[A + 1].name) + T[A].height;
      T[A].elev = I, G.set(T[A].name, I);
    }
  }
  ...
  for (const d of s) {
    const [A, i] = d.split("@"), I = v.get(A), o = G.get(i);
...
function lt(g) {
  ...
  const C = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), n = [], x = [], z = /* @__PURE__ */ new Map(), w = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), F = [];
  let B = "";
  for (const G of N) {
    const h = G.trim();
    ...
    switch (B) {
      ...
      case "FRAME SECTION PROPERTIES 01 - GENERAL": {
        ...
        e && v.set(e, { material: f.get("Material") || "", shape: f.get("Shape") || "Rectangular", D: D(f.get("t3")), B: D(f.get("t2")), TF: D(f.get("tf")), TW: D(f.get("tw")), A: D(f.get("Area")), Iz: D(f.get("I33")), Iy: D(f.get("I22")), J: D(f.get("TorsConst")) });
        break;
      }
  ...
  return rt(T, M, C, v, U, E, n, x, z, w, H, F);
}
function pt(g) {
  const N = { force: "KN", length: "m" };
  let u = "UX,UY,UZ,RX,RY,RZ";
  const T = /* @__PURE__ */ new Map(), M = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), U = [], E = [], n = /* @__PURE__ */ new Map(), x = [];
  let z = "", w = "";
  for (const B of g) {
    const G = B.trim();
    ...
    switch (z) {
      ...
      case "JOINT": {
        const e = f[0];
        v.set(e, [D(h.get("X")), D(h.get("Y")), D(h.get("Z"))]);
        break;
      }
      case "RESTRAINT": {
  ...
  return rt(N, u, T, M, C, v, U, E, n, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), x);
}
function rt(g, N, u, T, M, C, v, U, E, n, x, z) {
  var _a;
  const w = [], H = /* @__PURE__ */ new Map(), F = [];
  for (const [c, y] of C) H.set(c, F.length), w.push(c), F.push(y);
  const B = [], G = [], h = /* @__PURE__ */ new Map();
  for (const c of v) {
    const y = H.get(c.j1), R = H.get(c.j2);
    if (y !== void 0 && R !== void 0) {
      const a = B.length;
      B.push([y, R]), G.push(c.name);
  ...
  return { units: g, dof: N, materials: u, frameSections: T, shellSections: M, nodes: F, nodeNames: w, nodeNameToIdx: H, elements: B, elementNames: G, elementSections: h, nodeInputs: O, elementInputs: t, sectionShapes: s, info: { nNodes: F.length, nFrames: f, nShells: e, title: `SAP2000 (${f} frames, ${e} shells)` } };
}
function At(g) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k;
  const { nodes: N, elements: u, nodeInputs: T, elementInputs: M } = g, C = g.units || { force: "KN", length: "m" }, v = g.title || "Awatif Model", U = [], E = (e) => U.push(e), n = () => U.push(" ");
  E(`File ${v}.$2k was saved on m/d/yy at h:mm:ss`), n(), E('TABLE:  "ACTIVE DEGREES OF FREEDOM"'), E("   UX=Yes   UY=Yes   UZ=Yes   RX=Yes   RY=Yes   RZ=Yes"), n();
  const x = [], z = [];
  if (u.forEach((e, t) => {
  ...
  const F = !!g.layeredSection && z.length > 0, B = g.layeredSection, G = /* @__PURE__ */ new Map(), h = /* @__PURE__ */ new Map();
  if (!F) for (const e of z) {
    ...
    G.has(O) || G.set(O, { t, matKey: r });
    const c = [...G.keys()].indexOf(O) + 1;
    h.set(e, `SSEC${c}`);
  }
  if (z.length > 0) {
    ...
    if (n(), E('TABLE:  "AREA SECTION PROPERTIES"'), F) {
      ...
      let e = 0;
      for (const [, t] of G) e++, E(`   Section=SSEC${e}   Material=${t.matKey}   MatAngle=0   AreaType=Shell   Type=ShellThin   DrillDOF=Yes   Thickness=${J(t.t)}   BendThick=${J(t.t)}   Color=Cyan`);
    }
    if (n(), F) {
...
function Tt(g) {
  const { nodes: N, elements: u, nodeInputs: T, elementInputs: M, title: C, e2kModel: v } = g, U = v == null ? void 0 : v.rawSections;
...
function dt(g) {
  var _a, _b, _c, _d;
  const { nodes: N, elements: u, nodeInputs: T, elementInputs: M, title: C, units: v } = g, U = (v == null ? void 0 : v.force) || "TONF", E = (v == null ? void 0 : v.length) || "M", n = [], x = (a) => Math.round(a * 1e4) / 1e4;
  ...
  (_a = M.elasticities) == null ? void 0 : _a.forEach((a) => G.add(a));
...
function Et(g, N) {
  const u = g[N[0]], T = g[N[1]], M = Math.abs(T[2] - u[2]), C = Math.sqrt((T[0] - u[0]) ** 2 + (T[1] - u[1]) ** 2), v = M > C * 0.5;
