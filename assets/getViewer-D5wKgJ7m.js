...
function ss(e, i, y) {
  ...
  const x = () => {
    ...
    let K = false, ae = 0, me = 0, de = 0, k = 0;
    v.addEventListener("mousedown", (H) => {
      K = true, ae = H.clientX, me = H.clientY;
      const ue = h.getBoundingClientRect();
      de = ue.left, k = ue.top, h.style.left = `${de}px`, h.style.top = `${k}px`;
    }), window.addEventListener("mousemove", (H) => {
      if (!K) return;
      const ue = H.clientX - ae, oe = H.clientY - me, ge = Math.max(0, Math.min(window.innerWidth - 40, de + ue)), q = Math.max(0, Math.min(window.innerHeight - 40, k + oe));
...
function is(e, i, y) {
  ...
  return Ao((S, g) => {
    ...
    if (S.length >= 2) {
      ...
      for (const P of S) for (let v = 0; v < 3; v++) w[v] = Math.min(w[v], P[v]), _[v] = Math.max(_[v], P[v]);
...
function Qn(e, i) {
  ...
  const _ = new qt(y.grid), P = new qt(y.grid).multiplyScalar(0.45), v = (ae, me, de, k) => {
    ...
    const q = new ht({ color: de, transparent: true, opacity: k, depthWrite: false }), U = new Qt(ge, q);
    return U.name = `grid-${ae}-${me === g ? "minor" : "major"}`, U;
  }, K = (ae, me, de) => {
    const k = ae === "xy" ? (U, X) => [U, X, 0] : ae === "xz" ? (U, X) => [U, 0, X] : (U, X) => [0, U, X], H = [[-x, -x], [x, -x], [x, x], [-x, x]], ue = [];
    for (const [U, X] of H) ue.push(...k(U, X));
    const oe = new he();
  ...
  for (const ae of c) h.add(v(ae, g, P, 0.12)), h.add(v(ae, S, _, 0.4)), h.add(K(ae, _, 0.55));
  return h.position.set(0, 0, 0), window.__hekatanGridConfig = { majorStep: S, minorStep: g, gridSize: e, planes: [...c] }, h;
}
function ls(e, i, y, h) {
  ...
  const x = new nt({ color: 10166822 }), w = new nt({ color: 2792847 }), _ = new nt({ color: 3835647 }), P = () => {
    ...
    for (const k of ae) for (let H = 0; H < 3; H++) k[H] < me[H] && (me[H] = k[H]), k[H] > de[H] && (de[H] = k[H]);
  ...
  return $.derive(() => {
    ...
    (_b = (_a = e.nodeInputs) == null ? void 0 : _a.val.supports) == null ? void 0 : _b.forEach((me, de) => {
      ...
      let ge;
      ue >= 3 && oe >= 3 ? ge = new Qe(S, x) : ue >= 3 && oe === 0 ? ge = new Qe(g, w) : ge = new Qe(g, _), ge.position.set(k[0], k[1], k[2]);
      const q = ae * K();
...
function cs(e, i, y) {
  ...
  return $.derive(() => {
    ...
    i.val.forEach((S, g) => {
      const x = new At(`${g}`);
      x.position.set(...S), x.updateScale(c * y.rawVal), h.add(x);
    });
  }), $.derive(() => {
    if (y.val, !e.nodesIndexes.rawVal) return;
    const c = 0.05 * e.gridSize.val * 0.6;
    h.children.forEach((S) => S.updateScale(c * y.rawVal));
  }), $.derive(() => {
...
function ds(e, i, y, h) {
  ...
  return $.derive(() => {
    ...
    (_a = e.elements) == null ? void 0 : _a.val.forEach((g, x) => {
      ...
      w.position.set(...ps(g.map((_) => y.rawVal[_]))), w.updateScale(S * h.rawVal), c.add(w);
    });
  }), $.derive(() => {
    if (h.val, !i.elementsIndexes.rawVal) return;
    const S = 0.05 * i.gridSize.val * 0.6;
    c.children.forEach((g) => g.updateScale(S * h.rawVal));
  }), $.derive(() => {
...
function So(e, i) {
  const y = new tt(), h = 0.05 * e * 1, c = cn(), S = new At("X", "red", "transparent"), g = new At(i ? "Z" : "Y", "green", "transparent"), x = new At(i ? "Y" : "Z", "blue", "transparent"), w = new rn(new m(1, 0, 0), new m(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), _ = new rn(new m(0, 1, 0), new m(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), P = new rn(new m(0, 0, 1), new m(0, 0, 0), 1, c.axisArrow, 0.2, 0.2);
  return S.position.set(1.3 * h, 0, 0), g.position.set(0, 1.3 * h, 0), x.position.set(0, 0, 1.3 * h), S.updateScale(0.4 * h), g.updateScale(0.4 * h), x.updateScale(0.4 * h), w.scale.set(h, h, h), _.scale.set(h, h, h), P.scale.set(h, h, h), y.add(w, _, P, S, g, x), y;
}
function no(e, i) {
  const y = new m(...e), c = new m(...i).clone().sub(y), S = c.length(), g = c.dot(new m(1, 0, 0)) / S, x = c.dot(new m(0, 1, 0)) / S, w = c.dot(new m(0, 0, 1)) / S, _ = Math.sqrt(g ** 2 + x ** 2);
...
function us(e, i, y) {
  const h = zn([i, y]), c = zn([e, y]), S = zn([e, i]), g = new m(...h).sub(new m(...c)).normalize(), x = new m(...y).sub(new m(...S)).normalize(), w = g.clone().cross(x).normalize(), _ = w.clone().cross(g).normalize();
  return new Vo().makeBasis(g, _, w);
}
function fs(e, i, y, h) {
  const c = new tt(), S = new he(), g = new ht({ vertexColors: true }), x = [0, 0, 0], w = [1, 0, 0], _ = [0, 1, 0], P = [0, 0, 1];
  S.setAttribute("position", new $t([...x, ...w, ...x, ..._, ...x, ...P], 3));
  const v = [255, 0, 0], K = [0, 255, 0], ae = [0, 0, 255];
  return S.setAttribute("color", new $t([...v, ...v, ...K, ...K, ...ae, ...ae], 3)), $.derive(() => {
    var _a;
    i.deformedShape.val, i.orientations.val && (c.clear(), (_a = e.elements) == null ? void 0 : _a.val.forEach((me) => {
      const de = new Qt(S, g), k = y.rawVal[me[0]], H = y.rawVal[me[1]];
      if (me.length === 2 && (de.position.set(...eo(k, H)), de.rotation.setFromRotationMatrix(no(k, H))), me.length === 3) {
        const ge = y.rawVal[me[2]];
        de.position.set(...zn([k, H, ge])), de.rotation.setFromRotationMatrix(us(k, H, ge));
      }
    ...
    c.children.forEach((k) => k.scale.set(de, de, de));
  }), $.derive(() => {
  ...
  return $.derive(() => {
    ...
    L.forEach((z, D) => {
      ...
      if (E.type === "CFT") {
        ...
        switch (E.type) {
          ...
          case "T":
            ...
            break;
          case "pipe":
            G = k(E.d, E.tw ?? E.d * 0.05), Ce = oe, ve = ge;
            break;
          default:
            return;
        }
      ...
      if (ie) {
        ...
        ve.updateScale(Me * ((h == null ? void 0 : h.rawVal) ?? 1)), S.add(ve);
      }
    });
  }), h && $.derive(() => {
...
class $n extends tt {
  constructor(i, y, h, c, S, g, x) {
    ...
    this.mesh = new Qe(v, K), this.mesh.position.set(...i), this.mesh.rotation.setFromRotationMatrix(c), x && this.mesh.rotateX(Math.PI / 2), this.add(this.mesh), this.text = new At(`${S[1].toFixed(2)}`), this.normalizedResult = g, this.textPosition = zn([i, y]), this.text.position.set(...this.textPosition), this.text.rotation.setFromRotationMatrix(c), this.add(this.text);
  }
  updateScale(i) {
...
class ko extends tt {
  constructor(i, y, h, c, S, g, x) {
    super();
    const w = S[0] * h / (S[0] + S[1]), _ = S[0] * S[1] > 0;
    if (this.text = new At(`${S[0].toFixed(2)}`), this.text2 = new At(`${(S[1] * -1).toFixed(2)}`), this.normalizedResult = g, this.textPosition = eo(i, y), this.text2Position = eo(y, i), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(c), this.text2.rotation.setFromRotationMatrix(c), this.add(this.text, this.text2), _) {
      const P = new Yn().moveTo(0, 0).lineTo(0, g[0]).lineTo(w, 0).lineTo(0, 0), v = new Yn().moveTo(w, 0).lineTo(h, -g[1]).lineTo(h, 0).lineTo(w, 0), K = P.getPoints(), ae = v.getPoints(), me = new he().setFromPoints(K), de = new he().setFromPoints(ae), k = new ht({ color: cn().resultOutline });
      this.lines = new It(me, k), this.lines2 = new It(de, k), this.lines.position.set(...i), this.lines2.position.set(...i), this.lines.rotation.setFromRotationMatrix(c), this.lines2.rotation.setFromRotationMatrix(c), x && this.lines.rotateX(Math.PI / 2), x && this.lines2.rotateX(Math.PI / 2), this.add(this.lines, this.lines2);
...
function ws(e, i, y, h) {
  ...
  return $.derive(() => {
    ...
    (_b = (_a = e.analyzeOutputs) == null ? void 0 : _a.rawVal[w]) == null ? void 0 : _b.forEach((_, P) => {
      var _a2, _b2;
      const v = ((_a2 = e.elements) == null ? void 0 : _a2.rawVal[P]) ?? [0, 1], K = y.rawVal[v[0]], ae = y.rawVal[v[1]], me = new m(...ae).distanceTo(new m(...K)), de = ys((_b2 = e.analyzeOutputs) == null ? void 0 : _b2.rawVal[w]), k = _ == null ? void 0 : _.map((ge) => ge / (de === 0 ? 1 : de)), H = no(K, ae), ue = new x[w](K, ae, me, H, _ ?? [0, 0], k ?? [0, 0], !!["normals", "shearsZ", "torsions", "bendingsY"].includes(w)), oe = g();
...
function gs(e, i, y, h) {
  ...
  return $.derive(() => {
    ...
    (_b = (_a = e.deformOutputs) == null ? void 0 : _a.val[S]) == null ? void 0 : _b.forEach((x, w) => {
      const _ = new xs(y.rawVal[w], S, x ?? [0, 0, 0, 0, 0, 0]);
...
function vs({ drawingObj: e, gridObj: i, scene: y, getActiveCamera: h, controls: c, gridSize: S, derivedDisplayScale: g, rendererElm: x, viewerRender: w }) {
  ...
  let k = false;
  const H = () => {
    if (k) return _.intersectObjects([K], false);
  ...
  Ae.add(Ct, ct, I);
  const te = (n) => {
    const o = new he().setFromPoints([new m(0, 0, 0), new m(0, 0, 0), new m(0, 0, 0), new m(0, 0, 0)]), a = new ht({ color: n, transparent: true, opacity: 0.45, depthTest: false }), t = new Eo(o, a);
  ...
  zt.id = "hk-refplane-badge", zt.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(zt), window.__hekatanSetOrthoPlanes = (n) => {
    ...
    if (window.__hekatanShowOrthoPlanes = n, Ye.visible = n, n) {
      ...
      N(le, s, "xy", l), N(pe, s, "xz", l), N(Te, s, "yz", l), Bt(De, s, "xy", l), Bt(wt, s, "xz", l), Bt(dt, s, "yz", l), De.material.opacity = 0.1, wt.material.opacity = 0.1, dt.material.opacity = 0.1;
    } else {
  ...
  const Ot = (n) => {
    if (De.material.opacity = n === "xy" ? 0.14 : 0.04, wt.material.opacity = n === "xz" ? 0.14 : 0.04, dt.material.opacity = n === "yz" ? 0.14 : 0.04, n) {
  ...
  window.__hekatanFinalizePolyArea = Ge, window.__hekatanSetInclinedPlaneFrom3 = (n, o, a) => {
    ...
    e.gridTarget && (e.gridTarget.val = { position: [0, 0, 0], rotation: [0, 0, 0] }), G.visible = false, w();
  };
  const Oe = new tt();
...
function bs(e, i, y) {
  const S = Math.round(14.999999999999998), g = { position: e.position.clone(), quaternion: e.quaternion.clone() }, x = setInterval(_, 1e3 / 30);
  let w = 0;
  function _() {
    w++;
    const P = w / S;
...
class Lo {
  ...
  setColorMap(i, y = 32) {
    ...
    const h = 1 / this.n, c = new qt(), S = new qt();
    ...
    for (let g = 1; g < y; g++) {
      ...
      for (let w = 0; w < this.map.length - 1; w++) if (x > this.map[w][0] && x <= this.map[w + 1][0]) {
        ...
        const v = new qt().lerpColors(c, S, (x - _) / (P - _));
        this.lut.push(v);
      }
    }
  ...
  updateCanvas(i) {
    ...
    for (let P = 1; P >= 0; P -= g) for (let v = this.map.length - 1; v >= 0; v--) if (P < this.map[v][0] && P >= this.map[v - 1][0]) {
      ...
      x.setHex(this.map[v - 1][1], Ln), w.setHex(this.map[v][1], Ln), _.lerpColors(x, w, (P - K) / (ae - K)), c[S * 4] = Math.round(_.r * 255), c[S * 4 + 1] = Math.round(_.g * 255), c[S * 4 + 2] = Math.round(_.b * 255), c[S * 4 + 3] = 255, S += 1;
    }
...
function Ms(e) {
  ...
  for (let y = 0; y < Cn.length - 1; y++) {
    const [h, c, S, g] = Cn[y], [x, w, _, P] = Cn[y + 1];
    if (e <= x) {
      const v = (e - h) / (x - h);
      return [c + (w - c) * v, S + (_ - S) * v, g + (P - g) * v];
    }
  }
...
function _s() {
  ...
  for (let h = 0; h < 256; h++) {
    const c = h / 255, [S, g, x] = Ms(c);
    i[h * 4 + 0] = S, i[h * 4 + 1] = g, i[h * 4 + 2] = x, i[h * 4 + 3] = 255;
  }
...
function Ss(e, i, y) {
  ...
  const h = _s(), c = new Wo({ uniforms: { cmap: { value: h }, ambient: { value: 0.95 } }, vertexShader: `
    ...
    `, side: Yt, transparent: false, clipping: true, depthWrite: true, depthTest: true }), S = new Qe(new he(), c);
  return S.renderOrder = -1, S.frustumCulled = false, S.userData.isShellArea = true, S.name = "__hekatan_shell_colormap", $.derive(() => {
    ...
    for (let k = 0; k < y.val.length; k++) {
      ...
      de[k] = Math.max(0, Math.min(1, oe));
    }
    S.geometry.setAttribute("scalar", new rt(de, 1));
  }), S;
}
function ks(e, i, y, h) {
...
function As(e, i, y, h) {
  ...
  return $.derive(() => {
    ...
    const de = Math.min(...ae), k = Math.max(...ae);
    S.setMin(de), S.setMax(k), x.val = ae;
    ...
    for (const { idx: z, vals: D } of me) {
      ...
      for (let Me = 0; Me < Ce; Me++) {
        const Le = Me / jn, Ae = O[0] + E.x * Y * Le, Pt = O[1] + E.y * Y * Le, Ct = O[2] + E.z * Y * Le, ct = D[0] + (D[1] - D[0]) * Le, I = S.getColor(ct) ?? new qt(0, 0, 0);
...
function Bs(e) {
  ...
  const S = new he(), g = new ht({ color: Po, linewidth: 4, transparent: true, opacity: 0.9, depthTest: false }), x = new Qt(S, g);
...
function Xs(e, i, y, h, c, S) {
  const g = c - y, x = S - h, w = g * g + x * x;
  if (w < 1e-9) {
    const me = e - y, de = i - h;
...
function Ys(e, i, y) {
  ...
  for (let c = 0, S = y.length - 1; c < y.length; S = c++) {
    const g = y[c].x, x = y[c].y, w = y[S].x, _ = y[S].y;
...
function Co(e, i = 8) {
  ...
  let S, g;
  c.forEach((w, _) => {
    S = document.createElement("div"), S.id = `marker-${_}`, S.className = "marker", S.style.marginTop = _ == 0 ? "0px" : `calc(${50 / i}vh - 1px)`, g = document.createElement("p"), g.id = `marker-text-${_}`, S.append(g), y.append(S);
  });
  const x = [];
...
function Os({ mesh: e, settingsObj: i, drawingObj: y, objects3D: h, solids: c }) {
  ...
  const k = as(i), H = $.derive(() => k.displayScale.val === 0 ? 1 : k.displayScale.val > 0 ? k.displayScale.val : -1 / k.displayScale.val), ue = Ns(e, k), oe = () => {
    const I = [];
    return k.gridXY.rawVal && I.push("xy"), k.gridXZ.rawVal && I.push("xz"), k.gridYZ.rawVal && I.push("yz"), I;
  }, ge = () => {
    const I = k.gridStep.rawVal, te = Math.max(I, k.gridMajor.rawVal);
  ...
  const U = () => {
    const I = Math.max(0, Math.min(1, k.gridOpacity.rawVal));
    q.traverse((te) => {
  ...
  U(), S.appendChild(ss(k, e, c)), S.setAttribute("id", "viewer"), S.appendChild(P.domElement), P.setPixelRatio(window.devicePixelRatio);
  const X = cn();
  P.setClearColor(X.background, 1);
  const L = k.gridSize.rawVal, A = L * 0.5 + L * 0.5 / Math.tan(45 * 0.5);
  ...
  let z = So(k.gridSize.rawVal, k.flipAxes.rawVal);
  g.add(q, z), $.derive(() => {
  ...
  $.derive(() => {
    ...
    const I = k.cursorSnap.val;
    window.__hekatanSnap2D = I;
  });
  let O = true;
  $.derive(() => {
    var _a;
    const I = k.gridSize.val, te = k.flipAxes.val;
    ...
    }), q = Qn(I, ge()), q.visible = k.gridVisible.rawVal, g.add(q), U(), g.remove(z), z.traverse((Te) => {
      var _a2, _b, _c, _d;
      (_b = (_a2 = Te.geometry) == null ? void 0 : _a2.dispose) == null ? void 0 : _b.call(_a2), (_d = (_c = Te.material) == null ? void 0 : _c.dispose) == null ? void 0 : _d.call(_c);
    }), z = So(I, te), g.add(z);
  ...
  function ve(I, te) {
    E = I, te && (Y = te);
    const le = S.clientWidth || 1, pe = S.clientHeight || 1, Ye = (I ? le / 2 : le) / pe;
    ...
    S.__splitMode = I, window.__hekatanSplitMode = I, window.__hekatanSplitCamera = I ? Y : null, G();
  }
  if (e) {
    g.add(is(k, ue, H), os(e, k, ue), cs(k, ue, H), ds(e, k, ue, H), ls(e, k, ue, H), rs(e, k, ue, H), fs(e, k, ue, H), ms(e, k, ue, H), gs(e, k, ue, H), ws(e, k, ue, H));
    const I = Bs({ scene: g, rendererElm: P.domElement, getActiveCamera: () => _, derivedNodes: ue, derivedDisplayScale: H, mesh: e, settings: k, render: G });
    g.add(I);
    const te = Gs(e, k), le = ks(e, k, ue, te), pe = Co(te);
    g.add(le), S.appendChild(pe);
    const Te = As(e, k, ue);
    g.add(Te);
    const Ye = Te.__colorMapValues, mt = Co(Ye);
    mt.id = "frame-legend", S.appendChild(mt), $.derive(() => {
      var _a;
      const De = k.shellResults.val != "none", wt = (((_a = k.solidResults) == null ? void 0 : _a.val) ?? "none") !== "none", dt = De || wt, Bt = k.frameResults.val.startsWith("contour:");
  ...
  if (c) {
    ...
    pe.color.setHSL(11, 43, 96), pe.position.set(-10, 0, 30), g.add(pe), $.derive(() => {
      ...
      c.rawVal.forEach((Te) => Te.visible = k.solids.val), G();
    });
  }
  if (h) {
    const I = [], te = (pe) => {
      var _a, _b;
      return ((_a = pe == null ? void 0 : pe.userData) == null ? void 0 : _a.isCota) ? k.showCotas.val : ((_b = pe == null ? void 0 : pe.userData) == null ? void 0 : _b.isDistLoad) ? k.loads.val : k.custom3D.val;
    }, le = () => {
  ...
  y && vs({ drawingObj: y, gridObj: q, scene: g, getActiveCamera: () => _, controls: v, gridSize: L, derivedDisplayScale: H, rendererElm: P.domElement, viewerRender: G }), Ao((I, te) => {
    ...
    }), q = Qn(k.gridSize.rawVal, { planes: oe() }), g.add(q), S.style.setProperty("--awatif-legend-color", te.legendMarker), G();
  });
  ...
  return Le.append(ct()), Le.append(Ae("\u2191", "Pan arriba", () => Pt(0, 1))), Le.append(Ae("\u2295", "Zoom in", () => Ct(0.85))), Le.append(Ae("\u2190", "Pan izquierda", () => Pt(-1, 0))), Le.append(Ae("\u2302", "Reset vista", () => {
    v.reset(), G();
  })), Le.append(Ae("\u2192", "Pan derecha", () => Pt(1, 0))), Le.append(Ae("\u2296", "Zoom out", () => Ct(1.18))), Le.append(Ae("\u2193", "Pan abajo", () => Pt(0, -1))), Le.append(ct()), getComputedStyle(S).position === "static" && (S.style.position = "relative"), S.appendChild(Le), S;
}
function Ns(e, i) {
  return $.derive(() => {
    var _a, _b, _c, _d;
