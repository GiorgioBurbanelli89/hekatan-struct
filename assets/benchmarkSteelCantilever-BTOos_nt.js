...
import { a as m, b as d, c as b } from "./units-yggW8zr_.js";
...
let __tla = Promise.all([
  ...
  P = {
    ...
    computedLabels(e) {
      const o = e.D_out, t = e.t_HSS, a = o - 2 * t, l = o * o - a * a, r = (o ** 4 - a ** 4) / 12, n = e.gamma_s * l, s = n * e.L, i = n * e.L * e.L / (2 * e.E_s * l) * 1e3;
      return {
    ...
    build(e, o) {
      ...
      if (r) {
        const s = r[2], i = r[0], _ = -t.q * a * a / (2 * t.E_col * t.A_eq), A = (s - _) / Math.abs(_) * 100;
  ...
  Uz_top   = ${m(s, 5)}  (anal\xEDtico ${m(_, 5)}, \u0394 ${A.toFixed(3)}%)
`;
        if (Math.abs(e.P_lat) > 1e-9) {
          ...
          c += `  Ux_top = ${m(i, 4)} (lateral, P=${d(e.P_lat)})
    Anal\xEDtico Timoshenko: ${m(p, 4)} (bending ${m(u, 4)} + shear ${m(f, 4)})
  Esperado en base: M = P\xB7L = ${b(M)},  V = P = ${d($)}
`;
        }
        n && (c += `  Reacci\xF3n base: Fz=${d(n[2], 3)}  My=${b(n[4], 3)}  (W=${d(t.q * a, 3)} esperado)`), console.log(c);
      }
    },
    runModal(e, o, t) {
