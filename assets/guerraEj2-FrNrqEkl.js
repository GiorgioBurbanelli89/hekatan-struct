...
let __tla = Promise.all([
  ...
  re = {
    ...
    build(t, s) {
      ...
      const $ = d * b, G = k * b, w = t.ks_tm3 * b, E = 14100 * Math.sqrt(t.fc_kgcm2) * 98.0665, N = 0.2, f = [];
      ...
      for (let e = 0; e < M; ++e) for (let n = 0; n < c; ++n) {
        const o = n === 0 || n === c - 1, _ = e === 0 || e === M - 1, S = o && _ ? 0.25 : o || _ ? 0.5 : 1, X = x * p * S, L = e * c + n;
        if (g.push({
          node: L,
          dof: 0,
          k: w * X
      ...
      u.forEach((e, n) => {
        I.set(n, e.map((S) => -Math.abs(w * D.nodeResults[S].w)));
