import { K as It, a5 as bn, p as Go, v as $, a6 as Wo, D as zt, d as We, B as fe, F as Mt, a7 as qo, w as at, a8 as Jo, a9 as Qo, g as ho, aa as mo, q as jt, ab as zn, ac as Pn, a3 as Fo, Z as je, L as it, c as Dt, u as Vo, b as Oo, ad as jo, e as et, V as m, _ as Ot, ae as Zn, G as Ao, a as vt, y as Cn, af as Fn, s as es, n as ts, H as Wt, a1 as hn, E as wo, S as rn, l as Un, ag as mn, f as yo, h as xo, i as go, C as vo, J as ns, N as os, U as ss, W as as, T as _n, P as Kn, X as is, Y as Mo, O as ls } from "./theme-DDCjfe25.js";
import { T as wt, O as bo } from "./Text-DAMC3bLh.js";
import { P as To } from "./tweakpane-BXg6ZhiP.js";
import { e as rs } from "./styles-B0wUJdk4.js";
class Eo {
  constructor(i, w = 32) {
  ...
  setColorMap(i, w = 32) {
    this.map = Hn[i] || Hn.rainbow, this.n = w;
    ...
    for (let x = 1; x < w; x++) {
      ...
      for (let v = 0; v < this.map.length - 1; v++) if (y > this.map[v][0] && y <= this.map[v + 1][0]) {
        ...
        const b = new It().lerpColors(d, _, (y - z) / (P - z));
        this.lut.push(b);
      }
    }
  ...
  updateCanvas(i) {
    const w = i.getContext("2d", { alpha: false }), h = w.getImageData(0, 0, 1, this.n), d = h.data;
    let _ = 0;
    ...
    for (let P = 1; P >= 0; P -= x) for (let b = this.map.length - 1; b >= 0; b--) if (P < this.map[b][0] && P >= this.map[b - 1][0]) {
      ...
      y.setHex(this.map[b - 1][1], bn), v.setHex(this.map[b][1], bn), z.lerpColors(y, v, (P - O) / (ae - O)), d[_ * 4] = Math.round(z.r * 255), d[_ * 4 + 1] = Math.round(z.g * 255), d[_ * 4 + 2] = Math.round(z.b * 255), d[_ * 4 + 3] = 255, _ += 1;
    }
...
function Io(e) {
  ...
  for (let h = 0; h < i.length - 1; h++) {
    const [d, _, x, y] = i[h], [v, z, P, b] = i[h + 1];
    if (e <= v) {
      const O = (e - d) / (v - d);
      return [_ + (z - _) * O, x + (P - x) * O, y + (b - y) * O];
    }
  }
...
function ds() {
  const i = [];
  for (let w = 0; w <= 12; w++) {
    const h = 1 - w / 12, [d, _, x] = Io(h);
    i.push(`rgb(${d | 0},${_ | 0},${x | 0}) ${(w / 12 * 100).toFixed(0)}%`);
  }
...
function ps(e, i, w) {
  ...
  return _.renderOrder = -1, _.frustumCulled = false, _.userData.isShellArea = true, _.name = "__hekatan_shell_colormap", $.derive(() => {
    ...
    let v, z;
    const P = jn.val;
    if (P ? (z = P[0], v = P[1]) : (v = y.length ? Math.max(...y) : 1, z = y.length ? Math.min(...y) : 0, z >= 0 && v > 0 && (z = 0)), v === z) {
      const k = Math.max(Math.abs(v) * 1e-6, 1e-9);
      v += k, z -= k;
    }
    const b = P && P[0] > P[1], O = Math.min(z, v), ae = Math.max(z, v), ie = ae - O, ue = new Float32Array(w.val.length);
...
function us(e, i, w) {
  ...
  w && d.addBinding(e.solids, "val", { label: "Solids" });
...
function Gn(e, i) {
  ...
  const z = new It(w.grid), P = new It(w.grid).multiplyScalar(0.45), b = (ae, ie, ue, k) => {
    const G = [], he = ae === "xy" ? (D, I) => [D, I, 0] : ae === "xz" ? (D, I) => [D, 0, I] : (D, I) => [0, D, I], ge = Math.floor(y / ie);
    for (let D = -ge; D <= ge; D++) {
      const I = D * ie, A = he(I, -y), E = he(I, y);
      G.push(...A, ...E);
    }
    for (let D = -ge; D <= ge; D++) {
      const I = D * ie, A = he(-y, I), E = he(y, I);
      G.push(...A, ...E);
    }
    const Pe = new fe();
    ...
    const k = ae === "xy" ? (Z, D) => [Z, D, 0] : ae === "xz" ? (Z, D) => [Z, 0, D] : (Z, D) => [0, Z, D], G = [[-y, -y], [y, -y], [y, y], [-y, y]], he = [];
...
function ms(e, i, w, h) {
  ...
  const y = new et({ color: 10166822 }), v = new et({ color: 2792847 }), z = new et({ color: 3835647 }), P = () => {
    const ae = w.rawVal ?? [];
  ...
  return $.derive(() => {
    ...
    (_b = (_a = e.nodeInputs) == null ? void 0 : _a.val.supports) == null ? void 0 : _b.forEach((ie, ue) => {
      ...
      let Pe;
      he >= 3 && ge >= 3 ? Pe = new We(_, y) : he >= 3 && ge === 0 ? Pe = new We(x, v) : Pe = new We(x, z), Pe.position.set(k[0], k[1], k[2]);
      const K = ae * O();
...
function ws(e, i, w, h) {
  ...
  function _(x) {
    ...
    for (const P of x) for (let b = 0; b < 3; b++) y[b] = Math.min(y[b], P[b]), v[b] = Math.max(v[b], P[b]);
  ...
  return $.derive(() => {
    ...
    (_c = (_b = (_a = e.nodeInputs) == null ? void 0 : _a.val) == null ? void 0 : _b.loads) == null ? void 0 : _c.forEach((v, z) => {
      ...
      b.normalize();
      const O = new Ot(b, new m(...P), 1, 15637248, 0.3, 0.3), ae = y * h.rawVal;
      O.scale.set(ae, ae, ae), d.add(O);
    });
  }), $.derive(() => {
    if (h.val, !i.loads.rawVal) return;
    const y = _(w.rawVal) * h.rawVal;
    d.children.forEach((v) => v.scale.set(y, y, y));
  }), $.derive(() => {
...
function ys(e, i, w) {
  ...
  return $.derive(() => {
    ...
    i.val.forEach((_, x) => {
      const y = new wt(`${x}`);
      y.position.set(..._), y.updateScale(d * w.rawVal), h.add(y);
    });
  }), $.derive(() => {
    if (w.val, !e.nodesIndexes.rawVal) return;
    const d = 0.05 * e.gridSize.val * 0.6;
    h.children.forEach((_) => _.updateScale(d * w.rawVal));
  }), $.derive(() => {
...
function xs(e, i, w, h) {
  ...
  return $.derive(() => {
    ...
    (_a = e.elements) == null ? void 0 : _a.val.forEach((x, y) => {
      const v = new wt(`${y}`, void 0, "#001219");
      v.position.set(...gs(x.map((z) => w.rawVal[z]))), v.updateScale(_ * h.rawVal), d.add(v);
    });
  }), $.derive(() => {
...
function gs(e) {
  const i = e.reduce((h, d) => [h[0] + d[0], h[1] + d[1], h[2] + d[2]], [0, 0, 0]), w = e.length;
  return [i[0] / w, i[1] / w, i[2] / w];
}
function So(e, i) {
  const w = new je(), h = 0.05 * e * 1, d = jt(), _ = new wt("X", "red", "transparent"), x = new wt(i ? "Z" : "Y", "green", "transparent"), y = new wt(i ? "Y" : "Z", "blue", "transparent"), v = new Ot(new m(1, 0, 0), new m(0, 0, 0), 1, d.axisArrow, 0.2, 0.2), z = new Ot(new m(0, 1, 0), new m(0, 0, 0), 1, d.axisArrow, 0.2, 0.2), P = new Ot(new m(0, 0, 1), new m(0, 0, 0), 1, d.axisArrow, 0.2, 0.2);
  return _.position.set(1.3 * h, 0, 0), x.position.set(0, 1.3 * h, 0), y.position.set(0, 0, 1.3 * h), _.updateScale(0.4 * h), x.updateScale(0.4 * h), y.updateScale(0.4 * h), v.scale.set(h, h, h), z.scale.set(h, h, h), P.scale.set(h, h, h), w.add(v, z, P, _, x, y), w;
}
function Qn(e, i) {
  const w = new m(...e), d = new m(...i).clone().sub(w), _ = d.length(), x = d.dot(new m(1, 0, 0)) / _, y = d.dot(new m(0, 1, 0)) / _, v = d.dot(new m(0, 0, 1)) / _, z = Math.sqrt(x ** 2 + y ** 2);
  let P = new Zn().fromArray([[x, y, v], [-y / z, x / z, 0], [-x * v / z, -y * v / z, z]].flat());
...
function vs(e, i, w) {
  const h = wn([i, w]), d = wn([e, w]), _ = wn([e, i]), x = new m(...h).sub(new m(...d)).normalize(), y = new m(...w).sub(new m(..._)).normalize(), v = x.clone().cross(y).normalize(), z = v.clone().cross(x).normalize();
  return new Ao().makeBasis(x, z, v);
}
function Ms(e, i, w, h) {
  const d = new je(), _ = new fe(), x = new it({ vertexColors: true }), y = [0, 0, 0], v = [1, 0, 0], z = [0, 1, 0], P = [0, 0, 1];
  _.setAttribute("position", new Mt([...y, ...v, ...y, ...z, ...y, ...P], 3));
...
function _s(e, i, w, h) {
  ...
  return $.derive(() => {
    ...
    I.forEach((C, N) => {
      ...
      const ne = w.rawVal[C[0]], W = w.rawVal[C[1]];
      if (!ne || !W) return;
      const H = K(ne, W);
      if (H && !Z || !H && !D) return;
      if (B >= 0) {
...
class Sn extends je {
  constructor(i, w, h, d, _, x, y) {
    super();
    const v = new Cn().moveTo(0, 0).lineTo(0, x[1]).lineTo(h, x[1]).lineTo(h, 0).lineTo(0, 0), z = v.getPoints(), P = new fe().setFromPoints(z);
    this.lines = new vt(P, new it({ color: jt().resultOutline })), this.lines.position.set(...i), this.lines.rotation.setFromRotationMatrix(d), y && this.lines.rotateX(Math.PI / 2), this.add(this.lines);
    const b = new Fn(v), O = new et({ color: x[1] > 0 ? 24435 : 11411474, side: zt });
    this.mesh = new We(b, O), this.mesh.position.set(...i), this.mesh.rotation.setFromRotationMatrix(d), y && this.mesh.rotateX(Math.PI / 2), this.add(this.mesh), this.text = new wt(`${_[1].toFixed(4)}`), this.normalizedResult = x, this.textPosition = wn([i, w]), this.text.position.set(...this.textPosition), this.text.rotation.setFromRotationMatrix(d), this.add(this.text);
  }
  updateScale(i) {
...
class ko extends je {
  constructor(i, w, h, d, _, x, y) {
    ...
    if (this.text = new wt(`${_[0].toFixed(4)}`), this.text2 = new wt(`${(_[1] * -1).toFixed(4)}`), this.normalizedResult = x, this.textPosition = qn(i, w), this.text2Position = qn(w, i), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(d), this.text2.rotation.setFromRotationMatrix(d), this.add(this.text, this.text2), z) {
      ...
      this.lines = new vt(ie, k), this.lines2 = new vt(ue, k), this.lines.position.set(...i), this.lines2.position.set(...i), this.lines.rotation.setFromRotationMatrix(d), this.lines2.rotation.setFromRotationMatrix(d), y && this.lines.rotateX(Math.PI / 2), y && this.lines2.rotateX(Math.PI / 2), this.add(this.lines, this.lines2);
      const G = new Fn(P), he = new Fn(b), ge = new et({ color: x[0] > 0 ? 24435 : 11411474, side: zt }), Pe = new et({ color: -x[1] > 0 ? 24435 : 11411474, side: zt });
      this.mesh = new We(G, ge), this.mesh2 = new We(he, Pe), this.mesh.position.set(...i), this.mesh2.position.set(...i), this.mesh.rotation.setFromRotationMatrix(d), this.mesh2.rotation.setFromRotationMatrix(d), y && this.mesh.rotateX(Math.PI / 2), y && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
      const P = new Cn().moveTo(0, 0).lineTo(0, x[0]).lineTo(h, -x[1]).lineTo(h, 0).lineTo(0, 0), b = P.getPoints(), O = new fe().setFromPoints(b);
      this.lines = new vt(O, new it({ color: jt().resultOutline })), this.lines.position.set(...i), this.lines.rotation.setFromRotationMatrix(d), y && this.lines.rotateX(Math.PI / 2), this.add(this.lines);
      const ae = new Fn(P), ie = new et({ color: x[0] > 0 ? 24435 : 11411474, side: zt });
      this.mesh = new We(ae, ie), this.mesh.position.set(...i), this.mesh.rotation.setFromRotationMatrix(d), y && this.mesh.rotateX(Math.PI / 2), this.add(this.mesh);
    }
  }
  updateScale(i) {
    var _a, _b;
...
function Ss(e, i, w, h) {
  ...
  return $.derive(() => {
    ...
    (_b = (_a = e.analyzeOutputs) == null ? void 0 : _a.rawVal[x]) == null ? void 0 : _b.forEach((y, v) => {
      var _a2, _b2;
      const z = ((_a2 = e.elements) == null ? void 0 : _a2.rawVal[v]) ?? [0, 1], P = w.rawVal[z[0]], b = w.rawVal[z[1]], O = new m(...b).distanceTo(new m(...P)), ae = ks((_b2 = e.analyzeOutputs) == null ? void 0 : _b2.rawVal[x]), ie = y == null ? void 0 : y.map((he) => he / (ae === 0 ? 1 : ae)), ue = Qn(P, b), k = new _[x](P, b, O, ue, y ?? [0, 0], ie ?? [0, 0], !!["normals", "shearsZ", "torsions", "bendingsY"].includes(x)), G = 0.05 * i.gridSize.rawVal;
      k.updateScale(G * h.rawVal), d.add(k);
    });
  }), $.derive(() => {
    if (h.val, i.frameResults.rawVal == "none") return;
    const x = 0.05 * i.gridSize.val;
    d.children.forEach((y) => y.updateScale(x * h.rawVal));
  }), $.derive(() => {
...
function Cs({ drawingObj: e, gridObj: i, scene: w, getActiveCamera: h, controls: d, gridSize: _, derivedDisplayScale: x, rendererElm: y, viewerRender: v }) {
  ...
  O.visible = true, O.frustumCulled = false, w.add(O);
  const ae = (n, o, a) => {
  ...
  const A = new m(), E = (n, o, a, t, r, s) => {
    ...
    const M = y.getBoundingClientRect(), c = M.left + (A.x * 0.5 + 0.5) * M.width, f = M.top + (-A.y * 0.5 + 0.5) * M.height;
  ...
  re.add(Fe, ce, _e), re.visible = false, re.frustumCulled = false, w.add(re);
  const j = new je();
  j.frustumCulled = false, j.visible = false, w.add(j);
  const Me = (n) => {
  ...
  me.frustumCulled = false, me.visible = false, w.add(me), me.add(we, de, ze);
  const tt = (n) => {
  ...
  Ie.renderOrder = 998, Ie.frustumCulled = false, Ie.visible = false, w.add(Ie);
  let Ue = -1, pt = -1, $e = -1;
  ...
  nt.renderOrder = 997, nt.frustumCulled = false, nt.visible = false, w.add(nt);
  ...
  he.geometry.setAttribute("position", new Mt(e.points.rawVal.flat(), 3)), he.geometry.computeBoundingSphere(), he.frustumCulled = false, ge.frustumCulled = false, w.add(ge), O.position.set(0, 0, 0), O.rotateX(Math.PI / 2), O.geometry.rotateX(Math.PI / 2), O.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (n, o, a) => {
  ...
  window.__hekatanShowRefPlanes = (n = [0, 3, 6, 9, 12], o = 20, a = 0, t = 0) => {
    ...
    n.forEach((s, l) => {
      ...
      ee.position.set(0, 0, s), ee.frustumCulled = false, ee.userData = { refPlaneZ: s }, w.add(ee), Ht.push(ee);
    }), Tt.visible = true, v();
  ...
  const Jt = new je();
  Jt.frustumCulled = false, w.add(Jt);
  const gn = () => {
    var _a, _b, _c, _d;
  ...
  const Yt = new je();
  Yt.frustumCulled = false, w.add(Yt);
  const eo = () => {
    var _a, _b, _c, _d;
  ...
  ut.add($n([-nn, 0, 0], [nn, 0, 0], 16711680)), ut.add($n([0, -nn, 0], [0, nn, 0], 65280)), ut.add($n([0, 0, -nn], [0, 0, nn], 35071)), ut.visible = false, ut.frustumCulled = false, w.add(ut);
    ...
    if (a.length) {
      ...
      if (p.length > 0 && g[p[p.length - 1]]) {
        ...
        if (Lt = null, !f && window.__hekatanAxisSnap !== false) {
          const Ve = y.getBoundingClientRect(), He = n.clientX, st = n.clientY, ht = ((_k = settings.gridSize) == null ? void 0 : _k.rawVal) ?? 10, St = new m(c[0], c[1], c[2]), Zt = [["x", new m(1, 0, 0)], ["y", new m(0, 1, 0)], ["z", new m(0, 0, 1)]], $t = (Vt) => {
  ...
  const Bn = (n, o, a, t, r) => {
    ...
    var _a, _b, _c, _d;
    const s = Math.min(n, a), l = Math.max(n, a), u = Math.min(o, t), p = Math.max(o, t), g = a < n, M = y.getBoundingClientRect(), c = h();
    c.updateMatrixWorld();
    const f = (ye) => {
  ...
  const Nt = new je();
  Nt.visible = false, Nt.frustumCulled = false, w.add(Nt);
...
function Fs(e, i, w) {
  ...
  function z() {
    v++;
    const P = v / _;
    e.position.lerpVectors(x.position, i.position, P), e.quaternion.slerpQuaternions(x.quaternion, i.quaternion, P), w && w(), v == _ && clearInterval(y);
  }
}
function Vs(e, i, w, h) {
  const d = ps(w, e.elements, h);
  return $.derive(() => {
...
function Is(e, i, w, h) {
  ...
  const x = new It(), y = $.state([]);
  return $.derive(() => {
    var _a, _b, _c;
    i.deformedShape.val;
    const v = w.val, z = ((_a = e.elements) == null ? void 0 : _a.val) ?? [], P = Es(i.frameResults.val);
    ...
    _.setMin(ue), _.setMax(k), y.val = ae;
...
function Zs(e) {
  ...
  const w = new rn(1, 16, 16), h = new et({ color: Ds, transparent: true, opacity: 0.85, depthTest: false }), d = new We(w, h);
  d.visible = false, d.renderOrder = 100, i.add(d);
  const _ = new fe(), x = new it({ color: zo, linewidth: 4, transparent: true, opacity: 0.9, depthTest: false }), y = new Dt(_, x);
  ...
  const k = new et({ color: kn, transparent: true, opacity: 0.95, depthTest: false }), G = new We(w, k);
...
Mz \u2248 ${rt(Yt * ot)} ${Ce.forceUnit}\xB7m`);
  ...
  function ne(j, Me, V) {
    var _a, _b, _c;
    if (d.visible = false, y.visible = false, z.visible = false, O.visible = false, ue.visible = false, !j || !e.mesh) {
  ...
  let le = null;
  const re = () => {
    W = "", d.visible = false, y.visible = false, z.visible = false, O.visible = false, ue.visible = false, B.style.display = "none", e.render();
  }, Fe = (j) => {
...
function Us(e, i, w, h, d, _) {
  const x = d - w, y = _ - h, v = x * x + y * y;
  if (v < 1e-9) {
    const ie = e - w, ue = i - h;
  ...
  const P = w + z * x, b = h + z * y, O = e - P, ae = i - b;
  return Math.sqrt(O * O + ae * ae);
}
function Ks(e, i, w) {
  let h = false;
  for (let d = 0, _ = w.length - 1; d < w.length; _ = d++) {
    const x = w[d].x, y = w[d].y, v = w[_].x, z = w[_].y;
...
function sa({ mesh: e, settingsObj: i, drawingObj: w, objects3D: h, solids: d }) {
  ...
  b.enableDamping = true, b.dampingFactor = 0.1, b.screenSpacePanning = true, b.zoomSpeed = 0.8, b.panSpeed = 1.2, b.rotateSpeed = 0.9, b.keyPanSpeed = 12, b.listenToKeyEvents(window), b.touches = { ONE: _n.ROTATE, TWO: _n.DOLLY_PAN }, P.domElement.addEventListener("wheel", (V) => {
    if (!V.ctrlKey && Math.abs(V.deltaX) > Math.abs(V.deltaY) * 1.5) {
      V.preventDefault();
      const X = b.target, J = new m().subVectors(y.position, X), q = new m();
      q.crossVectors(y.up, J).normalize();
      const de = J.length() * 1e-3 * b.panSpeed;
      X.addScaledVector(q, V.deltaX * de), y.position.addScaledVector(q, V.deltaX * de), b.update();
    }
  }, { passive: false });
  ...
  y.position.set(0, 0, A), y.up.set(0, 1, 0), b.target.set(0, 0, 0), b.minDistance = 0.1, b.maxDistance = 1e4, _.__settings = k, b.zoomSpeed = 1, b._getZoomScale = function() {
  ...
  $.derive(() => {
    ...
    y.position.distanceTo(b.target), Math.abs(y.position.x) < 0.1 && Math.abs(y.position.y) < 0.1 && y.position.z > 0 ? y.position.set(0, 0, J) : y.position.set(0.5 * V, -J, 0.5 * V), b.target.set(0, 0, 0), b.minDistance = Math.max(0.05, V * 0.01), b.maxDistance = Math.max(50, V * 50), b.update(), te();
    ...
    for (const X of V) {
      ...
      if (J === 0 || q === 0) continue;
      const de = (W ? J / 2 : J) / q;
      y.aspect = de, y.updateProjectionMatrix();
      const ze = v.top;
  ...
  function re(V, X) {
    ...
    y.isPerspectiveCamera && (y.aspect = de, y.updateProjectionMatrix());
    const ze = v.top;
  ...
  w && Cs({ drawingObj: w, gridObj: K, scene: x, getActiveCamera: () => z, controls: b, gridSize: I, derivedDisplayScale: G, rendererElm: P.domElement, viewerRender: te }), Fo((V, X) => {
    var _a;
  ...
  const Fe = { scene: x, perspCamera: y, orthoCamera: v, get camera() {
    return z;
...
function Gs(e, i) {
  return $.derive(() => {
    ...
    const d = i.deformScale.val, _ = i.deformScale.val * i.deformScaleZ.val, x = Number.isFinite(d) ? d : 1, y = Number.isFinite(_) ? _ : 1;
    return w.map((v, z) => {
      var _a2;
      ...
      return [v[0] + b * x, v[1] + O * x, v[2] + ae * y];
    });
  });
}
