...
import { g as we, a as Q, e as W } from "./getViewer-DRQ1dR7m.js";
...
Promise.all([
  ...
  l.derive(() => {
    const n = r.Lx.value.val, R = r.Ly.value.val, me = r.Lz.value.val, p = Math.round(r.nx.value.val), u = Math.round(r.ny.value.val), _ = Math.round(r.nz.value.val), I = r.Es.value.val, N = r.nu.value.val, Z = r.Rx.value.val, Y = r.Ry.value.val, ce = r.w.value.val, z = Math.round(r.showSlice.value.val) === 1, L = n / p, O = R / u, pe = me / _, d = (e, t, a) => a * (p + 1) * (u + 1) + t * (p + 1) + e, S = [];
    ...
    function g(e, t, a, s) {
      ...
      const o = v.length - 1;
      x.elasticities.set(o, I), x.poissonsRatios.set(o, N), x.thicknesses.set(o, 1e-3), x.shearModuli.set(o, I / (2 * (1 + N))), x.densities.set(o, 18 / 9.80665), x.areas.set(o, 0), x.momentsOfInertiaY.set(o, 0), x.momentsOfInertiaZ.set(o, 0), x.torsionalConstants.set(o, 0);
    }
    ...
    if (h) {
      ...
      E.forEach((a, s) => {
        ...
        let i = 0, c = 0;
        for (const m of o) i += Math.abs(m[2]), c++;
        i = c > 0 ? i / c : 0;
        for (const m of a) {
          const A = e.get(m) || {
            sum: 0,
