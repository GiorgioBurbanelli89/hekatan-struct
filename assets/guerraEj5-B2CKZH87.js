import { b as te, c as se, E as ae, L as le } from "./theme-CaJUXK9g.js";
import { p as ce, __tla as __tla_0 } from "./didacticCpp-CT_TM4EM.js";
let Me;
let __tla = Promise.all([
  ...
  Me = {
    ...
    build(t, c) {
      const _ = t.L, s = t.B1, m = t.B2, r = Math.max(s, m), u = t.h, M = Math.round(t.nx), p = Math.round(t.ny), a = M + 1, C = p + 1, h = _ / M, y = r / p, Z = (t.P_D_C1 + t.P_L_C1) * i, K = (t.M_D_C1 + t.M_L_C1) * i, q = (t.P_D_C2 + t.P_L_C2) * i, H = (t.M_D_C2 + t.M_L_C2) * i, P = t.ks_tm3 * i, z = 14100 * Math.sqrt(t.fc_kgcm2) * 98.0665, B = 0.2, $ = (n) => (s + (m - s) * (n / _)) / 2, J = (n, o) => {
        const e = $(n), l = r / 2;
        return Math.abs(o - l) <= e + 1e-6;
      }, d = [];
      for (let n = 0; n < C; ++n) for (let o = 0; o < a; ++o) d.push([
        o * h,
        n * y
      ]);
      const f = [];
      ...
      for (let n = 0; n < C; ++n) for (let o = 0; o < a; ++o) {
        const e = o === 0 || o === a - 1, l = n === 0 || n === C - 1, b = e && l ? 0.25 : e || l ? 0.5 : 1, G = h * y * b, v = n * a + o, ee = o * h, ne = n * y, X = J(ee, ne), oe = X ? P * G : 0;
        if (g.push({
          node: v,
          dof: 0,
          k: Math.max(oe, 1e-6)
      ...
      const w = r / 2, T = (n) => {
        ...
        for (let e = 0; e < d.length; e++) {
          const l = d[e][0], b = d[e][1];
          Math.abs(l - n) <= t.col_size / 2 + 1e-6 && Math.abs(b - w) <= t.col_size / 2 + 1e-6 && o.push(e);
        }
        return o;
      ...
      f.forEach((n, o) => {
        R.set(o, n.map((b) => -Math.abs(P * D.nodeResults[b].w)));
    ...
    computedLabels(t, c) {
      ...
      s === -1 / 0 && (s = 0, m = 0);
      const u = (_a = ie == null ? void 0 : ie.manual_libro) == null ? void 0 : _a.sigma_uniforme_tm2;
      return {
        "\u{1F4CA} \u03C3_max Hekatan": `${s.toFixed(3)} t/m\xB2`,
        "\u{1F4CA} \u03C3_min Hekatan": `${m.toFixed(3)} t/m\xB2`,
        "\u{1F4D8} \u03C3 uniforme libro p.95": `${u.toFixed(2)} t/m\xB2`,
