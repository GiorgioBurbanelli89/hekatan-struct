...
import { b as oe, M as J, c as I, d as he, E as xe, L as pe, C as Z, R as Me, e as ne, D as le } from "./Text-Dh9LKuSL.js";
let we;
let __tla = Promise.all([
  ...
  we = {
    ...
    build(e, c) {
      const a = [], _ = [], G = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), x = /* @__PURE__ */ new Map(), L = /* @__PURE__ */ new Map(), B = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), R = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map(), E = e.E_steel / 2.6, q = 77, m = (t, l, n) => (a.push([
        t,
        l,
        n
      ]), a.length - 1), M = (t, l, n, o, i) => {
        _.push([
          t,
          l,
          n,
        ...
        const s = _.length - 1;
        G.set(s, i), Y.set(s, e.E_steel), x.set(s, 0.3), L.set(s, q), B.set(s, 0), z.set(s, 0), C.set(s, 0), R.set(s, 0), A.set(s, E);
      }, w = ((t) => {
      ...
      for (let t = 0; t <= P; t++) {
        ...
        for (let n = 0; n <= g; n++) {
          ...
          for (const [b, v] of w) {
            const u = o - b, y = i - v, h = Math.sqrt(u * u + y * y);
            if (h < r * 0.35) {
              s = true;
              break;
            }
          ...
          s ? l.push(-1) : l.push(m(o, i, 0));
        }
        $.push(l);
      }
      for (let t = 0; t < P; t++) for (let l = 0; l < g; l++) {
        const n = $[t][l], o = $[t][l + 1], i = $[t + 1][l + 1], s = $[t + 1][l];
        if (n < 0 || o < 0 || i < 0 || s < 0) continue;
        const b = -e.B / 2 + (l + 0.5) * N, v = -e.H / 2 + (t + 0.5) * k;
        let u = false;
        for (const [y, h] of w) if (Math.sqrt((b - y) ** 2 + (v - h) ** 2) < r * 0.95) {
          u = true;
          break;
        }
        u || M(n, o, i, s, e.t_plate);
      }
      const F = e.t_plate;
      F + e.L_col_stub;
      const d = 5, f = [];
      for (let t = 0; t <= d; t++) {
        const l = F + t * e.L_col_stub / d;
        f.push([
          m(+e.d_col / 2, -e.bf_col / 2, l),
          m(+e.d_col / 2, +e.bf_col / 2, l)
        ]);
      }
      ...
      for (let t = 0; t < d; t++) for (let l = 0; l < W; l++) M(S[t][l], S[t][l + 1], S[t + 1][l + 1], S[t + 1][l], e.tw_col);
      const D = /* @__PURE__ */ new Map();
      for (const [t, l] of w) {
        const n = [];
        for (let o = 0; o < a.length; o++) {
          if (Math.abs(a[o][2]) > 1e-4) continue;
          const i = a[o][0] - t, s = a[o][1] - l, b = Math.sqrt(i * i + s * s);
      ...
      for (let t = 0; t < a.length; t++) {
        if (Math.abs(a[t][2]) > 1e-4) continue;
        const l = a[t][0], n = a[t][1], o = Math.abs(l - e.d_col / 2) < 0.02 && Math.abs(n) <= e.bf_col / 2 + 1e-6, i = Math.abs(l + e.d_col / 2) < 0.02 && Math.abs(n) <= e.bf_col / 2 + 1e-6, s = Math.abs(n) < 0.015 && Math.abs(l) <= e.d_col / 2 - e.tf_col + 1e-6;
        (o || i || s) && O.push(t);
      }
      if (O.length > 0) {
        const t = -e.Pu / O.length;
        for (const l of O) {
          const n = a[l][0], o = e.d_col / 2, i = e.Mu / (O.length * o) * (n > 0 ? 1 : -1);
          X.set(l, [
            0,
            0,
            t + i,
      ...
      c.nodes.val = a, c.elements.val = _, c.nodeInputs.val = {
        supports: D,
        loads: X
      }, c.elementInputs.val = {
        thicknesses: G,
      ...
      try {
        if (e.use_nonlinear > 0.5) {
          ...
          const l = t.analyzeOutputs;
          c.analyzeOutputs.val = l, c.__nlInfo = {
          ...
          c.deformOutputs.val = fe(a, _, {
            supports: D,
            loads: X
          }, c.elementInputs.val);
          const t = _e(a, _, c.elementInputs.val, c.deformOutputs.val);
      ...
      const p = [], K = new oe(e.B_ped, e.H_ped, e.h_ped), ae = new J({
        color: 12298888,
        transparent: true,
        opacity: 0.35,
        metalness: 0.1,
      ...
      for (const [t, l] of w) {
        ...
        s.position.set(t, l, b), p.push(s);
        const v = new Z(e.d_bolt * 0.9, e.d_bolt * 0.9, n, 6), u = new I(v, ce);
        u.rotation.x = Math.PI / 2, u.position.set(t, l, e.t_plate + n / 2), p.push(u);
        const y = new Z(e.d_hole / 2 * 1.4, e.d_hole / 2 * 1.4, 4e-3, 20), h = new J({
          color: 8947848,
          metalness: 0.6,
          roughness: 0.4
        }), j = new I(y, h);
        j.rotation.x = Math.PI / 2, j.position.set(t, l, e.t_plate + 2e-3), p.push(j);
        const me = new Me(r, r * 1.05, 32), ue = new ne({
          color: 16776960,
          side: le
        }), te = new I(me, ue);
        te.position.set(t, l, e.t_plate + 5e-4), p.push(te);
      }
      ...
      ee.position.set(0, 0, -e.h_ped + 1e-3), p.push(ee), c.objects3D.val = p, console.log(`[Placa Base AISC \xA7J8] Shells=${_.length}, Nodos=${a.length}
  ...
  Pedestal ${e.B_ped}\xD7${e.H_ped}\xD7${e.h_ped}m f'c=${e.fc / 1e3} MPa`);
    ...
    computedLabels(e, c) {
      const a = c.__nlInfo, _ = 0.65, G = 0.9, Y = 0.75, x = e.B * e.H, L = Math.min(e.B_ped * e.H_ped, x * 4), B = Math.min(Math.sqrt(L / x), 2), z = 0.85 * e.fc * x * B, C = _ * z, R = Math.max(0, (e.B - 0.95 * e.d_col) / 2), A = Math.max(0, (e.H - 0.8 * e.bf_col) / 2), E = Math.sqrt(e.d_col * e.bf_col) / 4, q = Math.max(R, A, E), m = e.Pu / x, M = q * Math.sqrt(2 * m / (G * e.Fy_plate)), T = e.t_plate / M, w = e.Mu / Math.max(e.Pu, 1e-3), r = e.bolt_layout / 2, g = e.H / 2 - e.edge_dist, P = Math.max(0, (e.Mu - e.Pu * g) / (2 * g)), N = P / Math.max(r, 1), k = Math.PI * (e.d_bolt / 2) ** 2, $ = 0.75 * e.Fu_bolt * k, F = Y * $, d = e.Pu / C, f = N / F;
      return {
        "\u2500\u2500 Geometr\xEDa \u2500\u2500": "",
        "A1 (\xE1rea placa)": `${(x * 1e4).toFixed(0)} cm\xB2`,
        "A2 (\xE1rea pedestal)": `${(L * 1e4).toFixed(0)} cm\xB2`,
        "\u221A(A2/A1)": B.toFixed(2),
        "m (voladizo X)": `${(R * 1e3).toFixed(0)} mm`,
        ...
        "Pp (nominal)": `${z.toFixed(0)} kN`,
        "\u03C6Pp (dise\xF1o)": `${C.toFixed(0)} kN`,
        ...
        "\u2500\u2500 Solver FEM \u2500\u2500": "",
        Tipo: a ? "NO-LINEAL (J2 secante)" : "Lineal el\xE1stico",
        ...a ? {
          ...
          Interpretaci\u00F3n: a.elementsYielded > 0 ? `${a.elementsYielded} shells alcanzaron fluencia \u2192 redistribuci\xF3n` : "Toda la placa en rango el\xE1stico"
        } : {}
      };
    }
  };
