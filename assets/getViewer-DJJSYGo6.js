import { X as Ft, B as se, Y as At, F as Ie, G as _e, d as ht, L as Ce, e as Ae, D as $e, b as ke, s as Ve, Z as vn, c as Xn, V as m, x as it, y as ze, _ as Nt, k as gn, a as Ee, f as Me, h as Tt, $ as Et, l as Yn, j as Ln, q as kt, I as vt, S as Pt, a0 as an, m as ln, o as rn, p as cn, a1 as dn, a2 as Ct, a3 as In, a4 as $n, a5 as Rn, a6 as Bn, a7 as Dn, n as pn, a8 as un, r as Zn, t as Nn, u as Wn, W as Un, v as hn, a9 as Vt, H as Wt, A as Gn, w as fn, O as Kn } from "./Text-BE8nxNWm.js";
...
function On(e, s, f) {
  ...
  return L.derive(() => {
    ...
    const W = s.elemColumns.rawVal, H = s.elemBeams.rawVal, D = f.val, ie = ((_a = e.elements) == null ? void 0 : _a.val) || [], x = ie.filter((N) => {
      ...
      const C = y(V, I);
      return !(C && !W || !C && !H);
    }).map((N) => jn(N).map((V) => [...D[V[0]], ...D[V[1]]]).flat()).flat();
...
function eo(e, s, f, d) {
  const l = new _e(), g = new Xn(0.5, 0.5, 0.5), w = new Ae({ color: 10166822 });
  return L.derive(() => {
    var _a, _b;
...
function oo(e, s, f, d) {
  ...
  return L.derive(() => {
    ...
    (_a = e.elements) == null ? void 0 : _a.val.forEach((w, p) => {
      ...
      b.position.set(...so(w.map((F) => f.rawVal[F]))), b.updateScale(g * d.rawVal), l.add(b);
    });
  }), L.derive(() => {
...
function ro(e, s, f, d) {
  ...
  const x = new Ae({ color: 52479, transparent: true, opacity: 0.35, side: $e, depthWrite: false }), ee = new Ce({ color: 52479 }), te = new Ae({ color: 16750848, transparent: true, opacity: 0.4, side: $e, depthWrite: false }), de = new Ce({ color: 16750848 });
  function N(V, I) {
...
class zt extends _e {
  constructor(s, f, d, l, g, w, p) {
    ...
    const y = new Et(b), W = new Ae({ color: w[1] > 0 ? 24435 : 11411474, side: $e });
...
class wn extends _e {
  constructor(s, f, d, l, g, w, p) {
    ...
    if (this.text = new ze(`${g[0].toFixed(4)}`), this.text2 = new ze(`${(g[1] * -1).toFixed(4)}`), this.normalizedResult = w, this.textPosition = Ht(s, f), this.text2Position = Ht(f, s), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(l), this.text2.rotation.setFromRotationMatrix(l), this.add(this.text, this.text2), F) {
      ...
      const ee = new Et(P), te = new Et(y), de = new Ae({ color: w[0] > 0 ? 24435 : 11411474, side: $e }), N = new Ae({ color: -w[1] > 0 ? 24435 : 11411474, side: $e });
      ...
      const H = new Et(P), D = new Ae({ color: w[0] > 0 ? 24435 : 11411474, side: $e });
...
function ho(e, s, f, d) {
  ...
  return L.derive(() => {
    ...
    (_b = (_a = e.deformOutputs) == null ? void 0 : _a.val[g]) == null ? void 0 : _b.forEach((p, b) => {
      const F = new uo(f.rawVal[b], g, p ?? [0, 0, 0, 0, 0, 0]);
...
function fo({ drawingObj: e, gridObj: s, scene: f, getActiveCamera: d, controls: l, gridSize: g, derivedDisplayScale: w, rendererElm: p, viewerRender: b }) {
  const F = new Yn(), P = new Ln(), y = (n) => {
  ...
  const H = (n, o, r) => {
    ...
    const o = [W];
    return D.visible && o.push(D), ie.visible && o.push(ie), Ue.visible && dt.length > 0 && o.push(...dt), F.intersectObjects(o, false);
  ...
  const Yt = (n, o, r, t, u, c, k, a, i) => {
    ...
    e.points.val = k, e.polylines.val = a, e.areas && (e.areas.val = e.areas.rawVal.filter((i) => i !== n).map((i) => i > n ? i - 1 : i)), Xe.visible = false, Qe = -1, ct = -1;
    try {
      (_a = window.__hekatanRebuild) == null ? void 0 : _a.call(window);
    } catch {
    }
  ...
  ee.geometry.setAttribute("position", new Ie(e.points.rawVal.flat(), 3)), ee.geometry.computeBoundingSphere(), ee.frustumCulled = false, te.frustumCulled = false, f.add(te), W.position.set(0, 0, 0), W.rotateX(Math.PI / 2), W.geometry.rotateX(Math.PI / 2), W.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (n, o, r) => {
    ...
    const we = c.distanceTo(q), be = new m().subVectors(c, q), Be = new m().subVectors(a, q), De = Math.acos(Math.max(-1, Math.min(1, be.dot(Be) / (we * we)))), Ke = e.points.rawVal.length, Ye = [], He = S.clone();
    for (let Le = 0; Le <= u; Le++) {
      const qe = Le / u, We = De * qe, ut = new an().setFromAxisAngle(He, We), at = be.clone().applyQuaternion(ut).add(q);
  ...
  Re.visible = false, f.add(Re), window.__hekatanShowAxes = (n, o, r = 12, t = 2) => {
    ...
    n.forEach((S, h) => {
      ...
      ne.computeLineDistances(), Re.add(ne), Re.add(v(M, S, u - 0.5, 0, "#60a5fa")), Re.add(v(M, S, c + 0.5, 0, "#60a5fa"));
    }), o.forEach((S, h) => {
      ...
      ne.computeLineDistances(), Re.add(ne), Re.add(v(M, k - 0.5, S, 0, "#fb7185")), Re.add(v(M, a + 0.5, S, 0, "#fb7185"));
    }), Re.visible = true, b();
  ...
  L.derive(() => {
    ...
    (n == null ? void 0 : n.val) && (n.val, Vn(), b());
  });
  ...
  l.addEventListener("change", () => {
    ...
    if (r.length) {
      ...
      if (k === "delete") {
        ...
        if (A ? (Je = M, Qe = -1, ct = -1, kn(M)) : h ? (Qe = h.polyIdx, ct = h.segIdx, Je = -1, Pn(h.polyIdx, h.segIdx)) : (Qe = -1, ct = -1, Je = -1, Xe.visible = false), z.visible = false, B.visible = false, X(), Z.style.left = n.clientX + "px", Z.style.top = n.clientY + "px", Z.style.display = "block", A) Z.textContent = `\u{1F5D1} Click para borrar l\xEDnea auxiliar #${Je + 1}`;
        else if (h) {
      ...
      if (i.length > 0 && v[i[i.length - 1]]) {
        ...
        const Be = J.material, De = oe.material, Ke = le.material;
        A === "x" ? (Be.opacity = 0.95, De.opacity = 0.1, Ke.opacity = 0.1) : A === "y" ? (Be.opacity = 0.1, De.opacity = 0.95, Ke.opacity = 0.1) : A === "z" ? (Be.opacity = 0.1, De.opacity = 0.1, Ke.opacity = 0.95) : (Be.opacity = 0.5, De.opacity = 0.5, Ke.opacity = 0.5);
    ...
    e.gridTarget && (mo(s, { position: new m(...e.gridTarget.val.position), quaternion: new an().setFromEuler(new dn(...e.gridTarget.val.rotation)) }, b), W.position.set(...e.gridTarget.val.position), W.quaternion.setFromEuler(new dn(...e.gridTarget.val.rotation)), W.updateMatrixWorld());
  }), L.derive(() => {
  ...
  const An = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496 }, tn = (n, o, r, t) => {
    ...
    const u = An[n] ?? 16777215, c = 0.05, k = new se().setFromPoints([new m(o - c, r - c, t), new m(o + c, r - c, t), new m(o + c, r - c, t), new m(o + c, r + c, t), new m(o + c, r + c, t), new m(o - c, r + c, t), new m(o - c, r + c, t), new m(o - c, r - c, t)]);
    Ge.add(new ht(k, new Ce({ color: u, linewidth: 2 }))), Ge.position.set(0, 0, 0), Ge.visible = true;
  }, Bt = () => {
    Ge.visible = false;
  }, Tn = (n, o, r, t) => {
    var _a;
    ...
    for (const h of k) if (!(h.length < 2)) for (let M = 0; M < h.length - 1; M++) {
      ...
      if (!(!A || !G) && (u.end && (i("end", A[0], A[1], A[2]), i("end", G[0], G[1], G[2])), u.mid && i("mid", (A[0] + G[0]) / 2, (A[1] + G[1]) / 2, (A[2] + G[2]) / 2), u.nea || u.per)) {
        ...
        const be = Math.max(0, Math.min(1, ((n - A[0]) * ne + (o - A[1]) * j + (r - A[2]) * q) / we)), Be = A[0] + be * ne, De = A[1] + be * j, Ke = A[2] + be * q;
        u.nea && i("nea", Be, De, Ke), u.per && i("per", Be, De, Ke);
      }
    }
    ...
    for (const h of S) {
      ...
      if (u.end && (i("end", M[0], M[1], M[2]), i("end", A[0], A[1], A[2])), u.mid && i("mid", (M[0] + A[0]) / 2, (M[1] + A[1]) / 2, (M[2] + A[2]) / 2), u.nea || u.per) {
        ...
        if (q < 1e-12) continue;
        const we = Math.max(0, Math.min(1, ((n - M[0]) * G + (o - M[1]) * ne + (r - M[2]) * j) / q)), be = M[0] + we * G, Be = M[1] + we * ne, De = M[2] + we * j;
        u.nea && i("nea", be, Be, De), u.per && i("per", be, Be, De);
      }
    }
  ...
  window.__hekatanFinalizeDraw = on, p.addEventListener("click", (n) => {
    ...
    if (k === "select" || k === "none" || !k) {
      if (et) {
        ...
        n.ctrlKey || n.metaKey || n.shiftKey || Ze.clear(), Ze.has(S) ? Ze.delete(S) : Ze.add(S), _t(), ce(`\u2713 Seleccionados ${Ze.size} elemento(s) \u2014 Ctrl+Click para multi-selecci\xF3n`);
      } else !n.ctrlKey && !n.metaKey && !n.shiftKey && Ze.size > 0 && (Ze.clear(), _t(), ce("Selecci\xF3n limpiada"));
      return;
    }
    if (k === "axis") {
    ...
    if (k === "delete") {
      if (Je >= 0) {
        const a = window.__hekatanDrawingAuxLines, i = (a == null ? void 0 : a.rawVal) ?? (a == null ? void 0 : a.val) ?? a ?? [], v = Je;
        if (v >= 0 && v < i.length) {
          st();
          const S = i.slice(0, v).concat(i.slice(v + 1));
          a && typeof a == "object" && "val" in a ? a.val = S : window.__hekatanDrawingAuxLines = S, ce(`\u{1F5D1} L\xEDnea auxiliar #${v + 1} borrada`), Je = -1, Xe.visible = false;
          try {
        ...
        const a = Qe, i = ct;
        ((_i = (_h = e.areas) == null ? void 0 : _h.rawVal) == null ? void 0 : _i.includes(a)) ?? false ? (St(a), ce(`\u{1F5D1} \xC1rea #${a + 1} (shell Q4) borrada`)) : i >= 0 ? (Cn(a, i), ce(`\u{1F5D1} Segmento ${i + 1} de polil\xEDnea #${a + 1} borrado`)) : (St(a), ce(`\u{1F5D1} Polil\xEDnea #${a + 1} borrada`));
...
function xo() {
  ...
  for (let d = 0; d < 256; d++) {
    const l = d / 255, [g, w, p] = wo(l);
    s[d * 4 + 0] = g, s[d * 4 + 1] = w, s[d * 4 + 2] = p, s[d * 4 + 3] = 255;
  }
  const f = new Bn(s, 256, 1, Dn);
...
function yo(e, s, f) {
  ...
  const d = xo(), l = new $n({ uniforms: { cmap: { value: d }, ambient: { value: 0.95 } }, vertexShader: `
    ...
    `, side: $e, transparent: false, clipping: true, depthWrite: true, depthTest: true }), g = new ke(new se(), l);
  return g.renderOrder = -1, g.frustumCulled = false, L.derive(() => {
    ...
    g.geometry.setIndex(new Rn(w, 1));
...
function So(e, s, f, d) {
  ...
  return L.derive(() => {
    var _a, _b, _c;
    s.deformedShape.val;
    const b = f.val, F = ((_a = e.elements) == null ? void 0 : _a.val) ?? [], P = Mo(s.frameResults.val);
...
function xn(e, s = 8) {
  ...
  return f.querySelectorAll("p").forEach((b) => p.push(b)), setTimeout(() => {
    L.derive(() => {
      l.forEach((b, F) => {
        const P = p[F];
        P && (P.innerText = ko(e.val, b).toString());
      });
    });
  }), f;
}
...
function Lo({ mesh: e, settingsObj: s, drawingObj: f, objects3D: d, solids: l }) {
  ...
  const g = document.createElement("div"), w = new Zn(), p = new Nn(45, 1, 0.1, 2 * 1e6), b = new Wn(-10, 10, 10, -10, -1e3, 2e6);
  let F = p;
  const P = new Un({ antialias: true });
  ...
  const x = Qn(s), ee = L.derive(() => x.displayScale.val === 0 ? 1 : x.displayScale.val > 0 ? x.displayScale.val : -1 / x.displayScale.val), te = Po(e, x), de = () => {
    const Y = [];
  ...
  f && fo({ drawingObj: f, gridObj: V, scene: w, getActiveCamera: () => F, controls: y, gridSize: _, derivedDisplayScale: ee, rendererElm: P.domElement, viewerRender: J }), Xt((Y, U) => {
    var _a;
