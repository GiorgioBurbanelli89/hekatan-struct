import { m as ot, p as et, __tla as __tla_0 } from "./didacticCpp-DDG05360.js";
let it;
let __tla = Promise.all([
  ...
  let w, B, st, at, K, z, U, v, F, R, Q, q, G, H;
  w = 0.0254;
  B = 0.3048;
  st = 0.04788;
  at = 6894.76;
  ...
  it = {
    ...
    build($, c) {
      const u = Math.round($.theoryType), r = Math.round($.mesh), o = r, e = r, m = 100 * st, l = et({
        E: Q,
        nu: q,
        thickness: R,
        theoryType: u,
      ...
      S.forEach((t, n) => {
        b.set(n, Q), h.set(n, q), x.set(n, 24);
      });
      const M = /* @__PURE__ */ new Map(), p = /* @__PURE__ */ new Map(), g = v / o, E = F / e, X = g * E;
      _.forEach((t, n) => {
        const s = Math.abs(t[0]) < 1e-6 || Math.abs(t[0] - v) < 1e-6, a = Math.abs(t[1]) < 1e-6 || Math.abs(t[1] - F) < 1e-6;
        (s || a) && M.set(n, [
          true,
          true,
          true,
      ...
      const P = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), N = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), Y = R * R / 6;
      ...
      function W(t, n) {
        var _a, _b, _c, _d;
        const s = Math.min(o - 1, Math.max(0, Math.floor(t / L))), a = Math.min(e - 1, Math.max(0, Math.floor(n / O))), i = (t - s * L) / L, d = (n - a * O) / O, A = a * (o + 1) + s, I = a * (o + 1) + s + 1, Z = (a + 1) * (o + 1) + s + 1, D = (a + 1) * (o + 1) + s, V = ((_a = f.get(A)) == null ? void 0 : _a[2]) ?? 0, J = ((_b = f.get(I)) == null ? void 0 : _b[2]) ?? 0, tt = ((_c = f.get(Z)) == null ? void 0 : _c[2]) ?? 0, nt = ((_d = f.get(D)) == null ? void 0 : _d[2]) ?? 0;
...
[SAFE Ex.1 \xB7 ${o}\xD7${e} ${u === 1 ? "Thin (Kirchhoff)" : "Thick (Mindlin)"}]  Geom ${K}'\xD7${z}'\xD7${U}"  E=${(Q / 1e6).toFixed(1)} GPa  \u03BD=${q}`), console.log(`              UL = ${m.toFixed(3)} kN/m\xB2 (= 100 psf)`), console.log("  Punto             X (in)  Y (in)  w_Hek (in)   w_Navier (in)  \u0394%   (bilineal)");
      for (const t of G) {
        const n = t.x * w, s = t.y * w, a = W(n, s), i = Math.abs(a) / w, d = t.label.split(" ")[0], A = H[d], I = (i / A - 1) * 100;
    ...
    computedLabels: ($, c) => {
      ...
      if (!r) return u;
      const o = Math.round($.mesh), e = o, m = o, l = v / e, _ = F / m;
      function S(y, b) {
        var _a2, _b, _c, _d;
        const h = Math.min(e - 1, Math.max(0, Math.floor(y / l))), x = Math.min(m - 1, Math.max(0, Math.floor(b / _))), M = (y - h * l) / l, p = (b - x * _) / _, g = x * (e + 1) + h, E = x * (e + 1) + h + 1, X = (x + 1) * (e + 1) + h + 1, f = (x + 1) * (e + 1) + h, P = ((_a2 = r.get(g)) == null ? void 0 : _a2[2]) ?? 0, T = ((_b = r.get(E)) == null ? void 0 : _b[2]) ?? 0, k = ((_c = r.get(X)) == null ? void 0 : _c[2]) ?? 0, N = ((_d = r.get(f)) == null ? void 0 : _d[2]) ?? 0;
        return (1 - M) * (1 - p) * P + M * (1 - p) * T + M * p * k + (1 - M) * p * N;
      }
      for (const y of G) {
        const b = y.x * w, h = y.y * w, x = S(b, h), M = Math.abs(x) / w, p = y.label.split(" ")[0], g = H[p], E = (M / g - 1) * 100;
        u[`${p} w_Hek/w_Navier`] = `${M.toFixed(4)} in / ${g.toFixed(4)} in (${E >= 0 ? "+" : ""}${E.toFixed(2)}%)`;
      }
      return u;
    },
    runModal($, c, u) {
