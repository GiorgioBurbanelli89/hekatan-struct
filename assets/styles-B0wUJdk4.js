...
import { K as q, q as Be, Z as We, c as Ue, B as ve, L as Ye, e as Xe, D as Ve, d as Ke, v as ue, F as pe, a3 as _e, a4 as qe, a2 as Ge } from "./theme-DDCjfe25.js";
const Je = new q(16746496), Ze = new q(52428), $e = new q(52292), Se = new q(3377407), Te = new q(16763904);
function Ae(I, e) {
...
function et(I, e, t, i, s, l, r = 8) {
  ...
  for (let E = 0; E <= r; E++) {
    const O = E / r, z = 1 - 3 * O * O + 2 * O * O * O, M = m * (O - 2 * O * O + O * O * O), K = 3 * O * O - 2 * O * O * O, P = m * (-O * O + O * O * O), V = z * A + M * x + K * k + P * T, X = z * S - M * R + K * C - P * L, G = B + (U - B) * O, le = [I[0] + p[0] * (O * m + G), I[1] + p[1] * (O * m + G), I[2] + p[2] * (O * m + G)];
...
function xt(I, e, t) {
  ...
  let h = new q(i.shellWall), o = new q(i.shellSlab), d = new q(i.shellTri);
  return _e((u, p) => {
    h = new q(p.shellWall), o = new q(p.shellSlab), d = new q(p.shellTri), r.opacity = p.shellOpacity, r.needsUpdate = true;
  }), ue.derive(() => {
      ...
      if (C > $ && (C = $), x >= $ - 1 && (x = $), n(this.box).find("#grid_" + this.name + "_footer .w2ui-footer-right").html((this.show.statusRange ? c.formatNumber(this.offset + C) + "-" + c.formatNumber(this.offset + x) + (this.total != -1 ? " " + c.lang("of") + " " + c.formatNumber(this.total) : "") : "") + (i && this.show.statusBuffered ? " (" + c.lang("buffered") + " " + c.formatNumber($) + (0 < this.offset ? ", skip " + c.formatNumber(this.offset) : "") + ")" : "")), i || this.fixedBody && !(this.total != -1 && this.total <= this.vs_start)) {
        let K = function() {
          t.markSearch && (clearTimeout(t.last.marker_timer), t.last.marker_timer = setTimeout(() => {
            var P = [];
            for (let G = 0; G < t.searchData.length; G++) {
              var V = t.searchData[G], X = t.getSearch(V.field);
              X && !X.hidden && (X = t.getColumn(V.field, true), P.push({ field: V.field, search: V.value, col: X }));
            }
            0 < P.length && P.forEach((G) => {
              var le = n(t.box).find('td[col="' + G.col + '"]:not(.w2ui-head)');
              c.marker(le, G.search);
            });
          }, 50));
