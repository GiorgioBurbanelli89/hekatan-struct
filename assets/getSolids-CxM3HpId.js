import { g as ee, __tla as __tla_0 } from "./getMesh-B5zl1JtK.js";
import { a as Z, s as V, n as te, c as ne, f as se, m as oe } from "./pureFunctionsAny.generated-cNLQlHDB.js";
import { h as re, B as C, L as ie, F as le, w as q, d as ae, x as ue, y as W, z as ce, A as F, m as _, G as j } from "./theme-Cr2LU0HL.js";
let Ee, Me, Ae, Te, xe;
    ...
    for (let i in l) {
      ...
      u.get(Number(i)).forEach((p) => {
        ...
        const b = d[p].map((M) => t[M]), w = b.map((M, A) => A), B = (M) => {
          ...
          polygon: w,
          maxMeshSize: I
        }), H = n.length, z = c.length, N = S.map((M) => M.map((A) => A + H)), k = P.map((M, A) => A + H), $ = S.map((M, A) => A + z);
        n = [
          ...n,
          ...fe(P, y)
        ], c = [
