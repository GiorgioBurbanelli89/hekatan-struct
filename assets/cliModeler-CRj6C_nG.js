...
import { d as se, __tla as __tla_0 } from "./didacticCpp-pa3uOomD.js";
let re, oe;
  ...
  oe = function(_) {
    ...
    for (let L = 0; L < v.length; L++) {
      ...
      if (p === "supports" && /^\d/.test(s[0])) {
        const o = parseInt(s[0], 10), i = s.slice(1).join(" ");
        t.supports.set(o, Q(i));
        continue;
      }
      p && !/^[\-\d]/.test(s[0]) && (p = null);
      try {
        switch (F) {
          case "node":
          case "n": {
            const o = parseInt(s[1], 10), i = parseFloat(s[2]), d = parseFloat(s[3]), u = parseFloat(s[4]);
            !isFinite(o) || !isFinite(i) || !isFinite(d) || !isFinite(u) ? t.errors.push(`L${L + 1}: node mal formado: ${m}`) : t.nodes.set(o, [
              i,
              d,
              u
            ]);
          ...
          case "shellmod": {
            ...
            if (!isFinite(o)) break;
            const i = s.slice(2).map(parseFloat);
            if (i.length >= 8) t.shellModsDir.set(o, i.slice(0, 8).map((d) => isFinite(d) ? d : 1));
            else {
              const d = i[0], u = i[1];
              t.shellMods.set(o, [
                isFinite(d) ? d : 1,
          ...
          case "qarea": {
            ...
            t.shellLoads.set(o, i);
            break;
          }
          case "support":
          case "fix": {
            const o = parseInt(s[1], 10), i = s.slice(2).join(" ");
            t.supports.set(o, Q(i));
            break;
          }
          case "load":
          case "l": {
            const o = parseInt(s[1], 10), i = parseFloat(s[2] ?? "0"), d = parseFloat(s[3] ?? "0"), u = parseFloat(s[4] ?? "0"), g = parseFloat(s[5] ?? "0"), y = parseFloat(s[6] ?? "0"), k = parseFloat(s[7] ?? "0");
            t.loads.set(o, [
              i,
              d,
              u,
          ...
          case "frameload":
          case "fl": {
            const o = parseInt(s[1], 10), i = parseFloat(s[2] ?? "0"), d = parseFloat(s[3] ?? "0"), u = parseFloat(s[4] ?? "0"), g = t.frameLoads.get(o) ?? [
              0,
              0,
              0
            ];
          ...
          case "spring": {
            const o = parseInt(s[1], 10), i = (s[2] ?? "uz").toLowerCase(), d = G[i] ?? 2, u = parseFloat(s[3] ?? "1000");
            t.springs.push({
              node: o,
              dof: d,
              k: u
...
`;
  re = {
    ...
    build(_, t) {
      ...
      const m = [], s = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), o = /* @__PURE__ */ new Map(), i = /* @__PURE__ */ new Map(), d = /* @__PURE__ */ new Map(), u = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), y = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), E = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map();
      for (const e of r.frames) {
        ...
        const I = e.nu ?? 0.2;
        s.set(l, e.E), F.set(l, e.E / (2 * (1 + I))), o.set(l, e.A), i.set(l, e.I), d.set(l, e.Iy ?? e.I), u.set(l, e.J ?? 0.14 * Math.pow(Math.sqrt(e.A), 4)), g.set(l, e.rho ?? 2.45), E.set(l, I), e.D !== void 0 && isFinite(e.D) && y.set(l, e.D), e.B !== void 0 && isFinite(e.B) && k.set(l, e.B);
      }
      for (const e of r.shells) {
        const n = e.pts.map((l) => f.get(l));
        if (n.some((l) => l === void 0)) {
      ...
      if (r.frameLoads.size) {
        ...
        for (const [n, a] of r.frameLoads.entries()) {
          const l = r.frames.find((T) => T.id === n);
          if (!l) {
            r.errors.push(`frameload ${n}: no existe esa barra`);
            continue;
          }
          const I = f.get(l.nI), c = f.get(l.nJ);
      ...
      for (const e of r.shells) {
        ...
        for (const [c, h] of V) {
          const C = [
            ...
            1,
            2
          ].map(($) => A.reduce((Y, Z, X) => Y + Z * l[X][$], 0)), x = [
            0,
            1,
            2
          ].map(($) => M.reduce((Y, Z, X) => Y + Z * l[X][$], 0)), z = [
            b[1] * x[2] - b[2] * x[1],
            b[2] * x[0] - b[0] * x[2],
            b[0] * x[1] - b[1] * x[0]
          ], T = Math.hypot(z[0], z[1], z[2]);
          for (let $ = 0; $ < 4; $++) I[$] += C[$] * n * T;
        }
        for (let c = 0; c < 4; c++) {
      ...
      const R = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), P = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map();
      for (const e of r.shells) {
        const n = O.get(e.id);
        ...
        if (I) {
          P.set(n, I), R.set(n, (I[0] + I[1]) / 2), U.set(n, (I[3] + I[4]) / 2);
          continue;
        }
        const c = r.shellMods.get(e.id);
        c && (R.set(n, c[0]), U.set(n, c[1]));
      }
      ...
      if ((_a = N == null ? void 0 : N.deformations) == null ? void 0 : _a.size) for (const [, e] of N.deformations) Math.abs(e[2]) > Math.abs(B) && (B = e[2]);
