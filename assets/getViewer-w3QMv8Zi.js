import { H as ce, B as k, I as de, F as q, G as U, b as Ie, L as te, r as Q, v as O, M as K, k as H, p as Le, x as ke, V as R, o as ee, T as D, J as be, f as Ee, a as ne, c as $, S as ue, K as he, R as Ze, e as We, N as Ne, U as _e, X as ze, Y as re, Z as $e, _ as He, j as De, l as Ge, m as qe, W as Ue, n as Ke, A as Qe, D as Pe, O as Je } from "./Text-B8K0MPNE.js";
import { v as F, P as Oe, g as j, o as pe } from "./theme-CzzIlc4y.js";
...
function nt(e, t, r) {
  ...
  const d = new Q({ vertexColors: true, transparent: true, opacity: s.shellOpacity, side: O, depthWrite: false }), u = new K(new k(), d);
...
function pt(e, t, r, s) {
  ...
  function u(M, y, c, o) {
    const i = c ?? y * 0.08, l = o ?? M * 0.07, h = M / 2, x = y / 2, S = x - i, V = l / 2, Y = [];
    function w(X, W, N, _) {
    ...
    const T = new Float32Array([0, -h, -x, 0, h, -x, 0, h, -S, 0, V, -S, 0, V, S, 0, h, S, 0, h, x, 0, -h, x, 0, -h, S, 0, -V, S, 0, -V, -S, 0, -h, -S, 0, -h, -x]), B = new k();
  ...
  function b(M, y, c) {
    const o = M / 2, i = y / 2, l = o - c, h = i - c, x = new k(), S = new Float32Array([0, -l, -h, 0, l, -h, 0, l, h, 0, -l, -h, 0, l, h, 0, -l, h]);
  ...
  function g(M, y, c, o) {
    ...
    for (const P of S) l.push(...h[P]);
    for (const P of S) l.push(...x[P]);
    const V = new k();
  ...
  function A(M, y, c, o, i) {
    const l = y / 2, h = i / 2, x = [], S = [[0, -M, -l], [0, -M, -l + c], [0, -h - o, -l + c], [0, -h - o, l - c], [0, -M, l - c], [0, -M, l], [0, -h, l], [0, -h, -l]], V = S.map((B) => [B[0], -B[1], B[2]]), Y = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
    for (const B of Y) x.push(...S[B]);
    for (const B of Y) x.push(...V[B]);
    const w = new k();
    w.setAttribute("position", new $(new Float32Array(x), 3));
    const P = [];
    for (const B of [S, V]) for (let X = 0; X < B.length; X++) {
  ...
  const L = new Q({ color: 52479, transparent: true, opacity: 0.35, side: O, depthWrite: false }), E = new te({ color: 52479 }), Z = new Q({ color: 16750848, transparent: true, opacity: 0.4, side: O, depthWrite: false }), z = new te({ color: 16750848 });
  function C(M, y) {
  ...
  return F.derive(() => {
    ...
    c.forEach((h, x) => {
      ...
      if (!w) return;
      const P = [(S[0] + V[0]) / 2, (S[1] + V[1]) / 2, (S[2] + V[2]) / 2], T = Se(S, V);
      if (w.type === "CFT") {
...
class Te extends U {
  constructor(t, r, s, n, a, d, u) {
    ...
    if (this.text = new D(`${a[0].toFixed(4)}`), this.text2 = new D(`${(a[1] * -1).toFixed(4)}`), this.normalizedResult = d, this.textPosition = ge(t, r), this.text2Position = ge(r, t), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(n), this.text2.rotation.setFromRotationMatrix(n), this.add(this.text, this.text2), b) {
      ...
      const E = new he(f), Z = new he(g), z = new Q({ color: d[0] > 0 ? 24435 : 11411474, side: O }), C = new Q({ color: -d[1] > 0 ? 24435 : 11411474, side: O });
      this.mesh = new K(E, z), this.mesh2 = new K(Z, C), this.mesh.position.set(...t), this.mesh2.position.set(...t), this.mesh.rotation.setFromRotationMatrix(n), this.mesh2.rotation.setFromRotationMatrix(n), u && this.mesh.rotateX(Math.PI / 2), u && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
...
function xt({ drawingObj: e, gridObj: t, scene: r, camera: s, controls: n, gridSize: a, derivedDisplayScale: d, rendererElm: u, viewerRender: p }) {
  const b = new Ze(), f = new We(), g = new K(new Ne(a, a), new Q({ side: O })), m = new ce(new k(), new de()), A = new ce(new k(), new de({ color: "gray" })), v = new ce(new k(), new de({ color: "orange", size: 0.8 }));
  ...
  u.addEventListener("pointermove", (z) => {
    ...
    if (C && L < 5 && (E = true, n.enabled = false, Z = M[0].index), !E || L % 2 !== 0) return;
...
function Mt(e, t, r) {
  const s = new Re(), n = new H(), a = new K(new k(), new Q({ side: O, vertexColors: true }));
...
function Vt(e, t, r, s) {
  ...
  return F.derive(() => {
    ...
    for (const x of p) for (let S = 0; S < 3; S++) E[S] = Math.min(E[S], x[S]), Z[S] = Math.max(Z[S], x[S]);
    const C = Math.max(Z[0] - E[0], Z[1] - E[1], Z[2] - E[2], 1) * Ft, M = [], y = [], c = [];
    let o = 0;
    for (const { idx: x, vals: S } of v) {
      ...
      for (let G = 0; G < N; G++) {
        const J = G / ye, se = Y[0] + P.x * T * J, ie = Y[1] + P.y * T * J, me = Y[2] + P.z * T * J, fe = S[0] + (S[1] - S[0]) * J, ae = a.getColor(fe) ?? new H(0, 0, 0);
        d.copy(ae).convertSRGBToLinear();
        for (let we = 0; we < _; we++) {
          const Ae = we / _ * Math.PI * 2, ve = Math.cos(Ae), xe = Math.sin(Ae);
          M.push(se + (X.x * ve + W.x * xe) * C, ie + (X.y * ve + W.y * xe) * C, me + (X.z * ve + W.z * xe) * C), y.push(d.r, d.g, d.b);
        }
      }
...
function Lt({ mesh: e, settingsObj: t, drawingObj: r, objects3D: s, solids: n }) {
  ...
  r && xt({ drawingObj: r, gridObj: I, scene: d, camera: u, controls: g, gridSize: E, derivedDisplayScale: A, rendererElm: f.domElement, viewerRender: C }), pe((c, o) => {
    f.setClearColor(o.background, 1), d.remove(I), I.geometry.dispose(), I.material.dispose(), I = Ye(m.gridSize.rawVal), d.add(I), a.style.setProperty("--awatif-legend-color", o.legendMarker), C();
  });
...
function It(e, t) {
  ...
  return ((n) => {
    ...
    const Z = { bendingXX: [n, 0], bendingYY: [a, 0], bendingXY: [d, 0], membraneXX: [u, 0], membraneYY: [p, 0], membraneXY: [b, 0], tranverseShearX: [f, 0], tranverseShearY: [g, 0], vonMises: [m, 0], pressure: [A, 0], displacementX: [(_x = (_w = e.deformOutputs) == null ? void 0 : _w.val) == null ? void 0 : _x.deformations, 0], displacementY: [(_z = (_y = e.deformOutputs) == null ? void 0 : _y.val) == null ? void 0 : _z.deformations, 1], displacementZ: [(_B = (_A = e.deformOutputs) == null ? void 0 : _A.val) == null ? void 0 : _B.deformations, 2] }, z = t.shellResults.val, C = Pt.val, M = Yt.val, y = z === "displacementX" || z === "displacementY" || z === "displacementZ", c = z === "bendingXX" || z === "bendingYY" || z === "bendingXY", o = z === "membraneXX" || z === "membraneYY" || z === "membraneXY", i = z === "vonMises" || z === "pressure", l = z === "tranverseShearX" || z === "tranverseShearY", h = y ? Xt[M] : c || o || i || l ? 1 / Tt[C] : 1, x = y ? M : c ? `${C}\xB7m/m` : o ? `${C}/m\xB2` : i ? `${C}/m\xB2` : l ? `${C}/m` : "";
    Fe.val = x;
    const S = [];
