import { v as I, P as Eo, q as cn, a7 as Xn, B as he, a8 as Yn, F as Lt, a4 as Vo, K as st, X as Jt, L as ft, h as Ot, u as To, g as Uo, a9 as Ko, i as at, d as je, V as m, $ as rn, aa as qn, H as Lo, D as Xt, a as Tt, x as dt, z as Dn, ab as Nn, s as Ho, m as Wo, I as an, a2 as Cn, E as yo, f as xn, Q as Jn, ac as Pn, C as xo, S as go, c as vo, ad as In, p as Go, ae as qo, af as Jo, ag as Qo, ah as Oo, b as bo, ai as Mo, e as _o, W as jo, N as es, O as ts, Y as ns, T as $n, o as Qn, Z as os, _ as ko, U as ss } from "./theme-BUyDDEHW.js";
import { T as Pt, O as So } from "./Text-DR6pe57W.js";
...
function On(e, i) {
  ...
  const _ = new Jt(y.grid), P = new Jt(y.grid).multiplyScalar(0.45), b = (se, ae, pe, S) => {
    ...
    const K = new ft({ color: pe, transparent: true, opacity: S, depthWrite: false }), D = new Ot(_e, K);
    ...
    const _e = new ft({ color: ae, transparent: true, opacity: pe, depthWrite: false }), K = new To(ye, _e);
...
function cs(e, i, y, h) {
  ...
  return I.derive(() => {
    ...
    (_b = (_a = e.nodeInputs) == null ? void 0 : _a.val.supports) == null ? void 0 : _b.forEach((ae, pe) => {
      ...
      let _e;
      me >= 3 && ye >= 3 ? _e = new je(k, x) : me >= 3 && ye === 0 ? _e = new je(v, w) : _e = new je(v, _), _e.position.set(S[0], S[1], S[2]);
      const K = se * H();
...
function ms(e, i, y, h) {
  ...
  return k.setAttribute("color", new Lt([...b, ...b, ...H, ...H, ...se, ...se], 3)), I.derive(() => {
    ...
    i.deformedShape.val, i.orientations.val && (u.clear(), (_a = e.elements) == null ? void 0 : _a.val.forEach((ae) => {
      const pe = new Ot(k, v), S = y.rawVal[ae[0]], G = y.rawVal[ae[1]];
      if (ae.length === 2 && (pe.position.set(...to(S, G)), pe.rotation.setFromRotationMatrix(oo(S, G))), ae.length === 3) {
        const _e = y.rawVal[ae[2]];
        pe.position.set(...Fn([S, G, _e])), pe.rotation.setFromRotationMatrix(hs(S, G, _e));
      }
...
class Po extends st {
  constructor(i, y, h, u, k, v, x) {
    ...
    if (this.text = new Pt(`${k[0].toFixed(2)}`), this.text2 = new Pt(`${(k[1] * -1).toFixed(2)}`), this.normalizedResult = v, this.textPosition = to(i, y), this.text2Position = to(y, i), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(u), this.text2.rotation.setFromRotationMatrix(u), this.add(this.text, this.text2), _) {
      const P = new Dn().moveTo(0, 0).lineTo(0, v[0]).lineTo(w, 0).lineTo(0, 0), b = new Dn().moveTo(w, 0).lineTo(h, -v[1]).lineTo(h, 0).lineTo(w, 0), H = P.getPoints(), se = b.getPoints(), ae = new he().setFromPoints(H), pe = new he().setFromPoints(se), S = new ft({ color: cn().resultOutline });
      this.lines = new Tt(ae, S), this.lines2 = new Tt(pe, S), this.lines.position.set(...i), this.lines2.position.set(...i), this.lines.rotation.setFromRotationMatrix(u), this.lines2.rotation.setFromRotationMatrix(u), x && this.lines.rotateX(Math.PI / 2), x && this.lines2.rotateX(Math.PI / 2), this.add(this.lines, this.lines2);
...
function Ms({ drawingObj: e, gridObj: i, scene: y, getActiveCamera: h, controls: u, gridSize: k, derivedDisplayScale: v, rendererElm: x, viewerRender: w }) {
  ...
  Le.add(N, ie, fe);
  const we = (n) => {
    const o = new he().setFromPoints([new m(0, 0, 0), new m(0, 0, 0), new m(0, 0, 0), new m(0, 0, 0)]), a = new ft({ color: n, transparent: true, opacity: 0.2, depthTest: false }), t = new To(o, a);
  ...
  const De = (n, o, a, t) => {
    ...
    for (const l of ze) {
      ...
      if (p === "pt") {
        ...
        if (!M || !c) continue;
        const f = new he().setFromPoints([new m(M[0], M[1], M[2]), new m(c[0], c[1], c[2])]), C = new Tt(f, new ft({ color: Wt, transparent: true, opacity: 0.95, depthTest: false }));
  ...
  me.geometry.setAttribute("position", new Lt(e.points.rawVal.flat(), 3)), me.geometry.computeBoundingSphere(), me.frustumCulled = false, ye.frustumCulled = false, y.add(ye), H.position.set(0, 0, 0), H.rotateX(Math.PI / 2), H.geometry.rotateX(Math.PI / 2), H.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (n, o, a) => {
    ...
    let g;
    if (S && e.gridTarget) {
      const M = e.gridTarget.rawVal, c = new Pn(...M.rotation), f = new m(1, 0, 0).applyEuler(c), C = new m(0, 1, 0).applyEuler(c), L = new m(...M.position), R = new m(t, r, s), Z = new m(l, p, d), E = R.clone().sub(L).dot(f), O = R.clone().sub(L).dot(C), j = Z.clone().sub(L).dot(f), ue = Z.clone().sub(L).dot(C), Te = (xe, ge) => L.clone().addScaledVector(f, xe).addScaledVector(C, ge).toArray();
  ...
  et.visible = false, y.add(et), window.__hekatanShowAxes = (n, o, a = 12, t = 2) => {
    ...
    n.forEach((M, c) => {
      const f = c < d.length ? d[c] : `X${c}`, C = new he().setFromPoints([new m(M, r, 0), new m(M, s, 0), new m(M, r, 0), new m(M, r, a)]), L = new Cn({ color: 6333946, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), R = new Ot(C, L);
      R.computeLineDistances(), et.add(R), et.add(g(f, M, r - 0.5, 0, "#60a5fa")), et.add(g(f, M, s + 0.5, 0, "#60a5fa"));
    }), o.forEach((M, c) => {
      const f = `${c + 1}`, C = new he().setFromPoints([new m(l, M, 0), new m(p, M, 0), new m(l, M, 0), new m(l, M, a)]), L = new Cn({ color: 16478597, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), R = new Ot(C, L);
  ...
  window.__hekatanUpdateSelectionPtScale = Vn, u.addEventListener("change", () => {
    ...
    if (a.length) {
      ...
      if (l === "select" || !l) {
        const M = (window.__hekatanSnap2D ?? 0.5) * 1.5, c = De(t.x, t.y, t.z, M), f = Be(t.x, t.y, t.z, M), C = be(t.x, t.y, t.z, M);
        if (c >= 0) {
          ...
          O && (Se.geometry.setFromPoints([new m(O[0], O[1], O[2]), new m(O[3], O[4], O[5])]), Se.visible = true, Ge.visible = false, vt = { kind: "aux", a: C });
      ...
      if (l === "delete") {
        ...
        C ? (Ye = f, ht = -1, pt = -1, lt(f)) : c ? (ht = c.polyIdx, pt = c.segIdx, Ye = -1, ut(c.polyIdx, c.segIdx)) : (ht = -1, pt = -1, Ye = -1, ke.visible = false), ne.visible = false, Le.visible = false, $(), W.style.left = n.clientX + "px", W.style.top = n.clientY + "px", W.style.display = "block";
        const L = `X=${t.x.toFixed(2)} Y=${t.y.toFixed(2)} Z=${t.z.toFixed(2)}`;
        let R = "";
        C ? R = `\u{1F5D1} l\xEDnea aux #${Ye + 1}` : c ? R = ((_i = (_h = e.areas) == null ? void 0 : _h.rawVal) == null ? void 0 : _i.includes(c.polyIdx)) ?? false ? `\u{1F5D1} \xE1rea #${c.polyIdx + 1}` : `\u{1F5D1} seg ${c.segIdx + 1} / poly #${c.polyIdx + 1}` : R = "\u{1F5D1} acerc\xE1 a l\xEDnea/\xE1rea", W.textContent = `${L}  \xB7  ${R}`;
        const Z = document.getElementById("hk-coord-fixed");
        Z && (Z.textContent = L), w();
        return;
  ...
  const Mt = (n, o, a, t) => {
    ...
    tt = new Eo({ container: Bt, title: M });
    {
      ...
      c.addBinding(At, "dx", { label: "\u0394x (m)", step: 0.1 }), c.addBinding(At, "dy", { label: "\u0394y (m)", step: 0.1 }), c.addBinding(At, "dz", { label: "\u0394z (m)", step: 0.1 }), c.addBinding(At, "copias", { label: "Copias", min: 1, max: 50, step: 1 }), c.addButton({ title: "\u29C9 Replicar selecci\xF3n" }).on("click", () => {
        var _a;
        const C = (_a = window.__hekatanReplicateSelection) == null ? void 0 : _a.call(window, At.dx, At.dy, At.dz, At.copias);
        oe(C ? `\u29C9 Replicado \xD7${C} (\u0394 ${At.dx},${At.dy},${At.dz} m)` : "\u26A0 Nada que replicar \u2014 seleccion\xE1 nodos/frames/\xE1reas");
      }), c.addButton({ title: "\u2192 Mover selecci\xF3n (1 copia, sin duplicar geometr\xEDa base)" }).on("click", () => {
        var _a;
        const C = (_a = window.__hekatanReplicateSelection) == null ? void 0 : _a.call(window, At.dx, At.dy, At.dz, 1);
        oe(C ? `\u2192 Copia desplazada \u0394 ${At.dx},${At.dy},${At.dz} m` : "\u26A0 Nada seleccionado");
      });
    ...
    if (s) {
      ...
      const C = tt.addFolder({ title: "\u2B07 Joint Loads (kN, kN\xB7m)" });
      C.addBinding(Y, "Fx", { step: 0.1 }), C.addBinding(Y, "Fy", { step: 0.1 }), C.addBinding(Y, "Fz", { step: 0.1 }), C.addBinding(Y, "Mx", { step: 0.1 }), C.addBinding(Y, "My", { step: 0.1 }), C.addBinding(Y, "Mz", { step: 0.1 }), tt.addFolder({ title: "\u2696 Additional Mass (kg)", expanded: false }).addBinding(Y, "mass", { label: "m", min: 0, step: 1 }), tt.addFolder({ title: "\u{1F517} Diaphragm (rigid link)", expanded: false }).addBinding(Y, "diaphragm", { label: "Diafragma", options: { Ninguno: "Ninguno", "D1 (rigid)": "D1 (rigid)", "D2 (rigid)": "D2 (rigid)", "D3 (rigid)": "D3 (rigid)" } }), tt.addButton({ title: `\u2713 Aplicar a ${o.length} nodo(s) seleccionado(s)` }).on("click", () => {
        let Z = 0;
...
function Ys(e) {
  ...
  const S = new at({ color: Bn, transparent: true, opacity: 0.95, depthTest: false }), G = new je(y, S);
...
function ea({ mesh: e, settingsObj: i, drawingObj: y, objects3D: h, solids: u }) {
  ...
  if (e) {
    ...
    Je.id = "frame-legend", k.appendChild(Je), I.derive(() => {
      var _a;
      const nt = S.shellResults.val != "none", Ht = (((_a = S.solidResults) == null ? void 0 : _a.val) ?? "none") !== "none", mt = nt || Ht, zt = S.frameResults.val.startsWith("contour:");
