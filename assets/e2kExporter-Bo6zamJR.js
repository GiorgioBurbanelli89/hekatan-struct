...
function Ut(I) {
  ...
  return Ot(D, p, Y, l, b, K, Z, z, ee, te, ae, ie, oe, se, ce, fe, Te, r, h, k, Q);
}
function xt(I) {
  const f = { force: "KN", length: "m" };
  let N = "UX,UY,UZ,RX,RY,RZ";
  const D = /* @__PURE__ */ new Map(), p = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), k = /* @__PURE__ */ new Map(), Q = [], l = [], b = /* @__PURE__ */ new Map(), K = [], Z = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), ae = [], ie = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map();
  let se = "", ce = "";
  for (const r of I) {
    const h = r.trim();
    ...
    switch (se) {
      ...
      case "JOINT": {
        const L = v[0];
        k.set(L, [P(T.get("X")), P(T.get("Y")), P(T.get("Z"))]);
        break;
      }
      case "RESTRAINT": {
        const L = T.get("ADD"), t = T.get("DOF");
        if (L && t) {
      ...
      case "SHELL": {
        const L = v[0], t = T.get("J");
        T.get("SEC"), t && l.push({ name: L, joints: t.split(",") });
        break;
      }
      case "SHELL SECTION": {
        const L = T.get("NAME");
        L && Y.set(L, { material: T.get("MAT") || "", type: T.get("TYPE") || "Shell", thickness: P(T.get("TH")) });
        break;
      }
      case "FRAME": {
        const L = v[0], t = T.get("J");
      ...
      case "LOAD": {
        const L = T.get("ADD");
        L && K.push({ joint: L, fx: P(T.get("UX")), fy: P(T.get("UY")), fz: P(T.get("UZ")), mx: P(T.get("MX")), my: P(T.get("MY")), mz: P(T.get("MZ")) });
        break;
      }
    }
  }
  return Ot(f, N, D, p, Y, k, Q, l, b, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), K, Z, z, ee, te, ae, ie, oe);
}
function Ot(I, f, N, D, p, Y, k, Q, l, b, K, Z, z = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map(), ie = [], oe = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), ce, fe) {
  var _a, _b;
  const Te = [], r = /* @__PURE__ */ new Map(), h = [];
  for (const [A, d] of Y) r.set(A, h.length), Te.push(A), h.push(d);
  const T = [], v = [], L = /* @__PURE__ */ new Map();
  for (const A of k) {
    const d = r.get(A.j1), $ = r.get(A.j2);
    if (d !== void 0 && $ !== void 0) {
      const B = T.length;
      T.push([d, $]), v.push(A.name);
  ...
  const s = T.length - t, a = [];
  for (const A of ie) {
    const d = A.joints.map((C) => r.get(C));
    if (d.some((C) => C === void 0)) continue;
    const $ = T.length;
    T.push([d[0], d[1], d[3], d[2], d[4], d[5], d[7], d[6]]), v.push(A.name), a.push($);
  ...
  for (let A = 0; A < T.length; A++) {
    const d = L.get(A), $ = d ? D.get(d) : null, B = d ? p.get(d) : null;
    if ($ || T[A].length === 2) {
  ...
  for (const [A, d] of ae) {
    const $ = v.indexOf(A);
    if ($ < 0 || T[$].length !== 2) continue;
    i.frameLoads ?? (i.frameLoads = /* @__PURE__ */ new Map()), i.frameLoads.set($, d);
    const B = h[T[$][0]], C = h[T[$][1]], j = [C[0] - B[0], C[1] - B[1], C[2] - B[2]], X = Math.hypot(j[0], j[1], j[2]);
    if (X < 1e-9) continue;
    ...
    G(T[$][0], [d[0] * X / 2, d[1] * X / 2, d[2] * X / 2, V * J[0], V * J[1], V * J[2]]), G(T[$][1], [d[0] * X / 2, d[1] * X / 2, d[2] * X / 2, -V * J[0], -V * J[1], -V * J[2]]);
  }
  for (const A of Z) {
  ...
  return { units: I, dof: f, materials: N, frameSections: D, shellSections: p, nodes: h, nodeNames: Te, nodeNameToIdx: r, elements: T, elementNames: v, elementSections: L, nodeInputs: x, elementInputs: i, sectionShapes: S, info: { nNodes: h.length, nFrames: t, nShells: s, title: `SAP2000 (${t} frames, ${s} shells)` } };
}
function Zt(I) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k, _l, _m;
  const { nodes: f, elements: N, nodeInputs: D, elementInputs: p } = I, Y = { force: "KN", length: "m" };
  ...
  const K = [], Z = (t) => {
    var _a2, _b2, _c2, _d2;
    const s = ((_a2 = p.elasticities) == null ? void 0 : _a2.get(t)) || 0, a = (_b2 = p.poissonsRatios) == null ? void 0 : _b2.get(t), i = ((_c2 = p.shearModuli) == null ? void 0 : _c2.get(t)) || 0, S = a !== void 0 ? a : s > 0 && i > 0 ? Math.max(0, Math.min(0.5, s / (2 * i) - 1)) : 0.2, w = i > 0 ? i : s > 0 ? s / (2 * (1 + S)) : 0, x = ((_d2 = p.densities) == null ? void 0 : _d2.get(t)) || 0;
  ...
  for (const t of K) {
    const s = ((_a = p.areas) == null ? void 0 : _a.get(t)) || 0, a = ((_b = p.momentsOfInertiaZ) == null ? void 0 : _b.get(t)) || 0, i = ((_c = p.momentsOfInertiaY) == null ? void 0 : _c.get(t)) || 0, S = ((_d = p.torsionalConstants) == null ? void 0 : _d.get(t)) || 0, w = ((_e = p.elasticities) == null ? void 0 : _e.get(t)) || 0, x = Z(t).key, A = ((_f = p.shearAreasZ) == null ? void 0 : _f.get(t)) ?? 0, d = ((_g = p.shearAreasY) == null ? void 0 : _g.get(t)) ?? 0, $ = (_h = p.sectionShapes) == null ? void 0 : _h.get(t);
    let B;
  ...
  }
  {
    const t = K.filter((s) => {
      var _a2;
      const a = (_a2 = p.localAngles) == null ? void 0 : _a2.get(s);
      return a !== void 0 && isFinite(a) && Math.abs(a) > 1e-9;
    });
    if (t.length > 0) {
      l('TABLE:  "FRAME LOCAL AXES ASSIGNMENTS 1 - TYPICAL"');
      for (const s of t) l(`   Frame=${s + 1}   Angle=${R(p.localAngles.get(s))}   AdvanceAxes=No`);
      b();
    }
  }
  {
    const t = p.endOffsets, s = K.filter((a) => {
  ...
  if (!se) for (const t of z) {
    const s = ((_i = p.thicknesses) == null ? void 0 : _i.get(t)) || 0.1;
    (_j = p.elasticities) == null ? void 0 : _j.get(t);
    const a = Z(t).key, i = ((_k = p.plateFormulations) == null ? void 0 : _k.get(t)) ?? 0, S = `t${s.toPrecision(6)}_f${i}`;
  ...
  if (z.length > 0) {
    ...
    b();
    const t = p.shellModifiers, s = z.filter((a) => {
      const i = t == null ? void 0 : t.get(a);
  ...
  const h = p.frameLoads, T = /* @__PURE__ */ new Map();
  if ((_m = D.loads) == null ? void 0 : _m.forEach((t, s) => T.set(s, [...t])), h && h.size > 0) {
    const t = (s, a) => {
      const i = T.get(s) ?? [0, 0, 0, 0, 0, 0];
      T.set(s, i.map((S, w) => S - a[w]));
    };
    for (const [s, a] of h) {
      const i = N[s];
  ...
  if (T.size > 0) {
    ...
    for (const [t, s] of T) s.some((a) => Math.abs(a) > 1e-12) && l(`   Joint=${t + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${R(s[0])}   F2=${R(s[1])}   F3=${R(s[2])}   M1=${R(s[3])}   M2=${R(s[4])}   M3=${R(s[5])}`);
    b();
  }
  ...
  if (ee.length > 0) {
    const t = p.solidIncompatible === false ? "No" : "Yes", s = /* @__PURE__ */ new Map();
    for (const a of ee) {
...
function Ht(I, f, N = 0.05) {
  const D = f.map(([p, Y]) => `${(+p).toFixed(4)} ${(+Y).toFixed(5)}`).join("  ");
  return [`  FUNCTION "${I}"  FUNCTYPE "SPECTRUM"  DAMPRATIO ${N}  SPECTYPE "USER"  `, `  FUNCTION "${I}"  TIMEVAL "${D}"  `];
}
function vt(I) {
  const { name: f, func: N, modalCase: D = "Modal", sfX: p = 9.81, sfY: Y = 9.81 } = I, k = [`  LOADCASE "${f}"  TYPE  "Response Spectrum"  MODALCASE  "${D}"  `];
  return p && k.push(`  LOADCASE "${f}"  ACCEL  "U1"  FUNC  "${N}"  SF  ${p}  `), Y && k.push(`  LOADCASE "${f}"  ACCEL  "U2"  FUNC  "${N}"  SF  ${Y}  `), k;
}
function mt(I) {
...
`) ? `\r
` : `
`, D = I.split(/\r?\n/), p = f.name ?? "NEC", Y = Ht(p, f.points, f.dampRatio ?? 0.05), k = f.modalCase ?? "Modal", Q = vt({ name: f.caseName ?? "Sismo NEC", func: p, modalCase: k, sfX: f.sfX, sfY: f.sfY });
  let l = [];
...
function dt(I, f, N) {
  ...
  const p = I.findIndex((k) => k.trim() === "END"), Y = p >= 0 ? p : I.length;
  I.splice(Y, 0, `$ ${f}`, ...N, "");
}
function zt(I, f) {
  ...
  for (const [p, Y] of I) if (!D.includes(p) && Y.length !== 0) {
    N.push(`$ ${p}`);
    for (const k of Y) N.push(k);
    N.push("");
  }
...
function jt(I) {
  ...
  const { nodes: f, elements: N, nodeInputs: D, elementInputs: p, title: Y, units: k } = I, Q = I.shellLoads ?? p.shellSurfaceLoads;
  let l;
  Q instanceof Map && (l = /* @__PURE__ */ new Map(), Q.forEach((e, o) => {
    l.set(o, typeof e == "number" ? { value: e } : e);
  }));
  const b = I.shellAngles ?? p.shellAngles, K = p.cargaDeArea, Z = !!(l && l.size > 0), z = p.selfWeight, ee = p.frameLoads, te = (I.weightMode ?? "auto") === "auto" && z !== void 0, ae = /* @__PURE__ */ new Map(), ie = (e, o) => {
  ...
  if (te) {
    if (ee) for (const [e, o] of ee) {
      ...
      const c = f[n[0]], E = f[n[1]], M = [E[0] - c[0], E[1] - c[1], E[2] - c[2]], u = Math.hypot(M[0], M[1], M[2]);
      if (u < 1e-9) continue;
      const m = [M[0] / u, M[1] / u, M[2] / u], O = u * u / 12, U = [m[1] * o[2] - m[2] * o[1], m[2] * o[0] - m[0] * o[2], m[0] * o[1] - m[1] * o[0]];
      ie(n[0], [o[0] * u / 2, o[1] * u / 2, o[2] * u / 2, O * U[0], O * U[1], O * U[2]]), ie(n[1], [o[0] * u / 2, o[1] * u / 2, o[2] * u / 2, -O * U[0], -O * U[1], -O * U[2]]), oe.add(e);
    }
    if (z && z > 0) {
      const o = p.endOffsets;
      N.forEach((n, c) => {
        var _a2, _b2, _c2;
        const E = ((_a2 = p.densities) == null ? void 0 : _a2.get(c)) ?? 0;
        if (E) {
          if (n.length === 2) {
            const M = ((_b2 = p.areas) == null ? void 0 : _b2.get(c)) ?? 0, u = f[n[0]], m = f[n[1]], O = [m[0] - u[0], m[1] - u[1], m[2] - u[2]];
            ...
            const y = M * U * E * 9.80665 * z;
            ie(n[0], [0, 0, -y / 2, 0, 0, 0]), ie(n[1], [0, 0, -y / 2, 0, 0, 0]);
          } else if (n.length === 4) {
            const M = ((_c2 = p.thicknesses) == null ? void 0 : _c2.get(c)) ?? 0, u = n.map((F) => f[F]);
            let m = 0, O = 0, U = 0;
            for (let F = 0; F < 4; F++) {
              const H = u[F], q = u[(F + 1) % 4];
              m += H[1] * q[2] - H[2] * q[1], O += H[2] * q[0] - H[0] * q[2], U += H[0] * q[1] - H[1] * q[0];
            }
            const g = Math.hypot(m, O, U) / 2, y = M * g * E * 9.80665 * z;
  ...
  }, fe = "N", Te = "MM", r = [], h = (e) => Math.round(e * 1e4) / 1e4, T = (e) => !isFinite(e) || e === 0 ? "0" : Number(e.toPrecision(10)).toString(), v = 1e3, L = 1e3, t = (e) => e * L, s = (e) => e * v, a = (e) => e * v, i = (e) => e * v * L, S = (e) => e * v / L ** 2, w = (e) => e * v / L ** 3, x = /* @__PURE__ */ new Date(), A = `${x.getMonth() + 1}/${x.getDate()}/${x.getFullYear()}  ${x.getHours()}:${String(x.getMinutes()).padStart(2, "0")}:${String(x.getSeconds()).padStart(2, "0")}`;
  ...
  const Ae = 980665e-8, de = (e) => {
    var _a2;
    const o = (_a2 = p.densities) == null ? void 0 : _a2.get(e);
    if (o !== void 0) return o > 100 ? o * Ae : o * 9.80665;
  }, Ie = (e) => {
    var _a2;
    const o = ((_a2 = p.elasticities) == null ? void 0 : _a2.get(e)) ?? 0, n = de(e);
    return `${o}|${n === void 0 ? "-" : n.toFixed(4)}`;
  }, qe = /* @__PURE__ */ new Set();
  (_a = p.elasticities) == null ? void 0 : _a.forEach((e, o) => qe.add(Ie(o)));
  ...
  for (const e of qe) {
    ...
    const M = n !== "-" ? parseFloat(n) : c ? 76.97 : 24, u = S(o), m = w(M), O = (() => {
    ...
    if (c) {
      r.push(`  MATERIAL  "${E}"    TYPE "Steel"    GRADE "Grade 50"    WEIGHTPERVOLUME ${T(m)}`), r.push(`  MATERIAL  "${E}"    SYMTYPE "Isotropic"  E ${h(u)}  U ${U}  A ${g}`);
      const y = 345e3, F = 45e4;
      r.push(`  MATERIAL  "${E}"  FY ${h(S(y))}  FU ${h(S(F))}  FYE ${h(S(y * 1.1))}  FUE ${h(S(F * 1.1))}`);
    } else r.push(`  MATERIAL  "${E}"    TYPE "Concrete"    WEIGHTPERVOLUME ${T(m)}`), r.push(`  MATERIAL  "${E}"    SYMTYPE "Isotropic"  E ${h(u)}  U ${U}  A ${g}`), r.push(`  MATERIAL  "${E}"    FC ${h(S(24e3))}`);
  }
  const Qe = /* @__PURE__ */ new Map();
  {
    ...
    (_b = p.sectionShapes) == null ? void 0 : _b.forEach((n, c) => {
      ...
      if (!(E > 0)) return;
      const M = n.fillE / E, u = de(c) ?? 76.97, m = `${n.fillE}|${(M * u).toFixed(4)}`;
      let O = e.get(m);
      O || (O = `ConcFill_${e.size + 1}`, e.set(m, O), r.push(`  MATERIAL  "${O}"    TYPE "Concrete"    WEIGHTPERVOLUME ${T(w(M * u))}`), r.push(`  MATERIAL  "${O}"    SYMTYPE "Isotropic"  E ${h(S(n.fillE))}  U 0.2  A 1.0e-5`), r.push(`  MATERIAL  "${O}"    FC ${h(S(24e3))}`)), Qe.set(c, O);
    });
  }
  ...
  N.forEach((e, o) => {
    ...
    const n = (_a2 = p.sectionShapes) == null ? void 0 : _a2.get(o), c = ((_b2 = p.elasticities) == null ? void 0 : _b2.get(o)) ?? 0, E = Oe.get(Ie(o)) || "Conc_1", M = ye.get(Ie(o)) ?? c >= 1e8, u = ((_c2 = p.areas) == null ? void 0 : _c2.get(o)) ?? 0, m = ((_d2 = p.momentsOfInertiaZ) == null ? void 0 : _d2.get(o)) ?? 0, O = ((_e3 = p.momentsOfInertiaY) == null ? void 0 : _e3.get(o)) ?? 0, U = ((_f2 = p.torsionalConstants) == null ? void 0 : _f2.get(o)) ?? 0;
    ...
    if (!n && y <= 0 && F <= 0 && H <= 0 && u > 0 && m > 0 && O > 0) {
      const pe = (_g2 = p.cantos) == null ? void 0 : _g2.get(o), Fe = (_h = p.anchos) == null ? void 0 : _h.get(o);
    ...
    if (he === "General") {
      const pe = ((_i = p.shearAreasZ) == null ? void 0 : _i.get(o)) || u * 5 / 6, Fe = ((_j = p.shearAreasY) == null ? void 0 : _j.get(o)) || u * 5 / 6;
      me += `  D ${h(t(y))} B ${h(t(F))} AREA ${T(u * 1e6)} AS2 ${T(pe * 1e6)} AS3 ${T(Fe * 1e6)} I33 ${T(m * 1e12)} I22 ${T(O * 1e12)} TORSION ${T((U || m + O) * 1e12)} S33POS ${T(2 * m / y * 1e9)} S33NEG ${T(2 * m / y * 1e9)} S22POS ${T(2 * O / F * 1e9)} S22NEG ${T(2 * O / F * 1e9)} Z33 ${T(2 * m / y * 1e9)} Z22 ${T(2 * O / F * 1e9)} R33 ${T(Math.sqrt(m / u) * 1e3)} R22 ${T(Math.sqrt(O / u) * 1e3)} `, r.push(me);
      return;
    }
  ...
  const ue = (e) => {
    ...
    const M = (_d2 = p.insertionPoints) == null ? void 0 : _d2.get(e);
    M && (Math.abs(M[0]) > 1e-9 || Math.abs(M[1]) > 1e-9) && o.push(`LATEROFFSET ${h(t(M[0]))} TRANSOFFSET ${h(t(M[1]))}`);
    const u = (_e3 = p.rigidOffsets) == null ? void 0 : _e3.get(e), m = (_f2 = p.endOffsets) == null ? void 0 : _f2.get(e), O = m ? [m[0], m[1]] : u, U = m && m.length > 2 ? m[2] : 0;
  ...
  N.forEach((e, o) => {
    ...
    const M = ue(c), u = ve.get(o) || `Sec_${o}`, m = `${M.pt}_${u}_${n}`;
    Ye.has(m) || Ye.set(m, []), Ye.get(m).push({ i: o, bot: c, top: E, zBot: h(f[c][2]), zTop: h(f[E][2]), planPt: M.pt, secName: u, type: n });
  }), Ye.forEach((e, o) => {
    e.sort((c, E) => c.zBot - E.zBot);
    let n = 0;
    for (let c = 1; c <= e.length; c++) if (c === e.length || Math.abs(e[c].zBot - e[c - 1].zTop) > 1e-6) {
      const M = e.slice(n, c);
      M.length >= 1 && (We.push({ elemIndices: M.map((u) => u.i), planPt: M[0].planPt, bottomNodeIdx: M[0].bot, topNodeIdx: M[M.length - 1].top, secName: M[0].secName, type: M[0].type, nSegments: M.length }), M.forEach((u) => tt.add(u.i))), n = c;
    }
  }), r.push("$ LINE CONNECTIVITIES");
  const st = [], ot = (e) => W.indexOf(e), nt = /* @__PURE__ */ new Map(), at = (e, o, n, c, E, M, u, m) => {
    const O = ue(c), U = ue(n);
    m !== void 0 && nt.set(m, { name: e, story: O.story });
    const g = ot(O.story) - ot(U.story);
    g <= 0 ? r.push(`  LINE  "${e}"  BEAM  "${U.pt}"  "${O.pt}"  0`) : r.push(`  LINE  "${e}"  ${o}  "${U.pt}"  "${O.pt}"  ${g}`), st.push(`  LINEASSIGN  "${e}"  "${O.story}"  SECTION "${E}" ${M} MINNUMSTA ${u} AUTOMESH "YES"  MESHATINTERSECTIONS "${p.meshAtIntersections === false ? "NO" : "YES"}"  `);
  };
  We.forEach((e, o) => {
    ...
    const n = gt(f, e), c = ve.get(o) || `Sec_${o}`, E = et(o), M = f[e[0]][2] <= f[e[1]][2] ? e[0] : e[1], u = f[e[0]][2] <= f[e[1]][2] ? e[1] : e[0];
    at(`E${o + 1}`, n === "BEAM" ? "BRACE" : n, M, u, c, E, 3, o);
  }), r.push("");
  ...
  it && We.forEach((e) => {
    ...
    const M = ue(o), u = `${M.pt}@${M.story}`;
    Re.has(u) || (r.push(`  POINTASSIGN  "${M.pt}"  "${M.story}"  DIAPH "DISCONNECTED"  `), Re.add(u));
  }), r.push(""), r.push("$ LINE ASSIGNS"), st.forEach((e) => r.push(e)), r.push("");
  const Ee = [], ct = p.areaObjects, rt = /* @__PURE__ */ new Set(), Et = /* @__PURE__ */ new Map(), lt = /* @__PURE__ */ new Map();
  ct == null ? void 0 : ct.forEach((e) => e.cells.forEach((o) => rt.add(o))), N.forEach((e, o) => {
    if (e.length === 4 || e.length === 3) {
      const n = f[e[0]], c = f[e[1]], E = f[e[2]], M = [c[0] - n[0], c[1] - n[1], c[2] - n[2]], u = [E[0] - n[0], E[1] - n[1], E[2] - n[2]], m = M[1] * u[2] - M[2] * u[1], O = M[2] * u[0] - M[0] * u[2], U = M[0] * u[1] - M[1] * u[0], g = Math.sqrt(m * m + O * O + U * U), y = g > 1e-10 && Math.abs(U) / g < 0.5;
  ...
  const je = [], ft = (e) => {
    ...
    const c = p.membraneModifiers, E = p.bendingModifiers, M = c == null ? void 0 : c.get(e), u = E == null ? void 0 : E.get(e);
    if (M === void 0 && u === void 0) return null;
    const m = M ?? 1, O = u ?? 1;
    ...
    const E = c.values().next().value, M = At.map((u, m) => Math.abs(E[m] - 1) > 1e-9 ? `${u} ${h(E[m])}` : "").filter(Boolean);
  ...
  for (const e of Ee) {
    ...
    const n = e.isWall, c = !n && _e(e.idx), E = n ? ++Pt : c ? ++Ft : ++Ct, M = Ie(e.idx);
    Pe.set(o, { nombre: (n ? "Muro" : c ? De : "Losa") + (E === 1 ? "" : String(E)), isWall: n, mem: c, t: pt == null ? void 0 : pt.get(e.idx), pf: Tt == null ? void 0 : Tt.get(e.idx), mat: Oe.get(M) ?? Me, acero: ye.get(M) ?? false });
  }
  const ke = (e) => {
    var _a2;
    return ((_a2 = Pe.get(Ce(e))) == null ? void 0 : _a2.nombre) ?? (e.isWall ? "Muro" : "Losa");
  }, Mt = (e) => e === 2 ? "Membrane" : e === 1 ? "ShellThin" : "ShellThick", yt = (e, o) => {
    const n = Ee.find((M) => Ce(M) === o), c = n ? Ge(n.idx) ?? null : null;
    if (!c) return "";
    const E = At.map((M, u) => Math.abs(c[u] - 1) > 1e-9 ? `${M} ${h(c[u])}` : "").filter(Boolean);
  ...
  if (Ee.length > 0) {
    ...
    const e = [];
    Ee.forEach((o, n) => {
      const { el: c, isWall: E } = o, M = E ? `W${n + 1}` : `F${n + 1}`, u = E ? "PANEL" : "FLOOR", m = c.map((O) => ue(O));
      if (E) {
        const O = (H) => W.indexOf(H);
        if (new Set(m.map((H) => H.pt)).size === 4) {
          const H = Math.max(...m.map((Se) => O(Se.story))), q = m.map((Se) => H - O(Se.story));
          r.push(`  AREA "${M}"  ${u}  4  "${m[0].pt}"  "${m[1].pt}"  "${m[2].pt}"  "${m[3].pt}"  ${q.join("  ")}  `), e.push(`  AREAASSIGN  "${M}"  "${W[H]}"  SECTION "${ke(o)}"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "MIDDLE"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
          return;
        }
        const g = f[c[0]][2] <= f[c[2]][2] ? 0 : 2, y = f[c[1]][2] <= f[c[3]][2] ? 1 : 3;
        r.push(`  AREA "${M}"  ${u}  4  "${m[g].pt}"  "${m[y].pt}"  "${m[y].pt}"  "${m[g].pt}"  1  1  0  0  `);
        const F = m[g === 0 ? 2 : 0].story;
        e.push(`  AREAASSIGN  "${M}"  "${F}"  SECTION "${ke(o)}"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "MIDDLE"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
      } else {
        const O = m.length, U = (q) => W.indexOf(q), g = Math.max(...m.map((q) => U(q.story))), y = m.map((q) => g - U(q.story)), F = W[g] ?? m[0].story;
        r.push(`  AREA "${M}"  ${u}  ${O}  ` + m.map((q) => `"${q.pt}"`).join("  ") + "  " + y.join("  ") + "  ");
        const H = lt.get(o.idx) ?? (b == null ? void 0 : b.get(o.idx));
        e.push(_e(o.idx) ? `  AREAASSIGN  "${M}"  "${F}"  SECTION "${ke(o)}"  ANG ${h(H ?? 0)} OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "No"  CARDINALPOINT "MIDDLE"  TRANSFORMSTIFFNESSFOROFFSETS "No"  ` : `  AREAASSIGN  "${M}"  "${F}"  SECTION "${ke(o)}" ${it ? ' DIAPH  "D1" ' : ""} OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "TOP"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `), je.push({ name: M, story: F, idx: o.idx });
      }
  ...
  if (D.loads && D.loads.size > 0 && D.loads.forEach((e, o) => {
    const [n, c, E] = se(o, e), [M, u, m] = ce(o, e);
    ut(o, [n, c, Bt ? E : 0, M, u, m]);
  }), D.moments && D.moments.size > 0 && D.moments.forEach((e, o) => {
    ut(o, [0, 0, 0, e[0] ?? 0, e[1] ?? 0, e[2] ?? 0]);
  }), Ze.forEach((e, o) => {
    if (e.every((c) => Math.abs(c) <= 1e-10)) return;
    const n = ue(o);
    Ke.push(`  POINTLOAD  "${n.pt}"  "${n.story}"  TYPE "FORCE"  LC "${Ne}"  FX ${T(a(e[0]))}  FY ${T(a(e[1]))}  FZ ${T(a(e[2]))}  MX ${T(i(e[3]))}  MY ${T(i(e[4]))}  MZ ${T(i(e[5]))}`);
  ...
  if (l && l.size > 0 && je.length > 0) {
    ...
    for (const o of je) {
      ...
      const E = c.dir ?? "GRAV", M = E === "GRAV" ? Math.abs(c.value) : c.value;
      e.push(`  AREALOAD  "${o.name}"  "${o.story}"  TYPE "UNIFF"  DIR "${E}"  LC "${c.pattern ?? Ne}"  FVAL ${T(s(M) / (L * L))}`);
    }
...
function gt(I, f) {
  const N = I[f[0]], D = I[f[1]], p = Math.abs(D[2] - N[2]), Y = Math.sqrt((D[0] - N[0]) ** 2 + (D[1] - N[1]) ** 2), k = p > Y * 0.5;
