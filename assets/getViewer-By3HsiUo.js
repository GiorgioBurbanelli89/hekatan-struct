import { Y as cn, B as fe, Z as dn, F as nt, G as Ne, c as Tt, L as Oe, e as Ke, D as yt, d as Ze, u as it, i as eo, b as po, V as M, z as Xt, H as lt, _ as Mn, n as to, a as ht, j as Ue, l as pn, $ as fn, g as fo, f as uo, s as on, N as Jt, S as Nt, a0 as Bn, o as Rn, q as Dn, r as Nn, a1 as Zn, a2 as sn, a3 as ho, a4 as mo, a5 as wo, a6 as xo, a7 as yo, p as Un, a8 as Kn, C as Hn, t as go, v as vo, w as bo, W as Mo, x as Wn, a9 as an, J as _n, A as _o, y as Gn, O as So } from "./Text-BmY6zyQy.js";
import { v as X, P as no, g as $t, o as un } from "./theme-2eEBQPmF.js";
...
function zo(e, s, w) {
  const p = $t(), r = new cn(new fe(), new dn({ color: p.nodePoint }));
  return un((g, x) => {
...
function Po(e, s, w) {
  const p = $t(), r = new Ne(), g = new Tt(new fe(), new Oe({ color: p.elementLine }));
  un((q, O) => {
  ...
  return X.derive(() => {
    ...
    const q = s.elemColumns.rawVal, O = s.elemBeams.rawVal, J = w.val, we = ((_a = e.elements) == null ? void 0 : _a.val) || [], k = we.filter((N) => {
      ...
      const E = v(H, U);
      return !(E && !q || !E && !O);
    }).map((N) => Fo(N).map((H) => [...J[H[0]], ...J[H[1]]]).flat()).flat();
...
function Sn(e, s) {
  ...
  const C = new it(w.grid), z = new it(w.grid).multiplyScalar(0.45), v = (O, J, we, k) => {
    ...
    const H = new Oe({ color: we, transparent: true, opacity: k, depthWrite: false }), U = new Tt(N, H);
    ...
    const N = new Oe({ color: J, transparent: true, opacity: we, depthWrite: false }), H = new eo(be, N);
...
function Vo(e, s, w, p) {
  const r = new Ne(), g = new po(0.5, 0.5, 0.5), x = new Ke({ color: 10166822 });
  return X.derive(() => {
    var _a, _b;
...
function Eo(e, s, w, p) {
  ...
  return X.derive(() => {
    ...
    (_a = e.elements) == null ? void 0 : _a.val.forEach((x, f) => {
      ...
      b.position.set(...Lo(x.map((C) => w.rawVal[C]))), b.updateScale(g * p.rawVal), r.add(b);
    });
  }), X.derive(() => {
...
function qn(e, s) {
  const w = new Ne(), p = 0.05 * e * 1, r = $t(), g = new lt("X", "red", "transparent"), x = new lt(s ? "Z" : "Y", "green", "transparent"), f = new lt(s ? "Y" : "Z", "blue", "transparent"), b = new Xt(new M(1, 0, 0), new M(0, 0, 0), 1, r.axisArrow, 0.2, 0.2), C = new Xt(new M(0, 1, 0), new M(0, 0, 0), 1, r.axisArrow, 0.2, 0.2), z = new Xt(new M(0, 0, 1), new M(0, 0, 0), 1, r.axisArrow, 0.2, 0.2);
...
function Fn(e, s) {
  ...
  let z = new Mn().fromArray([[x, f, b], [-f / C, x / C, 0], [-x * b / C, -f * b / C, C]].flat());
  return b === 1 && (z = new Mn().fromArray([[0, 0, 1], [0, 1, 0], [-1, 0, 0]].flat())), b === -1 && (z = new Mn().fromArray([[0, 0, -1], [0, 1, 0], [1, 0, 0]].flat())), new to().setFromMatrix3(z);
}
function zn(e, s) {
...
function $o(e, s, w) {
  const p = Ot([s, w]), r = Ot([e, w]), g = Ot([e, s]), x = new M(...p).sub(new M(...r)).normalize(), f = new M(...w).sub(new M(...g)).normalize(), b = x.clone().cross(f).normalize(), C = b.clone().cross(x).normalize();
  return new to().makeBasis(x, C, b);
}
function Io(e, s, w, p) {
  ...
  return g.setAttribute("color", new nt([...v, ...v, ...q, ...q, ...O, ...O], 3)), X.derive(() => {
    ...
    s.deformedShape.val, s.orientations.val && (r.clear(), (_a = e.elements) == null ? void 0 : _a.val.forEach((J) => {
      const we = new Tt(g, x), k = w.rawVal[J[0]], j = w.rawVal[J[1]];
      if (J.length === 2 && (we.position.set(...zn(k, j)), we.rotation.setFromRotationMatrix(Fn(k, j))), J.length === 3) {
        const N = w.rawVal[J[2]];
        we.position.set(...Ot([k, j, N])), we.rotation.setFromRotationMatrix($o(k, j, N));
      }
...
function Yo(e, s, w, p) {
  ...
  return X.derive(() => {
    ...
    L.forEach((P, B) => {
      ...
      if (!F) return;
      const oe = [(te[0] + Z[0]) / 2, (te[1] + Z[1]) / 2, (te[2] + Z[2]) / 2], ie = Fn(te, Z);
      if (F.type === "CFT") {
...
class ln extends Ne {
  constructor(s, w, p, r, g, x, f) {
    ...
    this.mesh = new Ze(v, q), this.mesh.position.set(...s), this.mesh.rotation.setFromRotationMatrix(r), f && this.mesh.rotateX(Math.PI / 2), this.add(this.mesh), this.text = new lt(`${g[1].toFixed(4)}`), this.normalizedResult = x, this.textPosition = Ot([s, w]), this.text.position.set(...this.textPosition), this.text.rotation.setFromRotationMatrix(r), this.add(this.text);
  }
  updateScale(s) {
...
class Jn extends Ne {
  constructor(s, w, p, r, g, x, f) {
    ...
    if (this.text = new lt(`${g[0].toFixed(4)}`), this.text2 = new lt(`${(g[1] * -1).toFixed(4)}`), this.normalizedResult = x, this.textPosition = zn(s, w), this.text2Position = zn(w, s), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(r), this.text2.rotation.setFromRotationMatrix(r), this.add(this.text, this.text2), C) {
      const z = new pn().moveTo(0, 0).lineTo(0, x[0]).lineTo(b, 0).lineTo(0, 0), v = new pn().moveTo(b, 0).lineTo(p, -x[1]).lineTo(p, 0).lineTo(b, 0), q = z.getPoints(), O = v.getPoints(), J = new fe().setFromPoints(q), we = new fe().setFromPoints(O), k = new Oe({ color: $t().resultOutline });
...
function Bo(e, s, w, p) {
  const r = new Ne(), g = { normals: ln, shearsY: ln, shearsZ: ln, torsions: ln, bendingsY: Jn, bendingsZ: Jn };
  return X.derive(() => {
    var _a, _b;
    if (s.deformedShape.val, w.val, s.frameResults.val == "none") return;
    r.children.forEach((f) => f.dispose()), r.clear();
    const x = oo[s.frameResults.rawVal];
    (_b = (_a = e.analyzeOutputs) == null ? void 0 : _a.rawVal[x]) == null ? void 0 : _b.forEach((f, b) => {
      var _a2, _b2;
      const C = ((_a2 = e.elements) == null ? void 0 : _a2.rawVal[b]) ?? [0, 1], z = w.rawVal[C[0]], v = w.rawVal[C[1]], q = new M(...v).distanceTo(new M(...z)), O = Ro((_b2 = e.analyzeOutputs) == null ? void 0 : _b2.rawVal[x]), J = f == null ? void 0 : f.map((xe) => xe / (O === 0 ? 1 : O)), we = Fn(z, v), k = new g[x](z, v, q, we, f ?? [0, 0], J ?? [0, 0], !!["normals", "shearsZ", "torsions", "bendingsY"].includes(x)), j = 0.05 * s.gridSize.rawVal;
...
function No(e, s, w, p) {
  ...
  return X.derive(() => {
    ...
    r.children.forEach((f) => f.dispose()), r.clear();
    const g = Vn[s.nodeResults.rawVal], x = 0.05 * s.gridSize.val;
    (_b = (_a = e.deformOutputs) == null ? void 0 : _a.val[g]) == null ? void 0 : _b.forEach((f, b) => {
      const C = new Do(w.rawVal[b], g, f ?? [0, 0, 0, 0, 0, 0]);
...
function Zo({ drawingObj: e, gridObj: s, scene: w, getActiveCamera: p, controls: r, gridSize: g, derivedDisplayScale: x, rendererElm: f, viewerRender: b }) {
  const C = new fo(), z = new uo(), v = (n) => {
  ...
  const F = new ht(new fe().setFromPoints([new M(0, 0, 0), new M(0, 0, 0)]), new Jt({ color: 2282478, dashSize: 0.2, gapSize: 0.1, transparent: true, opacity: 0.85, linewidth: 2 }));
  F.frustumCulled = false, F.visible = false, w.add(F);
  const oe = new Ne();
  oe.frustumCulled = false, oe.visible = false, w.add(oe);
  const ie = (n) => {
    const o = new fe().setFromPoints([new M(0, 0, 0), new M(0, 0, 0)]), a = new Jt({ color: n, dashSize: 0.15, gapSize: 0.08, transparent: true, opacity: 0.5, linewidth: 1 });
    return new ht(o, a);
  }, ye = ie(16711680), ue = ie(65280), Ce = ie(35071);
  oe.add(ye, ue, Ce);
  const re = (n) => {
    const o = new fe().setFromPoints([new M(0, 0, 0), new M(0, 0, 0), new M(0, 0, 0), new M(0, 0, 0)]), a = new Oe({ color: n, transparent: true, opacity: 0.45, depthTest: false }), t = new eo(o, a);
  ...
  j.geometry.setAttribute("position", new nt(e.points.rawVal.flat(), 3)), j.geometry.computeBoundingSphere(), j.frustumCulled = false, xe.frustumCulled = false, w.add(xe), q.position.set(0, 0, 0), q.rotateX(Math.PI / 2), q.geometry.rotateX(Math.PI / 2), q.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (n, o, a) => {
    ...
    for (let Ye = 0; Ye <= h; Ye++) {
      const Ge = Ye / h, Mt = Me * Ge, _t = new Bn().setFromAxisAngle(ut, Mt), xt = Se.clone().applyQuaternion(_t).add(I);
  ...
  st.visible = false, w.add(st), window.__hekatanShowAxes = (n, o, a = 12, t = 2) => {
    ...
    const h = Math.min(...o) - t, d = Math.max(...o) + t, S = Math.min(...n) - t, i = Math.max(...n) + t, l = "ABCDEFGHIJKLMNOPQRSTUVWXYZ", u = (y, c, m, _, D) => {
      ...
      const I = new Rn(G), ee = new Dn({ map: I, transparent: true }), Se = new Nn(ee);
      return Se.position.set(c, m, _), Se.scale.set(1.2, 0.6, 1), Se;
    };
    n.forEach((y, c) => {
      const m = c < l.length ? l[c] : `X${c}`, _ = new fe().setFromPoints([new M(y, h, 0), new M(y, d, 0), new M(y, h, 0), new M(y, h, a)]), D = new Jt({ color: 6333946, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), G = new Tt(_, D);
      G.computeLineDistances(), st.add(G), st.add(u(m, y, h - 0.5, 0, "#60a5fa")), st.add(u(m, y, d + 0.5, 0, "#60a5fa"));
    }), o.forEach((y, c) => {
      const m = `${c + 1}`, _ = new fe().setFromPoints([new M(S, y, 0), new M(i, y, 0), new M(S, y, 0), new M(S, y, a)]), D = new Jt({ color: 16478597, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), G = new Tt(_, D);
  ...
  window.__hekatanShowRefPlanes = (n = [0, 3, 6, 9, 12], o = 20, a = 0, t = 0) => {
    ...
    n.forEach((d, S) => {
      ...
      const D = new Rn(m), G = new Dn({ map: D, transparent: true }), Q = new Nn(G);
      ...
      Se.position.set(0, 0, d), Se.frustumCulled = false, Se.userData = { refPlaneZ: d }, w.add(Se), Pt.push(Se);
    }), St.visible = true, b();
  ...
  const Yt = () => {
    ...
    for (const a of o) {
      ...
      const t = new fe().setFromPoints([new M(a[0], a[1], a[2]), new M(a[3], a[4], a[5])]), h = new Jt({ color: 2282478, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.8 }), d = new ht(t, h);
  ...
  X.derive(() => {
    ...
    (n == null ? void 0 : n.val) !== void 0 && (n.val, jt(), b());
  ...
  const je = new Ne(), hn = new Ze(new Nt(0.02, 12, 12), new Ke({ color: 16724804, transparent: true, opacity: 0.95 })), mn = new Ze(new Nt(0.04, 12, 12), new Ke({ color: 16498468, transparent: true, opacity: 0.25, depthWrite: false }));
  je.add(hn, mn);
  ...
  window.__hekatanUpdateSelectionPtScale = en, r.addEventListener("change", () => {
    ...
    if (a.length) {
      ...
      if (S === "delete") {
        ...
        _ ? (rt = m, Be = -1, vt = -1, Et(m)) : c ? (Be = c.polyIdx, vt = c.segIdx, rt = -1, zt(c.polyIdx, c.segIdx)) : (Be = -1, vt = -1, rt = -1, et.visible = false), F.visible = false, oe.visible = false, A(), Z.style.left = n.clientX + "px", Z.style.top = n.clientY + "px", Z.style.display = "block";
      ...
      if (l.length > 0 && u[l[l.length - 1]]) {
        ...
        if (_) {
          ...
          const _t = !!ze, vn = { x: "#ff3344", y: "#34d399", z: "#60a5fa" }[_];
          ...
          let qt = null;
          bn === $ ? qt = "xy" : bn === W ? qt = "xz" : bn === K && (qt = "yz");
          const Yn = qt ? ` (plano ${qt.toUpperCase()})` : "";
          Pe.textContent = _t ? `\u{1F512} LOCK ${_.toUpperCase()}${Yn}` : `\u22A5 ORTO ${_.toUpperCase()}${Yn}`, Pe.style.left = n.clientX + 20 + "px", Pe.style.top = n.clientY + 18 + "px", Pe.style.transform = "none", Pe.style.display = "block";
      ...
      b();
    } else xn(), Z.style.display = "none", je.visible = false, F.visible = false, oe.visible = false, A(), b();
  }), X.derive(() => {
    e.gridTarget && (Uo(s, { position: new M(...e.gridTarget.val.position), quaternion: new Bn().setFromEuler(new Zn(...e.gridTarget.val.rotation)) }, b), q.position.set(...e.gridTarget.val.position), q.quaternion.setFromEuler(new Zn(...e.gridTarget.val.rotation)), q.updateMatrixWorld());
  }), X.derive(() => {
  ...
  const tt = (n, o, a, t) => {
    ...
    if (De = new no({ container: wt, title: y }), d) {
  ...
  const lo = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496 }, $n = (n, o, a, t) => {
    ...
    const h = lo[n] ?? 16777215, d = 0.05, S = new fe().setFromPoints([new M(o - d, a - d, t), new M(o + d, a - d, t), new M(o + d, a - d, t), new M(o + d, a + d, t), new M(o + d, a + d, t), new M(o - d, a + d, t), new M(o - d, a + d, t), new M(o - d, a - d, t)]);
  ...
  window.__hekatanOsnapCompute = ro, window.__hekatanOsnapShow = $n, window.__hekatanOsnapHide = xn;
  let $e = [], dt = 0;
  const Wt = document.createElement("div");
  ...
  const Gt = [], It = () => {
    ...
    e.points.val = n.p, e.polylines && (e.polylines.val = n.l), e.areas && (e.areas.val = n.a), $e = [], F.visible = false, oe.visible = false, A(), ve(`\u21B6 Undo \u2014 ${Gt.length} estados restantes`);
    try {
      (_a = window.__hekatanRebuild) == null ? void 0 : _a.call(window);
    } catch {
    }
  ...
  window.__hekatanFinalizeDraw = Xn, f.addEventListener("click", (n) => {
    ...
    if (S === "select" || S === "none" || !S) {
      if (Le) {
        ...
        gt ? (Tn(gt.x, gt.y, l, u, i), gt = null) : i || (gt = { x: l, y: u }, ve("\u{1F5B1} Click 2 para cerrar el rect\xE1ngulo (\u2192 derecha=Window azul, \u2190izquierda=Crossing verde). Esc=cancelar."), wn(l, u, l + 1, u + 1, false));
      }
      return;
    }
    if (S === "axis") {
  ...
  f.addEventListener("pointermove", (n) => {
    ...
    if (a && Ct < 5 && (yn = true, r.enabled = false, gn = t[0].index), !yn || Ct % 2 !== 0) return;
    const d = [...e.points.rawVal];
    if (gn !== void 0) {
      let S = h[0].point;
      (n.ctrlKey || n.metaKey) && (S = new M(Math.round(S.x), Math.round(S.y), Math.round(S.z))), d[gn] = S.toArray();
    }
    e.points.val = d;
  }), f.addEventListener("pointerup", () => {
    r.enabled = true, yn = false;
  }), f.addEventListener("contextmenu", (n) => {
    var _a;
    const o = v(n);
    if (!o) return;
...
function Ko(e) {
  ...
  for (let w = 0; w < Qt.length - 1; w++) {
    const [p, r, g, x] = Qt[w], [f, b, C, z] = Qt[w + 1];
    if (e <= f) {
      const v = (e - p) / (f - p);
...
function Ho() {
  ...
  for (let p = 0; p < 256; p++) {
    const r = p / 255, [g, x, f] = Ko(r);
    s[p * 4 + 0] = g, s[p * 4 + 1] = x, s[p * 4 + 2] = f, s[p * 4 + 3] = 255;
  }
  const w = new xo(s, 256, 1, yo);
...
function Wo(e, s, w) {
  ...
  return g.renderOrder = -1, g.frustumCulled = false, g.userData.isShellArea = true, g.name = "__hekatan_shell_colormap", X.derive(() => {
    ...
    g.geometry.setIndex(new wo(x, 1));
...
function jo(e, s, w, p) {
  const r = new Ne(), g = new so();
  g.setColorMap("rainbow");
  const x = new it(), f = X.state([]);
  return X.derive(() => {
    var _a, _b, _c;
    s.deformedShape.val;
    const b = w.val, C = ((_a = e.elements) == null ? void 0 : _a.val) ?? [], z = Qo(s.frameResults.val);
    ...
    const N = Math.max(xe[0] - j[0], xe[1] - j[1], xe[2] - j[2], 1) * Jo, H = [], U = [], E = [];
    let L = 0;
    for (const { idx: P, vals: B } of J) {
      ...
      F.normalize();
      const ie = Math.abs(F.y) < 0.99 ? new M(0, 1, 0) : new M(1, 0, 0), ye = new M().crossVectors(F, ie).normalize(), ue = new M().crossVectors(F, ye).normalize(), Ce = Cn + 1, re = qo;
      for (let ge = 0; ge < Ce; ge++) {
        const Te = ge / Cn, ae = Z[0] + F.x * oe * Te, pe = Z[1] + F.y * oe * Te, T = Z[2] + F.z * oe * Te, $ = B[0] + (B[1] - B[0]) * Te, W = g.getColor($) ?? new it(0, 0, 0);
      ...
      for (let ge = 0; ge < Cn; ge++) for (let Te = 0; Te < re; Te++) {
...
function ls(e) {
  ...
  const w = new Nt(1, 16, 16), p = new Ke({ color: ss, transparent: true, opacity: 0.85, depthTest: false }), r = new Ze(w, p);
  r.visible = false, r.renderOrder = 100, s.add(r);
  const g = new fe(), x = new Oe({ color: Qn, linewidth: 4, transparent: true, opacity: 0.9, depthTest: false }), f = new Tt(g, x);
  f.visible = false, f.renderOrder = 100, s.add(f);
  const b = new Ke({ color: Qn, transparent: true, opacity: 0.7, depthTest: false }), C = new Ze(new Hn(1, 1, 1, 12), b);
  C.visible = false, C.renderOrder = 100, s.add(C);
  const z = new fe(), v = new Ke({ color: as, transparent: true, opacity: 0.45, side: yt, depthTest: false }), q = new Ze(z, v);
  q.visible = false, q.renderOrder = 100, s.add(q);
  const O = new fe(), J = new Oe({ color: is, linewidth: 3, transparent: true, opacity: 0.95, depthTest: false }), we = new Tt(O, J);
  ...
  const xe = new Ke({ color: rn, transparent: true, opacity: 0.85, depthTest: false }), be = new Ze(new Hn(1, 1, 1, 12), xe);
  ...
  const E = new fe(), L = new Oe({ color: rn, linewidth: 4, transparent: true, opacity: 1, depthTest: false }), V = new Tt(E, L);
    ...
    for (let me = 0; me < se.length; me++) {
      ...
      if (!(!_e2 || _e2.length < 2)) {
        if (_e2.length === 2) {
          ...
          const de = rs(W, K, le.x, le.y, Ee.x, Ee.y);
...
nodos: [${me.join(", ")}]`, rt === "shell" && ((_l = e.mesh) == null ? void 0 : _l.analyzeOutputs)) {
        const Re = e.mesh.analyzeOutputs.rawVal, Le = os[Fe.stressUnit] ?? 1, bt = [["bendingXX", "Mxx", He, `${Fe.forceUnit}\xB7m/m`], ["bendingYY", "Myy", He, `${Fe.forceUnit}\xB7m/m`], ["bendingXY", "Mxy", He, `${Fe.forceUnit}\xB7m/m`], ["membraneXX", "Nxx", He, `${Fe.forceUnit}/m`], ["membraneYY", "Nyy", He, `${Fe.forceUnit}/m`], ["membraneXY", "Nxy", He, `${Fe.forceUnit}/m`], ["shearX", "Qx", He, `${Fe.forceUnit}/m`], ["shearY", "Qy", He, `${Fe.forceUnit}/m`], ["vonMises", "\u03C3VM", Le, Fe.stressUnit], ["pressure", "p", Le, Fe.stressUnit]], Ve = [];
        for (const [Ie, ot, ct, Et] of bt) {
          const zt = Re == null ? void 0 : Re[Ie];
      ...
      if (rt === "frame" && ((_m = e.mesh) == null ? void 0 : _m.deformOutputs) && e.mesh.elementInputs) {
        ...
        if (bt && me.length === 2) {
          ...
          if (Ve && Ie && ot && ct) {
            ...
            if (pt > 1e-9) {
              const st = Et / pt, St = zt / pt, Pt = mt / pt, Ft = (Ie[0] - Ve[0]) * st + (Ie[1] - Ve[1]) * St + (Ie[2] - Ve[2]) * Pt, Yt = ((_n2 = Le.elasticities) == null ? void 0 : _n2.get(Be)) ?? 0, Lt = ((_o2 = Le.areas) == null ? void 0 : _o2.get(Be)) ?? 0, jt = ((_p = Le.momentsOfInertiaY) == null ? void 0 : _p.get(Be)) ?? 0, je = ((_q = Le.momentsOfInertiaZ) == null ? void 0 : _q.get(Be)) ?? 0, hn = ((_r = Le.torsionalConstants) == null ? void 0 : _r.get(Be)) ?? 0, mn = ((_s = Le.shearModuli) == null ? void 0 : _s.get(Be)) ?? Yt / 2.6, Vt = Yt * Lt * (Ft / pt), Zt = (Ie[3] - Ve[3]) * st + (Ie[4] - Ve[4]) * St + (Ie[5] - Ve[5]) * Pt, Ut = mn * hn * (Zt / pt), Kt = Ie[4] - Ve[4], en = Ie[5] - Ve[5], Bt = Yt * jt * Kt / pt, Ct = Yt * je * en / pt;
              le += `
\u2500\u2500\u2500\u2500 frame \u2500\u2500\u2500\u2500`, le += `
L = ${Qe(pt, 3)} m`, le += `
\u0394L = ${Qe(Ft * Je, 3)} ${Fe.dispUnit}`, le += `
\u03B5 = ${Qe(Ft / pt, 6)}`, Math.abs(Vt) > 1e-6 && (le += `
N \u2248 ${Qe(Vt * He)} ${Fe.forceUnit}`), Math.abs(Ut) > 1e-6 && (le += `
T \u2248 ${Qe(Ut * He)} ${Fe.forceUnit}\xB7m`), Math.abs(Bt) > 1e-6 && (le += `
My \u2248 ${Qe(Bt * He)} ${Fe.forceUnit}\xB7m`), Math.abs(Ct) > 1e-6 && (le += `
...
function On(e, s = 8) {
  ...
  return w.querySelectorAll("p").forEach((b) => f.push(b)), setTimeout(() => {
    X.derive(() => {
      r.forEach((b, C) => {
        const z = f[C];
        z && (z.innerText = ds(e.val, b).toString());
      });
    });
  }), w;
}
...
function bs({ mesh: e, settingsObj: s, drawingObj: w, objects3D: p, solids: r }) {
  So.DEFAULT_UP = new M(0, 0, 1);
  const g = document.createElement("div"), x = new go(), f = new vo(45, 1, 0.1, 2 * 1e6), b = new bo(-10, 10, 10, -10, -1e3, 2e6);
  let C = f;
  const z = new Mo({ antialias: true });
  z.localClippingEnabled = true;
  const v = new Wn(f, z.domElement);
  ...
  const q = new _n(new M(-1, 0, 0), 0), O = new _n(new M(0, -1, 0), 0), J = new _n(new M(0, 0, -1), 0);
  ...
  const k = Co(s), j = X.derive(() => k.displayScale.val === 0 ? 1 : k.displayScale.val > 0 ? k.displayScale.val : -1 / k.displayScale.val), xe = ps(e, k), be = () => {
    const T = [];
  ...
  U(), g.appendChild(ko(k, e, r)), g.setAttribute("id", "viewer"), g.appendChild(z.domElement), z.setPixelRatio(window.devicePixelRatio);
  const E = $t();
  ...
  let A = qn(k.gridSize.rawVal, k.flipAxes.rawVal);
  x.add(H, A), X.derive(() => {
  ...
  X.derive(() => {
    ...
    }), H = Sn(T, N()), H.visible = k.gridVisible.rawVal, x.add(H), U(), x.remove(A), A.traverse((ce) => {
      var _a2, _b, _c, _d;
      (_b = (_a2 = ce.geometry) == null ? void 0 : _a2.dispose) == null ? void 0 : _b.call(_a2), (_d = (_c = ce.material) == null ? void 0 : _c.dispose) == null ? void 0 : _d.call(_c);
    }), A = qn(T, $), x.add(A);
  ...
  function ue(T, $) {
    ...
    if (b.left = -ke * se, b.right = ke * se, b.updateProjectionMatrix(), T && ne) {
      if (F ? (F.object = ne, F.update()) : (F = new Wn(ne, z.domElement), F.enableDamping = true, F.dampingFactor = 0.1, F.screenSpacePanning = true, F.zoomSpeed = 0.8, F.panSpeed = 1.2, F.rotateSpeed = 0.9, F.touches = { ONE: an.ROTATE, TWO: an.DOLLY_PAN }, F.target.copy(v.target), F.addEventListener("change", ie), F.enabled = false), !oe) {
        const he = (ze) => {
          if (!Z || !F) return;
  ...
  if (e) {
    x.add(zo(k, xe, j), Po(e, k, xe), To(k, xe, j), Eo(e, k, xe, j), Vo(e, k, xe, j), Ao(e, k, xe, j), Io(e, k, xe, j), Yo(e, k, xe, j), No(e, k, xe, j), Bo(e, k, xe, j));
    const T = ls({ scene: x, rendererElm: z.domElement, getActiveCamera: () => C, derivedNodes: xe, derivedDisplayScale: j, mesh: e, settings: k, render: ie });
    x.add(T);
    const $ = xs(e, k), W = Go(e, k, xe, $), K = On($);
    x.add(W), g.appendChild(K);
    const ce = jo(e, k, xe);
    x.add(ce);
    const se = ce.__colorMapValues, ke = On(se);
  ...
  if (r) {
    const T = new _o(16777215, 0.5);
    x.add(T);
    const $ = new Gn(16777215, 0.5);
  ...
  w && Zo({ drawingObj: w, gridObj: H, scene: x, getActiveCamera: () => C, controls: v, gridSize: L, derivedDisplayScale: j, rendererElm: z.domElement, viewerRender: ie }), un((T, $) => {
    var _a;
    ...
    }), H = Sn(k.gridSize.rawVal, { planes: be() }), x.add(H), g.style.setProperty("--awatif-legend-color", $.legendMarker), ie();
  });
...
const An = X.state(null), Pn = X.state(""), fs = X.state("kN"), us = X.state("mm"), hs = X.state("kN/m\xB2"), ms = { kN: 1, tonf: 9.80665, kip: 4.4482216 }, jn = { mm: 1e3, cm: 100, m: 1, in: 39.3700787402 }, ws = { "kN/m\xB2": 1, kPa: 1, MPa: 1 / 1e3, GPa: 1 / 1e6, "kgf/cm\xB2": 1 / 98.0665, "tonf/m\xB2": 1 / 9.80665, psi: 1 / 6.89476, ksi: 1 / 6894.76 };
function xs(e, s) {
  const w = X.state([]);
  let p;
  return ((r) => {
