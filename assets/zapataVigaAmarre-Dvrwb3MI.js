import { L as Ve, v as rt, V as M, a as Ie, B as Be } from "./Text-CSfbf1xJ.js";
import { a as dt } from "./analyze-BydHtRcI.js";
import { m as mt, d as ut, __tla as __tla_0 } from "./didacticCpp-B5f-GyHC.js";
    ...
    build(s, o) {
      ...
      function Y(e, t, n, c) {
        ...
        for (let l = 0; l < a.length - 1; l++) {
          const d = a[l], i = a[l + 1], m = Math.max(1, Math.round((i - d) / ((t - e) / c)));
          for (let h = 0; h < m; h++) f.push(d + (i - d) * h / m);
        }
      ...
      const ge = Math.round(s.vigaLevel) === 0 ? q : Ce, Fe = E(W, X, 0), ce = E(W, X, ge), Xe = E(J, $, 0), ie = E(J, $, ge);
      for (const [e, t] of [
        [
          Fe,
          ce
      ...
      try {
        ...
        const e = dt(o.nodes.val, o.elements.val, o.elementInputs.val, o.deformOutputs.val), t = o.deformOutputs.rawVal.deformations, n = /* @__PURE__ */ new Map();
        o.elements.rawVal.forEach((c, a) => {
          if (c.length !== 4) return;
          const f = [];
          for (const l of c) {
            const d = t == null ? void 0 : t.get(l), i = d ? d[2] : 0;
            f.push(-_ * i);
          }
          n.set(a, f);
      ...
      for (const e of G) {
        ...
        t && Number.isFinite(t[2]) && (de = Math.max(de, Math.abs(t[2])));
      }
      const Ze = new Set(G), me = Ne * 12, ue = (_a = document.querySelector("#viewer")) == null ? void 0 : _a.__settings, ze = (e, t) => {
        const n = e ? t : 0, a = -(de * Math.max(n, 1) + pt), f = [];
        for (const l of G) {
          if (!Ze.has(l)) continue;
          const d = o.nodes.rawVal[l];
          if (!d) continue;
          const i = d[0], m = d[1], h = we == null ? void 0 : we.get(l), pe = (u) => Number.isFinite(u) ? u : 0, qe = h ? pe(h[0]) : 0, We = h ? pe(h[1]) : 0, Je = h ? pe(h[2]) : 0, ye = i + qe * n, Se = m + We * n, ke = 0 + Je * n, Ke = ke - a, fe = (u) => [
            i + (ye - i) * u,
            m + (Se - m) * u,
            a + Ke * u
          ...
          for (let u = 0; u <= me; u++) {
            const at = 0.05 + 0.9 * (u / me), [lt, ct, it] = fe(at), Le = 2 * Math.PI * Ne * (u / me);
            he.push(new M(lt + Pe * Math.cos(Le), ct + Pe * Math.sin(Le), it));
          }
