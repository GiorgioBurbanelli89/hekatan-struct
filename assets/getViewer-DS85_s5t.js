import { N as ce, B as Z, U as de, F as q, G as U, d as Ie, L as te, e as Q, D as O, b as K, r as H, y as Le, c as Ze, V as R, w as ee, x as D, X as be, k as Ee, a as ne, f as $, h as ue, Y as he, l as ke, j as We, Z as Ne, _ as _e, $ as ze, a0 as re, a1 as $e, a2 as He, q as De, s as Ge, t as qe, W as Ue, u as Ke, A as Qe, v as Ye, O as Je } from "./Text-DyNVkyur.js";
import { v as F, P as Oe, g as j, o as pe } from "./theme-2eEBQPmF.js";
...
function tt(e, t, r) {
  const s = j(), n = new ce(new Z(), new de({ color: s.nodePoint }));
  return pe((a, d) => {
...
function nt(e, t, r) {
  const s = j(), n = new U(), a = new Ie(new Z(), new te({ color: s.elementLine }));
  pe((m, A) => {
    a.material.color.setHex(A.elementLine);
  }), a.frustumCulled = false, n.add(a);
  const d = new Q({ vertexColors: true, transparent: true, opacity: s.shellOpacity, side: O, depthWrite: false }), u = new K(new Z(), d);
  ...
  return F.derive(() => {
    ...
    const m = t.elemColumns.rawVal, A = t.elemBeams.rawVal, v = r.val, I = ((_a = e.elements) == null ? void 0 : _a.val) || [], L = I.filter((C) => {
      if (C.length !== 2) return true;
      const M = v[C[0]], y = v[C[1]];
      if (!M || !y) return true;
      const c = g(M, y);
      return !(c && !m || !c && !A);
    }).map((C) => ot(C).map((M) => [...v[M[0]], ...v[M[1]]]).flat()).flat();
    a.geometry.setAttribute("position", new q(L, 3));
    const E = [], k = [];
    function z(C, M, y, c) {
      const o = [M[0] - C[0], M[1] - C[1], M[2] - C[2]], i = [c[0] - C[0], c[1] - C[1], c[2] - C[2]], l = o[1] * i[2] - o[2] * i[1], h = o[2] * i[0] - o[0] * i[2], x = o[0] * i[1] - o[1] * i[0], S = Math.sqrt(l * l + h * h + x * x);
      return S < 1e-12 ? false : Math.abs(x / S) < 0.5;
    }
    for (const C of I) if (C.length === 3) {
      const [M, y, c] = C;
      if (v[M] && v[y] && v[c]) {
        E.push(...v[M], ...v[y], ...v[c]);
        for (let o = 0; o < 3; o++) k.push(f.r, f.g, f.b);
      }
    } else if (C.length === 4) {
      const [M, y, c, o] = C;
      if (v[M] && v[y] && v[c] && v[o]) {
    ...
    E.length > 0 ? (u.geometry.dispose(), u.geometry = new Z(), u.geometry.setAttribute("position", new q(E, 3)), u.geometry.setAttribute("color", new q(k, 3)), u.geometry.computeVertexNormals(), u.visible = true) : u.visible = false;
  }), F.derive(() => {
...
function st(e, t, r, s) {
  const n = new U(), a = new Ze(0.5, 0.5, 0.5), d = new Q({ color: 10166822 });
  return F.derive(() => {
    var _a, _b;
...
function ut(e, t, r, s) {
  const n = new U(), a = new Z(), d = new te({ vertexColors: true }), u = [0, 0, 0], p = [1, 0, 0], b = [0, 1, 0], f = [0, 0, 1];
  ...
  return a.setAttribute("color", new q([...g, ...g, ...m, ...m, ...A, ...A], 3)), F.derive(() => {
    ...
    t.deformedShape.val, t.orientations.val && (n.clear(), (_a = e.elements) == null ? void 0 : _a.val.forEach((v) => {
      ...
      if (v.length === 2 && (I.position.set(...ge(L, E)), I.rotation.setFromRotationMatrix(Se(L, E))), v.length === 3) {
        const C = r.rawVal[v[2]];
        I.position.set(...oe([L, E, C])), I.rotation.setFromRotationMatrix(dt(L, E, C));
      }
...
function pt(e, t, r, s) {
  const n = new U();
  function a(M, y) {
    const c = M / 2, o = y / 2, i = new Float32Array([0, -c, -o, 0, c, -o, 0, c, o, 0, -c, -o, 0, c, o, 0, -c, o]), l = new Z();
    l.setAttribute("position", new $(i, 3));
    const h = new Float32Array([0, -c, -o, 0, c, -o, 0, c, o, 0, -c, o, 0, -c, -o]), x = new Z();
    return x.setAttribute("position", new $(h, 3)), { fill: l, outline: x };
  }
  function d(M, y = 24) {
    const c = M / 2, o = new Float32Array(y * 9);
    for (let x = 0; x < y; x++) {
      const S = x / y * Math.PI * 2, V = (x + 1) / y * Math.PI * 2;
      o[x * 9] = 0, o[x * 9 + 1] = 0, o[x * 9 + 2] = 0, o[x * 9 + 3] = 0, o[x * 9 + 4] = c * Math.cos(S), o[x * 9 + 5] = c * Math.sin(S), o[x * 9 + 6] = 0, o[x * 9 + 7] = c * Math.cos(V), o[x * 9 + 8] = c * Math.sin(V);
    }
    const i = new Z();
    i.setAttribute("position", new $(o, 3));
    const l = new Float32Array((y + 1) * 3);
    for (let x = 0; x <= y; x++) {
      const S = x / y * Math.PI * 2;
      l[x * 3] = 0, l[x * 3 + 1] = c * Math.cos(S), l[x * 3 + 2] = c * Math.sin(S);
    }
    const h = new Z();
    return h.setAttribute("position", new $(l, 3)), { fill: i, outline: h };
  }
  function u(M, y, c, o) {
    const i = c ?? y * 0.08, l = o ?? M * 0.07, h = M / 2, x = y / 2, S = x - i, V = l / 2, P = [];
    function w(T, W, N, _) {
      P.push(0, T, W, 0, N, W, 0, N, _, 0, T, W, 0, N, _, 0, T, _);
    }
    w(-h, -x, h, -S), w(-V, -S, V, S), w(-h, S, h, x);
    const Y = new Z();
    Y.setAttribute("position", new $(new Float32Array(P), 3));
    const X = new Float32Array([0, -h, -x, 0, h, -x, 0, h, -S, 0, V, -S, 0, V, S, 0, h, S, 0, h, x, 0, -h, x, 0, -h, S, 0, -V, S, 0, -V, -S, 0, -h, -S, 0, -h, -x]), B = new Z();
    return B.setAttribute("position", new $(X, 3)), { fill: Y, outline: B };
  }
  function p(M, y, c) {
    const o = M / 2, i = y / 2, l = o - c, h = i - c, x = [];
    function S(Y, X, B, T) {
      x.push(0, Y, X, 0, B, X, 0, B, T, 0, Y, X, 0, B, T, 0, Y, T);
    }
    S(-o, -i, o, -h), S(-o, h, o, i), S(-o, -h, -l, h), S(l, -h, o, h);
    const V = new Z();
    V.setAttribute("position", new $(new Float32Array(x), 3));
    const P = new Float32Array([0, -o, -i, 0, o, -i, 0, o, -i, 0, o, i, 0, o, i, 0, -o, i, 0, -o, i, 0, -o, -i, 0, -l, -h, 0, l, -h, 0, l, -h, 0, l, h, 0, l, h, 0, -l, h, 0, -l, h, 0, -l, -h]), w = new Z();
    return w.setAttribute("position", new $(P, 3)), { fill: V, outline: w };
  }
  function b(M, y, c) {
    const o = M / 2, i = y / 2, l = o - c, h = i - c, x = new Z(), S = new Float32Array([0, -l, -h, 0, l, -h, 0, l, h, 0, -l, -h, 0, l, h, 0, -l, h]);
    x.setAttribute("position", new $(S, 3));
    const V = [];
    function P(B, T, W, N) {
      V.push(0, B, T, 0, W, T, 0, W, N, 0, B, T, 0, W, N, 0, B, N);
    }
    P(-o, -i, o, -h), P(-o, h, o, i), P(-o, -h, -l, h), P(l, -h, o, h);
    const w = new Z();
    w.setAttribute("position", new $(new Float32Array(V), 3));
    const Y = new Float32Array([0, -o, -i, 0, o, -i, 0, o, -i, 0, o, i, 0, o, i, 0, -o, i, 0, -o, i, 0, -o, -i, 0, -l, -h, 0, l, -h, 0, l, -h, 0, l, h, 0, l, h, 0, -l, h, 0, -l, h, 0, -l, -h]), X = new Z();
    return X.setAttribute("position", new $(Y, 3)), { concFill: x, steelFillGeom: w, outline: X };
  }
  function f(M, y, c) {
    ...
    for (let V = 0; V < i.length; V++) {
      const P = (V + 1) % i.length;
      x.push(...i[V], ...i[P]);
    }
    const S = new Z();
    return S.setAttribute("position", new $(new Float32Array(x), 3)), { fill: h, outline: S };
  }
  function g(M, y, c, o) {
    const i = o / 2, l = [], h = [[0, -M - i, -y / 2], [0, -c - i, -y / 2], [0, -c - i, y / 2 - c], [0, -i, y / 2 - c], [0, -i, y / 2], [0, -M - i, y / 2]], x = [[0, i, -y / 2], [0, i + c, -y / 2], [0, i + c, y / 2 - c], [0, M + i, y / 2 - c], [0, M + i, y / 2], [0, i, y / 2]], S = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
    for (const Y of S) l.push(...h[Y]);
    for (const Y of S) l.push(...x[Y]);
    const V = new Z();
    V.setAttribute("position", new $(new Float32Array(l), 3));
    const P = [];
    for (const Y of [h, x]) for (let X = 0; X < Y.length; X++) {
      const B = (X + 1) % Y.length;
      P.push(...Y[X], ...Y[B]);
    }
    const w = new Z();
    return w.setAttribute("position", new $(new Float32Array(P), 3)), { fill: V, outline: w };
  }
  function m(M, y, c, o) {
    const i = y / 2, l = M, h = [[0, -l, -i], [0, -l, -i + c], [0, -o, -i + c], [0, -o, i - c], [0, -l, i - c], [0, -l, i], [0, 0, i], [0, 0, -i]], x = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], S = [];
    for (const Y of x) S.push(...h[Y]);
    const V = new Z();
    V.setAttribute("position", new $(new Float32Array(S), 3));
    const P = [];
    for (let Y = 0; Y < h.length; Y++) {
      const X = (Y + 1) % h.length;
      P.push(...h[Y], ...h[X]);
    }
    const w = new Z();
    return w.setAttribute("position", new $(new Float32Array(P), 3)), { fill: V, outline: w };
  ...
  function A(M, y, c, o, i) {
    const l = y / 2, h = i / 2, x = [], S = [[0, -M, -l], [0, -M, -l + c], [0, -h - o, -l + c], [0, -h - o, l - c], [0, -M, l - c], [0, -M, l], [0, -h, l], [0, -h, -l]], V = S.map((B) => [B[0], -B[1], B[2]]), P = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
    for (const B of P) x.push(...S[B]);
    for (const B of P) x.push(...V[B]);
    const w = new Z();
    w.setAttribute("position", new $(new Float32Array(x), 3));
    const Y = [];
    for (const B of [S, V]) for (let T = 0; T < B.length; T++) {
      const W = (T + 1) % B.length;
      Y.push(...B[T], ...B[W]);
    }
    const X = new Z();
    return X.setAttribute("position", new $(new Float32Array(Y), 3)), { fill: w, outline: X };
  }
  function v(M, y, c, o) {
    const i = M / 2, l = y / 2, h = o / 2, x = [[0, -h, -l], [0, h, -l], [0, h, l - c], [0, i, l - c], [0, i, l], [0, -i, l], [0, -i, l - c], [0, -h, l - c]], S = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], V = [];
    for (const X of S) V.push(...x[X]);
    const P = new Z();
    P.setAttribute("position", new $(new Float32Array(V), 3));
    const w = [];
    for (let X = 0; X < x.length; X++) {
      const B = (X + 1) % x.length;
      w.push(...x[X], ...x[B]);
    }
    const Y = new Z();
    return Y.setAttribute("position", new $(new Float32Array(w), 3)), { fill: P, outline: Y };
  }
  function I(M, y, c = 24) {
    const o = M / 2, i = o - y, l = [];
    for (let V = 0; V < c; V++) {
      const P = V / c * Math.PI * 2, w = (V + 1) / c * Math.PI * 2, Y = Math.cos(P), X = Math.sin(P), B = Math.cos(w), T = Math.sin(w);
      l.push(0, o * Y, o * X, 0, o * B, o * T, 0, i * B, i * T), l.push(0, o * Y, o * X, 0, i * B, i * T, 0, i * Y, i * X);
    }
    const h = new Z();
    h.setAttribute("position", new $(new Float32Array(l), 3));
    const x = [];
    for (let V = 0; V < c; V++) {
      const P = V / c * Math.PI * 2, w = (V + 1) / c * Math.PI * 2;
      x.push(0, o * Math.cos(P), o * Math.sin(P), 0, o * Math.cos(w), o * Math.sin(w)), x.push(0, i * Math.cos(P), i * Math.sin(P), 0, i * Math.cos(w), i * Math.sin(w));
    }
    const S = new Z();
    return S.setAttribute("position", new $(new Float32Array(x), 3)), { fill: h, outline: S };
  }
  const L = new Q({ color: 52479, transparent: true, opacity: 0.35, side: O, depthWrite: false }), E = new te({ color: 52479 }), k = new Q({ color: 16750848, transparent: true, opacity: 0.4, side: O, depthWrite: false }), z = new te({ color: 16750848 });
  function C(M, y) {
  ...
  return F.derive(() => {
    ...
    c.forEach((h, x) => {
      ...
      const S = r.rawVal[h[0]], V = r.rawVal[h[1]];
      if (!S || !V) return;
      const P = C(S, V);
      if (P && !M || !P && !y) return;
      if (l >= 0) {
        const T = Math.min(S[1], V[1]);
        Math.max(S[1], V[1]);
      ...
      if (!w) return;
      const Y = [(S[0] + V[0]) / 2, (S[1] + V[1]) / 2, (S[2] + V[2]) / 2], X = Se(S, V);
      if (w.type === "CFT") {
        const T = b(w.b, w.h, w.tw ?? w.b * 0.05), W = new K(T.concFill, L);
        W.position.set(...Y), W.rotation.setFromRotationMatrix(X), n.add(W);
        const N = new K(T.steelFillGeom, k);
        N.position.set(...Y), N.rotation.setFromRotationMatrix(X), n.add(N);
        const _ = new ne(T.outline, z);
        _.position.set(...Y), _.rotation.setFromRotationMatrix(X), n.add(_);
      } else {
        let T, W, N;
        switch (w.type) {
          case "rect":
          ...
          case "circ":
            ...
            break;
          case "I":
            T = u(w.b, w.h, w.tf, w.tw), W = k, N = z;
            break;
          case "HSS":
            T = p(w.b, w.h, w.tw ?? w.b * 0.05), W = k, N = z;
            break;
          case "CFT":
            T = b(w.b, w.h, w.tw ?? w.b * 0.05), W = k, N = z;
            break;
          case "L":
            T = f(w.b ?? w.h, w.h, w.t ?? w.tw ?? 3e-3), W = k, N = z;
            break;
          case "2L":
            T = g(w.b ?? w.h, w.h, w.t ?? w.tw ?? 3e-3, w.dis ?? 0.01), W = k, N = z;
            break;
          case "C":
          case "coldC":
            T = m(w.b, w.h, w.tf ?? w.t ?? 3e-3, w.tw ?? w.t ?? 3e-3), W = k, N = z;
            break;
          case "2C":
            T = A(w.b, w.h, w.tf ?? 5e-3, w.tw ?? 5e-3, w.dis ?? 0.01), W = k, N = z;
            break;
          case "T":
            T = v(w.b, w.h, w.tf ?? 0.01, w.tw ?? 6e-3), W = k, N = z;
            break;
          case "pipe":
            T = I(w.d, w.tw ?? w.d * 0.05), W = k, N = z;
            break;
          default:
            return;
        }
        const _ = new K(T.fill, W);
        _.position.set(...Y), _.rotation.setFromRotationMatrix(X), n.add(_);
        const G = new ne(T.outline, N);
        G.position.set(...Y), G.rotation.setFromRotationMatrix(X), n.add(G);
      }
      const B = ht(w);
      if (B) {
        const W = ["I", "HSS", "CFT", "L", "2L", "C", "2C", "T", "pipe", "coldC"].includes(w.type) ? "#ff9900" : "#00ccff", N = new D(B, W, "transparent");
        N.position.set(Y[0], Y[1], Y[2]);
...
class le extends U {
  constructor(t, r, s, n, a, d, u) {
    super();
    const p = new ue().moveTo(0, 0).lineTo(0, d[1]).lineTo(s, d[1]).lineTo(s, 0).lineTo(0, 0), b = p.getPoints(), f = new Z().setFromPoints(b);
    ...
    const g = new he(p), m = new Q({ color: d[1] > 0 ? 24435 : 11411474, side: O });
...
class Xe extends U {
  constructor(t, r, s, n, a, d, u) {
    ...
    if (this.text = new D(`${a[0].toFixed(4)}`), this.text2 = new D(`${(a[1] * -1).toFixed(4)}`), this.normalizedResult = d, this.textPosition = ge(t, r), this.text2Position = ge(r, t), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(n), this.text2.rotation.setFromRotationMatrix(n), this.add(this.text, this.text2), b) {
      const f = new ue().moveTo(0, 0).lineTo(0, d[0]).lineTo(p, 0).lineTo(0, 0), g = new ue().moveTo(p, 0).lineTo(s, -d[1]).lineTo(s, 0).lineTo(p, 0), m = f.getPoints(), A = g.getPoints(), v = new Z().setFromPoints(m), I = new Z().setFromPoints(A), L = new te({ color: j().resultOutline });
      ...
      const E = new he(f), k = new he(g), z = new Q({ color: d[0] > 0 ? 24435 : 11411474, side: O }), C = new Q({ color: -d[1] > 0 ? 24435 : 11411474, side: O });
      this.mesh = new K(E, z), this.mesh2 = new K(k, C), this.mesh.position.set(...t), this.mesh2.position.set(...t), this.mesh.rotation.setFromRotationMatrix(n), this.mesh2.rotation.setFromRotationMatrix(n), u && this.mesh.rotateX(Math.PI / 2), u && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
      const f = new ue().moveTo(0, 0).lineTo(0, d[0]).lineTo(s, -d[1]).lineTo(s, 0).lineTo(0, 0), g = f.getPoints(), m = new Z().setFromPoints(g);
      ...
      const A = new he(f), v = new Q({ color: d[0] > 0 ? 24435 : 11411474, side: O });
...
function vt(e, t, r, s) {
  ...
  return F.derive(() => {
    ...
    n.children.forEach((u) => u.dispose()), n.clear();
    const a = Ce[t.nodeResults.rawVal], d = 0.05 * t.gridSize.val;
...
function xt({ drawingObj: e, gridObj: t, scene: r, camera: s, controls: n, gridSize: a, derivedDisplayScale: d, rendererElm: u, viewerRender: p }) {
  const b = new ke(), f = new We(), g = new K(new Ne(a, a), new Q({ side: O })), m = new ce(new Z(), new de()), A = new ce(new Z(), new de({ color: "gray" })), v = new ce(new Z(), new de({ color: "orange", size: 0.8 }));
  ...
  u.addEventListener("pointerdown", () => {
    ...
    if (C.length) {
      let M = C[0].point;
      (z.ctrlKey || z.metaKey) && (M = new R(Math.round(C[0].point.x), Math.round(C[0].point.y), Math.round(C[0].point.z))), e.points.val = [...e.points.rawVal, M.toArray()], e.polylines && (e.polylines.val = [...e.polylines.rawVal.slice(0, -1), [...e.polylines.rawVal.length ? e.polylines.rawVal.pop() : [], e.points.rawVal.length - 1]]);
    }
    ...
    if (A.geometry.deleteAttribute("position"), C.length) {
      let M = C[0].point;
      (z.ctrlKey || z.metaKey) && (M = new R(Math.round(C[0].point.x), Math.round(C[0].point.y), Math.round(C[0].point.z))), A.geometry.setAttribute("position", new q(M.toArray(), 3));
    }
    p();
  }), u.addEventListener("pointermove", (z) => {
    var _a;
  ...
  u.addEventListener("pointermove", (z) => {
    ...
    if (M.length && y.length) {
      ...
      h.transformDirection(g.matrixWorld), Math.abs(l.dot(h)) < 1e-4 && (C = true);
    }
    if (C && L < 5 && (E = true, n.enabled = false, k = M[0].index), !E || L % 2 !== 0) return;
    const c = [...e.points.rawVal];
    if (k !== void 0) {
      let o = y[0].point;
      (z.ctrlKey || z.metaKey) && (o = new R(Math.round(o.x), Math.round(o.y), Math.round(o.z))), c[k] = o.toArray();
    }
    e.points.val = c;
  }), u.addEventListener("pointerup", () => {
...
function Mt(e, t, r) {
  const s = new Re(), n = new H(), a = new K(new Z(), new Q({ side: O, vertexColors: true }));
...
function Vt(e, t, r, s) {
  ...
  return F.derive(() => {
    var _a, _b, _c;
    t.deformedShape.val;
    const p = r.val, b = ((_a = e.elements) == null ? void 0 : _a.val) ?? [], f = St(t.frameResults.val);
    ...
    const E = [1 / 0, 1 / 0, 1 / 0], k = [-1 / 0, -1 / 0, -1 / 0];
    for (const x of p) for (let S = 0; S < 3; S++) E[S] = Math.min(E[S], x[S]), k[S] = Math.max(k[S], x[S]);
    const C = Math.max(k[0] - E[0], k[1] - E[1], k[2] - E[2], 1) * Ft, M = [], y = [], c = [];
    let o = 0;
    for (const { idx: x, vals: S } of v) {
      const V = b[x], P = p[V[0]], w = p[V[1]];
      if (!P || !w) continue;
      const Y = new R(w[0] - P[0], w[1] - P[1], w[2] - P[2]), X = Y.length();
      if (X < 1e-10) continue;
      Y.normalize();
      const B = Math.abs(Y.y) < 0.99 ? new R(0, 1, 0) : new R(1, 0, 0), T = new R().crossVectors(Y, B).normalize(), W = new R().crossVectors(Y, T).normalize(), N = ye + 1, _ = gt;
      for (let G = 0; G < N; G++) {
        const J = G / ye, se = P[0] + Y.x * X * J, ie = P[1] + Y.y * X * J, me = P[2] + Y.z * X * J, fe = S[0] + (S[1] - S[0]) * J, ae = a.getColor(fe) ?? new H(0, 0, 0);
        d.copy(ae).convertSRGBToLinear();
        for (let we = 0; we < _; we++) {
          const Ae = we / _ * Math.PI * 2, ve = Math.cos(Ae), xe = Math.sin(Ae);
          M.push(se + (T.x * ve + W.x * xe) * C, ie + (T.y * ve + W.y * xe) * C, me + (T.z * ve + W.z * xe) * C), y.push(d.r, d.g, d.b);
        }
      }
    ...
    const l = new Q({ vertexColors: true, side: O }), h = new K(i, l);
...
function Lt({ mesh: e, settingsObj: t, drawingObj: r, objects3D: s, solids: n }) {
  ...
  const E = m.gridSize.rawVal, k = E * 0.5 + E * 0.5 / Math.tan(45 * 0.5);
  u.position.set(0.5 * E, 0.8 * -k, 0.5 * E), g.target.set(0.5 * E, 0.5 * E, 0), g.minDistance = 1, g.maxDistance = k * 2.5, a.__settings = m, g.zoomSpeed = 1, g._getZoomScale = function() {
    ...
    for (const o of c) {
      ...
      const h = i / l, x = p.top;
      p.left = -x * h, p.right = x * h, p.updateProjectionMatrix(), f.setSize(i, l), C();
    }
  ...
  if (s) {
    ...
    F.derive(() => {
      const l = s.val;
      c.length && (d.remove(...c), c.length = 0), l.length && (d.add(...l), c.push(...l), i()), C();
    }), F.derive(() => {
      m.custom3D.val, i();
    }), F.derive(() => {
      m.showCotas.val, i();
  ...
  r && xt({ drawingObj: r, gridObj: I, scene: d, camera: u, controls: g, gridSize: E, derivedDisplayScale: A, rendererElm: f.domElement, viewerRender: C }), pe((c, o) => {
    f.setClearColor(o.background, 1), d.remove(I), I.geometry.dispose(), I.material.dispose(), I = Pe(m.gridSize.rawVal), d.add(I), a.style.setProperty("--awatif-legend-color", o.legendMarker), C();
  });
  const y = { scene: d, perspCamera: u, orthoCamera: p, get camera() {
    return b;
  }, controls: g, renderer: f, rendererElm: f.domElement, render: C, setActiveCamera: M, settings: m };
  return a.__ctx = y, a;
}
function zt(e, t) {
  return F.derive(() => {
...
const Ve = F.state(null), Fe = F.state(""), Yt = F.state("kN"), Pt = F.state("mm"), Xt = { kN: 1, tonf: 9.80665, kip: 4.4482216 }, Tt = { mm: 1e3, cm: 100, m: 1, in: 39.3700787402 };
function It(e, t) {
  const r = F.state([]);
  let s;
  return ((n) => {
    ...
    const n = /* @__PURE__ */ new Map(), a = /* @__PURE__ */ new Map(), d = /* @__PURE__ */ new Map(), u = /* @__PURE__ */ new Map(), p = /* @__PURE__ */ new Map(), b = /* @__PURE__ */ new Map(), f = /* @__PURE__ */ new Map(), g = /* @__PURE__ */ new Map(), m = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map(), v = (V, P) => {
      V == null ? void 0 : V.forEach((w, Y) => {
        const X = e.elements.val[Y];
        if (X) for (let B = 0; B < X.length; B++) P.set(X[B], [w[B] ?? w[0]]);
      });
    };
    ...
    const k = { bendingXX: [n, 0], bendingYY: [a, 0], bendingXY: [d, 0], membraneXX: [u, 0], membraneYY: [p, 0], membraneXY: [b, 0], tranverseShearX: [f, 0], tranverseShearY: [g, 0], vonMises: [m, 0], pressure: [A, 0], displacementX: [(_x = (_w = e.deformOutputs) == null ? void 0 : _w.val) == null ? void 0 : _x.deformations, 0], displacementY: [(_z = (_y = e.deformOutputs) == null ? void 0 : _y.val) == null ? void 0 : _z.deformations, 1], displacementZ: [(_B = (_A = e.deformOutputs) == null ? void 0 : _A.val) == null ? void 0 : _B.deformations, 2] }, z = t.shellResults.val, C = Yt.val, M = Pt.val, y = z === "displacementX" || z === "displacementY" || z === "displacementZ", c = z === "bendingXX" || z === "bendingYY" || z === "bendingXY", o = z === "membraneXX" || z === "membraneYY" || z === "membraneXY", i = z === "vonMises" || z === "pressure", l = z === "tranverseShearX" || z === "tranverseShearY", h = y ? Tt[M] : c || o || i || l ? 1 / Xt[C] : 1, x = y ? M : c ? `${C}\xB7m/m` : o ? `${C}/m\xB2` : i ? `${C}/m\xB2` : l ? `${C}/m` : "";
    Fe.val = x;
    const S = [];
    e.nodes.val.forEach((V, P) => {
      const w = k[z];
      ...
      const Y = w[0].get(P), X = Y ? Y[w[1]] ?? 0 : 0;
      S.push(X * h);
    }), r.val = S;
  }), r;
}
