import { v as L, p as jt, a5 as Pn, B as fe, a6 as Cn, F as Mt, a3 as Co, Z as je, K as It, L as it, h as Dt, t as Fo, g as Uo, a7 as Ko, i as et, d as Ge, V as m, _ as Ot, a8 as Zn, G as Vo, D as zt, a as vt, w as at, y as Fn, a9 as Vn, r as Ho, m as Wo, H as Gt, a1 as hn, E as ho, f as rn, Q as Un, aa as mn, C as mo, S as wo, c as yo, ab as _n, o as Go, ac as qo, ad as Jo, ae as Qo, af as Oo, b as xo, ag as go, e as vo, J as jo, N as es, U as ts, W as ns, T as Sn, P as Kn, X as os, Y as Mo, O as ss } from "./theme-Cr2LU0HL.js";
...
function Hn(e, i) {
  ...
  const z = new It(y.grid), P = new It(y.grid).multiplyScalar(0.45), b = (ae, ie, ue, k) => {
    const W = [], he = ae === "xy" ? (D, $) => [D, $, 0] : ae === "xz" ? (D, $) => [D, 0, $] : (D, $) => [0, D, $], ge = Math.floor(w / ie);
    for (let D = -ge; D <= ge; D++) {
      const $ = D * ie, A = he($, -w), E = he($, w);
      W.push(...A, ...E);
    }
    for (let D = -ge; D <= ge; D++) {
      const $ = D * ie, A = he(-w, $), E = he(w, $);
...
function ms(e, i, y, h) {
  ...
  return _.setAttribute("color", new Mt([...b, ...b, ...te, ...te, ...ae, ...ae], 3)), L.derive(() => {
    ...
    i.deformedShape.val, i.orientations.val && (u.clear(), (_a = e.elements) == null ? void 0 : _a.val.forEach((ie) => {
      const ue = new Dt(_, x), k = y.rawVal[ie[0]], W = y.rawVal[ie[1]];
...
function ys(e, i, y, h) {
  ...
  u.add(_);
  function x(Z, D) {
    const $ = Z / 2, A = D / 2, E = new Float32Array([0, -$, -A, 0, $, -A, 0, $, A, 0, -$, -A, 0, $, A, 0, -$, A]), R = new fe();
  ...
  function w(Z, D = 24) {
    const $ = Z / 2, A = new Float32Array(D * 9);
    for (let N = 0; N < D; N++) {
    ...
    const R = new Float32Array((D + 1) * 3);
    for (let N = 0; N <= D; N++) {
  ...
  function v(Z, D, $, A) {
    const E = $ ?? D * 0.08, R = A ?? Z * 0.07, C = Z / 2, N = D / 2, ne = N - E, G = R / 2, H = [];
  ...
  function P(Z, D, $) {
    const A = Z / 2, E = D / 2, R = A - $, C = E - $, N = new fe(), ne = new Float32Array([0, -R, -C, 0, R, -C, 0, R, C, 0, -R, -C, 0, R, C, 0, -R, C]);
  ...
  function b(Z, D, $) {
    const A = [], E = [[0, -Z / 2, -D / 2], [0, -Z / 2 + $, -D / 2], [0, -Z / 2 + $, D / 2 - $], [0, Z / 2, D / 2 - $], [0, Z / 2, D / 2], [0, -Z / 2, D / 2]], R = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
  ...
  function te(Z, D, $, A) {
    const E = A / 2, R = [], C = [[0, -Z - E, -D / 2], [0, -$ - E, -D / 2], [0, -$ - E, D / 2 - $], [0, -E, D / 2 - $], [0, -E, D / 2], [0, -Z - E, D / 2]], N = [[0, E, -D / 2], [0, E + $, -D / 2], [0, E + $, D / 2 - $], [0, Z + E, D / 2 - $], [0, Z + E, D / 2], [0, E, D / 2]], ne = [0, 1, 2, 0, 2, 5, 2, 3, 4, 2, 4, 5];
  ...
  function ae(Z, D, $, A) {
    const E = D / 2, R = Z, C = [[0, -R, -E], [0, -R, -E + $], [0, -A, -E + $], [0, -A, E - $], [0, -R, E - $], [0, -R, E], [0, 0, E], [0, 0, -E]], N = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5], ne = [];
  ...
  function ie(Z, D, $, A, E) {
    const R = D / 2, C = E / 2, N = [], ne = [[0, -Z, -R], [0, -Z, -R + $], [0, -C - A, -R + $], [0, -C - A, R - $], [0, -Z, R - $], [0, -Z, R], [0, -C, R], [0, -C, -R]], G = ne.map((le) => [le[0], -le[1], le[2]]), H = [0, 1, 7, 1, 6, 7, 1, 2, 6, 2, 5, 6, 2, 3, 5, 3, 4, 5];
  ...
  function ue(Z, D, $, A) {
    const E = Z / 2, R = D / 2, C = A / 2, N = [[0, -C, -R], [0, C, -R], [0, C, R - $], [0, E, R - $], [0, E, R], [0, -E, R], [0, -E, R - $], [0, -C, R - $]], ne = [0, 1, 7, 1, 2, 7, 6, 7, 5, 2, 3, 4, 2, 4, 5, 2, 5, 7], G = [];
  ...
  function K(Z, D) {
    const $ = Math.abs(D[0] - Z[0]), A = Math.abs(D[1] - Z[1]), E = Math.abs(D[2] - Z[2]);
    return E > $ && E > A || A > $ && A > E;
  }
  return L.derive(() => {
    var _a, _b;
...
function bs({ drawingObj: e, gridObj: i, scene: y, getActiveCamera: h, controls: u, gridSize: _, derivedDisplayScale: x, rendererElm: w, viewerRender: v }) {
  ...
  const W = () => {
    ...
    if (ie.visible = !!window.__hekatanGridPlaneXZ, ue.visible = !!window.__hekatanGridPlaneYZ, window.__hekatanOrthoRaycast === true && De.visible) {
      const a = z.intersectObjects([De, Re, We], false);
  ...
  const A = new m(), E = (n, o, a, t, r, s) => {
    ...
    Z = [n, o, a], D = [l / g, p / g, d / g], A.set((n + t) / 2, (o + r) / 2, (a + s) / 2), A.project(h());
    ...
    K.style.display = "none", Z = null, D = null, $ = false, document.activeElement === K && K.blur();
  }, C = (n) => {
    var _a, _b, _c, _d;
  ...
  let le = [];
  const re = new je(), Fe = new Ge(new Gt(1, 1), new et({ color: 2282478, transparent: true, opacity: 0.08, side: zt, depthWrite: false })), ce = new Dt(new ho(new Gt(1, 1)), new it({ color: 2282478, transparent: true, opacity: 0.85 })), _e = new Dt(new fe(), new it({ color: 2282478, transparent: true, opacity: 0.3 })), Ze = (n, o) => {
  ...
  }, De = tt(3462041), Re = tt(16724804), We = tt(6333946);
  me.add(De, Re, We);
  const Ce = (n, o, a, t) => {
  ...
  Ke.id = "hk-refplane-badge", Ke.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(Ke), window.__hekatanSetOrthoPlanes = (n) => {
    ...
    if (window.__hekatanShowOrthoPlanes = n, me.visible = n, n) {
      ...
      Ft(we, s, "xy", l), Ft(de, s, "xz", l), Ft(ze, s, "yz", l), Ce(De, s, "xy", l), Ce(Re, s, "xz", l), Ce(We, s, "yz", l), De.material.opacity = 0.05, Re.material.opacity = 0.05, We.material.opacity = 0.05;
    } else {
    ...
    Ft(we, s, "xy", n), Ft(de, s, "xz", n), Ft(ze, s, "yz", n), Ce(De, s, "xy", n), Ce(Re, s, "xz", n), Ce(We, s, "yz", n), v();
  };
  const ot = (n) => {
    if (De.material.opacity = n === "xy" ? 0.09 : 0.025, Re.material.opacity = n === "xz" ? 0.09 : 0.025, We.material.opacity = n === "yz" ? 0.09 : 0.025, n) {
  ...
  Pt.visible = false, y.add(Pt), window.__hekatanShowAxes = (n, o, a = 12, t = 2) => {
    ...
    n.forEach((M, c) => {
      const f = c < d.length ? d[c] : `X${c}`, S = new fe().setFromPoints([new m(M, r, 0), new m(M, s, 0), new m(M, r, 0), new m(M, r, a)]), I = new hn({ color: 6333946, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), B = new Dt(S, I);
      B.computeLineDistances(), Pt.add(B), Pt.add(g(f, M, r - 0.5, 0, "#60a5fa")), Pt.add(g(f, M, s + 0.5, 0, "#60a5fa"));
    }), o.forEach((M, c) => {
      const f = `${c + 1}`, S = new fe().setFromPoints([new m(l, M, 0), new m(p, M, 0), new m(l, M, 0), new m(l, M, a)]), I = new hn({ color: 16478597, dashSize: 0.3, gapSize: 0.15, transparent: true, opacity: 0.6 }), B = new Dt(S, I);
  ...
  const eo = () => {
    ...
    for (const a of o) {
      ...
      t.position.set(a[0], a[1], a[2]), t.renderOrder = 996, t.scale.setScalar(Rt(t.position)), Yt.add(t);
    }
  };
  L.derive(() => {
    ...
    if (a.length) {
      ...
      if (d.length > 0 && g[d[d.length - 1]]) {
        ...
        if (Lt = null, !f && window.__hekatanAxisSnap !== false) {
          ...
          for (const [Vt, gt] of Zt) {
            ...
            if (mt === null || fn < mt.dpx) {
              const Yn = z.ray, po = St.clone().sub(Yn.origin), Nn = gt.dot(Yn.direction), uo = gt.dot(po), No = Yn.direction.dot(po), fo = 1 - Nn * Nn, Zo = Math.abs(fo) < 1e-6 ? -uo : (Nn * No - uo) / fo;
        ...
        if (!f && S) {
          ...
          let St = null;
          ht === De ? St = "xy" : ht === Re ? St = "xz" : ht === We && (St = "yz"), St === "xy" ? f = Ve >= He ? "x" : "y" : St === "xz" ? f = Ve >= st ? "x" : "z" : St === "yz" ? f = He >= st ? "y" : "z" : f = Ve >= He && Ve >= st ? "x" : He >= st ? "y" : "z";
        }
        ...
        if (f) {
          ...
          let mt = null;
          $t === De ? mt = "xy" : $t === Re ? mt = "xz" : $t === We && (mt = "yz");
        ...
        me.visible = pe, pe || ot(null), pe && (Ft(we, c, "xy", j), Ft(de, c, "xz", j), Ft(ze, c, "yz", j), Ce(De, c, "xy", j), Ce(Re, c, "xz", j), Ce(We, c, "yz", j));
        const Le = pe ? z.intersectObjects([De, Re, We], false) : [];
        let ye = null;
        if (Le.length > 0) {
          const Ve = Le[0].object;
          Ve === De ? ye = "xy" : Ve === Re ? ye = "xz" : Ve === We && (ye = "yz");
        }
        ...
        if (c && (c.textContent = M), oe.visible = false, O.visible = false, (/* @__PURE__ */ new Set(["line", "polyline", "area", "node", "column", "wall", "rect", "circle", "arc", "polyline-multi", "axis", "chaflan"])).has(l)) {
          if (Z = null, D = null, K.style.left = n.clientX + 20 + "px", K.style.top = n.clientY - 28 + "px", K.style.display = "block", !$) {
  ...
  const dt = (n, o, a, t) => {
    ...
    if (s) {
      ...
      S.addBinding(Y, "Fx", { step: 0.1 }), S.addBinding(Y, "Fy", { step: 0.1 }), S.addBinding(Y, "Fz", { step: 0.1 }), S.addBinding(Y, "Mx", { step: 0.1 }), S.addBinding(Y, "My", { step: 0.1 }), S.addBinding(Y, "Mz", { step: 0.1 }), Qe.addFolder({ title: "\u2696 Additional Mass (kg)", expanded: false }).addBinding(Y, "mass", { label: "m", min: 0, step: 1 }), Qe.addFolder({ title: "\u{1F517} Diaphragm (rigid link)", expanded: false }).addBinding(Y, "diaphragm", { label: "Diafragma", options: { Ninguno: "Ninguno", "D1 (rigid)": "D1 (rigid)", "D2 (rigid)": "D2 (rigid)", "D3 (rigid)": "D3 (rigid)" } }), Qe.addButton({ title: `\u2713 Aplicar a ${o.length} nodo(s) seleccionado(s)` }).on("click", () => {
        ...
        if (j.some((pe) => pe !== 0) && (dt("nodes", o, "springs", j), U++), Y.mass !== 0 && (dt("nodes", o, "mass", Y.mass), U++), Y.diaphragm !== "Ninguno" && (dt("nodes", o, "diaphragm", Y.diaphragm), U++), U === 0) {
    ...
    if (l) {
      ...
      j.addBinding(Y, "qx", { step: 0.1 }), j.addBinding(Y, "qy", { step: 0.1 }), j.addBinding(Y, "qz", { step: 0.1 }), Qe.addFolder({ title: "\u2696 Additional Mass (kg/m)", expanded: false }).addBinding(Y, "massPerM", { label: "m/L", min: 0, step: 1 }), Qe.addButton({ title: "\u2713 Aplicar a segmentos seleccionados" }).on("click", () => {
  ...
  window.__hekatanOsnapCompute = Do, window.__hekatanOsnapShow = io, window.__hekatanOsnapHide = Bn;
  let Te = [], xt = 0;
  ...
  const ro = () => {
    ...
    Ee = null, Ye(), oe.visible = false, O.visible = false, R(), se("\u23F9 Dibujo finalizado \u2014 click para empezar otra serie"), v();
  };
  ...
  w.addEventListener("pointermove", (n) => {
    ...
    if (a && Qt < 5 && (Xn = true, u.enabled = false, Dn = t[0].index), !Xn || Qt % 2 !== 0) return;
...
const Wn = { rainbow: [[0, 255], [0.2, 65535], [0.5, 65280], [0.8, 16776960], [1, 16711680]], cooltowarm: [[0, 3952322], [0.2, 10206463], [0.5, 14474460], [0.8, 16163717], [1, 11797542]], blackbody: [[0, 0], [0.2, 7864320], [0.5, 15086080], [0.8, 16776960], [1, 16777215]], grayscale: [[0, 0], [0.2, 4210752], [0.5, 8355712], [0.8, 12566463], [1, 16777215]] }, wn = [[0, 222, 20, 55], [0.083, 248, 62, 42], [0.166, 253, 112, 45], [0.25, 254, 152, 46], [0.333, 248, 196, 50], [0.416, 236, 232, 48], [0.5, 200, 226, 22], [0.583, 100, 212, 28], [0.666, 8, 202, 62], [0.75, 0, 196, 150], [0.833, 0, 188, 218], [0.916, 6, 158, 230], [1, 10, 125, 205]];
function Ss(e) {
...
function Ts(e, i, y, h) {
  ...
  return L.derive(() => {
    ...
    const Pe = Math.max(he[0] - W[0], he[1] - W[1], he[2] - W[2], 1) * Fs, K = [], Z = [], D = [];
    let $ = 0;
...
function Ds(e) {
  ...
  const _ = new fe(), x = new it({ color: ko, linewidth: 4, transparent: true, opacity: 0.9, depthTest: false }), w = new Dt(_, x);
  ...
  const ae = new fe(), ie = new it({ color: Xs, linewidth: 3, transparent: true, opacity: 0.95, depthTest: false }), ue = new Dt(ae, ie);
  ...
  const D = new fe(), $ = new it({ color: zn, linewidth: 4, transparent: true, opacity: 1, depthTest: false }), A = new Dt(D, $);
    ...
    for (let Ye = 0; Ye < de.length; Ye++) {
      ...
      if (!(!Ae || Ae.length < 2)) {
        if (Ae.length === 2) {
          ...
          const Se = Ys(J, q, be.x, be.y, Ne.x, Ne.y);
...
My \u2248 ${rt(vn * ot)} ${Ce.forceUnit}\xB7m`), Math.abs(Yt) > 1e-6 && (be += `
  ...
  function ne(O, Me, V) {
    ...
    if (O.type === "node") {
      ...
      if (de) {
        ...
        u.position.copy(de), u.scale.setScalar(De), u.visible = true;
      }
    } else if (O.type === "frame" && X) {
      const de = X[O.idx], ze = C(de[0]), me = C(de[1]);
      if (ze && me) {
        const tt = ze.clone().add(me).multiplyScalar(0.5), De = me.clone().sub(ze), Re = De.length(), Ke = e.getActiveCamera().position.distanceTo(tt) * 35e-4;
        z.position.copy(tt);
  ...
  function Ze() {
    ...
    if (E.type === "node") {
      ...
      if (V && X) {
        ...
        ge.position.copy(J);
        const tt = new m(0, 1, 0), De = tt.clone().cross(q).normalize(), Re = tt.angleTo(q);
        ge.quaternion.setFromAxisAngle(De, Re), ge.scale.set(me, we, me), ge.visible = true;
      }
...
function ta({ mesh: e, settingsObj: i, drawingObj: y, objects3D: h, solids: u }) {
  ...
  function re(V, X) {
    ...
    if (v.left = -ze * de, v.right = ze * de, v.updateProjectionMatrix(), V && H) {
      if (F ? (F.object = H, F.update()) : (F = new bo(H, P.domElement), F.enableDamping = true, F.dampingFactor = 0.1, F.screenSpacePanning = true, F.zoomSpeed = 0.8, F.panSpeed = 1.2, F.rotateSpeed = 0.9, F.touches = { ONE: Sn.ROTATE, TWO: Sn.DOLLY_PAN }, F.target.copy(b.target), F.addEventListener("change", ee), F.enabled = false), !oe) {
        const me = (tt) => {
          if (!G || !F) return;
          const De = P.domElement.getBoundingClientRect(), Re = tt.clientX - De.left, We = De.width / 2, Ce = Re >= We;
  ...
  if (e) {
    ...
    const V = Ds({ scene: x, rendererElm: P.domElement, getActiveCamera: () => z, derivedNodes: he, derivedDisplayScale: W, mesh: e, settings: k, render: ee });
    x.add(V);
    ...
    ze.id = "frame-legend", _.appendChild(ze), L.derive(() => {
      var _a;
      const me = k.shellResults.val != "none", tt = (((_a = k.solidResults) == null ? void 0 : _a.val) ?? "none") !== "none", De = me || tt, Re = k.frameResults.val.startsWith("contour:");
      q.hidden = !De, J.visible = De, ze.hidden = !Re;
    });
  }
  if (u) {
