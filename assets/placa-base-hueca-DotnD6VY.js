...
import { v as l, L as $e, M as ce, C as ie, b as re, P as Re, V as Ze, B as Ke, a as Qe } from "./Text-CSfbf1xJ.js";
import { a as We } from "./analyze-BydHtRcI.js";
import { d as ta, __tla as __tla_0 } from "./didacticCpp-B5f-GyHC.js";
import { g as ea, a as pe, h as ue, b as aa } from "./styles-CXNYye6A.js";
import { e as sa } from "./makeDraggable-zx2br6Yh.js";
import { g as oa } from "./getParameters-DsthXvNv.js";
...
Promise.all([
  ...
  const zt = 2e8, At = 0.3, de = zt / (2 * (1 + At)), me = 78, na = 25e4, la = 6e5, r = {
    B: {
      value: l.state(0.5),
      min: 0.3,
      max: 1.2,
  ...
  l.derive(() => {
    const s = r.B.value.val, x = r.H.value.val, Ft = r.t_plate.value.val, d = r.bc.value.val, m = r.hc.value.val, O = r.t_col.value.val, it = r.L_col.value.val, It = Math.round(r.nBoltsX.value.val), rt = Math.round(r.nBoltsY.value.val), dt = r.sx.value.val, Tt = r.sy.value.val, b = r.d_bolt.value.val, U = r.L_bolt.value.val, V = r.L_proj.value.val, X = r.B_ped.value.val, $ = r.H_ped.value.val, mt = r.h_ped.value.val, pt = r.fc.value.val, R = r.Pu.value.val, ut = r.Mx.value.val, ht = r.My.value.val, q = Math.round(r.nx.value.val), E = Math.round(r.ny.value.val), v = Math.round(r.nz_col.value.val), p = 0.04, f = [], M = [], Z = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), tt = /* @__PURE__ */ new Map(), et = /* @__PURE__ */ new Map(), at = /* @__PURE__ */ new Map(), st = /* @__PURE__ */ new Map(), ot = /* @__PURE__ */ new Map();
    function _(t, e, a) {
      return f.push([
        t,
        e,
    ...
    const y = Math.max(2, Math.round(d / Lt)), g = Math.max(2, Math.round(m / Ot)), qt = d / y, Et = m / g, H = it / v, w = [];
    for (let t = 0; t <= v; t++) {
      const e = [];
      for (let a = 0; a <= y; a++) {
        const o = -d / 2 + a * qt;
    ...
    const we = Math.min(0.2, it * 0.4), Ht = Math.min(0.1, (s - d) / 2 * 0.7), ze = Math.max(1, Math.round(we / H));
    function nt(t, e, a, o) {
    ...
    const Gt = Math.PI * b * b / 4, ft = Math.PI * b ** 4 / 64, Dt = 2 * ft, G = [], Ae = (s - 2 * dt) / Math.max(1, It - 1), Ce = (x - 2 * Tt) / Math.max(1, rt - 1);
    for (let t = 0; t < It; t++) for (let e = 0; e < rt; e++) {
    ...
    for (let t = 0; t < C; t++) for (let e = 0; e < A; e++) {
      T(i[0][t][e], i[0][t][e + 1], i[0][t + 1][e + 1], i[0][t + 1][e]);
      const a = -X / 2 + (e + 0.5) * xt, o = -$ / 2 + (t + 0.5) * Ut;
      ke(a, o) || T(i[B][t][e], i[B][t][e + 1], i[B][t + 1][e + 1], i[B][t + 1][e]);
    }
    for (let t = 0; t < B; t++) for (let e = 0; e < A; e++) T(i[t][0][e], i[t][0][e + 1], i[t + 1][0][e + 1], i[t + 1][0][e]), T(i[t][C][e], i[t][C][e + 1], i[t + 1][C][e + 1], i[t + 1][C][e]);
    for (let t = 0; t < B; t++) for (let e = 0; e < C; e++) T(i[t][e][0], i[t][e + 1][0], i[t + 1][e + 1][0], i[t + 1][e][0]), T(i[t][e][A], i[t][e + 1][A], i[t + 1][e + 1][A], i[t + 1][e][A]);
    ...
    for (const [t, e] of G) {
      const a = new ie(b / 2, b / 2, U + V, 12), o = new re(a, Le);
      o.position.set(t, e, p + (-U + V) / 2), o.rotation.x = Math.PI / 2, lt.push(o);
      const c = new ie(Kt, Kt, Zt, 6), n = new re(c, Oe);
  ...
  document.body.append(oa(r), ge, aa({
