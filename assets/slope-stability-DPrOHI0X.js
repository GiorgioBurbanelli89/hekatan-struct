...
import { g as Y, b as C } from "./styles-CXNYye6A.js";
import { g as L } from "./getParameters-DsthXvNv.js";
  ...
  a.derive(() => {
    ...
    try {
      ...
      for (const [t, l] of o.displacements) {
        const d = Math.sqrt(t * t + l * l);
        f = Math.max(f, d);
      }
      let b = 0;
      for (const t of o.plasticStrain) b = Math.max(b, t);
      console.log(`Talud SRM: ${n.length} nodos, ${r.length} tri\xE1ngulos | H=${m} \u03B1=${g}\xB0 c=${M} \u03C6=${w}\xB0 \u03B3=${S} | FOS=${o.fos.toFixed(3)} max|u|=${f.toExponential(3)} \u03B5_pl_max=${b.toExponential(3)}`), o.fos < 1 ? console.warn("\u26A0 TALUD INESTABLE (FOS < 1.0)") : o.fos < 1.5 && console.warn("\u26A0 FOS < 1.5 \u2014 revisar estabilidad");
    } catch (e) {
