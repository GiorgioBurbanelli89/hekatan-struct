import { M as ms, __tla as __tla_0 } from "./deform-BeOgLxmb.js";
let Vs, Ts, Rs, Ls, Ks;
let __tla = Promise.all([
  ...
  Ts = function(s, o, r, t, a) {
    ...
    e.push(R);
    const B = o.map((A) => A.length), V = z(B, Uint32Array, h.HEAPU32);
    e.push(V);
    ...
    }, g = m(t.elasticities), E = m(t.elasticitiesOrthogonal), _ = m(t.areas), w = m(t.momentsOfInertiaZ), k = m(t.momentsOfInertiaY), O = m(t.shearModuli), y = m(t.torsionalConstants), T = m(t.thicknesses), Q = m(t.poissonsRatios), q = m(t.shearAreasY), C = m(t.shearAreasZ), W = t.rigidOffsets ? Array.from(t.rigidOffsets.keys()) : [], j = t.rigidOffsets ? Array.from(t.rigidOffsets.values()).flat() : [], D = z(W, Uint32Array, h.HEAPU32);
    e.push(D);
    const G = z(j, Float64Array, h.HEAPF64);
    e.push(G);
    ...
    e.push(H);
    const $ = t.plateFormulations, es = $ ? Array.from($.keys()) : [], os = $ ? Array.from($.values()) : [], i = z(es, Uint32Array, h.HEAPU32);
    e.push(i);
  ...
  Rs = function(s, o, r, t, a = 10) {
    ...
    e.push(R);
    const B = o.map((u) => u.length), V = is(B, Uint32Array, f.HEAPU32);
    e.push(V);
    ...
    }, F = c(t.elasticities), n = c(t.areas), U = c(t.momentsOfInertiaZ), m = c(t.momentsOfInertiaY), g = c(t.shearModuli), E = c(t.torsionalConstants), _ = c(t.densities), w = c(t.thicknesses), k = c(t.poissonsRatios), O = c(t.membraneModifiers), y = c(t.bendingModifiers), T = t.plateFormulations, Q = T ? Array.from(T.keys()) : [], q = T ? Array.from(T.values()) : [], C = is(Q, Uint32Array, f.HEAPU32);
    e.push(C);
    const W = is(q, Uint32Array, f.HEAPU32);
    e.push(W);
    const j = f._malloc(4);
    e.push(j);
    const D = f._malloc(4);
    ...
    e.push(ss);
    const Z = f._malloc(4);
    e.push(Z), f._modal(K, s.length, R, L.length, V, o.length, N, l, Y.length, F.keysPtr, F.valuesPtr, F.size, n.keysPtr, n.valuesPtr, n.size, U.keysPtr, U.valuesPtr, U.size, m.keysPtr, m.valuesPtr, m.size, g.keysPtr, g.valuesPtr, g.size, E.keysPtr, E.valuesPtr, E.size, _.keysPtr, _.valuesPtr, _.size, w.keysPtr, w.valuesPtr, w.size, k.keysPtr, k.valuesPtr, k.size, O.keysPtr, O.valuesPtr, O.size, y.keysPtr, y.valuesPtr, y.size, C, W, Q.length, a, j, D, G, v, X, I, ss, Z);
    const x = f.HEAPU32[j / 4], as = f.HEAPU32[D / 4], J = f.HEAPU32[G / 4], ts = f.HEAPU32[v / 4], H = f.HEAPU32[X / 4], $ = f.HEAPU32[I / 4], es = f.HEAPU32[ss / 4], os = f.HEAPU32[Z / 4];
    let i = [], d = [], P = [];
    if (as > 0 && x) {
    ...
    if (es > 0 && os > 0 && $) {
      const u = new Float64Array(f.HEAPF64.buffer, $, es * os);
  ...
  const ns = await ms();
  Ks = function(s) {
    const { nodes: o, elements: r, E: t, nu: a, gamma: e, c: K, phi: L, thickness: R = 1, supports: B, surcharge: V = 0, surfaceYThreshold: Y = -1e10 } = s, b = [], N = o.flat(), l = Os(N);
    b.push(l);
    const c = r.flat(), F = Ms(c);
    b.push(F);
    const n = [];
    for (const y of B) n.push(y.node, y.fixX ? 1 : 0, y.fixY ? 1 : 0);
    const U = Ms(n);
    b.push(U);
    ...
    b.push(_);
    const w = ns._slopeStabilitySolver(l, g, F, m, t, a, e, K, L, R, U, B.length, V, Y, E, _), k = [];
  ...
  Ls = function(s) {
    ...
    o.push(q);
    const C = M._malloc(4);
    o.push(C), M._plate_q4_solve(a, t, L, K, s.E, s.nu, s.thickness, V, B, s.pressure ?? 0, N, b, l, c, F, n, m, g, w, _, y, O, T, Q, q, C);
    const W = M.HEAPU32[T / 4], j = M.HEAPU32[Q / 4], D = M.HEAPU32[q / 4], G = M.HEAPU32[C / 4], v = new Float64Array(M.HEAPF64.buffer, W, j), X = v[0], I = v[1], ss = [];
    let Z = 0;
    for (let i = 0; i < X; i++) {
      const d = 2 + i * 5, P = {
        x: v[d],
      ...
      ss.push(P), Math.abs(P.w) > Math.abs(Z) && (Z = P.w);
    }
    const x = new Float64Array(M.HEAPF64.buffer, D, G), as = [];
    let J = 0, ts = 0, H = 0, $ = 0, es = 0;
    for (let i = 0; i < I; i++) {
      const d = i * 9, P = {
        nodes: [
          x[d],
      ...
      as.push(P), Math.abs(P.Mxx) > Math.abs(J) && (J = P.Mxx), Math.abs(P.Myy) > Math.abs(ts) && (ts = P.Myy), Math.abs(P.Mxy) > Math.abs(H) && (H = P.Mxy), Math.abs(P.Qx) > Math.abs($) && ($ = P.Qx), Math.abs(P.Qy) > Math.abs(es) && (es = P.Qy);
    }
    let os;
    if (l > 0 && c > 0) {
      const i = l / 2, d = c / 2;
  ...
  Vs = function(s, o, r, t) {
    ...
    a.push(n);
    const U = (H) => {
      const $ = H ? Array.from(H.keys()) : [], es = H ? Array.from(H.values()) : [], os = Ps($, Uint32Array, p.HEAPU32);
      a.push(os);
    ...
    }, m = U(t.elasticities), g = U(t.areas), E = U(t.momentsOfInertiaZ), _ = U(t.momentsOfInertiaY), w = U(t.shearModuli), k = U(t.torsionalConstants), O = U(t.thicknesses), y = U(t.poissonsRatios), T = U(t.shearAreasY), Q = U(t.shearAreasZ), q = p._malloc(4);
    a.push(q);
    const C = p._malloc(4);
    a.push(C);
    const W = p._malloc(4);
    ...
    a.push(D);
    const G = p._malloc(4);
    a.push(G), p._didactic_solve(e, s.length, L, K.length, B, o.length, b, N, V.length, F, n, l.length, m.keysPtr, m.valuesPtr, m.size, g.keysPtr, g.valuesPtr, g.size, E.keysPtr, E.valuesPtr, E.size, _.keysPtr, _.valuesPtr, _.size, w.keysPtr, w.valuesPtr, w.size, k.keysPtr, k.valuesPtr, k.size, O.keysPtr, O.valuesPtr, O.size, y.keysPtr, y.valuesPtr, y.size, T.keysPtr, T.valuesPtr, T.size, Q.keysPtr, Q.valuesPtr, Q.size, q, C, W, j, D, G);
    const v = p.HEAPU32[q / 4], X = p.HEAPU32[C / 4], I = p.HEAPU32[W / 4], ss = p.HEAPU32[j / 4], Z = p.HEAPU32[D / 4], x = p.HEAPU32[G / 4], as = v && X > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, v, X)) : [], J = I && ss > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, I, ss)) : [], ts = Z && x > 0 ? Array.from(new Float64Array(p.HEAPF64.buffer, Z, x)) : [];
    return v && a.push(v), I && a.push(I), Z && a.push(Z), a.forEach((H) => p._free(H)), Ss(as, J, ts, s.length, o.length);
  };
  function Ss(s, o, r, t, a) {
    ...
    if (s.length > 0) {
      ...
      for (let F = 0; F < l; F++) {
        ...
        n += 9;
        const y = s[n++], T = s[n++], Q = s[n++], q = s[n++], C = s[n++], W = s[n++], j = s[n++], D = s[n++], G = s[n++], v = s[n++], X = s[n++];
        K.push({
          index: U,
    ...
    if (r.length > 0) {
      let l = 0;
      const c = r[l++];
      B = r.slice(l, l + c), l += c, V = r.slice(l, l + c), l += c, Y = r.slice(l, l + c), l += c;
      const F = r[l++];
