import { a as Ie } from "./analyze-Baqb28rE.js";
import { m as Ae, d as fe, __tla as __tla_0 } from "./didacticCpp-PqvqKlgs.js";
import { b as Ne, a as Jt } from "./cotas3D-BRJLBeVj.js";
import { S as Be, f as Te, M as Bo, e as Go, a as Zt, B as co, V as Z, d as he, b as De, L as mo, E as Ye, I as qe, D as He } from "./theme-Co6w-pfC.js";
let ss, es;
let __tla = Promise.all([
  ...
  function je(t, e, N = {}) {
    ...
    for (const xt of e) {
      ...
      for (const p of J) {
        ...
        V.areas.set(Q, X), V.momentsOfInertiaY.set(Q, j), V.momentsOfInertiaZ.set(Q, j), V.torsionalConstants.set(Q, K), V.elasticities.set(Q, Y), V.shearModuli.set(Q, it), V.densities.set(Q, 0);
      }
    }
  ...
  function Ke(t, e, N, x, S, h, y, V = {}) {
    var _a, _b;
    const Y = V.Fy_steel ?? 345e3;
    V.fc_concrete;
    const X = V.Fy_rebar ?? 42e4, j = V.omega ?? 0.15, K = V.phi ?? 0.9, it = S < 0.5 ? K * j * X * (1 - 0.59 * j) : K * Y, xt = h < 0.5 ? K * j * X * (1 - 0.59 * j) : K * Y, J = N.frameBendingMoments, gt = [];
    for (let U = 0; U < e.length; U++) {
      ...
      A && (Q = A.Mi, St = A.Mj);
      const v = ((_a = x.areas) == null ? void 0 : _a.get(U)) ?? 0.16, B = ((_b = x.momentsOfInertiaZ) == null ? void 0 : _b.get(U)) ?? 213e-5, ut = Xe(v, B, p ? it : xt), vt = Q / ut, Ct = St / ut;
      gt.push({
        nodeIdx: b,
        elementIdx: U,
        end: "i",
        classification: ue(vt)
      }), gt.push({
        nodeIdx: l,
        elementIdx: U,
        end: "j",
        classification: ue(Ct)
      });
    }
    return gt;
  }
  function Je(t, e, N, x = {}) {
    const S = x.showElastic ?? false, h = (x.radiusFactor ?? 0.02) * N, y = [], V = new Be(h, 12, 8);
    for (const Y of t) {
  ...
  const Xo = 9.80665;
  function ge(t, e, N, x, S = 0.01) {
    const h = Math.abs(t) < S, y = Math.abs(t - N) < S, V = Math.abs(e) < S, Y = Math.abs(e - x) < S, X = [
      h,
      y,
      V,
      Y
  ...
  function pe(t) {
    const { P_kN: e, Mx_kN: N, My_kN: x, tipo: S, q_adm_tonf: h, ks: y } = t, V = t.Lz_min ?? 1, Y = t.Lz_max ?? 4, X = t.t_min ?? 0.3;
    if (e <= 0) return {
      tipo: S,
      Lz: V,
      Bz: V,
    ...
    let J = Math.max(V, Math.sqrt(e / j)), gt = J, U = 1 / 0, mt = 0, b = false;
  ...
  ss = {
    ...
    params: {
      ...
      slabDisc: et("Avanzado", "Discretizaci\xF3n losa", 0.5, Ze),
    ...
    computedLabels(t, e) {
      ...
      x.forEach((Q, St) => {
        ...
        Math.abs(v) > Math.abs(h) && (h = v, Y = St, A[0], A[1]), v > 0 && v > Math.abs(X) && (X = v, j = St), Math.abs(B) > Math.abs(y) && (y = B), Math.abs(w) > Math.abs(V) && (V = w), K.push({
          idx: St,
          x: A[0],
          y: A[1],
          P_kN: Math.abs(v),
      ...
      const J = Math.abs(h) / 9.80665, gt = Math.abs(y) / 9.80665, U = Math.abs(V) / 9.80665, mt = X / 9.80665, b = Math.round(t.nPisos), l = {
    ...
    build(t, e) {
      var _a, _b;
      const N = Math.round(t.nVanosX), x = Math.round(t.nVanosY), S = Math.round(t.nPisos), h = Math.max(1, Math.round(t.nSubViga)), y = Math.max(1, Math.round(t.nSubCol)), V = t.fcConcr * 0.0981, Y = 4700 * Math.sqrt(V) * 1e3, X = 2e8, j = 0.2, K = 0.3, it = Y / (2 * (1 + j)), xt = X / (2 * (1 + K)), J = (o, a, n) => Array.from({
        length: a
      }, (f, i) => {
        const u = t[`${o}${i + 1}`];
      ...
      if (t.slabOn >= 0.5) {
        ...
        for (let f = 1; f < p.length; f++) {
          ...
          for (let u = 0; u < b.length - 1; u++) for (let r = 0; r < l.length - 1; r++) {
            const d = b[u], O = b[u + 1], c = l[r], C = l[r + 1], { n: R } = me(Math.abs(O - d), n), { n: rt } = me(Math.abs(C - c), n), zt = [];
            for (let q = 0; q <= rt; q++) {
              const st = [];
              for (let Ht = 0; Ht <= R; Ht++) {
                const yo = d + Ht / R * (O - d), Eo = c + q / rt * (C - c), Vo = a(yo, Eo, i), bo = o.get(Vo);
      ...
      const pt = Math.round(t.loadCase ?? 0), go = pt === 1 ? [
        ...
        1,
        1
      ], [po, Mo, Do, Yo] = go, ro = /* @__PURE__ */ new Map(), Ko = po * t.CM + Mo * t.CV;
      if (Ko !== 0) for (let o = 1; o < p.length; o++) for (let a = 0; a < l.length; a++) for (let n = 0; n < b.length; n++) {
        const f = `${n},${a},${o}`;
      ...
      const qo = [
        ...
        const a = xe[o] ?? t.vigaB, n = _e[o] ?? t.vigaH;
        return {
          A: a * n,
          Iy: a * n ** 3 / 12,
          Iz: n * a ** 3 / 12,
      ...
      }, be = t.matCol < 0.5 ? Y : X, Ce = t.matCol < 0.5 ? it : xt, we = t.matCol < 0.5 ? j : K, ze = t.matViga < 0.5 ? Y : X, $e = t.matViga < 0.5 ? it : xt, Se = t.matViga < 0.5 ? j : K, xo = /* @__PURE__ */ new Map(), _o = /* @__PURE__ */ new Map(), So = /* @__PURE__ */ new Map(), ko = /* @__PURE__ */ new Map(), Oo = /* @__PURE__ */ new Map(), Lo = /* @__PURE__ */ new Map(), vo = /* @__PURE__ */ new Map(), Fo = /* @__PURE__ */ new Map(), Qo = /* @__PURE__ */ new Map(), Wo = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), oe = Math.round(t.slabType), ke = oe === 2 ? 0 : 1, Oe = oe === 1 ? 0 : 1, Po = t.crackedSections > 0.5, ee = t.matCol < 0.5 && Po ? 0.7 : 1, se = t.matViga < 0.5 && Po ? 0.35 : 1, Le = Po ? 0.25 : 1, Fe = 1, Ro = t.massSource > 0.5, Pe = t.qDead + 0.25 * t.qLive, Ee = Ro ? Pe / Me / Math.max(t.slabT, 0.05) : uo;
      for (let o = 0; o < w.length; o++) {
        const a = k.get(o) ?? 0;
        if (Ct.has(o)) xo.set(o, Y), _o.set(o, it), Fo.set(o, j), Qo.set(o, t.slabT), Wo.set(o, ke * Fe), te.set(o, Oe * Le), vo.set(o, Ee);
        else if (ut.has(o)) {
          const n = ve(Math.min(a, 7));
          xo.set(o, be), _o.set(o, Ce), Fo.set(o, we), So.set(o, n.A), ko.set(o, n.Iz * ee), Oo.set(o, n.Iy * ee), Lo.set(o, n.J), vo.set(o, Ro ? 0 : uo);
        } else {
          const n = ye(Math.min(a, 7));
          xo.set(o, ze), _o.set(o, $e), Fo.set(o, Se), So.set(o, n.A), ko.set(o, n.Iz * se), Oo.set(o, n.Iy * se), Lo.set(o, n.J), vo.set(o, Ro ? 0 : uo);
        }
      }
      if (t.diafragmaRigido >= 0.5) {
        const o = [];
        for (let i = 1; i < p.length; i++) o.push(p[i]);
        const a = je(v, o), n = w.length;
      ...
      e.nodes.val = v, e.elements.val = w, e.nodeInputs.val = {
        ...
        poissonsRatios: Fo,
        thicknesses: Qo,
        membraneModifiers: Wo,
        bendingModifiers: te
      };
      const ne = fe(v, w, e.nodeInputs.val, e.elementInputs.val);
      e.deformOutputs.val = ne, e.analyzeOutputs.val = Ie(v, w, e.elementInputs.val, ne);
      const Zo = Ne(b, l, p);
      try {
        const o = Ke(v, w, e.analyzeOutputs.rawVal, e.elementInputs.rawVal, Math.round(t.matCol), Math.round(t.matViga), ut);
        ...
        const d = Math.sqrt((i - a) ** 2 + (u - n) ** 2 + (r - f) ** 2) || 1, O = Je(o, v, d, {
          showElastic: false,
          radiusFactor: 0.015
        });
        Zo.push(...O), e.__plasticHinges = Ue(o);
      } catch (o) {
          ...
          if (o.forEach((i, u) => {
            ...
            for (const P of rt) {
              ...
              if (Ut > 1.5 ? Qt = 15680580 : Ut > 1 ? Qt = 16096779 : Ut > 0.8 && (Qt = 16498468), r > 1e-3) {
                ...
                q.push(new he(_t, new Zt({
                  color: 6333946,
                  linewidth: 4
                }))), q.push(Jt(`Df=${r.toFixed(2)}m`, P.x + 0.1, P.y + 0.1, -r / 2, "#60a5fa"));
              }
              if (R === 0) {
                const _t = new De(H, nt, Rt), jt = new Bo(_t, st(Qt));
                jt.position.set(G, L, Pt), q.push(jt);
                const Gt = new mo(new Ye(_t), Ht);
                Gt.position.copy(jt.position), q.push(Gt);
              } else {
                const _t = new qe(H, nt), jt = new Go({
                  color: Qt,
                  transparent: true,
                  opacity: 0.45,
                  roughness: 0.6,
      ...
      if ((t.modoCimentacion ?? 0) >= 0.5) try {
        ...
        if (a && a.size > 0) {
          ...
          if (a.forEach((u, r) => {
            ...
            const q = qo[0] ?? t.colSize, st = Ho[0] ?? t.colSize, Ht = q * st, yo = q * st ** 3 / 12, Eo = st * q ** 3 / 12, Vo = 0.14 * Math.pow(Math.min(q, st), 4), bo = t.matCol < 0.5 ? Y : X, Co = t.matCol < 0.5 ? it : xt, ae = t.matCol < 0.5 ? j : K, P = [], H = [], nt = /* @__PURE__ */ new Map(), Rt = /* @__PURE__ */ new Map(), Tt = /* @__PURE__ */ new Map(), Dt = /* @__PURE__ */ new Map(), G = /* @__PURE__ */ new Map(), L = /* @__PURE__ */ new Map(), I = /* @__PURE__ */ new Map(), Pt = /* @__PURE__ */ new Map(), Nt = /* @__PURE__ */ new Map(), Ut = /* @__PURE__ */ new Map(), Qt = /* @__PURE__ */ new Map(), wo = [], _t = [], jt = (s, z, g) => `${Math.round(s * 1e4)},${Math.round(z * 1e4)},${Math.round(g * 1e4)}`, Gt = /* @__PURE__ */ new Map(), Wt = (s, z, g) => {
            ...
            for (const s of rt) {
              ...
              const Et = s.x - T, Lt = s.y - at, Xt = -d, Vt = z / c, Kt = g / c, lt = [];
              for (let D = 0; D <= c; D++) {
                const tt = [];
                for (let ft = 0; ft <= c; ft++) {
                  const At = Et - z / 2 + ft * Vt, bt = Lt - g / 2 + D * Kt;
                  tt.push(Wt(At, bt, Xt));
                }
                lt.push(tt);
              }
              ...
              const to = 0.5;
              for (let D = 0; D <= c; D++) for (let tt = 0; tt <= c; tt++) {
                const ft = Vt * Kt * (tt === 0 || tt === c ? 0.5 : 1) * (D === 0 || D === c ? 0.5 : 1), At = r * ft, bt = At * to, $ = lt[D][tt];
                wo.push({
                  node: $,
                  dof: 0,
                  k: bt
              ...
              const Bt = lt[_][E], kt = n.find((D) => D.idx === s.idx);
              Qt.set(Bt, [
                0,
                0,
                -kt.P_kN,
              ...
              }
              {
                ...
                for (let bt = 0; bt <= c; bt++) {
                  const $ = -z / 2 + bt * tt;
                  At.push(new Z(Et + $, Lt - g / 2, -d), new Z(Et + $, Lt + g / 2, -d));
                }
                for (let bt = 0; bt <= c; bt++) {
                  const $ = -g / 2 + bt * ft;
                  At.push(new Z(Et - z / 2, Lt + $, -d), new Z(Et + z / 2, Lt + $, -d));
                }
              ...
              if ((t.mostrarLabelsZapatas ?? 1) >= 0.5) {
                ...
                _t.push(Jt(`P=${D.toFixed(2)} tonf`, s.x, s.y, 0.3, "#fbbf24")), _t.push(Jt(`Mx=${tt.toFixed(2)}  My=${ft.toFixed(2)} tonf\xB7m`, s.x, s.y, 0.1, "#fbbf24")), _t.push(Jt(`${s.tipo[0].toUpperCase()} ${z.toFixed(2)}\xD7${g.toFixed(2)}\xD7${F.toFixed(2)}m \u03C3/q=${oo.toFixed(2)}`, Et, Lt, -d - F - 0.2, oo <= 1 ? "#4ade80" : oo <= 1.5 ? "#f59e0b" : "#ef4444"));
              }
            }
            const Yt = Math.round(t.sistemaCimentacion ?? 0);
            if (Yt === 1) {
              const s = Math.round(t.vigaAmarre_pos ?? 0), z = t.vigaAmarre_h ?? 0.4, g = t.vigaAmarre_b ?? 0.25, F = g * z, T = g * z ** 3 / 12, at = z * g ** 3 / 12, Et = 0.21 * Math.pow(Math.min(g, z), 3) * Math.max(g, z), Lt = /* @__PURE__ */ new Map();
              for (const _ of rt) {
                let E;
                s === 0 ? E = -d : E = -d / 2;
                const M = Wt(_.x, _.y, E);
                if (Lt.set(_.idx, M), s === 1 && d > 1e-3) {
                  const dt = Wt(_.x, _.y, -d / 2), Bt = Wt(_.x, _.y, 0), kt = Wt(_.x, _.y, -d);
                }
              }
              const Xt = /* @__PURE__ */ new Map(), Vt = /* @__PURE__ */ new Map();
              for (const _ of n) {
                const E = _.y.toFixed(4), M = _.x.toFixed(4);
                Xt.has(E) || Xt.set(E, []), Vt.has(M) || Vt.set(M, []), Xt.get(E).push(_), Vt.get(M).push(_);
              }
              const Kt = (_, E) => {
                const M = H.length;
                H.push([
                  _,
                  E
                ]), nt.set(M, bo), Rt.set(M, Co), Pt.set(M, ae), Tt.set(M, F), Dt.set(M, at), G.set(M, T), L.set(M, Et), I.set(M, uo);
              };
              let lt = 0;
              for (const _ of Xt.values()) {
                _.sort((E, M) => E.x - M.x);
                for (let E = 0; E < _.length - 1; E++) {
                  const M = Lt.get(_[E].idx), dt = Lt.get(_[E + 1].idx);
                  M !== void 0 && dt !== void 0 && (Kt(M, dt), lt++);
                }
              }
              for (const _ of Vt.values()) {
                _.sort((E, M) => E.y - M.y);
                for (let E = 0; E < _.length - 1; E++) {
                  const M = Lt.get(_[E].idx), dt = Lt.get(_[E + 1].idx);
                  M !== void 0 && dt !== void 0 && (Kt(M, dt), lt++);
              ...
              for (const _ of Xt.values()) {
                ...
                for (let M = 0; M < E.length - 1; M++) {
                  const dt = E[M], Bt = E[M + 1], kt = s === 0 ? -d : -d / 2;
            ...
            for (const s of rt) {
              ...
              let at = 0, Et = 0;
              s.tipo === "esquinera" ? (at = s.x < f / 2 ? -(g / 2 - C) : g / 2 - C, Et = s.y < i / 2 ? -(F / 2 - C) : F / 2 - C) : s.tipo === "lindero" && (Math.abs(s.x) < 1e-3 || Math.abs(s.x - f) < 1e-3 ? at = s.x < f / 2 ? -(g / 2 - C) : g / 2 - C : (Math.abs(s.y) < 1e-3 || Math.abs(s.y - i) < 1e-3) && (Et = s.y < i / 2 ? -(F / 2 - C) : F / 2 - C));
              const Lt = s.x - at, Xt = s.y - Et, Vt = [], Kt = [], lt = {
                elasticities: /* @__PURE__ */ new Map(),
                shearModuli: /* @__PURE__ */ new Map(),
              ...
              if (zo === 1 && no === 0) {
                const $ = t.vigaAmarre_b ?? 0.25, ot = s.idx, ht = n.filter((Mt) => Math.abs(Mt.y - s.y) < 1e-3 && Mt.idx !== ot).sort((Mt, $o) => Mt.x - $o.x), Ft = n.filter((Mt) => Math.abs(Mt.x - s.x) < 1e-3 && Mt.idx !== ot).sort((Mt, $o) => Mt.y - $o.y), Ot = ht.find((Mt) => Mt.x > s.x), Io = [
                  ...
                  for (let eo = 0; eo <= c; eo++) {
                    const Ve = eo === 0 || eo === c ? re / (2 * c) : re / c, le = r * $ * Ve, de = le * dt;
                    let io;
                    switch (Mt) {
                      case "x+":
                        io = _[eo][c];
    ...
    runModal(t, e, N) {
      ...
      if (!(!x.length || !S.length || !((_a = h.supports) == null ? void 0 : _a.size) || !((_b = y.densities) == null ? void 0 : _b.size))) try {
        const V = [], Y = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), it = /* @__PURE__ */ new Map(), xt = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), gt = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map();
        let mt = 0, b = 0;
        const l = [];
        let p = 0;
        ...
        const Q = {
          ...
          thicknesses: gt,
          poissonsRatios: U
        }, St = Math.round(t.nPisos), A = Math.min(60, Math.max(15, 3 * St + 6)), v = Ae(x, V, h, Q, A), B = Math.round(t.nVanosX), w = Math.round(t.nVanosY), ut = Math.round(t.nPisos), vt = b > 0 ? 1 + mt / b : 1;
        N.render(v, {
        ...
        console.warn("Modal edificio error:", V.message);
      }
    }
  };
});
