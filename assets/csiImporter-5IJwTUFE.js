import { K as Te, B as Ve, V as xe, L as ct, a as pt, C as ut, b as mt, S as ft, c as ht, a2 as St, q as $t, a4 as gt, v as It, d as et, g as tt, a6 as nt, h as Mt } from "./theme-BUyDDEHW.js";
...
function Tt(e) {
  ...
  let l = t.toUpperCase(), s = l.length - 1;
  const n = l.split("");
...
function Bt(e) {
  const t = new Te();
  t.name = `axis-${e.label}`;
  const l = new Ve().setFromPoints([new xe(...e.start), new xe(...e.end)]), s = new ct({ color: Lt, transparent: true, opacity: 0.85 }), n = new pt(l, s);
  t.add(n);
...
function At(e, t = 20) {
  ...
  a.computeLineDistances(), l.add(a);
...
O cambi\xE1 a otro provider (Gemini/Groq/OpenRouter) que solo requiere API key.`);
  ...
  if (!n.ok) {
    ...
    throw n.status === 404 ? new Error(`Modelo "${l}" no instalado. Ejecut\xE1: ollama pull ${l}`) : new Error(`Ollama error ${n.status}: ${r}`);
  }
  return ((_b = (await n.json()).message) == null ? void 0 : _b.content) ?? "";
} };
async function ot() {
  try {
    const e = new AbortController(), t = setTimeout(() => e.abort(), 1500), l = await fetch("http://localhost:11434/api/tags", { signal: e.signal }).catch(() => null);
    return clearTimeout(t), !l || !l.ok ? [] : ((await l.json()).models ?? []).map((n) => n.name);
  } catch {
    return [];
  }
}
async function at() {
  try {
    const e = new AbortController(), t = setTimeout(() => e.abort(), 1e3), l = await fetch("http://localhost:11434/api/tags", { signal: e.signal }).catch(() => null);
    return clearTimeout(t), !!l && l.ok;
  } catch {
    return false;
  }
}
const Pt = { id: "gemini", name: "\u2728 Gemini Flash (free tier)", supportsVision: true, models: [{ id: "gemini-2.0-flash-exp", name: "Gemini 2.0 Flash (m\xE1s nuevo)", vision: true }, { id: "gemini-1.5-flash", name: "Gemini 1.5 Flash (estable)", vision: true }, { id: "gemini-1.5-pro", name: "Gemini 1.5 Pro (mejor calidad)", vision: true }], defaultModel: "gemini-2.0-flash-exp", requiresKey: true, requiresLocal: false, async send({ msg: e, system: t, apiKey: l, model: s }) {
  var _a, _b, _c, _d, _e;
  const n = [{ text: e.text }];
  for (const o of e.images ?? []) n.push({ inline_data: { mime_type: o.mimeType, data: o.base64 } });
  const a = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${s}:generateContent?key=${l}`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ systemInstruction: { parts: [{ text: t }] }, contents: [{ role: "user", parts: n }], generationConfig: { temperature: 0.2, maxOutputTokens: 4096 } }) });
...
function Nt(e) {
  return new Promise((t, l) => {
    ...
    s.onload = () => {
      ...
      t(a >= 0 ? n.slice(a + 1) : n);
    }, s.onerror = () => l(s.error), s.readAsDataURL(e);
  });
}
...
function _n(e) {
  ...
  r.addButton({ title: "\u{1F5B1} Seleccionar" }).on("click", () => i("select")), r.addButton({ title: "\u25CF Nodo" }).on("click", () => i("node")), r.addButton({ title: "\uFF0F L\xEDnea (frame)" }).on("click", () => i("line")), r.addButton({ title: "\u25A6 \xC1rea 4-clics (shell Q4)" }).on("click", () => i("area")), r.addButton({ title: "\u25AD \xC1rea rectangular (2 clics)" }).on("click", () => i("rectarea")), r.addButton({ title: "\u2B21 \xC1rea libre (pol\xEDgono \u2192 malla)" }).on("click", () => i("polyarea")), r.addButton({ title: "\u25E3 Plano inclinado (3 puntos)" }).on("click", () => i("plane3")), r.addButton({ title: "\u2B1B Plano XY (reset horizontal)" }).on("click", () => {
    var _a2;
    return (_a2 = window.__hekatanResetPlaneXY) == null ? void 0 : _a2.call(window);
  }), r.addButton({ title: "\u258C Columna 3D (1 click + altura)" }).on("click", () => i("col")), r.addButton({ title: "\u25A5 Pared Q4 3D (2 clicks + altura)" }).on("click", () => i("wall")), r.addButton({ title: "\u2312 Polil\xEDnea" }).on("click", () => i("polyline")), r.addButton({ title: "\u25AD Rect\xE1ngulo" }).on("click", () => i("rect")), r.addButton({ title: "\u25CB C\xEDrculo" }).on("click", () => i("circle")), r.addButton({ title: "\u2312 Arco (3 ptos)" }).on("click", () => i("arc")), r.addButton({ title: "\u250A L\xEDnea auxiliar" }).on("click", () => i("aux")), r.addButton({ title: "\u2726 Punto auxiliar" }).on("click", () => i("auxp")), r.addButton({ title: "\u2197 Prolongar l\xEDnea" }).on("click", () => i("extend")), r.addButton({ title: "\u25B1 Losa con chaflanes (rect + arcos)" }).on("click", () => i("chaflan")), r.addButton({ title: "\u{1F5D1} Borrar (hover + click)" }).on("click", () => i("delete"));
  const d = r.addFolder({ title: "\u{1F3AF} Modos de dibujo", expanded: true }), b = { ortho: false, polar: false, segs: 12 };
  d.addBinding(b, "ortho", { label: "ORTO (90\xB0)" }).on("change", (c) => {
    window.__hekatanOrtho = c.value;
  }), d.addBinding(b, "polar", { label: "POLAR (45\xB0)" }).on("change", (c) => {
    window.__hekatanPolar = c.value;
  }), d.addBinding(b, "segs", { min: 4, max: 64, step: 1, label: "Segmentos arc/c\xEDrc" }).on("change", (c) => {
    window.__hekatanArcSegs = c.value;
  });
  const v = { r: 1 };
  d.addBinding(v, "r", { min: 0.1, max: 5, step: 0.1, label: "Chafl\xE1n r (m)" }).on("change", (c) => {
    window.__hekatanChaflanR = c.value;
  }), window.__hekatanChaflanR = 1;
  const p = r.addFolder({ title: "\u{1F3AF} Object Snap (OSNAP)", expanded: false }), y = window.__hekatanOsnap ?? { end: true, mid: true, node: true, cen: true, per: false, nea: false, int: false };
  ...
  const k = r.addFolder({ title: "\u{1F4D0} Plano de trabajo", expanded: true }), E = { workZ: 0 }, x = (c, m, I = true) => {
    var _a2, _b;
  ...
  const u = { snapEnabled: true }, g = r.addBinding(u, "snapEnabled", { label: "\u{1F9F2} Grid snap (F9)" }).on("change", (c) => {
  ...
  const w = { step: 0.5 };
  r.addBinding(w, "step", { label: "Paso snap (m)", options: { "0.01 m (mm)": 0.01, "0.05 m (5cm)": 0.05, "0.10 m": 0.1, "0.20 m": 0.2, "0.25 m": 0.25, "0.50 m": 0.5, "1.00 m": 1, "2.00 m": 2, "5.00 m": 5 } }).on("change", (c) => {
    var _a2, _b;
  ...
  }), r.addBinding(E, "workZ", { min: -10, max: 50, step: 0.1, label: "Cota Z (m)" }).on("change", (c) => {
  ...
  P.addBinding(Y, "Ux", { label: "DOF Ux (restringido)" }), P.addBinding(Y, "Uy", { label: "DOF Uy (restringido)" }), P.addBinding(Y, "Uz", { label: "DOF Uz (restringido)" }), P.addBinding(Y, "Rx", { label: "DOF Rx (restringido)" }), P.addBinding(Y, "Ry", { label: "DOF Ry (restringido)" }), P.addBinding(Y, "Rz", { label: "DOF Rz (restringido)" }), P.addButton({ title: "\u{1F4CC} Aplicar apoyo a nodos seleccionados" }).on("click", () => {
    ...
    const m = window.__hekatanCadSupports ?? {}, I = [Y.Ux, Y.Uy, Y.Uz, Y.Rx, Y.Ry, Y.Rz];
    let O = 0;
    for (const z of c) {
      const A = z.split(":");
      if (A[0] !== "pt") continue;
  ...
  P.addBinding(Z, "height", { min: 0.1, max: 50, step: 0.1, label: "Altura extrusi\xF3n (m)" }), P.addBinding(Z, "dirX", { min: -1, max: 1, step: 1, label: "Dir X" }), P.addBinding(Z, "dirY", { min: -1, max: 1, step: 1, label: "Dir Y" }), P.addBinding(Z, "dirZ", { min: -1, max: 1, step: 1, label: "Dir Z" }), P.addButton({ title: "\u2B06 Extruir nodo\u2192frame (1 nodo seleccionado + altura)" }).on("click", () => {
    ...
    const m = [Z.dirX, Z.dirY, Z.dirZ], I = Math.hypot(...m);
    if (I < 0.01) {
      alert("Dir X/Y/Z son todos cero. Eleg\xED al menos uno (default +Z = vertical).");
      return;
    }
    const O = m.map((H) => H / I), z = Z.height, A = n.points, R = n.polylines;
    if (!A || !R) return;
...
Inici\xE1 Ollama o instalalo desde ollama.com`);
    ...
    const O = m.map((q) => q / I), z = Z.height, A = n.points, R = n.polylines, X = n.areas;
    if (!A || !R || !X) return;
...
function Ht(e) {
  const t = e.length, l = e[0].length, s = Be(l, t);
  for (let n = 0; n < t; n++) for (let a = 0; a < l; a++) s[a][n] = e[n][a];
  return s;
}
function st(e, t) {
  const l = e.length, s = t[0].length, n = t.length, a = Be(l, s);
  for (let r = 0; r < l; r++) for (let o = 0; o < s; o++) {
    let i = 0;
    for (let d = 0; d < n; d++) i += e[r][d] * t[d][o];
    a[r][o] = i;
  }
  return a;
}
function Ut(e, t) {
  const { E: l, G: s, A: n, Iy: a, Iz: r, J: o } = e, i = Be(12), d = l * n / t;
  i[0][0] = d, i[6][6] = d, i[0][6] = -d, i[6][0] = -d;
  const b = s * o / t;
  i[3][3] = b, i[9][9] = b, i[3][9] = -b, i[9][3] = -b;
  const v = t, p = l * r, y = 12 * p / v ** 3, k = 6 * p / v ** 2, E = 4 * p / v, x = 2 * p / v;
  i[1][1] = y, i[1][5] = k, i[1][7] = -y, i[1][11] = k, i[5][1] = k, i[5][5] = E, i[5][7] = -k, i[5][11] = x, i[7][1] = -y, i[7][5] = -k, i[7][7] = y, i[7][11] = -k, i[11][1] = k, i[11][5] = x, i[11][7] = -k, i[11][11] = E;
  const f = l * a, h = 12 * f / v ** 3, S = 6 * f / v ** 2, u = 4 * f / v, g = 2 * f / v;
  return i[2][2] = h, i[2][4] = -S, i[2][8] = -h, i[2][10] = -S, i[4][2] = -S, i[4][4] = u, i[4][8] = S, i[4][10] = g, i[8][2] = -h, i[8][4] = S, i[8][8] = h, i[8][10] = S, i[10][2] = -S, i[10][4] = g, i[10][8] = S, i[10][10] = u, i;
}
function Vt(e, t) {
  const l = t[0] - e[0], s = t[1] - e[1], n = t[2] - e[2], a = Math.sqrt(l * l + s * s + n * n);
  if (a < 1e-12) throw new Error("Element length is zero");
  const r = [l / a, s / a, n / a], o = Math.abs(r[0]) < 1e-6 && Math.abs(r[1]) < 1e-6;
  let i;
  if (o) i = [0, 1, 0];
  else {
    const b = [0, 0, 1], v = r[1] * b[2] - r[2] * b[1], p = r[2] * b[0] - r[0] * b[2], y = r[0] * b[1] - r[1] * b[0], k = Math.sqrt(v * v + p * p + y * y), E = [v / k, p / k, y / k];
    i = [E[1] * r[2] - E[2] * r[1], E[2] * r[0] - E[0] * r[2], E[0] * r[1] - E[1] * r[0]];
  }
  const d = [r[1] * i[2] - r[2] * i[1], r[2] * i[0] - r[0] * i[2], r[0] * i[1] - r[1] * i[0]];
  return { L: a, R: [r, i, d] };
}
function Gt(e) {
  const t = Be(12);
...
function bt(e, t) {
  const { L: l, R: s } = Vt(e.ni, e.nj), n = Ut(t, l), a = Gt(s), r = st(st(Ht(a), n), a);
  return { L: l, K_local: n, T: a, K_global: r, R: s };
}
function it(e, t = 4) {
  return e.length, e.map((l) => l.map((s) => {
...
function Yt() {
  return Le || (Le = new Promise((e, t) => {
    ...
    l.rel = "stylesheet", l.href = "https://cdn.jsdelivr.net/npm/katex@0.16.10/dist/katex.min.css", document.head.appendChild(l);
...
async function Zt(e, t, l) {
  ...
  for (const o of a) {
    ...
    Object.assign(i.style, { marginBottom: "18px" });
    const d = document.createElement("div");
    d.textContent = o.title, Object.assign(d.style, { fontWeight: "600", color: "#a5b4fc", marginBottom: "8px", fontSize: "12px" }), i.appendChild(d);
    for (const b of o.content) {
      const v = document.createElement("div");
      if (Object.assign(v.style, { padding: "6px 10px", marginBottom: "4px", background: "rgba(255,255,255,0.04)", borderRadius: "4px", fontSize: "12px", lineHeight: "1.7" }), r) try {
        r.render(b, v, { throwOnError: false, displayMode: true });
      } catch {
        v.textContent = b;
      }
...
function Jt() {
  ...
  Object.assign(l.style, { background: "#f59e0b", color: "#1a1a1a", padding: "2px 8px", borderRadius: "4px", fontWeight: "600", fontSize: "11px" }), l.textContent = "Element \u2014", t.appendChild(l);
  ...
  const r = ["Tabla", "Matem\xE1tica", "Resumen"], o = [];
  let i = 0;
  for (let u = 0; u < r.length; u++) {
    const g = document.createElement("button");
    g.textContent = r[u], Object.assign(g.style, { flex: "1", background: "transparent", border: "none", color: u === 0 ? "#a5b4fc" : "#94a3b8", borderBottom: u === 0 ? "2px solid #a5b4fc" : "2px solid transparent", padding: "8px 0", cursor: "pointer", fontSize: "12px", fontWeight: u === 0 ? "600" : "400" }), g.onclick = () => y(u), a.appendChild(g), o.push(g);
  }
  const d = document.createElement("div");
  ...
  function E() {
    ...
    }), w.appendChild(C), d.appendChild(w), d.appendChild(h("2. K_local (12\xD712)", g.K_local)), d.appendChild(h("3. T \u2014 Transformaci\xF3n", g.T)), d.appendChild(h("4. K_global = T^T \xB7 K_local \xB7 T", g.K_global));
  }
  function x() {
...
`), p && ((_a = d.querySelector(".hk-copy-Klocal")) == null ? void 0 : _a.addEventListener("click", () => {
  ...
  const S = { el: e, show(u) {
    v = u, l.textContent = `Element ${u.index}`;
...
function en(e = {}) {
  ...
  Object.assign(l.style, { display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", borderBottom: "1px solid rgba(255,255,255,0.1)", cursor: "move", userSelect: "none", background: "rgba(255,255,255,0.03)" });
  const s = document.createElement("span");
  s.textContent = "\u{1F4C8} Modal \u2014 \u2014", Object.assign(s.style, { flex: "1", fontWeight: "600", color: "#a5b4fc", fontSize: "12px" }), l.appendChild(s);
  const n = document.createElement("button");
  n.textContent = "\xD7", Object.assign(n.style, { background: "transparent", border: "none", color: "#e2e8f0", fontSize: "20px", cursor: "pointer", padding: "0 6px", lineHeight: "1" }), n.onclick = () => g.hide(), l.appendChild(n), t.appendChild(l);
  const a = document.createElement("div");
  ...
  const i = document.createElement("div");
  Object.assign(i.style, { display: "flex", alignItems: "center", gap: "8px", padding: "8px 12px", borderTop: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.03)" });
  const d = Ke("\u25C0"), b = Ke("\u25B6"), v = Ke("\u25B6\u25B6");
  i.appendChild(d), i.appendChild(b), i.appendChild(v);
  const p = document.createElement("input");
  p.type = "range", p.min = "1", p.max = "1", p.value = "1", Object.assign(p.style, { flex: "1" }), i.appendChild(p);
  const y = document.createElement("span");
  Object.assign(y.style, { color: "#fde68a", fontSize: "11px", minWidth: "120px", textAlign: "right" }), y.textContent = "Modo 1", i.appendChild(y), t.appendChild(i);
  let k = null;
  ...
  d.onclick = () => h(E.val - 1), v.onclick = () => h(E.val + 1), p.oninput = () => h(parseInt(p.value) - 1), b.onclick = () => {
    x ? g.pause() : g.play();
  };
  function S() {
  ...
  function u(w, C) {
    var _a2;
    f = w, (C == null ? void 0 : C.title) && (s.textContent = `\u{1F4C8} ${C.title}`), a.innerHTML = ((C == null ? void 0 : C.properties) ?? []).map((M) => `<div>${tn(M)}</div>`).join(""), nn(o, w, r);
...
function nn(e, t, l) {
  ...
  let o = 0, i = 0, d = 0, b = -1, v = -1, p = -1, y = -1, k = -1;
  for (let x = 0; x < s; x++) {
    ...
    o += g, i += w, d += _, b < 0 && g > 0.5 && (b = x), v < 0 && w > 0.5 && (v = x), p < 0 && _ > 0.5 && (p = x), y < 0 && o > 0.9 && (y = x), k < 0 && i > 0.9 && (k = x);
    let M = "\u2014";
    ...
    $.dataset.mode = String(x), Object.assign($.style, { borderBottom: "1px solid rgba(255,255,255,0.04)", cursor: "pointer" }), $.onclick = () => void 0, $.innerHTML = `
      ...
      <td style="padding:3px 4px;text-align:right;color:#94a3b8">${le(i)}</td>
      <td style="padding:3px 4px;text-align:right;color:#94a3b8">${le(d)}</td>
      <td style="padding:3px 4px;text-align:left;color:#a5b4fc">${M}</td>
    `, r.appendChild($);
  }
  n.appendChild(r), e.appendChild(n), y >= 0 && k >= 0 ? (l.innerHTML = `<b>ASCE 7-22 \xA712.9.1:</b> \u2713 90% alcanzado en X (modo ${y + 1}) e Y (modo ${k + 1}) de ${s}`, l.style.color = "#86efac") : (l.innerHTML = `<b>ASCE 7-22 \xA712.9.1:</b> \u26A0 Solo \u03A3Ux=${le(o)} \u03A3Uy=${le(i)} con ${s} modos. Considera aumentar.`, l.style.color = "#fcd34d");
}
function le(e) {
...
function on() {
  ...
  const l = document.createElement("span");
  l.textContent = "\u{1F4DC} Solver Log", Object.assign(l.style, { flex: "1", fontWeight: "600", color: "#a5b4fc", fontSize: "12px" }), t.appendChild(l);
  ...
  t.addEventListener("mousedown", (i) => {
    ...
    a = { x: i.clientX - d.left, y: i.clientY - d.top }, i.preventDefault();
  ...
  function r(i) {
    var _a, _b, _c;
    const d = [];
    if (d.push('<div style="font-weight:600;color:#fde68a;margin-bottom:4px">FEM Solver</div>'), d.push('<div style="margin-bottom:8px;color:#cbd5e1">'), d.push(`  <div>Modelo: <b>${i.nNodes}</b> nodos, <b>${i.nElements}</b> elem</div>`), i.nFrames != null && d.push(`  <div>Frames: ${i.nFrames}${i.nShells ? ` &nbsp;|&nbsp; Shells: ${i.nShells}` : ""}${i.nSolids ? ` &nbsp;|&nbsp; Solids: ${i.nSolids}` : ""}</div>`), d.push(`  <div>Apoyos: ${i.nSupports} &nbsp;|&nbsp; Cargas: ${i.nLoads}</div>`), d.push(`  <div>DOFs: ${i.totalDOFs} total, ~${i.freeDOFs} libres</div>`), d.push("</div>"), d.push(`<div style="color:#a5b4fc;font-weight:600">1. Ensamblaje K global (${i.totalDOFs}\xD7${i.totalDOFs})</div>`), d.push('<div style="margin-left:10px;color:#cbd5e1;font-style:italic;font-size:10.5px">K_global = \u03A3 T^T \xB7 K_local \xB7 T</div>'), ((_a = i.timings) == null ? void 0 : _a.solve) != null && d.push(`<div style="color:#a5b4fc;font-weight:600;margin-top:6px">2. K \xB7 u = F  \u2192  ${i.solverName ?? "SparseLU"}  \u2192  <span style="color:#86efac">${i.timings.solve.toFixed(1)} ms</span></div>`), i.maxDisplacement) {
      const b = i.maxDisplacement;
      d.push('<div style="color:#a5b4fc;font-weight:600;margin-top:6px">3. Desplazamientos:</div>'), d.push(`<div style="margin-left:10px;color:#cbd5e1">max|u| = <b style="color:#fde68a">${b.value.toExponential(4)} m</b> (nodo ${b.nodeIdx}, ${b.component})</div>`);
    }
    ((_b = i.timings) == null ? void 0 : _b.internalForces) != null && (d.push(`<div style="color:#a5b4fc;font-weight:600;margin-top:6px">4. Fuerzas internas: <span style="color:#86efac">${i.timings.internalForces.toFixed(1)} ms</span></div>`), d.push('<div style="margin-left:10px;color:#cbd5e1;font-style:italic;font-size:10.5px">F_int = K_local \xB7 T \xB7 u</div>')), ((_c = i.timings) == null ? void 0 : _c.total) != null && d.push(`<div style="margin-top:10px;padding:6px 8px;background:rgba(134,239,172,0.1);border-left:3px solid #86efac;border-radius:3px">\u2713 Completado: <b>${i.timings.total.toFixed(1)} ms</b></div>`), n.innerHTML = d.join(`
`);
  }
  gt((i, d) => {
    d.background && (e.style.background = `${d.background}EE`);
  }), document.body.appendChild(e);
  const o = { el: e, update(i) {
    r(i);
  }, show() {
...
function dn(e, t = {}) {
  ...
  for (const a of l) {
    ...
    const o = r.match(sn);
    if (o) {
      s.push({ type: "section", source: r, title: o[1] });
      continue;
    }
    const i = r.match(ln);
    if (i) {
      s.push({ type: "comment", source: r, value: i[1] });
      continue;
    }
    const d = r.match(rn);
    let b, v;
    d ? (v = d[1], b = d[2]) : b = r.trim();
    try {
      const p = an(b, n);
      v && (n[v] = p), s.push({ type: v ? "assign" : "expr", source: r, name: v, value: p, latex: yt(v, p) });
    } catch (p) {
      s.push({ type: "error", source: r, error: p.message ?? String(p) });
    }
  }
  return s;
}
...
function lt(e) {
  ...
  if (!Array.isArray(e[0])) {
    const a = e.map((r) => typeof r == "number" ? je(r) : String(r));
  ...
  const t = e, l = t.length, s = t[0].length;
  return l > 8 || s > 8 ? `\\text{Matriz } ${l} \\times ${s}\\text{ (truncada)}` : `\\begin{bmatrix} ${t.map((a) => a.map((r) => typeof r == "number" ? je(r) : String(r)).join(" & ")).join(" \\\\ ")} \\end{bmatrix}`;
}
function cn(e) {
  const t = {};
  for (const l of Object.keys(e)) e[l] != null && (t[l] = e[l]);
...
` }];
  ...
  const r = y("\u2753 Funciones", "rgba(255,255,255,0.06)", "rgba(255,255,255,0.2)");
...
T1 = 0.1 * N    % aprox NEC-SE-DS
  ...
  i.textContent = "\u{1F9EE} Calculadora FEM", Object.assign(i.style, { flex: "1", fontWeight: "600", color: "#a5b4fc", fontSize: "13px", textAlign: "center" }), l.appendChild(i);
  const d = document.createElement("button");
  d.textContent = "\u25B6 Ejecutar (Ctrl+Enter)", Object.assign(d.style, { background: "#2d8659", border: "none", color: "white", padding: "5px 14px", borderRadius: "4px", cursor: "pointer", fontSize: "11.5px", fontWeight: "600" }), d.onclick = () => _.evaluate(), l.appendChild(d);
  ...
  p.textContent = "\xD7", Object.assign(p.style, { background: "transparent", border: "none", color: "#e2e8f0", fontSize: "20px", cursor: "pointer", padding: "0 6px", lineHeight: "1" }), p.onclick = () => _.hide(), l.appendChild(p), t.appendChild(l);
  function y(M, $, L) {
...
`;
  ...
  l.addEventListener("mousedown", (M) => {
    if (M.target !== l && M.target !== i) return;
...
async function mn(e, t) {
  ...
  t.forEach((s, n) => {
    ...
    if (Object.assign(r.style, { fontFamily: "ui-monospace, Menlo, monospace", fontSize: "10.5px", color: "#64748b", marginBottom: "3px" }), r.textContent = `${n + 1}: ${s.source}`, a.appendChild(r), s.type === "section") Object.assign(a.style, { marginTop: "12px", marginBottom: "8px", padding: "5px 10px", background: "rgba(165,180,252,0.12)", borderLeft: "3px solid #a5b4fc", borderRadius: "3px" }), a.innerHTML = `<div style="font-weight:700;color:#a5b4fc;font-size:13px">${fn(s.title || "")}</div>`;
      ...
      const o = document.createElement("div");
      if (Object.assign(o.style, { padding: "6px 10px", background: "rgba(255,255,255,0.03)", borderLeft: "2px solid #fde68a", borderRadius: "3px", overflow: "auto" }), s.latex && l) try {
        l.render(s.latex, o, { throwOnError: false, displayMode: true });
      } catch {
...
function hn(e) {
  ...
  return { addNode(s, n, a) {
    ...
    return r.push([s, n, a]), e.nodes.val = r, t(), r.length - 1;
  }, addFrame(s, n) {
    ...
    var _a, _b;
    const s = e.nodes.val.length, n = e.elements.val.length, a = ((_a = e.nodeInputs.val.supports) == null ? void 0 : _a.size) ?? 0, r = ((_b = e.nodeInputs.val.loads) == null ? void 0 : _b.size) ?? 0, o = `Model: ${s} nodes, ${n} elements (${a} supports, ${r} loads)`;
...
function xn(e, t) {
  const l = e.trim();
  if (!l || l.startsWith("//") || l.startsWith("#")) return { ok: true, result: void 0 };
  if (l === "help" || l === "?") return { ok: true, result: wt };
  try {
    return { ok: true, result: new Function("cad", `return (${l});`)(t) };
  } catch (s) {
    try {
      return new Function("cad", l)(t), { ok: true, result: void 0 };
    } catch (n) {
...
function bn(e) {
  ...
  const l = document.createElement("div");
  Object.assign(l.style, { display: "flex", alignItems: "center", gap: "8px", padding: "6px 10px", borderBottom: "1px solid rgba(255,255,255,0.1)", cursor: "move", userSelect: "none", background: "rgba(255,255,255,0.03)", color: "#a5b4fc", fontWeight: "600", fontSize: "12px" });
  const s = document.createElement("span");
  s.textContent = "\u{1F4BB} CLI", Object.assign(s.style, { flex: "1" }), l.appendChild(s);
  const n = document.createElement("button");
  ...
  const a = document.createElement("button");
  a.textContent = "\xD7", Object.assign(a.style, { background: "transparent", border: "none", color: "#e2e8f0", fontSize: "18px", cursor: "pointer", padding: "0 4px", lineHeight: "1" }), a.onclick = () => E.hide(), l.appendChild(a), t.appendChild(l);
  const r = document.createElement("div");
  ...
  const i = document.createElement("span");
  i.textContent = "\u203A ", Object.assign(i.style, { color: "#fde68a", marginRight: "6px", fontWeight: "600" }), o.appendChild(i);
  const d = document.createElement("input");
  d.type = "text", d.placeholder = "cad.addNode(0,0,0)  \xB7  type 'help'", Object.assign(d.style, { flex: "1", border: "none", background: "transparent", color: "#e2e8f0", outline: "none", fontFamily: "inherit", fontSize: "12px" }), o.appendChild(d), t.appendChild(o);
  let b = null;
  l.addEventListener("mousedown", (x) => {
    if (x.target !== l && x.target !== s) return;
  ...
  y(gn(), "info"), d.addEventListener("keydown", (x) => {
    if (x.key === "Enter") k(d.value), d.value = "";
    else if (x.key === "ArrowUp") p > 0 && (p--, d.value = v[p], x.preventDefault());
    else if (x.key === "ArrowDown") p < v.length - 1 ? (p++, d.value = v[p]) : (p = v.length, d.value = ""), x.preventDefault();
    else if (x.key === "l" && x.ctrlKey) r.innerHTML = "", x.preventDefault();
    else if (x.key === "Tab") {
      const f = d.value, h = f.match(/cad\.(\w*)$/);
      if (h) {
        const u = Object.keys(e.cad).filter((g) => typeof e.cad[g] == "function").filter((g) => g.startsWith(h[1]));
        u.length === 1 ? d.value = f.slice(0, -h[1].length) + u[0] + "(" : u.length > 1 && y(u.join("  "), "info");
      }
      x.preventDefault();
    }
  }), document.body.appendChild(t);
...
function wn(e) {
  ...
  const { nodes: t, elements: l, nodeInputs: s, elementInputs: n, deformOutputs: a, modalOutputs: r } = e, o = [];
  ...
  const i = l.filter((p) => p.length === 2).length, d = l.filter((p) => p.length === 3 || p.length === 4).length, b = l.filter((p) => p.length === 8).length, v = t.length * 6;
  o.push(`<table style="width:auto"><tr><td style="text-align:left">Number of nodes</td><td>${t.length}</td></tr>`), o.push(`<tr><td style="text-align:left">Number of elements</td><td>${l.length} (${i} frames, ${d} shells, ${b} solids)</td></tr>`), o.push('<tr><td style="text-align:left">DOFs per node</td><td>6 (u<sub>x</sub>, u<sub>y</sub>, u<sub>z</sub>, \u03B8<sub>x</sub>, \u03B8<sub>y</sub>, \u03B8<sub>z</sub>)</td></tr>'), o.push(`<tr><td style="text-align:left">Total DOFs</td><td>${v}</td></tr></table>`), o.push("<h3>1.1 Node Coordinates</h3>"), o.push("<table><tr><th>Node</th><th>x</th><th>y</th><th>z</th></tr>");
  ...
  for (let p = 0; p < Math.min(l.length, 40); p++) {
    const y = l[p], k = y.length === 2 ? "Frame" : y.length === 3 ? "Shell-CST" : y.length === 4 ? "Shell-Q4" : "Solid-H8";
    o.push(`<tr><td>${p}</td><td>${k}</td><td>${y.join(" \u2192 ")}</td></tr>`);
  }
  l.length > 40 && o.push(`<tr><td colspan="3" style="text-align:center;font-style:italic">... ${l.length - 40} m\xE1s</td></tr>`), o.push("</table>"), o.push("<h3>1.3 Section Properties (Frames)</h3>"), o.push("<table><tr><th>Element</th><th>E</th><th>A</th><th>I<sub>y</sub></th><th>I<sub>z</sub></th><th>J</th></tr>");
  for (let p = 0; p < Math.min(i, 30); p++) {
  ...
  if (i > 30 && o.push(`<tr><td colspan="6" style="text-align:center;font-style:italic">... ${i - 30} m\xE1s</td></tr>`), o.push("</table>"), o.push("<h3>1.4 Boundary Conditions (Supports)</h3>"), s.supports && s.supports.size > 0) {
...
function vn(e) {
  const t = wn(e), l = window.open("", "_blank", "width=1100,height=800");
  if (!l) {
    alert("Popup bloqueado. Permite popups para ver el reporte.");
    return;
  }
  l.document.open(), l.document.write(t), l.document.close();
}
function Fe(e) {
...
class kn {
  ...
  toggleInspect(t) {
    ...
    const l = t.nodes.rawVal, s = t.elements.rawVal, n = t.elementInputs.rawVal, a = s.findIndex((y) => (y == null ? void 0 : y.length) === 2);
    if (a < 0) {
      alert("No hay elementos frame en el modelo. Carga un ejemplo con frames primero.");
      return;
    }
    const r = s[a], o = ((_a = n.elasticities) == null ? void 0 : _a.get(a)) ?? 2e8, i = ((_b = n.shearModuli) == null ? void 0 : _b.get(a)) ?? o / 2.6, d = ((_c = n.areas) == null ? void 0 : _c.get(a)) ?? 0.01, b = ((_d = n.momentsOfInertiaY) == null ? void 0 : _d.get(a)) ?? 1e-4, v = ((_e = n.momentsOfInertiaZ) == null ? void 0 : _e.get(a)) ?? 1e-4, p = ((_f = n.torsionalConstants) == null ? void 0 : _f.get(a)) ?? 1e-4;
    this._inspect.show({ index: a, type: "frame", nodes: [l[r[0]], l[r[1]]], nodeIndices: [r[0], r[1]], frameProps: { E: o, G: i, A: d, Iy: b, Iz: v, J: p }, sectionLabel: ((_g = n.sectionLabels) == null ? void 0 : _g.get(a)) ?? "\u2014" });
  }
  toggleModal(t) {
    var _a;
    ...
    if (((_a = t.currentExample) == null ? void 0 : _a.hasModal) && t.currentExample.runModal) {
      const l = { nodes: t.nodes, elements: t.elements, nodeInputs: t.nodeInputs, elementInputs: t.elementInputs, deformOutputs: t.deformOutputs, analyzeOutputs: t.analyzeOutputs, objects3D: t.objects3D };
      try {
        t.currentExample.runModal(t.currentParams ?? {}, l, t.modalPanelLegacy);
      } catch (s) {
  ...
  toggleSolverLog(t) {
    ...
    const l = t.nodes.rawVal, s = t.elements.rawVal, n = t.nodeInputs.rawVal, a = t.deformOutputs.rawVal, r = ((_a = n.supports) == null ? void 0 : _a.size) ?? 0, o = ((_b = n.loads) == null ? void 0 : _b.size) ?? 0, i = l.length * 6;
    let d = 0, b = -1, v = "uz";
    if (a == null ? void 0 : a.deformations) for (const [E, x] of a.deformations) for (let f = 0; f < 3; f++) Math.abs(x[f]) > d && (d = Math.abs(x[f]), b = E, v = ["ux", "uy", "uz"][f]);
    const p = s.filter((E) => E.length === 2).length, y = s.filter((E) => E.length === 3 || E.length === 4).length, k = s.filter((E) => E.length === 8).length;
    this._solverLog.update({ nNodes: l.length, nElements: s.length, nFrames: p, nShells: y, nSolids: k, nSupports: r, nLoads: o, totalDOFs: i, freeDOFs: i - r * 6, timings: { assembly: 1, solve: 2, internalForces: 1, total: 6 }, maxDisplacement: b >= 0 ? { value: d, nodeIdx: b, component: v } : void 0, solverName: "SparseLU (deformCpp)" }), this._solverLog.show();
  }
  toggleCalc(t) {
  ...
  toggleCli(t) {
    if (!this._cli) {
      const l = hn({ nodes: t.nodes, elements: t.elements, nodeInputs: t.nodeInputs, elementInputs: t.elementInputs, deformOutputs: t.deformOutputs, onModelChange: () => {
        var _a;
  ...
  forceRecalc(t) {
    ...
    try {
      ...
      alert("Error al recalcular: " + l.message);
    }
  }
  destroy() {
    var _a, _b, _c, _d, _e;
...
function Sn(e, t, l = {}) {
  var _a;
  const s = En(), n = { inspect: true, modal: true, solverLog: true, calc: true, cli: true, report: true, recalc: true, ...l.visibleButtons ?? {} }, a = e.addFolder({ title: l.title ?? "\u{1F6E0} Herramientas FEM", expanded: l.expanded ?? false });
...
function $n(e, t = 2, l = 5) {
  ...
  const a = (h, S, u) => `${h.toFixed(4)}_${S.toFixed(4)}_${u.toFixed(4)}`, r = /* @__PURE__ */ new Map();
  ...
  for (const h of n) {
    ...
    for (let C = 0; C <= l; C++) {
      const _ = [];
      for (let M = 0; M <= l; M++) {
        const $ = C / l, L = M / l, j = (1 - $) * (1 - L) * u[0][0] + $ * (1 - L) * u[1][0] + $ * L * u[2][0] + (1 - $) * L * u[3][0], B = (1 - $) * (1 - L) * u[0][1] + $ * (1 - L) * u[1][1] + $ * L * u[2][1] + (1 - $) * L * u[3][1], N = a(j, B, g);
        let W = r.get(N);
        W === void 0 && (e.nodes.push([j, B, g]), W = e.nodes.length - 1, r.set(N, W), s.newNodes++), _.push(W);
      }
      w.push(_);
    }
    for (let C = 0; C < l; C++) for (let _ = 0; _ < l; _++) {
      const M = e.elements.length + o.length;
      o.push([w[C][_], w[C + 1][_], w[C + 1][_ + 1], w[C][_ + 1]]), i.set(M, h), s.newSlabElements++;
    }
  }
  for (let h = 0; h < e.elements.length; h++) {
    const S = e.elements[h];
    ...
    for (let B = 1; B < j.length; B++) {
      ...
      o.push(j[B]), i.set(N, h), s.splitFrameSegments++;
    }
  }
  for (const h of o) e.elements.push(h);
  const d = e.elementInputs, b = [d.elasticities, d.shearModuli, d.poissonsRatios, d.thicknesses, d.densities, d.areas, d.momentsOfInertiaZ, d.momentsOfInertiaY, d.torsionalConstants, d.shearAreasY, d.shearAreasZ, d.plateFormulations];
  for (const [h, S] of i) {
    for (const u of b) u && u.has(S) && u.set(h, u.get(S));
    ((_a = d.sectionShapes) == null ? void 0 : _a.has(S)) && d.sectionShapes.set(h, d.sectionShapes.get(S)), ((_b = d.rigidOffsets) == null ? void 0 : _b.has(S)) && d.rigidOffsets.set(h, [...d.rigidOffsets.get(S)]);
  }
  ...
  return e.elements = y, e.elementTypes = k, e.elementNames = E, e.elementStories = x, e.elementInputs.elasticities = f(d.elasticities), e.elementInputs.shearModuli = f(d.shearModuli), e.elementInputs.poissonsRatios = f(d.poissonsRatios), e.elementInputs.thicknesses = f(d.thicknesses), e.elementInputs.densities = f(d.densities), e.elementInputs.areas = f(d.areas), e.elementInputs.momentsOfInertiaZ = f(d.momentsOfInertiaZ), e.elementInputs.momentsOfInertiaY = f(d.momentsOfInertiaY), e.elementInputs.torsionalConstants = f(d.torsionalConstants), e.elementInputs.shearAreasY = f(d.shearAreasY), e.elementInputs.shearAreasZ = f(d.shearAreasZ), e.elementInputs.plateFormulations = f(d.plateFormulations), e.elementInputs.sectionShapes = f(d.sectionShapes), e.elementInputs.rigidOffsets = f(d.rigidOffsets), e.elementSections = f(e.elementSections), s;
}
function In(e) {
  const { mesh: t, viewerElm: l, onStatusChange: s } = e, n = e.scalePercent ?? 5, [a, r] = e.visFrequencyRange ?? [0.5, 3];
  let o = null, i = 0, d = 0, b = [], v = [];
  function p() {
    s == null ? void 0 : s();
  }
  function y() {
    ...
    const f = o.frequencies[i] ?? 0, h = f > 0 ? 1 / f : 0, S = ["Ux", "Uy", "Uz", "Rx", "Ry", "Rz"], u = (_a = o.massParticipation) == null ? void 0 : _a[i];
    let g = "\u2014";
    if (u) {
      let w = 0, C = 0;
    ...
    return { mode: `Modo ${i + 1} / ${o.frequencies.length}`, frequency: `${f.toFixed(4)} Hz`, period: `${h.toFixed(4)} s`, dominant: g, state: d !== 0 ? "\u25B6 Reproduciendo" : "\u23F8 Pausado" };
  }
  function k() {
    return l.__ctx;
  }
  ...
  function x() {
    var _a, _b;
    if (!o || !o.modeShapes || o.modeShapes.length === 0 || !o.modeShapes[i]) return;
    E(false);
    const f = o.modeShapes[i], h = ((_a = o.frequencies) == null ? void 0 : _a[i]) || 1, S = ((_b = o.frequencies) == null ? void 0 : _b[0]) || 1, u = Math.max(a, Math.min(r, h / S));
    ...
    const N = B > 1e-12 ? j * n / 100 / B : 1, W = performance.now(), V = () => {
      ...
      t.nodes.val = ne, (_a2 = k()) == null ? void 0 : _a2.render(), d = requestAnimationFrame(V);
    };
    d = requestAnimationFrame(V), p();
  }
  return { setResults(f) {
    var _a;
    o = f, i >= (((_a = f == null ? void 0 : f.frequencies) == null ? void 0 : _a.length) ?? 0) && (i = 0), b = t.nodes.rawVal.map((h) => [...h]), p();
  }, setMode(f) {
    var _a;
    if (!o) return;
    const h = ((_a = o.frequencies) == null ? void 0 : _a.length) ?? 0;
    i = Math.max(0, Math.min(h - 1, f)), d !== 0 ? x() : p();
  }, play() {
    o && d === 0 && x();
  }, stop() {
    E(true), p();
    ...
    var _a;
    return ((_a = o == null ? void 0 : o.frequencies) == null ? void 0 : _a[i]) ?? 0;
  }, getStatus() {
    return y();
  }, dispose() {
    E(true), o = null;
...
const Mn = { id: "csi-importer", name: "Importar CSI (F2K/E2K/S2K)", category: "Importar", defaultShellResult: "none", availableShellResults: [], params: {}, build(e, t) {
  var _a, _b;
  const l = window.__hekatanImportedCim, s = [], n = [], a = [];
  if (!l || !((_a = l.zapatas) == null ? void 0 : _a.length)) {
  ...
  let r = 0;
  const o = l.Z ?? 0;
  for (const i of l.zapatas) {
    const d = i.Lz / 2, b = i.Bz / 2;
    s.push([i.xC - d, i.yC - b, o]);
    const v = r++;
    s.push([i.xC + d, i.yC - b, o]);
    const p = r++;
    s.push([i.xC + d, i.yC + b, o]);
    ...
    const k = r++;
    n.push([v, p, y, k]);
    const E = new et(new tt(i.bc, i.bc, 0.5), new nt({ color: 8421504 }));
    E.position.set(i.xCol, i.yCol, o + 0.25), a.push(E);
  }
  if (l.vigasAmarre) {
    const i = [];
    for (const d of l.vigasAmarre) {
      const b = d.z ?? o, v = d.x2 - d.x1, p = d.y2 - d.y1, y = Math.hypot(v, p);
      if (y < 1e-6) continue;
      i.push(new xe(d.x1, d.y1, b)), i.push(new xe(d.x2, d.y2, b));
      const k = new tt(d.b, y, d.h), E = new nt({ color: 2282478, transparent: true, opacity: 0.35 }), x = new et(k, E);
      x.position.set((d.x1 + d.x2) / 2, (d.y1 + d.y2) / 2, b), x.rotateZ(Math.atan2(p, v) - Math.PI / 2), a.push(x);
    }
    if (i.length > 0) {
      const d = new Ve().setFromPoints(i), b = new ct({ color: 2282478, linewidth: 3 });
      a.push(new Mt(d, b));
    }
  }
  t.nodes.val = s, t.elements.val = n, t.nodeInputs.val = { supports: /* @__PURE__ */ new Map(), loads: /* @__PURE__ */ new Map() }, t.elementInputs.val = { elasticities: /* @__PURE__ */ new Map(), shearModuli: /* @__PURE__ */ new Map(), areas: /* @__PURE__ */ new Map(), momentsOfInertiaZ: /* @__PURE__ */ new Map(), momentsOfInertiaY: /* @__PURE__ */ new Map(), torsionalConstants: /* @__PURE__ */ new Map(), densities: /* @__PURE__ */ new Map(), poissonsRatios: /* @__PURE__ */ new Map() }, t.objects3D.val = a, console.log(`[CSI Importer] Renderizadas ${l.zapatas.length} zapatas + ${((_b = l.vigasAmarre) == null ? void 0 : _b.length) ?? 0} vigas.`);
} };
export {
  In as a,
  $n as b,
