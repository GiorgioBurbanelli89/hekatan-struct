import { a as Kt, v as de, V as E, d as Ht, B as Zt } from "./theme-Co6w-pfC.js";
import { a as _e } from "./analyze-Baqb28rE.js";
import { m as ue, d as xe, __tla as __tla_0 } from "./didacticCpp-PqvqKlgs.js";
...
let __tla = Promise.all([
  ...
  const Me = 9.81, F = 25e6, U = 0.2, he = F / (2 * (1 + U)), Wt = 24 / Me, p = 9.80665, jt = [
    {
      name: "Custom",
      q_adm: 20,
      ks_factor: 10.5,
  ...
  ze = {
    ...
    computedLabels(t, e) {
      var _a, _b;
      const i = (t.q_adm ?? 20) * p, s = t.ks ?? i * (t.ks_factor ?? 10.5), l = t.tz ?? 0.15, n = t.Lz ?? 2.5, f = F * l ** 3 / (12 * (1 - U ** 2)), m = f / (s * n ** 4), C = (t.useSimple ?? 0) >= 0.5, mt = (t.useD ?? 0) >= 0.5, y = (t.useL ?? 0) >= 0.5, ft = (t.useS ?? 0) >= 0.5, R = (t.useFactors ?? 1) >= 0.5, kt = R ? t.fD ?? 1.2 : 1, bt = R ? t.fL ?? 1.6 : 1, B = R ? t.fS ?? 0 : 1, Y = mt ? 1 : 0, K = y ? 1 : 0, J = ft ? 1 : 0;
      let I = 0;
    ...
    build(t, e) {
      var _a;
      const { Lz: i, Bz: s, tz: l, bc: n, Hp: f } = t, m = t.q_adm, C = t.ks_factor, mt = m * p, y = t.ks ?? mt * C, ft = (t.useSimple ?? 0) >= 0.5, R = (t.useD ?? 0) >= 0.5, kt = (t.useL ?? 0) >= 0.5, bt = (t.useS ?? 0) >= 0.5, B = (t.useFactors ?? 1) >= 0.5, Y = B ? t.fD ?? 1.2 : 1, K = B ? t.fL ?? 1.6 : 1, J = B ? t.fS ?? 0 : 1, I = R ? 1 : 0, S = kt ? 1 : 0, dt = bt ? 1 : 0, _t = Y, V = K, Q = J;
      let H = 0, Z = 0, tt = 0;
      ...
      try {
        ...
        const ht = G / p, zt = L / t.q_adm, rt = F * l ** 3 / (12 * (1 - U ** 2)) / (y * i ** 4), P = [];
  ...
  k_r\xEDgidez = ${rt.toFixed(2)} (${rt < 1 ? "FLEXIBLE" : "R\xCDGIDA"} \u2014 flexible muestra concentraci\xF3n, r\xEDgida uniforme)`);
      ...
      const yt = Yt * 12, Jt = new Set(b), Dt = (_a = document.querySelector("#viewer")) == null ? void 0 : _a.__settings, $t = (o, a, c = 1) => {
        ...
        for (const rt of b) {
          ...
          if (!P) continue;
          const v = P[0], h = P[1], q = Mt == null ? void 0 : Mt.get(rt), w = (N) => Number.isFinite(N) ? N : 0, Bt = q ? w(q[0]) : 0, Et = q ? w(q[1]) : 0, pt = q ? w(q[2]) : 0, Ot = v + Bt * u, Gt = h + Et * u, Rt = 0 + pt * u, te = Rt - L, It = (N) => [
            v + (Ot - v) * N,
            h + (Gt - h) * N,
            L + te * N
