import { a as Ve } from "./analyze-DoaxThCI.js";
import { m as Ie, d as de, __tla as __tla_0 } from "./didacticCpp-DDG05360.js";
import { b as Ne, a as Xt } from "./cotas3D-ChjswSxQ.js";
import { f as Ae, i as Be, d as To, M as Xo, L as Dt, B as io, V as q, a as fe, g as Te, h as mo, E as Ye, H as De, D as He } from "./theme-Cr2LU0HL.js";
let es, os;
let __tla = Promise.all([
  ...
  function Ge(t, e, B, p, $, f, y, E = {}) {
    ...
    for (let K = 0; K < e.length; K++) {
      ...
      let J = 0, it = 0;
      const m = tt == null ? void 0 : tt.get(K);
      m && (J = m.Mi, it = m.Mj);
      const I = ((_a = p.areas) == null ? void 0 : _a.get(K)) ?? 0.16, S = ((_b = p.momentsOfInertiaZ) == null ? void 0 : _b.get(K)) ?? 213e-5, ht = Xe(I, S, V ? at : ut), $t = J / ht, gt = it / ht;
      ft.push({
        nodeIdx: b,
        elementIdx: K,
        end: "i",
  ...
  function ge(t) {
    ...
    for (let I = 0; I < 50 && tt <= D; I++) {
      const S = $ === "esquinera" ? 0.3 : $ === "lindero" ? 0.2 : 0, pt = tt + S, ht = ft + S, $t = pt * ht, gt = Math.max(G, at), k = gt === G ? pt : ht;
      if (b = gt > k / 6, !b) K = e / $t * (1 + 6 * gt / k), x = e / $t * (1 - 6 * gt / k);
      else {
        const A = 1.5 * k - 3 * gt, Lt = gt === G ? ht : pt;
  ...
  es = {
    ...
    dynamicParams(t) {
      ...
      for (let f = 1; f <= B; f++) e[`hP_${f}`] = h("Alturas por piso", `h Piso ${f} (m)`, 0, 0, 6, 0.1), e[`colB_p${f}`] = h("Secciones por piso", `b col P${f} (m)`, 0, 0, 1, 0.05), e[`colH_p${f}`] = h("Secciones por piso", `h col P${f} (m)`, 0, 0, 1, 0.05), e[`vigaB_p${f}`] = h("Secciones por piso", `b viga P${f} (m)`, 0, 0, 0.8, 0.05), e[`vigaH_p${f}`] = h("Secciones por piso", `h viga P${f} (m)`, 0, 0, 1, 0.05);
      for (let f = 1; f <= p; f++) e[`svX_${f}`] = h("Luces por vano", `svX #${f} (m)`, 0, 0, 12, 0.5);
      for (let f = 1; f <= $; f++) e[`svY_${f}`] = h("Luces por vano", `svY #${f} (m)`, 0, 0, 12, 0.5);
      return e;
    },
    computedLabels(t, e) {
      var _a;
      ...
      p.forEach((J, it) => {
        ...
        Math.abs(I) > Math.abs(f) && (f = I, D = it, m[0], m[1]), I > 0 && I > Math.abs(X) && (X = I, R = it), Math.abs(S) > Math.abs(y) && (y = S), Math.abs(pt) > Math.abs(E) && (E = pt), G.push({
          idx: it,
          x: m[0],
          y: m[1],
          P_kN: Math.abs(I),
      ...
      if (x > 0.01 && (g["\u26A0 Uplift"] = `${x.toFixed(2)} tonf (nodo ${R})`), g.Pisos = `${b}`, g["Copiar a \u2192 zapata-aislada"] = `P=${tt.toFixed(1)}, Mx=${ft.toFixed(1)}, My=${K.toFixed(1)}`, G.length > 0 && at > 0 && ut > 0) {
        ...
        try {
          const m = Yo(G, at, ut, J, it);
          let I = 0, S = 0, pt = 0, ht = 0, $t = -1, gt = "", k = 0, A = 0;
          for (const U of m) U.tipo === "esquinera" ? I++ : U.tipo === "lindero" ? S++ : pt++, U.sigmaMax_tonf > ht && (ht = U.sigmaMax_tonf, $t = U.idx, gt = U.tipo), U.status === "OK" && k++, U.Lz > A && (A = U.Lz);
          g["\u2500\u2500 Cimentaci\xF3n (auto) \u2500\u2500"] = "", g["Tipos zapata"] = `${I} esquineras, ${S} linderas, ${pt} centrales`, g["\u03C3_max global"] = `${ht.toFixed(2)} tonf/m\xB2 (nodo ${$t}, ${gt})`, g["\u03C3/q_adm"] = `${(ht / J).toFixed(2)}` + (ht / J <= 1 ? " \u2713" : " \u26A0"), g["Lz m\xE1x zapata"] = `${A.toFixed(2)} m`, g.Cumplen = `${k}/${m.length}` + (k === m.length ? " \u2713" : " \u26A0");
    ...
    build(t, e) {
      ...
      const V = (o) => t.Lvix > 0 && o === 0 || t.Lvdx > 0 && o === x.length - 1, J = (o) => t.Lviy > 0 && o === 0 || t.Lvdy > 0 && o === b.length - 1, it = (o, n) => V(o) || J(n), m = [], I = {};
      for (let o = 0; o < g.length; o++) for (let n = 0; n < b.length; n++) for (let a = 0; a < x.length; a++) o === 0 && it(a, n) || (I[`${a},${n},${o}`] = m.length, m.push([
        x[a],
        b[n],
        g[o]
      ]));
      const S = [], pt = /* @__PURE__ */ new Set(), ht = /* @__PURE__ */ new Set(), $t = /* @__PURE__ */ new Set(), gt = /* @__PURE__ */ new Map(), k = (o, n, a, d, c) => {
        if (a <= 1) {
      ...
      for (let o = 1; o < g.length; o++) for (let n = 0; n < b.length; n++) for (let a = 0; a < x.length - 1; a++) k(I[`${a},${n},${o}`], I[`${a + 1},${n},${o}`], f, ht, o - 1);
      for (let o = 1; o < g.length; o++) for (let n = 0; n < x.length; n++) for (let a = 0; a < b.length - 1; a++) k(I[`${n},${a},${o}`], I[`${n},${a + 1},${o}`], f, ht, o - 1);
      if (t.vSecOn >= 0.5 && t.nVSec >= 1) {
        const o = Math.round(t.nVSec), n = (d, c, C) => {
          for (let l = 0; l < m.length; l++) if (Math.abs(m[l][0] - d) < 1e-6 && Math.abs(m[l][1] - c) < 1e-6 && Math.abs(m[l][2] - C) < 1e-6) return l;
          const r = m.length;
          return m.push([
            d,
            c,
        ...
        for (let d = 1; d < g.length; d++) if (a === "x") for (let c = 0; c < b.length - 1; c++) {
          ...
          for (let l = 1; l <= o; l++) {
            ...
            for (let v = 0; v < x.length - 1; v++) ht.add(S.length), S.push([
              i[v],
              i[v + 1]
            ]);
          }
          ...
          for (let l = 1; l <= o; l++) {
            ...
            for (let v = 0; v < b.length - 1; v++) ht.add(S.length), S.push([
              i[v],
              i[v + 1]
            ]);
          }
      ...
      if (t.slabOn >= 0.5) {
        ...
        for (let d = 0; d < m.length; d++) o.set(n(m[d][0], m[d][1], m[d][2]), d);
        const a = t.slabDisc > 0 ? t.slabDisc : 0.5;
        for (let d = 1; d < g.length; d++) {
          const c = g[d];
          for (let C = 0; C < x.length - 1; C++) for (let r = 0; r < b.length - 1; r++) {
            const l = x[C], Q = x[C + 1], i = b[r], v = b[r + 1], { n: Gt } = he(Math.abs(Q - l), a), { n: Nt } = he(Math.abs(v - i), a), Ht = [];
            for (let Z = 0; Z <= Nt; Z++) {
              const xt = [];
      ...
      const Ho = [
        ...
        const n = Ho[o] ?? t.colSize, a = qo[o] ?? t.colSize;
        return {
          A: n * a,
          Iz: n * a ** 3 / 12,
          Iy: a * n ** 3 / 12,
      ...
      e.nodes.val = m, e.elements.val = S, e.nodeInputs.val = {
        supports: It,
        loads: ro
      }, e.elementInputs.val = {
        elasticities: xo,
      ...
      const se = de(m, S, e.nodeInputs.val, e.elementInputs.val);
      e.deformOutputs.val = se, e.analyzeOutputs.val = Ve(m, S, e.elementInputs.val, se);
      const Zo = Ne(x, b, g);
      try {
        const o = Ge(m, S, e.analyzeOutputs.rawVal, e.elementInputs.rawVal, Math.round(t.matCol), Math.round(t.matViga), pt);
        let n = 1 / 0, a = 1 / 0, d = 1 / 0, c = -1 / 0, C = -1 / 0, r = -1 / 0;
        for (const i of m) i[0] < n && (n = i[0]), i[0] > c && (c = i[0]), i[1] < a && (a = i[1]), i[1] > C && (C = i[1]), i[2] < d && (d = i[2]), i[2] > r && (r = i[2]);
        const l = Math.sqrt((c - n) ** 2 + (C - a) ** 2 + (r - d) ** 2) || 1, Q = Ke(o, m, l, {
          showElastic: false,
          radiusFactor: 0.015
        });
      ...
      if ((t.mostrarZapatas ?? 1) >= 0.5) try {
        ...
        if (o) {
          ...
          if (o.forEach((c, C) => {
            ...
            const c = t.q_adm_zapata ?? 10, C = t.ks_zapata ?? 1030, r = Math.max(0, t.Hf_pedestal ?? 0.5), l = Math.max(0.1, t.t_zapata ?? 0.3), Q = Math.max(2, Math.round(t.nSubZapata ?? 4)), i = Math.max(0, t.voladoExtra ?? 0.3), v = Math.round(t.tipoZapataOverride ?? 0) | 0, Gt = Math.round(t.estiloZapata ?? 1), Nt = Yo(n, a, d, c, C), Ht = [
              "central",
              "lindero",
              "esquinera"
            ];
            for (const F of Nt) v > 0 && (F.tipo = Ht[v - 1]), F.t = l;
            const Z = [], xt = (F) => new Xo({
              color: F,
              transparent: true,
              opacity: 0.55,
              ...
              color: 1118481,
              linewidth: 2
            }), bo = Ho[0] ?? t.colSize, Co = qo[0] ?? t.colSize;
            for (const F of Nt) {
              const H = F.Lz, st = F.Bz, Yt = F.t;
              let At = 0, Bt = 0;
              F.tipo === "esquinera" ? (At = F.x < a / 2 ? -(H / 2 - i) : H / 2 - i, Bt = F.y < d / 2 ? -(st / 2 - i) : st / 2 - i) : F.tipo === "lindero" && (Math.abs(F.x) < 1e-3 || Math.abs(F.x - a) < 1e-3 ? At = F.x < a / 2 ? -(H / 2 - i) : H / 2 - i : (Math.abs(F.y) < 1e-3 || Math.abs(F.y - d) < 1e-3) && (Bt = F.y < d / 2 ? -(st / 2 - i) : st / 2 - i));
              ...
              if (Gt === 0) {
                const Mt = new Te(H, st, Yt), qt = new To(Mt, xt(Jt));
                qt.position.set(j, O, St), Z.push(qt);
                const Rt = new mo(new Ye(Mt), eo);
                Rt.position.copy(qt.position), Z.push(Rt);
              } else {
                const Mt = new De(H, st), qt = new Xo({
                  color: Jt,
                  transparent: true,
                  opacity: 0.45,
                  roughness: 0.6,
                ...
                const Ut = new To(Mt.clone(), qt.clone());
                Ut.position.set(j, O, Et), Z.push(Ut);
                const ae = H / Q, ie = st / Q, Tt = [];
                for (let yt = 0; yt <= Q; yt++) {
                  const _t = -H / 2 + yt * ae;
                  Tt.push(new q(j + _t, O - st / 2, N), new q(j + _t, O + st / 2, N)), Tt.push(new q(j + _t, O - st / 2, Et), new q(j + _t, O + st / 2, Et));
                }
                for (let yt = 0; yt <= Q; yt++) {
                  const _t = -st / 2 + yt * ie;
                  Tt.push(new q(j - H / 2, O + _t, N), new q(j + H / 2, O + _t, N)), Tt.push(new q(j - H / 2, O + _t, Et), new q(j + H / 2, O + _t, Et));
                }
                ...
                for (let yt = 0; yt < 4; yt++) {
                  ...
                  so.push(new q(j + _t, O + s, N), new q(j + w, O + u, N)), so.push(new q(j + _t, O + s, Et), new q(j + w, O + u, Et)), so.push(new q(j + _t, O + s, N), new q(j + _t, O + s, Et));
                }
                const fo = new io().setFromPoints(so);
                Z.push(new mo(fo, eo));
              }
              (t.mostrarLabelsZapatas ?? 1) >= 0.5 && Z.push(Xt(`${F.tipo[0].toUpperCase()} ${H.toFixed(2)}\xD7${st.toFixed(2)}\xD7${Yt.toFixed(2)}m \u03C3/q=${F.ratio.toFixed(2)}`, j, O, Et - 0.2, Kt <= 1 ? "#4ade80" : Kt <= 1.5 ? "#f59e0b" : "#ef4444"));
            }
            if (Math.round(t.sistemaCimentacion ?? 0) === 1) {
              const F = Math.round(t.vigaAmarre_pos ?? 0), H = F === 0 ? -r : -r / 2, st = t.vigaAmarre_b ?? 0.25, Yt = t.vigaAmarre_h ?? 0.4, At = /* @__PURE__ */ new Map(), Bt = /* @__PURE__ */ new Map();
              for (const O of n) {
              ...
              for (const O of At.values()) {
                ...
                for (let N = 0; N < O.length - 1; N++) j.push(new q(O[N].x, O[N].y, H)), j.push(new q(O[N + 1].x, O[N + 1].y, H));
              }
              for (const O of Bt.values()) {
                O.sort((N, St) => N.y - St.y);
                for (let N = 0; N < O.length - 1; N++) j.push(new q(O[N].x, O[N].y, H)), j.push(new q(O[N + 1].x, O[N + 1].y, H));
              }
              j.length > 0 && (Z.push(new mo(new io().setFromPoints(j), new Dt({
                color: 2282478,
                linewidth: 3
              }))), Z.push(Xt(`Vigas amarre ${(st * 100).toFixed(0)}\xD7${(Yt * 100).toFixed(0)} cm @ ${F === 0 ? "zapatas" : "pedestales"}`, a / 2, d / 2, H + 0.2, "#22d3ee")));
            }
            Zo.push(...Z);
          }
        }
      ...
      if ((t.modoCimentacion ?? 0) >= 0.5) try {
        ...
        if (n && n.size > 0) {
          ...
          if (n.forEach((C, r) => {
            ...
            const C = t.q_adm_zapata ?? 10, r = t.ks_zapata ?? 1030, l = Math.max(0, t.Hf_pedestal ?? 0.5), Q = Math.max(0.1, t.t_zapata ?? 0.3), i = Math.max(2, Math.round(t.nSubZapata ?? 4)), v = Math.max(0, t.voladoExtra ?? 0.3), Gt = Math.round(t.tipoZapataOverride ?? 0) | 0, Nt = Yo(a, d, c, C, r), Ht = [
              "central",
              "lindero",
              "esquinera"
            ];
            for (const s of Nt) Gt > 0 && (s.tipo = Ht[Gt - 1]), s.t = Q;
            const Z = Ho[0] ?? t.colSize, xt = qo[0] ?? t.colSize, eo = Z * xt, yo = Z * xt ** 3 / 12, Vo = xt * Z ** 3 / 12, Io = 0.14 * Math.pow(Math.min(Z, xt), 4), bo = t.matCol < 0.5 ? D : X, Co = t.matCol < 0.5 ? at : ut, ne = t.matCol < 0.5 ? R : G, F = [], H = [], st = /* @__PURE__ */ new Map(), Yt = /* @__PURE__ */ new Map(), At = /* @__PURE__ */ new Map(), Bt = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), O = /* @__PURE__ */ new Map(), N = /* @__PURE__ */ new Map(), St = /* @__PURE__ */ new Map(), Et = /* @__PURE__ */ new Map(), Kt = /* @__PURE__ */ new Map(), Jt = /* @__PURE__ */ new Map(), wo = [], Mt = [], qt = (s, w, u) => `${Math.round(s * 1e4)},${Math.round(w * 1e4)},${Math.round(u * 1e4)}`, Rt = /* @__PURE__ */ new Map(), Ut = (s, w, u) => {
              const P = qt(s, w, u), T = Rt.get(P);
            ...
            for (const s of Nt) {
              ...
              const Wt = s.ratio;
              let ho = 4906624;
              if (Wt > 1.5 ? ho = 15680580 : Wt > 1 ? ho = 16096779 : Wt > 0.8 && (ho = 16498468), l > 1e-3) {
              ...
              }
              {
                ...
                for (let vt = 0; vt <= i; vt++) {
                  const z = -w / 2 + vt * W;
                  Ft.push(new q(kt + z, wt - u / 2, -l), new q(kt + z, wt + u / 2, -l));
                }
                for (let vt = 0; vt <= i; vt++) {
                  const z = -u / 2 + vt * lt;
                  Ft.push(new q(kt - w / 2, wt + z, -l), new q(kt + w / 2, wt + z, -l));
                }
                Mt.push(new mo(new io().setFromPoints(Ft), Y));
              }
            ...
            if (Tt === 1) {
              ...
              for (const _ of Zt.values()) {
                ...
                for (let M = 0; M < L.length - 1; M++) {
                  ...
                  Pt.push(new q(rt.x, rt.y, bt)), Pt.push(new q(Vt.x, Vt.y, bt));
                }
              }
              for (const _ of Ot.values()) {
                const L = [
            ...
            for (const s of Nt) {
              ...
              for (let z = 0; z <= i; z++) {
                ...
                for (let dt = 0; dt <= i; dt++) {
                  ...
                  const No = wt + zt, Ao = Zt + Ct, Bo = qt(No, Ao, -l), no = Rt.get(Bo);
              ...
              if (zo === 1 && so === 0) {
                const z = t.vigaAmarre_b ?? 0.25, ot = s.idx, dt = a.filter((mt) => Math.abs(mt.y - s.y) < 1e-3 && mt.idx !== ot).sort((mt, $o) => mt.x - $o.x), zt = a.filter((mt) => Math.abs(mt.x - s.x) < 1e-3 && mt.idx !== ot).sort((mt, $o) => mt.y - $o.y), Ct = dt.find((mt) => mt.x > s.x), No = [
                  ...dt
                ].reverse().find((mt) => mt.x < s.x), Ao = zt.find((mt) => mt.y > s.y), Bo = [
                  ...zt
                ].reverse().find((mt) => mt.y < s.y), no = (mt, $o) => {
                  const ce = $o / 2;
              ...
              for (let z = 0; z <= i; z++) for (let ot = 0; ot <= i; ot++) {
                ...
                Ct < Y && (Y = Ct, ho = z, jo = ot);
              }
              const W = _[ho][jo], lt = /* @__PURE__ */ new Map(), Ft = fo.get(s.idx) ?? 0, vt = zo === 1 ? Math.max(0.4, 1 - lo * Ft / 4) : 1;
              lt.set(W, [
                0,
                0,
                -w.P_kN,
            ...
            e.nodes.val = F, e.elements.val = H, e.nodeInputs.val = {
              supports: Kt,
              loads: Jt
            }, e.elementInputs.val = {
              elasticities: st,
    ...
    runModal(t, e, B) {
      ...
      if (!(!p.length || !$.length || !((_a = f.supports) == null ? void 0 : _a.size) || !((_b = y.densities) == null ? void 0 : _b.size))) try {
        ...
        const J = {
          ...
          thicknesses: ft,
          poissonsRatios: K
        }, it = Math.round(t.nPisos), m = Math.min(60, Math.max(15, 3 * it + 6)), I = Ie(p, E, f, J, m), S = Math.round(t.nVanosX), pt = Math.round(t.nVanosY), ht = Math.round(t.nPisos), $t = b > 0 ? 1 + x / b : 1;
        B.render(I, {
          title: `Edificio ${S}\xD7${pt} vanos \xD7 ${ht} pisos \xB7 ${m} modos`,
          properties: [
        ...
        console.log(`[Edificio Modal] ${m} modos \xB7 f\u2081=${gt.toFixed(4)} Hz \xB7 m_slab=${x.toFixed(0)} m_cols=${b.toFixed(0)} factor=${$t.toFixed(2)}`);
      } catch (E) {
