...
function xs(e, a, u) {
  ...
  return w.renderOrder = -1, w.frustumCulled = false, w.userData.isShellArea = true, w.name = "__hekatan_shell_colormap", D.derive(() => {
    ...
    for (let _ = 0; _ < u.val.length; _++) {
      ...
      const _e = ((v ? ue + N - O : O) - N) / ne;
      Q[_] = Math.max(0, Math.min(1, _e));
    }
...
function gs(e, a, u) {
  ...
  const g = () => {
    ...
    v.addEventListener("mousedown", (O) => {
      ...
      if (!N) return;
      const oe = O.clientX - ue, _e = O.clientY - ne, me = Math.max(0, Math.min(window.innerWidth - 40, Q + oe)), B = Math.max(0, Math.min(window.innerHeight - 40, _ + _e));
...
function jn(e, a) {
  ...
  const b = new It(u.grid), z = new It(u.grid).multiplyScalar(0.45), v = (Q, _, O, oe) => {
    const _e = [], me = Q === "xy" ? (F, $) => [F, $, 0] : Q === "xz" ? (F, $) => [F, 0, $] : (F, $) => [0, F, $], B = Math.floor(g / _);
    for (let F = -B; F <= B; F++) {
    ...
    const oe = Q === "xy" ? (V, F) => [V, F, 0] : Q === "xz" ? (V, F) => [V, 0, F] : (V, F) => [0, V, F], _e = [[-g, -g], [g, -g], [g, g], [-g, g]], me = [];
    for (const [V, F] of _e) me.push(...oe(V, F));
    const B = new ye();
    ...
    const oe = Q === "xy" ? (I, X) => [I, X, 0] : Q === "xz" ? (I, X) => [I, 0, X] : (I, X) => [0, I, X], _e = _ === "u" ? [...oe(-g, 0), ...oe(g, 0)] : [...oe(0, -g), ...oe(0, g)], me = new ye();
...
function bs(e, a, u, m) {
  ...
  return D.derive(() => {
    ...
    (_b = (_a = e.nodeInputs) == null ? void 0 : _a.val.supports) == null ? void 0 : _b.forEach((ne, Q) => {
      const _ = u.val[Q];
      if (!_) return;
      const O = ne ?? [], oe = (O[0] ? 1 : 0) + (O[1] ? 1 : 0) + (O[2] ? 1 : 0), _e = (O[3] ? 1 : 0) + (O[4] ? 1 : 0) + (O[5] ? 1 : 0);
      let me;
      oe >= 3 && _e >= 3 ? me = new Ke(w, g) : oe >= 3 && _e === 0 ? me = new Ke(p, x) : me = new Ke(p, b), me.position.set(_[0], _[1], _[2]);
      const B = ue * N();
...
function _s(e, a, u, m) {
  ...
  return D.derive(() => {
    ...
    let z = b;
    if (b.length > x) {
      const v = b.map((V) => p[V][0]), N = b.map((V) => p[V][1]), ue = Math.min(...v), ne = Math.max(...v), Q = Math.min(...N), _ = Math.max(...N), O = b.map((V) => p[V][2]), oe = Math.max(1e-6, (Math.max(...O) - Math.min(...O)) / 40), _e = (V) => Math.round(V / oe), me = new Set(O.map(_e)), B = Math.max(4, Math.floor(x / Math.max(1, me.size))), I = Math.max(2, Math.round(Math.sqrt(B))), X = /* @__PURE__ */ new Map();
      for (const V of b) {
        const F = ne - ue < 1e-9 ? 0 : (p[V][0] - ue) / (ne - ue), $ = _ - Q < 1e-9 ? 0 : (p[V][1] - Q) / (_ - Q), T = Math.min(I - 1, Math.floor(F * I)), A = Math.min(I - 1, Math.floor($ * I)), H = `${T},${A},${_e(p[V][2])}`, se = Math.hypot(F * I - (T + 0.5), $ * I - (A + 0.5)), j = X.get(H);
...
function Co(e, a) {
  const u = new He(), m = Math.min(0.05 * e, 0.6), c = en(), w = new gt("X", "red", "transparent"), p = new gt(a ? "Z" : "Y", "green", "transparent"), g = new gt(a ? "Y" : "Z", "blue", "transparent"), x = new jt(new M(1, 0, 0), new M(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), b = new jt(new M(0, 1, 0), new M(0, 0, 0), 1, c.axisArrow, 0.2, 0.2), z = new jt(new M(0, 0, 1), new M(0, 0, 0), 1, c.axisArrow, 0.2, 0.2);
...
function Cs(e, a, u, m) {
  const c = new He(), w = new ye(), p = new it({ vertexColors: true }), g = [0, 0, 0], x = [1, 0, 0], b = [0, 1, 0], z = [0, 0, 1];
...
function Vs(e, a, u, m) {
  ...
  function x(I, X, V, F) {
    ...
    function E(pe, Ce, we, Ae) {
      J.push(0, pe, Ce, 0, we, Ce, 0, we, Ae, 0, pe, Ce, 0, we, Ae, 0, pe, Ae);
    }
  ...
  function z(I, X, V) {
    ...
    const j = [];
    function J(ce, pe, Ce, we) {
      j.push(0, ce, pe, 0, Ce, pe, 0, Ce, we, 0, ce, pe, 0, Ce, we, 0, ce, we);
    }
  ...
  const O = new je({ color: 52479, transparent: true, opacity: 0.35, side: vt, depthWrite: false }), oe = new it({ color: 52479 }), _e = new je({ color: 16750848, transparent: true, opacity: 0.4, side: vt, depthWrite: false }), me = new it({ color: 16750848 });
  function B(I, X) {
  ...
  return D.derive(() => {
    ...
    V.forEach((A, H) => {
      ...
      if (E.type === "CFT") {
        const pe = z(E.b, E.h, E.tw ?? E.b * 0.05), Ce = new Ke(pe.concFill, O);
        Ce.position.set(...ie), Ce.rotation.setFromRotationMatrix(Z), c.add(Ce);
        const we = new Ke(pe.steelFillGeom, _e);
        ...
        switch (E.type) {
          case "rect":
            pe = p(E.b, E.h), Ce = O, we = oe;
            break;
          case "circ":
            pe = g(E.d), Ce = O, we = oe;
            break;
          case "I":
            pe = x(E.b, E.h, E.tf, E.tw), Ce = _e, we = me;
            break;
          case "HSS":
            pe = b(E.b, E.h, E.tw ?? E.b * 0.05), Ce = _e, we = me;
            break;
          case "CFT":
            pe = z(E.b, E.h, E.tw ?? E.b * 0.05), Ce = _e, we = me;
            break;
          case "L":
            pe = v(E.b ?? E.h, E.h, E.t ?? E.tw ?? 3e-3), Ce = _e, we = me;
            break;
          case "2L":
            pe = N(E.b ?? E.h, E.h, E.t ?? E.tw ?? 3e-3, E.dis ?? 0.01), Ce = _e, we = me;
            break;
          case "C":
          case "coldC":
            pe = ue(E.b, E.h, E.tf ?? E.t ?? 3e-3, E.tw ?? E.t ?? 3e-3), Ce = _e, we = me;
            break;
          case "2C":
            pe = ne(E.b, E.h, E.tf ?? 5e-3, E.tw ?? 5e-3, E.dis ?? 0.01), Ce = _e, we = me;
            break;
          case "T":
            pe = Q(E.b, E.h, E.tf ?? 0.01, E.tw ?? 6e-3), Ce = _e, we = me;
            break;
          case "pipe":
            pe = _(E.d, E.tw ?? E.d * 0.05), Ce = _e, we = me;
            break;
          default:
            return;
        }
...
function $s(e, a, u) {
  const m = new He();
  m.name = "extrusion";
  const c = new qn({ color: 8369151, transparent: true, opacity: 0.92, side: vt }), w = new qn({ color: 12623968, transparent: true, opacity: 0.85, side: vt }), p = new qn({ color: 11583173, transparent: true, opacity: 0.85, side: vt }), g = new He();
  ...
  return D.derive(() => {
    ...
    try {
      ue.forEach((oe, _e2) => {
        var _a2, _b2, _c2;
        if (oe.length === 2) {
          let me = As(Q.get(_e2)), B = true;
          if (me || (me = Es((_a2 = ne.areas) == null ? void 0 : _a2.get(_e2), (_b2 = ne.momentsOfInertiaY) == null ? void 0 : _b2.get(_e2), (_c2 = ne.momentsOfInertiaZ) == null ? void 0 : _c2.get(_e2)), B = false), !me) return;
...
class Fo extends He {
  constructor(a, u, m, c, w, p, g) {
    ...
    if (this.text = new gt(`${w[0].toFixed(4)}`), this.text2 = new gt(`${(w[1] * -1).toFixed(4)}`), this.normalizedResult = p, this.textPosition = to(a, u), this.text2Position = to(u, a), this.text.position.set(...this.textPosition), this.text2.position.set(...this.text2Position), this.text.rotation.setFromRotationMatrix(c), this.text2.rotation.setFromRotationMatrix(c), this.add(this.text, this.text2), b) {
      ...
      const O = new Vn(z), oe = new Vn(v), _e = new je({ color: p[0] > 0 ? 24435 : 11411474, side: vt }), me = new je({ color: -p[1] > 0 ? 24435 : 11411474, side: vt });
      this.mesh = new Ke(O, _e), this.mesh2 = new Ke(oe, me), this.mesh.position.set(...a), this.mesh2.position.set(...a), this.mesh.rotation.setFromRotationMatrix(c), this.mesh2.rotation.setFromRotationMatrix(c), g && this.mesh.rotateX(Math.PI / 2), g && this.mesh2.rotateX(Math.PI / 2), this.add(this.mesh, this.mesh2);
    } else {
...
function Xs({ drawingObj: e, gridObj: a, scene: u, getActiveCamera: m, controls: c, gridSize: w, derivedDisplayScale: p, rendererElm: g, viewerRender: x }) {
  ...
  const O = () => {
    ...
    if (ne.visible = !!window.__hekatanGridPlaneXZ, Q.visible = !!window.__hekatanGridPlaneYZ, window.__hekatanOrthoRaycast === true && ze.visible) {
  ...
  }, oe = new Cn(new ye(), new Fn()), _e = new Cn(new ye(), new Fn({ color: "gray", sizeAttenuation: false, size: 6 })), me = new Cn(new ye(), new Fn({ color: "orange", size: 0.1 }));
  u.add(me);
  ...
  let ce = [];
  const pe = new He(), Ce = new Ke(new Wt(1, 1), new je({ color: 2282478, transparent: true, opacity: 0.08, side: vt, depthWrite: false })), we = new Xt(new Mo(new Wt(1, 1)), new it({ color: 2282478, transparent: true, opacity: 0.85 })), Ae = new Xt(new ye(), new it({ color: 2282478, transparent: true, opacity: 0.3 })), Ge = (n, o) => {
  ...
  pe.add(Ce, we, Ae), pe.visible = false, pe.frustumCulled = false, u.add(pe);
  const lt = new He();
  ...
  }, te = W(3462041), de = W(16724804), re = W(6333946), ve = new He();
  ve.frustumCulled = false, ve.visible = false, u.add(ve), ve.add(te, de, re);
  const Pe = (n) => {
  ...
  Fe.id = "hk-refplane-badge", Fe.style.cssText = ["position:fixed", "pointer-events:none", "z-index:99997", "padding:3px 10px", "border-radius:4px", "font-family:Consolas,monospace", "font-size:12px", "font-weight:bold", "transform:translate(20px,40px)", "white-space:nowrap", "display:none"].join(";") + ";", document.body.appendChild(Fe), window.__hekatanSetOrthoPlanes = (n) => {
    ...
    if (window.__hekatanShowOrthoPlanes = n, ve.visible = n, n) {
      ...
      Ue(te, s, "xy", l), Ue(de, s, "xz", l), Ue(re, s, "yz", l), Ne(ze, s, "xy", l), Ne($e, s, "xz", l), Ne(Ie, s, "yz", l), ze.material.opacity = 0.05, $e.material.opacity = 0.05, Ie.material.opacity = 0.05;
    } else {
    ...
    Ue(te, s, "xy", n), Ue(de, s, "xz", n), Ue(re, s, "yz", n), Ne(ze, s, "xy", n), Ne($e, s, "xz", n), Ne(Ie, s, "yz", n), x();
  };
  const rt = (n) => {
  ...
  oe.geometry.setAttribute("position", new ft(e.points.rawVal.flat(), 3)), oe.geometry.computeBoundingSphere(), oe.frustumCulled = false, _e.frustumCulled = false, u.add(_e), N.position.set(0, 0, 0), N.rotateX(Math.PI / 2), N.geometry.rotateX(Math.PI / 2), N.updateMatrixWorld(), e.polylines && (e.polylines.val = [...e.polylines.rawVal, []]), window.__hekatanDrawAt = (n, o, i) => {
    ...
    const ae = s.distanceTo(R), le = new M().subVectors(s, R), xe = new M().subVectors(f, R), Le = Math.acos(Math.max(-1, Math.min(1, le.dot(xe) / (ae * ae)))), Me = e.points.rawVal.length, be = [], wt = k.clone();
    ...
    let S;
    if (_ && e.gridTarget) {
      const k = e.gridTarget.rawVal, d = new xn(...k.rotation), y = new M(1, 0, 0).applyEuler(d), P = new M(0, 1, 0).applyEuler(d), U = new M(...k.position), K = new M(t, r, s), q = new M(l, f, h), R = K.clone().sub(U).dot(y), ae = K.clone().sub(U).dot(P), le = q.clone().sub(U).dot(y), xe = q.clone().sub(U).dot(P), Le = (Me, be) => U.clone().addScaledVector(y, Me).addScaledVector(P, be).toArray();
  ...
  window.__hekatanFinalizePolyArea = cn, window.__hekatanSetInclinedPlaneFrom3 = (n, o, i) => {
    ...
    Ce.geometry.dispose(), Ce.geometry = new Wt(k, k), we.geometry.dispose(), we.geometry = new Mo(new Wt(k, k)), Ge(d, 1), pe.position.copy(S), pe.quaternion.copy(f), pe.scale.set(1, 1, 1), pe.visible = true;
    try {
  ...
  window.__hekatanUpdateSelectionPtScale = lo, c.addEventListener("change", () => {
    ...
    if (i.length) {
      ...
      if (l === "delete") {
        ...
        P ? (dt = y, Ze = -1, qe = -1, $n(y)) : d ? (Ze = d.polyIdx, qe = d.segIdx, dt = -1, In(d.polyIdx, d.segIdx)) : (Ze = -1, qe = -1, dt = -1, he.visible = false), ie.visible = false, lt.visible = false, T(), J.style.left = n.clientX + "px", J.style.top = n.clientY + "px", J.style.display = "block";
      ...
      if (h.length > 0 && S[h[h.length - 1]]) {
        ...
        if (ot = null, !y && window.__hekatanAxisSnap !== false) {
          const Ve = g.getBoundingClientRect(), We = n.clientX, st = n.clientY, yt = ((_k = settings.gridSize) == null ? void 0 : _k.rawVal) ?? 10, Pt = new M(d[0], d[1], d[2]), Ut = [["x", new M(1, 0, 0)], ["y", new M(0, 1, 0)], ["z", new M(0, 0, 1)]], $t = (At) => {
        ...
        ve.visible = xe, xe || rt(null), xe && (Ue(te, d, "xy", le), Ue(de, d, "xz", le), Ue(re, d, "yz", le), Ne(ze, d, "xy", le), Ne($e, d, "xz", le), Ne(Ie, d, "yz", le));
  ...
  const Yn = (n, o, i, t, r) => {
    ...
    const y = (Me) => {
      ...
      return be.project(d), { x: k.left + (be.x * 0.5 + 0.5) * k.width, y: k.top + (-be.y * 0.5 + 0.5) * k.height };
    }, P = (Me) => Me.x >= s && Me.x <= l && Me.y >= f && Me.y <= h, U = (Me, be) => !(Me.x < s && be.x < s || Me.x > l && be.x > l || Me.y < f && be.y < f || Me.y > h && be.y > h);
    r || ge.clear();
    let K = 0;
    const q = ((_a = e.points) == null ? void 0 : _a.rawVal) ?? [];
    for (let Me = 0; Me < q.length; Me++) {
      const be = q[Me];
      be && P(y(be)) && (ge.add(`pt:${Me}`), K++);
    }
    const R = (Me, be) => S ? P(Me) || P(be) || U(Me, be) : P(Me) && P(be), ae = ((_b = e.polylines) == null ? void 0 : _b.rawVal) ?? [], le = ((_c = e.areas) == null ? void 0 : _c.rawVal) ?? [];
    for (let Me = 0; Me < ae.length; Me++) {
      ...
      if (le.includes(Me)) {
        ...
        const Ve = q[be[De]], We = q[be[De + 1]];
        !Ve || !We || R(y(Ve), y(We)) && (ge.add(`seg:${Me}:${De}`), K++);
      }
    }
  ...
  const ut = (n, o, i, t) => {
    ...
    if (l) {
      ...
      le.addBinding(G, "qx", { step: 0.1 }), le.addBinding(G, "qy", { step: 0.1 }), le.addBinding(G, "qz", { step: 0.1 }), Qe.addFolder({ title: "\u2696 Additional Mass (kg/m)", expanded: false }).addBinding(G, "massPerM", { label: "m/L", min: 0, step: 1 }), Qe.addButton({ title: "\u2713 Aplicar a segmentos seleccionados" }).on("click", () => {
        ...
        const Me = [G.relMxI, G.relMyI, G.relMzI], be = [G.relMxJ, G.relMyJ, G.relMzJ];
        (Me.some((Ve) => Ve) || be.some((Ve) => Ve)) && ut("segs", i, "releases", { i: Me, j: be }), G.hinges !== "None" && ut("segs", i, "hinges", G.hinges);
  ...
  const Ho = { end: 16724804, mid: 16498468, node: 6333946, cen: 3462041, per: 12616956, nea: 16744118, int: 16746496 }, uo = (n, o, i, t) => {
    ...
    for (const d of l) if (!(d.length < 2)) for (let y = 0; y < d.length - 1; y++) {
      ...
      if (!(!P || !U) && (r.end && (h("end", P[0], P[1], P[2]), h("end", U[0], U[1], U[2])), r.mid && h("mid", (P[0] + U[0]) / 2, (P[1] + U[1]) / 2, (P[2] + U[2]) / 2), r.nea || r.per)) {
        ...
        const le = Math.max(0, Math.min(1, ((n - P[0]) * K + (o - P[1]) * q + (i - P[2]) * R) / ae)), xe = P[0] + le * K, Le = P[1] + le * q, Me = P[2] + le * R;
...
function Gs(e, a, u, m) {
  ...
  return D.derive(() => {
    ...
    for (const { idx: A, vals: H } of ne) {
      ...
      E.normalize();
      const Z = Math.abs(E.y) < 0.99 ? new M(0, 1, 0) : new M(1, 0, 0), ce = new M().crossVectors(E, Z).normalize(), pe = new M().crossVectors(E, ce).normalize(), Ce = eo + 1, we = Ns;
      for (let Ae = 0; Ae < Ce; Ae++) {
        const Ge = Ae / eo, lt = j[0] + E.x * ie * Ge, ee = j[1] + E.y * ie * Ge, C = j[2] + E.z * ie * Ge, Y = H[0] + (H[1] - H[0]) * Ge, L = w.getColor(Y) ?? new It(0, 0, 0);
      ...
      for (let Ae = 0; Ae < eo; Ae++) for (let Ge = 0; Ge < we; Ge++) {
        const lt = (Ge + 1) % we, ee = V + Ae * we + Ge, C = V + Ae * we + lt, Y = V + (Ae + 1) * we + Ge, L = V + (Ae + 1) * we + lt;
...
Mz \u2248 ${ct(dn * Ue)} ${Fe.forceUnit}\xB7m`);
  ...
  function $(ee, C, Y) {
    ...
    if (ee.type === "node") {
      ...
      const re = L[ee.idx], ve = V(re[0]), Pe = V(re[1]);
      if (ve && Pe) {
      ...
      const re = L[ee.idx], ve = [], Pe = [];
      for (const ze of re) {
        const $e = V(ze);
        if (!$e) return;
        ve.push($e.x, $e.y, $e.z);
      }
      re.length === 4 ? Pe.push(0, 1, 2, 0, 2, 3) : re.length === 3 && Pe.push(0, 1, 2), z.setAttribute("position", new ft(ve, 3)), z.setIndex(Pe), z.computeVertexNormals(), N.visible = true;
    } else if (ee.type === "solid" && L) {
      const re = L[ee.idx], ve = [[0, 1], [1, 2], [2, 3], [3, 0], [4, 5], [5, 6], [6, 7], [7, 4], [0, 4], [1, 5], [2, 6], [3, 7]], Pe = [];
  ...
  e.rendererElm.addEventListener("pointerdown", (ee) => {
    ...
    L ? (Ge({ type: L.type, idx: L.idx }, ee.shiftKey), Ae()) : lt();
  ...
  function we(ee, C) {
    ...
    if (ee.type === "node") {
      ...
      if (!W || !te) return;
      const de = W.clone().add(te).multiplyScalar(0.5), re = te.clone().sub(W), ve = re.length(), Pe = e.getActiveCamera().position.distanceTo(de), ze = new Ke(oe, O);
      ze.position.copy(de);
      const $e = new M(0, 1, 0);
      ...
      for (const [ve, Pe] of W) {
        const ze = V(L[ve]), $e = V(L[Pe]);
        ze && $e && te.push(ze.x, ze.y, ze.z, $e.x, $e.y, $e.z);
      }
      const de = new ye();
...
function ma({ mesh: e, settingsObj: a, drawingObj: u, objects3D: m, solids: c }) {
  ...
  const _ = vs(a), O = D.derive(() => Math.pow(10, _.displayScale.val / 10)), oe = sa(e, _), _e = () => {
    const C = [];
  ...
  D.derive(() => {
    ...
    (_a = e == null ? void 0 : e.nodes) == null ? void 0 : _a.val, (_b = e == null ? void 0 : e.elements) == null ? void 0 : _b.val, (_c = e == null ? void 0 : e.nodeInputs) == null ? void 0 : _c.val, (_d = e == null ? void 0 : e.elementInputs) == null ? void 0 : _d.val, (_e2 = e == null ? void 0 : e.deformOutputs) == null ? void 0 : _e2.val, (_f = e == null ? void 0 : e.analyzeOutputs) == null ? void 0 : _f.val, _.displayScale.val, _.nodes.val, _.elements.val, (_g = _.edges) == null ? void 0 : _g.val, _.elemColumns.val, _.elemBeams.val, _.nodesIndexes.val, _.elementsIndexes.val, _.orientations.val, _.sections.val, _.secColumns.val, _.secBeams.val, _.secFloor.val, _.supports.val, _.loads.val, _.deformedShape.val, _.nodeResults.val, _.frameResults.val, _.shellResults.val, (_h = _.solidResults) == null ? void 0 : _h.val, (_i = _.extruded) == null ? void 0 : _i.val, setTimeout(Z);
  });
  ...
  function pe(C, Y) {
    ...
    if (x.left = -re * de, x.right = re * de, x.updateProjectionMatrix(), C && J) {
      if (E ? (E.object = J, E.update()) : (E = new Po(J, z.domElement), E.enableDamping = true, E.dampingFactor = 0.1, E.screenSpacePanning = true, E.zoomSpeed = 0.8, E.panSpeed = 1.2, E.rotateSpeed = 0.9, E.touches = { ONE: kn.ROTATE, TWO: kn.DOLLY_PAN }, E.target.copy(v.target), E.addEventListener("change", Z), E.enabled = false), !ie) {
        const ve = (Pe) => {
          if (!j || !E) return;
          const ze = z.domElement.getBoundingClientRect(), $e = Pe.clientX - ze.left, Ie = ze.width / 2, Ne = $e >= Ie;
          v.enabled = !Ne, E.enabled = Ne;
        };
        z.domElement.addEventListener("pointerdown", ve, true), z.domElement.addEventListener("wheel", ve, { capture: true, passive: true }), ie = true;
      }
  ...
  if (e) {
    ...
    re.id = "frame-legend", w.appendChild(re), D.derive(() => {
      var _a;
      const ve = _.shellResults.val != "none", Pe = (((_a = _.solidResults) == null ? void 0 : _a.val) ?? "none") !== "none", ze = ve || Pe, $e = _.frameResults.val.startsWith("contour:"), Ie = Y.val.some((Ne) => Number.isFinite(Ne));
      W.hidden = !ze || !Ie, L.visible = ze, re.hidden = !$e;
    });
  }
  if (c) {
  ...
  u && Xs({ drawingObj: u, gridObj: B, scene: p, getActiveCamera: () => b, controls: v, gridSize: V, derivedDisplayScale: O, rendererElm: z.domElement, viewerRender: Z }), $o((C, Y) => {
    ...
    }), B = jn(_.gridSize.rawVal, { planes: _e() }), p.add(B), w.style.setProperty("--awatif-legend-color", Y.legendMarker), Z();
  });
  const Ce = { scene: p, perspCamera: g, orthoCamera: x, get camera() {
    return b;
  ...
  const Ae = (C, Y, L) => {
    ...
    const ve = te * 0.05;
    L.addScaledVector(de, -C * ve), L.addScaledVector(re, Y * ve), b.position.addScaledVector(de, -C * ve), b.position.addScaledVector(re, Y * ve), v.update(), Z();
  }, lt = (C) => {
  ...
  return we.append(ee()), we.append(Ae("\u2191", "Pan arriba", () => Ge(0, 1))), we.append(Ae("\u2295", "Zoom in", () => lt(0.85))), we.append(Ae("\u2190", "Pan izquierda", () => Ge(-1, 0))), we.append(Ae("\u2302", "Reset vista", () => {
    v.reset(), Z();
  })), we.append(Ae("\u2192", "Pan derecha", () => Ge(1, 0))), we.append(Ae("\u2296", "Zoom out", () => lt(1.18))), we.append(Ae("\u2193", "Pan abajo", () => Ge(0, -1))), we.append(ee()), getComputedStyle(w).position === "static" && (w.style.position = "relative"), w.appendChild(we), w;
}
function sa(e, a) {
  return D.derive(() => {
    var _a, _b, _c, _d;
...
function da(e, a) {
  ...
  return ((c) => {
    ...
    ne((_b = (_a = e.analyzeOutputs) == null ? void 0 : _a.val) == null ? void 0 : _b.bendingXX, c), ne((_d = (_c = e.analyzeOutputs) == null ? void 0 : _c.val) == null ? void 0 : _d.bendingYY, w), ne((_f = (_e2 = e.analyzeOutputs) == null ? void 0 : _e2.val) == null ? void 0 : _f.bendingXY, p), ne((_h = (_g = e.analyzeOutputs) == null ? void 0 : _g.val) == null ? void 0 : _h.membraneXX, g), ne((_j = (_i = e.analyzeOutputs) == null ? void 0 : _i.val) == null ? void 0 : _j.membraneYY, x), ne((_l = (_k = e.analyzeOutputs) == null ? void 0 : _k.val) == null ? void 0 : _l.membraneXY, b), ne((_n = (_m = e.analyzeOutputs) == null ? void 0 : _m.val) == null ? void 0 : _n.tranverseShearX, z), ne((_p = (_o2 = e.analyzeOutputs) == null ? void 0 : _o2.val) == null ? void 0 : _p.tranverseShearY, v), ne((_r = (_q = e.analyzeOutputs) == null ? void 0 : _q.val) == null ? void 0 : _r.vonMises, N), ne((_t = (_s2 = e.analyzeOutputs) == null ? void 0 : _s2.val) == null ? void 0 : _t.pressure, ue);
    const Q = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map(), O = /* @__PURE__ */ new Map(), oe = /* @__PURE__ */ new Map(), _e = /* @__PURE__ */ new Map(), me = (Y, L, W, te, de) => {
      Y.forEach((re, ve) => {
        var _a2, _b2;
        const Pe = re[0] ?? 0, ze = ((_a2 = L.get(ve)) == null ? void 0 : _a2[0]) ?? 0, $e = ((_b2 = W.get(ve)) == null ? void 0 : _b2[0]) ?? 0, Ie = (Pe + ze) / 2, Ne = Math.hypot((Pe - ze) / 2, $e);
        te.set(ve, [Ie + Ne]), de.set(ve, [Ie - Ne]);
      });
    };
    me(g, x, b, Q, _), me(c, w, p, O, oe), z.forEach((Y, L) => {
      var _a2;
      _e.set(L, [Math.hypot(Y[0] ?? 0, ((_a2 = v.get(L)) == null ? void 0 : _a2[0]) ?? 0)]);
    });
    const B = (_v = (_u = e.analyzeOutputs) == null ? void 0 : _u.val) == null ? void 0 : _v.colorMapRanges, I = (_w = a.solidResults) == null ? void 0 : _w.val, V = I && I !== "none" ? I : a.shellResults.val, F = B == null ? void 0 : B[V], $ = { bendingXX: [c, 0], bendingYY: [w, 0], bendingXY: [p, 0], membraneXX: [g, 0], membraneYY: [x, 0], membraneXY: [b, 0], tranverseShearX: [z, 0], tranverseShearY: [v, 0], membranePrincipalMax: [Q, 0], membranePrincipalMin: [_, 0], bendingPrincipalMax: [O, 0], bendingPrincipalMin: [oe, 0], transverseShearMax: [_e, 0], vonMises: [N, 0], pressure: [ue, 0], displacementX: [(_y = (_x = e.deformOutputs) == null ? void 0 : _x.val) == null ? void 0 : _y.deformations, 0], displacementY: [(_A = (_z = e.deformOutputs) == null ? void 0 : _z.val) == null ? void 0 : _A.deformations, 1], displacementZ: [(_C = (_B = e.deformOutputs) == null ? void 0 : _B.val) == null ? void 0 : _C.deformations, 2] }, T = a.shellResults.val, A = aa.val, H = ia.val, se = T === "displacementX" || T === "displacementY" || T === "displacementZ", j = T === "bendingXX" || T === "bendingYY" || T === "bendingXY" || T === "bendingPrincipalMax" || T === "bendingPrincipalMin", J = T === "membraneXX" || T === "membraneYY" || T === "membraneXY" || T === "membranePrincipalMax" || T === "membranePrincipalMin", E = T === "vonMises" || T === "pressure", ie = T === "tranverseShearX" || T === "tranverseShearY" || T === "transverseShearMax", Z = (_D = a.solidResults) == null ? void 0 : _D.val, ce = Z === "vonMises" || Z === "sigmaXX" || Z === "sigmaYY" || Z === "sigmaZZ" || Z === "tauXY" || Z === "tauYZ" || Z === "tauXZ", pe = Z === "ux" || Z === "uy" || Z === "uz", Ce = la.val, we = ce ? ca[Ce] : pe || se ? To[H] : j || J || E || ie ? 1 / ra[A] : 1, Ae = ce ? Ce : pe || se ? H : j ? `${A}\xB7m/m` : J ? `${A}/m\xB2` : E ? `${A}/m\xB2` : ie ? `${A}/m` : "";
    no.val = Ae, Mn.val = Array.isArray(F) && F.length === 2 ? [F[0] * we, F[1] * we] : null;
    const Ge = Do.val, ee = Z && Z !== "none" ? [N, 0] : $[T], C = [];
    if (e.nodes.val.forEach((Y, L) => {
      ...
      for (const de of e.elements.val) {
        ...
        const re = W(de, 2), ve = !re && W(de, 0), Pe = !re && W(de, 1);
