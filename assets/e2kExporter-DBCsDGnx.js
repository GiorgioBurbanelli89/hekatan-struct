function Nt() {
  ...
  `;
  {
    let T = false, F = 0, v = 0, G = 0, m = 0;
    I.addEventListener("mousedown", (s) => {
      const d = s.target;
      if (!d.closest("#modal-header") || d.closest("button")) return;
      T = true;
      const w = I.getBoundingClientRect();
      F = s.clientX, v = s.clientY, G = w.left, m = w.top, I.style.bottom = "auto", I.style.right = "auto", I.style.left = `${w.left}px`, I.style.top = `${w.top}px`, s.preventDefault();
    }), document.addEventListener("mousemove", (s) => {
      if (!T) return;
      let d = G + (s.clientX - F), w = m + (s.clientY - v);
      d = Math.max(-I.offsetWidth + 80, Math.min(window.innerWidth - 80, d)), w = Math.max(0, Math.min(window.innerHeight - 30, w)), I.style.left = `${d}px`, I.style.top = `${w}px`;
  ...
  function N(T, F) {
    ...
    const v = ["Ux", "Uy", "Uz", "Rx", "Ry", "Rz"], G = [0, 0, 0, 0, 0, 0], m = T.frequencies.length;
    ...
    const u = w > 0 ? `<span style="color:#0f0">\u2713 ASCE 7-22 \xA712.9.1.1 \u2014 90 % alcanzado en X e Y al modo ${w} de ${m}</span>` : s > 0 && d < 0 ? `<span style="color:#fa0">\u26A0 X cumple en modo ${s}, Y todav\xEDa en ${(Z * 100).toFixed(1)} % \u2014 aumentar nModes</span>` : d > 0 && s < 0 ? `<span style="color:#fa0">\u26A0 Y cumple en modo ${d}, X todav\xEDa en ${(H * 100).toFixed(1)} % \u2014 aumentar nModes</span>` : `<span style="color:#f44">\u2717 ASCE 7-22 NO cumplido en ${m} modos \xB7 \u03A3Ux=${(H * 100).toFixed(1)} % \xB7 \u03A3Uy=${(Z * 100).toFixed(1)} % \u2014 aumentar nModes</span>`, p = (() => {
      const e = (l, y) => {
        var _a2;
        if (l < 0) return `<span style="color:#f44">${y}: no encontrado en ${m} modos</span>`;
        const f = ((_a2 = T.massParticipation) == null ? void 0 : _a2[l - 1]) || [0, 0, 0, 0, 0, 0], x = y === "Ux" ? 0 : y === "Uy" ? 1 : 5, P = T.frequencies[l - 1] > 0 ? 1 / T.frequencies[l - 1] : 0;
        return `<span style="color:#0f0">${y}: modo ${l}, T=${P.toFixed(3)} s, MPF=${(f[x] * 100).toFixed(1)} %</span>`;
      };
  ...
  <div style="display:flex; gap:4px; margin-left:12px;">
    ...
    if (o += '<div id="modal-body" style="padding:0 12px 10px 12px;">', o += `<div style="padding:6px 0; font-weight:bold; font-size:13px;">${u}</div>`, o += p, F.properties) for (const e of F.properties) o += `<span style="color:#888">${e}</span>
`;
    F.spectrumHtml && (o += F.spectrumHtml), o += `<table style="border-collapse:collapse; color:#0f0; font-size:11px; margin-top:4px">
<tr style="color:#ff0; border-bottom:1px solid #ff03">
  ...
  <th style="padding:2px 5px; color:#fff">Tipo</th></tr>`;
    ...
    if (T.frequencies.forEach((e, l) => {
      ...
      let P = 0, _ = x[0];
      for (let X = 1; X < 6; X++) x[X] > _ && (_ = x[X], P = X);
      const U = _ < 0.05 ? "\u2014" : `${v[P]} (${(_ * 100).toFixed(0)} %)`, V = P === 0 || P === 1 ? "#0f0" : P === 5 ? "#0ff" : P === 2 ? "#fa0" : "#888", q = l + 1 === s, tt = l + 1 === d, et = l + 1 === w;
...
<div style="margin-top:6px; font-size:10px; color:#888;">
  \u2605 = primer modo donde \u03A3Ux y \u03A3Uy \u2265 90 %  \xB7  Tipos: <span style="color:#0f0">Ux/Uy</span>=lateral \xB7 <span style="color:#0ff">Rz</span>=torsional \xB7 <span style="color:#fa0">Uz</span>=vertical (no relevante para sismo)
</div>`, o += "</div>", I.innerHTML = o, M) {
      const e = I.querySelector("#modal-body"), l = I.querySelector("#modal-minimize");
    ...
    (_c = I.querySelector("#modal-minimize")) == null ? void 0 : _c.addEventListener("click", () => {
      ...
      const e = [];
      e.push(`Modal Analysis \u2014 ${F.title}`), e.push(u.replace(/<[^>]+>/g, ""));
      const l = `Mode  Freq(Hz)  Period(s)  \u03C9(rad/s)  ${v.join("     ")}  \u03A3Ux    \u03A3Uy    \u03A3Rz   Tipo`;
      e.push(l), e.push("-".repeat(l.length));
      const y = [0, 0, 0, 0, 0, 0];
      T.frequencies.forEach((x, P) => {
        var _a2;
        const _ = x > 0 ? 1 / x : 0, U = x * 2 * Math.PI, V = ((_a2 = T.massParticipation) == null ? void 0 : _a2[P]) || [0, 0, 0, 0, 0, 0];
        ...
        e.push(`${String(P + 1).padStart(4)}  ${x.toFixed(4).padStart(9)}  ${_.toFixed(4).padStart(9)}  ${U.toFixed(2).padStart(9)}  ${at}  ${(y[0] * 100).toFixed(1).padStart(5)}%  ${(y[1] * 100).toFixed(1).padStart(5)}%  ${(y[5] * 100).toFixed(1).padStart(5)}%  ${et}`);
      }), navigator.clipboard.writeText(e.join(`
`));
      const f = I.querySelector("#modal-copy");
      f.textContent = "\u2713", setTimeout(() => f.textContent = "\u{1F4CB}", 1500);
    });
  }
  return { div: I, render: N };
}
function Ot(I) {
  var _a;
  const M = I.split(/\r?\n/), C = { force: "TONF", length: "M" }, N = [], T = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), G = [], m = [], s = /* @__PURE__ */ new Map(), d = /* @__PURE__ */ new Map(), w = [], H = [];
  let Z = "", B = "";
  ...
  for (const A of M) {
    ...
    if (B === "FRAME SECTIONS") {
      ...
      if (c) {
        const O = c[1];
        F.has(O) || F.set(O, { material: "", shape: "", D: 0, B: 0, TF: 0, TW: 0 });
        const i = F.get(O), h = g.match(/MATERIAL\s+"([^"]+)"/);
        h && (i.material = h[1]);
    ...
    if (B === "AREA CONNECTIVITIES") {
      ...
      if (c) {
        ...
        m.push({ name: c[1], pts: O, nStories: 0 });
      }
    }
  }
  ...
  const y = [], f = [], x = [], P = [], _ = /* @__PURE__ */ new Map();
  for (const A of G) for (const [g, c] of d) {
    if (!g.startsWith(A.name + "@")) continue;
    ...
    const r = y.length;
    if (y.push([t, n]), f.push(A.name), x.push(A.type), P.push(O), _.set(r, c.section), c.rigidZone > 0 && at.set(r, [c.rigidZone, c.rigidZone]), c.releases.length > 0) {
  ...
  for (let A = 0; A < f.length; A++) Et.set(`${f[A]}@${P[A]}`, A);
  for (const A of w) {
  ...
  return { units: C, stories: N.reverse(), materials: T, frameSections: F, nodes: R, nodeNames: u, nodeNameToIdx: p, elements: y, elementNames: f, elementTypes: x, elementStories: P, elementSections: _, nodeInputs: { supports: ft, loads: ct }, elementInputs: { elasticities: U, shearModuli: V, areas: q, momentsOfInertiaZ: it, momentsOfInertiaY: X, torsionalConstants: lt, shearAreasY: tt, shearAreasZ: et, rigidOffsets: at, momentReleases: j, densities: rt, sectionShapes: Q }, sectionShapes: Q, grids: H, info: { nNodes: R.length, nFrames: y.length, nAreas: m.length, title: Z }, rawSections: Y };
}
function k(I) {
...
function Tt(I) {
  ...
  let N;
  for (; (N = C.exec(I)) !== null; ) M.set(N[1], N[2] !== void 0 ? N[2] : N[3]);
  return M;
}
function Rt(I) {
  const M = I.split(/\r?\n/);
  return M.some((N) => N.trim().startsWith("TABLE:")) ? It(M) : Mt(M);
}
function It(I) {
  ...
  const F = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), G = /* @__PURE__ */ new Map(), m = /* @__PURE__ */ new Map(), s = [], d = [], w = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), B = [];
  let Y = "";
  for (const J of M) {
    const R = J.trim();
    ...
    switch (Y) {
      ...
      case "MATERIAL PROPERTIES 01 - GENERAL": {
        ...
        p && !F.has(p) && F.set(p, { E: 0, nu: 0, G: 0 });
        break;
      }
      case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
        const p = u.get("Material");
        if (p) {
          const o = F.get(p) || { E: 0, nu: 0, G: 0 };
          o.E = k(u.get("E1")), o.G = k(u.get("G12")), o.nu = k(u.get("U12")), o.density = k(u.get("UnitMass")), F.set(p, o);
        }
        break;
      }
      case "MATERIAL PROPERTIES 03A - STEEL DATA": {
        const p = u.get("Material");
        p && F.has(p) && (F.get(p).fy = k(u.get("Fy")));
        break;
      }
  ...
  return mt(N, T, F, v, G, m, s, d, w, H, Z, B);
}
function Mt(I) {
  const M = { force: "KN", length: "m" };
  let C = "UX,UY,UZ,RX,RY,RZ";
  const N = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map(), F = /* @__PURE__ */ new Map(), v = /* @__PURE__ */ new Map(), G = [], m = [], s = /* @__PURE__ */ new Map(), d = [];
  let w = "", H = "";
  for (const Y of I) {
    const J = Y.trim();
    ...
    switch (w) {
      ...
      case "SHELL": {
        ...
        R.get("SEC"), o && m.push({ name: p, joints: o.split(",") });
        break;
      }
      case "SHELL SECTION": {
        const p = R.get("NAME");
        p && F.set(p, { material: R.get("MAT") || "", type: R.get("TYPE") || "Shell", thickness: k(R.get("TH")) });
        break;
      }
      case "FRAME": {
        const p = u[0], o = R.get("J");
  ...
  return mt(M, C, N, T, F, v, G, m, s, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), d);
}
function mt(I, M, C, N, T, F, v, G, m, s, d, w) {
  var _a;
  const H = [], Z = /* @__PURE__ */ new Map(), B = [];
  for (const [f, x] of F) Z.set(f, B.length), H.push(f), B.push(x);
  const Y = [], J = [], R = /* @__PURE__ */ new Map();
  for (const f of v) {
    const x = Z.get(f.j1), P = Z.get(f.j2);
    if (x !== void 0 && P !== void 0) {
      const _ = Y.length;
      Y.push([x, P]), J.push(f.name);
  ...
  const u = Y.length;
  for (const f of G) {
    const x = f.joints.map((P) => Z.get(P)).filter((P) => P !== void 0);
    if (x.length >= 3) {
      const P = Y.length;
  ...
  for (let f = 0; f < Y.length; f++) {
    const x = R.get(f), P = x ? N.get(x) : null, _ = x ? T.get(x) : null;
    if (P || Y[f].length === 2) {
      const U = P || { material: "", A: 0, Iz: 0, Iy: 0, J: 0, D: 0.3, B: 0.3, shape: "Rectangular" }, V = C.get(U.material) || l, q = V.E || l.E, tt = V.nu || 0.3, et = V.G || q / (2 * (1 + tt));
  ...
  for (const [f, x] of m) {
    const P = Z.get(f);
    P !== void 0 && y.supports.set(P, x);
  }
  for (const f of w) {
    const x = Z.get(f.joint);
    if (x !== void 0) {
      const P = y.forces.get(x) || [0, 0, 0, 0, 0, 0];
      P[0] += f.fx, P[1] += f.fy, P[2] += f.fz, P[3] += f.mx, P[4] += f.my, P[5] += f.mz, y.forces.set(x, P);
    }
  }
  return { units: I, dof: M, materials: C, frameSections: N, shellSections: T, nodes: B, nodeNames: H, nodeNameToIdx: Z, elements: Y, elementNames: J, elementSections: R, nodeInputs: y, elementInputs: o, sectionShapes: e, info: { nNodes: B.length, nFrames: u, nShells: p, title: `SAP2000 (${u} frames, ${p} shells)` } };
}
function Lt(I) {
  var _a, _b, _c, _d, _e, _f, _g, _h, _i, _j, _k;
  const { nodes: M, elements: C, nodeInputs: N, elementInputs: T } = I, F = I.units || { force: "KN", length: "m" }, v = I.title || "Awatif Model", G = [], m = (o) => G.push(o), s = () => G.push(" ");
  m(`File ${v}.$2k was saved on m/d/yy at h:mm:ss`), s(), m('TABLE:  "ACTIVE DEGREES OF FREEDOM"'), m("   UX=Yes   UY=Yes   UZ=Yes   RX=Yes   RY=Yes   RZ=Yes"), s();
  const d = [], w = [];
  if (C.forEach((o, e) => {
  ...
  for (const o of d) {
    const e = ((_a = T.areas) == null ? void 0 : _a.get(o)) || 0, l = ((_b = T.momentsOfInertiaZ) == null ? void 0 : _b.get(o)) || 0, y = ((_c = T.momentsOfInertiaY) == null ? void 0 : _c.get(o)) || 0, f = ((_d = T.torsionalConstants) == null ? void 0 : _d.get(o)) || 0, x = ((_e = T.elasticities) == null ? void 0 : _e.get(o)) || 0, P = `MAT_${Math.round(x)}`, _ = `A${e.toPrecision(6)}_Iz${l.toPrecision(6)}`;
    if (!H.has(_)) {
      let V = 0.3, q = 0.3;
      e > 0 && l > 0 && (V = Math.sqrt(12 * l / e), q = e / V), H.set(_, { A: e, Iz: l, Iy: y, J: f, b: q, h: V, matKey: P });
    }
  ...
  if (H.size > 0) {
    ...
    for (const [, e] of H) {
      o++;
      const l = e.A * 5 / 6;
      m(`   SectionName=SEC${o}   Material=${e.matKey}   Shape=Rectangular   t3=${W(e.h)}   t2=${W(e.b)}   Area=${W(e.A)}   TorsConst=${W(e.J)}   I33=${W(e.Iz)}   I22=${W(e.Iy)}   I23=0   AS2=${W(l)}   AS3=${W(l)} _`), m("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
    }
    s();
  }
  const B = !!I.layeredSection && w.length > 0, Y = I.layeredSection, J = /* @__PURE__ */ new Map(), R = /* @__PURE__ */ new Map();
  ...
  if (w.length > 0) {
    ...
    if (s(), m('TABLE:  "AREA SECTION PROPERTIES"'), B) {
      ...
      m(`   Section=${o.name}   Material=${e}   MatAngle=0   AreaType=Shell   Type=Shell-Layered   Thickness=${W(o.totalThickness)}   BendThick=${W(o.totalThickness)}   Color=Magenta`);
    } else {
      let o = 0;
      for (const [, e] of J) o++, m(`   Section=SSEC${o}   Material=${e.matKey}   MatAngle=0   AreaType=Shell   Type=ShellThin   DrillDOF=Yes   Thickness=${W(e.t)}   BendThick=${W(e.t)}   Color=Cyan`);
    }
    if (s(), B) {
      ...
      for (const e of o.layers) {
        ...
        m(`   Section=${o.name}   LayerName=${e.name}   Distance=${W(e.distance)}   Thickness=${W(e.thickness)}   Type=Shell   NumIntPts=${y}   Material=${e.material}   MatAngle=${W(l * 180 / Math.PI)}   MatBehave=Directional   S11Opt=Linear   S22Opt=Linear   S12Opt=Linear`);
      }
      s();
    }
  }
  ...
  for (let o = 0; o < M.length; o++) {
    const e = M[o];
    m(`   Joint=${o + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${W(e[0])}   Y=${W(e[1])}   Z=${W(e[2])}   SpecialJt=No`);
  }
  ...
  const u = I.selfWtMult ?? 1;
  if (m('TABLE:  "LOAD PATTERN DEFINITIONS"'), m(`   LoadPat=DEAD   DesignType=Dead   SelfWtMult=${u}`), s(), m('TABLE:  "LOAD CASE DEFINITIONS"'), m('   Case=DEAD   Type=LinStatic   InitialCond=Zero   DesTypeOpt="Prog Det"   DesignType=Dead   DesActOpt="Prog Det"   DesignAct=Non-Composite   AutoType=None   RunCase=Yes'), s(), m('TABLE:  "CASE - STATIC 1 - LOAD ASSIGNMENTS"'), m('   Case=DEAD   LoadType="Load pattern"   LoadName=DEAD   LoadSF=1'), s(), N.forces && N.forces.size > 0) {
    m('TABLE:  "JOINT LOADS - FORCE"');
    for (const [o, e] of N.forces) e.some((l) => Math.abs(l) > 1e-12) && m(`   Joint=${o + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${W(e[0])}   F2=${W(e[1])}   F3=${W(e[2])}   M1=${W(e[3])}   M2=${W(e[4])}   M3=${W(e[5])}`);
    s();
  }
  ...
  for (const [o] of p) m(`   Material=${o}   Type=Concrete   SymType=Isotropic   TempDepend=No   Color=Green`);
  s(), m('TABLE:  "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES"');
  for (const [o, e] of p) m(`   Material=${o}   UnitWeight=${W(e.rho * 9.81)}   UnitMass=${W(e.rho)}   E1=${W(e.E)}   G12=${W(e.G)}   U12=${W(e.nu)}   A1=9.9E-06`);
  s(), m('TABLE:  "MATERIAL PROPERTIES 03B - CONCRETE DATA"');
  for (const [o] of p) m(`   Material=${o}   Fc=27579   eFc=27579   LtWtConc=No   SSCurveOpt=Mander   SSHysType=Takeda   SFc=0.00222   SCap=0.005   FinalSlope=-0.1   FAngle=0   DAngle=0`);
  return s(), m('TABLE:  "PROGRAM CONTROL"'), m(`   ProgramName=SAP2000   Version=24.1.0   CurrUnits="${F.force}, ${F.length}, C"   SteelCode="AISC 360-16"   ConcCode="ACI 318-19"   AlumCode="AA 2015"   ColdCode=AISI-16   RegenHinge=Yes`), s(), m("END TABLE DATA"), m(""), G.join(`\r
`);
}
function W(I) {
...
function Ct(I) {
  const { nodes: M, elements: C, nodeInputs: N, elementInputs: T, title: F, e2kModel: v } = I, G = v == null ? void 0 : v.rawSections;
  return G && G.size > 0 ? gt(G) : $t(I);
}
function gt(I, M) {
  ...
  for (const T of N) {
    ...
    if (!(!F || F.length === 0)) {
      C.push(`$ ${T}`);
      for (const v of F) C.push(v);
      C.push("");
    }
  }
  for (const [T, F] of I) if (!N.includes(T) && F.length !== 0) {
    C.push(`$ ${T}`);
    for (const v of F) C.push(v);
    C.push("");
  }
...
function $t(I) {
  var _a, _b, _c, _d;
  const { nodes: M, elements: C, nodeInputs: N, elementInputs: T, title: F, units: v } = I, G = (v == null ? void 0 : v.force) || "Tonf", m = (v == null ? void 0 : v.length) || "m", s = [], d = (t) => Math.round(t * 1e4) / 1e4, w = (() => {
  ...
  s.push(`$ File   "Hekatan_export.e2k"  saved ${J} in ETABS 22.6.0`), s.push(""), s.push("$ PROGRAM INFORMATION"), s.push('  PROGRAM  "ETABS"  VERSION "22.6.0"  '), s.push(""), s.push("$ CONTROLS"), s.push(`  UNITS  "${G}"  "${m}"  "C"  `), s.push('  TITLE1  "Hekatan Struct export"  '), F && s.push(`  TITLE2  "${F}"  `), s.push("  PREFERENCE  MERGETOL 0.001"), s.push('  RLLF  METHOD "ASCE7-10"  USEDEFAULTMIN "YES"  '), s.push("");
  ...
  C.forEach((t, n) => {
    ...
    const r = (_a2 = T.sectionShapes) == null ? void 0 : _a2.get(n), a = ((_b2 = T.elasticities) == null ? void 0 : _b2.get(n)) ?? 0, $ = P.get(a) || "Conc_1", E = _.get(a) ?? a >= 1e8, S = ((_c2 = T.areas) == null ? void 0 : _c2.get(n)) ?? 0, b = ((_d2 = T.momentsOfInertiaY) == null ? void 0 : _d2.get(n)) ?? 0;
  ...
  const Q = (t) => {
    ...
    var _a2, _b2, _c2, _d2;
    const n = [], r = (_a2 = I.propertyModifiers) == null ? void 0 : _a2.get(t);
  ...
  const g = I.weightMode ?? "auto", c = /* @__PURE__ */ new Set();
...
function At(I, M) {
  const C = I[M[0]], N = I[M[1]], T = Math.abs(N[2] - C[2]), F = Math.sqrt((N[0] - C[0]) ** 2 + (N[1] - C[1]) ** 2), v = T > F * 0.5;
  return v && F > 0.01 ? "BRACE" : v ? "COLUMN" : "BEAM";
}
export {
  Lt as a,
  Rt as b,
