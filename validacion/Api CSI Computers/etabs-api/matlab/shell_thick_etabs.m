%% ════════════════════════════════════════════════════════════════
%  SHELL THICK cantilever — ETABS 18+ API verification (R2017a)
%
%  Reproduce FE06:
%    0.5×0.5 m, t=0.05, E=200000, nu=0.3
%    Empotramiento en x=0, P=100 lateral en x=W (Fx global)
%    Malla 3×3 plana en XY, sección Slab con eShellType.ShellThick
%
%  Compara contra HekatanLab FE06: u_max = 1.2528e-02
%% ════════════════════════════════════════════════════════════════
...
ModelDirectory   = 'C:\CSi_ETABS_API_ShellThick';
if ~exist(ModelDirectory, 'dir'); mkdir(ModelDirectory); end
ModelPath = strcat(ModelDirectory, filesep, 'shell_thick.edb');
...
ret = PropArea.SetSlab('SHELL_THICK', ETABSv1.eSlabType.Slab, ...
                       ETABSv1.eShellType.ShellThick, 'SHELL_MAT', t);
...
fprintf('\n═══════════════════════════════════════════════════════════\n');
fprintf('  SHELL THICK (eShellType.ShellThick) — ETABS vs HekatanLab vs Membrana\n');
fprintf('═══════════════════════════════════════════════════════════\n');
fprintf('  %.2fx%.2f m, t=%.3f (t/a=%.3f), P=%.0f, malla %dx%d\n', W, H, t, t/W, P, nx, ny);
fprintf('  ─────────────────────────────────────────────────────────\n');
...
ETABSResult_sthk = u_max_etabs;
IndResult_sthk   = delta_mem;
PercentDiff_sthk = (u_max_etabs/delta_mem) - 1;
fprintf('\nETABSResult_sthk = %.6e\n', ETABSResult_sthk);
