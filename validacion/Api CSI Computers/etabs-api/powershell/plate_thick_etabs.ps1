# Plate Thick — ETABS API (PowerShell) — FE03 SS Mindlin t=0.25
...
$ModelPath = Join-Path $ModelDirectory "plate_thick.edb"
...
[void] $SapModel.PropArea.SetSlab("PLATE_THICK", 1, 2, "PLATE_MAT", $t_m)  # eShellType.ShellThick=2
...
Write-Output ("═" * 60)
Write-Output "  PLATE THICK (eShellType.ShellThick) — ETABS vs HekatanLab vs Reissner"
Write-Output ("═" * 60)
...
"`nETABSResult_thick = {0:e6}" -f $w_max
"IndResult_thick   = {0:e6}" -f $w_reiss
"PercentDiff_thick = {0:f4}" -f (($w_max/$w_reiss)-1)
