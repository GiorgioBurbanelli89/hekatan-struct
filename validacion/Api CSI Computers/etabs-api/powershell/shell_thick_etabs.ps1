# Shell Thick Cantilever — ETABS API (PowerShell) — FE06
...
$ModelPath = Join-Path $ModelDirectory "shell_thick.edb"
...
[void] $SapModel.PropArea.SetSlab("SHELL_THICK", 1, 2, "SHELL_MAT", $t)  # eShellType.ShellThick=2
...
Write-Output ("═" * 60)
Write-Output "  SHELL THICK (eShellType.ShellThick) — ETABS vs HekatanLab vs Membrana"
Write-Output ("═" * 60)
...
"`nETABSResult_sthk = {0:e6}" -f $u_max
"IndResult_sthk   = {0:e6}" -f $delta_mem
"PercentDiff_sthk = {0:f4}" -f (($u_max/$delta_mem)-1)
