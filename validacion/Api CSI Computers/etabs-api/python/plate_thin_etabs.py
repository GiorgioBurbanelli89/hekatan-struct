"""
Plate Thin — ETABS API verification (Python 3.8+, comtypes)
FE02: SS Mindlin t=0.05, q=1, malla 4x4. Esperado HekatanLab w_max=1.371347e-2.
"""
import os
import comtypes.client as cc

APIDLLPath     = r"C:\Program Files\Computers and Structures\ETABS 18\ETABSv1.dll"
ModelDirectory = r"C:\CSi_ETABS_API_PlateThin"
os.makedirs(ModelDirectory, exist_ok=True)
ModelPath = os.path.join(ModelDirectory, "plate_thin.edb")
...
SapModel.SetPresentUnits(6)   # N, m, C
...
SapModel.PropArea.SetSlab("PLATE_THIN", 1, 1, "PLATE_MAT", t_m)
...
for jj in range(ny):
    for ii in range(nx):
        ...
        SapModel.AreaObj.AddByCoord(4, X, Y, [0]*4, "", "PLATE_THIN", "", "Global")
...
print("\n" + "═"*60)
print("  PLATE THIN (eShellType.ShellThin) — ETABS vs HekatanLab vs Navier")
print("═"*60)
print(f"  ETABS API       w_max = {w_max:.6e}  ({(w_max/w_heklab-1)*100:+.2f}%)")
print(f"  HekatanLab Web  w_max = {w_heklab:.6e}  ---")
print(f"  Navier teórica  w_max = {w_navier:.6e}  ({(w_navier/w_heklab-1)*100:+.2f}%)")
print(f"\nETABSResult_thin = {w_max:.6e}")
print(f"IndResult_thin   = {w_navier:.6e}")
print(f"PercentDiff_thin = {(w_max/w_navier)-1:+.4f}")
