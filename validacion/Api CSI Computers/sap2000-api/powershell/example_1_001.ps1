# SAP2000 Example 1-001 — Portal Frame, 7 load patterns (PowerShell translation)
...
$ModelPath = Join-Path $ModelDirectory "API_1-001.sdb"
...
# Unidades kip-ft-F = 5
[void] $SapModel.SetPresentUnits(5)
...
for ($i=2; $i -le 7; $i++) {
    [void] $SapModel.LoadPatterns.Add($i.ToString(), 8, 0, $true)
}
...
for ($i=1; $i -le 7; $i++) {
    ...
    $SapResult[$i-1] = if ($i -le 4) { $U3[0] } else { $U1[0] }
}
...
Write-Output ("═" * 60)
Write-Output "  SAP2000 Example 1-001 (PowerShell)"
Write-Output ("═" * 60)
"  {0,-7}{1,-16}{2,-16}{3}" -f "Case", "SapResult", "IndResult", "PercentDiff"
for ($i=0; $i -lt 7; $i++) {
    "  {0,-7}{1,14:f6}  {2,14:f6}  {3,+10:f4}%" -f ($i+1), $SapResult[$i], $IndResult[$i], ($PercentDiff[$i]*100)
}
