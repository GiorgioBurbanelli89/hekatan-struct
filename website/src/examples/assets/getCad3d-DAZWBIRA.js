...
import { d as Ft, _ as ra, p as ss, m as tl, s as ol, __tla as __tla_0 } from "./didacticCpp-C93PWeaP.js";
import { v as Ko, P as cn, g as nl, a as sl, o as al } from "./theme-CzzIlc4y.js";
import { G as pn, b as ll, M as ia, D as ca, B as ro, c as Sn, d as rl, C as il, e as ha, V as Ne, P as zo, R as da, f as pa, g as Jo, h as Uo, i as cl, S as dl, j as pl, F as Oo, a as Xo, L as No, k as fl, l as ul, A as hn, T as as, m as xn, n as vn, o as ml, p as gl } from "./Text-CBH-tcJP.js";
import { g as In, b as kn, a as $o } from "./analyze-ClLKGn9k.js";
import { g as Mo, __tla as __tla_1 } from "./getMesh-B1dmlgUt.js";
...
let __tla = Promise.all([
  ...
  function _o(e, g) {
    const F = fs.find((Y) => Y.id === e), M = Zo.find((Y) => Y.id === g), J = F.toKN, H = M.toM, Z = (Y, ye, A) => A / (Math.pow(J, Y) * Math.pow(H, ye));
    let V, _;
    switch (e) {
      case "kN":
        V = 10, _ = [
    ...
    return {
      id: `${e}-${g}`,
      label: `${F.label}, ${M.label}`,
      force: F.label,
      length: M.label,
      stress: bl(e, g),
      moment: `${F.label}\xB7${M.label}`,
      E: Z(1, -2, qo.E),
      G: Z(1, -2, qo.G),
      A: Z(0, 2, qo.A),
      Iz: Z(0, 4, qo.Iz),
  ...
  function xl(e) {
    const g = e.force, [F, M, J] = e.forceRange;
    return {
      truss: [
        {
          key: "CM",
  ...
  <div style="display:flex; gap:4px; margin-left:12px;">
    ...
    <button id="modal-minimize" style="padding:2px 8px; font-size:10px; cursor:pointer;
      ...
      if (V += '<div id="modal-body" style="padding:0 12px 10px 12px;">', J.properties) for (const _ of J.properties) V += `<span style="color:#888">${_}</span>
`;
...
<tr style="color:#ff0; border-bottom:1px solid #ff03">
  ...
  <th style="padding:2px 6px">\u03C9 (rad/s)</th>`;
      for (const _ of H) V += `<th style="padding:2px 5px">${_}</th>`;
      if (V += `<th style="padding:2px 5px; color:#0ff">\u03A3Ux</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Uy</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Rz</th></tr>`, M.frequencies.forEach((_, Y) => {
        var _a2;
        const ye = _ > 0 ? 1 / _ : 0, A = _ * 2 * Math.PI, W = ((_a2 = M.massParticipation) == null ? void 0 : _a2[Y]) || [
          0,
          0,
          0,
          0,
  ...
  <td style="padding:2px 6px; text-align:right">${A.toFixed(2)}</td>`;
        for (let ve = 0; ve < 6; ve++) {
          ...
          V += `<td style="padding:2px 5px; text-align:right; color:${pe}">${me}%</td>`;
        }
  ...
  <td style="padding:2px 5px; text-align:right; color:#0ff">${(Z[5] * 100).toFixed(1)}%</td></tr>`;
        ...
        const _ = e.querySelector("#modal-body"), Y = e.querySelector("#modal-minimize");
        _ && (_.style.display = "none"), Y && (Y.textContent = "\u25A2", Y.title = "Restaurar");
      }
      (_a = e.querySelector("#modal-minimize")) == null ? void 0 : _a.addEventListener("click", () => {
        g = !g;
        const _ = e.querySelector("#modal-body"), Y = e.querySelector("#modal-minimize");
        g ? (_.style.display = "none", Y.textContent = "\u25A2", Y.title = "Restaurar") : (_.style.display = "block", Y.textContent = "\u25AC", Y.title = "Minimizar");
        ...
        _.push(Y), _.push("-".repeat(Y.length));
        const ye = [
          0,
          0,
          0,
        ...
        M.frequencies.forEach((W, ve) => {
          ...
          for (let B = 0; B < 6; B++) ye[B] += se[B];
          const q = se.map((B) => ((B * 100).toFixed(1) + "%").padStart(6)).join(" ");
...
`));
  ...
  const Re = 64516e-8, O = 416231e-12, ne = 0.0254, Lo = [
    {
      name: "W6x9",
      type: "W",
      A: 2.68 * Re,
  ...
  function yl(e) {
    ...
    if (A += "<h3>5.1 Supports (fixed DOFs)</h3>", J.supports && J.supports.size > 0) {
      ...
      for (const pe of W) A += `<th>${pe}</th>`;
      A += "</tr></thead><tbody>", J.supports.forEach((pe, se) => {
        A += `<tr><td>${se}</td>`, pe.forEach((q, B) => {
          q && ve.push(se * 6 + B), A += `<td class="${q ? "fixed" : ""}">${q ? "Fixed" : "Free"}</td>`;
        }), A += "</tr>";
      }), A += "</tbody></table>";
    }
    if (A += `<div class="rpt-eq-small">Fixed DOFs: [${ve.join(", ")}] \u2192 ${ve.length} constraints<br/>`, A += `Free DOFs: ${Z} \u2212 ${ve.length} = <b>${Z - ve.length}</b></div>`, A += "<h3>5.2 Applied Loads</h3>", J.loads && J.loads.size > 0) {
      A += '<table class="rpt-data"><thead><tr><th>Node</th>';
      const pe = [
        "F<sub>x</sub>",
        "F<sub>y</sub>",
    ...
    if (A += "<h2>6. Solution</h2>", A += "<p>After removing fixed DOFs, the reduced system is:</p>", A += '<div class="rpt-eq rpt-eq-highlight">K<sub>free</sub> \xB7 u<sub>free</sub> = F<sub>free</sub></div>', A += "<p>Solved using LU decomposition with partial pivoting (sparse matrix).</p>", A += "<h3>6.1 Nodal Displacements</h3>", H == null ? void 0 : H.deformations) {
      A += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const pe of W) A += `<th>${pe}</th>`;
      A += "</tr></thead><tbody>", H.deformations.forEach((pe, se) => {
        A += `<tr><td>${se}</td>`, pe.forEach((q) => {
    ...
    if (A += "<h3>6.2 Reactions</h3>", A += '<div class="rpt-eq">R = K<sub>total</sub> \xB7 u (extract at fixed DOFs)</div>', H == null ? void 0 : H.reactions) {
      A += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const pe of W) A += `<th>${pe}</th>`;
      A += "</tr></thead><tbody>", H.reactions.forEach((pe, se) => {
        A += `<tr><td>${se}</td>`, pe.forEach((q) => {
          const B = Math.abs(q) > 1e-10;
    ...
    if (A += "<h2>7. Internal Forces</h2>", A += "<p>For each element, transform global displacements to local and multiply by K<sub>local</sub>:</p>", A += '<div class="rpt-eq">u<sub>local</sub> = T \xB7 u<sub>global</sub></div>', A += '<div class="rpt-eq rpt-eq-highlight">f<sub>local</sub> = K<sub>local</sub> \xB7 u<sub>local</sub></div>', H == null ? void 0 : H.deformations) {
      ...
      for (const se of pe) A += `<th>${se}<sub>i</sub></th>`;
      for (const se of pe) A += `<th>${se}<sub>j</sub></th>`;
      A += "</tr></thead><tbody>";
  ...
  `;
    return ye.innerHTML = me + A, (_h = ye.querySelector("#rpt-close")) == null ? void 0 : _h.addEventListener("click", () => ye.remove()), ye.querySelectorAll("[data-toggle]").forEach((pe) => {
      pe.addEventListener("click", () => {
        const se = pe.dataset.toggle, q = ye.querySelector(`#rpt-${se}`);
        if (q) {
          const B = q.style.display !== "none";
          q.style.display = B ? "none" : "", pe.textContent = pe.textContent.replace(/^[▼▶]/, B ? "\u25B6" : "\u25BC");
        }
      });
    }), ye;
  }
  ...
  function ls(e, g) {
    ...
    for (let J = 0; J < F; J++) {
      M += "<tr>";
      for (let H = 0; H < F; H++) {
        const Z = ((_a = e[J]) == null ? void 0 : _a[H]) ?? 0, V = Math.abs(Z) < 1e-10;
        M += `<td class="${V ? "z" : ""} ${J === H && !V ? "diag" : ""}">${V ? "0" : $l(Z)}</td>`;
      }
      M += "</tr>";
    }
    return M += "</table>", g > F && (M += `<div style="color:#888;font-size:9pt">(showing ${F}\xD7${F} of ${g}\xD7${g})</div>`), M += "</div>", M;
  }
  function $l(e) {
  ...
  function Ml() {
    ...
    for (const _ of Z) {
      ...
      for (let ve = 0; ve <= 80; ve++) {
        ...
        Y += (ve === 0 ? "M" : "L") + `${pe.toFixed(1)},${se.toFixed(1)}`;
      }
      V += `<path d="${Y}" fill="none" stroke="${_.color}" stroke-width="2.5"/>`;
      const ye = 0.75, A = 30 + ye * 540 + 8, W = 180 / 2 - _.fn(ye) * 60 - 6;
      V += `<text x="${A}" y="${W}" fill="${_.color}" font-size="11" font-weight="bold" font-family="sans-serif">${_.name}</text>`;
    }
    return V += "</svg>", V;
  }
  function wl(e, g) {
    const F = g * 6, M = Math.min(F, 30);
    ...
    for (let Z = 0; Z < e.length; Z++) {
      const V = e[Z].map((_) => _ * 6);
      for (const _ of V) for (const Y of V) for (let ye = 0; ye < 6; ye++) for (let A = 0; A < 6; A++) {
        const W = _ + ye, ve = Y + A;
        W < M && ve < M && H[W][ve]++;
      }
    }
    for (let Z = 0; Z < M; Z++) {
      ...
      for (let V = 0; V < M; V++) {
        const _ = H[Z][V], Y = _ === 0 ? "#fff" : _ === 1 ? "#e8f0fe" : _ === 2 ? "#c6dcf5" : "#a0c4e8", ye = _ === 0 ? "" : _.toString();
        J += `<td style="background:${Y};color:#003366">${ye}</td>`;
      }
      J += "</tr>";
    }
    return J += "</table></div>", F > M && (J += `<div style="color:#888;font-size:9pt">(showing ${M}\xD7${M} of ${F}\xD7${F})</div>`), J;
  }
  let rs = false;
  function El(e) {
    if (rs || window.katex) {
  ...
  function Sl(e, g, F, M, J, H) {
    var _a, _b, _c, _d, _e, _f;
    const Z = F[e], V = Z.map((ge) => g[ge]), _ = Z.length === 2, Y = _ ? Bo(wo(V[1], V[0])) : 0, ye = ((_a = M.elasticities) == null ? void 0 : _a.get(e)) ?? 0, A = ((_b = M.areas) == null ? void 0 : _b.get(e)) ?? 0, W = ((_c = M.momentsOfInertiaZ) == null ? void 0 : _c.get(e)) ?? 0, ve = ((_d = M.momentsOfInertiaY) == null ? void 0 : _d.get(e)) ?? 0, me = ((_e = M.shearModuli) == null ? void 0 : _e.get(e)) ?? 0, pe = ((_f = M.torsionalConstants) == null ? void 0 : _f.get(e)) ?? 0;
    let se = null, q = null, B = null;
    try {
      se = In(V, M, e), q = kn(V), B = po(us(q), po(se, q));
    } catch {
    }
    const G = _ ? wo(V[1], V[0]) : [
      0,
      0,
      0
    ], fe = Y > 0 ? G[0] / Y : 0, $e = Y > 0 ? G[1] / Y : 0, Ae = Y > 0 ? G[2] / Y : 0, Xe = Math.sqrt(fe ** 2 + $e ** 2), he = [];
    if ((J == null ? void 0 : J.deformations) && _) for (const ge of Z) {
  ...
  function Il(e, g, F, M, J, H) {
    var _a, _b;
    const Z = Sl(e, g, F, M, J, H), V = document.createElement("div");
    ...
    <div class="er-body" id="er-body-tabla">${kl(Z)}</div>
    <div class="er-body" id="er-body-math" style="display:none">${ua(Z)}</div>
    <div class="er-body" id="er-body-resumen" style="display:none">${Tl(Z)}</div>
  `, V.querySelectorAll(".er-tab").forEach((_) => {
      _.addEventListener("click", () => {
        V.querySelectorAll(".er-tab").forEach((ye) => ye.classList.remove("active")), _.classList.add("active");
        const Y = _.dataset.tab;
        V.querySelectorAll(".er-body").forEach((ye) => ye.style.display = "none"), V.querySelector(`#er-body-${Y}`).style.display = "";
      });
      ...
      const _ = V.classList.toggle("er-fullscreen-mode"), Y = V.querySelector("#er-fullscreen");
      Y && (Y.textContent = _ ? "\u22A1" : "\u26F6");
    }), setTimeout(() => {
      const _ = V.querySelector("#er-sf-canvas");
      _ && is(_);
      const Y = V.querySelector("#er-sf-canvas-math");
      Y && is(Y);
    }, 50), El(() => {
      const _ = V.querySelector("#er-body-math");
      _ && (_.innerHTML = ua(Z)), setTimeout(() => {
        const Y = V.querySelector("#er-sf-canvas-math");
        Y && is(Y);
      }, 50), V.querySelectorAll(".er-deriv-header").forEach((Y) => {
        Y.addEventListener("click", () => {
          const ye = Y.dataset.toggle, A = V.querySelector(`#er-${ye}`);
  ...
  function ua(e) {
    ...
    let g = "";
    const F = (ye) => fa(ye), M = (ye) => fa(ye, true);
    g += '<div class="er-section-title">1. Geometria del elemento</div>', g += "<p>Viga Euler-Bernoulli con 2 nodos y 6 GDL por nodo:</p>", g += `<div class="er-eq">${M("\\text{DOFs} = [u_x,\\, u_y,\\, u_z,\\, \\theta_x,\\, \\theta_y,\\, \\theta_z] \\quad \\Rightarrow \\quad 12 \\text{ GDL totales}")}</div>`, g += '<div class="er-eq-num">', g += `${F("\\text{Nodo } i")} = (${e.elmNodes[0].map((ye) => Je(ye)).join(", ")})<br/>`, g += `${F("\\text{Nodo } j")} = (${e.elmNodes[1].map((ye) => Je(ye)).join(", ")})<br/>`, g += `${M(`L = \\sqrt{(x_j - x_i)^2 + (y_j - y_i)^2 + (z_j - z_i)^2} = \\mathbf{${Je(e.L)}}`)}`, g += "</div>", g += '<div class="er-section-title">2. Funciones de forma</div>', g += "<p>La viga usa <b>interpolacion lineal</b> para axial/torsion y <b>polinomios cubicos de Hermite</b> para flexion.</p>", g += '<div class="er-subsec">2.1 Axial y Torsion (lineal)</div>', g += `<div class="er-eq">${M("N_1(\\xi) = 1 - \\xi \\qquad N_2(\\xi) = \\xi \\qquad \\text{donde } \\xi = \\frac{x}{L} \\in [0,1]")}</div>`, g += "<p>Primera derivada:</p>", g += `<div class="er-eq">${M("\\frac{dN_1}{d\\xi} = -1 \\qquad \\frac{dN_2}{d\\xi} = 1")}</div>`, g += '<div class="er-subsec">2.2 Flexion (Hermite cubicos)</div>', g += `<p>Las funciones de Hermite garantizan continuidad ${F("C^1")} (desplazamiento y pendiente continuos):</p>`, g += `<div class="er-eq">${M("H_1(\\xi) = 1 - 3\\xi^2 + 2\\xi^3 \\qquad \\text{(desplazamiento nodo } i\\text{)}")}</div>`, g += `<div class="er-eq">${M("H_2(\\xi) = L\\,\\xi\\,(1-\\xi)^2 \\qquad \\text{(rotacion nodo } i\\text{)}")}</div>`, g += `<div class="er-eq">${M("H_3(\\xi) = 3\\xi^2 - 2\\xi^3 \\qquad \\text{(desplazamiento nodo } j\\text{)}")}</div>`, g += `<div class="er-eq">${M("H_4(\\xi) = L\\,\\xi^2(\\xi - 1) \\qquad \\text{(rotacion nodo } j\\text{)}")}</div>`, g += `<div class="er-subsec">Derivadas segunda (curvatura ${F("\\kappa = \\frac{d^2v}{dx^2}")}):</div>`, g += `<div class="er-eq">${M("H_1'' = \\frac{-6}{L^2}(1-2\\xi) \\qquad H_2'' = \\frac{-2}{L}(2-3\\xi)")}</div>`, g += `<div class="er-eq">${M("H_3'' = \\frac{6}{L^2}(1-2\\xi) \\qquad H_4'' = \\frac{-2}{L}(1-3\\xi)")}</div>`, g += '<canvas id="er-sf-canvas-math" width="500" height="250" style="width:100%;border:1px solid var(--fem-border);border-radius:4px;margin:8px 0;"></canvas>', g += '<div class="er-section-title">3. Matriz B (strain-displacement)</div>', g += "<p>La matriz B relaciona desplazamientos nodales con deformaciones internas:</p>", g += `<div class="er-eq">${M("\\boldsymbol{\\varepsilon} = \\mathbf{B} \\cdot \\mathbf{u}")}</div>`, g += '<div class="er-subsec">3.1 Deformacion axial</div>', g += `<div class="er-eq">${M("\\varepsilon_{axial} = \\frac{du}{dx} = \\frac{1}{L} \\begin{bmatrix} -1 & 1 \\end{bmatrix} \\begin{Bmatrix} u_i \\\\ u_j \\end{Bmatrix}")}</div>`, g += `<div class="er-subsec">3.2 Curvatura por flexion (plano XY \u2192 ${F("I_z")})</div>`, g += `<div class="er-eq">${M("\\kappa_z = \\frac{d^2 v}{dx^2} = \\mathbf{B}_{bz} \\cdot \\begin{Bmatrix} v_i \\\\ \\theta_{zi} \\\\ v_j \\\\ \\theta_{zj} \\end{Bmatrix}")}</div>`, g += `<div class="er-eq">${M("\\mathbf{B}_{bz}(\\xi) = \\frac{1}{L^2} \\begin{bmatrix} H_1'' & H_2'' & H_3'' & H_4'' \\end{bmatrix}")}</div>`, g += `<div class="er-subsec">3.3 Curvatura (plano XZ \u2192 ${F("I_y")})</div>`, g += `<div class="er-eq">${M("\\kappa_y = \\frac{d^2 w}{dx^2} = \\mathbf{B}_{by} \\cdot \\begin{Bmatrix} w_i \\\\ \\theta_{yi} \\\\ w_j \\\\ \\theta_{yj} \\end{Bmatrix}")}</div>`, g += '<div class="er-subsec">3.4 Torsion</div>', g += `<div class="er-eq">${M("\\phi' = \\frac{d\\theta_x}{dx} = \\frac{1}{L} \\begin{bmatrix} -1 & 1 \\end{bmatrix} \\begin{Bmatrix} \\theta_{xi} \\\\ \\theta_{xj} \\end{Bmatrix}")}</div>`, g += '<div class="er-section-title">4. Relaciones constitutivas D</div>', g += "<p>Cada modo de deformacion tiene su rigidez material:</p>", g += `<div class="er-eq">${M(`\\text{Axial: } \\sigma = E \\cdot \\varepsilon \\;\\Rightarrow\\; D_{ax} = EA = ${Je(e.E)} \\times ${Je(e.A)} = \\mathbf{${Je(e.E * e.A)}}`)}</div>`, g += `<div class="er-eq">${M(`\\text{Flex Z: } M_z = EI_z \\cdot \\kappa \\;\\Rightarrow\\; D_{bz} = EI_z = ${Je(e.E)} \\times ${Je(e.Iz)} = \\mathbf{${Je(e.E * e.Iz)}}`)}</div>`, g += `<div class="er-eq">${M(`\\text{Flex Y: } M_y = EI_y \\cdot \\kappa \\;\\Rightarrow\\; D_{by} = EI_y = ${Je(e.E)} \\times ${Je(e.Iy)} = \\mathbf{${Je(e.E * e.Iy)}}`)}</div>`, g += `<div class="er-eq">${M(`\\text{Torsion: } T = GJ \\cdot \\phi' \\;\\Rightarrow\\; D_t = GJ = ${Je(e.G)} \\times ${Je(e.J)} = \\mathbf{${Je(e.G * e.J)}}`)}</div>`, g += `<div class="er-section-title">5. Integracion \u2192 ${F("\\mathbf{K}_{local}")}</div>`, g += "<p>La matriz de rigidez local se obtiene integrando analiticamente:</p>", g += `<div class="er-eq er-eq-main">${M("\\mathbf{K}_{local} = \\int_0^L \\mathbf{B}^T \\cdot \\mathbf{D} \\cdot \\mathbf{B} \\; dx")}</div>`;
    const J = e.E * e.A / e.L, H = e.E * e.Iz / e.L ** 3, Z = e.E * e.Iy / e.L ** 3, V = e.G * e.J / e.L;
    if (g += '<div class="er-deriv-block">', g += '<div class="er-deriv-header" data-toggle="deriv-axial">\u{1F4D6} K[0,0] = EA/L \u2014 <i>click para ver derivacion completa</i></div>', g += '<div id="er-deriv-axial" class="er-deriv-body" style="display:none">', g += "<p><b>Paso 1:</b> Funcion de forma axial</p>", g += `<div class="er-eq">${M("u(\\xi) = N_1 \\cdot u_i + N_2 \\cdot u_j = (1-\\xi)\\,u_i + \\xi\\,u_j")}</div>`, g += "<p><b>Paso 2:</b> Derivada (deformacion)</p>", g += `<div class="er-eq">${M("\\varepsilon = \\frac{du}{dx} = \\frac{1}{L}\\frac{du}{d\\xi} = \\frac{1}{L}(-u_i + u_j)")}</div>`, g += `<div class="er-eq">${M("\\mathbf{B}_{ax} = \\frac{1}{L}\\begin{bmatrix} -1 & 1 \\end{bmatrix}")}</div>`, g += `<p><b>Paso 3:</b> Integracion ${F("K = \\int_0^L B^T \\cdot EA \\cdot B \\; dx")}</p>`, g += `<div class="er-eq">${M("K_{ax} = \\int_0^L \\frac{1}{L}\\begin{bmatrix}-1\\\\1\\end{bmatrix} \\cdot EA \\cdot \\frac{1}{L}\\begin{bmatrix}-1 & 1\\end{bmatrix} dx")}</div>`, g += `<div class="er-eq">${M("= \\frac{EA}{L^2} \\begin{bmatrix}1 & -1\\\\-1 & 1\\end{bmatrix} \\int_0^L dx = \\frac{EA}{L^2} \\cdot L \\begin{bmatrix}1 & -1\\\\-1 & 1\\end{bmatrix}")}</div>`, g += `<div class="er-eq er-eq-main">${M(`K_{ax} = \\frac{EA}{L}\\begin{bmatrix}1 & -1\\\\-1 & 1\\end{bmatrix} = \\frac{${Je(e.E)}\\times${Je(e.A)}}{${Je(e.L)}}\\begin{bmatrix}1 & -1\\\\-1 & 1\\end{bmatrix}`)}</div>`, g += `<div class="er-eq">${M(`K[0,0] = K[6,6] = \\frac{EA}{L} = \\mathbf{${Je(J)}}`)}</div>`, g += "</div></div>", g += '<div class="er-deriv-block">', g += '<div class="er-deriv-header" data-toggle="deriv-bend">\u{1F4D6} K[1,1] = 12EI<sub>z</sub>/L\xB3 \u2014 <i>click para ver derivacion completa</i></div>', g += '<div id="er-deriv-bend" class="er-deriv-body" style="display:none">', g += `<p><b>Paso 1:</b> Funcion de forma Hermite para ${F("v(\\xi)")}</p>`, g += `<div class="er-eq">${M("v(\\xi) = H_1 v_i + H_2 \\theta_i + H_3 v_j + H_4 \\theta_j")}</div>`, g += "<p><b>Paso 2:</b> Segunda derivada (curvatura)</p>", g += `<div class="er-eq">${M("\\kappa = \\frac{d^2v}{dx^2} = \\frac{1}{L^2}\\frac{d^2v}{d\\xi^2}")}</div>`, g += `<div class="er-eq">${M("H_1'' = -6+12\\xi, \\quad H_2'' = L(-4+6\\xi), \\quad H_3'' = 6-12\\xi, \\quad H_4'' = L(-2+6\\xi)")}</div>`, g += `<div class="er-eq">${M("\\mathbf{B}_b = \\frac{1}{L^2}\\begin{bmatrix} H_1'' & H_2'' & H_3'' & H_4'' \\end{bmatrix}")}</div>`, g += `<p><b>Paso 3:</b> Integracion para K[1,1] (termino ${F("v_i \\cdot v_i")})</p>`, g += `<div class="er-eq">${M("K[1,1] = \\int_0^L \\frac{(H_1'')^2}{L^4} \\cdot EI_z \\; dx = \\frac{EI_z}{L^4} \\int_0^L (-6+12\\xi)^2 \\; dx")}</div>`, g += `<p>Expandimos: ${F("(-6+12\\xi)^2 = 36 - 144\\xi + 144\\xi^2")}</p>`, g += `<div class="er-eq">${M("\\int_0^L (36-144\\xi+144\\xi^2)\\,dx = 36L - 72L + 48L = 12L")}</div>`, g += `<div class="er-eq er-eq-main">${M(`K[1,1] = \\frac{EI_z}{L^4} \\cdot 12L = \\frac{12EI_z}{L^3} = \\frac{12 \\times ${Je(e.E)} \\times ${Je(e.Iz)}}{${Je(e.L)}^3} = \\mathbf{${Je(12 * H)}}`)}</div>`, g += "</div></div>", g += '<div class="er-deriv-block">', g += '<div class="er-deriv-header" data-toggle="deriv-tors">\u{1F4D6} K[3,3] = GJ/L \u2014 <i>click para ver derivacion</i></div>', g += '<div id="er-deriv-tors" class="er-deriv-body" style="display:none">', g += `<p>Mismo proceso que axial pero con ${F("\\theta_x")} y ${F("GJ")}:</p>`, g += `<div class="er-eq">${M(`K_{torsion} = \\frac{GJ}{L}\\begin{bmatrix}1 & -1\\\\-1 & 1\\end{bmatrix} = \\frac{${Je(e.G)}\\times${Je(e.J)}}{${Je(e.L)}} = \\mathbf{${Je(V)}}`)}</div>`, g += "</div></div>", g += '<div class="er-deriv-block">', g += '<div class="er-deriv-header" data-toggle="deriv-coup">\u{1F4D6} K[1,5] = 6EI<sub>z</sub>/L\xB2 \u2014 <i>acoplamiento corte-momento</i></div>', g += '<div id="er-deriv-coup" class="er-deriv-body" style="display:none">', g += `<p>Termino cruzado ${F("v_i \\cdot \\theta_{zi}")} (acoplamiento corte-momento):</p>`, g += `<div class="er-eq">${M("K[1,5] = \\frac{EI_z}{L^4} \\int_0^L H_1'' \\cdot H_2'' \\; dx")}</div>`, g += `<div class="er-eq">${M("= \\frac{EI_z}{L^4} \\int_0^L (-6+12\\xi) \\cdot L(-4+6\\xi) \\; dx")}</div>`, g += `<div class="er-eq">${M("= \\frac{EI_z}{L^3} \\int_0^L (24-36\\xi-48\\xi+72\\xi^2) \\; dx = \\frac{EI_z}{L^3} \\cdot 6L")}</div>`, g += `<div class="er-eq er-eq-main">${M(`K[1,5] = \\frac{6EI_z}{L^2} = \\mathbf{${Je(6 * e.E * e.Iz / e.L ** 2)}}`)}</div>`, g += "</div></div>", g += '<div class="er-subsec">Resumen de coeficientes:</div>', g += `<div class="er-eq">${M(`\\frac{EA}{L} = \\mathbf{${Je(J)}} \\qquad \\frac{12EI_z}{L^3} = \\mathbf{${Je(12 * H)}} \\qquad \\frac{12EI_y}{L^3} = \\mathbf{${Je(12 * Z)}}`)}</div>`, g += `<div class="er-eq">${M(`\\frac{GJ}{L} = \\mathbf{${Je(V)}} \\qquad \\frac{4EI_y}{L} = \\mathbf{${Je(4 * e.E * e.Iy / e.L)}} \\qquad \\frac{4EI_z}{L} = \\mathbf{${Je(4 * e.E * e.Iz / e.L)}}`)}</div>`, g += `<div class="er-eq">${M(`\\frac{6EI_z}{L^2} = \\mathbf{${Je(6 * e.E * e.Iz / e.L ** 2)}} \\qquad \\frac{6EI_y}{L^2} = \\mathbf{${Je(6 * e.E * e.Iy / e.L ** 2)}}`)}</div>`, e.kLocal && (g += `<div class="er-subsec">Resultado: ${F("\\mathbf{K}_{local}")} (12x12)</div>`, g += fn(e.kLocal)), g += '<div class="er-section-title">6. Transformacion de coordenadas</div>', g += "<p>Los cosenos directores del eje del elemento:</p>", g += `<div class="er-eq">${M(`l = \\frac{x_j - x_i}{L} = ${En(e.l)} \\qquad m = \\frac{y_j - y_i}{L} = ${En(e.m)} \\qquad n = \\frac{z_j - z_i}{L} = ${En(e.n)}`)}</div>`, g += `<div class="er-eq">${M(`D = \\sqrt{l^2 + m^2} = ${En(e.D)}`)}</div>`, Math.abs(e.n) > 0.999) {
  ...
  function fn(e) {
    ...
    for (let J = 0; J < F; J++) {
      ...
      for (let H = 0; H < F; H++) {
        const Z = ((_a = e[J]) == null ? void 0 : _a[H]) ?? 0, V = Math.abs(Z) < 1e-10;
        M += `<td class="${V ? "z" : ""} ${J === H && !V ? "diag" : ""}">${V ? "0" : Al(Z)}</td>`;
      }
      M += "</tr>";
    }
    return M += "</table>", g > F && (M += `<div style="color:var(--fem-label);font-size:9px">(${F}\xD7${F} de ${g}\xD7${g})</div>`), M += "</div>", M;
  }
  function Al(e) {
    return Math.abs(e) >= 1e6 || Math.abs(e) < 0.01 && e !== 0 ? e.toExponential(1) : Math.abs(e) >= 100 ? e.toFixed(0) : e.toFixed(2);
  }
  function is(e) {
    ...
    if (!g) return;
    const F = e.width, M = e.height, J = 30, H = F - 2 * J, Z = (M - 3 * J) / 2;
    g.fillStyle = getComputedStyle(document.body).getPropertyValue("--fem-bg").trim() || "#111", g.fillRect(0, 0, F, M);
    const V = (_, Y, ye) => {
      g.strokeStyle = "#333", g.lineWidth = 1, g.strokeRect(J, _, H, Z), g.strokeStyle = "#444", g.beginPath(), g.moveTo(J, _ + Z / 2), g.lineTo(J + H, _ + Z / 2), g.stroke(), g.fillStyle = "#888", g.font = "11px sans-serif", g.fillText(Y, J + 4, _ + 14);
      for (const W of ye) {
        g.strokeStyle = W.color, g.lineWidth = 2.5, g.beginPath();
        for (let ve = 0; ve <= 100; ve++) {
          const me = ve / 100, pe = J + me * H, se = _ + Z / 2 - W.fn(me) * (Z / 2 * 0.85);
          ve === 0 ? g.moveTo(pe, se) : g.lineTo(pe, se);
        }
        g.stroke();
      }
      let A = J + H - 90;
      for (const W of ye) g.fillStyle = W.color, g.font = "bold 10px sans-serif", g.fillText(W.label, A, _ + Z - 6), A += 36;
      g.fillStyle = "#666", g.font = "9px monospace", g.fillText("0", J, _ + Z + 12), g.fillText("1", J + H - 6, _ + Z + 12), g.fillText("\u03BE", J + H / 2, _ + Z + 12);
    };
    V(J, "Axial (lineal)", [
      {
        fn: (_) => 1 - _,
    ...
    ]), V(J + Z + J, "Flexi\xF3n (Hermite c\xFAbicos)", [
      {
  ...
  function Fl() {
    ...
    g(false), Nt.addEventListener("contextmenu", (M) => {
      M.preventDefault(), e = !e, g(e), Nt.innerHTML = "?";
  ...
  function Qo(e) {
    ...
    const M = F.getBoundingClientRect(), J = window.innerWidth, H = window.innerHeight, Z = 320, V = 180;
  ...
  `, document.body.appendChild(uo);
    const _ = J - M.right, Y = M.left, ye = H - M.bottom, A = M.top;
    let W = g.position || "bottom";
    W === "bottom" && ye < V + 20 && (W = "top"), W === "top" && A < V + 20 && (W = "right"), W === "right" && _ < Z + 20 && (W = "left"), W === "left" && Y < Z + 20 && (W = "bottom");
    let ve, me, pe = "";
    switch (W) {
      case "bottom":
        ...
        break;
      case "top":
        ve = M.left + M.width / 2 - Z / 2, me = M.top - V - 14, pe = "position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #0099ff;";
        break;
      case "right":
        ve = M.right + 14, me = M.top + M.height / 2 - V / 2, pe = "position:absolute;left:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-right:8px solid #0099ff;";
        break;
      case "left":
        ve = M.left - Z - 14, me = M.top + M.height / 2 - V / 2, pe = "position:absolute;right:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-left:8px solid #0099ff;";
        break;
    }
    ve = Math.max(10, Math.min(ve, J - Z - 10)), me = Math.max(10, Math.min(me, H - V - 10)), Gt = document.createElement("div"), Gt.style.cssText = `
    position: fixed;
  ...
  function Nl(e) {
    ...
    const g = e.split(/\r?\n/), F = {
      force: "TONF",
      length: "M"
    }, M = [], J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), V = [], _ = [], Y = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), A = [], W = [];
    let ve = "", me = "";
    ...
    for (const Ce of g) {
      ...
      if (me === "POINT ASSIGNS") {
        ...
        xe && Y.set(`${xe[1]}@${xe[2]}`, xe[3].split(/\s+/));
      }
      if (me === "LINE ASSIGNS") {
        ...
        if (xe) {
          ...
          Ve && (Pe.angle = parseFloat(Ve[1])), ye.set(`${xe[1]}@${xe[2]}`, Pe);
        }
      }
      if (me === "GRIDS") {
    ...
    if (M.length > 0) {
      ...
      for (let _e = Ce - 1; _e >= 0; _e--) {
        const Pe = se.get(M[_e + 1].name) + M[_e].height;
        M[_e].elev = Pe, se.set(M[_e].name, Pe);
      }
    }
    const q = [], B = [], G = /* @__PURE__ */ new Map(), fe = (Ce, _e) => `${Ce}@${_e}`, $e = /* @__PURE__ */ new Set(), Ae = /* @__PURE__ */ new Map();
    for (const Ce of V) Ae.set(Ce.name, Ce);
    for (const Ce of V) for (const [_e, xe] of ye) {
      if (!_e.startsWith(Ce.name + "@")) continue;
      const Pe = xe.story, be = M.findIndex((ke) => ke.name === Pe);
      if (!(be < 0)) if (Ce.type === "COLUMN" || Ce.type === "BRACE") {
        $e.add(fe(Ce.pt2, Pe));
        const ke = Math.max(Ce.nStories, 1), Ve = Math.min(be + ke, M.length - 1);
        $e.add(fe(Ce.pt1, M[Ve].name));
      } else $e.add(fe(Ce.pt1, Pe)), $e.add(fe(Ce.pt2, Pe));
    }
    for (const [Ce] of Y) $e.add(Ce);
    for (const Ce of $e) {
      const [_e, xe] = Ce.split("@"), Pe = Z.get(_e), be = se.get(xe);
      Pe === void 0 || be === void 0 || (q.push([
        Pe[0],
        Pe[1],
        be
      ]), B.push(Ce), G.set(Ce, q.length - 1));
    }
    const Xe = [], he = [], je = [], We = [], ge = /* @__PURE__ */ new Map();
    for (const Ce of V) for (const [_e, xe] of ye) {
      if (!_e.startsWith(Ce.name + "@")) continue;
      const Pe = xe.story, be = M.findIndex((Ge) => Ge.name === Pe);
      if (be < 0) continue;
      let ke, Ve;
      if (Ce.type === "COLUMN" || Ce.type === "BRACE") {
        const Ge = Math.max(Ce.nStories, 1), Ze = Math.min(be + Ge, M.length - 1);
        ke = fe(Ce.pt1, M[Ze].name), Ve = fe(Ce.pt2, Pe);
      } else ke = fe(Ce.pt1, Pe), Ve = fe(Ce.pt2, Pe);
      const Be = G.get(ke), ut = G.get(Ve);
    ...
    const Ie = /* @__PURE__ */ new Map(), ze = /* @__PURE__ */ new Map(), Ue = /* @__PURE__ */ new Map(), dt = /* @__PURE__ */ new Map(), Ke = /* @__PURE__ */ new Map(), pt = /* @__PURE__ */ new Map(), mt = /* @__PURE__ */ new Map(), At = /* @__PURE__ */ new Map(), qt = /* @__PURE__ */ new Map(), Dt = /* @__PURE__ */ new Map(), $t = /* @__PURE__ */ new Map();
    for (const [Ce, _e] of ge) {
      const xe = H.get(_e);
      if (!xe) continue;
      const Pe = J.get(xe.material);
      Pe && (Ie.set(Ce, Pe.E), ze.set(Ce, Pe.G));
      ...
      xe.modI2 && (Ge *= xe.modI2), xe.modI3 && (xt *= xe.modI3), Ue.set(Ce, ut), At.set(Ce, xt), qt.set(Ce, Ge), Dt.set(Ce, Ze), gt > 0 && dt.set(Ce, gt), ft > 0 && Ke.set(Ce, ft), $t.set(Ce, {
        type: vt,
        b: ke || void 0,
        h: be || void 0,
    ...
    for (const Ce of A) {
      ...
      const [xe, Pe] = Xe[_e], be = q[xe], ke = q[Pe], Ve = Math.sqrt((ke[0] - be[0]) ** 2 + (ke[1] - be[1]) ** 2 + (ke[2] - be[2]) ** 2);
  ...
  function xa(e) {
    ...
    let M;
    for (; (M = F.exec(e)) !== null; ) g.set(M[1], M[2] !== void 0 ? M[2] : M[3]);
    return g;
  }
  function ql(e) {
    const g = e.split(/\r?\n/);
    return g.some((M) => M.trim().startsWith("TABLE:")) ? _l(g) : Bl(g);
  }
  function _l(e) {
    var _a, _b, _c, _d, _e, _f;
    const g = [];
    ...
    const H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map(), Y = [], ye = [], A = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), ve = /* @__PURE__ */ new Map(), me = [];
    let pe = "";
    for (const se of g) {
      const q = se.trim();
      ...
      switch (pe) {
        ...
        case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
          ...
          if (G) {
            ...
            fe.E = it(B.get("E1")), fe.G = it(B.get("G12")), fe.nu = it(B.get("U12")), fe.density = it(B.get("UnitMass")), H.set(G, fe);
          }
          break;
        }
        case "MATERIAL PROPERTIES 03A - STEEL DATA": {
          const G = B.get("Material");
          G && H.has(G) && (H.get(G).fy = it(B.get("Fy")));
          break;
        }
        ...
        case "JOINT COORDINATES": {
          const G = B.get("Joint");
          if (G) {
            const fe = it(B.get("XorR")), $e = it(B.get("Y")), Ae = it(B.get("Z"));
            _.set(G, [
              fe,
              $e,
              Ae
        ...
        case "CONNECTIVITY - FRAME": {
          const G = B.get("Frame"), fe = B.get("JointI"), $e = B.get("JointJ");
          G && fe && $e && Y.push({
            name: G,
            j1: fe,
            j2: $e
        ...
        case "CONNECTIVITY - AREA": {
          const G = B.get("Area");
          if (G) {
            const fe = parseInt(B.get("NumJoints") || "4"), $e = [];
            for (let Ae = 1; Ae <= fe; Ae++) {
              const Xe = B.get(`Joint${Ae}`);
              Xe && $e.push(Xe);
            }
            $e.length >= 3 && ye.push({
              name: G,
        ...
        case "FRAME SECTION ASSIGNMENTS": {
          const G = B.get("Frame"), fe = B.get("AnalSect");
          G && fe && W.set(G, fe);
          break;
        }
        case "AREA SECTION ASSIGNMENTS": {
          const G = B.get("Area"), fe = B.get("Section");
          G && fe && ve.set(G, fe);
          break;
        }
    ...
    return va(M, J, H, Z, V, _, Y, ye, A, W, ve, me);
  }
  function Bl(e) {
    const g = {
      force: "KN",
    ...
    const M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), V = [], _ = [], Y = /* @__PURE__ */ new Map(), ye = [];
    let A = "", W = "";
    for (const pe of e) {
      const se = pe.trim();
      if (!se || se.startsWith(";")) continue;
      if (!pe.startsWith(" ") && !pe.startsWith("	")) {
        const G = se.toUpperCase();
        if (G === "END") break;
        G.startsWith("SHELL SECTION") ? A = "SHELL SECTION" : G.startsWith("FRAME SECTION") ? A = "FRAME SECTION" : A = G.split(/\s+/)[0];
        continue;
      }
      ...
      switch (A) {
        ...
        case "MATERIAL": {
          ...
          if (G) W = G, M.set(G, {
            ...
            $e && (fe.E = it($e));
            const Ae = q.get("U");
            Ae && (fe.nu = it(Ae)), fe.G = fe.E / (2 * (1 + fe.nu));
            const Xe = q.get("M");
            Xe && (fe.density = it(Xe));
          }
          break;
    ...
    return va(g, F, M, J, H, Z, V, _, Y, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), ye);
  }
  function va(e, g, F, M, J, H, Z, V, _, Y, ye, A) {
    var _a;
    ...
    for (const he of V) {
      ...
      if (je.length >= 3) {
        ...
        const ge = ye.get(he.name);
        ge && q.set(We, ge);
      }
    }
    const G = pe.length - B, fe = {
    ...
    }, $e = /* @__PURE__ */ new Map(), Ae = F.values().next().value || {
      E: 29e3,
      nu: 0.3,
      G: 11153
    };
    for (let he = 0; he < pe.length; he++) {
      const je = q.get(he), We = je ? M.get(je) : null, ge = je ? J.get(je) : null;
      if (We || pe[he].length === 2) {
        const Ie = We || {
          ...
          B: 0.3,
          shape: "Rectangular"
        }, ze = F.get(Ie.material) || Ae, Ue = ze.E || Ae.E, dt = ze.nu || 0.3, Ke = ze.G || Ue / (2 * (1 + dt));
        fe.elasticities.set(he, Ue), fe.shearModuli.set(he, Ke), fe.areas.set(he, Ie.A || Ie.D * Ie.B), fe.momentsOfInertiaZ.set(he, Ie.Iz || Ie.B * Ie.D ** 3 / 12), fe.momentsOfInertiaY.set(he, Ie.Iy || Ie.D * Ie.B ** 3 / 12), fe.torsionalConstants.set(he, Ie.J || 0), fe.densities.set(he, ze.density || 0), ((_a = Ie.shape) == null ? void 0 : _a.includes("Wide Flange")) || Ie.shape === "I" ? $e.set(he, {
          type: "I",
          b: Ie.B,
          h: Ie.D,
          name: je || "I-section"
        ...
        const Ie = F.get(ge.material) || Ae, ze = Ie.E || Ae.E, Ue = Ie.nu || 0.2, dt = Ie.G || ze / (2 * (1 + Ue));
        fe.elasticities.set(he, ze), fe.shearModuli.set(he, dt), fe.thicknesses.set(he, ge.thickness), fe.poissonsRatios.set(he, Ue), fe.densities.set(he, Ie.density || 0);
      }
    }
    const Xe = {
  ...
  function Dl(e) {
    ...
    const { nodes: g, elements: F, nodeInputs: M, elementInputs: J } = e, H = e.units || {
      force: "KN",
      length: "m"
    }, Z = e.title || "Awatif Model", V = [], _ = (q) => V.push(q), Y = () => V.push(" ");
    _(`File ${Z}.$2k was saved on m/d/yy at h:mm:ss`), Y(), _('TABLE:  "ACTIVE DEGREES OF FREEDOM"'), _("   UX=Yes   UY=Yes   UZ=Yes   RX=Yes   RY=Yes   RZ=Yes"), Y();
    const ye = [], A = [];
    if (F.forEach((q, B) => {
      q.length === 2 ? ye.push(B) : A.push(B);
    }), ye.length > 0) {
    ...
    if (A.length > 0) {
      ...
      for (const q of A) {
        const B = F[q], G = B.map((fe, $e) => `Joint${$e + 1}=${fe + 1}`).join("   ");
        _(`   Area=${q + 1}   NumJoints=${B.length}   ${G}`);
      }
      Y();
    }
    _('TABLE:  "COORDINATE SYSTEMS"'), _("   Name=GLOBAL   Type=Cartesian   X=0   Y=0   Z=0   AboutZ=0   AboutY=0   AboutX=0"), Y(), _('TABLE:  "DATABASE FORMAT TYPES"'), _("   UnitsCurr=Yes   OverrideE=No"), Y();
    const W = /* @__PURE__ */ new Map(), ve = /* @__PURE__ */ new Map();
    for (const q of ye) {
      const B = ((_a = J.areas) == null ? void 0 : _a.get(q)) || 0, G = ((_b = J.momentsOfInertiaZ) == null ? void 0 : _b.get(q)) || 0, fe = ((_c = J.momentsOfInertiaY) == null ? void 0 : _c.get(q)) || 0, $e = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(q)) || 0, Ae = ((_e = J.elasticities) == null ? void 0 : _e.get(q)) || 0, Xe = `MAT_${Math.round(Ae)}`, he = `A${B.toPrecision(6)}_Iz${G.toPrecision(6)}`;
      if (!W.has(he)) {
        let We = 0.3, ge = 0.3;
        B > 0 && G > 0 && (We = Math.sqrt(12 * G / B), ge = B / We), W.set(he, {
          A: B,
          Iz: G,
          Iy: fe,
          J: $e,
    ...
    if (W.size > 0) {
      ...
      for (const [, B] of W) {
        q++;
        const G = B.A * 5 / 6;
        _(`   SectionName=SEC${q}   Material=${B.matKey}   Shape=Rectangular   t3=${Rt(B.h)}   t2=${Rt(B.b)}   Area=${Rt(B.A)}   TorsConst=${Rt(B.J)}   I33=${Rt(B.Iz)}   I22=${Rt(B.Iy)}   I23=0   AS2=${Rt(G)}   AS3=${Rt(G)} _`), _("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
      }
      Y();
    }
    const me = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map();
    for (const q of A) {
      const B = ((_f = J.thicknesses) == null ? void 0 : _f.get(q)) || 0.1, G = ((_g = J.elasticities) == null ? void 0 : _g.get(q)) || 0, fe = `MAT_${Math.round(G)}`, $e = `t${B.toPrecision(6)}`;
      me.has($e) || me.set($e, {
        t: B,
        matKey: fe
      });
    ...
    if (A.length > 0) {
      ...
      for (const B of A) {
        const G = pe.get(B) || "SSEC1";
        _(`   Area=${B + 1}   Section=${G}   MatProp=Default`);
      }
      Y(), _('TABLE:  "AREA SECTION PROPERTIES"');
      let q = 0;
      for (const [, B] of me) q++, _(`   Section=SSEC${q}   Material=${B.matKey}   MatAngle=0   AreaType=Shell   Type=ShellThin   DrillDOF=Yes   Thickness=${Rt(B.t)}   BendThick=${Rt(B.t)}   Color=Cyan`);
      Y();
    }
    _('TABLE:  "JOINT COORDINATES"');
    for (let q = 0; q < g.length; q++) {
      const B = g[q];
      _(`   Joint=${q + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${Rt(B[0])}   Y=${Rt(B[1])}   Z=${Rt(B[2])}   SpecialJt=No`);
    }
    if (Y(), M.supports && M.supports.size > 0) {
      _('TABLE:  "JOINT RESTRAINT ASSIGNMENTS"');
      for (const [q, B] of M.supports) {
        if (!B.some((fe) => fe)) continue;
        const G = (fe) => fe ? "Yes" : "No";
        _(`   Joint=${q + 1}   U1=${G(B[0])}   U2=${G(B[1])}   U3=${G(B[2])}   R1=${G(B[3])}   R2=${G(B[4])}   R3=${G(B[5])}`);
      }
      Y();
    }
    if (_('TABLE:  "LOAD PATTERN DEFINITIONS"'), _("   LoadPat=DEAD   DesignType=Dead   SelfWtMult=0"), Y(), _('TABLE:  "LOAD CASE DEFINITIONS"'), _('   Case=DEAD   Type=LinStatic   InitialCond=Zero   DesTypeOpt="Prog Det"   DesignType=Dead   DesActOpt="Prog Det"   DesignAct=Non-Composite   AutoType=None   RunCase=Yes'), Y(), _('TABLE:  "CASE - STATIC 1 - LOAD ASSIGNMENTS"'), _('   Case=DEAD   LoadType="Load pattern"   LoadName=DEAD   LoadSF=1'), Y(), M.forces && M.forces.size > 0) {
      _('TABLE:  "JOINT LOADS - FORCE"');
      for (const [q, B] of M.forces) B.some((G) => Math.abs(G) > 1e-12) && _(`   Joint=${q + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${Rt(B[0])}   F2=${Rt(B[1])}   F3=${Rt(B[2])}   M1=${Rt(B[3])}   M2=${Rt(B[4])}   M3=${Rt(B[5])}`);
      Y();
    }
    const se = /* @__PURE__ */ new Map();
    for (let q = 0; q < F.length; q++) {
      const B = ((_h = J.elasticities) == null ? void 0 : _h.get(q)) || 0, G = ((_i = J.shearModuli) == null ? void 0 : _i.get(q)) || 0, fe = B > 0 && G > 0 ? Math.max(0, Math.min(0.5, B / (2 * G) - 1)) : 0.2, $e = ((_j = J.densities) == null ? void 0 : _j.get(q)) || 0, Ae = `MAT_${Math.round(B)}`;
      se.has(Ae) || se.set(Ae, {
        E: B,
        nu: fe,
        G,
    ...
    Y(), _('TABLE:  "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES"');
    for (const [q, B] of se) _(`   Material=${q}   UnitWeight=${Rt(B.rho * 9.81)}   UnitMass=${Rt(B.rho)}   E1=${Rt(B.E)}   G12=${Rt(B.G)}   U12=${Rt(B.nu)}   A1=9.9E-06`);
    Y(), _('TABLE:  "MATERIAL PROPERTIES 03B - CONCRETE DATA"');
    ...
    return Y(), _('TABLE:  "PROGRAM CONTROL"'), _(`   ProgramName=SAP2000   Version=24.1.0   CurrUnits="${H.force}, ${H.length}, C"   SteelCode="AISC 360-16"   ConcCode="ACI 318-19"   AlumCode="AA 2015"   ColdCode=AISI-16   RegenHinge=Yes`), Y(), _("END TABLE DATA"), _(""), V.join(`\r
`);
  }
  function Rt(e) {
...
`);
  ...
  function Wl(e) {
    var _a, _b, _c, _d;
    const { nodes: g, elements: F, nodeInputs: M, elementInputs: J, title: H, units: Z } = e, V = (Z == null ? void 0 : Z.force) || "TONF", _ = (Z == null ? void 0 : Z.length) || "M", Y = [], ye = (ge) => Math.round(ge * 1e4) / 1e4;
    Y.push("$ File exported from Awatif FEM Studio"), Y.push(""), Y.push("$ PROGRAM INFORMATION"), Y.push('  PROGRAM  "AWATIF"  VERSION "1.0.0"  '), Y.push(""), Y.push("$ CONTROLS"), Y.push(`  UNITS  "${V}"  "${_}"  "C"  `), H && Y.push(`  TITLE2  "${H}"  `), Y.push("");
    const A = /* @__PURE__ */ new Set();
    g.forEach((ge) => A.add(ye(ge[1])));
    const W = [
      ...A
    ].sort((ge, Ie) => ge - Ie), ve = [], me = /* @__PURE__ */ new Map();
    ve.push("Base"), me.set(W[0], "Base");
    for (let ge = 1; ge < W.length; ge++) {
      const Ie = `Level_${ge}`;
      ve.push(Ie), me.set(W[ge], Ie);
    }
    Y.push("$ STORIES - IN SEQUENCE FROM TOP");
    for (let ge = W.length - 1; ge >= 1; ge--) Y.push(`  STORY "${ve[ge]}"  HEIGHT ${ye(W[ge] - W[ge - 1])} MASTERSTORY "Yes"  `);
    W.length > 0 && Y.push(`  STORY "Base"  ELEV ${W[0]} `), Y.push(""), F.some((ge) => ge.length === 4) && (Y.push("$ DIAPHRAGM NAMES"), Y.push('  DIAPHRAGM "D1"    TYPE RIGID'), Y.push("")), Y.push("$ MATERIAL PROPERTIES");
    ...
    for (const ge of se) {
      const Ie = `Mat_${++B}`;
      q.set(ge, Ie), Y.push(`  MATERIAL  "${Ie}"    TYPE "Concrete"    WEIGHTPERVOLUME 2.4`), Y.push(`  MATERIAL  "${Ie}"    SYMTYPE "Isotropic"  E ${ge}  U 0.2  A 1E-05`);
    }
    ...
    F.forEach((ge, Ie) => {
      ...
      const ze = (_a2 = J.sectionShapes) == null ? void 0 : _a2.get(Ie), Ue = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(Ie)) ?? 0, dt = q.get(Ue) || "Mat_1", Ke = (ze == null ? void 0 : ze.h) ?? 0, pt = (ze == null ? void 0 : ze.b) ?? 0, mt = (ze == null ? void 0 : ze.d) ?? 0, At = (ze == null ? void 0 : ze.tf) ?? 0, qt = (ze == null ? void 0 : ze.tw) ?? 0, Dt = (ze == null ? void 0 : ze.type) || "rect", $t = `${Dt}_${Ke}_${pt}_${mt}_${At}_${qt}`;
      (ze == null ? void 0 : ze.name) && !$e.has($t) && $e.set($t, ze.name);
      let ht = $e.get($t);
      if (ht || (Dt === "rect" ? ht = `R${ye(pt * 100)}x${ye(Ke * 100)}` : Dt === "circ" ? ht = `C_D${ye(mt * 100)}` : Dt === "I" ? ht = `I_${ye(Ke * 100)}` : ht = `Sec_${G.size + 1}`, $e.set($t, ht)), fe.set(Ie, ht), G.has(ht)) return;
      G.add(ht);
      const qe = {
      ...
      Ke && (St += `  D ${Ke}`), pt && (St += `  B ${pt}`), mt && !Ke && (St += `  D ${mt}`), At && (St += `  TF ${At}`), qt && (St += `  TW ${qt}`), Y.push(St);
    }), Y.push("");
    const Ae = /* @__PURE__ */ new Map();
    let Xe = 0;
    g.forEach((ge) => {
      const Ie = `${ye(ge[0])},${ye(ge[2])}`;
      Ae.has(Ie) || Ae.set(Ie, `${++Xe}`);
    }), Y.push("$ POINT COORDINATES");
    for (const [ge, Ie] of Ae) {
      const [ze, Ue] = ge.split(",").map(Number);
      Y.push(`  POINT "${Ie}"  ${ze} ${Ue} `);
    }
    Y.push("");
    const he = (ge) => {
      const Ie = g[ge], ze = `${ye(Ie[0])},${ye(Ie[2])}`;
      return {
        pt: Ae.get(ze) || "1",
    ...
    F.forEach((ge, Ie) => {
      if (ge.length !== 2) return;
      const ze = Yl(g, ge), Ue = fe.get(Ie) || `Sec_${Ie}`;
      if (ze === "BEAM") {
        const dt = he(ge[0]), Ke = he(ge[1]);
        Y.push(`  LINE  "E${Ie + 1}"  BEAM  "${dt.pt}"  "${Ke.pt}"  0`), je.push(`  LINEASSIGN  "E${Ie + 1}"  "${dt.story}"  SECTION "${Ue}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
      } else {
        const dt = g[ge[0]][1] <= g[ge[1]][1] ? ge[0] : ge[1], Ke = g[ge[0]][1] <= g[ge[1]][1] ? ge[1] : ge[0];
        he(dt);
        const pt = he(Ke), mt = ye(g[dt][1]), At = ye(g[Ke][1]), qt = W.indexOf(mt), Dt = W.indexOf(At), $t = Math.max(1, Dt >= 0 && qt >= 0 ? Dt - qt : 1);
        Y.push(`  LINE  "E${Ie + 1}"  ${ze}  "${pt.pt}"  "${pt.pt}"  ${$t}`);
        for (let ht = 0; ht < $t; ht++) {
          const mo = Dt - ht;
          mo >= 0 && mo < ve.length && je.push(`  LINEASSIGN  "E${Ie + 1}"  "${ve[mo]}"  SECTION "${Ue}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
        }
      }
    }), Y.push(""), Y.push("$ POINT ASSIGNS"), (_b = M.supports) == null ? void 0 : _b.forEach((ge, Ie) => {
      const ze = [];
      if (ge[0] && ze.push("UX"), ge[1] && ze.push("UY"), ge[2] && ze.push("UZ"), ge[3] && ze.push("RX"), ge[4] && ze.push("RY"), ge[5] && ze.push("RZ"), ze.length > 0) {
        const Ue = he(Ie);
        Y.push(`  POINTASSIGN  "${Ue.pt}"  "${Ue.story}"  RESTRAINT "${ze.join(" ")}"  `);
      }
    }), Y.push(""), Y.push("$ LINE ASSIGNS"), je.forEach((ge) => Y.push(ge)), Y.push("");
    const We = [];
    if (F.forEach((ge, Ie) => {
      if (ge.length === 4) {
        const ze = g[ge[0]], Ue = g[ge[1]], dt = g[ge[2]], Ke = [
          Ue[0] - ze[0],
          Ue[1] - ze[1],
          Ue[2] - ze[2]
        ], pt = [
      ...
      Y.push(`  SHELLPROP  "Losa"  PROPTYPE  "Slab"  MATERIAL "${q.values().next().value || "Mat_1"}"  MODELINGTYPE "ShellThin"  SLABTYPE "Slab"  SLABTHICKNESS ${ge} `), Y.push("");
    }
    if (We.some((ge) => ge.isWall)) {
      Y.push("$ WALL PROPERTIES");
      const ge = ((_d = J.thicknesses) == null ? void 0 : _d.values().next().value) ?? 0.2;
      Y.push(`  SHELLPROP  "Muro"  PROPTYPE  "Wall"  MATERIAL "${q.values().next().value || "Mat_1"}"  MODELINGTYPE "ShellThick"  WALLTHICKNESS ${ge} `), Y.push("");
    }
    if (We.length > 0) {
      Y.push("$ AREA CONNECTIVITIES");
      const ge = [];
      We.forEach((Ie, ze) => {
        const { el: Ue, isWall: dt } = Ie, Ke = dt ? `W${ze + 1}` : `F${ze + 1}`, pt = dt ? "PANEL" : "FLOOR", mt = Ue.map((At) => he(At));
        if (dt) {
          const At = g[Ue[0]][1] <= g[Ue[2]][1] ? 0 : 2, qt = g[Ue[1]][1] <= g[Ue[3]][1] ? 1 : 3;
          Y.push(`  AREA "${Ke}"  ${pt}  4  "${mt[At].pt}"  "${mt[qt].pt}"  "${mt[qt].pt}"  "${mt[At].pt}"  1  1  0  0  `);
        ...
        } else Y.push(`  AREA "${Ke}"  ${pt}  4  "${mt[0].pt}"  "${mt[1].pt}"  "${mt[2].pt}"  "${mt[3].pt}"  0  0  0  0  `), ge.push(`  AREAASSIGN  "${Ke}"  "${mt[0].story}"  SECTION "Losa"  DIAPH  "D1"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "TOP"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
      }), Y.push(""), Y.push("$ AREA ASSIGNS"), ge.forEach((Ie) => Y.push(Ie)), Y.push("");
    }
    return Y.push("$ LOAD PATTERNS"), Y.push('  LOADPATTERN "Dead"  TYPE  "Dead"  SELFWEIGHT  1'), Y.push('  LOADPATTERN "Live"  TYPE  "Live"  SELFWEIGHT  0'), Y.push(""), M.loads && M.loads.size > 0 && (Y.push("$ POINT OBJECT LOADS"), M.loads.forEach((ge, Ie) => {
      const [ze, Ue, dt] = ge, Ke = he(Ie);
      Math.abs(ze) > 1e-10 && Y.push(`  POINTLOAD  "${Ke.pt}"  "${Ke.story}"  "Dead"  TYPE "FORCE"  FX ${ze}`), Math.abs(Ue) > 1e-10 && Y.push(`  POINTLOAD  "${Ke.pt}"  "${Ke.story}"  "Dead"  TYPE "FORCE"  FY ${Ue}`), Math.abs(dt) > 1e-10 && Y.push(`  POINTLOAD  "${Ke.pt}"  "${Ke.story}"  "Dead"  TYPE "FORCE"  FZ ${dt}`);
    }), Y.push("")), Y.push("  END"), Y.push("$ END OF MODEL FILE"), Y.join(`\r
`);
  }
  function Yl(e, g) {
    const F = e[g[0]], M = e[g[1]], J = Math.abs(M[1] - F[1]), H = Math.sqrt((M[0] - F[0]) ** 2 + (M[2] - F[2]) ** 2), Z = J > H * 0.5;
    return Z && H > 0.01 ? "BRACE" : Z ? "COLUMN" : "BEAM";
  }
  function Gl(e) {
    var _a, _b;
    const { nodes: g, elements: F, nodeInputs: M, elementInputs: J } = e, H = [];
    return H.push("# OpenSeesPy model exported from Awatif FEM Studio"), H.push(`# ${g.length} nodes, ${F.length} elements`), H.push(""), H.push("import openseespy.opensees as ops"), H.push(""), H.push("ops.wipe()"), H.push("ops.model('basic', '-ndm', 3, '-ndf', 6)"), H.push(""), H.push("# --- Nodes ---"), g.forEach((Z, V) => {
      H.push(`ops.node(${V + 1}, ${Z[0]}, ${Z[1]}, ${Z[2]})`);
    }), H.push(""), H.push("# --- Boundary Conditions ---"), (_a = M.supports) == null ? void 0 : _a.forEach((Z, V) => {
      const _ = Z.map((Y) => Y ? 1 : 0).join(", ");
      H.push(`ops.fix(${V + 1}, ${_})`);
    }), H.push(""), H.push("# --- Geometric Transformations ---"), H.push("ops.geomTransf('Linear', 1, 0.0, 0.0, 1.0)  # beams (vecxz = Z)"), H.push("ops.geomTransf('Linear', 2, -1.0, 0.0, 0.0)  # columns (vecxz = -X)"), H.push(""), H.push("# --- Elements (elasticBeamColumn) ---"), F.forEach((Z, V) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (Z.length !== 2) return;
      const _ = g[Z[0]], Y = g[Z[1]], A = Math.abs(Y[2] - _[2]) > Math.max(Math.abs(Y[0] - _[0]), Math.abs(Y[1] - _[1])) ? 2 : 1, W = ((_a2 = J.areas) == null ? void 0 : _a2.get(V)) ?? 1, ve = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(V)) ?? 2e5, me = ((_c = J.shearModuli) == null ? void 0 : _c.get(V)) ?? 8e4, pe = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(V)) ?? 1, se = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(V)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(V)) ?? 1;
      H.push(`ops.element('elasticBeamColumn', ${V + 1}, ${Z[0] + 1}, ${Z[1] + 1}, ${W}, ${ve}, ${me}, ${pe}, ${se}, ${q}, ${A})`);
...
`);
  ...
  function Vl(e) {
    var _a, _b;
    const { nodes: g, elements: F, nodeInputs: M, elementInputs: J } = e, H = [];
    return H.push("# OpenSees Tcl model exported from Awatif FEM Studio"), H.push(`# ${g.length} nodes, ${F.length} elements`), H.push(""), H.push("wipe"), H.push("model basic -ndm 3 -ndf 6"), H.push(""), H.push("# --- Nodes ---"), g.forEach((Z, V) => {
      H.push(`node ${V + 1} ${Z[0]} ${Z[1]} ${Z[2]}`);
    }), H.push(""), H.push("# --- Boundary Conditions ---"), (_a = M.supports) == null ? void 0 : _a.forEach((Z, V) => {
      const _ = Z.map((Y) => Y ? 1 : 0).join(" ");
      H.push(`fix ${V + 1} ${_}`);
    }), H.push(""), H.push("# --- Geometric Transformations ---"), H.push("geomTransf Linear 1 0.0 0.0 1.0"), H.push("geomTransf Linear 2 -1.0 0.0 0.0"), H.push(""), H.push("# --- Elements ---"), F.forEach((Z, V) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (Z.length !== 2) return;
      const _ = g[Z[0]], Y = g[Z[1]], A = Math.abs(Y[2] - _[2]) > Math.max(Math.abs(Y[0] - _[0]), Math.abs(Y[1] - _[1])) ? 2 : 1, W = ((_a2 = J.areas) == null ? void 0 : _a2.get(V)) ?? 1, ve = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(V)) ?? 2e5, me = ((_c = J.shearModuli) == null ? void 0 : _c.get(V)) ?? 8e4, pe = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(V)) ?? 1, se = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(V)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(V)) ?? 1;
      H.push(`element elasticBeamColumn ${V + 1} ${Z[0] + 1} ${Z[1] + 1} ${W} ${ve} ${me} ${pe} ${se} ${q} ${A}`);
...
`);
  ...
  function Jl(e) {
    const g = [], F = [], M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map();
    for (const ve of e.split(/\r?\n/)) {
      const me = ve.trim(), pe = me.match(/ops\.node\(\s*(\d+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)/);
      if (pe) {
        const G = parseInt(pe[1]), fe = g.length;
        g.push([
          parseFloat(pe[2]),
          parseFloat(pe[3]),
          parseFloat(pe[4])
      ...
      if (se) {
        const G = parseInt(se[1]), fe = A.get(G);
        fe !== void 0 && M.set(fe, [
          se[2] === "1",
          se[3] === "1",
          se[4] === "1",
      ...
      if (q) {
        const G = parseInt(q[1]), fe = A.get(parseInt(q[2])), $e = A.get(parseInt(q[3]));
        if (fe !== void 0 && $e !== void 0) {
          const Ae = F.length;
          F.push([
            fe,
            $e
          ]), W.set(G, Ae), V.set(Ae, parseFloat(q[4])), H.set(Ae, parseFloat(q[5])), Z.set(Ae, parseFloat(q[6])), ye.set(Ae, parseFloat(q[7])), _.set(Ae, parseFloat(q[8])), Y.set(Ae, parseFloat(q[9]));
        }
        continue;
      }
      const B = me.match(/ops\.load\(\s*(\d+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)/);
      if (B) {
        const G = A.get(parseInt(B[1]));
        G !== void 0 && J.set(G, [
          parseFloat(B[2]),
          parseFloat(B[3]),
          parseFloat(B[4]),
  ...
  function Ul(e) {
    const g = [], F = [], M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map();
    for (const W of e.split(/\r?\n/)) {
      const ve = W.trim();
      if (ve.startsWith("#") || ve.startsWith("//")) continue;
      const me = ve.split(/\s+/);
      if (me[0] === "node" && me.length >= 5) {
        const pe = parseInt(me[1]), se = g.length;
        g.push([
          parseFloat(me[2]),
          parseFloat(me[3]),
          parseFloat(me[4])
      ...
      if (me[0] === "element" && me[1] === "elasticBeamColumn" && me.length >= 12) {
        const pe = A.get(parseInt(me[3])), se = A.get(parseInt(me[4]));
        if (pe !== void 0 && se !== void 0) {
          const q = F.length;
          F.push([
            pe,
            se
          ]), V.set(q, parseFloat(me[5])), H.set(q, parseFloat(me[6])), Z.set(q, parseFloat(me[7])), ye.set(q, parseFloat(me[8])), _.set(q, parseFloat(me[9])), Y.set(q, parseFloat(me[10]));
        }
        continue;
      }
  ...
  function Xl(e) {
    const g = {
      ...
      project: "",
      app: ""
    }, F = {}, M = {}, J = e.match(/FILE_SCHEMA\s*\(\s*\(\s*'([^']*)'/i);
    J && (g.schema = J[1]);
    const H = /^#(\d+)\s*=\s*([A-Z][A-Z0-9_]*)\s*\(([\s\S]*?)\)\s*;\s*$/gm;
    let Z;
    for (; (Z = H.exec(e)) !== null; ) {
      const V = parseInt(Z[1]), _ = Z[2].toUpperCase();
      F[V] = {
        id: V,
        type: _,
        args: Z[3]
  ...
  function $a(e, g) {
    const F = oo(g.args), M = Kt(e, F[0]), J = M ? Kl(e, M) : [
      0,
      0,
      0
    ];
  ...
  function Ma(e, g) {
    ...
    if (M && (J = $a(e, M)), F[0] && F[0] !== "$") {
      const H = Kt(e, F[0]);
      if (H && H.type === "IFCLOCALPLACEMENT") {
        const Z = Ma(e, H), V = ps(J.origin, Z.dirX, ds(Z.dirZ, Z.dirX), Z.dirZ);
        J.origin = [
          Z.origin[0] + V[0],
          Z.origin[1] + V[1],
          Z.origin[2] + V[2]
        ], J.dirZ = ps(J.dirZ, Z.dirX, ds(Z.dirZ, Z.dirX), Z.dirZ), J.dirX = ps(J.dirX, Z.dirX, ds(Z.dirZ, Z.dirX), Z.dirZ);
      }
    }
    return J;
  }
  ...
  function ps(e, g, F, M) {
    return [
      e[0] * g[0] + e[1] * F[0] + e[2] * M[0],
      e[0] * g[1] + e[1] * F[1] + e[2] * M[1],
      e[0] * g[2] + e[1] * F[2] + e[2] * M[2]
    ];
  }
  const Zl = 0.01;
  function Ql(e) {
    const g = Xl(e), { entities: F, typeIndex: M } = g, J = [], H = [], Z = /* @__PURE__ */ new Map();
    Z.set("Hormigon", {
      E: 2132888792e-2,
      nu: 0.2,
      rho: 2.4
    ...
    function Y(se, q, B) {
      for (const G of J) {
        const fe = G.x - se, $e = G.y - q, Ae = G.z - B;
        if (Math.sqrt(fe * fe + $e * $e + Ae * Ae) < Zl) return G.id;
      }
      return J.push({
        id: V,
        x: se,
    ...
    function ye(se) {
      const q = ya(se.args, 2) || "", B = M.IFCRELASSOCIATESMATERIAL || [];
      for (const fe of B) {
        const $e = F[fe];
        if (!$e) continue;
        const Ae = oo($e.args);
    ...
    function A(se) {
      ...
      if (q === "IFCRECTANGLEPROFILEDEF") {
        const B = oo(se.args), G = (B[1] || "").replace(/'/g, ""), fe = parseFloat(B[3]) || 0.3, $e = parseFloat(B[4]) || 0.3;
        return {
          b: fe,
          h: $e,
          name: G
      ...
      if (q === "IFCMATERIALPROFILE") {
        const B = oo(se.args), G = B[2] || B[1] || "", fe = Kt(F, G);
        if (fe) return A(fe);
      }
      if (q === "IFCMATERIALPROFILESET") {
        const B = oo(se.args), fe = (B[2] || B[1] || "").match(/#(\d+)/);
        if (fe) {
          const $e = F[parseInt(fe[1])];
          if ($e) return A($e);
        }
      }
      if (q === "IFCMATERIALPROFILESETUSAGE") {
        const G = oo(se.args)[0], fe = Kt(F, G);
        if (fe) return A(fe);
      }
      return {
        b: 0.3,
    ...
    function W(se, q, B, G) {
      ...
      for (const $e of fe) {
        const Ae = F[$e];
        if (!Ae) continue;
        const Xe = oo(Ae.args), he = Xe[5] || Xe[4] || "", je = Kt(F, he);
        if (!je) continue;
        const We = Ma(F, je), ge = ye(Ae);
        let Ie = G, ze = null, Ue = null;
        const dt = Xe[6] || Xe[5] || "", Ke = Kt(F, dt);
        if (Ke) {
          const ht = Tn(F, Ke);
          ht && (Ie = ht.depth || G, ze = ht.origin, Ue = ht.direction);
        }
        const pt = ze ? ze[0] : We.origin[0], mt = ze ? ze[1] : We.origin[1], At = ze ? ze[2] : We.origin[2], qt = Ue || (B === "Z" ? We.dirZ : We.dirX), Dt = Y(pt, mt, At), $t = Y(pt + qt[0] * Ie, mt + qt[1] * Ie, At + qt[2] * Ie);
        H.push({
          id: _++,
          type: "frame",
          nodeIds: [
    ...
    function ve(se, q, B) {
      ...
      for (const fe of G) {
        const $e = F[fe];
        if (!$e) continue;
        const Ae = oo($e.args), Xe = Ae[5] || Ae[4] || "";
        if (!Kt(F, Xe)) continue;
        let je = B;
    ...
    for (const se of pe) {
      const q = F[se];
      if (!q) continue;
      const B = oo(q.args), G = (B[2] || "").replace(/'/g, ""), fe = parseFloat(B[9]) || 0;
      me.push({
        name: G,
        elevation: fe
      });
      ...
      for (const H of J) {
        ...
        if (V) {
          if (V.type === "IFCEXTRUDEDAREASOLID") {
            const _ = oo(V.args), Y = parseFloat(_[3]) || 0, ye = Kt(e, _[1]);
            let A = [
              0,
              0,
              0
          ...
          if (V.type === "IFCMAPPEDITEM") {
            const _ = oo(V.args), Y = Kt(e, _[0]);
            if (Y && Y.type === "IFCREPRESENTATIONMAP") {
              const ye = oo(Y.args), A = Kt(e, ye[1]);
              if (A) {
                const W = Tn(e, A);
                if (W) return W;
              }
  ...
  async function nr(e, g) {
    ...
    const M = new F.IfcAPI(), J = window.location.pathname.replace(/\/[^/]*$/, "/");
    M.SetWasmPath(J), await M.Init();
    const H = M.OpenModel(new Uint8Array(g)), Z = /* @__PURE__ */ new Map(), V = /* @__PURE__ */ new Map(), _ = {
      843113511: "Columna",
      753842376: "Viga",
      1529196076: "Losa",
      900683007: "Zapata",
    ...
    for (const ve of Ea) {
      const me = new pn();
      me.name = `ifc-${ve}`, e.add(me), Y.set(ve, me);
    }
    const ye = new ll();
    let A = 0;
    const W = new ia({
    ...
    return M.StreamAllMeshes(H, (ve) => {
      const me = Z.get(ve.expressID) ?? "other", pe = Y.get(me), se = ve.geometries;
      for (let q = 0; q < se.size(); q++) {
        const B = se.get(q), G = M.GetGeometry(H, B.geometryExpressID), fe = M.GetVertexArray(G.GetVertexData(), G.GetVertexDataSize()), $e = M.GetIndexArray(G.GetIndexData(), G.GetIndexDataSize()), Ae = new ro(), Xe = new Float32Array(fe.length / 2), he = new Float32Array(fe.length / 2);
        for (let ze = 0; ze < fe.length; ze += 6) {
          const Ue = ze / 2;
          Xe[Ue] = fe[ze], Xe[Ue + 1] = fe[ze + 1], Xe[Ue + 2] = fe[ze + 2], he[Ue] = fe[ze + 3], he[Ue + 1] = fe[ze + 4], he[Ue + 2] = fe[ze + 5];
        }
        Ae.setAttribute("position", new Sn(Xe, 3)), Ae.setAttribute("normal", new Sn(he, 3)), Ae.setIndex(new Sn(new Uint32Array($e), 1));
        const je = new rl();
        ...
        Ie.applyMatrix4(je), Ie.userData.expressID = ve.expressID, Ie.userData.category = me, pe.add(Ie), ye.expandByObject(Ie), A++, G.delete();
      }
    }), M.CloseModel(H), {
      meshCount: A,
      bbox: ye,
  ...
  dr = function(e) {
    ...
    let _ = false;
    const Y = /* @__PURE__ */ new Set(), ye = /* @__PURE__ */ new Map();
    let A = "", W = {}, ve = null, me = "", pe = [], se = [], q = [], B = /* @__PURE__ */ new Set(), G = /* @__PURE__ */ new Set(), fe = /* @__PURE__ */ new Set(), $e = /* @__PURE__ */ new Map(), Ae = /* @__PURE__ */ new Map(), Xe = null, he = [], je = 0.2, We = 2, ge = 2, Ie = false, ze = 2, Ue = "x", dt = /* @__PURE__ */ new Set(), Ke = true, pt = 0.15, mt = 2, At = 2, qt = /* @__PURE__ */ new Set(), Dt = false, $t = "perimeter";
    const ht = () => ({
      b: 0.3,
      h: 0.4,
      profileIdx: 0,
    ...
    function bs(t, o, n, l, s) {
      ...
      for (const h of n) for (const T of t) for (const k of o) {
        const $ = [
          ...
          new Ne(T, h, k - f),
          new Ne(T, h, k + f)
        ], C = new ro().setFromPoints($), D = new Xo({
          color: 5596791,
          transparent: true,
          opacity: 0.4
        }), x = new Uo(C, D);
    ...
    function en(t, o, n, l, s) {
      ...
      const $ = h * 1.8, C = E * 1.2, D = E * 1.2;
      for (let x = 0; x < t.length - 1; x++) {
        const u = t[x], I = t[x + 1], z = Math.abs(I - u), R = (u + I) / 2, j = `${z.toFixed(2)} m`;
        T ? (Ln(j, R, 0, -C, $, Ht), Cn(u, 0, -C * 0.7, I, 0, -C * 0.7, 16763904, Ht)) : (Ln(j, R, p - D, 0, $, Ht), Cn(u, p - D * 0.7, 0, I, p - D * 0.7, 0, 16763904, Ht));
      }
      if (!T) for (let x = 0; x < o.length - 1; x++) {
        const u = o[x], I = o[x + 1], z = Math.abs(I - u), R = (u + I) / 2, j = `${z.toFixed(2)} m`;
        Ln(j, a - C, R, 0, $, Ht), Cn(a - C * 0.7, u, 0, a - C * 0.7, I, 0, 16763904, Ht);
      }
      Ht.traverse((x) => {
    ...
    function zn(t, o, n) {
      ...
      for (const f of t) {
        const h = f.elev;
        w ? (tn(s - m, 0, h, d + m, 0, h, E, Zt), vs(f.name, d + m * 1.5, 0, h, y, Zt)) : (tn(s, a, h, d, a, h, E, Zt), tn(d, a, h, d, i, h, E, Zt), tn(d, i, h, s, i, h, E, Zt), tn(s, i, h, s, a, h, E, Zt), vs(f.name, s - m * 1.5, a, h, y, Zt));
      }
      Zt.traverse((f) => {
    ...
    function Cn(t, o, n, l, s, d, a, i) {
      const p = [
        new Ne(t, o, n),
        new Ne(l, s, d)
      ], r = new ro().setFromPoints(p), c = new Xo({
        color: a,
        transparent: true,
        opacity: 0.6,
        depthTest: false
    ...
    const tt = {
      ...
      refgrid(t, o, n) {
        ...
        })), en(Ge.map((a) => a.coord), Ze.map((a) => a.coord), gt, Ge.map((a) => a.label), Ze.map((a) => a.label));
        {
          ...
          zn(a, Ge.map((i) => i.coord), Ze.map((i) => i.coord));
        }
      ...
      build(t) {
        ...
        const o = (t == null ? void 0 : t.col) || "40x40", n = (t == null ? void 0 : t.viga) || "30x40", l = (t == null ? void 0 : t.fc) || 210, [s, d] = o.split("x").map((N) => parseFloat(N) / 100), [a, i] = n.split("x").map((N) => parseFloat(N) / 100), p = Ge.map((N) => N.coord), r = Ze.map((N) => N.coord), c = ft.map((N) => N.elev), m = p.length, w = r.length, E = c.length, y = E - 1, f = [], h = {};
        for (let N = 0; N < E; N++) for (let ie = 0; ie < w; ie++) for (let te = 0; te < m; te++) h[`${te},${ie},${N}`] = f.length, f.push([
          p[te],
          r[ie],
          c[N]
        ]);
        const T = [], k = /* @__PURE__ */ new Set(), $ = /* @__PURE__ */ new Set(), C = /* @__PURE__ */ new Map();
        for (let N = 0; N < y; N++) for (let ie = 0; ie < w; ie++) for (let te = 0; te < m; te++) {
          const de = T.length;
          T.push([
            h[`${te},${ie},${N}`],
            h[`${te},${ie},${N + 1}`]
        ...
        for (let N = 1; N < E; N++) for (let ie = 0; ie < w; ie++) for (let te = 0; te < m - 1; te++) {
          const de = T.length;
          T.push([
            h[`${te},${ie},${N}`],
            h[`${te + 1},${ie},${N}`]
        ...
        for (let N = 1; N < E; N++) for (let ie = 0; ie < m; ie++) for (let te = 0; te < w - 1; te++) {
          const de = T.length;
          T.push([
            h[`${ie},${te},${N}`],
            h[`${ie},${te + 1},${N}`]
        ...
        if (D) {
          const N = D === "all" || D === "x" || D === "perimeter", ie = D === "all" || D === "y" || D === "perimeter";
          for (let te = 0; te < y; te++) {
            if (N) for (let de = 0; de < w; de++) {
              if (D === "perimeter" && de !== 0 && de !== w - 1) continue;
              const ee = Math.floor((m - 1) / 2);
              for (let ue = 0; ue < m - 1; ue++) {
                if (D === "perimeter" && ue !== ee) continue;
                const Ee = T.length;
                T.push([
                  h[`${ue},${de},${te}`],
                  h[`${ue + 1},${de},${te + 1}`]
            ...
            if (ie) for (let de = 0; de < m; de++) {
              if (D === "perimeter" && de !== 0 && de !== m - 1) continue;
        ...
        const u = 15100 * Math.sqrt(l) * 10, I = u / (2 * (1 + 0.2)), z = s * d, R = s * d ** 3 / 12, j = d * s ** 3 / 12, b = s * d * (s ** 2 + d ** 2) / 12, S = a * i, v = a * i ** 3 / 12, P = i * a ** 3 / 12, K = a * i * (a ** 2 + i ** 2) / 12, U = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), Me = /* @__PURE__ */ new Map();
        for (let N = 0; N < T.length; N++) U.set(N, u), ae.set(N, I), k.has(N) ? (Q.set(N, z), X.set(N, R), ce.set(N, j), re.set(N, b), Me.set(N, {
          type: "rect",
          b: s,
          h: d,
          name: `COL${o}`
        })) : x.has(N) ? (Q.set(N, z), X.set(N, R), ce.set(N, j), re.set(N, b), Me.set(N, {
          type: "rect",
          b: s,
          h: d,
          name: `BR${o}`
        })) : (Q.set(N, S), X.set(N, v), ce.set(N, P), re.set(N, K), Me.set(N, {
          type: "rect",
          b: a,
          h: i,
          name: `V${n}`
        ...
        for (let N = 0; N < w; N++) for (let ie = 0; ie < m; ie++) Te.set(h[`${ie},${N},0`], [
          true,
          true,
          true,
          true,
        ...
        return e.nodes.val = f, e.elements.val = T, e.nodeInputs.val = {
          ...
          sectionShapes: Me
        }, B = k, G = $, $e = C, console.log(`Built: ${f.length} nodes, ${T.length} elements (${k.size} cols, ${$.size} beams, ${x.size} braces)`), console.log(`  Col: ${o}, Viga: ${n}, f'c=${l}${D ? `, braces=${D}` : ""}`), {
          nodes: f.length,
          elements: T.length
        };
      },
      addCol(t, o, n) {
        ...
        for (const y of c) {
          ...
          p.push([
            f,
            h
          ]), B.add(p.length - 1), $e.set(p.length - 1, y - 1), E++;
        }
      ...
      addBeam(t, o, n, l, s) {
        ...
        const c = Ge[d].coord, m = Ze[a].coord, w = Ge[i].coord, E = Ze[p].coord, y = ft[r].elev, f = [
          ...e.nodes.val
        ], h = [
          ...((_a2 = e.elements) == null ? void 0 : _a2.val) || []
        ], T = (C, D, x) => {
          const u = f.findIndex((I) => Math.abs(I[0] - C) < 1e-3 && Math.abs(I[1] - D) < 1e-3 && Math.abs(I[2] - x) < 1e-3);
          return u >= 0 ? u : (f.push([
            C,
            D,
            x
        ...
        return h.push([
          k,
          $
        ]), G.add(h.length - 1), $e.set(h.length - 1, r - 1), e.nodes.val = f, e.elements.val = h, console.log(`Added beam ${t}-${o} \u2192 ${n}-${l} at ${s}`), h.length - 1;
      },
      addBrace(t, o, n, l, s, d) {
        ...
        const w = Ge[a].coord, E = Ze[i].coord, y = ft[p].elev, f = Ge[r].coord, h = Ze[c].coord, T = ft[m].elev, k = [
          ...e.nodes.val
        ], $ = [
          ...((_a2 = e.elements) == null ? void 0 : _a2.val) || []
        ], C = (u, I, z) => {
          const R = k.findIndex((j) => Math.abs(j[0] - u) < 1e-3 && Math.abs(j[1] - I) < 1e-3 && Math.abs(j[2] - z) < 1e-3);
          return R >= 0 ? R : (k.push([
            u,
            I,
            z
...
`;
      ...
      model3d(t) {
        ...
        const C = [], D = /* @__PURE__ */ new Set(), x = /* @__PURE__ */ new Set(), u = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < T; ee++) for (let ue = 0; ue < f; ue++) for (let Ee = 0; Ee < y; Ee++) {
          const oe = C.length;
          C.push([
            $[`${Ee},${ee},${ue}`],
            $[`${Ee},${ee + 1},${ue}`]
          ]), D.add(oe), u.set(oe, ee);
        }
        ...
        const z = 15100 * Math.sqrt(a) * 10, R = z / (2 * (1 + 0.2)), j = i * p, b = i * p ** 3 / 12, S = p * i ** 3 / 12, v = i * p * (i ** 2 + p ** 2) / 12, P = r * c, K = r * c ** 3 / 12, U = c * r ** 3 / 12, ae = r * c * (r ** 2 + c ** 2) / 12, Q = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), Me = /* @__PURE__ */ new Map(), Te = /* @__PURE__ */ new Map(), N = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < C.length; ee++) Q.set(ee, z), X.set(ee, R), D.has(ee) ? (ce.set(ee, j), re.set(ee, b), Me.set(ee, S), Te.set(ee, v), N.set(ee, {
          type: "rect",
          b: i,
          h: p,
          name: `COL${s}`
        })) : (ce.set(ee, P), re.set(ee, K), Me.set(ee, U), Te.set(ee, ae), N.set(ee, {
          type: "rect",
          b: r,
          h: c,
          name: `V${d}`
        ...
        for (let ee = 0; ee < f; ee++) for (let ue = 0; ue < y; ue++) ie.set($[`${ue},0,${ee}`], [
          true,
          true,
          true,
          true,
        ...
        e.nodes.val = k, e.elements.val = C, e.nodeInputs.val = {
          ...
          sectionShapes: N
        }, B = D, G = x, $e = u, Ge = m.map((ee, ue) => ({
          label: String.fromCharCode(65 + ue),
          coord: ee
        })), Ze = w.map((ee, ue) => ({
          label: `${ue + 1}`,
          coord: ee
        })), gt = E[E.length - 1], en(Ge.map((ee) => ee.coord), Ze.map((ee) => ee.coord), gt, Ge.map((ee) => ee.label), Ze.map((ee) => ee.label));
        {
        ...
        if (te) {
          ...
          const ee = Ge.map((Ee) => Ee.label), ue = Ze.map((Ee) => Ee.label);
        ...
        return bs(m, w, E), nt(), console.log(`Model3D: ${k.length}n ${C.length}e | ${y}x${f} grid, ${T} floors | COL${s} V${d} f'c=${a}`), {
          nodes: k.length,
          elements: C.length,
          columns: D.size,
          beams: x.size
      ...
      clear() {
        e.nodes.val = [], e.elements.val = [], e.nodeInputs && (e.nodeInputs.val = {}), e.elementInputs && (e.elementInputs.val = {}), B = /* @__PURE__ */ new Set(), G = /* @__PURE__ */ new Set(), $e = /* @__PURE__ */ new Map(), Ae = /* @__PURE__ */ new Map(), Ge = [], Ze = [], gt = 0, hs(), xs(), Vt();
      ...
      frame(t, o, n = 0, l = 0) {
        ...
        for (let y = 1; y < a.length; y++) for (let f = 0; f < s.length - 1; f++) G.add(m.length), m.push([
          r[`${f},${y}`],
          r[`${f + 1},${y}`]
        ]);
      ...
      building(t, o, n, l = 3, s = 0, d = 0, a = 0, i = 0, p = 1) {
        ...
        for (let u = 0; u < m.length; u++) for (let I = 0; I < c.length; I++) for (let z = 0; z < r.length; z++) u === 0 && y(z, I) || (h[`${z},${I},${u}`] = f.length, f.push([
          r[z],
          c[I],
          m[u]
        ]));
        ...
        const $ = [];
        for (let u = 0; u < m.length - 1; u++) for (let I = 0; I < c.length; I++) for (let z = 0; z < r.length; z++) y(z, I) || $.push({
          el: [
            h[`${z},${I},${u}`],
            h[`${z},${I},${u + 1}`]
          ],
        ...
        for (const { el: [u, I], floor: z } of $) {
          if (p <= 1) {
            B.add(k.length), $e.set(k.length, z), k.push([
              u,
              I
            ]);
            continue;
        ...
        Ae = /* @__PURE__ */ new Map();
        const C = [];
        for (let u = 1; u < m.length; u++) for (let I = 0; I < c.length; I++) for (let z = 0; z < r.length - 1; z++) C.push({
          el: [
            h[`${z},${I},${u}`],
            h[`${z + 1},${I},${u}`]
          ],
        ...
        for (let u = 1; u < m.length; u++) for (let I = 0; I < r.length; I++) for (let z = 0; z < c.length - 1; z++) C.push({
          el: [
            h[`${I},${z},${u}`],
            h[`${I},${z + 1},${u}`]
          ],
        ...
        for (const { el: [u, I], floor: z, dir: R, bay: j } of C) {
          const b = f[u], S = f[I];
          let v = u;
          for (let K = 1; K < l; K++) {
        ...
        if (dt = /* @__PURE__ */ new Set(), Ie && ze > 0) {
          const u = (I, z, R) => {
            for (let b = 0; b < f.length; b++) if (Math.abs(f[b][0] - I) < 1e-6 && Math.abs(f[b][1] - z) < 1e-6 && Math.abs(f[b][2] - R) < 1e-6) return b;
            const j = f.length;
            return f.push([
              I,
              z,
          ...
          for (let I = 1; I < m.length; I++) if (Ue === "x") for (let z = 0; z < c.length - 1; z++) {
            const R = c[z], j = c[z + 1];
            for (let b = 1; b <= ze; b++) {
              const S = R + b / (ze + 1) * (j - R), v = [];
              for (let P = 0; P < r.length; P++) v.push(u(r[P], S, m[I]));
              for (let P = 0; P < r.length - 1; P++) {
                const K = k.length;
                dt.add(K), G.add(K), $e.set(K, I - 1), Ae.set(K, {
                  dir: "x",
                  bay: P
                }), k.push([
                  v[P],
          ...
          else for (let z = 0; z < r.length - 1; z++) {
            const R = r[z], j = r[z + 1];
            for (let b = 1; b <= ze; b++) {
              const S = R + b / (ze + 1) * (j - R), v = [];
        ...
        const D = /* @__PURE__ */ new Map(), x = xt();
        for (let u = 0; u < c.length; u++) for (let I = 0; I < r.length; I++) y(I, u) || D.set(h[`${I},${u},0`], [
          ...x
        ]);
        fe = /* @__PURE__ */ new Set();
        for (const u of he) {
          const I = m.length - 1, z = u.floors.includes(-1) ? Array.from({
            length: I
          }, (R, j) => j) : u.floors.filter((R) => R < I);
          for (const R of z) {
            let j, b, S, v;
            u.dir === "x" ? (j = u.bay, S = u.bay + 1, b = u.axisIdx, v = u.axisIdx) : (j = u.axisIdx, S = u.axisIdx, b = u.bay, v = u.bay + 1);
            const P = h[`${j},${b},${R}`], K = h[`${j},${b},${R + 1}`];
            let U, ae;
            if (u.dir === "x" ? (U = h[`${S},${v},${R}`], ae = h[`${S},${v},${R + 1}`]) : (U = h[`${S},${v},${R}`], ae = h[`${S},${v},${R + 1}`]), P === void 0 || U === void 0 || K === void 0 || ae === void 0) continue;
            const Q = ge, X = We, ce = f[P], re = f[U], Me = f[K], Te = f[ae], N = [];
            for (let ie = 0; ie <= X; ie++) {
              const te = [], de = ie / X;
              for (let ee = 0; ee <= Q; ee++) {
                const ue = ee / Q, Ee = (1 - de) * ((1 - ue) * ce[0] + ue * re[0]) + de * ((1 - ue) * Me[0] + ue * Te[0]), oe = (1 - de) * ((1 - ue) * ce[1] + ue * re[1]) + de * ((1 - ue) * Me[1] + ue * Te[1]), Se = (1 - de) * ((1 - ue) * ce[2] + ue * re[2]) + de * ((1 - ue) * Me[2] + ue * Te[2]);
                ie === 0 && ee === 0 ? te.push(P) : ie === 0 && ee === Q ? te.push(U) : ie === X && ee === 0 ? te.push(K) : ie === X && ee === Q ? te.push(ae) : (te.push(f.length), f.push([
                  Ee,
                  oe,
                  Se
                ]));
            ...
            for (let ie = 0; ie < X; ie++) for (let te = 0; te < Q; te++) {
              const de = N[ie][te], ee = N[ie][te + 1], ue = N[ie + 1][te + 1], Ee = N[ie + 1][te], oe = k.length;
        ...
        if (qt = /* @__PURE__ */ new Set(), Ke) {
          const u = l, I = l, z = /* @__PURE__ */ new Map(), R = (j, b, S) => `${Math.round(j * 1e4)},${Math.round(b * 1e4)},${Math.round(S * 1e4)}`;
          for (let j = 0; j < f.length; j++) z.set(R(f[j][0], f[j][1], f[j][2]), j);
          for (let j = 1; j < m.length; j++) {
            const b = m[j];
            for (let S = 0; S < r.length - 1; S++) for (let v = 0; v < c.length - 1; v++) {
              const P = r[S], K = r[S + 1], U = c[v], ae = c[v + 1], Q = [];
              for (let X = 0; X <= I; X++) {
                const ce = [];
                for (let re = 0; re <= u; re++) {
                  const Me = P + re / u * (K - P), Te = U + X / I * (ae - U);
                  if (X === 0 && re === 0) ce.push(h[`${S},${v},${j}`]);
                  else if (X === 0 && re === u) ce.push(h[`${S + 1},${v},${j}`]);
                  else if (X === I && re === 0) ce.push(h[`${S},${v + 1},${j}`]);
                  else if (X === I && re === u) ce.push(h[`${S + 1},${v + 1},${j}`]);
                  else {
                    const N = R(Me, Te, b), ie = z.get(N);
                    if (ie !== void 0) ce.push(ie);
                      ...
                      f.push([
                        ...
                        Te,
                        b
                      ]), z.set(N, te), ce.push(te);
                    }
                  }
                }
                Q.push(ce);
              }
              for (let X = 0; X < I; X++) for (let ce = 0; ce < u; ce++) {
                const re = Q[X][ce], Me = Q[X][ce + 1], Te = Q[X + 1][ce + 1], N = Q[X + 1][ce], ie = k.length;
                qt.add(ie), $e.set(ie, j - 1), k.push([
                  re,
                  Me,
                  Te,
                  N
        ...
        if (Dt && $t) {
          const u = $t === "all" || $t === "x" || $t === "perimeter", I = $t === "all" || $t === "y" || $t === "perimeter", z = m.length - 1;
          for (let R = 0; R < z; R++) {
            if (u) for (let j = 0; j < c.length; j++) {
              if ($t === "perimeter" && j !== 0 && j !== c.length - 1) continue;
              const b = Math.floor((r.length - 1) / 2);
              for (let S = 0; S < r.length - 1; S++) {
                if ($t === "perimeter" && S !== b || y(S, j) || y(S + 1, j)) continue;
                const v = h[`${S},${j},${R}`], P = h[`${S + 1},${j},${R + 1}`], K = h[`${S + 1},${j},${R}`], U = h[`${S},${j},${R + 1}`];
            ...
            if (I) for (let j = 0; j < r.length; j++) {
              ...
              for (let S = 0; S < c.length - 1; S++) {
                if ($t === "perimeter" && S !== b || y(j, S) || y(j, S + 1)) continue;
                const v = h[`${j},${S},${R}`], P = h[`${j},${S + 1},${R + 1}`], K = h[`${j},${S + 1},${R}`], U = h[`${j},${S},${R + 1}`];
                v !== void 0 && P !== void 0 && (k.push([
                  v,
                  P
                ]), $e.set(k.length - 1, R)), K !== void 0 && U !== void 0 && (k.push([
      ...
      example(t) {
        ...
        switch (t) {
          ...
          case "edif-acero":
          case "edificio-acero": {
            st("edificio"), qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, he = [], Ie = true, ze = 2;
            const o = pe.reduce((l, s) => l + s, 0) / pe.length, n = se.reduce((l, s) => l + s, 0) / se.length;
            Ue = o >= n ? "y" : "x", Ke = true, pt = 0.08, Dt = false, Oe();
            break;
          }
          ...
          case "edificio-acero-diag": {
            st("edificio"), qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, he = [], Ie = true, ze = 2;
            const o = pe.reduce((l, s) => l + s, 0) / pe.length, n = se.reduce((l, s) => l + s, 0) / se.length;
            Ue = o >= n ? "y" : "x", Ke = true, pt = 0.08, Dt = true, $t = "perimeter", Oe();
            break;
          }
          case "edif-muros":
          case "edificio-muros": {
          ...
          case "mezzanine": {
            st("edificio"), W.nPisos && (W.nPisos.val = 1), W.hPiso && (W.hPiso.val = 4.5), W.nVanosX && (W.nVanosX.val = 3), W.nVanosY && (W.nVanosY.val = 2), W.nSubViga && (W.nSubViga.val = 3), pe = [
              ...
              5,
              5
            ], qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, he = [], Ie = true, ze = 2, Ue = pe[0] >= se[0] ? "y" : "x", Ke = true, pt = 0.08, mt = 3, At = 3, Oe();
            break;
          }
          case "galpon": {
      ...
      plateQ4(t = 10, o = 10, n = 16, l = 16, s = "simply-supported", d = -10, a = 0.2, i = 3e7, p = 0.3, r = 0) {
        ...
        if (e.nodeInputs && (e.nodeInputs.val = {
          ...
          const $ = /* @__PURE__ */ new Map(), C = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map();
      ...
      setParam(t, o) {
        W[t] ? (W[t].val = o, console.log(`${t} = ${o}`), fo(), Oe()) : bt[t] ? (bt[t].val = o, console.log(`${t} = ${o}`), fo(), Oe()) : console.error(`Par\xE1metro "${t}" no encontrado. Disponibles: ${Object.keys({
          ...W,
          ...bt
        }).join(", ")}`);
      },
...
Util:     cad.info()  cad.clear()  cad.help()  cad.helpFull()
      ...
      units(t, o) {
        t && (g = t), o && (F = o), M = _o(g, F);
        const n = Le.querySelector("#cad3d-force-unit"), l = Le.querySelector("#cad3d-length-unit");
        return n && (n.textContent = g), l && (l.textContent = F), A && st(A), console.log(`Unidades: ${M.label} | E=${M.E.toExponential(3)} ${M.stress}`), M.id;
      },
      view(t) {
      },
      get mesh() {
    ...
    function le(t) {
      var _a2, _b;
      return ((_a2 = W[t]) == null ? void 0 : _a2.val) ?? ((_b = bt[t]) == null ? void 0 : _b.val) ?? 0;
    }
    function Oe() {
      switch (A) {
        case "truss":
      ...
      if (![
        ...
        if (Z.size > 0 || V.size > 0 || _) {
          const o = e.elements.val, n = o.filter((l, s) => !(Z.has(s) || V.has(s) || _ && !Y.has(s)));
    ...
    function Ts() {
      ...
      for (let b = 0; b <= w; b++) for (let S = 0; S <= m; S++) f.push([
        S * E,
        b * y
      ]);
      ...
      for (const b of r) for (const S of c) {
        ...
        for (let K = 0; K < f.length; K++) {
          const U = Math.hypot(f[K][0] - b, f[K][1] - S);
          U < v && (v = U, P = K);
        }
      ...
      const u = D.nodeResults.map((b) => [
        b.x,
        b.y,
        0
      ]), I = D.elementResults.map((b) => [
        ...b.nodes
      ]);
      e.nodes.val = u, e.elements.val = I;
      const z = /* @__PURE__ */ new Map();
      D.nodeResults.forEach((b, S) => {
        z.set(S, [
          0,
          0,
          b.w,
    ...
    function As() {
      ...
      try {
        const x = Ft(w, c, D, C), u = $o(w, c, C, x), I = w.map((R) => [
          R[0],
          0,
          R[1]
        ]);
        ...
        x && x.deformations && x.deformations.forEach((R) => {
          ...
          z = Math.max(z, j);
        }), console.log(`Viga Alta: ${w.length} nodos, ${c.length} triangulos`), console.log(`  L=${t}, H=${o}, t=${n}, E=${l}, nu=${s}`), console.log(`  Carga: q=${a} kN/m sobre ${i}m central`), console.log(`  max|u| = ${z.toExponential(4)}`);
      } catch (x) {
    ...
    function zs() {
      const t = le("H") || 4, o = le("B") || 3, n = le("tw") || 0.3, l = le("tb") || 0.4, s = le("meshSize") || 0.2, d = le("E") || 25e6, a = le("nu") || 0.2, i = d / (2 * (1 + a)), p = le("gamma") || 18, r = le("Ka") || 0.33, c = le("Es") || 5e4, m = le("nus") || 0.3, w = c / (2 * (1 + m)), E = le("kn") || 1e6, y = le("ks") || 1e4, f = le("gammaW") || 9.81, h = le("Hw") || 3.5, T = le("qs") || 0, k = St, $ = o * 0.3, C = o * 0.7, D = [
        [
          -$,
          0,
          0
      ...
      let x = [], u = [], I = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), R;
      if (k === 0) {
        const S = Mo({
          points: D,
          polygon: [
        ...
        for (let P = 0; P < v.length; P++) {
          const { idx: K, y: U } = v[P], ae = l + t - U, Q = r * p * ae + r * T;
          let X = s;
          P > 0 && P < v.length - 1 ? X = (v[P + 1].y - v[P - 1].y) / 2 : P === 0 && v.length > 1 ? X = (v[1].y - v[0].y) / 2 : P === v.length - 1 && v.length > 1 && (X = (v[P].y - v[P - 1].y) / 2);
          const ce = Q * X;
          Math.abs(ce) > 1e-10 && z.set(K, [
            ce,
            0,
            0,
            0,
        ...
        if (k === 2) {
          ...
          for (let oe = 1; oe < X.length; oe++) {
            const Se = x[X[oe]][1] - x[ce[ce.length - 1]][1];
            Math.abs(Se) > s * 0.05 && ce.push(X[oe]);
          }
          ...
          const ie = u.length - N, te = /* @__PURE__ */ new Map(), de = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), ue = /* @__PURE__ */ new Map(), Ee = /* @__PURE__ */ new Map();
          ...
          if (T > 0) {
            ...
            for (let Se = 0; Se < oe.length; Se++) {
              ...
              const at = -T * He, rt = z.get(oe[Se].idx) || [
                0,
                0,
                0,
                0,
              ...
              rt[1] += at, z.set(oe[Se].idx, rt);
            }
          }
          console.log(`  Interfaz Goodman: ${X.length} nodos interfaz, ${ie} elem interfaz, kn=${E}, ks=${y}`);
        } else {
          const P = [
            [
              -$,
          ...
          const ae = (N) => {
            const ie = (x[N[0]][0] + x[N[1]][0] + x[N[2]][0]) / 3, te = (x[N[0]][1] + x[N[1]][1] + x[N[2]][1]) / 3, de = ie >= -$ && ie <= C && te >= 0 && te <= l, ee = ie >= 0 && ie <= n && te >= l && te <= l + t;
            return de || ee;
          }, Q = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), Me = /* @__PURE__ */ new Map(), Te = [];
          for (let N = 0; N < u.length; N++) {
            const ie = ae(u[N]);
            Te.push(!ie), ie ? (Q.set(N, d), X.set(N, d), re.set(N, a), Me.set(N, i), ce.set(N, 1)) : (Q.set(N, c), X.set(N, c), re.set(N, m), Me.set(N, w), ce.set(N, 1));
          }
          R = {
            elasticities: Q,
          ...
          for (let N = 0; N < u.length; N++) {
            if (!Te[N]) continue;
            const ie = u[N], te = x[ie[0]], de = x[ie[1]], ee = x[ie[2]], ue = Math.abs((de[0] - te[0]) * (ee[1] - te[1]) - (ee[0] - te[0]) * (de[1] - te[1])) / 2, Ee = -p * ue / 3;
            for (const oe of ie) {
              const Se = z.get(oe) || [
                0,
                0,
          ...
          if (T > 0) {
            ...
            for (let ie = 0; ie < N.length; ie++) {
              ...
              const de = -T * te, ee = z.get(N[ie].idx) || [
                0,
                0,
                0,
                0,
      ...
      try {
        const S = Ft(x, u, j, R), v = u.filter((ce) => ce.length === 3), P = {};
        ...
        S && S.deformations && S.deformations.forEach((ce) => {
          ...
          X = Math.max(X, re);
        }), console.log(`Muro Contencion [${b[k]}]: ${x.length} nodos, ${u.length} triangulos`), console.log(`  H=${t}, B=${o}, tw=${n}, tb=${l}, Ka=${r}, gamma=${p}, qs=${T}`), k === 1 && console.log(`  Es=${c}, nus=${m}`), k === 2 && console.log(`  Es=${c}, nus=${m}, kn=${E}, ks=${y}`), k === 3 && console.log(`  gammaW=${f}, Hw=${h}`), console.log(`  max|u| = ${X.toExponential(4)}`);
      } catch (S) {
    ...
    function Ls() {
      ...
      for (let I = 0; I <= a; I++) for (let z = 0; z <= d; z++) {
        const R = I * (d + 1) + z;
        let j = E, b = y;
        (z === 0 || z === d) && (j = E / 2), (I === 0 || I === a) && (b = y / 2), k.push({
          node: R,
          dof: 0,
          k: w * j * b
        });
      ...
      let $ = 0;
      for (let I = 0; I <= a; I++) for (let z = 0; z <= d; z++) Math.abs(z * E - f) <= T + 1e-6 && Math.abs(I * y - h) <= T + 1e-6 && $++;
      const C = r / Math.max($, 1), D = [];
      for (let I = 0; I <= a; I++) for (let z = 0; z <= d; z++) {
        const R = z * E, j = I * y;
        Math.abs(R - f) <= T + 1e-6 && Math.abs(j - h) <= T + 1e-6 && D.push({
          node: I * (d + 1) + z,
          dof: 0,
          value: C
        });
      ...
      try {
        ...
        j.push([
          ...
          R + 3,
          R
        ]), e.nodes.val = z, e.elements.val = j;
    ...
    function Cs() {
      ...
      for (let b = 0; b < x.length; b++) {
        ...
        for (const [P, K] of T) {
          const U = Math.sqrt((S - P) * (S - P) + (v - K) * (v - K));
          U >= f * 0.7 && U <= f * 1.5 && u.set(b, [
            true,
            true,
            true,
            false,
      ...
      for (let b = 0; b < x.length; b++) {
        const S = x[b][0], v = x[b][1];
        Math.abs(S - E) <= h && Math.abs(v - y) <= h && z++;
      }
      const R = m / Math.max(z, 1);
      for (let b = 0; b < x.length; b++) {
        const S = x[b][0], v = x[b][1];
      ...
      const j = {
        elasticities: new Map(D.map((b, S) => [
          S,
          p
        ])),
        elasticitiesOrthogonal: new Map(D.map((b, S) => [
          S,
          p
        ])),
        thicknesses: new Map(D.map((b, S) => [
          S,
          n
        ])),
        poissonsRatios: new Map(D.map((b, S) => [
          S,
          r
        ])),
        shearModuli: new Map(D.map((b, S) => [
          S,
          c
        ]))
      };
      console.log(`Placa Base: ${t * 1e3}x${o * 1e3}mm, t=${n * 1e3}mm, ${w} pernos d=${l * 1e3}mm`), console.log(`  P=${m} kN, col=${a * 1e3}mm, mesh=${i * 1e3}mm`), console.log(`  ${D.length} triangulos, ${x.length} nodos`);
      try {
        const b = Ft(x, D, {
          supports: u,
          loads: I
        }, j), S = $o(x, D, j, b);
        e.nodes.val = x, e.elements.val = D, b && e.deformOutputs && (e.deformOutputs.val = b), e.nodeInputs && (e.nodeInputs.val = {
          supports: u,
          loads: I
    ...
    function Ps() {
      const t = le("colB") || 0.3, o = le("colH") || 0.3, n = le("colT") || 8e-3, l = le("colLen") || 1.5, s = le("Lx") || 0.45, d = le("Ly") || 0.45, a = le("tPlaca") || 0.025, i = le("dBolt") || 0.022, p = le("sx") || 0.32, r = le("sy") || 0.32, c = Math.round(le("nSubColV") || 6), m = Math.round(le("nSubColH") || 4), w = Math.round(le("nSubPlaca") || 10), E = le("E") || 2e8, y = le("nu") || 0.3, f = E / (2 * (1 + y)), h = le("P") || -300, T = s / 2, k = d / 2, $ = i / 2, C = t / 2, D = o / 2, x = [], u = [], I = w, z = s / I, R = d / I, j = (de, ee) => ee * (I + 1) + de;
      ...
      const b = [
        ...
        for (const [ue, Ee] of b) if ((de - ue) * (de - ue) + (ee - Ee) * (ee - Ee) < $ * $) return true;
        return false;
      }, v = u.length;
      for (let de = 0; de < I; de++) for (let ee = 0; ee < I; ee++) {
        const ue = (ee + 0.5) * z, Ee = (de + 0.5) * R;
        S(ue, Ee) || u.push([
          j(ee, de),
          j(ee + 1, de),
          j(ee + 1, de + 1),
      ...
      const P = u.length - v, K = c, U = m, ae = [
        ...
        for (let ue = 0; ue < (I + 1) * (I + 1); ue++) if (Math.abs(x[ue][0] - de) < z * 0.3 && Math.abs(x[ue][1] - ee) < R * 0.3 && Math.abs(x[ue][2]) < 1e-6) return ue;
        return -1;
      };
      for (const [de, ee] of X) {
        ...
        for (let at = 0; at <= K; at++) {
          ...
          for (let Tt = 0; Tt <= U; Tt++) {
            const Et = Tt / U, Ot = ue + Et * (oe - ue), yo = Ee + Et * (Se - Ee);
            if (at === 0) {
              const Ut = ce(Ot, yo);
              if (Ut >= 0) {
                rt.push(Ut);
            ...
            let to = -1;
            for (let Ut = 0; Ut < x.length; Ut++) if (Math.abs(x[Ut][0] - Ot) < 1e-6 && Math.abs(x[Ut][1] - yo) < 1e-6 && Math.abs(x[Ut][2] - wt) < 1e-6) {
              to = Ut;
              break;
            }
      ...
      for (let de = v; de < v + P; de++) te.elasticities.set(de, E), te.poissonsRatios.set(de, y), te.thicknesses.set(de, a), te.shearModuli.set(de, f);
    ...
    function Fs() {
      ...
      try {
        ...
        console.log(`Talud SRM: ${T.length} nodos, ${h.length} triangulos`), console.log(`  H=${t}, angulo=${o}\xB0, c=${p} kPa, \u03C6=${r}\xB0, \u03B3=${i}`), console.log("  \u2550\u2550\u2550 Strength Reduction Method (Mohr-Coulomb) \u2550\u2550\u2550"), console.log(`  FOS = ${u.fos.toFixed(3)}`), console.log(`  max|u| = ${j.toExponential(4)}`), console.log(`  max \u03B5_pl = ${b.toExponential(4)}`), u.fos < 1 ? console.warn("  \u26A0 TALUD INESTABLE (FOS < 1.0)") : u.fos < 1.5 && console.warn("  \u26A0 FOS < 1.5 \u2014 revisar estabilidad");
      } catch (D) {
    ...
    function Rs(t, o, n, l, s) {
      ...
      if (qe.vigaMat === 0) {
        for (let i = 0; i < o.length; i++) {
          ...
          m[r] = +zt(p.b).toFixed(2), m[c] = +zt(p.h).toFixed(2), t.addBinding(m, r, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `b sv${n}${i + 1}`
        ...
        for (let i = 0; i < o.length; i++) {
          const p = o[i], r = {}, c = `${n}${i}`;
          r[`bf${c}`] = +zt(p.bf ?? 0.2).toFixed(3), r[`h${c}`] = +zt(p.hf ?? 0.4).toFixed(3), r[`tf${c}`] = +zt(p.tf ?? 0.015).toFixed(3), r[`tw${c}`] = +zt(p.tw ?? 0.01).toFixed(3), t.addBinding(r, `bf${c}`, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `bf sv${n}${i + 1}`
        ...
        for (let i = 0; i < o.length; i++) {
          const p = o[i], r = {}, c = `${n}${i}`;
          r[`bc${c}`] = +zt(p.bc ?? 0.2).toFixed(3), r[`hc${c}`] = +zt(p.hc ?? 0.3).toFixed(3), r[`t${c}`] = +zt(p.t ?? 8e-3).toFixed(3), t.addBinding(r, `bc${c}`, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `b sv${n}${i + 1}`
    ...
    function Ho() {
      ...
      if (!o) return;
      o.style.display = "";
      const n = M, l = Math.round(((_a2 = W.nPisos) == null ? void 0 : _a2.val) ?? 3), s = Ta(), d = Aa(), a = pe.length || 1, i = se.length || 1;
      ...
      for (let x = 0; x < l; x++) {
        ...
        if (qe.colMat === 0) if (qe.colShape === 1) {
          ...
          const R = +Co(1e8).toFixed(0), j = +Co(3e8).toFixed(0), b = Math.max(1, Math.round((j - R) / 200));
          I.addBinding(z, "Es", {
            min: R,
            max: j,
            step: b,
      ...
      w.addBinding(E, "activar", {
        ...
        var _a3, _b, _c;
        ((_a3 = x.target) == null ? void 0 : _a3.key) === "activar" && (Ie = x.value, Oe()), ((_b = x.target) == null ? void 0 : _b.key) === "direccion" && (Ue = x.value === 0 ? "x" : "y", Oe()), ((_c = x.target) == null ? void 0 : _c.key) === "cantidad" && (ze = Math.round(x.value), Oe());
      }), _t.addBlade({
        view: "separator"
      });
      ...
      y.addBinding(f, "activar", {
        ...
        var _a3, _b, _c, _d;
        ((_a3 = x.target) == null ? void 0 : _a3.key) === "activar" && (Ke = x.value, Oe()), ((_b = x.target) == null ? void 0 : _b.key) === "espesor" && (pt = Lt(x.value), Oe()), ((_c = x.target) == null ? void 0 : _c.key) === "subdivX" && (mt = Math.round(x.value), Oe()), ((_d = x.target) == null ? void 0 : _d.key) === "subdivY" && (At = Math.round(x.value), Oe());
      }), _t.addBlade({
        view: "separator"
      });
      ...
      h.addBinding(T, "espesor", {
        ...
        var _a3, _b, _c;
        ((_a3 = x.target) == null ? void 0 : _a3.key) === "espesor" && (je = Lt(x.value), Oe()), ((_b = x.target) == null ? void 0 : _b.key) === "subdivH" && (We = Math.round(x.value), Oe()), ((_c = x.target) == null ? void 0 : _c.key) === "subdivW" && (ge = Math.round(x.value), Oe());
      });
      const k = pe.length || 1, $ = se.length || 1, C = k + 1, D = $ + 1;
      if (k > 0) {
        const x = h.addFolder({
          title: `Muros dir X (${k} vanos)`,
          expanded: false
        });
        for (let u = 0; u < k; u++) for (let I = 0; I < D; I++) {
          const z = `wx_${u}_${I}`, R = he.some((S) => S.dir === "x" && S.bay === u && S.axisIdx === I), j = {};
          j[z] = R;
      ...
      if ($ > 0) {
        ...
        for (let u = 0; u < $; u++) for (let I = 0; I < C; I++) {
          const z = `wy_${u}_${I}`, R = he.some((S) => S.dir === "y" && S.bay === u && S.axisIdx === I), j = {};
          j[z] = R;
      ...
      if (n) {
        ...
        for (const p of a) {
          ...
          i.addBinding(r, "min", {
            ...
            W[p.key] && (W[p.key].min = r.min, W[p.key].max = r.max, W[p.key].val < r.min && (W[p.key].val = r.min), W[p.key].val > r.max && (W[p.key].val = r.max)), fo(), Oe();
          });
        }
        ve.on("change", (p) => {
          var _a2, _b;
          ...
          if (r && W[r]) {
            if (W[r].val = typeof p.value == "boolean" ? p.value ? 1 : 0 : p.value, A === "edificio" && (r === "nVanosX" || r === "nVanosY" || r === "nPisos")) {
...
`);
    ...
    function nn() {
      ...
      if (!t) return;
      const o = ((_a2 = Pe.frequencies) == null ? void 0 : _a2[xe]) || 1, n = Math.max(0.5, Math.min(3, o * 0.1)), l = performance.now(), s = ke.length, d = e.elements.rawVal, { extent: a } = So(), i = Le.querySelector("#cad3d-modal-scale"), p = i && parseFloat(i.value) || 5;
      let r = 0;
      for (let $ = 0; $ < s; $++) {
        const C = t[$ * 6] || 0, D = t[$ * 6 + 1] || 0, x = t[$ * 6 + 2] || 0;
        r = Math.max(r, Math.sqrt(C * C + D * D + x * x));
      }
      ...
      function k() {
        ...
        for (let D = 0; D < s; D++) {
          const x = ke[D];
          f[D * 3] = x[0] + (t[D * 6] || 0) * C, f[D * 3 + 1] = x[1] + (t[D * 6 + 1] || 0) * C, f[D * 3 + 2] = x[2] + (t[D * 6 + 2] || 0) * C;
        }
        if (w) {
          const D = w.geometry, x = D.getAttribute("position");
          x && x.array.length === f.length ? (x.array.set(f), x.needsUpdate = true) : D.setAttribute("position", new Oo(f.slice(), 3));
        }
        if (E) {
          for (let u = 0; u < h.length; u++) {
            const [I, z] = h[u];
            T[u * 6] = f[I * 3], T[u * 6 + 1] = f[I * 3 + 1], T[u * 6 + 2] = f[I * 3 + 2], T[u * 6 + 3] = f[z * 3], T[u * 6 + 4] = f[z * 3 + 1], T[u * 6 + 5] = f[z * 3 + 2];
          }
          const D = E.geometry, x = D.getAttribute("position");
          x && x.array.length === T.length ? (x.array.set(T), x.needsUpdate = true) : D.setAttribute("position", new Oo(T.slice(), 3));
        }
        if (y) {
          const D = [];
          for (const x of d) if (x.length === 3) {
            const [u, I, z] = x;
            D.push(f[u * 3], f[u * 3 + 1], f[u * 3 + 2]), D.push(f[I * 3], f[I * 3 + 1], f[I * 3 + 2]), D.push(f[z * 3], f[z * 3 + 1], f[z * 3 + 2]);
          } else if (x.length === 4) {
            const [u, I, z, R] = x;
            D.push(f[u * 3], f[u * 3 + 1], f[u * 3 + 2]), D.push(f[I * 3], f[I * 3 + 1], f[I * 3 + 2]), D.push(f[z * 3], f[z * 3 + 1], f[z * 3 + 2]), D.push(f[u * 3], f[u * 3 + 1], f[u * 3 + 2]), D.push(f[z * 3], f[z * 3 + 1], f[z * 3 + 2]), D.push(f[R * 3], f[R * 3 + 1], f[R * 3 + 2]);
          }
          if (D.length > 0) {
            const x = y.geometry, u = new Float32Array(D), I = x.getAttribute("position");
    ...
    function Bn() {
      ...
      if (!n.loads || n.loads.size === 0) {
        ...
        for (const S of C) x.has(`${D(t[S][0])},${D(t[S][1])}`) && u.add(S);
        const I = /* @__PURE__ */ new Set(), z = /* @__PURE__ */ new Set();
        if (T !== 0 || k !== 0) {
          let S = -1 / 0, v = -1 / 0;
          for (const K of u) S = Math.max(S, D(t[K][0])), v = Math.max(v, D(t[K][1]));
          ...
          P.forEach((K) => {
            if (T !== 0) {
              const U = /* @__PURE__ */ new Set();
              for (const ae of K) if (D(t[ae][0]) === S) {
                const Q = D(t[ae][1]);
            ...
            if (k !== 0) {
              const U = /* @__PURE__ */ new Set();
              for (const ae of K) if (D(t[ae][1]) === v) {
                const Q = D(t[ae][0]);
                U.has(Q) || (U.add(Q), z.add(ae));
              }
            }
          });
        }
        ...
        for (let S = 0; S < o.length; S++) {
          ...
          if (!(Math.abs(P) < 1e-15)) {
            if (v.length === 2) {
              const K = ((_b = l.areas) == null ? void 0 : _b.get(S)) ?? 0, U = t[v[0]], ae = t[v[1]], Q = Math.sqrt((ae[0] - U[0]) ** 2 + (ae[1] - U[1]) ** 2 + (ae[2] - U[2]) ** 2), ce = -(P * K * Q * R) / 2;
              j.set(v[0], (j.get(v[0]) ?? 0) + ce), j.set(v[1], (j.get(v[1]) ?? 0) + ce);
            } else if (v.length >= 3) {
              const K = ((_c = l.thicknesses) == null ? void 0 : _c.get(S)) ?? 0;
              let U = 0;
              if (v.length === 3) {
                const [X, ce, re] = v.map((Me) => t[Me]);
                U = 0.5 * Math.abs((ce[0] - X[0]) * (re[1] - X[1]) - (re[0] - X[0]) * (ce[1] - X[1]));
              } else if (v.length === 4) {
                const [X, ce, re, Me] = v.map((Te) => t[Te]);
                if (U = 0.5 * Math.abs((ce[0] - X[0]) * (re[1] - X[1]) - (re[0] - X[0]) * (ce[1] - X[1])) + 0.5 * Math.abs((re[0] - X[0]) * (Me[1] - X[1]) - (Me[0] - X[0]) * (re[1] - X[1])), U < 1e-10) {
                  const Te = [
                    ce[0] - X[0],
                    ce[1] - X[1],
                    ce[2] - X[2]
        ...
        for (const S of C) {
          const v = I.has(S) ? T : 0, P = z.has(S) ? k : 0, K = j.get(S) ?? 0, U = b.has(S) ? h : 0, ae = K + U;
      ...
      try {
        ...
        if (y) {
          ...
          let h = 0, T = -1, k = 0, $ = 0;
          y.deformations && y.deformations.forEach((I, z) => {
            const R = Math.sqrt(I[0] * I[0] + I[1] * I[1] + I[2] * I[2]);
            R > h && (h = R, T = z, k = I[0], $ = I[2]);
          ...
          const C = performance.now(), D = $o(t, o, l, y), x = performance.now() - C;
          D && (e.analyzeOutputs.val = D, E(`<span style="color:#888">4.</span> Fuerzas internas: <span style="color:#00cc88">${x.toFixed(0)} ms</span>`), E("&nbsp;&nbsp;&nbsp;<i>F<sub>int</sub> = K<sub>local</sub> &middot; T &middot; u</i>"));
      ...
      window.__femLog = w, console.log(`FEM Solver: ${t.length}n ${o.length}e \u2192 ${(performance.now() - s).toFixed(0)}ms`), _e && setTimeout(() => qn(), 50);
    }
    function Dn(t, o) {
    ...
    function Fa(t, o, n, l, s, d, a) {
      const p = 4700 * Math.sqrt(d / 1e3) * 1e3 / l, r = t - 2 * n, c = o - 2 * n, m = t * o - r * c, w = (t * o * o * o - r * c * c * c) / 12, E = (o * t * t * t - c * r * r * r) / 12, y = r * c, f = r * c * c * c / 12, h = c * r * r * r / 12, T = m + p * y, k = w + p * f, $ = E + p * h, C = l / (2 * (1 + s)), D = (t - n) * (o - n), x = 2 * ((t - n) / n + (o - n) / n), u = 4 * D * D / (x > 0 ? x : 1);
      return {
        A: T,
        Iz: k,
        Iy: $,
    ...
    function Eo() {
      ...
      if ((A === "edificio" || A === "frame") && B.size > 0) {
        ...
        for (let f = 0; f < t.length; f++) {
          if (fe.has(f)) {
            const I = 4700 * Math.sqrt(i / 1e3) * 1e3, z = 0.2;
            n.elasticities.set(f, I), n.poissonsRatios.set(f, z), n.thicknesses.set(f, je), n.shearModuli.set(f, I / (2 * (1 + z))), n.densities.set(f, 24 / 9.80665);
            continue;
          }
          if (qt.has(f)) {
            const I = 4700 * Math.sqrt(i / 1e3) * 1e3, z = 0.2;
            n.elasticities.set(f, I), n.poissonsRatios.set(f, z), n.thicknesses.set(f, pt), n.shearModuli.set(f, I / (2 * (1 + z))), n.densities.set(f, 24 / 9.80665);
            continue;
          }
          const h = B.has(f), T = $e.get(f) ?? 0, k = p[T] ?? p[0] ?? {
            bCol: 0.4,
            hCol: 0.4,
            dCol: 0.4
          };
          let $, C, D, x;
          if (h) if (s === 0) C = r, D = c, x = m, $ = a === 1 ? qs(k.dCol) : Dn(k.bCol, k.hCol), n.sectionShapes.set(f, a === 1 ? {
            type: "circ",
            d: k.dCol
          } : {
            type: "rect",
            ...
            if (I <= 1) {
              const z = Lo[k.colProfileIdx] ?? Lo[0];
              $ = {
                A: z.A,
                Iz: z.Iz,
                Iy: z.Iy,
              ...
              const z = k.colBf ?? 0.3, R = k.colHf ?? 0.3, j = k.colTf ?? 0.02, b = k.colTw ?? 0.012;
              $ = Hn(z, R, j, b);
              const S = `I${(R * 100).toFixed(0)}x${(z * 100).toFixed(0)}`;
              n.sectionShapes.set(f, {
                type: "I",
                b: z,
                h: R,
              ...
              const z = k.colBc ?? 0.3, R = k.colHc ?? 0.3, j = k.colT ?? 0.01;
              $ = jn(z, R, j);
              const b = `\u25A1${(R * 100).toFixed(0)}x${(z * 100).toFixed(0)}x${(j * 1e3).toFixed(0)}`;
              n.sectionShapes.set(f, {
                type: "HSS",
                b: z,
                h: R,
            ...
            const I = k.colBc ?? 0.3, z = k.colHc ?? 0.3, R = k.colT ?? 0.01, j = k.colFc ?? 28e3, b = k.colEs ?? 2e8, S = k.colNuS ?? 0.3;
            k.colNuC;
            const v = Fa(I, z, R, b, S, j);
            $ = {
              A: v.A,
              Iz: v.Iz,
              Iy: v.Iy,
            ...
            x = (P * v.A_steel + K * v.A_conc) / (v.A_steel + v.A_conc);
            const U = `CFT ${(z * 1e3).toFixed(0)}X${(I * 1e3).toFixed(0)}X${(R * 1e3).toFixed(0)}`;
            n.sectionShapes.set(f, {
              type: "CFT",
              b: I,
              h: z,
            ...
            const I = Ae.get(f), z = I ? I.dir === "x" ? k.vigasX : k.vigasY : [], R = I ? z[I.bay] ?? z[0] ?? ht() : ht();
            if (d === 0) C = r, D = c, x = m, $ = Dn(R.b, R.h), n.sectionShapes.set(f, {
              type: "rect",
              b: R.b,
              h: R.h
            });
              ...
              if (j <= 1) {
                const b = Lo[R.profileIdx ?? 0] ?? Lo[0];
                $ = {
                  A: b.A,
                  Iz: b.Iz,
                  Iy: b.Iy,
                ...
                $ = Hn(b, S, v, P);
                const K = `I${(S * 100).toFixed(0)}x${(b * 100).toFixed(0)}`;
                n.sectionShapes.set(f, {
                  type: "I",
                  b,
                  h: S,
          ...
          const u = ye.get(f);
          if (u) {
            if ((u.material ?? 1) === 0 ? (C = r, D = c, x = m) : (C = w, D = E, x = y), u.secType === "rect" && u.b && u.h) $ = Dn(u.b, u.h), n.sectionShapes.set(f, {
              type: "rect",
              b: u.b,
              h: u.h
            });
          ...
          n.elasticities.set(f, C), n.shearModuli.set(f, D), n.areas.set(f, $.A), n.momentsOfInertiaZ.set(f, $.Iy), n.momentsOfInertiaY.set(f, $.Iz), n.torsionalConstants.set(f, $.J), n.densities.set(f, x), u && u.releases12 && u.releases12.some((I) => I) && (n.momentReleases || (n.momentReleases = /* @__PURE__ */ new Map()), n.momentReleases.set(f, u.releases12)), u && u.springs12 && u.springs12.some((I) => I > 0) && (n.partialFixitySprings || (n.partialFixitySprings = /* @__PURE__ */ new Map()), n.partialFixitySprings.set(f, u.springs12));
        }
    ...
    window.innerWidth <= 600 && Le.classList.add("collapsed"), setTimeout(() => {
      ...
      (_a2 = Le.querySelector("#cad3d-toggle")) == null ? void 0 : _a2.addEventListener("click", (b) => {
        b.stopPropagation(), Le.classList.add("collapsed");
      }), (_b = Le.querySelector("#cad3d-expand")) == null ? void 0 : _b.addEventListener("click", (b) => {
        b.stopPropagation(), Le.classList.remove("collapsed");
      }), Le.querySelectorAll("[data-ex]").forEach((b) => {
        b.addEventListener("click", (S) => {
          S.stopPropagation();
          const v = b.dataset.ex;
          Wn(v), tt.example(v);
        });
      }), Le.querySelectorAll("[data-view]").forEach((b) => {
        b.addEventListener("click", (S) => {
          S.stopPropagation();
          const v = b.dataset.view;
          Io(v), Le.querySelectorAll("[data-view]").forEach((P) => P.classList.remove("view-active")), b.classList.add("view-active");
        });
      }), (_c = Le.querySelector("#cad3d-btn-clear")) == null ? void 0 : _c.addEventListener("click", (b) => {
        b.stopPropagation(), A = "", ba.val = false, La(), tt.clear();
      }), (_d = Le.querySelector("#cad3d-select")) == null ? void 0 : _d.addEventListener("click", (b) => {
        var _a3;
        b.stopPropagation(), io && (io = false, Fo()), ho && gn(), Qt = !Qt, (_a3 = Le.querySelector("#cad3d-select")) == null ? void 0 : _a3.classList.toggle("inspect-active", Qt);
        const v = lt();
        v && (v.controls.enabled = !Qt), Qt || mn();
      }), (_e2 = Le.querySelector("#cad3d-draw")) == null ? void 0 : _e2.addEventListener("click", (b) => {
        var _a3;
        b.stopPropagation(), io && (io = false, Fo()), Qt && mn(), ho = !ho, (_a3 = Le.querySelector("#cad3d-draw")) == null ? void 0 : _a3.classList.toggle("inspect-active", ho), ho ? _a() : gn();
      }), (_f = Le.querySelector("#cad3d-inspect")) == null ? void 0 : _f.addEventListener("click", (b) => {
        var _a3;
        b.stopPropagation(), Qt && mn(), ho && gn(), io = !io, (_a3 = Le.querySelector("#cad3d-inspect")) == null ? void 0 : _a3.classList.toggle("inspect-active", io), io || Fo();
      }), (_g = Le.querySelector("#cad3d-new-model")) == null ? void 0 : _g.addEventListener("click", (b) => {
        b.stopPropagation(), tt.clear(), Xe = null;
      });
      ...
      function o(b) {
        ...
        const P = 15e3 * Math.sqrt(210) * 10, K = 0.2, U = P / (2 * (1 + K)), ae = 0.09, Q = 0.3 ** 4 / 12, X = 0.141 * 0.3 ** 4, ce = 0.25 * 0.4, re = 0.25 * 0.4 ** 3 / 12, Me = 0.4 * 0.25 ** 3 / 12, Te = 1e-3, N = 5 / 6 * ae, ie = 5 / 6 * ce, te = [];
        function de(ee, ue, Ee) {
          ...
          for (const Se of ue) oe.elasticities.set(Se, P), oe.shearModuli.set(Se, U), oe.areas.set(Se, ae), oe.momentsOfInertiaZ.set(Se, Q), oe.momentsOfInertiaY.set(Se, Q), oe.torsionalConstants.set(Se, X), oe.shearAreasY.set(Se, N), oe.shearAreasZ.set(Se, N);
          for (const Se of Ee) oe.elasticities.set(Se, P), oe.shearModuli.set(Se, U), oe.areas.set(Se, ce), oe.momentsOfInertiaZ.set(Se, Me), oe.momentsOfInertiaY.set(Se, re), oe.torsionalConstants.set(Se, Te), oe.shearAreasY.set(Se, ie), oe.shearAreasZ.set(Se, ie);
          return oe;
        }
        if (b === "test-cantilever" || b === "test-all") {
          const Ee = 270 / (3 * P * Q), oe = [
            [
              0,
              0,
              0
          ...
          He.elasticities.set(0, P), He.shearModuli.set(0, U), He.areas.set(0, ae), He.momentsOfInertiaZ.set(0, Q), He.momentsOfInertiaY.set(0, Q), He.torsionalConstants.set(0, X);
          const at = Ft(oe, Se, {
            supports: /* @__PURE__ */ new Map([
              [
                0,
        ...
        if (b === "test-portal-wall" || b === "test-all") {
          ...
          Ee.elasticities.set(6, P), Ee.shearModuli.set(6, U), Ee.thicknesses = /* @__PURE__ */ new Map([
            [
              6,
              0.2
            ]
        ...
        if (b === "test-wilson-beam" || b === "test-all") {
          ...
          try {
            const to = Ft(rt, wt, {
              supports: Et,
              loads: Ot
            }, Tt), Ut = Math.abs(((_b2 = (_a3 = to.deformations) == null ? void 0 : _a3.get(2)) == null ? void 0 : _b2[1]) ?? 0), ot = Math.abs(((_d2 = (_c2 = to.deformations) == null ? void 0 : _c2.get(3)) == null ? void 0 : _d2[1]) ?? 0), yt = (Ut + ot) / 2, Xt = yt / yo;
            te.push({
        ...
        if (b === "test-scordelis" || b === "test-all") {
          ...
          for (let ot = 0; ot <= rt; ot++) for (let yt = 0; yt <= wt; yt++) {
            const Xt = 25 * ot / rt, Pt = at * yt / wt, Fe = 25 * Math.sin(Pt), Ye = 25 * Math.cos(Pt) - 25 * Math.cos(at);
            Tt.push([
              Xt,
              Fe,
              Ye
          ...
          const Et = [];
          for (let ot = 0; ot < rt; ot++) for (let yt = 0; yt < wt; yt++) {
            const Xt = ot * (wt + 1) + yt, Pt = (ot + 1) * (wt + 1) + yt, Fe = (ot + 1) * (wt + 1) + (yt + 1), Ye = ot * (wt + 1) + (yt + 1);
            Et.push([
              Xt,
              Pt,
              Fe,
          ...
          for (let ot = 0; ot <= rt; ot++) for (let yt = 0; yt <= wt; yt++) {
            ...
            ot === 0 && (Pt[0] = true, Pt[4] = true, Pt[5] = true), ot === rt && (Pt[1] = true, Pt[2] = true, Pt[3] = true), yt === 0 && (Pt[1] = true, Pt[3] = true, Pt[5] = true), Pt.some((Fe) => Fe) && to.set(Xt, Pt);
          }
          const Ut = /* @__PURE__ */ new Map();
          for (const ot of Et) {
            const yt = Tt[ot[0]], Xt = Tt[ot[1]], Pt = Tt[ot[2]], Fe = Tt[ot[3]], Ye = [
              Pt[0] - yt[0],
              Pt[1] - yt[1],
              Pt[2] - yt[2]
            ], Qe = [
      ...
      function n(b) {
        ...
        const K = [
          ...P
        ].sort((N, ie) => N - ie), U = K.map((N, ie) => ie === 0 ? "Base" : `Level_${ie}`), ae = /* @__PURE__ */ new Map();
        K.forEach((N, ie) => ae.set(N, U[ie])), v.push("$ STORIES - IN SEQUENCE FROM TOP");
        ...
        const Q = b.elements.some((N) => N.length === 4);
        ...
        if (b.elements.forEach((N, ie) => {
          ...
          const te = b.nodes[N[0]], de = b.nodes[N[1]], ee = Math.abs(de[1] - te[1]), ue = Math.sqrt((de[0] - te[0]) ** 2 + (de[2] - te[2]) ** 2), Ee = ee > ue * 0.5, oe = re(N[0]), Se = re(N[1]), He = Ee ? "COL30" : "VIGA";
          Ee ? (v.push(`  LINE  "E${ie + 1}"  COLUMN  "${oe.pt}"  "${oe.pt}"  1`), Me.push(`  LINEASSIGN  "E${ie + 1}"  "${Se.story}"  SECTION "${He}"  `)) : (v.push(`  LINE  "E${ie + 1}"  BEAM  "${oe.pt}"  "${Se.pt}"  0`), Me.push(`  LINEASSIGN  "E${ie + 1}"  "${oe.story}"  SECTION "${He}"  `));
        }), v.push(""), Q) {
          v.push("$ AREA CONNECTIVITIES");
          const N = [];
          b.elements.forEach((ie, te) => {
            if (ie.length !== 4) return;
            const de = ie.map((ee) => re(ee));
...
`);
      ...
      function l(b) {
        const S = 15e3 * Math.sqrt(210) * 10, v = [];
        v.push(`"""ETABS API Validation: ${b.name}`), v.push('Generated by Awatif FEM Studio"""'), v.push("import comtypes.client, time, math"), v.push(""), v.push("helper = comtypes.client.CreateObject('ETABSv1.Helper')"), v.push("helper = helper.QueryInterface(comtypes.gen.ETABSv1.cHelper)"), v.push('myETABS = helper.CreateObjectProgID("CSI.ETABS.API.ETABSObject")'), v.push("myETABS.ApplicationStart()"), v.push("time.sleep(10)"), v.push("SapModel = myETABS.SapModel"), v.push("SapModel.InitializeNewModel()"), v.push("SapModel.File.NewBlank()"), v.push("SapModel.SetPresentUnits(12)  # tonf_m_C"), v.push(""), v.push(`E = ${S}`), v.push('SapModel.PropMaterial.SetMaterial("CONC", 2)'), v.push('SapModel.PropMaterial.SetMPIsotropic("CONC", E, 0.2, 5.5e-6)'), v.push('SapModel.PropFrame.SetRectangle("COL30", "CONC", 0.30, 0.30)'), v.push('SapModel.PropFrame.SetRectangle("VIGA", "CONC", 0.40, 0.25)'), b.elements.some((U) => U.length === 4) && v.push('SapModel.PropArea.SetWall("Muro20", 6, False, "CONC", 0.20)'), v.push(""), v.push("# Add elements"), v.push("FN = ' '"), b.elements.forEach((U, ae) => {
          if (U.length === 2) {
            const Q = b.nodes[U[0]], X = b.nodes[U[1]], ce = Math.abs(X[1] - Q[1]), re = Math.sqrt((X[0] - Q[0]) ** 2 + (X[2] - Q[2]) ** 2), Me = ce > re * 0.5 ? "COL30" : "VIGA";
            v.push(`[FN,r]=SapModel.FrameObj.AddByCoord(${Q[0]},${Q[2]},${Q[1]}, ${X[0]},${X[2]},${X[1]}, FN,"${Me}","E${ae + 1}","Global")`);
          } else if (U.length === 4) {
            const Q = U.map((X) => b.nodes[X]);
        ...
        const K = Math.max(...b.nodes.map((U) => U[1]));
        ...
        for (const U of b.results) v.push(`print(f"  ${U.label}: Awatif=${U.awatif.toFixed(4)}, ETABS=${U.reference.toFixed(4)}, Ratio={${U.awatif.toFixed(4)}/${U.reference.toFixed(4)}:.4f}")`);
...
`);
      ...
      function s(b, S) {
        ...
        K.href = P, K.download = S, K.click(), URL.revokeObjectURL(P);
      }
      function d(b) {
        ...
        for (let U = 0; U < b.length; U++) {
          ...
          for (const Q of ae.results) {
            const X = Q.reference !== 0 ? Q.awatif / Q.reference : 1, ce = Math.abs(X - 1) < 0.05;
            ce || (P = false);
            const re = ce ? "#4caf50" : "#f44336", Me = ce ? "PASS" : "FAIL";
        ...
        v += P ? '<div style="color:#4caf50;font-weight:bold;text-align:center;margin-top:8px">ALL TESTS PASSED (< 5% error vs ETABS)</div>' : '<div style="color:#f44336;font-weight:bold;text-align:center;margin-top:8px">Some tests exceeded 5% tolerance</div>', S.innerHTML = v, document.body.appendChild(S), window.__awatifDownloadE2k = (U) => {
      ...
      (_h = Le.querySelector("#cad3d-export")) == null ? void 0 : _h.addEventListener("click", (b) => {
      ...
      function r(b, S) {
        ...
        const v = b.elements.filter((K) => K.length === 2).length, P = b.elements.filter((K) => K.length >= 3).length;
        console.log(`${S} (${b.nodes.length} nodos, ${v} frames, ${P} shells): ${b.nodes.length} nodes, ${b.elements.length} elements`), setTimeout(() => It(), 50);
      }
      function c(b, S) {
        var _a3, _b2, _c2;
        const v = {};
        b.elementInfo.forEach((X) => v[X.category] = (v[X.category] || 0) + 1), (_a3 = document.getElementById("ifc-filter-panel")) == null ? void 0 : _a3.remove();
        ...
        for (const X of U) {
          const ce = v[X] || 0;
          ce !== 0 && (Q += `<label style="display:flex;align-items:center;gap:6px;padding:2px 0;color:#888">
        ...
        </div>`, P.innerHTML = Q, document.body.appendChild(P), P.querySelectorAll("input[data-ifc-cat]").forEach((X) => {
          X.addEventListener("change", () => {
            const ce = X.dataset.ifcCat, re = b.detailCategories.get(ce);
            if (re) {
      ...
      function m(b) {
        B = /* @__PURE__ */ new Set(), G = /* @__PURE__ */ new Set(), $e = /* @__PURE__ */ new Map(), Ae = /* @__PURE__ */ new Map();
        const S = /* @__PURE__ */ new Map();
        for (let re = 0; re < b.stories.length; re++) S.set(b.stories[re].name, re);
        for (let re = 0; re < b.elementTypes.length; re++) {
          const Me = b.elementTypes[re], Te = b.elementStories[re], N = S.get(Te) ?? 0;
          $e.set(re, N), Me === "COLUMN" || Me === "BRACE" ? B.add(re) : G.add(re);
        }
        A = "edificio";
        const v = b.grids.filter((re) => re.dir === "X").sort((re, Me) => re.coord - Me.coord), P = b.grids.filter((re) => re.dir === "Y").sort((re, Me) => re.coord - Me.coord);
        let K, U, ae, Q;
        if (v.length > 0 || P.length > 0) K = v.map((re) => re.coord), U = P.map((re) => re.coord), ae = v.map((re) => re.label), Q = P.map((re) => re.label);
        else {
          const re = new Set(b.nodes.map((Te) => Te[0])), Me = new Set(b.nodes.map((Te) => Te[1]));
          K = [
            ...re
          ].sort((Te, N) => Te - N), U = [
            ...Me
        ...
        const X = b.stories.length > 0 ? Math.max(...b.stories.map((re) => re.elev)) : Math.max(...b.nodes.map((re) => re[2]));
        Ge = K, Ze = U, gt = X, setTimeout(() => {
          It(), en(K, U, X, ae, Q), zn(b.stories, K, U), Yn(), Gn();
        }, 100);
        const ce = {
          COLUMN: 0,
          BEAM: 0,
        ...
        console.log(`E2K grids: X=[${ae.join(",")}] Y=[${Q.join(",")}]`), console.log(`E2K stories: ${b.stories.map((re) => `${re.name}@${re.elev.toFixed(2)}`).join(", ")}`), console.log(`E2K elements: ${ce.COLUMN} columns, ${ce.BEAM} beams, ${ce.BRACE} braces`), nt();
      }
      function w(b, S) {
        const v = new Blob([
          b
        ...
        if (a = i.value, i.value = "", a.startsWith("import")) a === "import-e2k" ? p.accept = ".e2k,.E2K" : a === "import-s2k" ? p.accept = ".s2k,.S2K,.$2k" : a === "import-ifc" ? p.accept = ".ifc,.IFC" : a === "import-py" ? p.accept = ".py" : a === "import-tcl" && (p.accept = ".tcl"), p.click();
          ...
          try {
            a === "export-e2k" ? w(Hl({
              ...
              title: "Awatif Model",
              e2kModel: Xe ?? void 0
            }), "model.e2k") : a === "export-s2k" ? w(Dl({
              ...b,
              title: "Awatif Model"
            }), "model.s2k") : a === "export-py" ? w(Gl(b), "model_opensees.py") : a === "export-tcl" && w(Vl(b), "model_opensees.tcl");
          } catch (S) {
        ...
        if (a === "import-ifc") {
          ...
          v.onload = async () => {
            ...
            try {
              ...
              K.controls.target.copy(ae), K.camera.position.set(ae.x + X, ae.y + X * 0.5, ae.z + X), K.camera.lookAt(ae), K.controls.maxDistance = X * 5, K.controls.update(), K.render(), window.__ifcLoadResult = U, window.__ifcArrayBuffer = P;
              const ce = new FileReader();
              ce.onload = () => {
                const re = ce.result, Me = Ql(re);
      ...
      E && (E.value = g, E.addEventListener("change", (b) => {
        b.stopPropagation(), g = E.value, M = _o(g, F), A && st(A);
      }));
      const y = Le.querySelector("#cad3d-length-unit");
      y && (y.value = F, y.addEventListener("change", (b) => {
        b.stopPropagation(), F = y.value, M = _o(g, F), A && st(A);
      })), Le.querySelectorAll("[data-preset]").forEach((b) => {
        b.addEventListener("click", (S) => {
          S.stopPropagation();
          const v = b.dataset.preset, P = H[v];
          P && (g = P.force, F = P.length, J = P.stress, M = _o(g, F), E && (E.value = g), y && (y.value = F), Le.querySelectorAll("[data-preset]").forEach((K) => {
            K.classList.toggle("active", K.dataset.preset === v);
          }), A && st(A), console.log(`Preset: ${v} \u2192 ${g}+${F}, stress: ${J.label}`));
        });
      }), (_i = Le.querySelector("#cad3d-log")) == null ? void 0 : _i.addEventListener("click", (b) => {
        b.stopPropagation(), Va();
      }), (_j = Le.querySelector("#cad3d-pushover")) == null ? void 0 : _j.addEventListener("click", (b) => {
        b.stopPropagation(), Ja();
      }), (_k = Le.querySelector("#cad3d-nonlinear")) == null ? void 0 : _k.addEventListener("click", (b) => {
        b.stopPropagation(), Xa();
      }), (_l2 = Le.querySelector("#cad3d-fem-solver")) == null ? void 0 : _l2.addEventListener("click", (b) => {
        b.stopPropagation(), Za();
      }), (_m = Le.querySelector("#cad3d-calc")) == null ? void 0 : _m.addEventListener("click", (b) => {
      ...
      }), (_n2 = Le.querySelector("#cad3d-modal")) == null ? void 0 : _n2.addEventListener("click", (b) => {
        var _a3, _b2;
        b.stopPropagation(), _e = !_e, (_a3 = Le.querySelector("#cad3d-modal")) == null ? void 0 : _a3.classList.toggle("active", _e);
        ...
        if (_e) {
          const ae = lt();
          ((_b2 = ae == null ? void 0 : ae.settings) == null ? void 0 : _b2.loads) && (Be = ae.settings.loads.rawVal, ae.settings.loads.val = false), qn(), v.style.display = "", P.style.display = "", K.style.display = "", U && (U.style.display = ""), f();
        } else _n(), v.style.display = "none", P.style.display = "none", K.style.display = "none", U && (U.style.display = "none"), A && A !== "placa-q4" && A !== "placa-3q" && Oe(), setTimeout(() => {
          var _a4;
          const ae = lt();
      ...
      function f() {
        var _a3;
        const b = Le.querySelector("#cad3d-mode-label");
      ...
      (_o2 = Le.querySelector("#cad3d-mode-prev")) == null ? void 0 : _o2.addEventListener("click", (b) => {
        ...
        for (let K = 0; K < ke.length; K++) {
          ...
          P = Math.max(P, Math.sqrt(U * U + ae * ae + Q * Q));
        }
        Ve = P > 1e-12 ? v * 0.05 / P : 1, nn(), f();
      }), (_p = Le.querySelector("#cad3d-mode-next")) == null ? void 0 : _p.addEventListener("click", (b) => {
        if (b.stopPropagation(), !(Pe == null ? void 0 : Pe.modeShapes)) return;
      ...
      const T = Le.querySelector("#cad3d-cli-toggle"), k = Le.querySelector("#cad3d-cli-panel"), $ = Le.querySelector("#cad3d-cli-output"), C = Le.querySelector("#cad3d-cmd"), D = [];
      let x = -1;
...
`));
        ...
        if ((b.key === "Delete" || b.key === "Backspace") && Mt.size > 0) {
          b.preventDefault(), Mt.forEach((S) => Z.add(S)), Mt.clear(), xo && (xo.remove(), xo = null), Oe();
          return;
        }
        if (b.key === "Escape") {
          if (ho) if (kt !== null) {
          ...
          Qt && mn(), io && (io = false, Fo(), (_a3 = Le.querySelector("#cad3d-inspect")) == null ? void 0 : _a3.classList.remove("inspect-active"));
        }
      }), C == null ? void 0 : C.addEventListener("keydown", (b) => {
        if (b.stopPropagation(), b.key === "Enter") {
          const S = C.value.trim();
          if (S) {
            D.unshift(S), x = -1, $ && ($.textContent += `> ${S}
`);
            try {
...
`);
        ...
        } else b.key === "ArrowUp" ? (b.preventDefault(), D.length > 0 && x < D.length - 1 && (x++, C.value = D[x])) : b.key === "ArrowDown" && (b.preventDefault(), x > 0 ? (x--, C.value = D[x]) : (x = -1, C.value = ""));
      });
      let u = false, I = 0, z = 0, R = 0, j = 0;
      Le.addEventListener("mousedown", (b) => {
        const S = b.target.tagName;
        if (S === "BUTTON" || S === "INPUT" || S === "SELECT") return;
        u = true;
        const v = Le.getBoundingClientRect();
        Le.style.bottom = "unset", I = b.clientX, z = b.clientY, R = v.left, j = v.top, b.preventDefault();
      }), window.addEventListener("mousemove", (b) => {
        u && (b.preventDefault(), Le.style.left = R + (b.clientX - I) + "px", Le.style.top = j + (b.clientY - z) + "px");
    ...
    function It(t = false) {
      ...
      a.rotation.x = Math.PI / 2, a.position.set(0.5 * l, 0.5 * l, 0), o.scene.add(a), o.scene.children.filter((m) => m.type === "Group" && m.name !== "gridAxes" && m.name !== "loadsGroup" && (m.name === "viewerAxes" || m.children.some((w) => w instanceof hn))).forEach((m) => {
        m.traverse((w) => {
    ...
    function Yn() {
      ...
      const o = (d, a, i) => {
        ...
        return p.textContent = d, p.dataset.view = a, p.title = i, p.style.cssText = "min-width:22px;padding:1px 5px;font-weight:bold", p.addEventListener("click", (r) => {
          ...
          Le.querySelectorAll("[data-view]").forEach((m) => m.classList.remove("view-active")), c ? (Io("3d"), (_a2 = Le.querySelector('[data-view="3d"]')) == null ? void 0 : _a2.classList.add("view-active")) : (Io(a), p.classList.add("view-active"));
        }), p;
    ...
    function Gn() {
      ...
      const n = (s, d, a) => {
        ...
        return i.textContent = s, i.dataset.view = d, i.title = a, i.style.cssText = "min-width:22px;padding:1px 5px;font-weight:bold", i.addEventListener("click", (p) => {
          ...
          Le.querySelectorAll("[data-view]").forEach((c) => c.classList.remove("view-active")), r ? (Io("3d"), (_a3 = Le.querySelector('[data-view="3d"]')) == null ? void 0 : _a3.classList.add("view-active")) : (Io(d), i.classList.add("view-active"));
        }), i;
    ...
    function Ra() {
      Io("3d"), Le.querySelectorAll("[data-view]").forEach((t) => t.classList.toggle("view-active", t.dataset.view === "3d"));
    }
    tt.view = (t) => {
      t = {
        planta: "plan",
        ...
        elevationY: "elevY",
        corte: "section"
      }[t] || t, Io(t), Le.querySelectorAll("[data-view]").forEach((n) => n.classList.toggle("view-active", n.dataset.view === t));
    };
...
${r.join(", ")}`);
        ...
        Mt.forEach((n) => V.add(n)), Mt.clear(), To(), Ao(), Oe();
      }), o.querySelector("#sel-isolate").addEventListener("click", () => {
        _ = true, Y.clear(), Mt.forEach((n) => Y.add(n)), Mt.clear(), To(), Ao(), Oe();
      }), o.querySelector("#sel-showall").addEventListener("click", () => {
        V.clear(), _ = false, Y.clear(), Ao(), Oe();
      }), o.querySelector("#sel-delete").addEventListener("click", () => {
        Yo(), Mt.forEach((n) => Z.add(n)), Mt.clear(), To(), Ao(), Oe();
    ...
    function mn() {
      var _a2;
      Qt = false, Mt.clear(), To(), xo && (xo.remove(), xo = null), (_a2 = Le.querySelector("#cad3d-select")) == null ? void 0 : _a2.classList.remove("inspect-active");
      const o = lt();
    ...
    function gn() {
      var _a2;
      const t = lt();
      jt && t && (t.scene.remove(jt), jt.geometry.dispose(), jt.material.dispose(), jt = null), Wt && t && (t.scene.remove(Wt), Wt.geometry.dispose(), Wt.material.dispose(), Wt = null), Xn(), kt = null, ao = null, ho = false, jo && (jo.remove(), jo = null), (_a2 = Le.querySelector("#cad3d-draw")) == null ? void 0 : _a2.classList.remove("inspect-active"), t == null ? void 0 : t.render();
    }
    function _a() {
      jo && jo.remove();
    ...
    `, document.body.appendChild(t), jo = t;
      ...
      t.querySelector("#dt-line").addEventListener("click", () => {
        ...
        Ct.track = !Ct.track, Ct.track || Xn(), l();
    ...
    function Ws(t) {
      ...
      if (jt && (o.scene.remove(jt), jt.geometry.dispose(), jt.material.dispose(), jt = null), kt !== null && t.worldPos) {
        ...
        jt = new No(s, d), Jt === "arc" && ao !== null && (jt = new Uo(s, d)), jt.renderOrder = 9999, o.scene.add(jt);
      }
      o.render();
    }
    function Ys(t, o, n, l) {
    ...
    function Ba(t) {
      ...
      if (Jt === "arc") {
        ...
        if (o === kt || o === ao) return;
        const n = e.nodes.val, l = new Ne(...n[kt]), s = new Ne(...n[ao]), d = new Ne(...n[o]), a = Math.max(4, Math.round(((_a2 = W.nSubViga) == null ? void 0 : _a2.val) ?? 8)), i = Ys(l, s, d, a);
        Yo();
        const p = [
          ...e.nodes.val
        ], r = [
    ...
    function Gs(t) {
      ...
      const d = new ro();
      d.setAttribute("position", new Oo(s, 3)), eo = new Uo(d, new Xo({
        color: 16776960,
        linewidth: 3,
        depthTest: false
    ...
    function Zn(t) {
      ...
      for (let m = 0; m < a.length; m++) {
        const w = a[m];
        if (w.length === 2) {
          const E = new Ne(...d[w[0]]), y = new Ne(...d[w[1]]), f = new fl(E, y), h = new Ne(), T = new Ne();
    ...
    function Da(t, o, n, l, s, d, a) {
      ...
      <div style="text-align:left;margin-bottom:6px;color:var(--fem-eq-sub)">
        ${L("A", "s")} = ${De("5", "6")} \xB7 ${L("A")} = <span class="highlight">${we(i)}</span>
        &nbsp;&nbsp; \u03C6<sub>z</sub> = ${De("12\xB7" + L("E") + "\xB7" + L("I", "z"), L("G") + "\xB7" + L("A", "s") + "\xB7" + L("L") + "\xB2")} = <span class="highlight">${we(r)}</span>
      ...
      <div>${L("t", "z")} = ${De("12\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB3\xB7(1+\u03C6<sub>z</sub>)")} = <span class="highlight">${we(E)}</span> &nbsp;(cortante)</div>
      <div>${L("b", "z")} = ${De("6\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB2\xB7(1+\u03C6<sub>z</sub>)")} = <span class="highlight">${we(y)}</span> &nbsp;(acoplamiento)</div>
      <div>${L("k", "z")} = ${De("4\xB7" + L("E") + "\xB7" + L("I", "z") + "\xB7(1+\u03C6/4)", L("L") + "\xB7(1+\u03C6<sub>z</sub>)")} = <span class="highlight">${we(f)}</span> &nbsp;(flexi\xF3n diagonal)</div>
      <div>${L("a", "z")} = ${De("2\xB7" + L("E") + "\xB7" + L("I", "z") + "\xB7(1\u2212\u03C6/2)", L("L") + "\xB7(1+\u03C6<sub>z</sub>)")} = <span class="highlight">${we(h)}</span> &nbsp;(flexi\xF3n off-diag)</div>
      ` : `
      <div style="text-align:left;margin-bottom:4px"><strong style="color:var(--fem-section-title)">Coeficientes de rigidez:</strong></div>
      `}
      <div>${De(L("E") + "\xB7" + L("A"), L("L"))} = <span class="highlight">${we(m)}</span> &nbsp;(axial)</div>
      <div>${De(L("G") + "\xB7" + L("J"), L("L"))} = <span class="highlight">${we(w)}</span> &nbsp;(torsi\xF3n)</div>
      ${T ? "" : `
      <div>${De("12\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB3")} = <span class="highlight">${we(E)}</span></div>
      <div>${De("4\xB7" + L("E") + "\xB7" + L("I", "z"), L("L"))} = <span class="highlight">${we(f)}</span></div>
      `}
    </div>
    <div class="fem-eq">
    ...
    function Ha(t) {
      if (t.length === 2) {
        ...
        <div>${L("l")} = cos(\u03B1) = ${De("\u0394x", L("L"))} = ${De(we(n[0]), we(l))} = <span class="highlight">${we(s)}</span></div>
        <div>${L("m")} = cos(\u03B2) = ${De("\u0394y", L("L"))} = ${De(we(n[1]), we(l))} = <span class="highlight">${we(d)}</span></div>
        <div>${L("n")} = cos(\u03B3) = ${De("\u0394z", L("L"))} = ${De(we(n[2]), we(l))} = <span class="highlight">${we(a)}</span></div>
      </div>
      <div class="fem-eq">
    ...
    function Vs() {
      const t = "0", o = De(L("EA"), L("L")), n = De("\u2212" + L("EA"), L("L")), l = De("12" + L("EI", "z"), L("L") + "\xB3"), s = De("\u221212" + L("EI", "z"), L("L") + "\xB3"), d = De("12" + L("EI", "y"), L("L") + "\xB3"), a = De("\u221212" + L("EI", "y"), L("L") + "\xB3"), i = De("6" + L("EI", "z"), L("L") + "\xB2"), p = De("\u22126" + L("EI", "z"), L("L") + "\xB2"), r = De("6" + L("EI", "y"), L("L") + "\xB2"), c = De("\u22126" + L("EI", "y"), L("L") + "\xB2"), m = De(L("GJ"), L("L")), w = De("\u2212" + L("GJ"), L("L")), E = De("4" + L("EI", "z"), L("L")), y = De("2" + L("EI", "z"), L("L")), f = De("4" + L("EI", "y"), L("L")), h = De("2" + L("EI", "y"), L("L")), T = '<span style="color:var(--fem-eq-dots);font-style:italic">sym</span>', k = [
        "P\u2081",
        "P\u2082",
        "P\u2083",
        "P\u2084",
      ...
      for (const x of $) D += `<td class="hdr">${x}</td>`;
      D += "</tr>";
      for (let x = 0; x < 12; x++) {
        D += `<tr><td class="hdr">${k[x]}</td>`;
        for (let u = 0; u < 12; u++) if (u < x) D += `<td style="color:var(--fem-border-cell)">${u === 0 && x > 0 ? T : ""}</td>`;
        else {
          const I = C[x][u], z = (x === u ? "diag " : "") + (I !== "0" ? "nz" : "");
          D += `<td class="${z}">${I}</td>`;
        }
        D += "</tr>";
      }
      ...
      return `<div class="coeff-grid">${[
        {
          ...
          calc: `${De(we(t) + "\xD7" + we(o), we(a))}`,
          val: t * o / a,
          label: "Axial"
        },
        {
          name: `${De("12\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB3")}`,
          calc: `${De("12\xD7" + we(t) + "\xD7" + we(n), we(a) + "\xB3")}`,
          val: 12 * t * n / a ** 3,
          label: "Corte Y"
        },
        {
          name: `${De("6\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB2")}`,
          calc: `${De("6\xD7" + we(t) + "\xD7" + we(n), we(a) + "\xB2")}`,
          val: 6 * t * n / a ** 2,
        ...
        },
        {
          ...
          calc: `${De("4\xD7" + we(t) + "\xD7" + we(n), we(a))}`,
          val: 4 * t * n / a,
          label: "Flexi\xF3n Z (4EI/L)"
        },
        {
          name: `${De("2\xB7" + L("E") + "\xB7" + L("I", "z"), L("L"))}`,
          calc: `${De("2\xD7" + we(t) + "\xD7" + we(n), we(a))}`,
          val: 2 * t * n / a,
    ...
    function Js(t) {
      ...
      if (d) {
        ...
        let K = "";
        for (let U = 0; U < 6; U++) {
          const ae = U, Q = U + 6, X = (S.length >= 12 ? S[ae] : U >= 3 && S.length >= 6 && S[U - 3]) ? "checked" : "", ce = (S.length >= 12 ? S[Q] : U >= 3 && S.length >= 6 && S[U]) ? "checked" : "", re = v.length >= 12 && v[ae] > 0 ? v[ae].toFixed(1) : "", Me = v.length >= 12 && v[Q] > 0 ? v[Q].toFixed(1) : "";
          K += `<tr>
          <td style="text-align:left;color:var(--fem-key)">${P[U]}</td>
          <td style="text-align:center"><input type="checkbox" data-rel="${ae}" ${X}></td>
          <td style="text-align:center"><input type="checkbox" data-rel="${Q}" ${ce}></td>
        ...
        `${l[0]}${l[1]}${we(x)}${we(u)}${we(I)}${we(z)}${we(R)}${we(j)}${we(b)}${K}`;
      } else {
        const x = ((_l2 = a.elasticities) == null ? void 0 : _l2.get(t)) ?? 0, u = ((_m = a.thicknesses) == null ? void 0 : _m.get(t)) ?? 0, I = ((_n2 = a.poissonsRatios) == null ? void 0 : _n2.get(t)) ?? 0, z = x / (2 * (1 + I)), R = l.length === 4, j = x / (1 - I * I);
        `${l.length}${l.join(", ")}${we(x)}${we(z)}${we(u)}${we(I)}`, R && (w = `<div class="fem-eq eq-box">
          ...
          </span> = ${De(we(x), "1\u2212" + we(I) + "\xB2")} = <span class="highlight">${we(j)}</span></div>
          ...
          <div>${L("D", "b")} = ${De(L("E") + "\xB7" + L("t") + "\xB3", "12\xB7(1\u2212\u03BD\xB2)")} = <span class="highlight">${we(x * u ** 3 / (12 * (1 - I ** 2)))}</span></div>
          <div>${L("D", "s")} = \u03BA\xB7${L("G")}\xB7${L("t")} = <span class="highlight">${we(5 / 6 * z * u)}</span> &nbsp; <sub style="color:var(--fem-label)">\u03BA = 5/6</sub></div>
      ...
      try {
        if (T = In(s, a, t), k = kn(s), $ = po(us(k), po(T, k)), C = d ? [
          ...
          const z = Bo(wo(s[1], s[0])), R = ((_o2 = a.elasticities) == null ? void 0 : _o2.get(t)) ?? 0, j = ((_p = a.areas) == null ? void 0 : _p.get(t)) ?? 0, b = ((_q = a.momentsOfInertiaZ) == null ? void 0 : _q.get(t)) ?? 0, S = ((_r = a.momentsOfInertiaY) == null ? void 0 : _r.get(t)) ?? 0, v = ((_s2 = a.shearModuli) == null ? void 0 : _s2.get(t)) ?? 0, P = ((_t2 = a.torsionalConstants) == null ? void 0 : _t2.get(t)) ?? 0;
          w = Da(R, j, b, S, v, P, z);
        }
        E = Ha(s), y = ja(), f = Wa(l), h = Ya(d);
      ...
      if (i == null ? void 0 : i.deformations) {
        ...
        l.map((u, I) => {
          var _a3;
          const z = ((_a3 = i.deformations) == null ? void 0 : _a3.get(u)) || [
            0,
            0,
            0,
            0,
            0,
            0
          ], R = x.map((j, b) => `<span class="prop-key">${j}</span>: <span class="${Math.abs(z[b]) > 1e-10 ? "result-val" : ""}">${we(z[b])}</span>`).join(" &nbsp;");
          return `<div style="margin-bottom:2px"><strong>Nodo ${u}:</strong> ${R}</div>`;
        }).join("");
      }
      if (p && d && (i == null ? void 0 : i.deformations) && T && k) {
        const x = (_u = p.normals) == null ? void 0 : _u.get(t), u = (_v = p.shearsY) == null ? void 0 : _v.get(t), I = (_w = p.shearsZ) == null ? void 0 : _w.get(t), z = (_x = p.torsions) == null ? void 0 : _x.get(t), R = (_y = p.bendingsY) == null ? void 0 : _y.get(t), j = (_z = p.bendingsZ) == null ? void 0 : _z.get(t), b = [
          "ux",
          "uy",
          "uz",
          "\u03B8x",
        ...
        const K = (Q, X) => Q.map((ce, re) => `<span style="color:${Math.abs(ce) > 1e-10 ? "var(--fem-nonzero)" : "var(--fem-eq-dots)"}">${X[re % 6]}=${we(ce)}</span>`).join(", "), ae = [
          "N",
          "Vy",
          "Vz",
          "Mx",
        ...
        `${L("u", "global")}${l.map((Q, X) => `<span style="color:var(--fem-label)">nodo ${Q}:</span> ${b.map((ce, re) => `<span style="color:${Math.abs(S[X * 6 + re]) > 1e-10 ? "var(--fem-eq-var)" : "var(--fem-eq-dots)"}">${we(S[X * 6 + re])}</span>`).join(", ")}`).join(" | ")}${L("u", "local")}${L("T")}${L("u", "global")}${L("u", "local")}${K(v, [
          ...b,
          ...b
        ])}${L("f", "local")}${L("k", "local")}${L("u", "local")}${L("f", "local")}${P.map((Q, X) => `<span style="color:${Math.abs(Q) > 1e-10 ? "var(--fem-nonzero)" : "var(--fem-eq-dots)"}">${ae[X]}=${we(Q)}</span>`).join(", ")}${L("P", "1")}${L("N", "i")}${we(P[0])}${L("P", "7")}${L("N", "j")}${we(P[6])}${L("P", "2")}${L("V", "y,i")}${we(P[1])}${L("P", "8")}${L("V", "y,j")}${we(P[7])}${L("P", "3")}${L("V", "z,i")}${we(P[2])}${L("P", "9")}${L("V", "z,j")}${we(P[8])}${L("P", "4")}${L("M", "x,i")}${we(P[3])}${L("P", "10")}${L("M", "x,j")}${we(P[9])}${L("P", "5")}${L("M", "y,i")}${we(P[4])}${L("P", "11")}${L("M", "y,j")}${we(P[10])}${L("P", "6")}${L("M", "z,i")}${we(P[5])}${L("P", "12")}${L("M", "z,j")}${we(P[11])}${x ? x.map((Q) => we(Q)).join(", ") : "\u2014"}${u ? u.map((Q) => we(Q)).join(", ") : "\u2014"}${I ? I.map((Q) => we(Q)).join(", ") : "\u2014"}${z ? z.map((Q) => we(Q)).join(", ") : "\u2014"}${R ? R.map((Q) => we(Q)).join(", ") : "\u2014"}${j ? j.map((Q) => we(Q)).join(", ") : "\u2014"}`;
      } else if (p && d) {
        const x = (_B = p.normals) == null ? void 0 : _B.get(t), u = (_C = p.shearsY) == null ? void 0 : _C.get(t), I = (_D = p.shearsZ) == null ? void 0 : _D.get(t), z = (_E = p.torsions) == null ? void 0 : _E.get(t), R = (_F = p.bendingsY) == null ? void 0 : _F.get(t), j = (_G = p.bendingsZ) == null ? void 0 : _G.get(t);
        `${x ? x.map((b) => we(b)).join(", ") : "\u2014"}${u ? u.map((b) => we(b)).join(", ") : "\u2014"}${I ? I.map((b) => we(b)).join(", ") : "\u2014"}${z ? z.map((b) => we(b)).join(", ") : "\u2014"}${R ? R.map((b) => we(b)).join(", ") : "\u2014"}${j ? j.map((b) => we(b)).join(", ") : "\u2014"}`;
      } else if (p && !d) {
        const x = (_H = p.bendingXX) == null ? void 0 : _H.get(t), u = (_I = p.bendingYY) == null ? void 0 : _I.get(t), I = (_J = p.bendingXY) == null ? void 0 : _J.get(t), z = (_K = p.membraneXX) == null ? void 0 : _K.get(t), R = (_L = p.membraneYY) == null ? void 0 : _L.get(t), j = (_M = p.membraneXY) == null ? void 0 : _M.get(t);
        `${x ? x.map((b) => we(b)).join(", ") : "\u2014"}${u ? u.map((b) => we(b)).join(", ") : "\u2014"}${I ? I.map((b) => we(b)).join(", ") : "\u2014"}${z ? z.map((b) => we(b)).join(", ") : "\u2014"}${R ? R.map((b) => we(b)).join(", ") : "\u2014"}${j ? j.map((b) => we(b)).join(", ") : "\u2014"}`;
      }
      `${l[0]}`, 6 * l[0], 6 * l[0] + 5, `${l[1]}`, 6 * l[1], 6 * l[1] + 5, l.length === 3 && (`${l[2]}`, 6 * l[2], 6 * l[2] + 5), o.length * 6, o.length * 6, so = Il(t, o, n, a, i, p), so.id = "fem-inspect-panel", document.body.appendChild(so), (_N = so.querySelector("#er-close")) == null ? void 0 : _N.addEventListener("click", () => Fo()), (_O = so.querySelector("#rel-apply")) == null ? void 0 : _O.addEventListener("click", () => {
        const x = so.querySelectorAll("input[data-rel]"), u = so.querySelectorAll("input[data-spr]"), I = new Array(12).fill(false), z = new Array(12).fill(0);
        x.forEach((j) => {
          I[parseInt(j.dataset.rel)] = j.checked;
        }), u.forEach((j) => {
          const b = parseFloat(j.value);
          b > 0 && (z[parseInt(j.dataset.spr)] = b);
        }), a.momentReleases || (a.momentReleases = /* @__PURE__ */ new Map()), a.partialFixitySprings || (a.partialFixitySprings = /* @__PURE__ */ new Map()), I.some((j) => j) ? a.momentReleases.set(t, I) : a.momentReleases.delete(t), z.some((j) => j > 0) ? a.partialFixitySprings.set(t, z) : a.partialFixitySprings.delete(t), console.log(`Releases elem ${t}:`, I.map((j, b) => j ? relIds[b] : "").filter(Boolean).join(" ") || "none"), console.log(`Springs elem ${t}:`, z);
      ...
      const D = d ? (() => {
        var _a3, _b2, _c2, _d2, _e3, _f2;
        const x = Bo(wo(s[1], s[0])), u = ((_a3 = a.elasticities) == null ? void 0 : _a3.get(t)) ?? 0, I = ((_b2 = a.areas) == null ? void 0 : _b2.get(t)) ?? 0, z = ((_c2 = a.momentsOfInertiaZ) == null ? void 0 : _c2.get(t)) ?? 0, R = ((_d2 = a.momentsOfInertiaY) == null ? void 0 : _d2.get(t)) ?? 0, j = ((_e3 = a.shearModuli) == null ? void 0 : _e3.get(t)) ?? 0, b = ((_f2 = a.torsionalConstants) == null ? void 0 : _f2.get(t)) ?? 0;
        return Ga(u, I, z, R, j, b, x);
      })() : void 0;
      so.querySelectorAll("[data-full]").forEach((x) => {
        x.addEventListener("click", (u) => {
          ...
          if (I === "kLocal" && T) {
            const z = d ? Vs() : "<em>Shell 18\xD718 \u2014 ver tabla num\xE9rica</em>";
            ts(`Elemento ${t} \u2014 Rigidez Local k_local`, z, es(T, C), D);
          } else if (I === "T" && k) ts(`Elemento ${t} \u2014 Transformaci\xF3n T`, E, es(k, C));
          else if (I === "kGlobal" && $) {
            const z = d ? Vs() : "<em>Shell 18\xD718</em>";
            ts(`Elemento ${t} \u2014 Rigidez Global K = T^T \xB7 k \xB7 T`, z, es($, C), D);
          }
        });
      });
    }
    ...
    function Zs() {
      ...
      for (let h = 0; h <= 12; h++) {
        ...
        for (let $ = 0; $ < 6; $++) {
          const C = k + 2 * Math.PI * $ / 6, D = 5 * Math.cos(C), x = 5 * Math.sin(C);
          d.push([
            D,
            x,
            T
    ...
    function Qs() {
      ...
      for (let f = 0; f <= 20; f++) {
        ...
        for (let C = 0; C < 3; C++) {
          const D = C * 2 * Math.PI / 3 - Math.PI / 2, x = k * Math.cos(D), u = k * Math.sin(D), I = s.length;
          s.push([
            x,
            u,
            T
    ...
    function ea() {
      ...
      for (let m = 0; m < 3; m++) {
        ...
        for (let k = 0; k <= 4; k++) {
          const $ = k / 4, C = -f / 2 + f * $, D = y * (1 - $ * $ * 0.3);
          for (let x = 0; x <= 12; x++) {
            const u = x / 12, I = w + D * u, z = E * Math.sin(Math.PI * u) * (1 - $ * $ * 0.5), R = C;
            t.push([
              I,
              R,
              z
        ...
        for (let k = 0; k < 4; k++) for (let $ = 0; $ < 12; $++) {
          const C = h + k * T + $, D = h + k * T + $ + 1, x = h + (k + 1) * T + $ + 1, u = h + (k + 1) * T + $;
          o.push([
            C,
            D,
            x,
    ...
    function os() {
      ...
      try {
        ...
        if ($ && e.deformOutputs) {
          ...
          const D = s * f + Math.floor(l / 2), x = $.deformations.get(D), u = x ? x[0] : 0;
    ...
    function oa() {
      ...
      try {
        ...
        if (k && e.deformOutputs) {
          ...
          const $ = $o(m, w, T, k);
          e.analyzeOutputs && (e.analyzeOutputs.val = $);
          const C = k.deformations.get(h), D = C ? C[2] : 0, x = n * o * o * o / 12, u = i * t * t * t / (3 * d * x);
          console.log(`Viga Q4: Uz_tip=${D.toExponential(4)} | Analitico=${u.toExponential(4)} | ratio=${(Math.abs(D) / u).toFixed(4)}`);
        }
      } catch (k) {
    ...
    function na() {
      ...
      try {
        ...
        if ($ && e.deformOutputs) {
          ...
          const C = $o(m, w, k, $);
          e.analyzeOutputs && (e.analyzeOutputs.val = C);
          const D = (s / 2 | 0) * f + l, x = $.deformations.get(D), u = x ? x[1] : 0;
    ...
    function sa() {
      ...
      const t = M, o = ((_a2 = W.Lx) == null ? void 0 : _a2.val) ?? 5.5, n = ((_b = W.Ly) == null ? void 0 : _b.val) ?? 8, l = ((_c = W.H1) == null ? void 0 : _c.val) ?? 3, s = ((_d = W.H2) == null ? void 0 : _d.val) ?? 4, d = Math.round(((_e2 = W.nCol) == null ? void 0 : _e2.val) ?? 4), a = Math.round(((_f = W.nCorr) == null ? void 0 : _f.val) ?? 8), i = ((_g = W.E) == null ? void 0 : _g.val) ?? t.E, p = ((_h = W.t) == null ? void 0 : _h.val) ?? 5e-4, r = ((_i = W.q) == null ? void 0 : _i.val) ?? 1, c = (((_j = W.supUx) == null ? void 0 : _j.val) ?? 1) >= 0.5, m = (((_k = W.supUy) == null ? void 0 : _k.val) ?? 1) >= 0.5, w = (((_l2 = W.supUz) == null ? void 0 : _l2.val) ?? 1) >= 0.5, E = (((_m = W.supRx) == null ? void 0 : _m.val) ?? 1) >= 0.5, y = (((_n2 = W.supRy) == null ? void 0 : _n2.val) ?? 1) >= 0.5, f = (((_o2 = W.supRz) == null ? void 0 : _o2.val) ?? 1) >= 0.5, h = ((_p = W.colD) == null ? void 0 : _p.val) ?? 0.16, T = ((_q = W.colBf) == null ? void 0 : _q.val) ?? 0.16, k = ((_r = W.colTf) == null ? void 0 : _r.val) ?? 0.013, $ = ((_s2 = W.colTw) == null ? void 0 : _s2.val) ?? 8e-3, C = ((_t2 = W.vigD) == null ? void 0 : _t2.val) ?? 0.2, D = ((_u = W.vigBf) == null ? void 0 : _u.val) ?? 0.1, x = ((_v = W.vigTf) == null ? void 0 : _v.val) ?? 85e-4, u = ((_w = W.vigTw) == null ? void 0 : _w.val) ?? 56e-4, I = ((_x = W.corrB) == null ? void 0 : _x.val) ?? 0.06, z = ((_y = W.corrT) == null ? void 0 : _y.val) ?? 4e-3, R = 0.3, j = i / (2 * (1 + R));
      ...
      const S = b(h, T, k, $), v = b(C, D, x, u), P = I * I - (I - 2 * z) * (I - 2 * z), K = (I ** 4 - (I - 2 * z) ** 4) / 12, U = K, ae = 2 * z * (I - z) ** 2 * (I - z) ** 2 / (2 * (I - z) + 2 * (I - z)), Q = 3, X = [
        0,
        o / 2,
        o
      ], ce = [];
      ...
      const Me = Array.from(re).sort((Fe, Ye) => Fe - Ye), Te = Me.length;
      function N(Fe) {
        return l + (s - l) * Fe / n;
      }
      const ie = [], te = [], de = [], ee = [];
      ...
      for (let Fe = 0; Fe < Q; Fe++) for (let Ye = 0; Ye < d; Ye++) {
        const Qe = Me.indexOf(ce[Ye]);
        if (Qe < 0) continue;
        const et = te.length;
        te.push([
          de[Fe][Ye],
      ...
      for (let Fe = 0; Fe < Q; Fe++) for (let Ye = 0; Ye < Te - 1; Ye++) {
        const Qe = te.length;
        te.push([
          ee[Fe][Ye],
          ee[Fe][Ye + 1]
      ...
      te.length;
      for (let Fe = 0; Fe < Te; Fe++) for (let Ye = 0; Ye < Q - 1; Ye++) {
        const Qe = te.length;
        te.push([
          ee[Ye][Fe],
          ee[Ye + 1][Fe]
        ]), ue.set(Qe, i), Ee.set(Qe, j), oe.set(Qe, P), Se.set(Qe, U), He.set(Qe, K), at.set(Qe, ae), Tt.set(Qe, Et), rt.set(Qe, {
          type: "rect",
          b: I,
          h: I,
          name: "Corr"
      ...
      for (let Fe = 0; Fe < Q - 1; Fe++) for (let Ye = 0; Ye < Te - 1; Ye++) {
        const Qe = te.length;
        te.push([
          ee[Fe][Ye],
          ee[Fe + 1][Ye],
      ...
      for (let Fe = 0; Fe < Q; Fe++) for (let Ye = 0; Ye < d; Ye++) Ot.set(de[Fe][Ye], yo);
      const to = /* @__PURE__ */ new Map();
      for (let Fe = 0; Fe < Q; Fe++) for (let Ye = 0; Ye < Te; Ye++) {
        let Qe;
        Fe === 0 ? Qe = (X[1] - X[0]) / 2 : Fe === Q - 1 ? Qe = (X[Q - 1] - X[Q - 2]) / 2 : Qe = (X[Fe + 1] - X[Fe - 1]) / 2;
        let et;
        Ye === 0 ? et = (Me[1] - Me[0]) / 2 : Ye === Te - 1 ? et = (Me[Te - 1] - Me[Te - 2]) / 2 : et = (Me[Ye + 1] - Me[Ye - 1]) / 2;
        const Yt = -r * Qe * et;
        to.set(ee[Fe][Ye], [
          0,
          0,
      ...
      const Ut = te.filter((Fe) => Fe.length === 2).length, ot = {
        elasticities: ue,
        shearModuli: Ee,
        areas: oe,
      ...
      for (let Fe = 0; Fe < te.length; Fe++) te[Fe].length === 4 && (yt.set(Fe, p), Xt.set(Fe, R));
      ...
      try {
        ...
        if (Ye && e.deformOutputs) {
          ...
          }), console.log(`P\xE9rgola: Uz_max=${Yt.toExponential(4)} m en nodo ${Ro} | ${Ut} frames + ${te.length - Ut} shells | ${Qe.toFixed(0)} ms`);
        }
      } catch (Fe) {
    ...
    `, document.body.appendChild(t), (_b = t.querySelector("#pushover-close")) == null ? void 0 : _b.addEventListener("click", () => t.remove()), (_c = t.querySelector("#pushover-run")) == null ? void 0 : _c.addEventListener("click", async () => {
        ...
        try {
          ...
          w.textContent = `Completado: ${T.nSteps} pasos`, Ua(t.querySelector("#pushover-canvas"), T.displacements, T.forces, `Pushover: ${n * 100}x${l * 100}cm, f'c=${s / 1e3}MPa, Fy=${d / 1e3}MPa`);
        } catch (h) {
    ...
    function Ua(t, o, n, l) {
      ...
      for (let C = 0; C <= 5; C++) {
        const D = c + k * C;
        s.fillText((D * 1e3).toFixed(1), h(D), a - i.bottom + 15);
      }
      s.textAlign = "right";
      const $ = f / 5;
      for (let C = 0; C <= 5; C++) {
        const D = w + $ * C;
        s.fillText(D.toFixed(0), i.left - 5, T(D) + 3);
      }
    }
    let sn = null;
    function Xa() {
    ...
    function Ka(t) {
      ...
      const D = [];
      for (const U of p) {
        let ae = E, Q = y, X = f, ce = h, re = T, Me = k, Te = $, N = C, ie;
        const te = U - m;
        if (Math.abs(te) < 1e-20) {
          D.push(w);
          continue;
        }
        if ((N === 0 || N === 3) && (te < 0 ? (N = 2, ce = -r, re = -o, X = ce, Me = 0, Te = 0) : (N = 1, ce = r, re = o, X = ce, Me = 0, Te = 0)), N === 2 && te > 0) {
          N = 1, Me = m, Te = w, m < ae && (ae = m);
          const Se = (Q - ae) / (2 * 1 * r), He = 1 + 0 * Math.pow(Se, 0.8);
          ce = (o * He - c * r * He - Te + n * Me) / (n - c), re = o * He + c * (ce - r * He), X = Q;
        } else if (N === 1 && te < 0) {
          N = 2, Me = m, Te = w, m > Q && (Q = m);
          const Se = (Q - ae) / (2 * 1 * r), He = 1 + 0 * Math.pow(Se, 0.8);
          ce = (-o * He + c * r * He - Te + n * Me) / (n - c), re = -o * He + c * (ce + r * He), X = ae;
        }
        const de = Math.abs((X - ce) / r);
        let ee = s - 0.925 * de / (0.15 + de);
        ee < 0.1 && (ee = 0.1);
        const ue = (U - Me) / (ce - Me), Ee = 1 + Math.pow(Math.abs(ue), ee), oe = Math.pow(Ee, 1 / ee);
        ie = l * ue + (1 - l) * ue / oe, ie = ie * (re - Te) + Te, D.push(ie), m = U, w = ie, E = ae, y = Q, f = X, h = ce, T = re, k = Me, $ = Te, C = N;
      }
      const x = t.querySelector("#nl-canvas"), u = x.getContext("2d"), I = x.width, z = x.height;
      u.clearRect(0, 0, I, z);
      const R = Math.max(...p.map(Math.abs)), j = Math.max(...D.map(Math.abs)), b = (I - 40) / (2 * R), S = (z - 40) / (2 * j), v = I / 2, P = z / 2;
      u.strokeStyle = "#444", u.lineWidth = 1, u.beginPath(), u.moveTo(20, P), u.lineTo(I - 20, P), u.stroke(), u.beginPath(), u.moveTo(v, 20), u.lineTo(v, z - 20), u.stroke(), u.fillStyle = "#888", u.font = "10px monospace", u.textAlign = "center", u.fillText("\u03B5 (strain)", I - 40, P - 5), u.fillText("\u03C3 (stress)", v + 30, 15), u.fillText(`\xB1${(R * 100).toFixed(1)}%`, I - 30, P + 12), u.fillText(`\xB1${(j / 1e3).toFixed(0)} MPa`, v + 40, 30), u.strokeStyle = "#00ccff", u.lineWidth = 1.5, u.beginPath();
      for (let U = 0; U < p.length; U++) {
        const ae = v + p[U] * b, Q = P - D[U] * S;
    ...
    `, document.body.appendChild(o), Go = o;
      ...
      a.addEventListener("change", p), p(), o.querySelector("#asgn-close").addEventListener("click", () => {
        ...
        t.forEach((r) => ye.delete(r)), o.remove(), Go = null, Eo(), e.elementInputs.val = {
          ...e.elementInputs.val
        };
      });
    }
    ...
    `, document.body.appendChild(E), Vo = E, E.querySelector("#ep-close").addEventListener("click", () => {
        E.remove(), Vo = null, Fo();
      }), E.querySelector("#ep-delete").addEventListener("click", () => {
        Z.add(t), E.remove(), Vo = null, Fo(), Oe();
      }), E.querySelector("#ep-inspect").addEventListener("click", () => {
        E.remove(), Vo = null, Js(t);
      });
    }
    setTimeout(() => {
      const t = document.getElementById("viewer");
      ...
      function a(p, r, c, m, w) {
        ...
        for (let C = 0; C < k.length; C++) {
          const D = k[C], x = D.map((u) => d(T[u])).filter(Boolean);
          if (x.length !== 0) if (w) x.every((I) => I.x >= E && I.x <= y && I.y >= f && I.y <= h) && $.push(C);
            ...
            if (D.length === 2) {
              const I = x[0], z = x[1];
              i(I.x, I.y, z.x, z.y, E, f, y, h) && $.push(C);
            }
          }
        }
        return $;
      ...
      function i(p, r, c, m, w, E, y, f) {
        ...
        const T = (k, $, C, D, x, u, I, z) => {
          const R = (C - k) * (z - u) - (D - $) * (I - x);
          if (Math.abs(R) < 1e-10) return false;
          const j = ((x - k) * (z - u) - (u - $) * (I - x)) / R, b = ((x - k) * (D - $) - (u - $) * (C - k)) / R;
      ...
      o.addEventListener("mousedown", (p) => {
        ...
        if (ho) {
          ...
          if (Ct.track && m.snapType === "node" && m.nodeIdx !== null && m.nodeIdx !== Po && Na(m.nodeIdx), Ct.track && Po !== null && m.worldPos && m.snapType !== "node") {
            const w = qa(m.worldPos, Po);
            w && (m.worldPos = w, m.snapType = "grid");
          }
          if (Po !== null && m.worldPos) {
            const w = e.nodes.val[Po];
            w && Hs(p.clientX, p.clientY, new Ne(...w), m.worldPos);
        ...
        if (Qt && n) {
          ...
          if (Math.abs(r) > s || Math.abs(c) > s) {
            ...
            p.ctrlKey || p.metaKey || (Mt.clear(), To()), w.forEach((y) => {
              Mt.has(y) || (Mt.add(y), Un(y));
            }), Ao();
          }
        ...
        if (Qt) {
          ...
          if (r >= 0) {
            if (c) if (Mt.has(r)) {
              Mt.delete(r);
              const m = ko.findIndex((w) => w.__elemIdx === r);
              if (m >= 0) {
                const w = lt();
                w == null ? void 0 : w.scene.remove(ko[m]), ko[m].geometry.dispose(), ko[m].material.dispose(), ko.splice(m, 1), w == null ? void 0 : w.render();
              }
            } else Mt.add(r), Un(r);
            else Mt.clear(), To(), Mt.add(r), Un(r);
            Ao();
      ...
      if (t === void 0 && (t = !_e), _e = t, (_a2 = Le.querySelector("#cad3d-modal")) == null ? void 0 : _a2.classList.toggle("active", _e), _e) {
        const n = lt();
        ((_b = n == null ? void 0 : n.settings) == null ? void 0 : _b.loads) && (Be = n.settings.loads.rawVal, n.settings.loads.val = false), qn(), Le.querySelector("#cad3d-mode-prev").style.display = "", Le.querySelector("#cad3d-mode-next").style.display = "", Le.querySelector("#cad3d-mode-label").style.display = "";
      } else _n(), Le.querySelector("#cad3d-mode-prev").style.display = "none", Le.querySelector("#cad3d-mode-next").style.display = "none", Le.querySelector("#cad3d-mode-label").style.display = "none", A && A !== "placa-q4" && A !== "placa-3q" && Oe(), setTimeout(() => {
        var _a3;
        const n = lt();
  ...
  `, co.addEventListener("mouseenter", () => {
      ...
      const t = cs() === "es" ? "en" : "es";
      Ll(t), co.textContent = t === "es" ? "EN" : "ES", co.title = t === "es" ? "Switch to English" : "Cambiar a Espa\xF1ol", Pl();
