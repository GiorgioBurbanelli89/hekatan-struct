const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./calcPanel-DKQr_H-y.js","./getMesh-B1dmlgUt.js","./__vite-browser-external-D7Ct-6yo.js","./pureFunctionsAny.generated-JAcEVsJ7.js","./analyze-ClLKGn9k.js","./didacticCpp-ClDOlw7M.js","./cyclicPushoverCpp-4nPop-zu.js"])))=>i.map(i=>d[i]);
import { d as kt, _ as ea, p as Uo, m as Va, s as Ua, __tla as __tla_0 } from "./didacticCpp-ClDOlw7M.js";
...
let __tla = Promise.all([
  ...
  function Rn(e, b) {
    const P = ns.find((j) => j.id === e), $ = Vn.find((j) => j.id === b), J = P.toKN, D = $.toM, U = (j, ge, z) => z / (Math.pow(J, j) * Math.pow(D, ge));
    let Y, H;
    switch (e) {
      case "kN":
        Y = 10, H = [
  ...
  function pl(e) {
    const b = e.force, [P, $, J] = e.forceRange;
    return {
      truss: [
        {
          key: "CM",
...
<tr style="color:#ff0; border-bottom:1px solid #ff03; position:sticky; top:36px; background:rgba(0,0,0,0.95); z-index:1;">
  ...
  <th style="padding:2px 6px">\u03C9 (rad/s)</th>`;
      for (const j of D) Y += `<th style="padding:2px 5px">${j}</th>`;
      if (Y += `<th style="padding:2px 5px; color:#0ff">\u03A3Ux</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Uy</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Rz</th></tr>`, $.frequencies.forEach((j, ge) => {
        var _a2;
        const z = j > 0 ? 1 / j : 0, Z = j * 2 * Math.PI, ye = ((_a2 = $.massParticipation) == null ? void 0 : _a2[ge]) || [
          0,
          0,
          0,
          0,
  ...
  <td style="padding:2px 5px; text-align:right; color:#0ff">${(U[5] * 100).toFixed(1)}%</td></tr>`;
      ...
      (_a = e.querySelector("#modal-minimize")) == null ? void 0 : _a.addEventListener("click", () => {
        b = !b;
        const j = e.querySelector("#modal-body"), ge = e.querySelector("#modal-minimize");
        b ? (j.style.display = "none", ge.textContent = "\u25A2", ge.title = "Restaurar") : (j.style.display = "block", ge.textContent = "\u25AC", ge.title = "Minimizar");
      });
      const H = e.querySelector("#modal-header");
      if (H) {
        let j = false, ge = 0, z = 0, Z = 0, ye = 0;
        H.addEventListener("mousedown", (ie) => {
          if (ie.target.tagName === "BUTTON") return;
          j = true, ge = ie.clientX, z = ie.clientY;
          const re = e.getBoundingClientRect();
          Z = re.left, ye = re.top, ie.preventDefault();
        }), document.addEventListener("mousemove", (ie) => {
          if (!j) return;
          const re = ie.clientX - ge, te = ie.clientY - z;
      ...
      (_b = e.querySelector("#modal-copy")) == null ? void 0 : _b.addEventListener("click", () => {
        ...
        const ge = `Mode  Freq(Hz)  Period(s)  \u03C9(rad/s)  ${D.join("     ")}  \u03A3Ux    \u03A3Uy    \u03A3Rz`;
        ...
        $.frequencies.forEach((ye, ie) => {
          var _a2;
          const re = ye > 0 ? 1 / ye : 0, te = ye * 2 * Math.PI, q = ((_a2 = $.massParticipation) == null ? void 0 : _a2[ie]) || [
            0,
            0,
            0,
            0,
          ...
          for (let G = 0; G < 6; G++) z[G] += q[G];
          const B = q.map((G) => ((G * 100).toFixed(1) + "%").padStart(6)).join(" ");
          j.push(`${String(ie + 1).padStart(4)}  ${ye.toFixed(4).padStart(9)}  ${re.toFixed(4).padStart(9)}  ${te.toFixed(2).padStart(9)}  ${B}  ${(z[0] * 100).toFixed(1).padStart(5)}%  ${(z[1] * 100).toFixed(1).padStart(5)}%  ${(z[5] * 100).toFixed(1).padStart(5)}%`);
...
`));
  ...
  function ul(e) {
    ...
    const { nodes: b, elements: P, elementInputs: $, nodeInputs: J, deformOutputs: D } = e, U = b.length * 6, Y = P.length, H = P.filter((re) => re.length === 2).length, j = P.filter((re) => re.length >= 3).length, ge = document.createElement("div");
    ge.className = "rpt-overlay";
    let z = "";
    z += '<button class="rpt-close" id="rpt-close">\u2715 Close</button>', z += "<h1>Finite Element Analysis \u2014 Step-by-Step Report</h1>", z += '<div class="rpt-subtitle">Complete FEM derivation from element formulation to final results</div>', z += '<hr class="rpt-sep"/>', z += "<h2>1. Input Data</h2>", z += '<table class="rpt-info"><tbody>', z += `<tr><td>Number of nodes</td><td class="val">${b.length}</td></tr>`, z += `<tr><td>Number of elements</td><td class="val">${Y} (${H} frames, ${j} shells)</td></tr>`, z += '<tr><td>DOFs per node</td><td class="val">6 (u<sub>x</sub>, u<sub>y</sub>, u<sub>z</sub>, \u03B8<sub>x</sub>, \u03B8<sub>y</sub>, \u03B8<sub>z</sub>)</td></tr>', z += `<tr><td>Total DOFs</td><td class="val">${U}</td></tr>`, z += "</tbody></table>", z += "<h3>1.1 Node Coordinates</h3>", z += '<table class="rpt-data"><thead><tr><th>Node</th><th>x</th><th>y</th><th>z</th></tr></thead><tbody>', b.forEach((re, te) => {
      z += `<tr><td>${te}</td><td>${ot(re[0])}</td><td>${ot(re[1])}</td><td>${ot(re[2])}</td></tr>`;
    }), z += "</tbody></table>", z += "<h3>1.2 Element Connectivity</h3>", z += '<table class="rpt-data"><thead><tr><th>Elem</th><th>Type</th><th>Nodes</th><th>L</th><th>E</th><th>A</th><th>I<sub>z</sub></th><th>I<sub>y</sub></th></tr></thead><tbody>', P.forEach((re, te) => {
      var _a2, _b2, _c2, _d2;
      const q = re.length === 2, B = re.map((xe) => b[xe]), G = q ? Pn(xn(B[1], B[0])) : 0, de = ((_a2 = $.elasticities) == null ? void 0 : _a2.get(te)) ?? 0, $e = ((_b2 = $.areas) == null ? void 0 : _b2.get(te)) ?? 0, Te = ((_c2 = $.momentsOfInertiaZ) == null ? void 0 : _c2.get(te)) ?? 0, Ve = ((_d2 = $.momentsOfInertiaY) == null ? void 0 : _d2.get(te)) ?? 0;
      z += `<tr><td>${te}</td><td>${q ? "Frame" : "Shell"}</td><td>${re.join(" \u2192 ")}</td>`, z += `<td>${ot(G)}</td><td>${ot(de)}</td><td>${ot($e)}</td><td>${ot(Te)}</td><td>${ot(Ve)}</td></tr>`;
    ...
    for (let re = 0; re < Y; re++) {
      const te = P[re], q = te.map((at) => b[at]);
      if (!(te.length === 2)) continue;
      const G = Pn(xn(q[1], q[0])), de = ((_a = $.elasticities) == null ? void 0 : _a.get(re)) ?? 0, $e = ((_b = $.areas) == null ? void 0 : _b.get(re)) ?? 0, Te = ((_c = $.momentsOfInertiaZ) == null ? void 0 : _c.get(re)) ?? 0, Ve = ((_d = $.momentsOfInertiaY) == null ? void 0 : _d.get(re)) ?? 0, xe = ((_e = $.shearModuli) == null ? void 0 : _e.get(re)) ?? 0, De = ((_f = $.torsionalConstants) == null ? void 0 : _f.get(re)) ?? 0;
      let He = null, be = null, Ee = null;
      try {
        He = vo(q, $, re), be = yo(q), Ee = on(os(be), on(He, be));
      } catch {
        continue;
      }
      const ze = xn(q[1], q[0]), Je = ze[0] / G, st = ze[1] / G, Ue = ze[2] / G;
      z += '<div class="rpt-elem-block">', z += `<h3 class="rpt-elem-title" data-toggle="elem${re}">\u25B6 Element ${re} \u2014 Nodes ${te[0]} \u2192 ${te[1]}, L = ${ot(G)}</h3>`, z += `<div id="rpt-elem${re}" class="rpt-elem-body" style="display:none">`, z += "<h4>Properties (numerical substitution)</h4>", z += '<div class="rpt-eq-small">', z += `E = ${ot(de)} &nbsp;&nbsp; A = ${ot($e)} &nbsp;&nbsp; I<sub>z</sub> = ${ot(Te)} &nbsp;&nbsp; I<sub>y</sub> = ${ot(Ve)} &nbsp;&nbsp; G = ${ot(xe)} &nbsp;&nbsp; J = ${ot(De)}<br/>`, z += `EA/L = ${ot(de)}\xB7${ot($e)}/${ot(G)} = <b>${ot(de * $e / G)}</b><br/>`, z += `12EI<sub>z</sub>/L\xB3 = 12\xB7${ot(de)}\xB7${ot(Te)}/${ot(G)}\xB3 = <b>${ot(12 * de * Te / G ** 3)}</b><br/>`, z += `12EI<sub>y</sub>/L\xB3 = 12\xB7${ot(de)}\xB7${ot(Ve)}/${ot(G)}\xB3 = <b>${ot(12 * de * Ve / G ** 3)}</b><br/>`, z += `GJ/L = ${ot(xe)}\xB7${ot(De)}/${ot(G)} = <b>${ot(xe * De / G)}</b>`, z += "</div>", z += "<h4>Direction cosines</h4>", z += `<div class="rpt-eq-small">l = ${go(Je)}, m = ${go(st)}, n = ${go(Ue)}, D = ${go(Math.sqrt(Je ** 2 + st ** 2))}</div>`, z += "<h4>K<sub>local</sub> (12\xD712)</h4>", z += Ko(He, 12), z += "<h4>T \u2014 Transformation (12\xD712)</h4>", z += Ko(be, 12), z += "<h4>K<sub>global</sub> = T\u1D40 \xB7 K<sub>local</sub> \xB7 T</h4>", z += Ko(Ee, 12), z += "<h4>Assembly</h4>", z += `<div class="rpt-eq-small">Global DOFs: node ${te[0]} \u2192 [${te[0] * 6}..${te[0] * 6 + 5}], node ${te[1]} \u2192 [${te[1] * 6}..${te[1] * 6 + 5}]</div>`, z += "</div></div>";
    }
    ...
    if (z += "<h3>5.1 Supports (fixed DOFs)</h3>", J.supports && J.supports.size > 0) {
      ...
      for (const re of Z) z += `<th>${re}</th>`;
      z += "</tr></thead><tbody>", J.supports.forEach((re, te) => {
        z += `<tr><td>${te}</td>`, re.forEach((q, B) => {
          q && ye.push(te * 6 + B), z += `<td class="${q ? "fixed" : ""}">${q ? "Fixed" : "Free"}</td>`;
        }), z += "</tr>";
      }), z += "</tbody></table>";
    }
    if (z += `<div class="rpt-eq-small">Fixed DOFs: [${ye.join(", ")}] \u2192 ${ye.length} constraints<br/>`, z += `Free DOFs: ${U} \u2212 ${ye.length} = <b>${U - ye.length}</b></div>`, z += "<h3>5.2 Applied Loads</h3>", J.loads && J.loads.size > 0) {
      z += '<table class="rpt-data"><thead><tr><th>Node</th>';
      const re = [
        "F<sub>x</sub>",
        "F<sub>y</sub>",
      ...
      for (const te of re) z += `<th>${te}</th>`;
      z += "</tr></thead><tbody>", J.loads.forEach((te, q) => {
        z += `<tr><td>${q}</td>`, te.forEach((B) => {
          const G = Math.abs(B) > 1e-10;
          z += `<td class="${G ? "nz" : ""}">${G ? ot(B) : "0"}</td>`;
        }), z += "</tr>";
      }), z += "</tbody></table>";
    }
    if (z += "<h2>6. Solution</h2>", z += "<p>After removing fixed DOFs, the reduced system is:</p>", z += '<div class="rpt-eq rpt-eq-highlight">K<sub>free</sub> \xB7 u<sub>free</sub> = F<sub>free</sub></div>', z += "<p>Solved using LU decomposition with partial pivoting (sparse matrix).</p>", z += "<h3>6.1 Nodal Displacements</h3>", D == null ? void 0 : D.deformations) {
      z += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const re of Z) z += `<th>${re}</th>`;
      z += "</tr></thead><tbody>", D.deformations.forEach((re, te) => {
        z += `<tr><td>${te}</td>`, re.forEach((q) => {
          const B = Math.abs(q) > 1e-10;
          z += `<td class="${B ? "nz" : ""}">${ot(q, 6)}</td>`;
        }), z += "</tr>";
      }), z += "</tbody></table>";
    }
    if (z += "<h3>6.2 Reactions</h3>", z += '<div class="rpt-eq">R = K<sub>total</sub> \xB7 u (extract at fixed DOFs)</div>', D == null ? void 0 : D.reactions) {
      z += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const re of Z) z += `<th>${re}</th>`;
      z += "</tr></thead><tbody>", D.reactions.forEach((re, te) => {
        z += `<tr><td>${te}</td>`, re.forEach((q) => {
          const B = Math.abs(q) > 1e-10;
    ...
    if (z += "<h2>7. Internal Forces</h2>", z += "<p>For each element, transform global displacements to local and multiply by K<sub>local</sub>:</p>", z += '<div class="rpt-eq">u<sub>local</sub> = T \xB7 u<sub>global</sub></div>', z += '<div class="rpt-eq rpt-eq-highlight">f<sub>local</sub> = K<sub>local</sub> \xB7 u<sub>local</sub></div>', D == null ? void 0 : D.deformations) {
      const re = [
        "N",
        "V<sub>y</sub>",
        "V<sub>z</sub>",
      ...
      for (const te of re) z += `<th>${te}<sub>i</sub></th>`;
      for (const te of re) z += `<th>${te}<sub>j</sub></th>`;
      z += "</tr></thead><tbody>";
      for (let te = 0; te < Y; te++) {
        ...
        const B = q.map((G) => b[G]);
        try {
          const G = vo(B, $, te), de = yo(B), $e = [];
          for (const xe of q) {
            const De = ((_g = D.deformations) == null ? void 0 : _g.get(xe)) || [
              0,
              0,
              0,
              0,
          ...
          z += `<tr><td>${te}</td><td>${q.join("\u2192")}</td>`;
          for (let xe = 0; xe < 12; xe++) {
            const De = Math.abs(Ve[xe]) > 1e-10;
            z += `<td class="${De ? "nz" : ""}">${ot(Ve[xe], 2)}</td>`;
          }
          z += "</tr>";
        } catch {
        }
  ...
  `;
    return ge.innerHTML = ie + z, (_h = ge.querySelector("#rpt-close")) == null ? void 0 : _h.addEventListener("click", () => ge.remove()), ge.querySelectorAll("[data-toggle]").forEach((re) => {
  ...
  function Ko(e, b) {
    ...
    for (let J = 0; J < P; J++) {
      $ += "<tr>";
      for (let D = 0; D < P; D++) {
        const U = ((_a = e[J]) == null ? void 0 : _a[D]) ?? 0, Y = Math.abs(U) < 1e-10;
        $ += `<td class="${Y ? "z" : ""} ${J === D && !Y ? "diag" : ""}">${Y ? "0" : ml(U)}</td>`;
      }
      $ += "</tr>";
    }
    return $ += "</table>", b > P && ($ += `<div style="color:#888;font-size:9pt">(showing ${P}\xD7${P} of ${b}\xD7${b})</div>`), $ += "</div>", $;
  }
  function ml(e) {
  ...
  function bl() {
    ...
    for (const H of U) {
      ...
      const ge = 0.75, z = 30 + ge * 540 + 8, Z = 180 / 2 - H.fn(ge) * 60 - 6;
      Y += `<text x="${z}" y="${Z}" fill="${H.color}" font-size="11" font-weight="bold" font-family="sans-serif">${H.name}</text>`;
    }
    return Y += "</svg>", Y;
  }
  function gl(e, b) {
    const P = b * 6, $ = Math.min(P, 30);
    ...
    for (let U = 0; U < e.length; U++) {
      ...
      for (const H of Y) for (const j of Y) for (let ge = 0; ge < 6; ge++) for (let z = 0; z < 6; z++) {
        const Z = H + ge, ye = j + z;
        Z < $ && ye < $ && D[Z][ye]++;
      }
    }
    for (let U = 0; U < $; U++) {
      ...
      for (let Y = 0; Y < $; Y++) {
        const H = D[U][Y], j = H === 0 ? "#fff" : H === 1 ? "#e8f0fe" : H === 2 ? "#c6dcf5" : "#a0c4e8", ge = H === 0 ? "" : H.toString();
        J += `<td style="background:${j};color:#003366">${ge}</td>`;
      }
      J += "</tr>";
    }
    return J += "</table></div>", P > $ && (J += `<div style="color:#888;font-size:9pt">(showing ${$}\xD7${$} of ${P}\xD7${P})</div>`), J;
  }
  let Zo = false;
  function hl(e) {
    if (Zo || window.katex) {
  ...
  function xl(e, b, P, $, J, D) {
    var _a, _b, _c, _d, _e, _f;
    const U = P[e], Y = U.map((be) => b[be]), H = U.length === 2, j = H ? Pn(xn(Y[1], Y[0])) : 0, ge = ((_a = $.elasticities) == null ? void 0 : _a.get(e)) ?? 0, z = ((_b = $.areas) == null ? void 0 : _b.get(e)) ?? 0, Z = ((_c = $.momentsOfInertiaZ) == null ? void 0 : _c.get(e)) ?? 0, ye = ((_d = $.momentsOfInertiaY) == null ? void 0 : _d.get(e)) ?? 0, ie = ((_e = $.shearModuli) == null ? void 0 : _e.get(e)) ?? 0, re = ((_f = $.torsionalConstants) == null ? void 0 : _f.get(e)) ?? 0;
    let te = null, q = null, B = null;
    try {
      te = vo(Y, $, e), q = yo(Y), B = on(os(q), on(te, q));
    } catch {
    }
    const G = H ? xn(Y[1], Y[0]) : [
      ...
      0,
      0
    ], de = j > 0 ? G[0] / j : 0, $e = j > 0 ? G[1] / j : 0, Te = j > 0 ? G[2] / j : 0, Ve = Math.sqrt(de ** 2 + $e ** 2), xe = [];
  ...
  function vl(e, b, P, $, J, D) {
    var _a, _b;
    const U = xl(e, b, P, $, J, D), Y = document.createElement("div");
  ...
  `, Y.querySelectorAll(".er-tab").forEach((H) => {
      ...
      H && (H.innerHTML = la(U)), setTimeout(() => {
        ...
        j.addEventListener("click", () => {
          const ge = j.dataset.toggle, z = Y.querySelector(`#er-${ge}`);
  ...
  function yl(e) {
    ...
    if (b += '<div class="er-section-title">1. Propiedades</div>', b += '<table class="er-props">', b += `<tr><td>E</td><td>${Ye(e.E)}</td><td>A</td><td>${Ye(e.A)}</td></tr>`, b += `<tr><td>I<sub>z</sub></td><td>${Ye(e.Iz)}</td><td>I<sub>y</sub></td><td>${Ye(e.Iy)}</td></tr>`, b += `<tr><td>G</td><td>${Ye(e.G)}</td><td>J</td><td>${Ye(e.J)}</td></tr>`, b += "</table>", e.kLocal && (b += `<div class="er-section-title">2. K<sub>local</sub> (${e.kLocal.length}\xD7${e.kLocal.length})</div>`, b += ao(e.kLocal)), e.T && (b += '<div class="er-section-title">3. T \u2014 Transformaci\xF3n</div>', b += ao(e.T)), e.kGlobal && (b += '<div class="er-section-title">4. K<sub>global</sub> = T<sup>T</sup>\xB7K\xB7T</div>', b += ao(e.kGlobal)), b += '<div class="er-section-title">5. Desplazamientos</div>', e.uGlobal.length > 0) {
      ...
      for (let $ = 0; $ < e.elem.length; $++) {
        ...
        for (let J = 0; J < 6; J++) {
          const D = e.uGlobal[$ * 6 + J];
          b += `${P[J]}=<span class="${Math.abs(D) > 1e-10 ? "nz" : ""}">${Ye(D, 6)}</span> `;
        }
        b += "</div>";
      }
  ...
  function la(e) {
    ...
    if (b += `<div class="er-eq">${$(`\\text{Nodo } ${e.elem[0]} \\rightarrow \\text{DOFs } [${H} \\ldots ${H + 5}]`)}</div>`, b += `<div class="er-eq">${$(`\\text{Nodo } ${e.elem[1]} \\rightarrow \\text{DOFs } [${j} \\ldots ${j + 5}]`)}</div>`, b += `<div class="er-eq">${$("\\mathbf{K}_{total}[\\text{DOFs}_i, \\text{DOFs}_j] \\mathrel{+}= \\mathbf{K}_{global}[i, j]")}</div>`, b += '<div class="er-section-title">9. Recuperacion de fuerzas internas</div>', b += `<div class="er-eq">${$("\\mathbf{u}_{local} = \\mathbf{T} \\cdot \\mathbf{u}_{global}")}</div>`, b += `<div class="er-eq er-eq-main">${$("\\mathbf{f}_{local} = \\mathbf{K}_{local} \\cdot \\mathbf{u}_{local}")}</div>`, e.fLocal.length > 0 && e.fLocal.some((ge) => ge !== 0)) {
      const ge = [
        "N",
        "V<sub>y</sub>",
        "V<sub>z</sub>",
  ...
  function ao(e) {
    ...
    return $ += "</table>", b > P && ($ += `<div style="color:var(--fem-label);font-size:9px">(${P}\xD7${P} de ${b}\xD7${b})</div>`), $ += "</div>", $;
  }
  function Ml(e) {
    return Math.abs(e) >= 1e6 || Math.abs(e) < 0.01 && e !== 0 ? e.toExponential(1) : Math.abs(e) >= 100 ? e.toFixed(0) : e.toFixed(2);
  }
  function Qo(e) {
    ...
    if (!b) return;
    const P = e.width, $ = e.height, J = 30, D = P - 2 * J, U = ($ - 3 * J) / 2;
    b.fillStyle = getComputedStyle(document.body).getPropertyValue("--fem-bg").trim() || "#111", b.fillRect(0, 0, P, $);
    const Y = (H, j, ge) => {
      b.strokeStyle = "#333", b.lineWidth = 1, b.strokeRect(J, H, D, U), b.strokeStyle = "#444", b.beginPath(), b.moveTo(J, H + U / 2), b.lineTo(J + D, H + U / 2), b.stroke(), b.fillStyle = "#888", b.font = "11px sans-serif", b.fillText(j, J + 4, H + 14);
      for (const Z of ge) {
        b.strokeStyle = Z.color, b.lineWidth = 2.5, b.beginPath();
        for (let ye = 0; ye <= 100; ye++) {
          const ie = ye / 100, re = J + ie * D, te = H + U / 2 - Z.fn(ie) * (U / 2 * 0.85);
          ye === 0 ? b.moveTo(re, te) : b.lineTo(re, te);
        }
        b.stroke();
      }
      let z = J + D - 90;
      for (const Z of ge) b.fillStyle = Z.color, b.font = "bold 10px sans-serif", b.fillText(Z.label, z, H + U - 6), z += 36;
      b.fillStyle = "#666", b.font = "9px monospace", b.fillText("0", J, H + U + 12), b.fillText("1", J + D - 6, H + U + 12), b.fillText("\u03BE", J + D / 2, H + U + 12);
    };
    Y(J, "Axial (lineal)", [
      {
        fn: (H) => 1 - H,
  ...
  let Mo = false, On = null, ln = null, Dt = null, Tt = null;
  function El() {
  ...
  function ss() {
    Mo = false, Tt && (Tt.innerHTML = "?", Tt.style.background = "linear-gradient(135deg, #0066cc, #0099ff)", Tt.style.animation = "helpPulse 2s infinite"), ln && (ln.remove(), ln = null), Dt && (Dt.remove(), Dt = null), On && (On.remove(), On = null);
  }
  function Un(e) {
    var _a, _b;
    if (e >= oo.length) {
    ...
    P.scrollIntoView({
      behavior: "smooth",
      block: "nearest"
    }), ln && ln.remove(), Dt && Dt.remove();
    const $ = P.getBoundingClientRect(), J = window.innerWidth, D = window.innerHeight, U = 320, Y = 180;
  ...
  `, document.body.appendChild(ln);
    const H = J - $.right, j = $.left, ge = D - $.bottom, z = $.top;
    let Z = b.position || "bottom";
    Z === "bottom" && ge < Y + 20 && (Z = "top"), Z === "top" && z < Y + 20 && (Z = "right"), Z === "right" && H < U + 20 && (Z = "left"), Z === "left" && j < U + 20 && (Z = "bottom");
    let ye, ie, re = "";
    switch (Z) {
      case "bottom":
        ye = $.left + $.width / 2 - U / 2, ie = $.bottom + 14, re = "position:absolute;top:-8px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:8px solid #0099ff;";
        break;
      case "top":
        ye = $.left + $.width / 2 - U / 2, ie = $.top - Y - 14, re = "position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #0099ff;";
        break;
      case "right":
        ye = $.right + 14, ie = $.top + $.height / 2 - Y / 2, re = "position:absolute;left:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-right:8px solid #0099ff;";
        break;
      case "left":
        ye = $.left - U - 14, ie = $.top + $.height / 2 - Y / 2, re = "position:absolute;right:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-left:8px solid #0099ff;";
        break;
    }
    ye = Math.max(10, Math.min(ye, J - U - 10)), ie = Math.max(10, Math.min(ie, D - Y - 10)), Dt = document.createElement("div"), Dt.style.cssText = `
    position: fixed;
  ...
  function Il() {
    var _a;
    ln && (ln.remove(), ln = null), Dt && (Dt.remove(), Dt = null), Dt = document.createElement("div"), Dt.style.cssText = `
    position: fixed;
  ...
  `, document.body.appendChild(Dt), (_a = Dt.querySelector("#tour-done")) == null ? void 0 : _a.addEventListener("click", () => ss());
  }
  function kl(e) {
    var _a;
    const b = e.split(/\r?\n/), P = {
      force: "TONF",
      length: "M"
    }, $ = [], J = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), Y = [], H = [], j = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), z = [], Z = [];
    let ye = "", ie = "";
    ...
    for (const Le of b) {
      ...
      if (ie && (re.has(ie) || re.set(ie, []), re.get(ie).push(Le)), ie === "CONTROLS") {
      ...
      if (ie === "STORIES - IN SEQUENCE FROM TOP") {
        ...
        if (ve) {
          const Ce = ve[1], he = ve[2] ? parseFloat(ve[2]) : 0, ke = ve[3] ? parseFloat(ve[3]) : void 0;
          $.push({
            name: Ce,
            height: he,
            elev: ke ?? 0
      ...
      if (ie === "LINE ASSIGNS") {
        ...
        if (ve) {
          ...
          Ge && (Ce.angle = parseFloat(Ge[1])), ge.set(`${ve[1]}@${ve[2]}`, Ce);
        }
      }
      if (ie === "GRIDS") {
      ...
      if (ie === "AREA CONNECTIVITIES") {
        ...
        if (ve) {
          const Ce = ((_a = ve[2].match(/"([^"]+)"/g)) == null ? void 0 : _a.map((he) => he.replace(/"/g, ""))) || [];
          H.push({
            name: ve[1],
            pts: Ce,
            nStories: 0
    ...
    const q = [], B = [], G = /* @__PURE__ */ new Map(), de = (Le, qe) => `${Le}@${qe}`, $e = /* @__PURE__ */ new Set(), Te = /* @__PURE__ */ new Map();
    for (const Le of Y) Te.set(Le.name, Le);
    for (const Le of Y) for (const [qe, ve] of ge) {
      if (!qe.startsWith(Le.name + "@")) continue;
      const Ce = ve.story, he = $.findIndex((ke) => ke.name === Ce);
      if (!(he < 0)) if (Le.type === "COLUMN" || Le.type === "BRACE") {
        $e.add(de(Le.pt2, Ce));
        const ke = Math.max(Le.nStories, 1), Ge = Math.min(he + ke, $.length - 1);
        $e.add(de(Le.pt1, $[Ge].name));
      } else $e.add(de(Le.pt1, Ce)), $e.add(de(Le.pt2, Ce));
    }
    for (const [Le] of j) $e.add(Le);
    for (const Le of $e) {
      const [qe, ve] = Le.split("@"), Ce = U.get(qe), he = te.get(ve);
      Ce === void 0 || he === void 0 || (q.push([
        Ce[0],
        Ce[1],
        he
    ...
    for (const Le of Y) for (const [qe, ve] of ge) {
      if (!qe.startsWith(Le.name + "@")) continue;
      const Ce = ve.story, he = $.findIndex((We) => We.name === Ce);
      if (he < 0) continue;
      let ke, Ge;
      if (Le.type === "COLUMN" || Le.type === "BRACE") {
        const We = Math.max(Le.nStories, 1), Xe = Math.min(he + We, $.length - 1);
        ke = de(Le.pt1, $[Xe].name), Ge = de(Le.pt2, Ce);
      } else ke = de(Le.pt1, Ce), Ge = de(Le.pt2, Ce);
    ...
    for (const [Le, qe] of be) {
      const ve = D.get(qe);
      if (!ve) continue;
      const Ce = J.get(ve.material);
      Ce && (Ee.set(Le, Ce.E), ze.set(Le, Ce.G));
      const he = ve.D, ke = ve.B, Ge = ve.TF, _e = ve.TW;
      ...
      switch (ve.shape) {
        ...
        case "Steel Channel":
        case "Cold Formed C":
          it = 2 * ke * Ge + (he - 2 * Ge) * _e, mt = (_e * he ** 3 + 2 * ke * Ge * (he - Ge) ** 2) / 12, We = (2 * Ge * ke ** 3 + (he - 2 * Ge) * _e ** 3) / 12, Xe = (2 * ke * Ge ** 3 + (he - 2 * Ge) * _e ** 3) / 3, pt = (he - 2 * Ge) * _e, lt = 2 * ke * Ge * 5 / 6, bt = ve.shape === "Cold Formed C" ? "coldC" : "C";
          break;
      ...
      ve.modI2 && (We *= ve.modI2), ve.modI3 && (mt *= ve.modI3), Je.set(Le, it), yt.set(Le, mt), zt.set(Le, We), Ft.set(Le, Xe), pt > 0 && st.set(Le, pt), lt > 0 && Ue.set(Le, lt), gt.set(Le, {
        type: bt,
        b: ke || void 0,
        h: he || void 0,
    ...
    for (const Le of z) {
      ...
      const [ve, Ce] = Ve[qe], he = q[ve], ke = q[Ce], Ge = Math.sqrt((ke[0] - he[0]) ** 2 + (ke[1] - he[1]) ** 2 + (ke[2] - he[2]) ** 2);
  ...
  function da(e) {
    ...
    let $;
    for (; ($ = P.exec(e)) !== null; ) b.set($[1], $[2] !== void 0 ? $[2] : $[3]);
    return b;
  }
  function Tl(e) {
    const b = e.split(/\r?\n/);
  ...
  function zl(e) {
    ...
    const D = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), j = [], ge = [], z = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), ie = [];
    let re = "";
    for (const te of b) {
      const q = te.trim();
      ...
      switch (re) {
        case "PROGRAM CONTROL": {
          ...
          if (G) {
            ...
            de[0] && ($.force = de[0]), de[1] && ($.length = de[1]);
          }
          break;
        }
        ...
        case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
          ...
          if (G) {
            ...
            de.E = tt(B.get("E1")), de.G = tt(B.get("G12")), de.nu = tt(B.get("U12")), de.density = tt(B.get("UnitMass")), D.set(G, de);
          }
          break;
        }
        case "MATERIAL PROPERTIES 03A - STEEL DATA": {
          const G = B.get("Material");
          G && D.has(G) && (D.get(G).fy = tt(B.get("Fy")));
          break;
        }
        ...
        case "JOINT COORDINATES": {
          const G = B.get("Joint");
          if (G) {
            const de = tt(B.get("XorR")), $e = tt(B.get("Y")), Te = tt(B.get("Z"));
            H.set(G, [
              de,
              $e,
              Te
        ...
        case "CONNECTIVITY - FRAME": {
          const G = B.get("Frame"), de = B.get("JointI"), $e = B.get("JointJ");
          G && de && $e && j.push({
            name: G,
            j1: de,
            j2: $e
        ...
        case "CONNECTIVITY - AREA": {
          const G = B.get("Area");
          if (G) {
            const de = parseInt(B.get("NumJoints") || "4"), $e = [];
        ...
        case "FRAME SECTION ASSIGNMENTS": {
          const G = B.get("Frame"), de = B.get("AnalSect");
          G && de && Z.set(G, de);
          break;
        }
        case "AREA SECTION ASSIGNMENTS": {
          const G = B.get("Area"), de = B.get("Section");
          G && de && ye.set(G, de);
          break;
        }
    ...
    return pa($, J, D, U, Y, H, j, ge, z, Z, ye, ie);
  }
  function Al(e) {
    const b = {
      force: "KN",
    ...
    const $ = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), Y = [], H = [], j = /* @__PURE__ */ new Map(), ge = [];
    let z = "", Z = "";
    for (const re of e) {
      const te = re.trim();
    ...
    return pa(b, P, $, J, D, U, Y, H, j, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), ge);
  }
  function pa(e, b, P, $, J, D, U, Y, H, j, ge, z) {
    var _a;
    const Z = [], ye = /* @__PURE__ */ new Map(), ie = [];
    for (const [xe, De] of D) ye.set(xe, ie.length), Z.push(xe), ie.push(De);
    const re = [], te = [], q = /* @__PURE__ */ new Map();
    for (const xe of U) {
      const De = ye.get(xe.j1), He = ye.get(xe.j2);
      if (De !== void 0 && He !== void 0) {
        ...
        const Ee = j.get(xe.name);
        Ee && q.set(be, Ee);
      }
    }
    const B = re.length;
    for (const xe of Y) {
      const De = xe.joints.map((He) => ye.get(He)).filter((He) => He !== void 0);
      if (De.length >= 3) {
        const He = re.length;
        re.push(De), te.push(xe.name);
        const be = ge.get(xe.name);
        be && q.set(He, be);
      }
    }
    const G = re.length - B, de = {
    ...
    for (let xe = 0; xe < re.length; xe++) {
      const De = q.get(xe), He = De ? $.get(De) : null, be = De ? J.get(De) : null;
      if (He || re[xe].length === 2) {
        ...
        de.elasticities.set(xe, Je), de.shearModuli.set(xe, Ue), de.areas.set(xe, Ee.A || Ee.D * Ee.B), de.momentsOfInertiaZ.set(xe, Ee.Iz || Ee.B * Ee.D ** 3 / 12), de.momentsOfInertiaY.set(xe, Ee.Iy || Ee.D * Ee.B ** 3 / 12), de.torsionalConstants.set(xe, Ee.J || 0), de.densities.set(xe, ze.density || 0), ((_a = Ee.shape) == null ? void 0 : _a.includes("Wide Flange")) || Ee.shape === "I" ? $e.set(xe, {
          type: "I",
          b: Ee.B,
          h: Ee.D,
          name: De || "I-section"
        ...
        de.elasticities.set(xe, ze), de.shearModuli.set(xe, st), de.thicknesses.set(xe, be.thickness), de.poissonsRatios.set(xe, Je), de.densities.set(xe, Ee.density || 0);
      }
    }
    const Ve = {
    ...
    for (const [xe, De] of H) {
      const He = ye.get(xe);
      He !== void 0 && Ve.supports.set(He, De);
    }
    for (const xe of z) {
      const De = ye.get(xe.joint);
      if (De !== void 0) {
  ...
  function Ll(e) {
    ...
    const { nodes: b, elements: P, nodeInputs: $, elementInputs: J } = e, D = e.units || {
      force: "KN",
      length: "m"
    ...
    const ge = [], z = [];
    if (P.forEach((q, B) => {
      q.length === 2 ? ge.push(B) : z.push(B);
    }), ge.length > 0) {
      H('TABLE:  "CONNECTIVITY - FRAME"');
      for (const q of ge) {
        const B = P[q];
        H(`   Frame=${q + 1}   JointI=${B[0] + 1}   JointJ=${B[1] + 1}   IsCurved=No`);
      }
      j();
    }
    if (z.length > 0) {
      ...
      for (const q of z) {
        const B = P[q], G = B.map((de, $e) => `Joint${$e + 1}=${de + 1}`).join("   ");
        H(`   Area=${q + 1}   NumJoints=${B.length}   ${G}`);
      }
      j();
    }
    ...
    for (const q of ge) {
      const B = ((_a = J.areas) == null ? void 0 : _a.get(q)) || 0, G = ((_b = J.momentsOfInertiaZ) == null ? void 0 : _b.get(q)) || 0, de = ((_c = J.momentsOfInertiaY) == null ? void 0 : _c.get(q)) || 0, $e = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(q)) || 0, Te = ((_e = J.elasticities) == null ? void 0 : _e.get(q)) || 0, Ve = `MAT_${Math.round(Te)}`, xe = `A${B.toPrecision(6)}_Iz${G.toPrecision(6)}`;
      if (!Z.has(xe)) {
        let He = 0.3, be = 0.3;
        B > 0 && G > 0 && (He = Math.sqrt(12 * G / B), be = B / He), Z.set(xe, {
          A: B,
          Iz: G,
          Iy: de,
          J: $e,
    ...
    if (ge.length > 0) {
      ...
      for (const q of ge) {
        const B = ye.get(q) || "SEC1";
        H(`   Frame=${q + 1}   AutoSelect=N.A.   AnalSect=${B}   MatProp=Default`);
      }
      j();
    }
    if (Z.size > 0) {
      ...
      for (const [, B] of Z) {
        q++;
        const G = B.A * 5 / 6;
        H(`   SectionName=SEC${q}   Material=${B.matKey}   Shape=Rectangular   t3=${It(B.h)}   t2=${It(B.b)}   Area=${It(B.A)}   TorsConst=${It(B.J)}   I33=${It(B.Iz)}   I22=${It(B.Iy)}   I23=0   AS2=${It(G)}   AS3=${It(G)} _`), H("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
      }
      j();
    }
    const ie = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map();
    for (const q of z) {
      const B = ((_f = J.thicknesses) == null ? void 0 : _f.get(q)) || 0.1, G = ((_g = J.elasticities) == null ? void 0 : _g.get(q)) || 0, de = `MAT_${Math.round(G)}`, $e = `t${B.toPrecision(6)}`;
      ie.has($e) || ie.set($e, {
        t: B,
        matKey: de
      });
    ...
    if (z.length > 0) {
      ...
      for (const B of z) {
        const G = re.get(B) || "SSEC1";
        H(`   Area=${B + 1}   Section=${G}   MatProp=Default`);
      }
      j(), H('TABLE:  "AREA SECTION PROPERTIES"');
      let q = 0;
      for (const [, B] of ie) q++, H(`   Section=SSEC${q}   Material=${B.matKey}   MatAngle=0   AreaType=Shell   Type=ShellThin   DrillDOF=Yes   Thickness=${It(B.t)}   BendThick=${It(B.t)}   Color=Cyan`);
      j();
    }
    H('TABLE:  "JOINT COORDINATES"');
    for (let q = 0; q < b.length; q++) {
      const B = b[q];
      H(`   Joint=${q + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${It(B[0])}   Y=${It(B[1])}   Z=${It(B[2])}   SpecialJt=No`);
    }
    if (j(), $.supports && $.supports.size > 0) {
      H('TABLE:  "JOINT RESTRAINT ASSIGNMENTS"');
      for (const [q, B] of $.supports) {
        if (!B.some((de) => de)) continue;
        const G = (de) => de ? "Yes" : "No";
        H(`   Joint=${q + 1}   U1=${G(B[0])}   U2=${G(B[1])}   U3=${G(B[2])}   R1=${G(B[3])}   R2=${G(B[4])}   R3=${G(B[5])}`);
      }
      j();
    }
    if (H('TABLE:  "LOAD PATTERN DEFINITIONS"'), H("   LoadPat=DEAD   DesignType=Dead   SelfWtMult=0"), j(), H('TABLE:  "LOAD CASE DEFINITIONS"'), H('   Case=DEAD   Type=LinStatic   InitialCond=Zero   DesTypeOpt="Prog Det"   DesignType=Dead   DesActOpt="Prog Det"   DesignAct=Non-Composite   AutoType=None   RunCase=Yes'), j(), H('TABLE:  "CASE - STATIC 1 - LOAD ASSIGNMENTS"'), H('   Case=DEAD   LoadType="Load pattern"   LoadName=DEAD   LoadSF=1'), j(), $.forces && $.forces.size > 0) {
      ...
      for (const [q, B] of $.forces) B.some((G) => Math.abs(G) > 1e-12) && H(`   Joint=${q + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${It(B[0])}   F2=${It(B[1])}   F3=${It(B[2])}   M1=${It(B[3])}   M2=${It(B[4])}   M3=${It(B[5])}`);
      j();
    }
    const te = /* @__PURE__ */ new Map();
    for (let q = 0; q < P.length; q++) {
      const B = ((_h = J.elasticities) == null ? void 0 : _h.get(q)) || 0, G = ((_i = J.shearModuli) == null ? void 0 : _i.get(q)) || 0, de = B > 0 && G > 0 ? Math.max(0, Math.min(0.5, B / (2 * G) - 1)) : 0.2, $e = ((_j = J.densities) == null ? void 0 : _j.get(q)) || 0, Te = `MAT_${Math.round(B)}`;
      te.has(Te) || te.set(Te, {
        E: B,
        nu: de,
        G,
    ...
    for (const [q, B] of te) H(`   Material=${q}   UnitWeight=${It(B.rho * 9.81)}   UnitMass=${It(B.rho)}   E1=${It(B.E)}   G12=${It(B.G)}   U12=${It(B.nu)}   A1=9.9E-06`);
    ...
    return j(), H('TABLE:  "PROGRAM CONTROL"'), H(`   ProgramName=SAP2000   Version=24.1.0   CurrUnits="${D.force}, ${D.length}, C"   SteelCode="AISC 360-16"   ConcCode="ACI 318-19"   AlumCode="AA 2015"   ColdCode=AISI-16   RegenHinge=Yes`), j(), H("END TABLE DATA"), H(""), Y.join(`\r
`);
  }
  function It(e) {
  ...
  function Fl(e, b) {
    ...
    for (const [J, D] of e) if (!$.includes(J) && D.length !== 0) {
      P.push(`$ ${J}`);
      for (const U of D) P.push(U);
      P.push("");
    }
...
`);
  ...
  function Rl(e) {
    var _a, _b, _c, _d;
    const { nodes: b, elements: P, nodeInputs: $, elementInputs: J, title: D, units: U } = e, Y = (U == null ? void 0 : U.force) || "TONF", H = (U == null ? void 0 : U.length) || "M", j = [], ge = (be) => Math.round(be * 1e4) / 1e4;
    j.push("$ File exported from Awatif FEM Studio"), j.push(""), j.push("$ PROGRAM INFORMATION"), j.push('  PROGRAM  "AWATIF"  VERSION "1.0.0"  '), j.push(""), j.push("$ CONTROLS"), j.push(`  UNITS  "${Y}"  "${H}"  "C"  `), D && j.push(`  TITLE2  "${D}"  `), j.push("");
    const z = /* @__PURE__ */ new Set();
    b.forEach((be) => z.add(ge(be[1])));
    const Z = [
      ...z
    ...
    Z.length > 0 && j.push(`  STORY "Base"  ELEV ${Z[0]} `), j.push(""), P.some((be) => be.length === 4) && (j.push("$ DIAPHRAGM NAMES"), j.push('  DIAPHRAGM "D1"    TYPE RIGID'), j.push("")), j.push("$ MATERIAL PROPERTIES");
    ...
    j.push("");
    const xe = (be) => {
      const Ee = b[be], ze = `${ge(Ee[0])},${ge(Ee[2])}`;
      return {
        pt: Te.get(ze) || "1",
    ...
    P.forEach((be, Ee) => {
      if (be.length !== 2) return;
      const ze = Pl(b, be), Je = de.get(Ee) || `Sec_${Ee}`;
      if (ze === "BEAM") {
        const st = xe(be[0]), Ue = xe(be[1]);
        j.push(`  LINE  "E${Ee + 1}"  BEAM  "${st.pt}"  "${Ue.pt}"  0`), De.push(`  LINEASSIGN  "E${Ee + 1}"  "${st.story}"  SECTION "${Je}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
      } else {
        const st = b[be[0]][1] <= b[be[1]][1] ? be[0] : be[1], Ue = b[be[0]][1] <= b[be[1]][1] ? be[1] : be[0];
        xe(st);
        const at = xe(Ue), dt = ge(b[st][1]), yt = ge(b[Ue][1]), zt = Z.indexOf(dt), Ft = Z.indexOf(yt), gt = Math.max(1, Ft >= 0 && zt >= 0 ? Ft - zt : 1);
        j.push(`  LINE  "E${Ee + 1}"  ${ze}  "${at.pt}"  "${at.pt}"  ${gt}`);
        for (let ut = 0; ut < gt; ut++) {
          const rn = Ft - ut;
          rn >= 0 && rn < ye.length && De.push(`  LINEASSIGN  "E${Ee + 1}"  "${ye[rn]}"  SECTION "${Je}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
        }
      }
    }), j.push(""), j.push("$ POINT ASSIGNS"), (_b = $.supports) == null ? void 0 : _b.forEach((be, Ee) => {
      const ze = [];
    ...
    if (He.length > 0) {
      ...
      const be = [];
      He.forEach((Ee, ze) => {
        const { el: Je, isWall: st } = Ee, Ue = st ? `W${ze + 1}` : `F${ze + 1}`, at = st ? "PANEL" : "FLOOR", dt = Je.map((yt) => xe(yt));
        if (st) {
    ...
    return j.push("$ LOAD PATTERNS"), j.push('  LOADPATTERN "Dead"  TYPE  "Dead"  SELFWEIGHT  1'), j.push('  LOADPATTERN "Live"  TYPE  "Live"  SELFWEIGHT  0'), j.push(""), $.loads && $.loads.size > 0 && (j.push("$ POINT OBJECT LOADS"), $.loads.forEach((be, Ee) => {
      const [ze, Je, st] = be, Ue = xe(Ee);
...
`);
  ...
  function Pl(e, b) {
    const P = e[b[0]], $ = e[b[1]], J = Math.abs($[1] - P[1]), D = Math.sqrt(($[0] - P[0]) ** 2 + ($[2] - P[2]) ** 2), U = J > D * 0.5;
    return U && D > 0.01 ? "BRACE" : U ? "COLUMN" : "BEAM";
  }
  function Ol(e) {
    var _a, _b;
    const { nodes: b, elements: P, nodeInputs: $, elementInputs: J } = e, D = [];
    return D.push("# OpenSeesPy model exported from Awatif FEM Studio"), D.push(`# ${b.length} nodes, ${P.length} elements`), D.push(""), D.push("import openseespy.opensees as ops"), D.push(""), D.push("ops.wipe()"), D.push("ops.model('basic', '-ndm', 3, '-ndf', 6)"), D.push(""), D.push("# --- Nodes ---"), b.forEach((U, Y) => {
      D.push(`ops.node(${Y + 1}, ${U[0]}, ${U[1]}, ${U[2]})`);
    }), D.push(""), D.push("# --- Boundary Conditions ---"), (_a = $.supports) == null ? void 0 : _a.forEach((U, Y) => {
      const H = U.map((j) => j ? 1 : 0).join(", ");
      D.push(`ops.fix(${Y + 1}, ${H})`);
    }), D.push(""), D.push("# --- Geometric Transformations ---"), D.push("ops.geomTransf('Linear', 1, 0.0, 0.0, 1.0)  # beams (vecxz = Z)"), D.push("ops.geomTransf('Linear', 2, -1.0, 0.0, 0.0)  # columns (vecxz = -X)"), D.push(""), D.push("# --- Elements (elasticBeamColumn) ---"), P.forEach((U, Y) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (U.length !== 2) return;
      const H = b[U[0]], j = b[U[1]], z = Math.abs(j[2] - H[2]) > Math.max(Math.abs(j[0] - H[0]), Math.abs(j[1] - H[1])) ? 2 : 1, Z = ((_a2 = J.areas) == null ? void 0 : _a2.get(Y)) ?? 1, ye = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(Y)) ?? 2e5, ie = ((_c = J.shearModuli) == null ? void 0 : _c.get(Y)) ?? 8e4, re = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(Y)) ?? 1, te = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(Y)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(Y)) ?? 1;
      D.push(`ops.element('elasticBeamColumn', ${Y + 1}, ${U[0] + 1}, ${U[1] + 1}, ${Z}, ${ye}, ${ie}, ${re}, ${te}, ${q}, ${z})`);
    }), D.push(""), $.loads && $.loads.size > 0 && (D.push("# --- Loads ---"), D.push("ops.timeSeries('Linear', 1)"), D.push("ops.pattern('Plain', 1, 1)"), $.loads.forEach((U, Y) => {
      const H = U.map((j) => j).join(", ");
      D.push(`ops.load(${Y + 1}, ${H})`);
      ...
      D.push(`print(f"Node {${Y + 1}}: {ops.nodeDisp(${Y + 1})}")`);
    }), D.push(""), D.push('print("\\n=== Reactions ===")'), D.push("ops.reactions()"), (_b = $.supports) == null ? void 0 : _b.forEach((U, Y) => {
      D.push(`print(f"Node {${Y + 1}}: {ops.nodeReaction(${Y + 1})}")`);
    }), D.join(`
`);
  }
  function Nl(e) {
    var _a, _b;
    const { nodes: b, elements: P, nodeInputs: $, elementInputs: J } = e, D = [];
    return D.push("# OpenSees Tcl model exported from Awatif FEM Studio"), D.push(`# ${b.length} nodes, ${P.length} elements`), D.push(""), D.push("wipe"), D.push("model basic -ndm 3 -ndf 6"), D.push(""), D.push("# --- Nodes ---"), b.forEach((U, Y) => {
      D.push(`node ${Y + 1} ${U[0]} ${U[1]} ${U[2]}`);
    }), D.push(""), D.push("# --- Boundary Conditions ---"), (_a = $.supports) == null ? void 0 : _a.forEach((U, Y) => {
      const H = U.map((j) => j ? 1 : 0).join(" ");
      D.push(`fix ${Y + 1} ${H}`);
    }), D.push(""), D.push("# --- Geometric Transformations ---"), D.push("geomTransf Linear 1 0.0 0.0 1.0"), D.push("geomTransf Linear 2 -1.0 0.0 0.0"), D.push(""), D.push("# --- Elements ---"), P.forEach((U, Y) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (U.length !== 2) return;
      const H = b[U[0]], j = b[U[1]], z = Math.abs(j[2] - H[2]) > Math.max(Math.abs(j[0] - H[0]), Math.abs(j[1] - H[1])) ? 2 : 1, Z = ((_a2 = J.areas) == null ? void 0 : _a2.get(Y)) ?? 1, ye = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(Y)) ?? 2e5, ie = ((_c = J.shearModuli) == null ? void 0 : _c.get(Y)) ?? 8e4, re = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(Y)) ?? 1, te = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(Y)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(Y)) ?? 1;
      D.push(`element elasticBeamColumn ${Y + 1} ${U[0] + 1} ${U[1] + 1} ${Z} ${ye} ${ie} ${re} ${te} ${q} ${z}`);
    }), D.push(""), $.loads && $.loads.size > 0 && (D.push("# --- Loads ---"), D.push("timeSeries Linear 1"), D.push("pattern Plain 1 1 {"), $.loads.forEach((U, Y) => {
      const H = U.map((j) => j).join(" ");
      D.push(`  load ${Y + 1} ${H}`);
      ...
      D.push(`puts "Node ${Y + 1}: [nodeDisp ${Y + 1}]"`);
    }), D.push('puts "\\n=== Reactions ==="'), D.push("reactions"), (_b = $.supports) == null ? void 0 : _b.forEach((U, Y) => {
      D.push(`puts "Node ${Y + 1}: [nodeReaction ${Y + 1}]"`);
    }), D.join(`
`);
  }
  function ql(e) {
    const b = [], P = [], $ = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map();
    for (const ye of e.split(/\r?\n/)) {
      const ie = ye.trim(), re = ie.match(/ops\.node\(\s*(\d+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)/);
      if (re) {
        const G = parseInt(re[1]), de = b.length;
        b.push([
          parseFloat(re[2]),
          parseFloat(re[3]),
          parseFloat(re[4])
      ...
      if (te) {
        const G = parseInt(te[1]), de = z.get(G);
        de !== void 0 && $.set(de, [
          te[2] === "1",
          te[3] === "1",
          te[4] === "1",
      ...
      if (q) {
        const G = parseInt(q[1]), de = z.get(parseInt(q[2])), $e = z.get(parseInt(q[3]));
        if (de !== void 0 && $e !== void 0) {
          const Te = P.length;
          P.push([
            de,
            $e
          ]), Z.set(G, Te), Y.set(Te, parseFloat(q[4])), D.set(Te, parseFloat(q[5])), U.set(Te, parseFloat(q[6])), ge.set(Te, parseFloat(q[7])), H.set(Te, parseFloat(q[8])), j.set(Te, parseFloat(q[9]));
        }
        continue;
      }
      const B = ie.match(/ops\.load\(\s*(\d+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)\s*,\s*([-\d.eE+]+)/);
      if (B) {
  ...
  function _l(e) {
    const b = [], P = [], $ = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), D = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), z = /* @__PURE__ */ new Map();
    for (const Z of e.split(/\r?\n/)) {
      const ye = Z.trim();
      if (ye.startsWith("#") || ye.startsWith("//")) continue;
      const ie = ye.split(/\s+/);
      if (ie[0] === "node" && ie.length >= 5) {
        const re = parseInt(ie[1]), te = b.length;
        b.push([
          parseFloat(ie[2]),
          parseFloat(ie[3]),
          parseFloat(ie[4])
      ...
      if (ie[0] === "element" && ie[1] === "elasticBeamColumn" && ie.length >= 12) {
        const re = z.get(parseInt(ie[3])), te = z.get(parseInt(ie[4]));
        if (re !== void 0 && te !== void 0) {
          const q = P.length;
          P.push([
            re,
            te
          ]), Y.set(q, parseFloat(ie[5])), D.set(q, parseFloat(ie[6])), U.set(q, parseFloat(ie[7])), ge.set(q, parseFloat(ie[8])), H.set(q, parseFloat(ie[9])), j.set(q, parseFloat(ie[10]));
        }
        continue;
      }
  ...
  function Bl(e) {
    const b = {
      ...
      project: "",
      app: ""
    }, P = {}, $ = {}, J = e.match(/FILE_SCHEMA\s*\(\s*\(\s*'([^']*)'/i);
    J && (b.schema = J[1]);
  ...
  function ua(e, b) {
    const P = Ut(b.args), $ = Wt(e, P[0]), J = $ ? Dl(e, $) : [
      0,
      0,
      0
    ];
  ...
  function ma(e, b) {
    ...
    if ($ && (J = ua(e, $)), P[0] && P[0] !== "$") {
      ...
      if (D && D.type === "IFCLOCALPLACEMENT") {
        const U = ma(e, D), Y = ts(J.origin, U.dirX, es(U.dirZ, U.dirX), U.dirZ);
        J.origin = [
          U.origin[0] + Y[0],
          U.origin[1] + Y[1],
          U.origin[2] + Y[2]
  ...
  function ts(e, b, P, $) {
    return [
      e[0] * b[0] + e[1] * P[0] + e[2] * $[0],
      e[0] * b[1] + e[1] * P[1] + e[2] * $[1],
      e[0] * b[2] + e[1] * P[2] + e[2] * $[2]
    ];
  }
  const Hl = 0.01;
  function jl(e) {
    const b = Bl(e), { entities: P, typeIndex: $ } = b, J = [], D = [], U = /* @__PURE__ */ new Map();
    U.set("Hormigon", {
      E: 2132888792e-2,
      nu: 0.2,
      rho: 2.4
    ...
    function j(te, q, B) {
      for (const G of J) {
        const de = G.x - te, $e = G.y - q, Te = G.z - B;
    ...
    function ge(te) {
      const q = fa(te.args, 2) || "", B = $.IFCRELASSOCIATESMATERIAL || [];
      for (const de of B) {
        const $e = P[de];
        if (!$e) continue;
        const Te = Ut($e.args);
    ...
    function z(te) {
      ...
      if (q === "IFCRECTANGLEPROFILEDEF") {
        const B = Ut(te.args), G = (B[1] || "").replace(/'/g, ""), de = parseFloat(B[3]) || 0.3, $e = parseFloat(B[4]) || 0.3;
        return {
          b: de,
          h: $e,
          name: G
      ...
      if (q === "IFCMATERIALPROFILE") {
        const B = Ut(te.args), G = B[2] || B[1] || "", de = Wt(P, G);
        if (de) return z(de);
      }
      if (q === "IFCMATERIALPROFILESET") {
        const B = Ut(te.args), de = (B[2] || B[1] || "").match(/#(\d+)/);
        if (de) {
          const $e = P[parseInt(de[1])];
          if ($e) return z($e);
        }
      }
      if (q === "IFCMATERIALPROFILESETUSAGE") {
        const G = Ut(te.args)[0], de = Wt(P, G);
        if (de) return z(de);
      }
      return {
        b: 0.3,
    ...
    function Z(te, q, B, G) {
      ...
      for (const $e of de) {
        ...
        const at = ze ? ze[0] : He.origin[0], dt = ze ? ze[1] : He.origin[1], yt = ze ? ze[2] : He.origin[2], zt = Je || (B === "Z" ? He.dirZ : He.dirX), Ft = j(at, dt, yt), gt = j(at + zt[0] * Ee, dt + zt[1] * Ee, yt + zt[2] * Ee);
        D.push({
          id: H++,
          type: "frame",
          nodeIds: [
    ...
    function ye(te, q, B) {
      ...
      for (const de of G) {
        ...
        be && (De = Wl(P, be) || B);
        const Ee = q === "slab" ? `Losa e=${(De * 100).toFixed(0)}cm` : q === "wall" ? `Muro e=${(De * 100).toFixed(0)}cm` : q === "footing" ? `Zapata e=${(De * 100).toFixed(0)}cm` : `${q} e=${(De * 100).toFixed(0)}cm`;
        D.push({
          id: H++,
          type: "shell",
          nodeIds: [],
    ...
    const ie = [], re = $.IFCBUILDINGSTOREY || [];
    for (const te of re) {
      const q = P[te];
      if (!q) continue;
      const B = Ut(q.args), G = (B[2] || "").replace(/'/g, ""), de = parseFloat(B[9]) || 0;
      ie.push({
        name: G,
        elevation: de
      });
  ...
  function $o(e, b) {
    ...
    for (const $ of P) {
      const J = $.match(/#(\d+)/g) || [];
      for (const D of J) {
        const U = parseInt(D.replace("#", "")), Y = e[U];
        if (Y) {
          ...
          if (Y.type === "IFCMAPPEDITEM") {
            ...
            if (j && j.type === "IFCREPRESENTATIONMAP") {
              const ge = Ut(j.args), z = Wt(e, ge[1]);
              if (z) {
                const Z = $o(e, z);
                if (Z) return Z;
              }
  ...
  async function Yl(e, b) {
    ...
    const $ = new P.IfcAPI(), J = window.location.pathname.replace(/\/[^/]*$/, "/");
    $.SetWasmPath(J), await $.Init();
    const D = $.OpenModel(new Uint8Array(b)), U = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), H = {
      843113511: "Columna",
      753842376: "Viga",
      1529196076: "Losa",
      900683007: "Zapata",
    ...
    for (const [ye] of ba) {
      const ie = Gl(ye);
      try {
        const re = $.GetLineIDsWithType(D, ye);
    ...
    return $.StreamAllMeshes(D, (ye) => {
      const ie = U.get(ye.expressID) ?? "other", re = j.get(ie), te = ye.geometries;
      for (let q = 0; q < te.size(); q++) {
        const B = te.get(q), G = $.GetGeometry(D, B.geometryExpressID), de = $.GetVertexArray(G.GetVertexData(), G.GetVertexDataSize()), $e = $.GetIndexArray(G.GetIndexData(), G.GetIndexDataSize()), Te = new tn(), Ve = new Float32Array(de.length / 2), xe = new Float32Array(de.length / 2);
        for (let ze = 0; ze < de.length; ze += 6) {
          const Je = ze / 2;
          Ve[Je] = de[ze], Ve[Je + 1] = de[ze + 1], Ve[Je + 2] = de[ze + 2], xe[Je] = de[ze + 3], xe[Je + 1] = de[ze + 4], xe[Je + 2] = de[ze + 5];
        }
        Te.setAttribute("position", new xo(Ve, 3)), Te.setAttribute("normal", new xo(xe, 3)), Te.setIndex(new xo(new Uint32Array($e), 1));
        const De = new el();
        ...
        Ee.applyMatrix4(De), Ee.userData.expressID = ye.expressID, Ee.userData.category = ie, re.add(Ee), ge.expandByObject(Ee), z++, G.delete();
      }
    }), $.CloseModel(D), {
      meshCount: z,
      bbox: ge,
  ...
  er = function(e) {
    ...
    let z = "", Z = {}, ye = null, ie = "", re = [], te = [], q = [], B = /* @__PURE__ */ new Set(), G = /* @__PURE__ */ new Set(), de = /* @__PURE__ */ new Set(), $e = /* @__PURE__ */ new Map(), Te = /* @__PURE__ */ new Map(), Ve = null, xe = [], De = 0.2, He = 2, be = 2, Ee = false, ze = 2, Je = "x", st = /* @__PURE__ */ new Set(), Ue = true, at = 0.15, dt = 2, yt = 2, zt = /* @__PURE__ */ new Set(), Ft = false, gt = "perimeter";
    const ut = () => ({
      b: 0.3,
      h: 0.4,
      profileIdx: 0,
    ...
    let xt = 0, Le = 3, qe = false, ve = 0, Ce = null, he = 0, ke = [], Ge = 1, _e = true;
    const it = fl();
    ...
    function as(t, n, o, l, s) {
      ...
      for (const h of o) for (const k of t) for (const A of n) {
        const I = [
          ...
          new Oe(k, h, A - p),
          new Oe(k, h, A + p)
        ], T = new tn().setFromPoints(I), R = new Yn({
          color: 5596791,
          transparent: true,
          opacity: 0.4
        }), x = new Gn(T, R);
    ...
    function Xn(t, n, o, l, s) {
      ...
      for (let x = 0; x < t.length; x++) {
        ...
        if (k) {
          const S = -w - h * 1.5;
          Io(u, 0, 0, u, 0, S + h, A, Pt), ko(y[x] || `${x}`, u, 0, S, h, Pt);
        } else {
          const S = f - w - h * 1.5;
          Io(u, f, 0, u, S + h, 0, A, Pt), ko(y[x] || `${x}`, u, S, 0, h, Pt);
        }
      }
      if (!k) for (let x = 0; x < n.length; x++) {
        const u = n[x], S = a - w - h * 1.5;
        Io(a, u, 0, S + h, u, 0, A, Pt), ko(p[x] || `${x}`, S, u, 0, h, Pt);
      }
      const I = h * 1.8, T = w * 1.2, R = w * 1.2;
      for (let x = 0; x < t.length - 1; x++) {
        const u = t[x], S = t[x + 1], C = Math.abs(S - u), O = (u + S) / 2, W = `${C.toFixed(2)} m`;
        k ? (Eo(W, O, 0, -T, I, Pt), So(u, 0, -T * 0.7, S, 0, -T * 0.7, 16763904, Pt)) : (Eo(W, O, f - R, 0, I, Pt), So(u, f - R * 0.7, 0, S, f - R * 0.7, 0, 16763904, Pt));
      }
      if (!k) for (let x = 0; x < n.length - 1; x++) {
        const u = n[x], S = n[x + 1], C = Math.abs(S - u), O = (u + S) / 2, W = `${C.toFixed(2)} m`;
        Eo(W, a - T, O, 0, I, Pt), So(a - T * 0.7, u, 0, a - T * 0.7, S, 0, 16763904, Pt);
      }
      Pt.traverse((x) => {
    ...
    const Ke = {
      ...
      build(t) {
        ...
        const n = (t == null ? void 0 : t.col) || "40x40", o = (t == null ? void 0 : t.viga) || "30x40", l = (t == null ? void 0 : t.fc) || 210, [s, d] = n.split("x").map((_) => parseFloat(_) / 100), [a, r] = o.split("x").map((_) => parseFloat(_) / 100), f = We.map((_) => _.coord), i = Xe.map((_) => _.coord), c = lt.map((_) => _.elev), m = f.length, M = i.length, w = c.length, y = w - 1, p = [], h = {};
        for (let _ = 0; _ < w; _++) for (let ue = 0; ue < M; ue++) for (let oe = 0; oe < m; oe++) h[`${oe},${ue},${_}`] = p.length, p.push([
          f[oe],
          i[ue],
          c[_]
        ]);
        const k = [], A = /* @__PURE__ */ new Set(), I = /* @__PURE__ */ new Set(), T = /* @__PURE__ */ new Map();
        for (let _ = 0; _ < y; _++) for (let ue = 0; ue < M; ue++) for (let oe = 0; oe < m; oe++) {
          const fe = k.length;
          k.push([
            h[`${oe},${ue},${_}`],
            h[`${oe},${ue},${_ + 1}`]
        ...
        for (let _ = 1; _ < w; _++) for (let ue = 0; ue < M; ue++) for (let oe = 0; oe < m - 1; oe++) {
          const fe = k.length;
          k.push([
            h[`${oe},${ue},${_}`],
            h[`${oe + 1},${ue},${_}`]
          ]), I.add(fe), T.set(fe, _ - 1);
        }
        for (let _ = 1; _ < w; _++) for (let ue = 0; ue < m; ue++) for (let oe = 0; oe < M - 1; oe++) {
          const fe = k.length;
          k.push([
        ...
        const R = ((_a2 = t == null ? void 0 : t.braces) == null ? void 0 : _a2.toLowerCase()) || "", x = /* @__PURE__ */ new Set();
        if (R) {
          const _ = R === "all" || R === "x" || R === "perimeter", ue = R === "all" || R === "y" || R === "perimeter";
          for (let oe = 0; oe < y; oe++) {
            if (_) for (let fe = 0; fe < M; fe++) {
              if (R === "perimeter" && fe !== 0 && fe !== M - 1) continue;
              const ee = Math.floor((m - 1) / 2);
              for (let me = 0; me < m - 1; me++) {
                if (R === "perimeter" && me !== ee) continue;
                const we = k.length;
                k.push([
                  h[`${me},${fe},${oe}`],
                  h[`${me + 1},${fe},${oe + 1}`]
        ...
        const u = 15100 * Math.sqrt(l) * 10, S = u / (2 * (1 + 0.2)), C = s * d, O = s * d ** 3 / 12, W = d * s ** 3 / 12, g = s * d * (s ** 2 + d ** 2) / 12, E = a * r, v = a * r ** 3 / 12, F = r * a ** 3 / 12, X = a * r * (a ** 2 + r ** 2) / 12, V = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map();
        for (let _ = 0; _ < k.length; _++) V.set(_, u), ae.set(_, S), A.has(_) ? (Q.set(_, C), K.set(_, O), pe.set(_, W), ce.set(_, g), Ie.set(_, {
          type: "rect",
          b: s,
          h: d,
          name: `COL${n}`
        })) : x.has(_) ? (Q.set(_, C), K.set(_, O), pe.set(_, W), ce.set(_, g), Ie.set(_, {
          type: "rect",
          b: s,
          h: d,
          name: `BR${n}`
        })) : (Q.set(_, E), K.set(_, v), pe.set(_, F), ce.set(_, X), Ie.set(_, {
          type: "rect",
          b: a,
          h: r,
          name: `V${o}`
        ...
        for (let _ = 0; _ < M; _++) for (let ue = 0; ue < m; ue++) Pe.set(h[`${ue},${_},0`], [
          true,
          true,
          true,
          true,
        ...
        return e.nodes.val = p, e.elements.val = k, e.nodeInputs.val = {
          ...
          sectionShapes: Ie
        }, B = A, G = I, $e = T, console.log(`Built: ${p.length} nodes, ${k.length} elements (${A.size} cols, ${I.size} beams, ${x.size} braces)`), console.log(`  Col: ${n}, Viga: ${o}, f'c=${l}${R ? `, braces=${R}` : ""}`), {
          nodes: p.length,
          elements: k.length
        };
      },
      ...
      addBeam(t, n, o, l, s) {
        ...
        const c = We[d].coord, m = Xe[a].coord, M = We[r].coord, w = Xe[f].coord, y = lt[i].elev, p = [
          ...
          const u = p.findIndex((S) => Math.abs(S[0] - T) < 1e-3 && Math.abs(S[1] - R) < 1e-3 && Math.abs(S[2] - x) < 1e-3);
          return u >= 0 ? u : (p.push([
            T,
            R,
            x
      ...
      addBrace(t, n, o, l, s, d) {
        ...
        const M = We[a].coord, w = Xe[r].coord, y = lt[f].elev, p = We[i].coord, h = Xe[c].coord, k = lt[m].elev, A = [
          ...
          const O = A.findIndex((W) => Math.abs(W[0] - u) < 1e-3 && Math.abs(W[1] - S) < 1e-3 && Math.abs(W[2] - C) < 1e-3);
          return O >= 0 ? O : (A.push([
            u,
            S,
            C
...
`;
      ...
      model3d(t) {
        ...
        for (let ee = 0; ee < h; ee++) for (let me = 0; me < p; me++) for (let we = 0; we < y; we++) I[`${we},${ee},${me}`] = A.length, A.push([
          m[we],
          w[ee],
          M[me]
        ]);
        const T = [], R = /* @__PURE__ */ new Set(), x = /* @__PURE__ */ new Set(), u = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < k; ee++) for (let me = 0; me < p; me++) for (let we = 0; we < y; we++) {
          const se = T.length;
          T.push([
            I[`${we},${ee},${me}`],
            I[`${we},${ee + 1},${me}`]
          ]), R.add(se), u.set(se, ee);
        }
        ...
        const C = 15100 * Math.sqrt(a) * 10, O = C / (2 * (1 + 0.2)), W = r * f, g = r * f ** 3 / 12, E = f * r ** 3 / 12, v = r * f * (r ** 2 + f ** 2) / 12, F = i * c, X = i * c ** 3 / 12, V = c * i ** 3 / 12, ae = i * c * (i ** 2 + c ** 2) / 12, Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), Pe = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < T.length; ee++) Q.set(ee, C), K.set(ee, O), R.has(ee) ? (pe.set(ee, W), ce.set(ee, g), Ie.set(ee, E), Pe.set(ee, v), _.set(ee, {
          type: "rect",
          b: r,
          h: f,
          name: `COL${s}`
        ...
        for (let ee = 0; ee < p; ee++) for (let me = 0; me < y; me++) ue.set(I[`${me},0,${ee}`], [
          true,
          true,
          true,
          true,
        ...
        const fe = Ae.querySelector("#cad3d-floor-buttons");
        if (fe) {
          fe.style.display = "flex", fe.innerHTML = '<span style="font-size:10px;color:var(--cad-heading);margin-right:4px">Planta:</span>';
          for (let ee = 0; ee < k; ee++) fe.innerHTML += `<button class="floor-btn" data-floor="${ee}">P${ee + 1}</button>`;
        }
      ...
      building(t, n, o, l = 3, s = 0, d = 0, a = 0, r = 0, f = 1) {
        ...
        for (const { el: [u, S], floor: C, dir: O, bay: W } of T) {
          const g = p[u], E = p[S];
          let v = u;
          for (let X = 1; X < l; X++) {
        ...
        const R = /* @__PURE__ */ new Map(), x = mt();
        for (let u = 0; u < c.length; u++) for (let S = 0; S < i.length; S++) y(S, u) || R.set(h[`${S},${u},0`], [
          ...x
        ]);
        de = /* @__PURE__ */ new Set();
        for (const u of xe) {
          const S = m.length - 1, C = u.floors.includes(-1) ? Array.from({
            length: S
          }, (O, W) => W) : u.floors.filter((O) => O < S);
          for (const O of C) {
            let W, g, E, v;
            u.dir === "x" ? (W = u.bay, E = u.bay + 1, g = u.axisIdx, v = u.axisIdx) : (W = u.axisIdx, E = u.axisIdx, g = u.bay, v = u.bay + 1);
            const F = h[`${W},${g},${O}`], X = h[`${W},${g},${O + 1}`];
            let V, ae;
        ...
        if (zt = /* @__PURE__ */ new Set(), Ue) {
          const u = l, S = l, C = /* @__PURE__ */ new Map(), O = (W, g, E) => `${Math.round(W * 1e4)},${Math.round(g * 1e4)},${Math.round(E * 1e4)}`;
          for (let W = 0; W < p.length; W++) C.set(O(p[W][0], p[W][1], p[W][2]), W);
            ...
            for (let E = 0; E < i.length - 1; E++) for (let v = 0; v < c.length - 1; v++) {
              ...
              for (let K = 0; K <= S; K++) {
                ...
                for (let ce = 0; ce <= u; ce++) {
                  const Ie = F + ce / u * (X - F), Pe = V + K / S * (ae - V);
                  if (K === 0 && ce === 0) pe.push(h[`${E},${v},${W}`]);
                  else if (K === 0 && ce === u) pe.push(h[`${E + 1},${v},${W}`]);
                  else if (K === S && ce === 0) pe.push(h[`${E},${v + 1},${W}`]);
                  else if (K === S && ce === u) pe.push(h[`${E + 1},${v + 1},${W}`]);
                  else {
                    const _ = O(Ie, Pe, g), ue = C.get(_);
                    if (ue !== void 0) pe.push(ue);
                      ...
                      p.push([
                        ...
                        Pe,
                        g
                      ]), C.set(_, oe), pe.push(oe);
                    }
                  }
                }
                Q.push(pe);
              }
              for (let K = 0; K < S; K++) for (let pe = 0; pe < u; pe++) {
        ...
        if (Ft && gt) {
          ...
          for (let O = 0; O < C; O++) {
            if (u) for (let W = 0; W < c.length; W++) {
              if (gt === "perimeter" && W !== 0 && W !== c.length - 1) continue;
              const g = Math.floor((i.length - 1) / 2);
              for (let E = 0; E < i.length - 1; E++) {
                if (gt === "perimeter" && E !== g || y(E, W) || y(E + 1, W)) continue;
                const v = h[`${E},${W},${O}`], F = h[`${E + 1},${W},${O + 1}`], X = h[`${E + 1},${W},${O}`], V = h[`${E},${W},${O + 1}`];
            ...
            if (S) for (let W = 0; W < i.length; W++) {
              ...
              for (let E = 0; E < c.length - 1; E++) {
                if (gt === "perimeter" && E !== g || y(W, E) || y(W, E + 1)) continue;
                const v = h[`${W},${E},${O}`], F = h[`${W},${E + 1},${O + 1}`], X = h[`${W},${E + 1},${O}`], V = h[`${W},${E},${O + 1}`];
                v !== void 0 && F !== void 0 && (A.push([
                  v,
                  F
                ]), $e.set(A.length - 1, O)), X !== void 0 && V !== void 0 && (A.push([
      ...
      example(t) {
        ...
        switch (t) {
          ...
          case "edificio": {
            et("edificio"), Ne.colMat = 0, Ne.vigaMat = 0, Ne.colShape = 0, xe = [], Ue = false, Ee = false, Ft = false, Re();
            break;
          }
          case "edif-acero":
          case "edificio-acero": {
            et("edificio"), Ne.colMat = 1, Ne.vigaMat = 1, Ne.steelColType = 0, Ne.steelVigaType = 0, xe = [], Ee = true, ze = 2;
          ...
          case "mezzanine": {
            et("edificio"), Z.nPisos && (Z.nPisos.val = 1), Z.hPiso && (Z.hPiso.val = 4.5), Z.nVanosX && (Z.nVanosX.val = 3), Z.nVanosY && (Z.nVanosY.val = 2), Z.nSubViga && (Z.nSubViga.val = 3), re = [
              ...
              5,
              5
            ], Ne.colMat = 1, Ne.vigaMat = 1, Ne.steelColType = 0, Ne.steelVigaType = 0, xe = [], Ee = true, ze = 2, Je = re[0] >= te[0] ? "y" : "x", Ue = true, at = 0.08, dt = 3, yt = 3, Re();
            break;
          }
          case "galpon": {
      ...
      plateQ4(t = 10, n = 10, o = 16, l = 16, s = "simply-supported", d = -10, a = 0.2, r = 3e7, f = 0.3, i = 0) {
        ...
        if (e.nodeInputs && (e.nodeInputs.val = {
          ...
          const I = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map(), R = /* @__PURE__ */ new Map();
...
Util:     cad.info()  cad.clear()  cad.help()
      ...
      units(t, n) {
        ...
        return o && (o.textContent = b), l && (l.textContent = P), z && et(z), console.log(`Unidades: ${$.label} | E=${$.E.toExponential(3)} ${$.stress}`), $.id;
      },
      view(t) {
      },
      get mesh() {
    ...
    function et(t) {
      ...
      if (t === "edificio") {
        ...
        re = Array(l).fill($.defaultSpan), te = Array(s).fill($.defaultSpan * 0.8);
    ...
    function hs() {
      ...
      console.log(`Solved in ${x.toFixed(1)} ms, w_max = ${R.maxW.toExponential(4)}`);
    ...
    function xs() {
      ...
      try {
        const x = kt(M, c, R, T), u = gn(M, c, T, x), S = M.map((O) => [
          O[0],
          0,
          O[1]
        ]);
    ...
    function vs() {
      const t = le("H") || 4, n = le("B") || 3, o = le("tw") || 0.3, l = le("tb") || 0.4, s = le("meshSize") || 0.2, d = le("E") || 25e6, a = le("nu") || 0.2, r = d / (2 * (1 + a)), f = le("gamma") || 18, i = le("Ka") || 0.33, c = le("Es") || 5e4, m = le("nus") || 0.3, M = c / (2 * (1 + m)), w = le("kn") || 1e6, y = le("ks") || 1e4, p = le("gammaW") || 9.81, h = le("Hw") || 3.5, k = le("qs") || 0, A = xt, I = n * 0.3, T = n * 0.7, R = [
        [
          -I,
          0,
          0
      ...
      if (A === 0) {
        ...
        if (A === 2) {
          ...
          for (let se = 1; se < K.length; se++) {
            const Se = x[K[se]][1] - x[pe[pe.length - 1]][1];
          ...
          for (let se = 0; se < Ie; se++) Pe[se] ? (oe.set(se, c), fe.set(se, c), me.set(se, m), we.set(se, M), ee.set(se, 1)) : (oe.set(se, d), fe.set(se, d), me.set(se, a), we.set(se, r), ee.set(se, 1));
          for (let se = _; se < u.length; se++) oe.set(se, w), fe.set(se, 0), me.set(se, 0), we.set(se, y), ee.set(se, 0);
          O = {
            elasticities: oe,
          ...
          console.log(`  Interfaz Goodman: ${K.length} nodos interfaz, ${ue} elem interfaz, kn=${w}, ks=${y}`);
        } else {
          const F = [
            [
              -I,
          ...
          const ae = (_) => {
            const ue = (x[_[0]][0] + x[_[1]][0] + x[_[2]][0]) / 3, oe = (x[_[0]][1] + x[_[1]][1] + x[_[2]][1]) / 3, fe = ue >= -I && ue <= T && oe >= 0 && oe <= l, ee = ue >= 0 && ue <= o && oe >= l && oe <= l + t;
            return fe || ee;
          }, Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), Pe = [];
          for (let _ = 0; _ < u.length; _++) {
            const ue = ae(u[_]);
            Pe.push(!ue), ue ? (Q.set(_, d), K.set(_, d), ce.set(_, a), Ie.set(_, r), pe.set(_, 1)) : (Q.set(_, c), K.set(_, c), ce.set(_, m), Ie.set(_, M), pe.set(_, 1));
          }
          O = {
            elasticities: Q,
          ...
          for (let _ = 0; _ < u.length; _++) {
            if (!Pe[_]) continue;
            const ue = u[_], oe = x[ue[0]], fe = x[ue[1]], ee = x[ue[2]], me = Math.abs((fe[0] - oe[0]) * (ee[1] - oe[1]) - (ee[0] - oe[0]) * (fe[1] - oe[1])) / 2, we = -f * me / 3;
            for (const se of ue) {
              const Se = C.get(se) || [
                0,
                0,
          ...
          if (k > 0) {
            ...
            for (let ue = 0; ue < _.length; ue++) {
              ...
              const fe = -k * oe, ee = C.get(_[ue].idx) || [
                0,
                0,
                0,
                0,
      ...
      try {
        const E = kt(x, u, W, O), v = u.filter((pe) => pe.length === 3), F = {};
        ...
        S.forEach((pe, ce) => ae.set(ce, pe));
        const Q = /* @__PURE__ */ new Map();
        C.forEach((pe, ce) => Q.set(ce, [
          pe[0],
          pe[2],
          pe[1],
          pe[3],
        ...
        let K = 0;
        E && E.deformations && E.deformations.forEach((pe) => {
    ...
    function ys() {
      ...
      for (let S = 0; S <= a; S++) for (let C = 0; C <= d; C++) {
        const O = S * (d + 1) + C;
        let W = w, g = y;
        (C === 0 || C === d) && (W = w / 2), (S === 0 || S === a) && (g = y / 2), A.push({
          node: O,
          dof: 0,
          k: M * W * g
        });
      ...
      const T = i / Math.max(I, 1), R = [];
      for (let S = 0; S <= a; S++) for (let C = 0; C <= d; C++) {
        const O = C * w, W = S * y;
        Math.abs(O - p) <= k + 1e-6 && Math.abs(W - h) <= k + 1e-6 && R.push({
          node: S * (d + 1) + C,
          dof: 0,
          value: T
        });
      ...
      if (Math.abs(c) > 1e-6) {
        ...
        for (let O = 0; O <= a; O++) for (let W = 0; W <= d; W++) {
          const g = W * w, E = O * y;
      ...
      try {
        ...
        W.push([
          ...
          O + 3,
          O
        ]), e.nodes.val = C, e.elements.val = W;
    ...
    function $s() {
      ...
      const W = {
        elasticities: new Map(R.map((g, E) => [
          E,
          f
        ])),
        elasticitiesOrthogonal: new Map(R.map((g, E) => [
          E,
          f
        ])),
        thicknesses: new Map(R.map((g, E) => [
          E,
          o
        ])),
        poissonsRatios: new Map(R.map((g, E) => [
          E,
          i
        ])),
        shearModuli: new Map(R.map((g, E) => [
          E,
          c
        ]))
      };
      console.log(`Placa Base: ${t * 1e3}x${n * 1e3}mm, t=${o * 1e3}mm, ${M} pernos d=${l * 1e3}mm`), console.log(`  P=${m} kN, col=${a * 1e3}mm, mesh=${r * 1e3}mm`), console.log(`  ${R.length} triangulos, ${x.length} nodos`);
      try {
        const g = kt(x, R, {
          supports: u,
          loads: S
        }, W), E = gn(x, R, W, g);
        e.nodes.val = x, e.elements.val = R, g && e.deformOutputs && (e.deformOutputs.val = g), e.nodeInputs && (e.nodeInputs.val = {
          supports: u,
          loads: S
    ...
    function Ms() {
      const t = le("colB") || 0.3, n = le("colH") || 0.3, o = le("colT") || 8e-3, l = le("colLen") || 1.5, s = le("Lx") || 0.45, d = le("Ly") || 0.45, a = le("tPlaca") || 0.025, r = le("dBolt") || 0.022, f = le("sx") || 0.32, i = le("sy") || 0.32, c = Math.round(le("nSubColV") || 6), m = Math.round(le("nSubColH") || 4), M = Math.round(le("nSubPlaca") || 10), w = le("E") || 2e8, y = le("nu") || 0.3, p = w / (2 * (1 + y)), h = le("P") || -300, k = s / 2, A = d / 2, I = r / 2, T = t / 2, R = n / 2, x = [], u = [], S = M, C = s / S, O = d / S, W = (fe, ee) => ee * (S + 1) + fe;
      for (let fe = 0; fe <= S; fe++) for (let ee = 0; ee <= S; ee++) x.push([
        ee * C,
        fe * O,
        0
      ]);
      const g = [
        ...
        for (const [me, we] of g) if ((fe - me) * (fe - me) + (ee - we) * (ee - we) < I * I) return true;
        return false;
      }, v = u.length;
      for (let fe = 0; fe < S; fe++) for (let ee = 0; ee < S; ee++) {
        const me = (ee + 0.5) * C, we = (fe + 0.5) * O;
        E(me, we) || u.push([
          W(ee, fe),
          W(ee + 1, fe),
          W(ee + 1, fe + 1),
      ...
      const F = u.length - v, X = c, V = m, ae = [
        ...
        for (let me = 0; me < (S + 1) * (S + 1); me++) if (Math.abs(x[me][0] - fe) < C * 0.3 && Math.abs(x[me][1] - ee) < O * 0.3 && Math.abs(x[me][2]) < 1e-6) return me;
        return -1;
      };
      for (const [fe, ee] of K) {
        ...
        for (let rt = 0; rt <= X; rt++) {
          ...
          for (let _t = 0; _t <= V; _t++) {
            const Rt = _t / V, Bt = me + Rt * (se - me), Sn = we + Rt * (Se - we);
            if (rt === 0) {
              const Vt = pe(Bt, Sn);
              if (Vt >= 0) {
                ct.push(Vt);
            ...
            let an = -1;
            for (let Vt = 0; Vt < x.length; Vt++) if (Math.abs(x[Vt][0] - Bt) < 1e-6 && Math.abs(x[Vt][1] - Sn) < 1e-6 && Math.abs(x[Vt][2] - Ct) < 1e-6) {
              an = Vt;
              break;
            }
      ...
      for (let fe = 0; fe < (S + 1) * (S + 1); fe++) {
        const ee = x[fe][0], me = x[fe][1];
        for (const [we, se] of g) {
          const Se = Math.sqrt((ee - we) * (ee - we) + (me - se) * (me - se));
          Se >= I * 0.5 && Se <= I * 2 && Ie.set(fe, [
            true,
            true,
            true,
            true,
      ...
      for (let fe = 0; fe < x.length; fe++) Math.abs(x[fe][2] - l) < 1e-6 && _.push(fe);
      ...
      for (let fe = v; fe < v + F; fe++) oe.elasticities.set(fe, w), oe.poissonsRatios.set(fe, y), oe.thicknesses.set(fe, a), oe.shearModuli.set(fe, p);
      for (let fe = Q; fe < Q + ce; fe++) oe.elasticities.set(fe, w), oe.poissonsRatios.set(fe, y), oe.thicknesses.set(fe, o), oe.shearModuli.set(fe, p);
      console.log(`Col+Placa 3D: col ${t * 1e3}x${n * 1e3}x${o * 1e3}mm, h=${l}m`), console.log(`  Placa ${s * 1e3}x${d * 1e3}mm, t=${a * 1e3}mm, 4 pernos d=${r * 1e3}mm`), console.log(`  ${F} Q4 placa + ${ce} Q4 columna = ${u.length} total`), console.log(`  ${x.length} nodos, P=${h} kN`);
      try {
        const fe = kt(x, u, {
          supports: Ie,
          loads: Pe
        }, oe), ee = gn(x, u, oe, fe);
        e.nodes.val = x, e.elements.val = u, fe && e.deformOutputs && (e.deformOutputs.val = fe), e.nodeInputs && (e.nodeInputs.val = {
          supports: Ie,
          loads: Pe
        ...
        console.warn("Col+Placa failed:", fe.message), e.nodes.val = x, e.elements.val = u, e.nodeInputs && (e.nodeInputs.val = {
          supports: Ie,
          loads: Pe
        });
      }
    ...
    function ws() {
      ...
      try {
        ...
        console.log(`Talud SRM: ${k.length} nodos, ${h.length} triangulos`), console.log(`  H=${t}, angulo=${n}\xB0, c=${f} kPa, \u03C6=${i}\xB0, \u03B3=${r}`), console.log("  \u2550\u2550\u2550 Strength Reduction Method (Mohr-Coulomb) \u2550\u2550\u2550"), console.log(`  FOS = ${u.fos.toFixed(3)}`), console.log(`  max|u| = ${W.toExponential(4)}`), console.log(`  max \u03B5_pl = ${g.toExponential(4)}`), u.fos < 1 ? console.warn("  \u26A0 TALUD INESTABLE (FOS < 1.0)") : u.fos < 1.5 && console.warn("  \u26A0 FOS < 1.5 \u2014 revisar estabilidad");
      } catch (R) {
    ...
    function Es(t, n, o, l, s) {
      ...
      if (Ne.vigaMat === 0) {
        ...
        t.on("change", (r) => {
          ...
          i && (n[parseInt(i[1])].b = Mt(r.value), Re()), c && (n[parseInt(c[1])].h = Mt(r.value), Re());
        });
      } else if (d <= 1) {
        for (let r = 0; r < n.length; r++) {
          const f = {};
        ...
        t.on("change", (r) => {
          ...
          for (let i = 0; i < n.length; i++) {
            const c = `${o}${i}`;
            f === `bf${c}` && (n[i].bf = Mt(r.value), Re()), f === `h${c}` && (n[i].hf = Mt(r.value), Re()), f === `tf${c}` && (n[i].tf = Mt(r.value), Re()), f === `tw${c}` && (n[i].tw = Mt(r.value), Re());
          }
        });
      } else {
        for (let r = 0; r < n.length; r++) {
        ...
        t.on("change", (r) => {
          ...
          for (let i = 0; i < n.length; i++) {
            const c = `${o}${i}`;
            f === `bc${c}` && (n[i].bc = Mt(r.value), Re()), f === `hc${c}` && (n[i].hc = Mt(r.value), Re()), f === `t${c}` && (n[i].t = Mt(r.value), Re());
          }
        });
      }
    }
    function Nn() {
      ...
      if (!n) return;
      n.style.display = "";
      const o = $, l = Math.round(((_a2 = Z.nPisos) == null ? void 0 : _a2.val) ?? 3), s = ya(), d = $a(), a = re.length || 1, r = te.length || 1;
      ...
      if (At.addBinding(f, "colMat", {
        ...
        At.addBinding(u, "fc", {
          ...
          var _a3;
          ((_a3 = S.target) == null ? void 0 : _a3.key) === "fc" && (Ne.fc = To(S.value), Re());
        });
      } else if (Ne.colMat === 1) {
        const x = {
      ...
      for (let x = 0; x < l; x++) {
        ...
        if (Ne.colMat === 0) if (Ne.colShape === 1) {
          ...
          const O = +Tn(1e8).toFixed(0), W = +Tn(3e8).toFixed(0), g = Math.max(1, Math.round((W - O) / 200));
          S.addBinding(C, "Es", {
            min: O,
            max: W,
            step: g,
            ...
            ((_a3 = E.target) == null ? void 0 : _a3.key) === "bc" && (u.colBc = Mt(E.value), Re()), ((_b = E.target) == null ? void 0 : _b.key) === "hc" && (u.colHc = Mt(E.value), Re()), ((_c = E.target) == null ? void 0 : _c.key) === "t" && (u.colT = Mt(E.value), Re()), ((_d = E.target) == null ? void 0 : _d.key) === "Es" && (u.colEs = To(E.value), Re()), ((_e2 = E.target) == null ? void 0 : _e2.key) === "nuS" && (u.colNuS = E.value, Re()), ((_f = E.target) == null ? void 0 : _f.key) === "fc" && (u.colFc = To(E.value), Re()), ((_g = E.target) == null ? void 0 : _g.key) === "nuC" && (u.colNuC = E.value, Re());
          });
        }
      ...
      const A = re.length || 1, I = te.length || 1, T = A + 1, R = I + 1;
      if (A > 0) {
        const x = h.addFolder({
        ...
        for (let u = 0; u < A; u++) for (let S = 0; S < R; S++) {
          const C = `wx_${u}_${S}`, O = xe.some((E) => E.dir === "x" && E.bay === u && E.axisIdx === S), W = {};
          W[C] = O;
          ...
          x.addBinding(W, C, {
            ...
            }) : xe = xe.filter((v) => !(v.dir === "x" && v.bay === u && v.axisIdx === S)), Re();
          });
        }
      }
      if (I > 0) {
        ...
        for (let u = 0; u < I; u++) for (let S = 0; S < T; S++) {
          const C = `wy_${u}_${S}`, O = xe.some((E) => E.dir === "y" && E.bay === u && E.axisIdx === S), W = {};
          W[C] = O;
          ...
          x.addBinding(W, C, {
            ...
            }) : xe = xe.filter((v) => !(v.dir === "y" && v.bay === u && v.axisIdx === S)), Re();
          });
        }
      }
      if (xe.length > 0) {
          ...
          if (r && Z[r]) {
            if (Z[r].val = a.value, z === "edificio" && (r === "nVanosX" || r === "nVanosY" || r === "nPisos")) {
              if (r === "nVanosX" || r === "nVanosY") {
                ...
                for (; re.length < f; ) re.push(re[re.length - 1] ?? $.defaultSpan);
                for (re.length > f && (re.length = f); te.length < i; ) te.push(te[te.length - 1] ?? $.defaultSpan * 0.8);
      ...
      if (z === "edificio") {
        ...
        if (s) {
          let d = function() {
            ...
            let w = "X: ";
            i > 0 && (w += `\u251C${i.toFixed(1)}\u2524`);
            for (let h = 0; h < re.length; h++) w += `[${f[h + (i > 0 ? 1 : 0)]}]\u2500\u2500${re[h].toFixed(1)}\u2500\u2500`;
            w += `[${f[re.length + (i > 0 ? 1 : 0)]}]`, c > 0 && (w += `\u251C${c.toFixed(1)}\u2524`);
            let y = "Y: ";
            m > 0 && (y += `\u251C${m.toFixed(1)}\u2524`);
            for (let h = 0; h < te.length; h++) y += `[${h + 1 + (m > 0 ? 1 : 0)}]\u2500\u2500${te[h].toFixed(1)}\u2500\u2500`;
            y += `[${te.length + 1 + (m > 0 ? 1 : 0)}]`, M > 0 && (y += `\u251C${M.toFixed(1)}\u2524`);
            let p = "Z: ";
...
`);
    ...
    function Qn() {
      ...
      if (!t) return;
      const n = ((_a2 = Ce.frequencies) == null ? void 0 : _a2[ve]) || 1, o = Math.max(0.5, Math.min(3, n * 0.1)), l = performance.now(), s = ke.length, d = e.elements.rawVal, { extent: a } = yn(), r = Ae.querySelector("#cad3d-modal-scale"), f = r && parseFloat(r.value) || 5;
      let i = 0;
      for (let I = 0; I < s; I++) {
        const T = t[I * 6] || 0, R = t[I * 6 + 1] || 0, x = t[I * 6 + 2] || 0;
        i = Math.max(i, Math.sqrt(T * T + R * R + x * x));
      }
      ...
      function A() {
        ...
        for (let R = 0; R < s; R++) {
          const x = ke[R];
          p[R * 3] = x[0] + (t[R * 6] || 0) * T, p[R * 3 + 1] = x[1] + (t[R * 6 + 1] || 0) * T, p[R * 3 + 2] = x[2] + (t[R * 6 + 2] || 0) * T;
        }
        if (M) {
          const R = M.geometry, x = R.getAttribute("position");
          x && x.array.length === p.length ? (x.array.set(p), x.needsUpdate = true) : R.setAttribute("position", new Ln(p.slice(), 3));
        }
        if (w) {
          ...
          const R = w.geometry, x = R.getAttribute("position");
          x && x.array.length === k.length ? (x.array.set(k), x.needsUpdate = true) : R.setAttribute("position", new Ln(k.slice(), 3));
        }
        if (y) {
          const R = [];
          for (const x of d) if (x.length === 3) {
            const [u, S, C] = x;
            R.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), R.push(p[S * 3], p[S * 3 + 1], p[S * 3 + 2]), R.push(p[C * 3], p[C * 3 + 1], p[C * 3 + 2]);
          } else if (x.length === 4) {
            const [u, S, C, O] = x;
            R.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), R.push(p[S * 3], p[S * 3 + 1], p[S * 3 + 2]), R.push(p[C * 3], p[C * 3 + 1], p[C * 3 + 2]), R.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), R.push(p[C * 3], p[C * 3 + 1], p[C * 3 + 2]), R.push(p[O * 3], p[O * 3 + 1], p[O * 3 + 2]);
          }
          if (R.length > 0) {
            const x = y.geometry, u = new Float32Array(R), S = x.getAttribute("position");
    ...
    function Fo() {
      ...
      if (!o.loads || o.loads.size === 0) {
        ...
        const R = (E) => Math.round(E * 1e3) / 1e3, x = /* @__PURE__ */ new Set();
        o.supports.forEach((E, v) => {
          x.add(`${R(t[v][0])},${R(t[v][1])}`);
        });
        const u = /* @__PURE__ */ new Set();
        for (const E of T) x.has(`${R(t[E][0])},${R(t[E][1])}`) && u.add(E);
        const S = /* @__PURE__ */ new Set(), C = /* @__PURE__ */ new Set();
        if (k !== 0 || A !== 0) {
          let E = -1 / 0, v = -1 / 0;
          for (const X of u) E = Math.max(E, R(t[X][0])), v = Math.max(v, R(t[X][1]));
          ...
          F.forEach((X) => {
            if (k !== 0) {
              ...
              for (const ae of X) if (R(t[ae][0]) === E) {
                const Q = R(t[ae][1]);
        ...
        for (let E = 0; E < n.length; E++) {
          ...
          if (!(Math.abs(F) < 1e-15)) {
            if (v.length === 2) {
              const X = ((_b = l.areas) == null ? void 0 : _b.get(E)) ?? 0, V = t[v[0]], ae = t[v[1]], Q = Math.sqrt((ae[0] - V[0]) ** 2 + (ae[1] - V[1]) ** 2 + (ae[2] - V[2]) ** 2), pe = -(F * X * Q * O) / 2;
        ...
        for (const E of T) {
          const v = S.has(E) ? k : 0, F = C.has(E) ? A : 0, X = W.get(E) ?? 0, V = g.has(E) ? h : 0, ae = X + V;
      ...
      try {
        ...
        if (y) {
          ...
          const T = performance.now(), R = gn(t, n, l, y), x = performance.now() - T;
          R && (e.analyzeOutputs.val = R, w(`<span style="color:#888">4.</span> Fuerzas internas: <span style="color:#00cc88">${x.toFixed(0)} ms</span>`), w("&nbsp;&nbsp;&nbsp;<i>F<sub>int</sub> = K<sub>local</sub> &middot; T &middot; u</i>"));
    ...
    function Ia(t, n, o, l, s, d, a) {
      const f = 4700 * Math.sqrt(d / 1e3) * 1e3 / l, i = t - 2 * o, c = n - 2 * o, m = t * n - i * c, M = (t * n * n * n - i * c * c * c) / 12, w = (n * t * t * t - c * i * i * i) / 12, y = i * c, p = i * c * c * c / 12, h = c * i * i * i / 12, k = m + f * y, A = M + f * p, I = w + f * h, T = l / (2 * (1 + s)), R = (t - o) * (n - o), x = 2 * ((t - o) / o + (n - o) / o), u = 4 * R * R / (x > 0 ? x : 1);
      return {
        A: k,
        Iz: A,
        Iy: I,
      ...
      if ((z === "edificio" || z === "frame") && B.size > 0) {
        ...
        for (let p = 0; p < t.length; p++) {
          if (de.has(p)) {
            ...
            o.elasticities.set(p, S), o.poissonsRatios.set(p, C), o.thicknesses.set(p, De), o.shearModuli.set(p, S / (2 * (1 + C))), o.densities.set(p, 24 / 9.80665);
            continue;
          }
          if (zt.has(p)) {
            const S = 4700 * Math.sqrt(r / 1e3) * 1e3, C = 0.2;
          ...
          let I, T, R, x;
          if (h) if (s === 0) T = i, R = c, x = m, I = a === 1 ? ks(A.dCol) : Ro(A.bCol, A.hCol), o.sectionShapes.set(p, a === 1 ? {
            type: "circ",
            d: A.dCol
          } : {
            type: "rect",
            ...
            if (S <= 1) {
              ...
              const C = A.colBf ?? 0.3, O = A.colHf ?? 0.3, W = A.colTf ?? 0.02, g = A.colTw ?? 0.012;
              I = Po(C, O, W, g);
              ...
              const C = A.colBc ?? 0.3, O = A.colHc ?? 0.3, W = A.colT ?? 0.01;
              I = Oo(C, O, W);
              const g = `\u25A1${(O * 100).toFixed(0)}x${(C * 100).toFixed(0)}x${(W * 1e3).toFixed(0)}`;
              o.sectionShapes.set(p, {
                type: "HSS",
                b: C,
                h: O,
            ...
            const S = A.colBc ?? 0.3, C = A.colHc ?? 0.3, O = A.colT ?? 0.01, W = A.colFc ?? 28e3, g = A.colEs ?? 2e8, E = A.colNuS ?? 0.3;
            A.colNuC;
            const v = Ia(S, C, O, g, E, W);
            I = {
              A: v.A,
              Iz: v.Iz,
              Iy: v.Iy,
            ...
            if (d === 0) T = i, R = c, x = m, I = Ro(O.b, O.h), o.sectionShapes.set(p, {
              type: "rect",
              b: O.b,
              h: O.h
            });
          ...
          const u = ge.get(p);
          if (u) {
            if ((u.material ?? 1) === 0 ? (T = i, R = c, x = m) : (T = M, R = w, x = y), u.secType === "rect" && u.b && u.h) I = Ro(u.b, u.h), o.sectionShapes.set(p, {
              type: "rect",
              b: u.b,
              h: u.h
            });
          ...
          o.elasticities.set(p, T), o.shearModuli.set(p, R), o.areas.set(p, I.A), o.momentsOfInertiaZ.set(p, I.Iy), o.momentsOfInertiaY.set(p, I.Iz), o.torsionalConstants.set(p, I.J), o.densities.set(p, x), u && u.releases12 && u.releases12.some((S) => S) && (o.momentReleases || (o.momentReleases = /* @__PURE__ */ new Map()), o.momentReleases.set(p, u.releases12)), u && u.springs12 && u.springs12.some((S) => S > 0) && (o.partialFixitySprings || (o.partialFixitySprings = /* @__PURE__ */ new Map()), o.partialFixitySprings.set(p, u.springs12));
        }
    ...
    window.innerWidth <= 600 && Ae.classList.add("collapsed"), setTimeout(() => {
      ...
      function n(g) {
        ...
        if (g === "test-scordelis" || g === "test-all") {
          ...
          for (let nt = 0; nt < Rt.length; nt++) Bt.elasticities.set(nt, 432e6), Bt.shearModuli.set(nt, Sn), Bt.thicknesses.set(nt, 0.25), Bt.poissonsRatios.set(nt, 0);
          ...
          try {
            const nt = kt(_t, Rt, {
              supports: an,
              loads: Vt
            }, Bt), St = Ct, en = ((_f2 = (_e3 = nt.deformations) == null ? void 0 : _e3.get(St)) == null ? void 0 : _f2[2]) ?? 0;
            oe.push({
      ...
      function o(g) {
        ...
        v.push("");
        const ce = (_) => {
          const ue = g.nodes[_], oe = `${ue[0]},${ue[2]}`;
          return {
            pt: K.get(oe) || "1",
        ...
        const Ie = [];
        if (g.elements.forEach((_, ue) => {
          if (_.length !== 2) return;
          const oe = g.nodes[_[0]], fe = g.nodes[_[1]], ee = Math.abs(fe[1] - oe[1]), me = Math.sqrt((fe[0] - oe[0]) ** 2 + (fe[2] - oe[2]) ** 2), we = ee > me * 0.5, se = ce(_[0]), Se = ce(_[1]), je = we ? "COL30" : "VIGA";
        ...
        v.push("$ POINT ASSIGNS"), g.nodes.forEach((_, ue) => {
...
`);
      ...
      function l(g) {
        ...
        v.push(`"""ETABS API Validation: ${g.name}`), v.push('Generated by Awatif FEM Studio"""'), v.push("import comtypes.client, time, math"), v.push(""), v.push("helper = comtypes.client.CreateObject('ETABSv1.Helper')"), v.push("helper = helper.QueryInterface(comtypes.gen.ETABSv1.cHelper)"), v.push('myETABS = helper.CreateObjectProgID("CSI.ETABS.API.ETABSObject")'), v.push("myETABS.ApplicationStart()"), v.push("time.sleep(10)"), v.push("SapModel = myETABS.SapModel"), v.push("SapModel.InitializeNewModel()"), v.push("SapModel.File.NewBlank()"), v.push("SapModel.SetPresentUnits(12)  # tonf_m_C"), v.push(""), v.push(`E = ${E}`), v.push('SapModel.PropMaterial.SetMaterial("CONC", 2)'), v.push('SapModel.PropMaterial.SetMPIsotropic("CONC", E, 0.2, 5.5e-6)'), v.push('SapModel.PropFrame.SetRectangle("COL30", "CONC", 0.30, 0.30)'), v.push('SapModel.PropFrame.SetRectangle("VIGA", "CONC", 0.40, 0.25)'), g.elements.some((V) => V.length === 4) && v.push('SapModel.PropArea.SetWall("Muro20", 6, False, "CONC", 0.20)'), v.push(""), v.push("# Add elements"), v.push("FN = ' '"), g.elements.forEach((V, ae) => {
          if (V.length === 2) {
            const Q = g.nodes[V[0]], K = g.nodes[V[1]], pe = Math.abs(K[1] - Q[1]), ce = Math.sqrt((K[0] - Q[0]) ** 2 + (K[2] - Q[2]) ** 2), Ie = pe > ce * 0.5 ? "COL30" : "VIGA";
...
`);
      ...
      function c(g, E) {
        ...
        <div style="display:flex;gap:8px">
          ...
          K.addEventListener("change", () => {
            const pe = K.dataset.ifcCat, ce = g.detailCategories.get(pe);
            if (ce) {
          ...
          for (const oe of E.elements) if (K.has(oe.category) && oe.type === "frame" && oe.nodeIds.length >= 2) {
            ce.push(oe.nodeIds);
            const fe = ((_a4 = E.materials) == null ? void 0 : _a4.get(oe.material)) || {
              E: 2132888792e-2,
              nu: 0.2,
              rho: 2.4
          ...
          for (const [, pe] of g.detailCategories) {
            const ce = Ze();
      ...
      function m(g) {
        ...
        const E = /* @__PURE__ */ new Map();
        for (let ce = 0; ce < g.stories.length; ce++) E.set(g.stories[ce].name, ce);
        for (let ce = 0; ce < g.elementTypes.length; ce++) {
          const Ie = g.elementTypes[ce], Pe = g.elementStories[ce], _ = E.get(Pe) ?? 0;
          $e.set(ce, _), Ie === "COLUMN" || Ie === "BRACE" ? B.add(ce) : G.add(ce);
        }
        z = "edificio";
        const v = g.grids.filter((ce) => ce.dir === "X").sort((ce, Ie) => ce.coord - Ie.coord), F = g.grids.filter((ce) => ce.dir === "Y").sort((ce, Ie) => ce.coord - Ie.coord);
        let X, V, ae, Q;
        if (v.length > 0 || F.length > 0) X = v.map((ce) => ce.coord), V = F.map((ce) => ce.coord), ae = v.map((ce) => ce.label), Q = F.map((ce) => ce.label);
        else {
          const ce = new Set(g.nodes.map((Pe) => Pe[0])), Ie = new Set(g.nodes.map((Pe) => Pe[1]));
          X = [
            ...ce
          ].sort((Pe, _) => Pe - _), V = [
            ...Ie
        ...
        const K = g.stories.length > 0 ? Math.max(...g.stories.map((ce) => ce.elev)) : Math.max(...g.nodes.map((ce) => ce[2]));
        ...
        for (const ce of g.elementTypes) pe[ce]++;
        console.log(`E2K grids: X=[${ae.join(",")}] Y=[${Q.join(",")}]`), console.log(`E2K stories: ${g.stories.map((ce) => `${ce.name}@${ce.elev.toFixed(2)}`).join(", ")}`), console.log(`E2K elements: ${pe.COLUMN} columns, ${pe.BEAM} beams, ${pe.BRACE} braces`), Qe();
      }
      function M(g, E) {
        const v = new Blob([
          g
      ...
      w && (w.value = b, w.addEventListener("change", (g) => {
        g.stopPropagation(), b = w.value, $ = Rn(b, P), z && et(z);
      }));
      const y = Ae.querySelector("#cad3d-length-unit");
      y && (y.value = P, y.addEventListener("change", (g) => {
        g.stopPropagation(), P = y.value, $ = Rn(b, P), z && et(z);
      })), Ae.querySelectorAll("[data-preset]").forEach((g) => {
        g.addEventListener("click", (E) => {
          E.stopPropagation();
          const v = g.dataset.preset, F = D[v];
          F && (b = F.force, P = F.length, J = F.stress, $ = Rn(b, P), w && (w.value = b), y && (y.value = P), Ae.querySelectorAll("[data-preset]").forEach((X) => {
            X.classList.toggle("active", X.dataset.preset === v);
          }), z && et(z), console.log(`Preset: ${v} \u2192 ${b}+${P}, stress: ${J.label}`));
        });
        ...
        g.stopPropagation(), ea(async () => {
          const { openCalcPanel: E } = await import("./calcPanel-DKQr_H-y.js").then(async (m2) => {
            await m2.__tla;
            return m2;
          });
          return {
        ...
        } else Co(), v.style.display = "none", F.style.display = "none", X.style.display = "none", V && (V.style.display = "none"), z && z !== "placa-q4" && z !== "placa-3q" && Re(), setTimeout(() => {
          var _a4;
          const ae = Ze();
      ...
      function p() {
        ...
        const E = Ce.frequencies[ve], v = E > 0 ? 1 / E : 0, F = [
          0,
          0,
          0,
          0,
        ...
        g.textContent = `Modo ${ve + 1} \u2014 ${E.toFixed(2)} Hz \u2014 T=${v.toFixed(3)}s \u2014 \u03A3Ux=${(F[0] * 100).toFixed(1)}% \u03A3Uy=${(F[1] * 100).toFixed(1)}% \u03A3Rz=${(F[5] * 100).toFixed(1)}%`;
      }
      (_o2 = Ae.querySelector("#cad3d-mode-prev")) == null ? void 0 : _o2.addEventListener("click", (g) => {
        ...
        const E = Ce.modeShapes[ve], { extent: v } = yn();
        let F = 0;
      ...
      const k = Ae.querySelector("#cad3d-cli-toggle"), A = Ae.querySelector("#cad3d-cli-panel"), I = Ae.querySelector("#cad3d-cli-output"), T = Ae.querySelector("#cad3d-cmd"), R = [];
      let x = -1;
...
`));
        ...
        if ((g.key === "Delete" || g.key === "Backspace") && ht.size > 0) {
          g.preventDefault(), ht.forEach((E) => U.add(E)), ht.clear(), fn && (fn.remove(), fn = null), Re();
          return;
        }
        if (g.key === "Escape") {
          if (pn) if (vt !== null) {
        ...
        if (g.stopPropagation(), g.key === "Enter") {
          ...
          if (E) {
            R.unshift(E), x = -1, I && (I.textContent += `> ${E}
`);
            try {
...
`);
        ...
        } else g.key === "ArrowUp" ? (g.preventDefault(), R.length > 0 && x < R.length - 1 && (x++, T.value = R[x])) : g.key === "ArrowDown" && (g.preventDefault(), x > 0 ? (x--, T.value = R[x]) : (x = -1, T.value = ""));
      });
      let u = false, S = 0, C = 0, O = 0, W = 0;
      Ae.addEventListener("mousedown", (g) => {
        ...
        Ae.style.bottom = "unset", S = g.clientX, C = g.clientY, O = v.left, W = v.top, g.preventDefault();
      }), window.addEventListener("mousemove", (g) => {
        u && (g.preventDefault(), Ae.style.left = O + (g.clientX - S) + "px", Ae.style.top = W + (g.clientY - C) + "px");
...
${i.join(", ")}`);
        ...
        Bn(), ht.forEach((o) => U.add(o)), ht.clear(), wn(), En(), Re();
    ...
    function za(t) {
      ...
      for (const [d, a] of s) {
        ...
        c.computeLineDistances(), c.renderOrder = 9990, n.scene.add(c), Bo.push(c);
      }
      n.render();
    }
    function Ho() {
      const t = Ze();
      for (const n of Bo) t == null ? void 0 : t.scene.remove(n), n.geometry.dispose(), n.material.dispose();
      Bo = [], zn = null, Qt && (Qt.remove(), Qt = null);
    }
    function Cs(t, n, o, l) {
    ...
    function Ca(t) {
      ...
      if (jt === "arc") {
        ...
        if (n === vt || n === Zt) return;
        const o = e.nodes.val, l = new Oe(...o[vt]), s = new Oe(...o[Zt]), d = new Oe(...o[n]), a = Math.max(4, Math.round(((_a2 = Z.nSubViga) == null ? void 0 : _a2.val) ?? 8)), r = Ps(l, s, d, a);
        Bn();
        const f = [
          ...e.nodes.val
        ], i = [
    ...
    function Fa(t, n, o, l, s, d, a) {
      ...
      <div>${L("k", "z")} = ${Be("4\xB7" + L("E") + "\xB7" + L("I", "z") + "\xB7(1+\u03C6/4)", L("L") + "\xB7(1+\u03C6<sub>z</sub>)")} = <span class="highlight">${Me(p)}</span> &nbsp;(flexi\xF3n diagonal)</div>
      ...
      <div>${Be(L("G") + "\xB7" + L("J"), L("L"))} = <span class="highlight">${Me(M)}</span> &nbsp;(torsi\xF3n)</div>
      ${k ? "" : `
      <div>${Be("12\xB7" + L("E") + "\xB7" + L("I", "z"), L("L") + "\xB3")} = <span class="highlight">${Me(w)}</span></div>
      <div>${Be("4\xB7" + L("E") + "\xB7" + L("I", "z"), L("L"))} = <span class="highlight">${Me(p)}</span></div>
      `}
    </div>
    <div class="fem-eq">
    ...
    function Ra(t) {
      if (t.length === 2) {
        const o = xn(t[1], t[0]), l = Pn(o), s = o[0] / l, d = o[1] / l, a = o[2] / l;
        return `<div class="fem-eq eq-box">
        <div style="text-align:left;margin-bottom:4px"><strong style="color:var(--fem-section-title)">Cosenos directores:</strong></div>
        <div>${L("l")} = cos(\u03B1) = ${Be("\u0394x", L("L"))} = ${Be(Me(o[0]), Me(l))} = <span class="highlight">${Me(s)}</span></div>
        <div>${L("m")} = cos(\u03B2) = ${Be("\u0394y", L("L"))} = ${Be(Me(o[1]), Me(l))} = <span class="highlight">${Me(d)}</span></div>
        <div>${L("n")} = cos(\u03B3) = ${Be("\u0394z", L("L"))} = ${Be(Me(o[2]), Me(l))} = <span class="highlight">${Me(a)}</span></div>
      </div>
      <div class="fem-eq">
    ...
    function Ns() {
      const t = "0", n = Be(L("EA"), L("L")), o = Be("\u2212" + L("EA"), L("L")), l = Be("12" + L("EI", "z"), L("L") + "\xB3"), s = Be("\u221212" + L("EI", "z"), L("L") + "\xB3"), d = Be("12" + L("EI", "y"), L("L") + "\xB3"), a = Be("\u221212" + L("EI", "y"), L("L") + "\xB3"), r = Be("6" + L("EI", "z"), L("L") + "\xB2"), f = Be("\u22126" + L("EI", "z"), L("L") + "\xB2"), i = Be("6" + L("EI", "y"), L("L") + "\xB2"), c = Be("\u22126" + L("EI", "y"), L("L") + "\xB2"), m = Be(L("GJ"), L("L")), M = Be("\u2212" + L("GJ"), L("L")), w = Be("4" + L("EI", "z"), L("L")), y = Be("2" + L("EI", "z"), L("L")), p = Be("4" + L("EI", "y"), L("L")), h = Be("2" + L("EI", "y"), L("L")), k = '<span style="color:var(--fem-eq-dots);font-style:italic">sym</span>', A = [
        "P\u2081",
        "P\u2082",
        "P\u2083",
        "P\u2084",
      ...
      for (const x of I) R += `<td class="hdr">${x}</td>`;
      R += "</tr>";
      for (let x = 0; x < 12; x++) {
        R += `<tr><td class="hdr">${A[x]}</td>`;
        for (let u = 0; u < 12; u++) if (u < x) R += `<td style="color:var(--fem-border-cell)">${u === 0 && x > 0 ? k : ""}</td>`;
        else {
    ...
    function qs(t) {
      ...
      if (d) {
        const x = Pn(xn(s[1], s[0])), u = ((_d = a.elasticities) == null ? void 0 : _d.get(t)) ?? 0, S = ((_e2 = a.areas) == null ? void 0 : _e2.get(t)) ?? 0, C = ((_f = a.momentsOfInertiaZ) == null ? void 0 : _f.get(t)) ?? 0, O = ((_g = a.momentsOfInertiaY) == null ? void 0 : _g.get(t)) ?? 0, W = ((_h = a.shearModuli) == null ? void 0 : _h.get(t)) ?? 0, g = ((_i = a.torsionalConstants) == null ? void 0 : _i.get(t)) ?? 0, E = ((_j = a.momentReleases) == null ? void 0 : _j.get(t)) || [], v = ((_k = a.partialFixitySprings) == null ? void 0 : _k.get(t)) || [], F = [
          "P (Axial)",
          "V2 (Corte)",
          "V3 (Corte)",
          "T (Torsi\xF3n)",
        ...
        let X = "";
        for (let V = 0; V < 6; V++) {
          const ae = V, Q = V + 6, K = (E.length >= 12 ? E[ae] : V >= 3 && E.length >= 6 && E[V - 3]) ? "checked" : "", pe = (E.length >= 12 ? E[Q] : V >= 3 && E.length >= 6 && E[V]) ? "checked" : "", ce = v.length >= 12 && v[ae] > 0 ? v[ae].toFixed(1) : "", Ie = v.length >= 12 && v[Q] > 0 ? v[Q].toFixed(1) : "";
          X += `<tr>
        ...
        `${l[0]}${l[1]}${Me(x)}${Me(u)}${Me(S)}${Me(C)}${Me(O)}${Me(W)}${Me(g)}${X}`;
      } else {
        const x = ((_l2 = a.elasticities) == null ? void 0 : _l2.get(t)) ?? 0, u = ((_m = a.thicknesses) == null ? void 0 : _m.get(t)) ?? 0, S = ((_n2 = a.poissonsRatios) == null ? void 0 : _n2.get(t)) ?? 0, C = x / (2 * (1 + S)), O = l.length === 4, W = x / (1 - S * S);
        `${l.length}${l.join(", ")}${Me(x)}${Me(C)}${Me(u)}${Me(S)}`, O && (M = `<div class="fem-eq eq-box">
          ...
          <div>${L("D", "b")} = ${Be(L("E") + "\xB7" + L("t") + "\xB3", "12\xB7(1\u2212\u03BD\xB2)")} = <span class="highlight">${Me(x * u ** 3 / (12 * (1 - S ** 2)))}</span></div>
      ...
      try {
        if (k = vo(s, a, t), A = yo(s), I = on(os(A), on(k, A)), T = d ? [
          ...
          const C = Pn(xn(s[1], s[0])), O = ((_o2 = a.elasticities) == null ? void 0 : _o2.get(t)) ?? 0, W = ((_p = a.areas) == null ? void 0 : _p.get(t)) ?? 0, g = ((_q = a.momentsOfInertiaZ) == null ? void 0 : _q.get(t)) ?? 0, E = ((_r = a.momentsOfInertiaY) == null ? void 0 : _r.get(t)) ?? 0, v = ((_s2 = a.shearModuli) == null ? void 0 : _s2.get(t)) ?? 0, F = ((_t = a.torsionalConstants) == null ? void 0 : _t.get(t)) ?? 0;
          M = Fa(O, W, g, E, v, F, C);
        }
        w = Ra(s), y = Pa(), p = Oa(l), h = Na(d);
      ...
      if (r == null ? void 0 : r.deformations) {
        ...
        l.map((u, S) => {
          ...
          const C = ((_a3 = r.deformations) == null ? void 0 : _a3.get(u)) || [
            ...
            0,
            0
          ], O = x.map((W, g) => `<span class="prop-key">${W}</span>: <span class="${Math.abs(C[g]) > 1e-10 ? "result-val" : ""}">${Me(C[g])}</span>`).join(" &nbsp;");
          return `<div style="margin-bottom:2px"><strong>Nodo ${u}:</strong> ${O}</div>`;
        }).join("");
      }
      if (f && d && (r == null ? void 0 : r.deformations) && k && A) {
        const x = (_u = f.normals) == null ? void 0 : _u.get(t), u = (_v = f.shearsY) == null ? void 0 : _v.get(t), S = (_w = f.shearsZ) == null ? void 0 : _w.get(t), C = (_x = f.torsions) == null ? void 0 : _x.get(t), O = (_y = f.bendingsY) == null ? void 0 : _y.get(t), W = (_z = f.bendingsZ) == null ? void 0 : _z.get(t), g = [
          "ux",
          "uy",
          "uz",
          "\u03B8x",
        ...
        `${L("u", "global")}${l.map((Q, K) => `<span style="color:var(--fem-label)">nodo ${Q}:</span> ${g.map((pe, ce) => `<span style="color:${Math.abs(E[K * 6 + ce]) > 1e-10 ? "var(--fem-eq-var)" : "var(--fem-eq-dots)"}">${Me(E[K * 6 + ce])}</span>`).join(", ")}`).join(" | ")}${L("u", "local")}${L("T")}${L("u", "global")}${L("u", "local")}${X(v, [
          ...g,
          ...g
        ...
        const x = (_B = f.normals) == null ? void 0 : _B.get(t), u = (_C = f.shearsY) == null ? void 0 : _C.get(t), S = (_D = f.shearsZ) == null ? void 0 : _D.get(t), C = (_E = f.torsions) == null ? void 0 : _E.get(t), O = (_F = f.bendingsY) == null ? void 0 : _F.get(t), W = (_G = f.bendingsZ) == null ? void 0 : _G.get(t);
        `${x ? x.map((g) => Me(g)).join(", ") : "\u2014"}${u ? u.map((g) => Me(g)).join(", ") : "\u2014"}${S ? S.map((g) => Me(g)).join(", ") : "\u2014"}${C ? C.map((g) => Me(g)).join(", ") : "\u2014"}${O ? O.map((g) => Me(g)).join(", ") : "\u2014"}${W ? W.map((g) => Me(g)).join(", ") : "\u2014"}`;
      } else if (f && !d) {
        const x = (_H = f.bendingXX) == null ? void 0 : _H.get(t), u = (_I = f.bendingYY) == null ? void 0 : _I.get(t), S = (_J = f.bendingXY) == null ? void 0 : _J.get(t), C = (_K = f.membraneXX) == null ? void 0 : _K.get(t), O = (_L = f.membraneYY) == null ? void 0 : _L.get(t), W = (_M = f.membraneXY) == null ? void 0 : _M.get(t);
        `${x ? x.map((g) => Me(g)).join(", ") : "\u2014"}${u ? u.map((g) => Me(g)).join(", ") : "\u2014"}${S ? S.map((g) => Me(g)).join(", ") : "\u2014"}${C ? C.map((g) => Me(g)).join(", ") : "\u2014"}${O ? O.map((g) => Me(g)).join(", ") : "\u2014"}${W ? W.map((g) => Me(g)).join(", ") : "\u2014"}`;
      }
      `${l[0]}`, 6 * l[0], 6 * l[0] + 5, `${l[1]}`, 6 * l[1], 6 * l[1] + 5, l.length === 3 && (`${l[2]}`, 6 * l[2], 6 * l[2] + 5), n.length * 6, n.length * 6, Kt = vl(t, n, o, a, r, f), Kt.id = "fem-inspect-panel", document.body.appendChild(Kt), (_N = Kt.querySelector("#er-close")) == null ? void 0 : _N.addEventListener("click", () => An()), (_O = Kt.querySelector("#rel-apply")) == null ? void 0 : _O.addEventListener("click", () => {
        ...
        x.forEach((W) => {
          ...
          g > 0 && (C[parseInt(W.dataset.spr)] = g);
        }), a.momentReleases || (a.momentReleases = /* @__PURE__ */ new Map()), a.partialFixitySprings || (a.partialFixitySprings = /* @__PURE__ */ new Map()), S.some((W) => W) ? a.momentReleases.set(t, S) : a.momentReleases.delete(t), C.some((W) => W > 0) ? a.partialFixitySprings.set(t, C) : a.partialFixitySprings.delete(t), console.log(`Releases elem ${t}:`, S.map((W, g) => W ? relIds[g] : "").filter(Boolean).join(" ") || "none"), console.log(`Springs elem ${t}:`, C);
      ...
      const R = d ? (() => {
        var _a3, _b2, _c2, _d2, _e3, _f2;
        const x = Pn(xn(s[1], s[0])), u = ((_a3 = a.elasticities) == null ? void 0 : _a3.get(t)) ?? 0, S = ((_b2 = a.areas) == null ? void 0 : _b2.get(t)) ?? 0, C = ((_c2 = a.momentsOfInertiaZ) == null ? void 0 : _c2.get(t)) ?? 0, O = ((_d2 = a.momentsOfInertiaY) == null ? void 0 : _d2.get(t)) ?? 0, W = ((_e3 = a.shearModuli) == null ? void 0 : _e3.get(t)) ?? 0, g = ((_f2 = a.torsionalConstants) == null ? void 0 : _f2.get(t)) ?? 0;
        return qa(u, S, C, O, W, g, x);
      })() : void 0;
      Kt.querySelectorAll("[data-full]").forEach((x) => {
        x.addEventListener("click", (u) => {
          ...
          if (S === "kLocal" && k) {
            ...
            Jo(`Elemento ${t} \u2014 Rigidez Local k_local`, C, Yo(k, T), R);
          } else if (S === "T" && A) Jo(`Elemento ${t} \u2014 Transformaci\xF3n T`, w, Yo(A, T));
          else if (S === "kGlobal" && I) {
            const C = d ? Ns() : "<em>Shell 18\xD718</em>";
            Jo(`Elemento ${t} \u2014 Rigidez Global K = T^T \xB7 k \xB7 T`, C, Yo(I, T), R);
          }
        });
      });
    }
    ...
    function Hs() {
      ...
      for (let h = 0; h <= 12; h++) {
        ...
        for (let I = 0; I < 6; I++) {
          const T = A + 2 * Math.PI * I / 6, R = 5 * Math.cos(T), x = 5 * Math.sin(T);
          d.push([
            R,
            x,
            k
    ...
    function js() {
      ...
      for (let p = 0; p <= 20; p++) {
        ...
        for (let T = 0; T < 3; T++) {
          const R = T * 2 * Math.PI / 3 - Math.PI / 2, x = A * Math.cos(R), u = A * Math.sin(R), S = s.length;
          s.push([
            x,
            u,
            k
    ...
    function Ws() {
      ...
      for (let m = 0; m < 3; m++) {
        ...
        for (let A = 0; A <= 4; A++) {
          const I = A / 4, T = -p / 2 + p * I, R = y * (1 - I * I * 0.3);
          for (let x = 0; x <= 12; x++) {
            const u = x / 12, S = M + R * u, C = w * Math.sin(Math.PI * u) * (1 - I * I * 0.5), O = T;
            t.push([
              S,
              O,
              C
        ...
        for (let A = 0; A < 4; A++) for (let I = 0; I < 12; I++) {
          const T = h + A * k + I, R = h + A * k + I + 1, x = h + (A + 1) * k + I + 1, u = h + (A + 1) * k + I;
          n.push([
            T,
            R,
            x,
    ...
    function Vo() {
      ...
      for (let T = 0; T <= s; T++) for (let R = 0; R <= l; R++) m.push([
        R * i,
        0,
        T * c
      ]);
      ...
      const A = {
        elasticities: new Map(M.map((T, R) => [
          R,
          d
        ])),
        poissonsRatios: new Map(M.map((T, R) => [
          R,
          a
        ])),
        thicknesses: new Map(M.map((T, R) => [
          R,
          o
        ])),
        shearModuli: new Map(M.map((T, R) => [
          R,
          f
        ])),
        densities: new Map(M.map((T, R) => [
          R,
          24 / 9.80665
        ]))
      };
    ...
    function Ys() {
      ...
      try {
        ...
        if (I && e.deformOutputs) {
          ...
          const R = I.deformations.get(h), x = R ? R[2] : 0, u = o * n * n * n / 12, S = r * t * t * t / (3 * d * u);
          console.log(`Viga Q4: Uz_tip=${x.toExponential(4)} | Analitico=${S.toExponential(4)} | ratio=${(Math.abs(x) / S).toFixed(4)}`);
        }
      } catch (I) {
        console.warn("VigaQ4:", I.message);
      }
      const A = Ze();
      A && (A.settings.shellResults.val = "displacementZ"), setTimeout(() => wt(), 50), Qe();
    }
    function Js() {
      var _a2, _b, _c, _d, _e2, _f, _g, _h;
    ...
    `, document.body.appendChild(t), (_b = t.querySelector("#pushover-close")) == null ? void 0 : _b.addEventListener("click", () => t.remove()), (_c = t.querySelector("#pushover-run")) == null ? void 0 : _c.addEventListener("click", async () => {
        ...
        try {
          const { cyclicPushover: h } = await ea(async () => {
            const { cyclicPushover: A } = await import("./cyclicPushoverCpp-4nPop-zu.js").then(async (m2) => {
              await m2.__tla;
              return m2;
            });
            return {
          ...
          M.textContent = `Completado: ${k.nSteps} pasos`, Da(t.querySelector("#pushover-canvas"), k.displacements, k.forces, `Pushover: ${o * 100}x${l * 100}cm, f'c=${s / 1e3}MPa, Fy=${d / 1e3}MPa`);
        } catch (h) {
    ...
    function Da(t, n, o, l) {
      ...
      for (let T = 0; T <= 5; T++) {
        const R = c + A * T;
        s.fillText((R * 1e3).toFixed(1), h(R), a - r.bottom + 15);
      }
      s.textAlign = "right";
      const I = p / 5;
      for (let T = 0; T <= 5; T++) {
        const R = M + I * T;
        s.fillText(R.toFixed(0), r.left - 5, k(R) + 3);
      }
    }
    let eo = null;
    function Ha() {
    ...
    function ja(t) {
      ...
      const R = [];
      for (const V of f) {
        let ae = w, Q = y, K = p, pe = h, ce = k, Ie = A, Pe = I, _ = T, ue;
        const oe = V - m;
    ...
    `, document.body.appendChild(n), Dn = n;
    ...
    `, document.body.appendChild(w), Hn = w, w.querySelector("#ep-close").addEventListener("click", () => {
        ...
        U.add(t), w.remove(), Hn = null, An(), Re();
    ...
    setTimeout(() => {
      ...
      function a(f, i, c, m, M) {
        ...
        for (let T = 0; T < A.length; T++) {
          const R = A[T], x = R.map((u) => d(k[u])).filter(Boolean);
      ...
      function r(f, i, c, m, M, w, y, p) {
        ...
        const k = (A, I, T, R, x, u, S, C) => {
          const O = (T - A) * (C - u) - (R - I) * (S - x);
          if (Math.abs(O) < 1e-10) return false;
          const W = ((x - A) * (C - u) - (u - I) * (S - x)) / O, g = ((x - A) * (R - I) - (u - I) * (T - A)) / O;
      ...
      n.addEventListener("mousedown", (f) => {
        ...
        if (pn) {
          ...
          m.nodeIdx, Rs(m), n.style.cursor = m.snapType !== "free" ? "pointer" : "crosshair";
          return;
        }
        if (!nn && !Yt) return;
        if (Yt && o) {
        ...
        if (pn) {
          ...
          (c.worldPos || c.nodeIdx !== null) && (Ca(c), Rs(c));
          return;
        }
        if (Yt) {
          if (l) return;
          ...
          if (i >= 0) {
            ...
            else ht.clear(), wn(), ht.add(i), Do(i);
            En();
      ...
      } else Co(), Ae.querySelector("#cad3d-mode-prev").style.display = "none", Ae.querySelector("#cad3d-mode-next").style.display = "none", Ae.querySelector("#cad3d-mode-label").style.display = "none", z && z !== "placa-q4" && z !== "placa-3q" && Re(), setTimeout(() => {
        var _a3;
        const o = Ze();
      ...
      const n = Ce.modeShapes[ve], { extent: o } = yn();
      let l = 0;
      ...
      s && Ce.frequencies && (s.textContent = `Modo ${ve + 1} \u2014 ${Ce.frequencies[ve].toFixed(2)} Hz`), console.log(`Modo ${ve + 1}: f = ${(_a2 = Ce.frequencies) == null ? void 0 : _a2[ve].toFixed(4)} Hz`);
