...
`));
  ...
  function ml(e) {
    ...
    T += '<button class="rpt-close" id="rpt-close">\u2715 Close</button>', T += "<h1>Finite Element Analysis \u2014 Step-by-Step Report</h1>", T += '<div class="rpt-subtitle">Complete FEM derivation from element formulation to final results</div>', T += '<hr class="rpt-sep"/>', T += "<h2>1. Input Data</h2>", T += '<table class="rpt-info"><tbody>', T += `<tr><td>Number of nodes</td><td class="val">${b.length}</td></tr>`, T += `<tr><td>Number of elements</td><td class="val">${Y} (${H} frames, ${W} shells)</td></tr>`, T += '<tr><td>DOFs per node</td><td class="val">6 (u<sub>x</sub>, u<sub>y</sub>, u<sub>z</sub>, \u03B8<sub>x</sub>, \u03B8<sub>y</sub>, \u03B8<sub>z</sub>)</td></tr>', T += `<tr><td>Total DOFs</td><td class="val">${Z}</td></tr>`, T += "</tbody></table>", T += "<h3>1.1 Node Coordinates</h3>", T += '<table class="rpt-data"><thead><tr><th>Node</th><th>x</th><th>y</th><th>z</th></tr></thead><tbody>', b.forEach((ce, oe) => {
      T += `<tr><td>${oe}</td><td>${it(ce[0])}</td><td>${it(ce[1])}</td><td>${it(ce[2])}</td></tr>`;
      ...
      var _a2, _b2, _c2, _d2;
      const _ = ce.length === 2, D = ce.map((ve) => b[ve]), G = _ ? Po(vo(D[1], D[0])) : 0, ue = ((_a2 = M.elasticities) == null ? void 0 : _a2.get(oe)) ?? 0, Me = ((_b2 = M.areas) == null ? void 0 : _b2.get(oe)) ?? 0, ze = ((_c2 = M.momentsOfInertiaZ) == null ? void 0 : _c2.get(oe)) ?? 0, Ue = ((_d2 = M.momentsOfInertiaY) == null ? void 0 : _d2.get(oe)) ?? 0;
      T += `<tr><td>${oe}</td><td>${_ ? "Frame" : "Shell"}</td><td>${ce.join(" \u2192 ")}</td>`, T += `<td>${it(G)}</td><td>${it(ue)}</td><td>${it(Me)}</td><td>${it(ze)}</td><td>${it(Ue)}</td></tr>`;
    ...
    for (let ce = 0; ce < Y; ce++) {
      const oe = P[ce], _ = oe.map((dt) => b[dt]);
      if (!(oe.length === 2)) continue;
      const G = Po(vo(_[1], _[0])), ue = ((_a = M.elasticities) == null ? void 0 : _a.get(ce)) ?? 0, Me = ((_b = M.areas) == null ? void 0 : _b.get(ce)) ?? 0, ze = ((_c = M.momentsOfInertiaZ) == null ? void 0 : _c.get(ce)) ?? 0, Ue = ((_d = M.momentsOfInertiaY) == null ? void 0 : _d.get(ce)) ?? 0, ve = ((_e = M.shearModuli) == null ? void 0 : _e.get(ce)) ?? 0, je = ((_f = M.torsionalConstants) == null ? void 0 : _f.get(ce)) ?? 0;
      let We = null, ge = null, ke = null;
      try {
        We = vn(_, M, ce), ge = yn(_), ke = ao(ns(ge), ao(We, ge));
      } catch {
        continue;
      }
      const Ae = vo(_[1], _[0]), Ve = Ae[0] / G, ct = Ae[1] / G, Xe = Ae[2] / G;
      T += '<div class="rpt-elem-block">', T += `<h3 class="rpt-elem-title" data-toggle="elem${ce}">\u25B6 Element ${ce} \u2014 Nodes ${oe[0]} \u2192 ${oe[1]}, L = ${it(G)}</h3>`, T += `<div id="rpt-elem${ce}" class="rpt-elem-body" style="display:none">`, T += "<h4>Properties (numerical substitution)</h4>", T += '<div class="rpt-eq-small">', T += `E = ${it(ue)} &nbsp;&nbsp; A = ${it(Me)} &nbsp;&nbsp; I<sub>z</sub> = ${it(ze)} &nbsp;&nbsp; I<sub>y</sub> = ${it(Ue)} &nbsp;&nbsp; G = ${it(ve)} &nbsp;&nbsp; J = ${it(je)}<br/>`, T += `EA/L = ${it(ue)}\xB7${it(Me)}/${it(G)} = <b>${it(ue * Me / G)}</b><br/>`, T += `12EI<sub>z</sub>/L\xB3 = 12\xB7${it(ue)}\xB7${it(ze)}/${it(G)}\xB3 = <b>${it(12 * ue * ze / G ** 3)}</b><br/>`, T += `12EI<sub>y</sub>/L\xB3 = 12\xB7${it(ue)}\xB7${it(Ue)}/${it(G)}\xB3 = <b>${it(12 * ue * Ue / G ** 3)}</b><br/>`, T += `GJ/L = ${it(ve)}\xB7${it(je)}/${it(G)} = <b>${it(ve * je / G)}</b>`, T += "</div>", T += "<h4>Direction cosines</h4>", T += `<div class="rpt-eq-small">l = ${bn(Ve)}, m = ${bn(ct)}, n = ${bn(Xe)}, D = ${bn(Math.sqrt(Ve ** 2 + ct ** 2))}</div>`, T += "<h4>K<sub>local</sub> (12\xD712)</h4>", T += Kn(We, 12), T += "<h4>T \u2014 Transformation (12\xD712)</h4>", T += Kn(ge, 12), T += "<h4>K<sub>global</sub> = T\u1D40 \xB7 K<sub>local</sub> \xB7 T</h4>", T += Kn(ke, 12), T += "<h4>Assembly</h4>", T += `<div class="rpt-eq-small">Global DOFs: node ${oe[0]} \u2192 [${oe[0] * 6}..${oe[0] * 6 + 5}], node ${oe[1]} \u2192 [${oe[1] * 6}..${oe[1] * 6 + 5}]</div>`, T += "</div></div>";
    }
      ...
      for (let oe = 0; oe < Y; oe++) {
        ...
        try {
          ...
          for (let ve = 0; ve < 12; ve++) {
            ...
            T += `<td class="${je ? "nz" : ""}">${it(Ue[ve], 2)}</td>`;
          }
          T += "</tr>";
        } catch {
        }
  ...
  `;
    return be.innerHTML = pe + T, (_h = be.querySelector("#rpt-close")) == null ? void 0 : _h.addEventListener("click", () => be.remove()), be.querySelectorAll("[data-toggle]").forEach((ce) => {
      ce.addEventListener("click", () => {
        const oe = ce.dataset.toggle, _ = be.querySelector(`#rpt-${oe}`);
        if (_) {
  ...
  function vl(e, b, P, M, J, B) {
    ...
    try {
      oe = vn(Y, M, e), _ = yn(Y), D = ao(ns(_), ao(oe, _));
    } catch {
    }
  ...
  `, document.body.appendChild(ro);
    ...
    $e = Math.max(10, Math.min($e, J - Z - 10)), pe = Math.max(10, Math.min(pe, B - Y - 10)), Ht = document.createElement("div"), Ht.style.cssText = `
    position: fixed;
    left: ${$e}px; top: ${pe}px;
    width: ${Z}px;
  ...
  function Tl(e) {
    ...
    for (const Fe of b) {
      ...
      if (pe === "FRAME SECTIONS") {
        ...
        if (he) {
          ...
          const pt = _e.match(/I2MOD\s+([\d.eE+-]+)/);
          pt && (xe.modI2 = parseFloat(pt[1]));
    ...
    for (const Fe of Y) for (const [_e, he] of be) {
      ...
      if (Ue.push([
        De,
        ft
      ]), ve.push(Fe.name), je.push(Fe.type), We.push(Le), ge.set(ht, he.section), he.rigidZone > 0 && dt.set(ht, [
        he.rigidZone,
        he.rigidZone
        ...
        for (const mt of he.releases) {
          const pt = Ke[mt];
          pt !== void 0 && (Ge[pt] = true);
        }
        ut.set(ht, Ge);
      }
    }
    const ke = /* @__PURE__ */ new Map(), Ae = /* @__PURE__ */ new Map(), Ve = /* @__PURE__ */ new Map(), ct = /* @__PURE__ */ new Map(), Xe = /* @__PURE__ */ new Map(), dt = /* @__PURE__ */ new Map(), ut = /* @__PURE__ */ new Map(), It = /* @__PURE__ */ new Map(), Ft = /* @__PURE__ */ new Map(), Ot = /* @__PURE__ */ new Map(), yt = /* @__PURE__ */ new Map();
    for (const [Fe, _e] of ge) {
      ...
      let ft = 0, ht = 0, Ge = 0, Ke = 0, mt = 0, pt = 0, xt = "rect";
      switch (he.shape) {
        case "Concrete Rectangular":
          ft = xe * Te, ht = Te * xe ** 3 / 12, Ge = xe * Te ** 3 / 12, Ke = Te * xe ** 3 * (1 / 3 - 0.21 * (xe / Te) * (1 - xe ** 4 / (12 * Te ** 4))), mt = pt = 5 / 6 * ft, xt = "rect";
          break;
        case "Concrete Circle":
          ft = Math.PI * xe ** 2 / 4, ht = Ge = Math.PI * xe ** 4 / 64, Ke = Math.PI * xe ** 4 / 32, mt = pt = 0.9 * ft, xt = "circ";
          break;
        case "Steel I/Wide Flange":
          ft = 2 * Te * Ye + (xe - 2 * Ye) * De, ht = (Te * xe ** 3 - (Te - De) * (xe - 2 * Ye) ** 3) / 12, Ge = (2 * Ye * Te ** 3 + (xe - 2 * Ye) * De ** 3) / 12, Ke = (2 * Te * Ye ** 3 + (xe - 2 * Ye) * De ** 3) / 3, mt = (xe - 2 * Ye) * De, pt = 2 * Te * Ye * 5 / 6, xt = "I";
          break;
        case "Steel Tube":
          ft = xe * Te - (xe - 2 * De) * (Te - 2 * De), ht = (Te * xe ** 3 - (Te - 2 * De) * (xe - 2 * De) ** 3) / 12, Ge = (xe * Te ** 3 - (xe - 2 * De) * (Te - 2 * De) ** 3) / 12, Ke = 2 * De * (xe - De) * (Te - De) * ((xe - De) * (Te - De)) / (xe - De + (Te - De)), mt = 2 * xe * De, pt = 2 * Te * De, xt = "HSS";
          break;
        case "Filled Steel Tube":
          ft = xe * Te, ht = Te * xe ** 3 / 12, Ge = xe * Te ** 3 / 12, Ke = 2 * De * (xe - De) * (Te - De) * ((xe - De) * (Te - De)) / (xe - De + (Te - De)), mt = 2 * xe * De + 5 / 6 * (xe - 2 * De) * (Te - 2 * De), pt = 2 * Te * De + 5 / 6 * (xe - 2 * De) * (Te - 2 * De), xt = "CFT";
          break;
        case "Steel Angle": {
          const jt = Ye || De;
          ft = jt * (xe + Te - jt), ht = jt * (xe ** 3 + Te * jt ** 2 + jt ** 2 * (xe - jt)) / 12, Ge = jt * (Te ** 3 + xe * jt ** 2 + jt ** 2 * (Te - jt)) / 12, Ke = (xe + Te - jt) * jt ** 3 / 3, mt = xe * jt, pt = Te * jt, xt = "L";
          break;
        }
        case "Steel Channel":
        case "Cold Formed C":
          ft = 2 * Te * Ye + (xe - 2 * Ye) * De, ht = (De * xe ** 3 + 2 * Te * Ye * (xe - Ye) ** 2) / 12, Ge = (2 * Ye * Te ** 3 + (xe - 2 * Ye) * De ** 3) / 12, Ke = (2 * Te * Ye ** 3 + (xe - 2 * Ye) * De ** 3) / 3, mt = (xe - 2 * Ye) * De, pt = 2 * Te * Ye * 5 / 6, xt = he.shape === "Cold Formed C" ? "coldC" : "C";
          break;
        case "Steel Double Channel":
          ft = 2 * (2 * Te * Ye + (xe - 2 * Ye) * De), ht = 2 * (De * xe ** 3 + 2 * Te * Ye * (xe - Ye) ** 2) / 12, Ge = 2 * (2 * Ye * Te ** 3 + (xe - 2 * Ye) * De ** 3) / 12, Ke = 2 * (2 * Te * Ye ** 3 + (xe - 2 * Ye) * De ** 3) / 3, mt = 2 * (xe - 2 * Ye) * De, pt = 4 * Te * Ye * 5 / 6, xt = "2C";
          break;
        default:
          xe > 0 && Te > 0 && (ft = xe * Te, ht = Te * xe ** 3 / 12, Ge = xe * Te ** 3 / 12, Ke = Math.min(xe, Te) * Math.max(xe, Te) ** 3 / 3 * 0.3, mt = pt = 5 / 6 * ft);
          break;
      }
      he.modI2 && (Ge *= he.modI2), he.modI3 && (ht *= he.modI3), Ve.set(Fe, ft), It.set(Fe, ht), Ft.set(Fe, Ge), Ot.set(Fe, Ke), mt > 0 && ct.set(Fe, mt), pt > 0 && Xe.set(Fe, pt), yt.set(Fe, {
        type: xt,
        b: Te || void 0,
        h: xe || void 0,
      ...
      switch (ce) {
        ...
        case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
          ...
          if (G) {
            ...
            ue.E = rt(D.get("E1")), ue.G = rt(D.get("G12")), ue.nu = rt(D.get("U12")), ue.density = rt(D.get("UnitMass")), B.set(G, ue);
          }
          break;
        }
        case "MATERIAL PROPERTIES 03A - STEEL DATA": {
          const G = D.get("Material");
          G && B.has(G) && (B.get(G).fy = rt(D.get("Fy")));
          break;
        }
        ...
        case "JOINT COORDINATES": {
          const G = D.get("Joint");
          if (G) {
            const ue = rt(D.get("XorR")), Me = rt(D.get("Y")), ze = rt(D.get("Z"));
            H.set(G, [
              ue,
              Me,
              ze
      ...
      switch (T) {
        ...
        case "MATERIAL": {
          ...
          if (G) K = G, M.set(G, {
            ...
            Me && (ue.E = rt(Me));
            const ze = _.get("U");
            ze && (ue.nu = rt(ze)), ue.G = ue.E / (2 * (1 + ue.nu));
            const Ue = _.get("M");
            Ue && (ue.density = rt(Ue));
          }
          break;
    ...
    for (let ve = 0; ve < ce.length; ve++) {
      ...
      if (We || ce[ve].length === 2) {
        const ke = We || {
          ...
          B: 0.3,
          shape: "Rectangular"
        }, Ae = P.get(ke.material) || ze, Ve = Ae.E || ze.E, ct = Ae.nu || 0.3, Xe = Ae.G || Ve / (2 * (1 + ct));
        ...
        const ke = P.get(ge.material) || ze, Ae = ke.E || ze.E, Ve = ke.nu || 0.2, ct = ke.G || Ae / (2 * (1 + Ve));
        ue.elasticities.set(ve, Ae), ue.shearModuli.set(ve, ct), ue.thicknesses.set(ve, ge.thickness), ue.poissonsRatios.set(ve, Ve), ue.densities.set(ve, ke.density || 0);
      }
    }
    const Ue = {
...
`);
  ...
  function Pl(e) {
    ...
    P.forEach((ge, ke) => {
      ...
      const Ae = (_a2 = J.sectionShapes) == null ? void 0 : _a2.get(ke), Ve = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(ke)) ?? 0, ct = _.get(Ve) || "Mat_1", Xe = (Ae == null ? void 0 : Ae.h) ?? 0, dt = (Ae == null ? void 0 : Ae.b) ?? 0, ut = (Ae == null ? void 0 : Ae.d) ?? 0, It = (Ae == null ? void 0 : Ae.tf) ?? 0, Ft = (Ae == null ? void 0 : Ae.tw) ?? 0, Ot = (Ae == null ? void 0 : Ae.type) || "rect", yt = `${Ot}_${Xe}_${dt}_${ut}_${It}_${Ft}`;
      (Ae == null ? void 0 : Ae.name) && !Me.has(yt) && Me.set(yt, Ae.name);
      let bt = Me.get(yt);
      if (bt || (Ot === "rect" ? bt = `R${be(dt * 100)}x${be(Xe * 100)}` : Ot === "circ" ? bt = `C_D${be(ut * 100)}` : Ot === "I" ? bt = `I_${be(Xe * 100)}` : bt = `Sec_${G.size + 1}`, Me.set(yt, bt)), ue.set(ke, bt), G.has(bt)) return;
      G.add(bt);
      const qe = {
      ...
      let Mt = `  FRAMESECTION  "${bt}"  MATERIAL "${ct}"  SHAPE "${qe}"`;
      Xe && (Mt += `  D ${Xe}`), dt && (Mt += `  B ${dt}`), ut && !Xe && (Mt += `  D ${ut}`), It && (Mt += `  TF ${It}`), Ft && (Mt += `  TW ${Ft}`), W.push(Mt);
    }), W.push("");
    ...
    P.forEach((ge, ke) => {
      ...
      if (Ae === "BEAM") {
        const ct = ve(ge[0]), Xe = ve(ge[1]);
        W.push(`  LINE  "E${ke + 1}"  BEAM  "${ct.pt}"  "${Xe.pt}"  0`), je.push(`  LINEASSIGN  "E${ke + 1}"  "${ct.story}"  SECTION "${Ve}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
      } else {
        const ct = b[ge[0]][1] <= b[ge[1]][1] ? ge[0] : ge[1], Xe = b[ge[0]][1] <= b[ge[1]][1] ? ge[1] : ge[0];
        ve(ct);
        const dt = ve(Xe), ut = be(b[ct][1]), It = be(b[Xe][1]), Ft = K.indexOf(ut), Ot = K.indexOf(It), yt = Math.max(1, Ot >= 0 && Ft >= 0 ? Ot - Ft : 1);
        W.push(`  LINE  "E${ke + 1}"  ${Ae}  "${dt.pt}"  "${dt.pt}"  ${yt}`);
        for (let bt = 0; bt < yt; bt++) {
          const io = Ot - bt;
          io >= 0 && io < $e.length && je.push(`  LINEASSIGN  "E${ke + 1}"  "${$e[io]}"  SECTION "${Ve}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
        }
      }
    ...
    if (P.forEach((ge, ke) => {
      if (ge.length === 4) {
        const Ae = b[ge[0]], Ve = b[ge[1]], ct = b[ge[2]], Xe = [
          Ve[0] - Ae[0],
          Ve[1] - Ae[1],
          Ve[2] - Ae[2]
        ], dt = [
          ...
          ct[1] - Ae[1],
          ct[2] - Ae[2]
        ], ut = Math.abs(Xe[2] * dt[0] - Xe[0] * dt[2]), It = Math.sqrt((Xe[1] * dt[2] - Xe[2] * dt[1]) ** 2 + ut ** 2 + (Xe[0] * dt[1] - Xe[1] * dt[0]) ** 2), Ft = It > 1e-10 && ut / It < 0.5;
        We.push({
          idx: ke,
          el: ge,
          isWall: Ft
    ...
    if (We.length > 0) {
      ...
      const ge = [];
      We.forEach((ke, Ae) => {
        const { el: Ve, isWall: ct } = ke, Xe = ct ? `W${Ae + 1}` : `F${Ae + 1}`, dt = ct ? "PANEL" : "FLOOR", ut = Ve.map((It) => ve(It));
        if (ct) {
          const It = b[Ve[0]][1] <= b[Ve[2]][1] ? 0 : 2, Ft = b[Ve[1]][1] <= b[Ve[3]][1] ? 1 : 3;
          W.push(`  AREA "${Xe}"  ${dt}  4  "${ut[It].pt}"  "${ut[Ft].pt}"  "${ut[Ft].pt}"  "${ut[It].pt}"  1  1  0  0  `);
          const Ot = ut[It === 0 ? 2 : 0].story;
        ...
        } else W.push(`  AREA "${Xe}"  ${dt}  4  "${ut[0].pt}"  "${ut[1].pt}"  "${ut[2].pt}"  "${ut[3].pt}"  0  0  0  0  `), ge.push(`  AREAASSIGN  "${Xe}"  "${ut[0].story}"  SECTION "Losa"  DIAPH  "D1"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "TOP"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
    ...
    return W.push("$ LOAD PATTERNS"), W.push('  LOADPATTERN "Dead"  TYPE  "Dead"  SELFWEIGHT  1'), W.push('  LOADPATTERN "Live"  TYPE  "Live"  SELFWEIGHT  0'), W.push(""), M.loads && M.loads.size > 0 && (W.push("$ POINT OBJECT LOADS"), M.loads.forEach((ge, ke) => {
      const [Ae, Ve, ct] = ge, Xe = ve(ke);
      Math.abs(Ae) > 1e-10 && W.push(`  POINTLOAD  "${Xe.pt}"  "${Xe.story}"  "Dead"  TYPE "FORCE"  FX ${Ae}`), Math.abs(Ve) > 1e-10 && W.push(`  POINTLOAD  "${Xe.pt}"  "${Xe.story}"  "Dead"  TYPE "FORCE"  FY ${Ve}`), Math.abs(ct) > 1e-10 && W.push(`  POINTLOAD  "${Xe.pt}"  "${Xe.story}"  "Dead"  TYPE "FORCE"  FZ ${ct}`);
...
`);
  ...
  function _l(e) {
    ...
    for (const $e of e.split(/\r?\n/)) {
      ...
      if (ce) {
        const G = parseInt(ce[1]), ue = b.length;
        b.push([
          parseFloat(ce[2]),
          parseFloat(ce[3]),
          parseFloat(ce[4])
  ...
  function ma(e, b) {
    const P = Xt(b.args), M = Yt(e, P[0]), J = M ? Hl(e, M) : [
      0,
      0,
      0
    ];
  ...
  function Wl(e) {
    ...
    function K(oe, _, D, G) {
      ...
      for (const Me of ue) {
        const ze = P[Me];
        if (!ze) continue;
        const Ue = Xt(ze.args), ve = Ue[5] || Ue[4] || "", je = Yt(P, ve);
        if (!je) continue;
        const We = ga(P, je), ge = be(ze);
        let ke = G, Ae = null, Ve = null;
        const ct = Ue[6] || Ue[5] || "", Xe = Yt(P, ct);
        if (Xe) {
          const bt = $n(P, Xe);
          bt && (ke = bt.depth || G, Ae = bt.origin, Ve = bt.direction);
        }
        const dt = Ae ? Ae[0] : We.origin[0], ut = Ae ? Ae[1] : We.origin[1], It = Ae ? Ae[2] : We.origin[2], Ft = Ve || (D === "Z" ? We.dirZ : We.dirX), Ot = W(dt, ut, It), yt = W(dt + Ft[0] * ke, ut + Ft[1] * ke, It + Ft[2] * ke);
        B.push({
          id: H++,
          type: "frame",
          nodeIds: [
      ...
      for (const B of J) {
        ...
        if (Y) {
          if (Y.type === "IFCEXTRUDEDAREASOLID") {
            const H = Xt(Y.args), W = parseFloat(H[3]) || 0, be = Yt(e, H[1]);
            let T = [
              0,
              0,
              0
          ...
          if (Y.type === "IFCMAPPEDITEM") {
            const H = Xt(Y.args), W = Yt(e, H[0]);
            if (W && W.type === "IFCREPRESENTATIONMAP") {
              const be = Xt(W.args), T = Yt(e, be[1]);
              if (T) {
                const K = $n(e, T);
                if (K) return K;
              }
  ...
  async function Jl(e, b) {
    ...
    return M.StreamAllMeshes(B, ($e) => {
      ...
      for (let _ = 0; _ < oe.size(); _++) {
        const D = oe.get(_), G = M.GetGeometry(B, D.geometryExpressID), ue = M.GetVertexArray(G.GetVertexData(), G.GetVertexDataSize()), Me = M.GetIndexArray(G.GetIndexData(), G.GetIndexDataSize()), ze = new no(), Ue = new Float32Array(ue.length / 2), ve = new Float32Array(ue.length / 2);
        ...
        ke.applyMatrix4(je), ke.userData.expressID = $e.expressID, ke.userData.category = pe, ce.add(ke), be.expandByObject(ke), T++, G.delete();
      }
    }), M.CloseModel(B), {
      meshCount: T,
      bbox: be,
  ...
  tr = function(e) {
    ...
    let T = "", K = {}, $e = null, pe = "", ce = [], oe = [], _ = [], D = /* @__PURE__ */ new Set(), G = /* @__PURE__ */ new Set(), ue = /* @__PURE__ */ new Set(), Me = /* @__PURE__ */ new Map(), ze = /* @__PURE__ */ new Map(), Ue = null, ve = [], je = 0.2, We = 2, ge = 2, ke = false, Ae = 2, Ve = "x", ct = /* @__PURE__ */ new Set(), Xe = true, dt = 0.15, ut = 2, It = 2, Ft = /* @__PURE__ */ new Set(), Ot = false, yt = "perimeter";
    const bt = () => ({
      b: 0.3,
      h: 0.4,
      profileIdx: 0,
    ...
    function wn(t, o, n) {
      ...
      for (const f of t) {
        const h = f.elev;
        S ? (Ko(a - g, 0, h, c + g, 0, h, $, Jt), is(f.name, c + g * 1.5, 0, h, y, Jt)) : (Ko(a, s, h, c, s, h, $, Jt), Ko(c, s, h, c, r, h, $, Jt), Ko(c, r, h, a, r, h, $, Jt), Ko(a, r, h, a, s, h, $, Jt), is(f.name, a - g * 1.5, s, h, y, Jt));
      }
      Jt.traverse((f) => {
    ...
    const Qe = {
      ...
      build(t) {
        var _a2;
        if (Ge.length === 0 || pt.length < 2) {
          console.log("Error: call cad.refgrid() first to define axes and levels");
          return;
        }
        const o = (t == null ? void 0 : t.col) || "40x40", n = (t == null ? void 0 : t.viga) || "30x40", l = (t == null ? void 0 : t.fc) || 210, [a, c] = o.split("x").map((q) => parseFloat(q) / 100), [s, r] = n.split("x").map((q) => parseFloat(q) / 100), p = Ge.map((q) => q.coord), i = Ke.map((q) => q.coord), d = pt.map((q) => q.elev), g = p.length, S = i.length, $ = d.length, y = $ - 1, f = [], h = {};
        ...
        const u = 15100 * Math.sqrt(l) * 10, w = u / (2 * (1 + 0.2)), A = a * c, R = a * c ** 3 / 12, j = c * a ** 3 / 12, x = a * c * (a ** 2 + c ** 2) / 12, E = s * r, v = s * r ** 3 / 12, F = r * s ** 3 / 12, X = s * r * (s ** 2 + r ** 2) / 12, V = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), le = /* @__PURE__ */ new Map(), Ee = /* @__PURE__ */ new Map();
        for (let q = 0; q < I.length; q++) V.set(q, u), ae.set(q, w), L.has(q) ? (Q.set(q, A), U.set(q, R), re.set(q, j), le.set(q, x), Ee.set(q, {
          type: "rect",
          b: a,
          h: c,
          name: `COL${o}`
        })) : m.has(q) ? (Q.set(q, A), U.set(q, R), re.set(q, j), le.set(q, x), Ee.set(q, {
          type: "rect",
          b: a,
          h: c,
          name: `BR${o}`
        })) : (Q.set(q, E), U.set(q, v), re.set(q, F), le.set(q, X), Ee.set(q, {
          type: "rect",
          b: s,
          h: r,
          name: `V${n}`
      ...
      addBeam(t, o, n, l, a) {
        var _a2;
        const c = Ge.findIndex((O) => O.label === t), s = Ke.findIndex((O) => O.label === o), r = Ge.findIndex((O) => O.label === n), p = Ke.findIndex((O) => O.label === l), i = pt.findIndex((O) => O.label === a);
        if (c < 0 || s < 0 || r < 0 || p < 0) {
        ...
        const d = Ge[c].coord, g = Ke[s].coord, S = Ge[r].coord, $ = Ke[p].coord, y = pt[i].elev, f = [
          ...e.nodes.val
        ], h = [
          ...((_a2 = e.elements) == null ? void 0 : _a2.val) || []
        ], I = (O, k, m) => {
      ...
      addBrace(t, o, n, l, a, c) {
        var _a2;
        const s = Ge.findIndex((u) => u.label === t), r = Ke.findIndex((u) => u.label === o), p = pt.findIndex((u) => u.label === n), i = Ge.findIndex((u) => u.label === l), d = Ke.findIndex((u) => u.label === a), g = pt.findIndex((u) => u.label === c);
        if (s < 0 || r < 0 || p < 0) {
        ...
        const S = Ge[s].coord, $ = Ke[r].coord, y = pt[p].elev, f = Ge[i].coord, h = Ke[d].coord, I = pt[g].elev, L = [
          ...e.nodes.val
        ], z = [
          ...((_a2 = e.elements) == null ? void 0 : _a2.val) || []
        ], O = (u, w, A) => {
...
`;
      ...
      model3d(t) {
        ...
        const A = 15100 * Math.sqrt(s) * 10, R = A / (2 * (1 + 0.2)), j = r * p, x = r * p ** 3 / 12, E = p * r ** 3 / 12, v = r * p * (r ** 2 + p ** 2) / 12, F = i * d, X = i * d ** 3 / 12, V = d * i ** 3 / 12, ae = i * d * (i ** 2 + d ** 2) / 12, Q = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), le = /* @__PURE__ */ new Map(), Ee = /* @__PURE__ */ new Map(), Re = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < O.length; ee++) Q.set(ee, A), U.set(ee, R), k.has(ee) ? (re.set(ee, j), le.set(ee, x), Ee.set(ee, E), Re.set(ee, v), q.set(ee, {
          type: "rect",
          b: r,
          h: p,
          name: `COL${a}`
        })) : (re.set(ee, F), le.set(ee, X), Ee.set(ee, V), Re.set(ee, ae), q.set(ee, {
          type: "rect",
          b: i,
          h: d,
          name: `V${c}`
        ...
        if (te) {
          ...
          const ee = Ge.map((Se) => Se.label), me = Ke.map((Se) => Se.label);
        ...
        return as(g, S, $), tt(), console.log(`Model3D: ${L.length}n ${O.length}e | ${y}x${f} grid, ${I} floors | COL${a} V${c} f'c=${s}`), {
          nodes: L.length,
          elements: O.length,
          columns: k.size,
          beams: m.size
      ...
      building(t, o, n, l = 3, a = 0, c = 0, s = 0, r = 0, p = 1) {
          ...
          for (let w = 1; w < g.length; w++) if (Ve === "x") for (let A = 0; A < d.length - 1; A++) {
            ...
            for (let x = 1; x <= Ae; x++) {
              ...
              for (let F = 0; F < i.length - 1; F++) {
                const X = L.length;
                ct.add(X), G.add(X), Me.set(X, w - 1), ze.set(X, {
                  dir: "x",
                  bay: F
                }), L.push([
                  v[F],
        ...
        for (const u of ve) {
          ...
          for (const R of A) {
            ...
            const Q = ge, U = We, re = f[F], le = f[V], Ee = f[X], Re = f[ae], q = [];
            for (let fe = 0; fe <= U; fe++) {
              const te = [], de = fe / U;
              for (let ee = 0; ee <= Q; ee++) {
                const me = ee / Q, Se = (1 - de) * ((1 - me) * re[0] + me * le[0]) + de * ((1 - me) * Ee[0] + me * Re[0]), se = (1 - de) * ((1 - me) * re[1] + me * le[1]) + de * ((1 - me) * Ee[1] + me * Re[1]), Ie = (1 - de) * ((1 - me) * re[2] + me * le[2]) + de * ((1 - me) * Ee[2] + me * Re[2]);
                fe === 0 && ee === 0 ? te.push(F) : fe === 0 && ee === Q ? te.push(V) : fe === U && ee === 0 ? te.push(X) : fe === U && ee === Q ? te.push(ae) : (te.push(f.length), f.push([
                  Se,
                  se,
                  Ie
                ]));
            ...
            for (let fe = 0; fe < U; fe++) for (let te = 0; te < Q; te++) {
              const de = q[fe][te], ee = q[fe][te + 1], me = q[fe + 1][te + 1], Se = q[fe + 1][te], se = L.length;
            ...
            for (let E = 0; E < i.length - 1; E++) for (let v = 0; v < d.length - 1; v++) {
              ...
              for (let U = 0; U <= w; U++) {
                ...
                for (let le = 0; le <= u; le++) {
                  const Ee = F + le / u * (X - F), Re = V + U / w * (ae - V);
                  if (U === 0 && le === 0) re.push(h[`${E},${v},${j}`]);
                  else if (U === 0 && le === u) re.push(h[`${E + 1},${v},${j}`]);
                  else if (U === w && le === 0) re.push(h[`${E},${v + 1},${j}`]);
                  else if (U === w && le === u) re.push(h[`${E + 1},${v + 1},${j}`]);
                  else {
                    const q = R(Ee, Re, x), fe = A.get(q);
                    if (fe !== void 0) re.push(fe);
                    else {
                      const te = f.length;
                      f.push([
                        Ee,
                        Re,
                        x
                      ]), A.set(q, te), re.push(te);
                    }
                  }
                }
                Q.push(re);
              }
              for (let U = 0; U < w; U++) for (let re = 0; re < u; re++) {
                const le = Q[U][re], Ee = Q[U][re + 1], Re = Q[U + 1][re + 1], q = Q[U + 1][re], fe = L.length;
        ...
        if (Ot && yt) {
          const u = yt === "all" || yt === "x" || yt === "perimeter", w = yt === "all" || yt === "y" || yt === "perimeter", A = g.length - 1;
          for (let R = 0; R < A; R++) {
            if (u) for (let j = 0; j < d.length; j++) {
              if (yt === "perimeter" && j !== 0 && j !== d.length - 1) continue;
              const x = Math.floor((i.length - 1) / 2);
              for (let E = 0; E < i.length - 1; E++) {
                if (yt === "perimeter" && E !== x || y(E, j) || y(E + 1, j)) continue;
            ...
            if (w) for (let j = 0; j < i.length; j++) {
              ...
              for (let E = 0; E < d.length - 1; E++) {
                if (yt === "perimeter" && E !== x || y(j, E) || y(j, E + 1)) continue;
      ...
      example(t) {
        ...
        switch (t) {
          ...
          case "edificio": {
            ot("edificio"), qe.colMat = 0, qe.vigaMat = 0, qe.colShape = 0, ve = [], Xe = false, ke = false, Ot = false, Oe();
            break;
          }
          case "edif-acero":
          case "edificio-acero": {
            ot("edificio"), qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, ve = [], ke = true, Ae = 2;
            const o = ce.reduce((l, a) => l + a, 0) / ce.length, n = oe.reduce((l, a) => l + a, 0) / oe.length;
            Ve = o >= n ? "y" : "x", Xe = true, dt = 0.08, Ot = false, Oe();
            break;
          }
          ...
          case "edificio-acero-diag": {
            ot("edificio"), qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, ve = [], ke = true, Ae = 2;
            const o = ce.reduce((l, a) => l + a, 0) / ce.length, n = oe.reduce((l, a) => l + a, 0) / oe.length;
            Ve = o >= n ? "y" : "x", Xe = true, dt = 0.08, Ot = true, yt = "perimeter", Oe();
            break;
          }
          case "edif-muros":
          case "edificio-muros": {
            ot("edificio"), qe.colMat = 0, qe.vigaMat = 0, qe.colShape = 0, ke = false;
          ...
          case "edif-mixto":
          case "edificio-mixto": {
            ot("edificio"), qe.colMat = 2, qe.vigaMat = 0, ke = false;
          ...
          case "mezzanine": {
            ot("edificio"), K.nPisos && (K.nPisos.val = 1), K.hPiso && (K.hPiso.val = 4.5), K.nVanosX && (K.nVanosX.val = 3), K.nVanosY && (K.nVanosY.val = 2), K.nSubViga && (K.nSubViga.val = 3), ce = [
              ...
              5,
              5
            ], qe.colMat = 1, qe.vigaMat = 1, qe.steelColType = 0, qe.steelVigaType = 0, ve = [], ke = true, Ae = 2, Ve = ce[0] >= oe[0] ? "y" : "x", Xe = true, dt = 0.08, ut = 3, It = 3, Oe();
            break;
          }
          case "galpon": {
...
Util:     cad.info()  cad.clear()  cad.help()
    ...
    function Oe() {
      ...
      switch (T) {
        ...
        case "edificio": {
          const o = ie("Lvix") || 0, n = ie("Lvdx") || 0, l = ie("Lviy") || 0, a = ie("Lvdy") || 0, c = Math.max(1, Math.round(ie("nSubViga") || 3)), s = Math.max(1, Math.round(ie("nSubCol") || 1)), r = ie("hPiso"), p = _.length > 0 ? [
            ..._
          ] : Array(Math.round(ie("nPisos"))).fill(r);
          Qe.building([
            ...ce
          ], [
            ...oe
        ...
        case "galpon":
          Qe.galpon(ie("span"), ie("length"), ie("height"), ie("archRise"), Math.round(ie("xDiv")), Math.round(ie("yDiv")));
          break;
        case "barra":
          va();
          break;
    ...
    function ps() {
      ...
      ]), p = (ie("CM") ?? 0) + (ie("CV") ?? 0), i = /* @__PURE__ */ new Map();
    ...
    function ms() {
      const t = ie("Lx") || 15, o = ie("Ly") || 10, n = ie("meshSize") || 0.5, l = ie("q") || -3, a = ie("t") || 1, c = ie("E") || 3e7, s = ie("nu") || 0.3, r = c / (2 * (1 + s)), p = Fe === 1 ? "Membrana" : Fe === 2 ? "Kirchhoff" : "Mindlin", { nodes: i, elements: d, boundaryIndices: g } = xo({
        points: [
          [
            0,
            0,
    ...
    function gs() {
      const t = ie("Lx") || 10, o = ie("Ly") || 10, n = Math.round(ie("nx") || 16), l = Math.round(ie("ny") || 16), a = ie("t") || 0.2, c = ie("q") || -10, s = ie("E") || 3e7, r = ie("nu") || 0.3, p = Mt === 1 ? "clamped" : "simply-supported", d = {
        1: 2,
        2: 1,
        3: 0
      }[Fe] ?? 0;
    ...
    function bs() {
      const t = ie("a") || 6, o = ie("b") || 4, n = Math.round(ie("nx") || 12), l = Math.round(ie("ny") || 8), a = ie("t") || 0.1, c = ie("q") || -10, s = ie("E") || 35e6, r = ie("nu") || 0.15, i = {
        1: 2,
        2: 1,
        3: 0
    ...
    function xs() {
      const t = ie("L") || 4, o = ie("H") || 2, n = ie("t") || 0.1, l = ie("E") || 2e7, a = ie("nu") || 0.2, c = l / (2 * (1 + a)), s = ie("q") || -100, r = ie("b") || 0.8, p = ie("meshSize") || 0.2, { nodes: i, elements: d, boundaryIndices: g } = xo({
        points: [
          [
            0,
            0,
    ...
    function vs() {
      const t = ie("H") || 4, o = ie("B") || 3, n = ie("tw") || 0.3, l = ie("tb") || 0.4, a = ie("meshSize") || 0.2, c = ie("E") || 25e6, s = ie("nu") || 0.2, r = c / (2 * (1 + s)), p = ie("gamma") || 18, i = ie("Ka") || 0.33, d = ie("Es") || 5e4, g = ie("nus") || 0.3, S = d / (2 * (1 + g)), $ = ie("kn") || 1e6, y = ie("ks") || 1e4, f = ie("gammaW") || 9.81, h = ie("Hw") || 3.5, I = ie("qs") || 0, L = Mt, z = o * 0.3, O = o * 0.7, k = [
        [
          -z,
          0,
          0
      ...
      if (L === 0) {
        ...
        if (L === 2) {
          ...
          for (let se = 0; se < m.length; se++) {
            const Ie = m[se][0], He = m[se][1];
            Math.abs(Ie - n) < Q && He >= l - Q && U.push(se);
          }
          U.sort((se, Ie) => m[se][1] - m[Ie][1]);
          const re = [
            U[0]
          ];
          for (let se = 1; se < U.length; se++) {
            const Ie = m[U[se]][1] - m[re[re.length - 1]][1];
            Math.abs(Ie) > a * 0.05 && re.push(U[se]);
          }
          U.length = 0, U.push(...re);
          ...
          const Ee = u.length, Re = [];
          for (let se = 0; se < Ee; se++) {
            const Ie = u[se], He = (m[Ie[0]][0] + m[Ie[1]][0] + m[Ie[2]][0]) / 3, at = (m[Ie[0]][1] + m[Ie[1]][1] + m[Ie[2]][1]) / 3, lt = He >= -z && He <= O && at >= 0 && at <= l, St = He >= 0 && He <= n && at >= l && at <= l + t, ye = lt || St;
            if (Re.push(!ye), !ye) for (let et = 0; et < Ie.length; et++) {
              const nt = le.get(Ie[et]);
              nt !== void 0 && (Ie[et] = nt);
            }
          }
          const q = u.length;
          for (let se = 0; se < U.length - 1; se++) {
            const Ie = U[se], He = U[se + 1], at = le.get(Ie), lt = le.get(He);
            u.push([
              He,
              Ie,
              at,
          ...
          const fe = u.length - q, te = /* @__PURE__ */ new Map(), de = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), me = /* @__PURE__ */ new Map(), Se = /* @__PURE__ */ new Map();
          for (let se = 0; se < Ee; se++) Re[se] ? (te.set(se, d), de.set(se, d), me.set(se, g), Se.set(se, S), ee.set(se, 1)) : (te.set(se, c), de.set(se, c), me.set(se, s), Se.set(se, r), ee.set(se, 1));
          for (let se = q; se < u.length; se++) te.set(se, $), de.set(se, 0), me.set(se, 0), Se.set(se, y), ee.set(se, 0);
          R = {
            elasticities: te,
          ...
          for (let se = 0; se < Ee; se++) {
            if (!Re[se]) continue;
            const Ie = u[se], He = m[Ie[0]], at = m[Ie[1]], lt = m[Ie[2]], St = Math.abs((at[0] - He[0]) * (lt[1] - He[1]) - (lt[0] - He[0]) * (at[1] - He[1])) / 2, ye = -p * St / 3;
            for (const et of Ie) {
              const nt = A.get(et) || [
                0,
                0,
          ...
          if (I > 0) {
            ...
            for (let Ie = 0; Ie < m.length; Ie++) {
              const He = m[Ie][0], at = m[Ie][1];
              Math.abs(at - v) < a * 0.1 && He > n - 1e-6 && se.push({
                idx: Ie,
                x: He
              });
            }
            se.sort((Ie, He) => Ie.x - He.x);
            for (let Ie = 0; Ie < se.length; Ie++) {
              let He = a;
              Ie > 0 && Ie < se.length - 1 ? He = (se[Ie + 1].x - se[Ie - 1].x) / 2 : Ie === 0 && se.length > 1 ? He = (se[1].x - se[0].x) / 2 : Ie === se.length - 1 && se.length > 1 && (He = (se[Ie].x - se[Ie - 1].x) / 2);
              const at = -I * He, lt = A.get(se[Ie].idx) || [
                0,
                0,
                0,
                0,
              ...
              lt[1] += at, A.set(se[Ie].idx, lt);
            }
          }
          console.log(`  Interfaz Goodman: ${U.length} nodos interfaz, ${fe} elem interfaz, kn=${$}, ks=${y}`);
        } else {
          const F = [
            [
              -z,
          ...
          const ae = (q) => {
            const fe = (m[q[0]][0] + m[q[1]][0] + m[q[2]][0]) / 3, te = (m[q[0]][1] + m[q[1]][1] + m[q[2]][1]) / 3, de = fe >= -z && fe <= O && te >= 0 && te <= l, ee = fe >= 0 && fe <= n && te >= l && te <= l + t;
            return de || ee;
          }, Q = /* @__PURE__ */ new Map(), U = /* @__PURE__ */ new Map(), re = /* @__PURE__ */ new Map(), le = /* @__PURE__ */ new Map(), Ee = /* @__PURE__ */ new Map(), Re = [];
          for (let q = 0; q < u.length; q++) {
            const fe = ae(u[q]);
            Re.push(!fe), fe ? (Q.set(q, c), U.set(q, c), le.set(q, s), Ee.set(q, r), re.set(q, 1)) : (Q.set(q, d), U.set(q, d), le.set(q, g), Ee.set(q, S), re.set(q, 1));
          }
          R = {
            elasticities: Q,
          ...
          for (let q = 0; q < u.length; q++) {
            if (!Re[q]) continue;
            const fe = u[q], te = m[fe[0]], de = m[fe[1]], ee = m[fe[2]], me = Math.abs((de[0] - te[0]) * (ee[1] - te[1]) - (ee[0] - te[0]) * (de[1] - te[1])) / 2, Se = -p * me / 3;
            for (const se of fe) {
              const Ie = A.get(se) || [
                0,
                0,
          ...
          if (I > 0) {
            ...
            for (let fe = 0; fe < m.length; fe++) {
              const te = m[fe][0], de = m[fe][1];
              Math.abs(de - v) < a * 0.1 && te > n - 1e-6 && q.push({
                idx: fe,
                x: te
              });
            }
            ...
            for (let fe = 0; fe < q.length; fe++) {
              ...
              const de = -I * te, ee = A.get(q[fe].idx) || [
                0,
                0,
                0,
                0,
      ...
      if (L === 3) {
        ...
        for (let ae = 0; ae < V.length; ae++) {
          const { idx: Q, y: U } = V[ae], re = Math.max(0, v - U);
          if (re <= 0 || U < X - 1e-6) continue;
          const le = Math.min(re, F), Ee = f * le;
          let Re = a;
      ...
      try {
        const E = At(m, u, j, R), v = u.filter((re) => re.length === 3), F = {};
        for (const re of Object.keys(R)) {
          const le = R[re];
          if (le && le instanceof Map) {
            const Ee = /* @__PURE__ */ new Map();
            let Re = 0;
            for (let q = 0; q < u.length; q++) u[q].length === 3 && (le.has(q) && Ee.set(Re, le.get(q)), Re++);
            F[re] = Ee;
          }
        }
        const X = bo(m, v, F, E), V = m.map((re) => [
          re[0],
          0,
          re[1]
        ]);
        ...
        w.forEach((re, le) => ae.set(le, re));
        const Q = /* @__PURE__ */ new Map();
        A.forEach((re, le) => Q.set(le, [
          re[0],
          re[2],
          re[1],
          re[3],
        ...
        let U = 0;
        E && E.deformations && E.deformations.forEach((re) => {
          const le = Math.sqrt(re[0] * re[0] + re[1] * re[1] + re[2] * re[2]);
          U = Math.max(U, le);
    ...
    function ys() {
      const t = ie("Lx") || 2, o = ie("Ly") || 2, n = ie("t") || 0.5, l = ie("colA") || 0.4, a = ie("colH") || 1.5, c = Math.round(ie("nx") || 8), s = Math.round(ie("ny") || 8), r = ie("E") || 25e6, p = ie("nu") || 0.2, i = ie("P") || -500, d = ie("Mx") || 0, g = ie("My") || 0, S = ie("ks") || 2e4, $ = t / c, y = o / s, f = t / 2, h = o / 2, I = l / 2, L = [];
    ...
    function $s() {
      const t = ie("Lx") || 0.4, o = ie("Ly") || 0.4, n = ie("t") || 0.025, l = ie("dBolt") || 0.022, a = ie("sx") || 0.28, c = ie("sy") || 0.28, s = ie("colA") || 0.2, r = ie("meshSize") || 8e-3, p = ie("E") || 2e8, i = ie("nu") || 0.3, d = p / (2 * (1 + i)), g = ie("P") || -200, S = Math.round(ie("nBolts") || 4), $ = t / 2, y = o / 2, f = l / 2, h = s / 2, I = [];
      S >= 4 && (I.push([
        $ - a / 2,
        y - c / 2
      ]), I.push([
    ...
    function Ms() {
      const t = ie("colB") || 0.3, o = ie("colH") || 0.3, n = ie("colT") || 8e-3, l = ie("colLen") || 1.5, a = ie("Lx") || 0.45, c = ie("Ly") || 0.45, s = ie("tPlaca") || 0.025, r = ie("dBolt") || 0.022, p = ie("sx") || 0.32, i = ie("sy") || 0.32, d = Math.round(ie("nSubColV") || 6), g = Math.round(ie("nSubColH") || 4), S = Math.round(ie("nSubPlaca") || 10), $ = ie("E") || 2e8, y = ie("nu") || 0.3, f = $ / (2 * (1 + y)), h = ie("P") || -300, I = a / 2, L = c / 2, z = r / 2, O = t / 2, k = o / 2, m = [], u = [], w = S, A = a / w, R = c / w, j = (de, ee) => ee * (w + 1) + de;
      ...
      const x = [
        ...
        for (const [me, Se] of x) if ((de - me) * (de - me) + (ee - Se) * (ee - Se) < z * z) return true;
        return false;
      }, v = u.length;
      for (let de = 0; de < w; de++) for (let ee = 0; ee < w; ee++) {
        const me = (ee + 0.5) * A, Se = (de + 0.5) * R;
        E(me, Se) || u.push([
          j(ee, de),
          j(ee + 1, de),
          j(ee + 1, de + 1),
      ...
      for (const [de, ee] of U) {
        const [me, Se] = ae[de], [se, Ie] = ae[ee], He = [];
        for (let at = 0; at <= X; at++) {
          const lt = [], St = at / X * l;
          for (let ye = 0; ye <= V; ye++) {
            const et = ye / V, nt = me + et * (se - me), Qt = Se + et * (Ie - Se);
            if (at === 0) {
              const qt = re(nt, Qt);
              if (qt >= 0) {
                lt.push(qt);
            ...
            let Gt = -1;
            for (let qt = 0; qt < m.length; qt++) if (Math.abs(m[qt][0] - nt) < 1e-6 && Math.abs(m[qt][1] - Qt) < 1e-6 && Math.abs(m[qt][2] - St) < 1e-6) {
              Gt = qt;
              break;
            }
        ...
        for (let at = 0; at < X; at++) for (let lt = 0; lt < V; lt++) u.push([
          He[at][lt],
          He[at][lt + 1],
          He[at + 1][lt + 1],
          He[at + 1][lt]
      ...
      for (let de = 0; de < m.length; de++) Math.abs(m[de][2] - l) < 1e-6 && q.push(de);
      const fe = h / Math.max(q.length, 1);
      for (const de of q) Re.set(de, [
        0,
        0,
        fe,
        0,
    ...
    function ws() {
      const t = ie("H") || 6, o = ie("angle") || 45, n = ie("bTop") || 3, l = ie("bBot") || 3, a = ie("meshSize") || 2, c = ie("E") || 5e4, s = ie("nu") || 0.3, r = ie("gamma") || 18, p = ie("c") || 15, i = ie("phi") || 30, d = ie("qs") || 0, g = t / Math.tan(o * Math.PI / 180), S = l + g + n, $ = t, y = [
        [
          0,
          -$,
          0
    ...
    function Es(t, o, n, l, a) {
      ...
      if (qe.vigaMat === 0) {
        for (let r = 0; r < o.length; r++) {
          ...
          g[i] = +kt(p.b).toFixed(2), g[d] = +kt(p.h).toFixed(2), t.addBinding(g, i, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `b sv${n}${r + 1}`
        ...
        t.on("change", (r) => {
          ...
          i && (o[parseInt(i[1])].b = Tt(r.value), Oe()), d && (o[parseInt(d[1])].h = Tt(r.value), Oe());
        });
      } else if (c <= 1) {
        for (let r = 0; r < o.length; r++) {
          const p = {};
        ...
        for (let r = 0; r < o.length; r++) {
          const p = o[r], i = {}, d = `${n}${r}`;
          i[`bf${d}`] = +kt(p.bf ?? 0.2).toFixed(3), i[`h${d}`] = +kt(p.hf ?? 0.4).toFixed(3), i[`tf${d}`] = +kt(p.tf ?? 0.015).toFixed(3), i[`tw${d}`] = +kt(p.tw ?? 0.01).toFixed(3), t.addBinding(i, `bf${d}`, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `bf sv${n}${r + 1}`
        ...
        t.on("change", (r) => {
          ...
          for (let i = 0; i < o.length; i++) {
            const d = `${n}${i}`;
            p === `bf${d}` && (o[i].bf = Tt(r.value), Oe()), p === `h${d}` && (o[i].hf = Tt(r.value), Oe()), p === `tf${d}` && (o[i].tf = Tt(r.value), Oe()), p === `tw${d}` && (o[i].tw = Tt(r.value), Oe());
          }
        });
      } else {
        for (let r = 0; r < o.length; r++) {
          const p = o[r], i = {}, d = `${n}${r}`;
          i[`bc${d}`] = +kt(p.bc ?? 0.2).toFixed(3), i[`hc${d}`] = +kt(p.hc ?? 0.3).toFixed(3), i[`t${d}`] = +kt(p.t ?? 8e-3).toFixed(3), t.addBinding(i, `bc${d}`, {
            min: l[0],
            max: l[1],
            step: l[2],
            label: `b sv${n}${r + 1}`
        ...
        t.on("change", (r) => {
          ...
          for (let i = 0; i < o.length; i++) {
            const d = `${n}${i}`;
            p === `bc${d}` && (o[i].bc = Tt(r.value), Oe()), p === `hc${d}` && (o[i].hc = Tt(r.value), Oe()), p === `t${d}` && (o[i].t = Tt(r.value), Oe());
          }
        });
      }
    }
    function No() {
      ...
      for (let m = 0; m < l; m++) {
        ...
        if (qe.colMat === 0) if (qe.colShape === 1) {
          ...
          w.addBinding(A, "dCol", {
            ...
            var _a3;
            ((_a3 = R.target) == null ? void 0 : _a3.key) === "dCol" && (u.dCol = Tt(R.value), Oe());
          });
        } else {
          const A = {
            bCol: +kt(u.bCol).toFixed(2),
            hCol: +kt(u.hCol).toFixed(2)
          };
          w.addBinding(A, "bCol", {
            min: a[0],
            max: a[1],
            ...
            var _a3, _b;
            ((_a3 = R.target) == null ? void 0 : _a3.key) === "bCol" && (u.bCol = Tt(R.value), Oe()), ((_b = R.target) == null ? void 0 : _b.key) === "hCol" && (u.hCol = Tt(R.value), Oe());
          });
        }
          ...
          const A = {
            bf: +kt(u.colBf ?? 0.3).toFixed(3),
            h: +kt(u.colHf ?? 0.3).toFixed(3),
            tf: +kt(u.colTf ?? 0.02).toFixed(3),
            tw: +kt(u.colTw ?? 0.012).toFixed(3)
          };
          w.addBinding(A, "bf", {
            min: a[0],
            max: a[1],
            ...
            var _a3, _b, _c, _d;
            ((_a3 = R.target) == null ? void 0 : _a3.key) === "bf" && (u.colBf = Tt(R.value), Oe()), ((_b = R.target) == null ? void 0 : _b.key) === "h" && (u.colHf = Tt(R.value), Oe()), ((_c = R.target) == null ? void 0 : _c.key) === "tf" && (u.colTf = Tt(R.value), Oe()), ((_d = R.target) == null ? void 0 : _d.key) === "tw" && (u.colTw = Tt(R.value), Oe());
          });
        } else {
          const A = {
            bc: +kt(u.colBc ?? 0.3).toFixed(3),
            hc: +kt(u.colHc ?? 0.3).toFixed(3),
            t: +kt(u.colT ?? 0.01).toFixed(3)
          };
          w.addBinding(A, "bc", {
            min: a[0],
            max: a[1],
            ...
            var _a3, _b, _c;
            ((_a3 = R.target) == null ? void 0 : _a3.key) === "bc" && (u.colBc = Tt(R.value), Oe()), ((_b = R.target) == null ? void 0 : _b.key) === "hc" && (u.colHc = Tt(R.value), Oe()), ((_c = R.target) == null ? void 0 : _c.key) === "t" && (u.colT = Tt(R.value), Oe());
          });
        }
        else {
          const A = {
          ...
          w.addBinding(A, "Es", {
            ...
            ((_a3 = E.target) == null ? void 0 : _a3.key) === "bc" && (u.colBc = Tt(E.value), Oe()), ((_b = E.target) == null ? void 0 : _b.key) === "hc" && (u.colHc = Tt(E.value), Oe()), ((_c = E.target) == null ? void 0 : _c.key) === "t" && (u.colT = Tt(E.value), Oe()), ((_d = E.target) == null ? void 0 : _d.key) === "Es" && (u.colEs = Tn(E.value), Oe()), ((_e2 = E.target) == null ? void 0 : _e2.key) === "nuS" && (u.colNuS = E.value, Oe()), ((_f = E.target) == null ? void 0 : _f.key) === "fc" && (u.colFc = Tn(E.value), Oe()), ((_g = E.target) == null ? void 0 : _g.key) === "nuC" && (u.colNuC = E.value, Oe());
          });
        }
      ...
      y.addBinding(f, "activar", {
        ...
        var _a3, _b, _c, _d;
        ((_a3 = m.target) == null ? void 0 : _a3.key) === "activar" && (Xe = m.value, Oe()), ((_b = m.target) == null ? void 0 : _b.key) === "espesor" && (dt = Tt(m.value), Oe()), ((_c = m.target) == null ? void 0 : _c.key) === "subdivX" && (ut = Math.round(m.value), Oe()), ((_d = m.target) == null ? void 0 : _d.key) === "subdivY" && (It = Math.round(m.value), Oe());
      }), Rt.addBlade({
        view: "separator"
      });
      ...
      h.addBinding(I, "espesor", {
        ...
        var _a3, _b, _c;
        ((_a3 = m.target) == null ? void 0 : _a3.key) === "espesor" && (je = Tt(m.value), Oe()), ((_b = m.target) == null ? void 0 : _b.key) === "subdivH" && (We = Math.round(m.value), Oe()), ((_c = m.target) == null ? void 0 : _c.key) === "subdivW" && (ge = Math.round(m.value), Oe());
      });
...
`);
    ...
    function Fn() {
      ...
      if (!n.loads || n.loads.size === 0) {
        ...
        for (let E = 0; E < o.length; E++) {
          ...
          if (!(Math.abs(F) < 1e-15)) {
            if (v.length === 2) {
              const X = ((_b = l.areas) == null ? void 0 : _b.get(E)) ?? 0, V = t[v[0]], ae = t[v[1]], Q = Math.sqrt((ae[0] - V[0]) ** 2 + (ae[1] - V[1]) ** 2 + (ae[2] - V[2]) ** 2), re = -(F * X * Q * R) / 2;
              j.set(v[0], (j.get(v[0]) ?? 0) + re), j.set(v[1], (j.get(v[1]) ?? 0) + re);
            } else if (v.length >= 3) {
              const X = ((_c = l.thicknesses) == null ? void 0 : _c.get(E)) ?? 0;
              let V = 0;
              if (v.length === 3) {
                const [U, re, le] = v.map((Ee) => t[Ee]);
                V = 0.5 * Math.abs((re[0] - U[0]) * (le[1] - U[1]) - (le[0] - U[0]) * (re[1] - U[1]));
              } else if (v.length === 4) {
                const [U, re, le, Ee] = v.map((Re) => t[Re]);
                if (V = 0.5 * Math.abs((re[0] - U[0]) * (le[1] - U[1]) - (le[0] - U[0]) * (re[1] - U[1])) + 0.5 * Math.abs((le[0] - U[0]) * (Ee[1] - U[1]) - (Ee[0] - U[0]) * (le[1] - U[1])), V < 1e-10) {
                  const Re = [
                    re[0] - U[0],
                    re[1] - U[1],
                    re[2] - U[2]
    ...
    function yo() {
      ...
      if ((T === "edificio" || T === "frame") && D.size > 0) {
        ...
        for (let f = 0; f < t.length; f++) {
          ...
          if (Ft.has(f)) {
            const w = 4700 * Math.sqrt(r / 1e3) * 1e3, A = 0.2;
            n.elasticities.set(f, w), n.poissonsRatios.set(f, A), n.thicknesses.set(f, dt), n.shearModuli.set(f, w / (2 * (1 + A))), n.densities.set(f, 24 / 9.80665);
            continue;
          }
    ...
    window.innerWidth <= 600 && Ce.classList.add("collapsed"), setTimeout(() => {
      ...
      function o(x) {
        ...
        const F = 15e3 * Math.sqrt(210) * 10, X = 0.2, V = F / (2 * (1 + X)), ae = 0.09, Q = 0.3 ** 4 / 12, U = 0.141 * 0.3 ** 4, re = 0.25 * 0.4, le = 0.25 * 0.4 ** 3 / 12, Ee = 0.4 * 0.25 ** 3 / 12, Re = 1e-3, q = 5 / 6 * ae, fe = 5 / 6 * re, te = [];
        function de(ee, me, Se) {
          const se = {
            elasticities: /* @__PURE__ */ new Map(),
            shearModuli: /* @__PURE__ */ new Map(),
            areas: /* @__PURE__ */ new Map(),
          ...
          for (const Ie of me) se.elasticities.set(Ie, F), se.shearModuli.set(Ie, V), se.areas.set(Ie, ae), se.momentsOfInertiaZ.set(Ie, Q), se.momentsOfInertiaY.set(Ie, Q), se.torsionalConstants.set(Ie, U), se.shearAreasY.set(Ie, q), se.shearAreasZ.set(Ie, q);
          for (const Ie of Se) se.elasticities.set(Ie, F), se.shearModuli.set(Ie, V), se.areas.set(Ie, re), se.momentsOfInertiaZ.set(Ie, Ee), se.momentsOfInertiaY.set(Ie, le), se.torsionalConstants.set(Ie, Re), se.shearAreasY.set(Ie, fe), se.shearAreasZ.set(Ie, fe);
          return se;
        }
        if (x === "test-cantilever" || x === "test-all") {
          const Se = 270 / (3 * F * Q), se = [
            [
              0,
              0,
              0
        ...
        if (x === "test-portal-wall" || x === "test-all") {
          ...
          Se.elasticities.set(6, F), Se.shearModuli.set(6, V), Se.thicknesses = /* @__PURE__ */ new Map([
            [
              6,
              0.2
            ]
        ...
        if (x === "test-wilson-beam" || x === "test-all") {
          const at = 0.6666666666666666, lt = [
            [
              0,
              -1,
              0
          ...
          const Qt = 5 ** 3 / (3 * 1500 * at);
          try {
            const Gt = At(lt, St, {
              supports: et,
              loads: nt
            }, ye), qt = Math.abs(((_b2 = (_a3 = Gt.deformations) == null ? void 0 : _a3.get(2)) == null ? void 0 : _b2[1]) ?? 0), st = Math.abs(((_d2 = (_c2 = Gt.deformations) == null ? void 0 : _c2.get(3)) == null ? void 0 : _d2[1]) ?? 0), vt = (qt + st) / 2, oo = vt / Qt;
            te.push({
        ...
        if (x === "test-scordelis" || x === "test-all") {
          const at = 40 * Math.PI / 180, lt = 8, St = 8, ye = [];
          for (let st = 0; st <= lt; st++) for (let vt = 0; vt <= St; vt++) {
            const oo = 25 * st / lt, _t = at * vt / St, ho = 25 * Math.sin(_t), go = 25 * Math.cos(_t) - 25 * Math.cos(at);
            ye.push([
              oo,
              ho,
              go
          ...
          const et = [];
          for (let st = 0; st < lt; st++) for (let vt = 0; vt < St; vt++) {
            const oo = st * (St + 1) + vt, _t = (st + 1) * (St + 1) + vt, ho = (st + 1) * (St + 1) + (vt + 1), go = st * (St + 1) + (vt + 1);
            et.push([
              oo,
              _t,
              ho,
          ...
          for (let st = 0; st < et.length; st++) nt.elasticities.set(st, 432e6), nt.shearModuli.set(st, Qt), nt.thicknesses.set(st, 0.25), nt.poissonsRatios.set(st, 0);
          const Gt = /* @__PURE__ */ new Map();
          for (let st = 0; st <= lt; st++) for (let vt = 0; vt <= St; vt++) {
            const oo = st * (St + 1) + vt, _t = [
              false,
              false,
              false,
              false,
            ...
            st === 0 && (_t[0] = true, _t[4] = true, _t[5] = true), st === lt && (_t[1] = true, _t[2] = true, _t[3] = true), vt === 0 && (_t[1] = true, _t[3] = true, _t[5] = true), _t.some((ho) => ho) && Gt.set(oo, _t);
          }
          const qt = /* @__PURE__ */ new Map();
          for (const st of et) {
            const vt = ye[st[0]], oo = ye[st[1]], _t = ye[st[2]], ho = ye[st[3]], go = [
              _t[0] - vt[0],
              _t[1] - vt[1],
              _t[2] - vt[2]
            ], jo = [
          ...
          try {
            const st = At(ye, et, {
              supports: Gt,
              loads: qt
            }, nt), vt = St, oo = ((_f2 = (_e3 = st.deformations) == null ? void 0 : _e3.get(vt)) == null ? void 0 : _f2[2]) ?? 0;
            te.push({
              name: "Scordelis-Lo Barrel Vault",
              formulation: `Shell Q4 (${lt}x${St} mesh), Mindlin-Reissner + incompatible modes`,
              nodes: ye,
              elements: et,
              results: [
                {
        ...
        if (c(te), te.length > 0) {
          ...
          ee.nodes.forEach((Ie, He) => {
            Math.abs(Ie[2]) < 0.01 && me.set(He, [
              true,
              true,
              true,
              true,
              true,
              true
            ]), Math.abs(Ie[2] - se) < 0.01 && Se.set(He, [
              10,
              0,
              0,
              0,
      ...
      function n(x) {
        ...
        X.forEach((q, fe) => ae.set(q, V[fe])), v.push("$ STORIES - IN SEQUENCE FROM TOP");
        for (let q = X.length - 1; q >= 1; q--) v.push(`  STORY "${V[q]}"  HEIGHT ${X[q] - X[q - 1]} MASTERSTORY "Yes"  `);
        ...
        x.nodes.forEach((q) => {
          const fe = `${q[0]},${q[2]}`;
          U.has(fe) || U.set(fe, `${++re}`);
        }), v.push("$ POINT COORDINATES");
        for (const [q, fe] of U) {
          const [te, de] = q.split(",").map(Number);
          v.push(`  POINT "${fe}"  ${te} ${de} `);
        }
        v.push("");
        const le = (q) => {
        ...
        if (x.elements.forEach((q, fe) => {
          ...
          const te = x.nodes[q[0]], de = x.nodes[q[1]], ee = Math.abs(de[1] - te[1]), me = Math.sqrt((de[0] - te[0]) ** 2 + (de[2] - te[2]) ** 2), Se = ee > me * 0.5, se = le(q[0]), Ie = le(q[1]), He = Se ? "COL30" : "VIGA";
          Se ? (v.push(`  LINE  "E${fe + 1}"  COLUMN  "${se.pt}"  "${se.pt}"  1`), Ee.push(`  LINEASSIGN  "E${fe + 1}"  "${Ie.story}"  SECTION "${He}"  `)) : (v.push(`  LINE  "E${fe + 1}"  BEAM  "${se.pt}"  "${Ie.pt}"  0`), Ee.push(`  LINEASSIGN  "E${fe + 1}"  "${se.story}"  SECTION "${He}"  `));
        }), v.push(""), Q) {
          v.push("$ AREA CONNECTIVITIES");
          const q = [];
          x.elements.forEach((fe, te) => {
            if (fe.length !== 4) return;
            const de = fe.map((ee) => le(ee));
            v.push(`  AREA "W${te + 1}"  PANEL  4  "${de[0].pt}"  "${de[1].pt}"  "${de[2].pt}"  "${de[3].pt}"  1  1  0  0  `), q.push(`  AREAASSIGN  "W${te + 1}"  "${de[2].story}"  SECTION "Muro20"  `);
...
`);
      ...
      function l(x) {
        ...
        v.push(`"""ETABS API Validation: ${x.name}`), v.push('Generated by Awatif FEM Studio"""'), v.push("import comtypes.client, time, math"), v.push(""), v.push("helper = comtypes.client.CreateObject('ETABSv1.Helper')"), v.push("helper = helper.QueryInterface(comtypes.gen.ETABSv1.cHelper)"), v.push('myETABS = helper.CreateObjectProgID("CSI.ETABS.API.ETABSObject")'), v.push("myETABS.ApplicationStart()"), v.push("time.sleep(10)"), v.push("SapModel = myETABS.SapModel"), v.push("SapModel.InitializeNewModel()"), v.push("SapModel.File.NewBlank()"), v.push("SapModel.SetPresentUnits(12)  # tonf_m_C"), v.push(""), v.push(`E = ${E}`), v.push('SapModel.PropMaterial.SetMaterial("CONC", 2)'), v.push('SapModel.PropMaterial.SetMPIsotropic("CONC", E, 0.2, 5.5e-6)'), v.push('SapModel.PropFrame.SetRectangle("COL30", "CONC", 0.30, 0.30)'), v.push('SapModel.PropFrame.SetRectangle("VIGA", "CONC", 0.40, 0.25)'), x.elements.some((V) => V.length === 4) && v.push('SapModel.PropArea.SetWall("Muro20", 6, False, "CONC", 0.20)'), v.push(""), v.push("# Add elements"), v.push("FN = ' '"), x.elements.forEach((V, ae) => {
          if (V.length === 2) {
            const Q = x.nodes[V[0]], U = x.nodes[V[1]], re = Math.abs(U[1] - Q[1]), le = Math.sqrt((U[0] - Q[0]) ** 2 + (U[2] - Q[2]) ** 2), Ee = re > le * 0.5 ? "COL30" : "VIGA";
            v.push(`[FN,r]=SapModel.FrameObj.AddByCoord(${Q[0]},${Q[2]},${Q[1]}, ${U[0]},${U[2]},${U[1]}, FN,"${Ee}","E${ae + 1}","Global")`);
          } else if (V.length === 4) {
...
`);
      ...
      function c(x) {
        ...
        for (let V = 0; V < x.length; V++) {
          ...
          for (const Q of ae.results) {
            const U = Q.reference !== 0 ? Q.awatif / Q.reference : 1, re = Math.abs(U - 1) < 0.05;
            re || (F = false);
            const le = re ? "#4caf50" : "#f44336", Ee = re ? "PASS" : "FAIL";
      ...
      function d(x, E) {
        ...
        for (const U of X) {
          ...
          <span>${ae[U] || U}</span>
          <span style="color:#888;margin-left:auto">(${re})</span>
        </label>`;
        }
        ...
        for (const U of V) {
          const re = v[U] || 0;
          re !== 0 && (Q += `<label style="display:flex;align-items:center;gap:6px;padding:2px 0;color:#888">
          <input type="checkbox" data-ifc-cat="${U}" disabled>
          <span>${ae[U] || U}</span>
          <span style="margin-left:auto">(${re})</span>
        </label>`);
        }
        Q += `</div>
        <div style="display:flex;gap:8px">
          ...
          U.addEventListener("change", () => {
            const re = U.dataset.ifcCat, le = x.detailCategories.get(re);
            if (le) {
          ...
          for (const te of E.elements) if (U.has(te.category) && te.type === "frame" && te.nodeIds.length >= 2) {
            ...
            const de = ((_a4 = E.materials) == null ? void 0 : _a4.get(te.material)) || {
              ...
              nu: 0.2,
              rho: 2.4
            }, ee = te.b || 0.3, me = te.h || 0.3, Se = ee * me, se = ee * me * me * me / 12, Ie = me * ee * ee * ee / 12, He = ee * me * (ee * ee + me * me) / 12, at = de.E / (2 * (1 + de.nu));
            Ee.elasticities.set(q, de.E), Ee.shearModuli.set(q, at), Ee.areas.set(q, Se), Ee.momentsOfInertiaZ.set(q, Ie), Ee.momentsOfInertiaY.set(q, se), Ee.torsionalConstants.set(q, He), Ee.densities.set(q, de.rho), Ee.sectionShapes.set(q, {
              type: "rect",
              b: ee,
              h: me,
              name: te.sectionName
          ...
          const fe = Math.min(...re.map((te) => te[2]));
          re.forEach((te, de) => {
            Math.abs(te[2] - fe) < 0.05 && Re.supports.set(de, [
              true,
              true,
              true,
              true,
          ...
          for (const [, re] of x.detailCategories) {
            const le = Ze();
            le && le.scene.remove(re);
          }
          const U = Ze();
          U && U.render(), F.remove();
        });
      ...
      function g(x) {
        ...
        for (let le = 0; le < x.elementTypes.length; le++) {
          const Ee = x.elementTypes[le], Re = x.elementStories[le], q = E.get(Re) ?? 0;
        ...
        if (v.length > 0 || F.length > 0) X = v.map((le) => le.coord), V = F.map((le) => le.coord), ae = v.map((le) => le.label), Q = F.map((le) => le.label);
          ...
          const le = new Set(x.nodes.map((Re) => Re[0])), Ee = new Set(x.nodes.map((Re) => Re[1]));
          X = [
            ...le
          ].sort((Re, q) => Re - q), V = [
            ...Ee
          ].sort((Re, q) => Re - q), ae = X.map((Re, q) => String(q + 1)), Q = V.map((Re, q) => String.fromCharCode(65 + q));
        }
        const U = x.stories.length > 0 ? Math.max(...x.stories.map((le) => le.elev)) : Math.max(...x.nodes.map((le) => le[2]));
        Ge = X, Ke = V, mt = U, setTimeout(() => {
        ...
        for (const le of x.elementTypes) re[le]++;
        console.log(`E2K grids: X=[${ae.join(",")}] Y=[${Q.join(",")}]`), console.log(`E2K stories: ${x.stories.map((le) => `${le.name}@${le.elev.toFixed(2)}`).join(", ")}`), console.log(`E2K elements: ${re.COLUMN} columns, ${re.BEAM} beams, ${re.BRACE} braces`), tt();
      }
      function S(x, E) {
        const v = new Blob([
          x
        ...
        if (s === "import-ifc") {
          ...
          v.onload = async () => {
            ...
            try {
              ...
              re.onload = () => {
                const le = re.result, Ee = Wl(le);
                window.__ifcAnalytical = Ee;
                const Re = {};
                V.elementInfo.forEach((q) => Re[q.category] = (Re[q.category] || 0) + 1), console.log("IFC categories:", Re), console.log(`IFC: ${V.elementInfo.size} geometric elements, ${Ee.elements.length} analytical elements`), d(V, Ee);
              }, re.readAsText(x);
            } catch (X) {
...
`));
        ...
        if ((x.key === "Delete" || x.key === "Backspace") && $t.size > 0) {
          x.preventDefault(), $t.forEach((E) => Z.add(E)), $t.clear(), uo && (uo.remove(), uo = null), Oe();
          return;
        }
        if (x.key === "Escape") {
          if (fo) if (Et !== null) {
...
`);
    ...
    let so = false, Vt = false, fo = false, Wt = "line", Kt = [], Et = null, Dt = null, Bt = null, qo = null, eo = null;
    const zt = {
      node: true,
      grid: true,
      midpoint: true,
...
${i.join(", ")}`);
        ...
        $t.forEach((n) => Y.add(n)), $t.clear(), Eo(), So(), Oe();
        ...
        Do(), $t.forEach((n) => Z.add(n)), $t.clear(), Eo(), So(), Oe();
    ...
    function cn() {
      var _a2;
      const t = Ze();
      Dt && t && (t.scene.remove(Dt), Dt.geometry.dispose(), Dt.material.dispose(), Dt = null), Bt && t && (t.scene.remove(Bt), Bt.geometry.dispose(), Bt.material.dispose(), Bt = null), Hn(), Et = null, eo = null, fo = false, qo && (qo.remove(), qo = null), (_a2 = Ce.querySelector("#cad3d-draw")) == null ? void 0 : _a2.classList.remove("inspect-active"), t == null ? void 0 : t.render();
    }
    function Ca() {
      qo && qo.remove();
      ...
      <button id="ds-node" style="${n(zt.node)}">Node</button>
      <button id="ds-grid" style="${n(zt.grid)}">Grid</button>
      <button id="ds-mid" style="${n(zt.midpoint)}">Mid</button>
      <button id="ds-track" style="${n(zt.track)}">Prolong</button>
      <span style="color:#666;margin:0 4px;">|</span>
    ...
    `, document.body.appendChild(t), qo = t;
      const l = () => {
        ...
        p && (p.style.cssText = n(zt.node)), i && (i.style.cssText = n(zt.grid)), d && (d.style.cssText = n(zt.midpoint)), g && (g.style.cssText = n(zt.track));
      };
      t.querySelector("#dt-line").addEventListener("click", () => {
        Wt = "line", Et = null, eo = null, Kt = [], l();
      }), t.querySelector("#dt-arc").addEventListener("click", () => {
        Wt = "arc", Et = null, eo = null, Kt = [], l();
      }), t.querySelector("#dt-node").addEventListener("click", () => {
        Wt = "node", Et = null, eo = null, Kt = [], l();
      }), t.querySelector("#dt-area").addEventListener("click", () => {
        Wt = "area", Et = null, eo = null, Kt = [], console.log("Area mode: click vertices del poligono. Doble-click o click cerca del 1er punto para cerrar."), l();
    ...
    function Rs(t) {
      ...
      if (Dt && (o.scene.remove(Dt), Dt.geometry.dispose(), Dt.material.dispose(), Dt = null), Et !== null && t.worldPos) {
    ...
    function Fa(t) {
      ...
      if (Wt === "area") {
        ...
        if (Kt.push(o), console.log(`Area vertex ${Kt.length}: node ${o}`), Kt.length >= 2) {
          const n = Kt[Kt.length - 2], l = e.nodes.val, a = Ze();
          if (a) {
    ...
    function qs(t) {
      ...
      if (c) {
        ...
        let X = "";
        for (let V = 0; V < 6; V++) {
          const ae = V, Q = V + 6, U = (E.length >= 12 ? E[ae] : V >= 3 && E.length >= 6 && E[V - 3]) ? "checked" : "", re = (E.length >= 12 ? E[Q] : V >= 3 && E.length >= 6 && E[V]) ? "checked" : "", le = v.length >= 12 && v[ae] > 0 ? v[ae].toFixed(1) : "", Ee = v.length >= 12 && v[Q] > 0 ? v[Q].toFixed(1) : "";
          X += `<tr>
          <td style="text-align:left;color:var(--fem-key)">${F[V]}</td>
          <td style="text-align:center"><input type="checkbox" data-rel="${ae}" ${U}></td>
          <td style="text-align:center"><input type="checkbox" data-rel="${Q}" ${re}></td>
          ...
          <td><input type="number" data-spr="${Q}" value="${Ee}" placeholder="0" style="width:50px;background:var(--fem-bg);color:var(--fem-val);border:1px solid var(--fem-border);font-size:10px;text-align:right"></td>
        </tr>`;
        }
      ...
      if (p && c && (r == null ? void 0 : r.deformations) && I && L) {
        ...
        const X = (Q, U) => Q.map((re, le) => `<span style="color:${Math.abs(re) > 1e-10 ? "var(--fem-nonzero)" : "var(--fem-eq-dots)"}">${U[le % 6]}=${we(re)}</span>`).join(", "), ae = [
          "N",
          "Vy",
          "Vz",
          "Mx",
        ...
        `${C("u", "global")}${l.map((Q, U) => `<span style="color:var(--fem-label)">nodo ${Q}:</span> ${x.map((re, le) => `<span style="color:${Math.abs(E[U * 6 + le]) > 1e-10 ? "var(--fem-eq-var)" : "var(--fem-eq-dots)"}">${we(E[U * 6 + le])}</span>`).join(", ")}`).join(" | ")}${C("u", "local")}${C("T")}${C("u", "global")}${C("u", "local")}${X(v, [
          ...x,
          ...x
      ...
      `${l[0]}`, 6 * l[0], 6 * l[0] + 5, `${l[1]}`, 6 * l[1], 6 * l[1] + 5, l.length === 3 && (`${l[2]}`, 6 * l[2], 6 * l[2] + 5), o.length * 6, o.length * 6, Zt = yl(t, o, n, s, r, p), Zt.id = "fem-inspect-panel", document.body.appendChild(Zt), (_N = Zt.querySelector("#er-close")) == null ? void 0 : _N.addEventListener("click", () => Ao()), (_O = Zt.querySelector("#rel-apply")) == null ? void 0 : _O.addEventListener("click", () => {
        const m = Zt.querySelectorAll("input[data-rel]"), u = Zt.querySelectorAll("input[data-spr]"), w = new Array(12).fill(false), A = new Array(12).fill(0);
        m.forEach((j) => {
    ...
    function Vs() {
      ...
      for (const ye of x) E.some((et) => Math.abs(et - ye) < 1e-6) || E.push(ye);
      const v = E.sort((ye, et) => ye - et), F = [], X = [];
      ...
      const Q = v.length, U = [], re = [];
      for (let ye = 0; ye < Q; ye++) {
        const et = v[ye], nt = x.findIndex((Qt) => Math.abs(Qt - et) < 1e-6);
        nt >= 0 ? (U.push(V[nt]), re.push(ae[nt])) : (U.push(w.length), w.push([
          0,
          et,
          l
        ]), re.push(w.length), w.push([
          o,
          et,
          a
        ]));
      ...
      for (let ye = 0; ye < c; ye++) A.push([
        F[ye],
        V[ye]
      ]), le.push("col"), Ee++, A.push([
        X[ye],
        ae[ye]
      ]), le.push("col"), Ee++;
      for (let ye = 0; ye < Q - 1; ye++) A.push([
        U[ye],
        U[ye + 1]
      ]), le.push("vig"), Ee++, A.push([
        re[ye],
        re[ye + 1]
      ]), le.push("vig"), Ee++;
      A.length;
      for (let ye = 0; ye < Q; ye++) A.push([
        U[ye],
        re[ye]
      ]), le.push("corr"), Ee++;
      A.length;
      ...
      for (let ye = 0; ye < Q; ye++) {
        let et;
        ye === 0 ? et = (v[1] - v[0]) / 2 : ye === Q - 1 ? et = (v[Q - 1] - v[Q - 2]) / 2 : et = (v[ye + 1] - v[ye - 1]) / 2;
        const nt = et * o, Gt = -i * nt * 1 / Re, qt = (st) => {
          const vt = j.get(st) || [
            0,
            0,
            0,
      ...
      const q = /* @__PURE__ */ new Map(), fe = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), de = /* @__PURE__ */ new Map(), ee = /* @__PURE__ */ new Map(), me = /* @__PURE__ */ new Map(), Se = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), He = /* @__PURE__ */ new Map(), at = t.rho;
      for (let ye = 0; ye < A.length; ye++) {
        q.set(ye, r), ee.set(ye, g), Se.set(ye, at);
        const et = le[ye];
        if (et === "col") {
          fe.set(ye, S), te.set(ye, $), de.set(ye, y), me.set(ye, f);
          const nt = new Array(12).fill(false);
          nt[10] = true, nt[11] = true, He.set(ye, nt);
        } else if (et === "vig") fe.set(ye, h), te.set(ye, I), de.set(ye, L), me.set(ye, z);
        else if (et === "corr") {
          fe.set(ye, O), te.set(ye, k), de.set(ye, m), me.set(ye, u);
          const nt = new Array(12).fill(false);
          nt[4] = true, nt[5] = true, nt[10] = true, nt[11] = true, He.set(ye, nt);
        } else et === "shell" && (se.set(ye, d), Ie.set(ye, p));
      }
      const lt = {
        elasticities: q,
        areas: fe,
      ...
      e.elementInputs && (e.elementInputs.val = lt);
      try {
        const ye = At(w, A, {
          supports: R,
          loads: j
        ...
        if (ye && e.deformOutputs) {
          ...
          const et = bo(w, A, lt, ye);
          e.analyzeOutputs && (e.analyzeOutputs.val = et);
          let nt = 0, Qt = 0;
          ye.deformations.forEach((Gt, qt) => {
            Math.abs(Gt[2]) > Math.abs(nt) && (nt = Gt[2], Qt = qt);
          }), console.log(`P\xE9rgola: Uz_max=${nt.toExponential(4)} m en nodo ${Qt} | ${Ee} frames + ${s - 1} shells`);
        }
      } catch (ye) {
        console.warn("Pergola:", ye.message);
      }
      const St = Ze();
      St && (St.settings.shellResults.val = "displacementZ"), setTimeout(() => wt(), 50), tt();
    }
    function Da() {
      var _a2, _b;
    ...
    function Wa(t) {
      ...
      const k = [];
      for (const V of p) {
        let ae = $, Q = y, U = f, re = h, le = I, Ee = L, Re = z, q = O, fe;
        const te = V - g;
        if (Math.abs(te) < 1e-20) {
          k.push(S);
          continue;
        }
        if ((q === 0 || q === 3) && (te < 0 ? (q = 2, re = -i, le = -o, U = re, Ee = 0, Re = 0) : (q = 1, re = i, le = o, U = re, Ee = 0, Re = 0)), q === 2 && te > 0) {
          q = 1, Ee = g, Re = S, g < ae && (ae = g);
          const Ie = (Q - ae) / (2 * 1 * i), He = 1 + 0 * Math.pow(Ie, 0.8);
          re = (o * He - d * i * He - Re + n * Ee) / (n - d), le = o * He + d * (re - i * He), U = Q;
        } else if (q === 1 && te < 0) {
          q = 2, Ee = g, Re = S, g > Q && (Q = g);
          const Ie = (Q - ae) / (2 * 1 * i), He = 1 + 0 * Math.pow(Ie, 0.8);
          re = (-o * He + d * i * He - Re + n * Ee) / (n - d), le = -o * He + d * (re + i * He), U = ae;
        }
        const de = Math.abs((U - re) / i);
        let ee = a - 0.925 * de / (0.15 + de);
        ee < 0.1 && (ee = 0.1);
        const me = (V - Ee) / (re - Ee), Se = 1 + Math.pow(Math.abs(me), ee), se = Math.pow(Se, 1 / ee);
        fe = l * me + (1 - l) * me / se, fe = fe * (le - Re) + Re, k.push(fe), g = V, S = fe, $ = ae, y = Q, f = U, h = re, I = le, L = Ee, z = Re, O = q;
      }
    ...
    setTimeout(() => {
      ...
      o.addEventListener("mousedown", (p) => {
        ...
        if (fo) {
          ...
          if (zt.track && g.snapType === "node" && g.nodeIdx !== null && g.nodeIdx !== zo && Aa(g.nodeIdx), zt.track && zo !== null && g.worldPos && g.snapType !== "node") {
