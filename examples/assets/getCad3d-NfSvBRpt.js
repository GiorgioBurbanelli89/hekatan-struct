  ...
  er = function(e) {
    ...
    function Xn(t, n, o, l, s) {
      ...
      for (let m = 0; m < t.length - 1; m++) {
        const u = t[m], E = t[m + 1], A = Math.abs(E - u), O = (u + E) / 2, W = `${A.toFixed(2)} m`;
      ...
      if (!I) for (let m = 0; m < n.length - 1; m++) {
        const u = n[m], E = n[m + 1], A = Math.abs(E - u), O = (u + E) / 2, W = `${A.toFixed(2)} m`;
    ...
    const Ke = {
      ...
      build(t) {
        ...
        const I = [], L = /* @__PURE__ */ new Set(), z = /* @__PURE__ */ new Set(), P = /* @__PURE__ */ new Map();
        ...
        const u = 15100 * Math.sqrt(l) * 10, E = u / (2 * (1 + 0.2)), A = s * c, O = s * c ** 3 / 12, W = c * s ** 3 / 12, x = s * c * (s ** 2 + c ** 2) / 12, w = a * r, v = a * r ** 3 / 12, F = r * a ** 3 / 12, X = a * r * (a ** 2 + r ** 2) / 12, V = /* @__PURE__ */ new Map(), ae = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map();
        for (let _ = 0; _ < I.length; _++) V.set(_, u), ae.set(_, E), L.has(_) ? (Q.set(_, A), K.set(_, O), pe.set(_, W), ce.set(_, x), Ie.set(_, {
          type: "rect",
          b: s,
          h: c,
          name: `COL${n}`
        })) : m.has(_) ? (Q.set(_, A), K.set(_, O), pe.set(_, W), ce.set(_, x), Ie.set(_, {
          type: "rect",
          b: s,
          h: c,
          name: `BR${n}`
        ...
        return e.nodes.val = p, e.elements.val = I, e.nodeInputs.val = {
          ...
          sectionShapes: Ie
        }, D = L, G = z, $e = P, console.log(`Built: ${p.length} nodes, ${I.length} elements (${L.size} cols, ${z.size} beams, ${m.size} braces)`), console.log(`  Col: ${n}, Viga: ${o}, f'c=${l}${k ? `, braces=${k}` : ""}`), {
          nodes: p.length,
          elements: I.length
        };
      },
      ...
      addBrace(t, n, o, l, s, c) {
        ...
        const S = We[a].coord, $ = Xe[r].coord, y = lt[f].elev, p = We[i].coord, h = Xe[d].coord, I = lt[b].elev, L = [
          ...
          const O = L.findIndex((W) => Math.abs(W[0] - u) < 1e-3 && Math.abs(W[1] - E) < 1e-3 && Math.abs(W[2] - A) < 1e-3);
          return O >= 0 ? O : (L.push([
            u,
            E,
            A
...
`;
      ...
      model3d(t) {
        ...
        const A = 15100 * Math.sqrt(a) * 10, O = A / (2 * (1 + 0.2)), W = r * f, x = r * f ** 3 / 12, w = f * r ** 3 / 12, v = r * f * (r ** 2 + f ** 2) / 12, F = i * d, X = i * d ** 3 / 12, V = d * i ** 3 / 12, ae = i * d * (i ** 2 + d ** 2) / 12, Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), Pe = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < P.length; ee++) Q.set(ee, A), K.set(ee, O), k.has(ee) ? (pe.set(ee, W), ce.set(ee, x), Ie.set(ee, w), Pe.set(ee, v), _.set(ee, {
          type: "rect",
          b: r,
          h: f,
          name: `COL${s}`
        ...
        e.nodes.val = L, e.elements.val = P, e.nodeInputs.val = {
          supports: ue,
          loads: /* @__PURE__ */ new Map()
        }, e.elementInputs.val = {
          elasticities: Q,
        ...
        return as(b, S, $), Qe(), console.log(`Model3D: ${L.length}n ${P.length}e | ${y}x${p} grid, ${I} floors | COL${s} V${c} f'c=${a}`), {
          nodes: L.length,
          elements: P.length,
          columns: k.size,
          beams: m.size
      ...
      building(t, n, o, l = 3, s = 0, c = 0, a = 0, r = 0, f = 1) {
        ...
        Te = /* @__PURE__ */ new Map();
        const P = [];
        for (let u = 1; u < b.length; u++) for (let E = 0; E < d.length; E++) for (let A = 0; A < i.length - 1; A++) P.push({
          el: [
            h[`${A},${E},${u}`],
            h[`${A + 1},${E},${u}`]
          ],
        ...
        for (let u = 1; u < b.length; u++) for (let E = 0; E < i.length; E++) for (let A = 0; A < d.length - 1; A++) P.push({
          el: [
            h[`${E},${A},${u}`],
            h[`${E},${A + 1},${u}`]
          ],
        ...
        for (const { el: [u, E], floor: A, dir: O, bay: W } of P) {
          const x = p[u], w = p[E];
          let v = u;
          for (let X = 1; X < l; X++) {
        ...
        if (st = /* @__PURE__ */ new Set(), Ee && ze > 0) {
          const u = (E, A, O) => {
            for (let x = 0; x < p.length; x++) if (Math.abs(p[x][0] - E) < 1e-6 && Math.abs(p[x][1] - A) < 1e-6 && Math.abs(p[x][2] - O) < 1e-6) return x;
            const W = p.length;
            return p.push([
              E,
              A,
        ...
        for (const u of ve) {
          ...
          for (const O of A) {
            ...
            for (let ue = 0; ue < K; ue++) for (let oe = 0; oe < Q; oe++) {
              const fe = _[ue][oe], ee = _[ue][oe + 1], me = _[ue + 1][oe + 1], we = _[ue + 1][oe], se = L.length;
              de.add(se), $e.set(se, O), L.push([
                fe,
                ee,
                me,
                we
        ...
        if (zt = /* @__PURE__ */ new Set(), Ue) {
          const u = l, E = l, A = /* @__PURE__ */ new Map(), O = (W, x, w) => `${Math.round(W * 1e4)},${Math.round(x * 1e4)},${Math.round(w * 1e4)}`;
          for (let W = 0; W < p.length; W++) A.set(O(p[W][0], p[W][1], p[W][2]), W);
            ...
            if (u) for (let W = 0; W < d.length; W++) {
              ...
              for (let w = 0; w < i.length - 1; w++) {
                ...
                v !== void 0 && F !== void 0 && (L.push([
                  v,
                  F
                ]), $e.set(L.length - 1, O)), X !== void 0 && V !== void 0 && (L.push([
                  X,
                  V
                ]), $e.set(L.length - 1, O));
              }
...
Util:     cad.info()  cad.clear()  cad.help()
    ...
    function vs() {
      const t = le("H") || 4, n = le("B") || 3, o = le("tw") || 0.3, l = le("tb") || 0.4, s = le("meshSize") || 0.2, c = le("E") || 25e6, a = le("nu") || 0.2, r = c / (2 * (1 + a)), f = le("gamma") || 18, i = le("Ka") || 0.33, d = le("Es") || 5e4, b = le("nus") || 0.3, S = d / (2 * (1 + b)), $ = le("kn") || 1e6, y = le("ks") || 1e4, p = le("gammaW") || 9.81, h = le("Hw") || 3.5, I = le("qs") || 0, L = xt, z = n * 0.3, P = n * 0.7, k = [
        [
          -z,
          0,
          0
      ...
      let m = [], u = [], E = /* @__PURE__ */ new Map(), A = /* @__PURE__ */ new Map(), O;
      if (L === 0) {
        const w = hn({
          points: k,
          polygon: [
        ...
        if (L === 2) {
          ...
          if (I > 0) {
            ...
            for (let Se = 0; Se < se.length; Se++) {
              ...
              const rt = -I * je, ct = A.get(se[Se].idx) || [
                0,
                0,
                0,
                0,
    ...
    function ys() {
      ...
      for (let E = 0; E <= a; E++) for (let A = 0; A <= c; A++) {
        const O = E * (c + 1) + A;
        let W = $, x = y;
        (A === 0 || A === c) && (W = $ / 2), (E === 0 || E === a) && (x = y / 2), L.push({
          node: O,
          dof: 0,
          k: S * W * x
        });
    ...
    function Ms() {
      const t = le("colB") || 0.3, n = le("colH") || 0.3, o = le("colT") || 8e-3, l = le("colLen") || 1.5, s = le("Lx") || 0.45, c = le("Ly") || 0.45, a = le("tPlaca") || 0.025, r = le("dBolt") || 0.022, f = le("sx") || 0.32, i = le("sy") || 0.32, d = Math.round(le("nSubColV") || 6), b = Math.round(le("nSubColH") || 4), S = Math.round(le("nSubPlaca") || 10), $ = le("E") || 2e8, y = le("nu") || 0.3, p = $ / (2 * (1 + y)), h = le("P") || -300, I = s / 2, L = c / 2, z = r / 2, P = t / 2, k = n / 2, m = [], u = [], E = S, A = s / E, O = c / E, W = (fe, ee) => ee * (E + 1) + fe;
      ...
      const F = u.length - v, X = d, V = b, ae = [
        ...
        for (let me = 0; me < (E + 1) * (E + 1); me++) if (Math.abs(m[me][0] - fe) < A * 0.3 && Math.abs(m[me][1] - ee) < O * 0.3 && Math.abs(m[me][2]) < 1e-6) return me;
        return -1;
      };
    ...
    function Nn() {
      ...
      const L = re.length || 1, z = te.length || 1, P = L + 1, k = z + 1;
      if (L > 0) {
        const m = h.addFolder({
        ...
        for (let u = 0; u < L; u++) for (let E = 0; E < k; E++) {
          const A = `wx_${u}_${E}`, O = ve.some((w) => w.dir === "x" && w.bay === u && w.axisIdx === E), W = {};
          W[A] = O;
      ...
      if (z > 0) {
        ...
        for (let u = 0; u < z; u++) for (let E = 0; E < P; E++) {
          const A = `wy_${u}_${E}`, O = ve.some((w) => w.dir === "y" && w.bay === u && w.axisIdx === E), W = {};
          W[A] = O;
...
`);
      ...
      function L() {
        ...
        if (y.length > 0) {
          ...
          for (const m of c) if (m.length === 3) {
            ...
            const [u, E, A, O] = m;
            k.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), k.push(p[E * 3], p[E * 3 + 1], p[E * 3 + 2]), k.push(p[A * 3], p[A * 3 + 1], p[A * 3 + 2]), k.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), k.push(p[A * 3], p[A * 3 + 1], p[A * 3 + 2]), k.push(p[O * 3], p[O * 3 + 1], p[O * 3 + 2]);
          }
          if (k.length > 0) {
            const m = new Float32Array(k);
            for (const u of y) {
              const E = u.geometry, A = E.getAttribute("position");
              if (A && A.array.length === m.length) A.array.set(m), A.needsUpdate = true;
              else {
                if (A && A.array.length !== m.length) continue;
                E.setAttribute("position", new Ln(m, 3));
              }
            }
          }
        }
    ...
    function Fo() {
      ...
      if (!o.loads || o.loads.size === 0) {
        ...
        for (const w of P) {
          const v = E.has(w) ? I : 0, F = A.has(w) ? L : 0, X = W.get(w) ?? 0, V = x.has(w) ? h : 0, ae = X + V;
    ...
    function Ia(t, n, o, l, s, c, a) {
      const f = 4700 * Math.sqrt(c / 1e3) * 1e3 / l, i = t - 2 * o, d = n - 2 * o, b = t * n - i * d, S = (t * n * n * n - i * d * d * d) / 12, $ = (n * t * t * t - d * i * i * i) / 12, y = i * d, p = i * d * d * d / 12, h = d * i * i * i / 12, I = b + f * y, L = S + f * p, z = $ + f * h, P = l / (2 * (1 + s)), k = (t - o) * (n - o), m = 2 * ((t - o) / o + (n - o) / o), u = 4 * k * k / (m > 0 ? m : 1);
      return {
        A: I,
        Iz: L,
        Iy: z,
    ...
    function vn() {
      ...
      if ((T === "edificio" || T === "frame") && D.size > 0) {
        ...
        for (let p = 0; p < t.length; p++) {
          ...
          let z, P, k, m;
          if (h) if (s === 0) P = i, k = d, m = b, z = a === 1 ? ks(L.dCol) : Ro(L.bCol, L.hCol), o.sectionShapes.set(p, a === 1 ? {
            type: "circ",
            d: L.dCol
          } : {
            type: "rect",
            ...
            if (E <= 1) {
              ...
              const A = L.colBf ?? 0.3, O = L.colHf ?? 0.3, W = L.colTf ?? 0.02, x = L.colTw ?? 0.012;
              z = Po(A, O, W, x);
              const w = `I${(O * 100).toFixed(0)}x${(A * 100).toFixed(0)}`;
              o.sectionShapes.set(p, {
                type: "I",
                b: A,
                h: O,
              ...
              const A = L.colBc ?? 0.3, O = L.colHc ?? 0.3, W = L.colT ?? 0.01;
              z = Oo(A, O, W);
              const x = `\u25A1${(O * 100).toFixed(0)}x${(A * 100).toFixed(0)}x${(W * 1e3).toFixed(0)}`;
              o.sectionShapes.set(p, {
                type: "HSS",
                b: A,
                h: O,
            ...
            const E = L.colBc ?? 0.3, A = L.colHc ?? 0.3, O = L.colT ?? 0.01, W = L.colFc ?? 28e3, x = L.colEs ?? 2e8, w = L.colNuS ?? 0.3;
            L.colNuC;
            const v = Ia(E, A, O, x, w, W);
            z = {
              A: v.A,
              Iz: v.Iz,
              Iy: v.Iy,
            ...
            const V = `CFT ${(A * 1e3).toFixed(0)}X${(E * 1e3).toFixed(0)}X${(O * 1e3).toFixed(0)}`;
            o.sectionShapes.set(p, {
              type: "CFT",
              b: E,
              h: A,
            ...
            const E = Te.get(p), A = E ? E.dir === "x" ? L.vigasX : L.vigasY : [], O = E ? A[E.bay] ?? A[0] ?? ut() : ut();
...
`);
      ...
      const I = Le.querySelector("#cad3d-cli-toggle"), L = Le.querySelector("#cad3d-cli-panel"), z = Le.querySelector("#cad3d-cli-output"), P = Le.querySelector("#cad3d-cmd"), k = [];
      let m = -1;
      I == null ? void 0 : I.addEventListener("click", (x) => {
        if (x.stopPropagation(), L) {
          const w = L.style.display !== "none";
          L.style.display = w ? "none" : "block", w || (P == null ? void 0 : P.focus(), z && !z.textContent && (z.textContent = `CLI ready. Commands:
...
`);
      ...
      Le.addEventListener("mousedown", (x) => {
        ...
        Le.style.bottom = "unset", E = x.clientX, A = x.clientY, O = v.left, W = v.top, x.preventDefault();
      }), window.addEventListener("mousemove", (x) => {
        u && (x.preventDefault(), Le.style.left = O + (x.clientX - E) + "px", Le.style.top = W + (x.clientY - A) + "px");
...
${i.join(", ")}`);
    ...
    function Fs(t, n, o, l) {
      ...
      if (Et.midpoint) {
        ...
        for (const p of i) {
          ...
          const h = f[p[0]], I = f[p[1]], L = new Oe((h[0] + I[0]) / 2, (h[1] + I[1]) / 2, (h[2] + I[2]) / 2), z = L.clone().project(o), P = Math.sqrt((z.x - c) ** 2 + (z.y - a) ** 2);
          P < d * 0.8 && P < $ && ($ = P, y = L);
        }
        if (y) return {
          nodeIdx: null,
    ...
    function Ns() {
      ...
      for (let m = 0; m < 12; m++) {
        ...
        for (let u = 0; u < 12; u++) if (u < m) k += `<td style="color:var(--fem-border-cell)">${u === 0 && m > 0 ? I : ""}</td>`;
          ...
          const E = P[m][u], A = (m === u ? "diag " : "") + (E !== "0" ? "nz" : "");
          k += `<td class="${A}">${E}</td>`;
        }
        k += "</tr>";
      }
    ...
    function qs(t) {
      ...
      if (c) {
        const m = Pn(xn(s[1], s[0])), u = ((_d = a.elasticities) == null ? void 0 : _d.get(t)) ?? 0, E = ((_e2 = a.areas) == null ? void 0 : _e2.get(t)) ?? 0, A = ((_f = a.momentsOfInertiaZ) == null ? void 0 : _f.get(t)) ?? 0, O = ((_g = a.momentsOfInertiaY) == null ? void 0 : _g.get(t)) ?? 0, W = ((_h = a.shearModuli) == null ? void 0 : _h.get(t)) ?? 0, x = ((_i = a.torsionalConstants) == null ? void 0 : _i.get(t)) ?? 0, w = ((_j = a.momentReleases) == null ? void 0 : _j.get(t)) || [], v = ((_k = a.partialFixitySprings) == null ? void 0 : _k.get(t)) || [], F = [
          "P (Axial)",
          "V2 (Corte)",
          "V3 (Corte)",
          "T (Torsi\xF3n)",
        ...
        `${l[0]}${l[1]}${Me(m)}${Me(u)}${Me(E)}${Me(A)}${Me(O)}${Me(W)}${Me(x)}${X}`;
      } else {
        const m = ((_l2 = a.elasticities) == null ? void 0 : _l2.get(t)) ?? 0, u = ((_m = a.thicknesses) == null ? void 0 : _m.get(t)) ?? 0, E = ((_n2 = a.poissonsRatios) == null ? void 0 : _n2.get(t)) ?? 0, A = m / (2 * (1 + E)), O = l.length === 4, W = m / (1 - E * E);
        `${l.length}${l.join(", ")}${Me(m)}${Me(A)}${Me(u)}${Me(E)}`, O && (S = `<div class="fem-eq eq-box">
      ...
      let i = "", d = "", b = "", S = "", $ = "", y = "", p = "", h = "", I = null, L = null, z = null, P = [];
      try {
        if (I = vo(s, a, t), L = yo(s), z = on(os(L), on(I, L)), P = c ? [
          "ux\u2080",
          "uy\u2080",
          "uz\u2080",
          "\u03B8x\u2080",
          ...
          const A = Pn(xn(s[1], s[0])), O = ((_o2 = a.elasticities) == null ? void 0 : _o2.get(t)) ?? 0, W = ((_p = a.areas) == null ? void 0 : _p.get(t)) ?? 0, x = ((_q = a.momentsOfInertiaZ) == null ? void 0 : _q.get(t)) ?? 0, w = ((_r = a.momentsOfInertiaY) == null ? void 0 : _r.get(t)) ?? 0, v = ((_s2 = a.shearModuli) == null ? void 0 : _s2.get(t)) ?? 0, F = ((_t = a.torsionalConstants) == null ? void 0 : _t.get(t)) ?? 0;
          S = Fa(O, W, x, w, v, F, A);
        }
        ...
        i = `<div class="matrix-label">k_local (${I.length}\xD7${I.length}) ${m}</div>${Go(I, P)}`, d = `<div class="matrix-label">T \u2014 Transformaci\xF3n (${L.length}\xD7${L.length}) ${u}</div>${Go(L, P)}`, b = `<div class="matrix-label">K_global = T^T \xB7 k \xB7 T ${E}</div>${Go(z, P)}`;
      } catch (m) {
      ...
      if (r == null ? void 0 : r.deformations) {
        ...
        l.map((u, E) => {
          ...
          const A = ((_a3 = r.deformations) == null ? void 0 : _a3.get(u)) || [
            ...
            0,
            0
          ], O = m.map((W, x) => `<span class="prop-key">${W}</span>: <span class="${Math.abs(A[x]) > 1e-10 ? "result-val" : ""}">${Me(A[x])}</span>`).join(" &nbsp;");
          return `<div style="margin-bottom:2px"><strong>Nodo ${u}:</strong> ${O}</div>`;
        }).join("");
      }
      if (f && c && (r == null ? void 0 : r.deformations) && I && L) {
        const m = (_u = f.normals) == null ? void 0 : _u.get(t), u = (_v = f.shearsY) == null ? void 0 : _v.get(t), E = (_w = f.shearsZ) == null ? void 0 : _w.get(t), A = (_x = f.torsions) == null ? void 0 : _x.get(t), O = (_y = f.bendingsY) == null ? void 0 : _y.get(t), W = (_z = f.bendingsZ) == null ? void 0 : _z.get(t), x = [
          "ux",
          "uy",
          "uz",
          "\u03B8x",
        ...
        const m = (_B = f.normals) == null ? void 0 : _B.get(t), u = (_C = f.shearsY) == null ? void 0 : _C.get(t), E = (_D = f.shearsZ) == null ? void 0 : _D.get(t), A = (_E = f.torsions) == null ? void 0 : _E.get(t), O = (_F = f.bendingsY) == null ? void 0 : _F.get(t), W = (_G = f.bendingsZ) == null ? void 0 : _G.get(t);
        `${m ? m.map((x) => Me(x)).join(", ") : "\u2014"}${u ? u.map((x) => Me(x)).join(", ") : "\u2014"}${E ? E.map((x) => Me(x)).join(", ") : "\u2014"}${A ? A.map((x) => Me(x)).join(", ") : "\u2014"}${O ? O.map((x) => Me(x)).join(", ") : "\u2014"}${W ? W.map((x) => Me(x)).join(", ") : "\u2014"}`;
      } else if (f && !c) {
        const m = (_H = f.bendingXX) == null ? void 0 : _H.get(t), u = (_I = f.bendingYY) == null ? void 0 : _I.get(t), E = (_J = f.bendingXY) == null ? void 0 : _J.get(t), A = (_K = f.membraneXX) == null ? void 0 : _K.get(t), O = (_L = f.membraneYY) == null ? void 0 : _L.get(t), W = (_M = f.membraneXY) == null ? void 0 : _M.get(t);
        `${m ? m.map((x) => Me(x)).join(", ") : "\u2014"}${u ? u.map((x) => Me(x)).join(", ") : "\u2014"}${E ? E.map((x) => Me(x)).join(", ") : "\u2014"}${A ? A.map((x) => Me(x)).join(", ") : "\u2014"}${O ? O.map((x) => Me(x)).join(", ") : "\u2014"}${W ? W.map((x) => Me(x)).join(", ") : "\u2014"}`;
      }
      `${l[0]}`, 6 * l[0], 6 * l[0] + 5, `${l[1]}`, 6 * l[1], 6 * l[1] + 5, l.length === 3 && (`${l[2]}`, 6 * l[2], 6 * l[2] + 5), n.length * 6, n.length * 6, Kt = vl(t, n, o, a, r, f), Kt.id = "fem-inspect-panel", document.body.appendChild(Kt), (_N = Kt.querySelector("#er-close")) == null ? void 0 : _N.addEventListener("click", () => An()), (_O = Kt.querySelector("#rel-apply")) == null ? void 0 : _O.addEventListener("click", () => {
        ...
        }), a.momentReleases || (a.momentReleases = /* @__PURE__ */ new Map()), a.partialFixitySprings || (a.partialFixitySprings = /* @__PURE__ */ new Map()), E.some((W) => W) ? a.momentReleases.set(t, E) : a.momentReleases.delete(t), A.some((W) => W > 0) ? a.partialFixitySprings.set(t, A) : a.partialFixitySprings.delete(t), console.log(`Releases elem ${t}:`, E.map((W, x) => W ? relIds[x] : "").filter(Boolean).join(" ") || "none"), console.log(`Springs elem ${t}:`, A);
      ...
      const k = c ? (() => {
        var _a3, _b2, _c2, _d2, _e3, _f2;
        const m = Pn(xn(s[1], s[0])), u = ((_a3 = a.elasticities) == null ? void 0 : _a3.get(t)) ?? 0, E = ((_b2 = a.areas) == null ? void 0 : _b2.get(t)) ?? 0, A = ((_c2 = a.momentsOfInertiaZ) == null ? void 0 : _c2.get(t)) ?? 0, O = ((_d2 = a.momentsOfInertiaY) == null ? void 0 : _d2.get(t)) ?? 0, W = ((_e3 = a.shearModuli) == null ? void 0 : _e3.get(t)) ?? 0, x = ((_f2 = a.torsionalConstants) == null ? void 0 : _f2.get(t)) ?? 0;
        return qa(u, E, A, O, W, x, m);
      })() : void 0;
      Kt.querySelectorAll("[data-full]").forEach((m) => {
        m.addEventListener("click", (u) => {
          ...
          if (E === "kLocal" && I) {
            ...
            Jo(`Elemento ${t} \u2014 Rigidez Local k_local`, A, Yo(I, P), k);
          } else if (E === "T" && L) Jo(`Elemento ${t} \u2014 Transformaci\xF3n T`, $, Yo(L, P));
          else if (E === "kGlobal" && z) {
            const A = c ? Ns() : "<em>Shell 18\xD718</em>";
            Jo(`Elemento ${t} \u2014 Rigidez Global K = T^T \xB7 k \xB7 T`, A, Yo(z, P), k);
          }
        });
      });
    }
    ...
    function Hs() {
      ...
      for (let h = 0; h <= 12; h++) {
        ...
        for (let z = 0; z < 6; z++) {
          const P = L + 2 * Math.PI * z / 6, k = 5 * Math.cos(P), m = 5 * Math.sin(P);
          c.push([
            k,
            m,
            I
    ...
    function js() {
      ...
      for (let p = 0; p <= 20; p++) {
        ...
        for (let P = 0; P < 3; P++) {
          const k = P * 2 * Math.PI / 3 - Math.PI / 2, m = L * Math.cos(k), u = L * Math.sin(k), E = s.length;
          s.push([
            m,
            u,
            I
    ...
    function Ws() {
      ...
      for (let b = 0; b < 3; b++) {
        ...
        for (let L = 0; L <= 4; L++) {
          const z = L / 4, P = -p / 2 + p * z, k = y * (1 - z * z * 0.3);
          for (let m = 0; m <= 12; m++) {
            const u = m / 12, E = S + k * u, A = $ * Math.sin(Math.PI * u) * (1 - z * z * 0.5), O = P;
            t.push([
              E,
              O,
              A
        ...
        for (let L = 0; L < 4; L++) for (let z = 0; z < 12; z++) {
          const P = h + L * I + z, k = h + L * I + z + 1, m = h + (L + 1) * I + z + 1, u = h + (L + 1) * I + z;
          n.push([
            P,
            k,
            m,
      ...
      for (let y = 0; y <= 15; y++) {
        ...
        if (p > 0.9) {
          ...
          for (let z = 0; z < 12; z++) {
            ...
            l.push([
              Math.max(L, 1) * Math.cos(P),
              Math.max(L, 1) * Math.sin(P),
              h
            ]);
          }
    ...
    function Ys() {
      ...
      try {
        ...
        if (P && e.deformOutputs) {
          ...
          const m = P.deformations.get(I), u = m ? m[2] : 0, E = l * o * o * o / 12, A = f * n * n * n / (3 * a * E);
    ...
    function ja(t) {
      ...
      const i = n / o, d = l * o;
      let b = 0, S = 0, $ = -i, y = i, p = 0, h = 0, I = 0, L = 0, z = 0, P = 0;
      const k = [];
      for (const V of f) {
        let ae = $, Q = y, K = p, pe = h, ce = I, Ie = L, Pe = z, _ = P, ue;
        const oe = V - b;
        ...
        ue = l * me + (1 - l) * me / se, ue = ue * (ce - Pe) + Pe, k.push(ue), b = V, S = ue, $ = ae, y = Q, p = K, h = pe, I = ce, L = Ie, z = Pe, P = _;
      }
      const m = t.querySelector("#nl-canvas"), u = m.getContext("2d"), E = m.width, A = m.height;
      u.clearRect(0, 0, E, A);
      const O = Math.max(...f.map(Math.abs)), W = Math.max(...k.map(Math.abs)), x = (E - 40) / (2 * O), w = (A - 40) / (2 * W), v = E / 2, F = A / 2;
    ...
    setTimeout(() => {
      ...
      function a(f, i, d, b, S) {
        ...
        for (let P = 0; P < L.length; P++) {
          const k = L[P], m = k.map((u) => c(I[u])).filter(Boolean);
          if (m.length !== 0) if (S) m.every((E) => E.x >= $ && E.x <= y && E.y >= p && E.y <= h) && z.push(P);
            ...
            if (k.length === 2) {
              const E = m[0], A = m[1];
              r(E.x, E.y, A.x, A.y, $, p, y, h) && z.push(P);
            }
          }
        }
        return z;
      ...
      function r(f, i, d, b, S, $, y, p) {
        ...
        const I = (L, z, P, k, m, u, E, A) => {
          const O = (P - L) * (A - u) - (k - z) * (E - m);
          if (Math.abs(O) < 1e-10) return false;
          const W = ((m - L) * (A - u) - (u - z) * (E - m)) / O, x = ((m - L) * (k - z) - (u - z) * (P - L)) / O;
