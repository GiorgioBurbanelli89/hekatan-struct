...
`);
  ...
  function Ca(e) {
    ...
    Y.push("");
    const Be = (de) => {
      const ze = g[de], Pe = `${me(ze[0])},${me(ze[1])}`;
      return {
        pt: Fe.get(Pe) || "1",
        story: le.get(me(ze[2])) || "Base"
      };
    };
    ...
    return N.forEach((de, ze) => {
      ...
      if (Pe === "BEAM") {
        ...
        const Je = g[de[0]][2] <= g[de[1]][2] ? de[0] : de[1], Ve = g[de[0]][2] <= g[de[1]][2] ? de[1] : de[0], yt = Be(Je), zt = Be(Ve), ht = me(g[Je][2]), wt = me(g[Ve][2]), At = V.indexOf(ht), Ht = V.indexOf(wt), Bt = Math.max(1, Ht >= 0 && At >= 0 ? Ht - At : 1);
...
`);
  ...
  function Pa(e, g) {
    const N = e[g[0]], S = e[g[1]], W = Math.abs(S[2] - N[2]), R = Math.sqrt((S[0] - N[0]) ** 2 + (S[1] - N[1]) ** 2), G = W > R * 0.5;
