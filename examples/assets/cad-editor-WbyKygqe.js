...
import { v as y, P as K } from "./theme-CzzIlc4y.js";
import { V as b, O as Q, t as ee, C as oe, u as ne, v as se, W as ie, w as te, x as ae, y as re, G as F, A as N, z as C, l as le, B as z, a as P, L as D, S as de, j as ce, e as pe, F as me, h as fe, P as he } from "./Text-CBH-tcJP.js";
import { t as M } from "./i18n-DrCrJaxE.js";
const ue = { x: { label: "X", axes: [] }, y: { label: "Y", axes: [] }, stories: [] };
function ge() {
  return { grid: y.state(ue), nodes: y.state([]), frames: y.state([]), areas: y.state([]), viewMode: y.state("3d"), activeStory: y.state(""), activeAxis: y.state(""), sectionPosition: y.state(0), showGrid: y.state(true), showLabels: y.state(true), showNodes: y.state(true), showFrames: y.state(true), showAreas: y.state(true) };
}
let J = 0, U = 0;
function ve(e, l, a, o) {
...
function ye(e, l) {
  return we(e.grid.val, l).map(([o, n, d]) => w(e, o, n, d));
}
function $e(e, l) {
...
function Se(e, l) {
  ...
  for (const c of a.x.axes) for (const t of a.y.axes) {
    const s = V(e, c.position, t.position, n.elevation), i = V(e, c.position, t.position, d.elevation);
    f.push(x(e, s, i));
  }
  return f;
}
...
function Me(e) {
  e.grid.val = { x: { label: "X", axes: [] }, y: { label: "Y", axes: [] }, stories: [] }, e.nodes.val = [], e.frames.val = [], e.areas.val = [], J = 0, U = 0;
}
function Ce() {
  ...
  e.id = "cad-viewport";
  const l = new ee();
  l.background = new oe(855309);
  const a = new ne(45, 1, 0.1, 2e6);
  a.position.set(15, -20, 12);
  const o = new se(-10, 10, 10, -10, 0.1, 2e6);
  o.position.set(15, -20, 12);
  const n = new ie({ antialias: true });
  n.setPixelRatio(window.devicePixelRatio), n.setClearColor(855309, 1), n.localClippingEnabled = true, e.appendChild(n.domElement);
  const d = new te(a, n.domElement);
  d.target.set(5, 5, 0), d.minDistance = 0.5, d.maxDistance = 500, d.zoomSpeed = 5, d.enableDamping = false, d.update();
  const f = new ae(16777215, 0.6), c = new re(16777215, 0.4);
  c.position.set(30, -20, 40), l.add(f, c);
  const t = new F(), s = 1, i = new N(new b(1, 0, 0), new b(0, 0, 0), s, 6710886, 0.2, 0.2), p = new N(new b(0, 1, 0), new b(0, 0, 0), s, 6710886, 0.2, 0.2), r = new N(new b(0, 0, 1), new b(0, 0, 0), s, 6710886, 0.2, 0.2), m = new C("X", "red", "transparent"), v = new C("Y", "green", "transparent"), u = new C("Z", "blue", "transparent");
  ...
  const h = new le(100, 100, 3355443, 2236962);
  ...
  new ResizeObserver(($) => {
    for (const A of $) {
      ...
      const Z = S / X, H = o.top;
      o.left = -H * Z, o.right = H * Z, o.updateProjectionMatrix(), n.setSize(S, X), G();
    }
  }).observe(e), d.addEventListener("change", G), requestAnimationFrame(() => {
    const $ = e.clientWidth, A = e.clientHeight;
    if ($ > 0 && A > 0) {
      a.aspect = $ / A, a.updateProjectionMatrix();
      const S = $ / A;
      o.left = -10 * S, o.right = 10 * S, o.updateProjectionMatrix(), n.setSize($, A), G();
    }
  });
  const k = { scene: l, perspCamera: a, orthoCamera: o, controls: d, renderer: n, render: G };
  return k.setActiveCamera = ($) => {
    g = $, d.object = $, d.update(), G();
...
function Pe(e, l) {
  ...
  o.name = "cad-labels", l.scene.add(a), l.scene.add(o), y.derive(() => {
    ...
    if (W(a), W(o), a.visible = d, o.visible = f, !d && !f) {
      l.render();
      return;
    }
    const c = L(n), t = 1.5, s = 1;
    for (const i of n.stories) {
      const p = i.elevation;
      for (const r of n.x.axes) {
        const m = r.position, v = new z().setFromPoints([new b(m, c.minY - t, p), new b(m, c.maxY + t, p)]), u = new P({ color: 4473924, transparent: true, opacity: 0.6 });
        a.add(new D(v, u));
      }
      for (const r of n.y.axes) {
        const m = r.position, v = new z().setFromPoints([new b(c.minX - t, m, p), new b(c.maxX + t, m, p)]), u = new P({ color: 4473924, transparent: true, opacity: 0.6 });
        a.add(new D(v, u));
      }
    }
    if (n.stories.length > 1) {
      ...
      for (const r of n.x.axes) for (const m of n.y.axes) {
        const v = new z().setFromPoints([new b(r.position, m.position, i), new b(r.position, m.position, p)]), u = new P({ color: 3355443, transparent: true, opacity: 0.3 });
        a.add(new D(v, u));
      }
    }
    ...
    if (t && (l.scene.remove(t), E(t)), s && (l.scene.remove(s), E(s)), f && n.length > 0) {
      const i = new F();
      i.name = "cad-nodes";
      const p = new de(0.15, 8, 8), r = new ce({ color: 16746496 });
      for (const m of n) {
        const v = new pe(p, r);
    ...
    if (c && d.length > 0) {
      ...
      if (p.length > 0) {
        const r = new z();
        r.setAttribute("position", new me(p, 3));
        const m = new P({ color: 52479, linewidth: 2 });
        i.add(new fe(r, m));
      }
      l.scene.add(i);
    }
...
function E(e) {
  ...
  e.dispose && e.dispose(), (_a = e.children) == null ? void 0 : _a.forEach(E);
}
function Ye(e, l) {
  const a = document.createElement("div");
  a.id = "cad-panel";
  const o = { viewMode: "3d", selector: "", sectionPos: 0, showGrid: true, showLabels: true, showNodes: true, showFrames: true, showAreas: true }, n = new K({ title: "FEM Studio", container: a, expanded: true });
  ...
  f();
  const c = n.addFolder({ title: M("Grid") });
  c.addBinding(o, "showGrid", { label: M("Mostrar grid") }), c.addBinding(o, "showLabels", { label: M("Mostrar labels") });
  const t = n.addFolder({ title: M("Modelo") });
  t.addBinding(o, "showNodes", { label: M("Nodes") }), t.addBinding(o, "showFrames", { label: "Frames" }), t.addBinding(o, "showAreas", { label: M("Placas") }), n.on("change", (p) => {
    var _a;
  ...
  function s() {
    var _a, _b, _c;
    const p = e.grid.val, r = L(p), m = new b(r.centerX, r.centerY, r.centerZ), v = r.maxDim;
  ...
  function i() {
    ...
    l.renderer.clippingPlanes = [new he(r, p)], l.render();
  }
...
function Ee(e, l) {
  const a = { addGrid(o, n, d) {
    return ve(e, o, n, d), console.log(`Grid ${o}: ${n.length} axes at [${n.join(", ")}]`), e.grid.val;
  }, addStory(o, n) {
    return xe(e, o, n), console.log(`Story "${o}" at z=${n}`), e.grid.val.stories;
  }, getGrid() {
    const o = e.grid.val;
    ...
    $e(e, o), console.log(`Node ${o} removed`);
  }, listNodes() {
    const o = e.nodes.val;
    ...
    Ae(e, o), console.log(`Frame ${o} removed`);
  }, addColumnsAtLevel(o) {
    const n = Se(e, o);
    ...
    Me(e), console.log("Model cleared");
  }, help() {
    console.log(`
...
const j = ge(), { element: B, ctx: _ } = Ce(), q = Ye(j, _);
Pe(j, _);
