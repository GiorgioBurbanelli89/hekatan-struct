...
let __tla = Promise.all([
  ...
  function vo(e, x) {
    const _ = Rn.find((Y) => Y.id === e), T = Fo.find((Y) => Y.id === x), K = _.toKN, B = T.toM, ne = (Y, xe, L) => L / (Math.pow(K, Y) * Math.pow(B, xe));
    let V, G;
    switch (e) {
      case "kN":
        V = 10, G = [
  ...
  <td style="padding:2px 5px; text-align:right; color:#0ff">${(ne[5] * 100).toFixed(1)}%</td></tr>`;
      ...
      (_a2 = e.querySelector("#modal-minimize")) == null ? void 0 : _a2.addEventListener("click", () => {
        ...
        T.frequencies.forEach((ie, Ee) => {
          var _a3;
          const ge = ie > 0 ? 1 / ie : 0, ce = ie * 2 * Math.PI, fe = ((_a3 = T.massParticipation) == null ? void 0 : _a3[Ee]) || [
            0,
            0,
            0,
            0,
          ...
          G.push(`${String(Ee + 1).padStart(4)}  ${ie.toFixed(4).padStart(9)}  ${ge.toFixed(4).padStart(9)}  ${ce.toFixed(2).padStart(9)}  ${be}  ${(xe[0] * 100).toFixed(1).padStart(5)}%  ${(xe[1] * 100).toFixed(1).padStart(5)}%  ${(xe[5] * 100).toFixed(1).padStart(5)}%`);
...
`));
  ...
  function ka(e) {
    ...
    L += '<button class="rpt-close" id="rpt-close">\u2715 Close</button>', L += "<h1>Finite Element Analysis \u2014 Step-by-Step Report</h1>", L += '<div class="rpt-subtitle">Complete FEM derivation from element formulation to final results</div>', L += '<hr class="rpt-sep"/>', L += "<h2>1. Input Data</h2>", L += '<table class="rpt-info"><tbody>', L += `<tr><td>Number of nodes</td><td class="val">${x.length}</td></tr>`, L += `<tr><td>Number of elements</td><td class="val">${V} (${G} frames, ${Y} shells)</td></tr>`, L += '<tr><td>DOFs per node</td><td class="val">6 (u<sub>x</sub>, u<sub>y</sub>, u<sub>z</sub>, \u03B8<sub>x</sub>, \u03B8<sub>y</sub>, \u03B8<sub>z</sub>)</td></tr>', L += `<tr><td>Total DOFs</td><td class="val">${ne}</td></tr>`, L += "</tbody></table>", L += "<h3>1.1 Node Coordinates</h3>", L += '<table class="rpt-data"><thead><tr><th>Node</th><th>x</th><th>y</th><th>z</th></tr></thead><tbody>', x.forEach((ce, fe) => {
      L += `<tr><td>${fe}</td><td>${Ue(ce[0])}</td><td>${Ue(ce[1])}</td><td>${Ue(ce[2])}</td></tr>`;
      ...
      var _a3, _b2, _c2, _d2;
      const be = ce.length === 2, Pe = ce.map((Ke) => x[Ke]), Be = be ? yo(oo(Pe[1], Pe[0])) : 0, Oe = ((_a3 = T.elasticities) == null ? void 0 : _a3.get(fe)) ?? 0, Ve = ((_b2 = T.areas) == null ? void 0 : _b2.get(fe)) ?? 0, st = ((_c2 = T.momentsOfInertiaZ) == null ? void 0 : _c2.get(fe)) ?? 0, Ye = ((_d2 = T.momentsOfInertiaY) == null ? void 0 : _d2.get(fe)) ?? 0;
      L += `<tr><td>${fe}</td><td>${be ? "Frame" : "Shell"}</td><td>${ce.join(" \u2192 ")}</td>`, L += `<td>${Ue(Be)}</td><td>${Ue(Oe)}</td><td>${Ue(Ve)}</td><td>${Ue(st)}</td><td>${Ue(Ye)}</td></tr>`;
    ...
    for (let ce = 0; ce < V; ce++) {
      const fe = _[ce], be = fe.map((it) => x[it]);
      ...
      L += '<div class="rpt-elem-block">', L += `<h3 class="rpt-elem-title" data-toggle="elem${ce}">\u25B6 Element ${ce} \u2014 Nodes ${fe[0]} \u2192 ${fe[1]}, L = ${Ue(Be)}</h3>`, L += `<div id="rpt-elem${ce}" class="rpt-elem-body" style="display:none">`, L += "<h4>Properties (numerical substitution)</h4>", L += '<div class="rpt-eq-small">', L += `E = ${Ue(Oe)} &nbsp;&nbsp; A = ${Ue(Ve)} &nbsp;&nbsp; I<sub>z</sub> = ${Ue(st)} &nbsp;&nbsp; I<sub>y</sub> = ${Ue(Ye)} &nbsp;&nbsp; G = ${Ue(Ke)} &nbsp;&nbsp; J = ${Ue(bt)}<br/>`, L += `EA/L = ${Ue(Oe)}\xB7${Ue(Ve)}/${Ue(Be)} = <b>${Ue(Oe * Ve / Be)}</b><br/>`, L += `12EI<sub>z</sub>/L\xB3 = 12\xB7${Ue(Oe)}\xB7${Ue(st)}/${Ue(Be)}\xB3 = <b>${Ue(12 * Oe * st / Be ** 3)}</b><br/>`, L += `12EI<sub>y</sub>/L\xB3 = 12\xB7${Ue(Oe)}\xB7${Ue(Ye)}/${Ue(Be)}\xB3 = <b>${Ue(12 * Oe * Ye / Be ** 3)}</b><br/>`, L += `GJ/L = ${Ue(Ke)}\xB7${Ue(bt)}/${Ue(Be)} = <b>${Ue(Ke * bt / Be)}</b>`, L += "</div>", L += "<h4>Direction cosines</h4>", L += `<div class="rpt-eq-small">l = ${en(et)}, m = ${en(Ze)}, n = ${en(De)}, D = ${en(Math.sqrt(et ** 2 + Ze ** 2))}</div>`, L += "<h4>K<sub>local</sub> (12\xD712)</h4>", L += On(vt, 12), L += "<h4>T \u2014 Transformation (12\xD712)</h4>", L += On(me, 12), L += "<h4>K<sub>global</sub> = T\u1D40 \xB7 K<sub>local</sub> \xB7 T</h4>", L += On(Ne, 12), L += "<h4>Assembly</h4>", L += `<div class="rpt-eq-small">Global DOFs: node ${fe[0]} \u2192 [${fe[0] * 6}..${fe[0] * 6 + 5}], node ${fe[1]} \u2192 [${fe[1] * 6}..${fe[1] * 6 + 5}]</div>`, L += "</div></div>";
    }
    ...
    if (L += "<h3>5.1 Supports (fixed DOFs)</h3>", K.supports && K.supports.size > 0) {
      ...
      L += "</tr></thead><tbody>", K.supports.forEach((ce, fe) => {
        L += `<tr><td>${fe}</td>`, ce.forEach((be, Pe) => {
          be && Ee.push(fe * 6 + Pe), L += `<td class="${be ? "fixed" : ""}">${be ? "Fixed" : "Free"}</td>`;
        }), L += "</tr>";
      }), L += "</tbody></table>";
    }
    if (L += `<div class="rpt-eq-small">Fixed DOFs: [${Ee.join(", ")}] \u2192 ${Ee.length} constraints<br/>`, L += `Free DOFs: ${ne} \u2212 ${Ee.length} = <b>${ne - Ee.length}</b></div>`, L += "<h3>5.2 Applied Loads</h3>", K.loads && K.loads.size > 0) {
      L += '<table class="rpt-data"><thead><tr><th>Node</th>';
      const ce = [
        "F<sub>x</sub>",
        "F<sub>y</sub>",
    ...
    if (L += "<h2>7. Internal Forces</h2>", L += "<p>For each element, transform global displacements to local and multiply by K<sub>local</sub>:</p>", L += '<div class="rpt-eq">u<sub>local</sub> = T \xB7 u<sub>global</sub></div>', L += '<div class="rpt-eq rpt-eq-highlight">f<sub>local</sub> = K<sub>local</sub> \xB7 u<sub>local</sub></div>', B == null ? void 0 : B.deformations) {
      ...
      for (let fe = 0; fe < V; fe++) {
        ...
        try {
          const Be = on(Pe, T, fe), Oe = nn(Pe), Ve = [];
          for (const Ke of be) {
  ...
  function On(e, x) {
    ...
    for (let K = 0; K < _; K++) {
      ...
      for (let B = 0; B < _; B++) {
        ...
        T += `<td class="${V ? "z" : ""} ${K === B && !V ? "diag" : ""}">${V ? "0" : za(ne)}</td>`;
      }
      T += "</tr>";
    }
  ...
  function Ta() {
    ...
    for (const G of ne) {
      ...
      for (let Ee = 0; Ee <= 80; Ee++) {
        const ge = Ee / 80, ce = 30 + ge * 540, fe = 180 / 2 - G.fn(ge) * 60;
        Y += (Ee === 0 ? "M" : "L") + `${ce.toFixed(1)},${fe.toFixed(1)}`;
      }
  ...
  function Ca(e, x, _, T, K, B) {
    var _a2, _b, _c, _d, _e2, _f;
    const ne = _[e], V = ne.map((me) => x[me]), G = ne.length === 2, Y = G ? yo(oo(V[1], V[0])) : 0, xe = ((_a2 = T.elasticities) == null ? void 0 : _a2.get(e)) ?? 0, L = ((_b = T.areas) == null ? void 0 : _b.get(e)) ?? 0, ie = ((_c = T.momentsOfInertiaZ) == null ? void 0 : _c.get(e)) ?? 0, Ee = ((_d = T.momentsOfInertiaY) == null ? void 0 : _d.get(e)) ?? 0, ge = ((_e2 = T.shearModuli) == null ? void 0 : _e2.get(e)) ?? 0, ce = ((_f = T.torsionalConstants) == null ? void 0 : _f.get(e)) ?? 0;
    let fe = null, be = null, Pe = null;
    try {
      fe = on(V, T, e), be = nn(V), Pe = Wt(_n(be), Wt(fe, be));
    } catch {
    }
    const Be = G ? oo(V[1], V[0]) : [
      0,
      0,
      0
    ], Oe = Y > 0 ? Be[0] / Y : 0, Ve = Y > 0 ? Be[1] / Y : 0, st = Y > 0 ? Be[2] / Y : 0, Ye = Math.sqrt(Oe ** 2 + Ve ** 2), Ke = [];
  ...
  function Pa(e, x, _, T, K, B) {
    var _a2, _b;
    const ne = Ca(e, x, _, T, K, B), V = document.createElement("div");
  ...
  `, V.querySelectorAll(".er-tab").forEach((G) => {
      ...
      G && (G.innerHTML = As(ne)), setTimeout(() => {
        const Y = V.querySelector("#er-sf-canvas-math");
        Y && qn(Y);
  ...
  function jo(e) {
    ...
    for (let K = 0; K < _; K++) {
      ...
      for (let B = 0; B < _; B++) {
        const ne = ((_a2 = e[K]) == null ? void 0 : _a2[B]) ?? 0, V = Math.abs(ne) < 1e-10;
        T += `<td class="${V ? "z" : ""} ${K === B && !V ? "diag" : ""}">${V ? "0" : Na(ne)}</td>`;
      }
      T += "</tr>";
    }
    ...
    const V = (G, Y, xe) => {
      ...
      for (const ie of xe) {
        ...
        for (let Ee = 0; Ee <= 100; Ee++) {
          const ge = Ee / 100, ce = K + ge * B, fe = G + ne / 2 - ie.fn(ge) * (ne / 2 * 0.85);
          Ee === 0 ? x.moveTo(ce, fe) : x.lineTo(ce, fe);
        }
        x.stroke();
      }
      let L = K + B - 90;
  ...
  `, document.body.appendChild(Yt);
    ...
    Ee = Math.max(10, Math.min(Ee, K - ne - 10)), ge = Math.max(10, Math.min(ge, B - V - 10)), Lt = document.createElement("div"), Lt.style.cssText = `
    position: fixed;
  ...
  `, document.body.appendChild(Lt), (_a2 = Lt.querySelector("#tour-next")) == null ? void 0 : _a2.addEventListener("click", () => {
    ...
    const fe = (be) => {
      ...
      (be.key === "ArrowRight" || be.key === "Enter") && (Oo(e + 1), document.removeEventListener("keydown", fe)), be.key === "ArrowLeft" && (Oo(Math.max(0, e - 1)), document.removeEventListener("keydown", fe)), be.key === "Escape" && (Hn(), document.removeEventListener("keydown", fe));
    };
...
`);
  ...
  function Wa(e) {
    ...
    for (let me = ie.length - 1; me >= 1; me--) Y.push(`  STORY "${Ee[me]}"  HEIGHT ${xe(ie[me] - ie[me - 1])} MASTERSTORY "Yes"  `);
    ...
    _.forEach((me, Ne) => {
      ...
      De && (ht += `  D ${De}`), it && (ht += `  B ${it}`), at && !De && (ht += `  D ${at}`), yt && (ht += `  TF ${yt}`), It && (ht += `  TW ${It}`), Y.push(ht);
    }), Y.push("");
    ...
    _.forEach((me, Ne) => {
      ...
      if (He === "BEAM") {
        ...
        Y.push(`  LINE  "E${Ne + 1}"  BEAM  "${Ze.pt}"  "${De.pt}"  0`), bt.push(`  LINEASSIGN  "E${Ne + 1}"  "${Ze.story}"  SECTION "${et}"  MINNUMSTA 3 AUTOMESH "YES"  MESHATINTERSECTIONS "YES"  `);
      } else {
        ...
        Y.push(`  LINE  "E${Ne + 1}"  ${He}  "${it.pt}"  "${it.pt}"  ${At}`);
      ...
      if (me[0] && He.push("UX"), me[1] && He.push("UY"), me[2] && He.push("UZ"), me[3] && He.push("RX"), me[4] && He.push("RY"), me[5] && He.push("RZ"), He.length > 0) {
        const et = Ke(Ne);
        Y.push(`  POINTASSIGN  "${et.pt}"  "${et.story}"  RESTRAINT "${He.join(" ")}"  `);
      }
    }), Y.push(""), Y.push("$ LINE ASSIGNS"), bt.forEach((me) => Y.push(me)), Y.push("");
    const vt = [];
    if (_.forEach((me, Ne) => {
      if (me.length === 4) {
    ...
    if (vt.length > 0) {
      ...
      vt.forEach((Ne, He) => {
        ...
        if (Ze) {
          ...
          Y.push(`  AREA "${De}"  ${it}  4  "${at[yt].pt}"  "${at[It].pt}"  "${at[It].pt}"  "${at[yt].pt}"  1  1  0  0  `);
        ...
        } else Y.push(`  AREA "${De}"  ${it}  4  "${at[0].pt}"  "${at[1].pt}"  "${at[2].pt}"  "${at[3].pt}"  0  0  0  0  `), me.push(`  AREAASSIGN  "${De}"  "${at[0].story}"  SECTION "Losa"  DIAPH  "D1"  OBJMESHTYPE "DEFAULT"  ADDRESTRAINT "Yes"  CARDINALPOINT "TOP"  TRANSFORMSTIFFNESSFOROFFSETS "No"  `);
    ...
    return Y.push("$ LOAD PATTERNS"), Y.push('  LOADPATTERN "Dead"  TYPE  "Dead"  SELFWEIGHT  1'), Y.push('  LOADPATTERN "Live"  TYPE  "Live"  SELFWEIGHT  0'), Y.push(""), T.loads && T.loads.size > 0 && (Y.push("$ POINT OBJECT LOADS"), T.loads.forEach((me, Ne) => {
      ...
      Math.abs(He) > 1e-10 && Y.push(`  POINTLOAD  "${De.pt}"  "${De.story}"  "Dead"  TYPE "FORCE"  FX ${He}`), Math.abs(et) > 1e-10 && Y.push(`  POINTLOAD  "${De.pt}"  "${De.story}"  "Dead"  TYPE "FORCE"  FY ${et}`), Math.abs(Ze) > 1e-10 && Y.push(`  POINTLOAD  "${De.pt}"  "${De.story}"  "Dead"  TYPE "FORCE"  FZ ${Ze}`);
...
`);
  ...
  function Ga(e) {
    ...
    return B.push("# OpenSeesPy model exported from Awatif FEM Studio"), B.push(`# ${x.length} nodes, ${_.length} elements`), B.push(""), B.push("import openseespy.opensees as ops"), B.push(""), B.push("ops.wipe()"), B.push("ops.model('basic', '-ndm', 3, '-ndf', 6)"), B.push(""), B.push("# --- Nodes ---"), x.forEach((ne, V) => {
      ...
      const G = x[ne[0]], Y = x[ne[1]], L = Math.abs(Y[2] - G[2]) > Math.max(Math.abs(Y[0] - G[0]), Math.abs(Y[1] - G[1])) ? 2 : 1, ie = ((_a3 = K.areas) == null ? void 0 : _a3.get(V)) ?? 1, Ee = ((_b2 = K.elasticities) == null ? void 0 : _b2.get(V)) ?? 2e5, ge = ((_c = K.shearModuli) == null ? void 0 : _c.get(V)) ?? 8e4, ce = ((_d = K.torsionalConstants) == null ? void 0 : _d.get(V)) ?? 1, fe = ((_e2 = K.momentsOfInertiaY) == null ? void 0 : _e2.get(V)) ?? 1, be = ((_f = K.momentsOfInertiaZ) == null ? void 0 : _f.get(V)) ?? 1;
      B.push(`ops.element('elasticBeamColumn', ${V + 1}, ${ne[0] + 1}, ${ne[1] + 1}, ${ie}, ${Ee}, ${ge}, ${ce}, ${fe}, ${be}, ${L})`);
...
`);
  ...
  function Ja(e) {
    ...
    return B.push("# OpenSees Tcl model exported from Awatif FEM Studio"), B.push(`# ${x.length} nodes, ${_.length} elements`), B.push(""), B.push("wipe"), B.push("model basic -ndm 3 -ndf 6"), B.push(""), B.push("# --- Nodes ---"), x.forEach((ne, V) => {
      ...
      if (ne.length !== 2) return;
      const G = x[ne[0]], Y = x[ne[1]], L = Math.abs(Y[2] - G[2]) > Math.max(Math.abs(Y[0] - G[0]), Math.abs(Y[1] - G[1])) ? 2 : 1, ie = ((_a3 = K.areas) == null ? void 0 : _a3.get(V)) ?? 1, Ee = ((_b2 = K.elasticities) == null ? void 0 : _b2.get(V)) ?? 2e5, ge = ((_c = K.shearModuli) == null ? void 0 : _c.get(V)) ?? 8e4, ce = ((_d = K.torsionalConstants) == null ? void 0 : _d.get(V)) ?? 1, fe = ((_e2 = K.momentsOfInertiaY) == null ? void 0 : _e2.get(V)) ?? 1, be = ((_f = K.momentsOfInertiaZ) == null ? void 0 : _f.get(V)) ?? 1;
      B.push(`element elasticBeamColumn ${V + 1} ${ne[0] + 1} ${ne[1] + 1} ${ie} ${Ee} ${ge} ${ce} ${fe} ${be} ${L}`);
...
`);
      ...
      if (be) {
        ...
        if (Oe !== void 0 && Ve !== void 0) {
          ...
          _.push([
            Oe,
            Ve
          ]), ie.set(Be, st), V.set(st, parseFloat(be[4])), B.set(st, parseFloat(be[5])), ne.set(st, parseFloat(be[6])), xe.set(st, parseFloat(be[7])), G.set(st, parseFloat(be[8])), Y.set(st, parseFloat(be[9]));
        }
        continue;
      }
  ...
  ol = function(e) {
    ...
    let L = "", ie = {}, Ee = null, ge = "", ce = [], fe = [], be = /* @__PURE__ */ new Set(), Pe = /* @__PURE__ */ new Set(), Be = /* @__PURE__ */ new Set(), Oe = /* @__PURE__ */ new Map(), Ve = /* @__PURE__ */ new Map(), st = null, Ye = [], Ke = 0.2, bt = 2, vt = 2, me = false, Ne = 2, He = "x", et = /* @__PURE__ */ new Set(), Ze = true, De = 0.15, it = 2, at = 2, yt = /* @__PURE__ */ new Set(), It = false, lt = "perimeter";
    const At = () => ({
      b: 0.3,
      h: 0.4,
      profileIdx: 0,
    ...
    const We = {
      ...
      build(t) {
        ...
        const u = 15100 * Math.sqrt(l) * 10, $ = u / (2 * (1 + 0.2)), z = s * c, C = s * c ** 3 / 12, y = c * s ** 3 / 12, E = s * c * (s ** 2 + c ** 2) / 12, m = a * r, A = a * r ** 3 / 12, F = r * a ** 3 / 12, H = a * r * (a ** 2 + r ** 2) / 12, J = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), he = /* @__PURE__ */ new Map(), ze = /* @__PURE__ */ new Map();
        for (let q = 0; q < S.length; q++) J.set(q, u), Q.set(q, $), I.has(q) ? (Z.set(q, z), te.set(q, C), j.set(q, y), he.set(q, E), ze.set(q, {
          type: "rect",
          b: s,
          h: c,
          name: `COL${o}`
        })) : h.has(q) ? (Z.set(q, z), te.set(q, C), j.set(q, y), he.set(q, E), ze.set(q, {
          type: "rect",
          b: s,
          h: c,
          name: `BR${o}`
...
`;
      ...
      model3d(t) {
        ...
        for (let W = 0; W < g; W++) for (let re = 0; re < f; re++) for (let pe = 0; pe < v; pe++) k[`${pe},${W},${re}`] = I.length, I.push([
          b[pe],
          w[W],
          M[re]
        ]);
        const O = [], N = /* @__PURE__ */ new Set(), h = /* @__PURE__ */ new Set(), u = /* @__PURE__ */ new Map();
        for (let W = 0; W < S; W++) for (let re = 0; re < f; re++) for (let pe = 0; pe < v; pe++) {
          const U = O.length;
          O.push([
            k[`${pe},${W},${re}`],
            k[`${pe},${W + 1},${re}`]
        ...
        for (let W = 1; W < g; W++) for (let re = 0; re < f; re++) for (let pe = 0; pe < v - 1; pe++) {
          const U = O.length;
          O.push([
            k[`${pe},${W},${re}`],
            k[`${pe + 1},${W},${re}`]
        ...
        for (let W = 1; W < g; W++) for (let re = 0; re < v; re++) for (let pe = 0; pe < f - 1; pe++) {
          const U = O.length;
          O.push([
            k[`${re},${W},${pe}`],
            k[`${re},${W},${pe + 1}`]
        ...
        const z = 15100 * Math.sqrt(a) * 10, C = z / (2 * (1 + 0.2)), y = r * p, E = r * p ** 3 / 12, m = p * r ** 3 / 12, A = r * p * (r ** 2 + p ** 2) / 12, F = d * i, H = d * i ** 3 / 12, J = i * d ** 3 / 12, Q = d * i * (d ** 2 + i ** 2) / 12, Z = /* @__PURE__ */ new Map(), te = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), he = /* @__PURE__ */ new Map(), ze = /* @__PURE__ */ new Map(), ue = /* @__PURE__ */ new Map(), q = /* @__PURE__ */ new Map();
        for (let W = 0; W < O.length; W++) Z.set(W, z), te.set(W, C), N.has(W) ? (j.set(W, y), he.set(W, E), ze.set(W, m), ue.set(W, A), q.set(W, {
          type: "rect",
          b: r,
          h: p,
          name: `COL${s}`
        ...
        for (let W = 0; W < f; W++) for (let re = 0; re < v; re++) oe.set(k[`${re},0,${W}`], [
          true,
          true,
          true,
          true,
        ...
        e.nodes.val = I, e.elements.val = O, e.nodeInputs.val = {
          ...
          label: `${re + 1}`,
          coord: W
        })), Qe = w[w.length - 1], No(qe.map((W) => W.coord), Fe.map((W) => W.coord), Qe, qe.map((W) => W.label), Fe.map((W) => W.label));
        {
          const W = w.map((re, pe) => ({
            name: pe === 0 ? "Base" : `P${pe}`,
      ...
      building(t, o, n, l = 3, s = 0, c = 0, a = 0, r = 0, p = 1) {
        ...
        for (const u of Ye) {
          ...
          for (const C of z) {
            ...
            const Z = vt, te = bt, j = f[F], he = f[J], ze = f[H], ue = f[Q], q = [];
            for (let oe = 0; oe <= te; oe++) {
              const se = [], ee = oe / te;
              for (let W = 0; W <= Z; W++) {
                const re = W / Z, pe = (1 - ee) * ((1 - re) * j[0] + re * he[0]) + ee * ((1 - re) * ze[0] + re * ue[0]), U = (1 - ee) * ((1 - re) * j[1] + re * he[1]) + ee * ((1 - re) * ze[1] + re * ue[1]), Ie = (1 - ee) * ((1 - re) * j[2] + re * he[2]) + ee * ((1 - re) * ze[2] + re * ue[2]);
          ...
          for (let C = 0; C < z; C++) {
            if (u) for (let y = 0; y < i.length; y++) {
              ...
              for (let m = 0; m < d.length - 1; m++) {
                ...
                const A = g[`${m},${y},${C}`], F = g[`${m + 1},${y},${C + 1}`], H = g[`${m + 1},${y},${C}`], J = g[`${m},${y},${C + 1}`];
            ...
            if ($) for (let y = 0; y < d.length; y++) {
              ...
              for (let m = 0; m < i.length - 1; m++) {
                ...
                const A = g[`${y},${m},${C}`], F = g[`${y},${m + 1},${C + 1}`], H = g[`${y},${m + 1},${C}`], J = g[`${y},${m},${C + 1}`];
                A !== void 0 && F !== void 0 && (I.push([
                  A,
                  F
                ]), Oe.set(I.length - 1, C)), H !== void 0 && J !== void 0 && (I.push([
        ...
        switch (t) {
          ...
          case "mezzanine": {
            Xe("edificio"), ie.nPisos && (ie.nPisos.val = 1), ie.hPiso && (ie.hPiso.val = 4.5), ie.nVanosX && (ie.nVanosX.val = 3), ie.nVanosY && (ie.nVanosY.val = 2), ie.nSubViga && (ie.nSubViga.val = 3), ce = [
              ...
              5,
              5
            ], Le.colMat = 1, Le.vigaMat = 1, Le.steelColType = 0, Le.steelVigaType = 0, Ye = [], me = true, Ne = 2, He = ce[0] >= fe[0] ? "y" : "x", Ze = true, De = 0.08, it = 3, at = 3, Se();
            break;
          }
          case "galpon": {
            Xe("galpon"), Se();
...
Util:     cad.info()  cad.clear()  cad.help()
      ...
      if (I === 0) {
        ...
        if (I === 2) {
          ...
          for (let U = 0; U < h.length; U++) {
            const Ie = h[U][0], je = h[U][1];
            Math.abs(Ie - n) < Z && je >= l - Z && te.push(U);
          }
          te.sort((U, Ie) => h[U][1] - h[Ie][1]);
          const j = [
            te[0]
          ];
          for (let U = 1; U < te.length; U++) {
            const Ie = h[te[U]][1] - h[j[j.length - 1]][1];
            Math.abs(Ie) > s * 0.05 && j.push(te[U]);
          }
          ...
          for (let U = 0; U < ze; U++) {
            const Ie = u[U], je = (h[Ie[0]][0] + h[Ie[1]][0] + h[Ie[2]][0]) / 3, pt = (h[Ie[0]][1] + h[Ie[1]][1] + h[Ie[2]][1]) / 3, $t = je >= -k && je <= O && pt >= 0 && pt <= l, zo = je >= 0 && je <= n && pt >= l && pt <= l + t, co = $t || zo;
          ...
          for (let U = 0; U < te.length - 1; U++) {
            const Ie = te[U], je = te[U + 1], pt = he.get(Ie), $t = he.get(je);
            u.push([
              je,
              Ie,
              pt,
          ...
          for (let U = q; U < u.length; U++) se.set(U, w), ee.set(U, 0), re.set(U, 0), pe.set(U, v), W.set(U, 0);
          C = {
            elasticities: se,
          ...
          for (let U = 0; U < ze; U++) {
            if (!ue[U]) continue;
            const Ie = u[U], je = h[Ie[0]], pt = h[Ie[1]], $t = h[Ie[2]], zo = Math.abs((pt[0] - je[0]) * ($t[1] - je[1]) - ($t[0] - je[0]) * (pt[1] - je[1])) / 2, co = -p * zo / 3;
            for (const Kt of Ie) {
              const Qt = z.get(Kt) || [
                0,
                0,
          ...
          if (S > 0) {
            ...
            for (let Ie = 0; Ie < h.length; Ie++) {
              ...
              Math.abs(pt - A) < s * 0.1 && je > n - 1e-6 && U.push({
                idx: Ie,
                x: je
              });
            }
            U.sort((Ie, je) => Ie.x - je.x);
            for (let Ie = 0; Ie < U.length; Ie++) {
              let je = s;
              Ie > 0 && Ie < U.length - 1 ? je = (U[Ie + 1].x - U[Ie - 1].x) / 2 : Ie === 0 && U.length > 1 ? je = (U[1].x - U[0].x) / 2 : Ie === U.length - 1 && U.length > 1 && (je = (U[Ie].x - U[Ie - 1].x) / 2);
              const pt = -S * je, $t = z.get(U[Ie].idx) || [
                0,
                0,
                0,
                0,
    ...
    function ns() {
      const t = X("colB") || 0.3, o = X("colH") || 0.3, n = X("colT") || 8e-3, l = X("colLen") || 1.5, s = X("Lx") || 0.45, c = X("Ly") || 0.45, a = X("tPlaca") || 0.025, r = X("dBolt") || 0.022, p = X("sx") || 0.32, d = X("sy") || 0.32, i = Math.round(X("nSubColV") || 6), b = Math.round(X("nSubColH") || 4), M = Math.round(X("nSubPlaca") || 10), w = X("E") || 2e8, v = X("nu") || 0.3, f = w / (2 * (1 + v)), g = X("P") || -300, S = s / 2, I = c / 2, k = r / 2, O = t / 2, N = o / 2, h = [], u = [], $ = M, z = s / $, C = c / $, y = (ee, W) => W * ($ + 1) + ee;
      ...
      for (const [ee, W] of te) {
        const [re, pe] = Q[ee], [U, Ie] = Q[W], je = [];
        for (let pt = 0; pt <= H; pt++) {
          const $t = [], zo = pt / H * l;
          for (let co = 0; co <= J; co++) {
            const Kt = co / J, Qt = re + Kt * (U - re), An = pe + Kt * (Ie - pe);
            if (pt === 0) {
              const eo = j(Qt, An);
              if (eo >= 0) {
                $t.push(eo);
        ...
        for (let pt = 0; pt < H; pt++) for (let $t = 0; $t < J; $t++) u.push([
          je[pt][$t],
          je[pt][$t + 1],
          je[pt + 1][$t + 1],
          je[pt + 1][$t]
      ...
      for (let ee = 0; ee < ($ + 1) * ($ + 1); ee++) {
        ...
        for (const [pe, U] of E) {
          const Ie = Math.sqrt((W - pe) * (W - pe) + (re - U) * (re - U));
      ...
      for (let ee = Z; ee < Z + he; ee++) se.elasticities.set(ee, w), se.poissonsRatios.set(ee, v), se.thicknesses.set(ee, n), se.shearModuli.set(ee, f);
    ...
    function uo(t) {
      const o = Rn.find((l) => l.id === K.forceId), n = Fo.find((l) => l.id === K.lengthId);
    ...
    function Mo() {
      ...
      if (!o) return;
      o.style.display = "";
      const n = T, l = Math.round(((_a2 = ie.nPisos) == null ? void 0 : _a2.val) ?? 3), s = Os(), c = Ns(), a = ce.length || 1, r = fe.length || 1;
...
`);
    ...
    function hn() {
      ...
      if (!n.loads || n.loads.size === 0) {
        ...
        for (const m of O) {
          const A = $.has(m) ? S : 0, F = z.has(m) ? I : 0, H = y.get(m) ?? 0, J = E.has(m) ? g : 0, Q = H + J;
    ...
    setTimeout(() => {
      ...
      function o(y) {
        ...
        if (y === "test-cantilever" || y === "test-all") {
          ...
          Ie.elasticities.set(0, A), Ie.shearModuli.set(0, H), Ie.areas.set(0, J), Ie.momentsOfInertiaZ.set(0, Q), Ie.momentsOfInertiaY.set(0, Q), Ie.torsionalConstants.set(0, Z);
          const je = Ft(pe, U, {
            supports: /* @__PURE__ */ new Map([
              [
                0,
          ...
          oe.push({
            ...
            results: [
              {
                label: "Uz tip (cm)",
                awatif: je.deformations.get(1)[2] * 100,
                reference: re * 100,
        ...
        if (c(oe), oe.length > 0) {
          ...
          ee.nodes.forEach((U, Ie) => {
            Math.abs(U[2]) < 0.01 && W.set(Ie, [
              true,
              true,
              true,
              true,
              true,
              true
            ]), Math.abs(U[2] - pe) < 0.01 && re.set(Ie, [
              10,
              0,
              0,
              0,
      ...
      function n(y) {
        ...
        const Q = y.elements.some((ue) => ue.length === 4);
        ...
        if (y.elements.forEach((ue, q) => {
          ...
          const oe = y.nodes[ue[0]], se = y.nodes[ue[1]], ee = Math.abs(se[1] - oe[1]), W = Math.sqrt((se[0] - oe[0]) ** 2 + (se[2] - oe[2]) ** 2), re = ee > W * 0.5, pe = j(ue[0]), U = j(ue[1]), Ie = re ? "COL30" : "VIGA";
          re ? (m.push(`  LINE  "E${q + 1}"  COLUMN  "${pe.pt}"  "${pe.pt}"  1`), he.push(`  LINEASSIGN  "E${q + 1}"  "${U.story}"  SECTION "${Ie}"  `)) : (m.push(`  LINE  "E${q + 1}"  BEAM  "${pe.pt}"  "${U.pt}"  0`), he.push(`  LINEASSIGN  "E${q + 1}"  "${pe.story}"  SECTION "${Ie}"  `));
        }), m.push(""), Q) {
...
`);
      ...
      function i(y) {
        ...
        qe = F, Fe = H, Qe = Z, setTimeout(() => {
          Et(), No(F, H, Z, J, Q), an(y.stories, F, H), yn(), $n();
        }, 100);
        const te = {
          COLUMN: 0,
          BEAM: 0,
        ...
        console.log(`E2K grids: X=[${J.join(",")}] Y=[${Q.join(",")}]`), console.log(`E2K stories: ${y.stories.map((j) => `${j.name}@${j.elev.toFixed(2)}`).join(", ")}`), console.log(`E2K elements: ${te.COLUMN} columns, ${te.BEAM} beams, ${te.BRACE} braces`), Je();
      }
      function b(y, E) {
        const m = new Blob([
          y
      ...
      w && (w.value = _, w.addEventListener("change", (y) => {
        ...
        y.addEventListener("click", (E) => {
          ...
          A && (x = A.force, _ = A.length, K = A.stress, T = vo(x, _), M && (M.value = x), w && (w.value = _), we.querySelectorAll("[data-preset]").forEach((F) => {
            F.classList.toggle("active", F.dataset.preset === m);
          }), L && Xe(L), console.log(`Preset: ${m} \u2192 ${x}+${_}, stress: ${K.label}`));
        });
...
${d.join(", ")}`);
        ...
        G = true, Y.clear(), ct.forEach((n) => Y.add(n)), ct.clear(), ro(), io(), Se();
      }), o.querySelector("#sel-showall").addEventListener("click", () => {
        V.clear(), G = false, Y.clear(), io(), Se();
    ...
    function sa(t) {
      ...
      const N = [];
      for (const J of p) {
        let Q = w, Z = v, te = f, j = g, he = S, ze = I, ue = k, q = O, oe;
        const se = J - b;
