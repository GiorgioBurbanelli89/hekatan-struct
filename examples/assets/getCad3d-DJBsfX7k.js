...
import { d as kt, _ as ea, p as Uo, m as Va, s as Ua, __tla as __tla_0 } from "./didacticCpp-BGUxSkhs.js";
import { v as Jn, P as no, g as Xa, a as Ka, o as Za } from "./theme-CzzIlc4y.js";
import { G as so, b as Qa, M as ta, D as na, B as tn, c as xo, d as el, C as tl, e as ca, V as Oe, P as In, R as oa, f as sa, g as jn, h as Wn, i as nl, S as ol, j as sl, F as Gn, a as Yn, L as Ln, k as al, l as ll, A as co, T as Xo, m as po, n as fo, o as rl, p as il } from "./Text-CBH-tcJP.js";
...
let __tla = Promise.all([
  ...
  function Fn(e, g) {
    const P = ns.find((W) => W.id === e), M = Vn.find((W) => W.id === g), J = P.toKN, H = M.toM, X = (W, ge, T) => T / (Math.pow(J, W) * Math.pow(H, ge));
    let Y, j;
    switch (e) {
      case "kN":
        Y = 10, j = [
...
<tr style="color:#ff0; border-bottom:1px solid #ff03; position:sticky; top:36px; background:rgba(0,0,0,0.95); z-index:1;">
  ...
  <th style="padding:2px 6px">\u03C9 (rad/s)</th>`;
      for (const W of H) Y += `<th style="padding:2px 5px">${W}</th>`;
      if (Y += `<th style="padding:2px 5px; color:#0ff">\u03A3Ux</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Uy</th>
  <th style="padding:2px 5px; color:#0ff">\u03A3Rz</th></tr>`, M.frequencies.forEach((W, ge) => {
        var _a2;
        const T = W > 0 ? 1 / W : 0, Z = W * 2 * Math.PI, ye = ((_a2 = M.massParticipation) == null ? void 0 : _a2[ge]) || [
          0,
          0,
          0,
          0,
        ...
        for (let ie = 0; ie < 6; ie++) X[ie] += ye[ie];
  ...
  <td style="padding:2px 6px; text-align:right">${Z.toFixed(2)}</td>`;
        ...
        Y += `<td style="padding:2px 5px; text-align:right; color:#0ff">${(X[0] * 100).toFixed(1)}%</td>
  ...
  <td style="padding:2px 5px; text-align:right; color:#0ff">${(X[5] * 100).toFixed(1)}%</td></tr>`;
        ...
        const W = e.querySelector("#modal-body"), ge = e.querySelector("#modal-minimize");
        W && (W.style.display = "none"), ge && (ge.textContent = "\u25A2", ge.title = "Restaurar");
      }
      (_a = e.querySelector("#modal-minimize")) == null ? void 0 : _a.addEventListener("click", () => {
        g = !g;
        const W = e.querySelector("#modal-body"), ge = e.querySelector("#modal-minimize");
        g ? (W.style.display = "none", ge.textContent = "\u25A2", ge.title = "Restaurar") : (W.style.display = "block", ge.textContent = "\u25AC", ge.title = "Minimizar");
      });
      const j = e.querySelector("#modal-header");
      if (j) {
        let W = false, ge = 0, T = 0, Z = 0, ye = 0;
        j.addEventListener("mousedown", (ie) => {
          if (ie.target.tagName === "BUTTON") return;
          W = true, ge = ie.clientX, T = ie.clientY;
      ...
      (_b = e.querySelector("#modal-copy")) == null ? void 0 : _b.addEventListener("click", () => {
        ...
        const ge = `Mode  Freq(Hz)  Period(s)  \u03C9(rad/s)  ${H.join("     ")}  \u03A3Ux    \u03A3Uy    \u03A3Rz`;
        W.push(ge), W.push("-".repeat(ge.length));
        const T = [
          0,
          0,
          0,
        ...
        M.frequencies.forEach((ye, ie) => {
          ...
          W.push(`${String(ie + 1).padStart(4)}  ${ye.toFixed(4).padStart(9)}  ${re.toFixed(4).padStart(9)}  ${te.toFixed(2).padStart(9)}  ${D}  ${(T[0] * 100).toFixed(1).padStart(5)}%  ${(T[1] * 100).toFixed(1).padStart(5)}%  ${(T[5] * 100).toFixed(1).padStart(5)}%`);
...
`));
  ...
  function ul(e) {
    ...
    const { nodes: g, elements: P, elementInputs: M, nodeInputs: J, deformOutputs: H } = e, X = g.length * 6, Y = P.length, j = P.filter((re) => re.length === 2).length, W = P.filter((re) => re.length >= 3).length, ge = document.createElement("div");
    ge.className = "rpt-overlay";
    let T = "";
    T += '<button class="rpt-close" id="rpt-close">\u2715 Close</button>', T += "<h1>Finite Element Analysis \u2014 Step-by-Step Report</h1>", T += '<div class="rpt-subtitle">Complete FEM derivation from element formulation to final results</div>', T += '<hr class="rpt-sep"/>', T += "<h2>1. Input Data</h2>", T += '<table class="rpt-info"><tbody>', T += `<tr><td>Number of nodes</td><td class="val">${g.length}</td></tr>`, T += `<tr><td>Number of elements</td><td class="val">${Y} (${j} frames, ${W} shells)</td></tr>`, T += '<tr><td>DOFs per node</td><td class="val">6 (u<sub>x</sub>, u<sub>y</sub>, u<sub>z</sub>, \u03B8<sub>x</sub>, \u03B8<sub>y</sub>, \u03B8<sub>z</sub>)</td></tr>', T += `<tr><td>Total DOFs</td><td class="val">${X}</td></tr>`, T += "</tbody></table>", T += "<h3>1.1 Node Coordinates</h3>", T += '<table class="rpt-data"><thead><tr><th>Node</th><th>x</th><th>y</th><th>z</th></tr></thead><tbody>', g.forEach((re, te) => {
      T += `<tr><td>${te}</td><td>${ot(re[0])}</td><td>${ot(re[1])}</td><td>${ot(re[2])}</td></tr>`;
    }), T += "</tbody></table>", T += "<h3>1.2 Element Connectivity</h3>", T += '<table class="rpt-data"><thead><tr><th>Elem</th><th>Type</th><th>Nodes</th><th>L</th><th>E</th><th>A</th><th>I<sub>z</sub></th><th>I<sub>y</sub></th></tr></thead><tbody>', P.forEach((re, te) => {
      var _a2, _b2, _c2, _d2;
      const q = re.length === 2, D = re.map((ve) => g[ve]), G = q ? Rn(xn(D[1], D[0])) : 0, de = ((_a2 = M.elasticities) == null ? void 0 : _a2.get(te)) ?? 0, $e = ((_b2 = M.areas) == null ? void 0 : _b2.get(te)) ?? 0, Te = ((_c2 = M.momentsOfInertiaZ) == null ? void 0 : _c2.get(te)) ?? 0, Ve = ((_d2 = M.momentsOfInertiaY) == null ? void 0 : _d2.get(te)) ?? 0;
    ...
    for (let re = 0; re < Y; re++) {
      const te = P[re], q = te.map((at) => g[at]);
      if (!(te.length === 2)) continue;
      const G = Rn(xn(q[1], q[0])), de = ((_a = M.elasticities) == null ? void 0 : _a.get(re)) ?? 0, $e = ((_b = M.areas) == null ? void 0 : _b.get(re)) ?? 0, Te = ((_c = M.momentsOfInertiaZ) == null ? void 0 : _c.get(re)) ?? 0, Ve = ((_d = M.momentsOfInertiaY) == null ? void 0 : _d.get(re)) ?? 0, ve = ((_e = M.shearModuli) == null ? void 0 : _e.get(re)) ?? 0, Be = ((_f = M.torsionalConstants) == null ? void 0 : _f.get(re)) ?? 0;
    ...
    T += "<h2>4. Global Assembly</h2>", T += `<div class="rpt-eq rpt-eq-highlight">K<sub>total</sub> = \u03A3<sub>e=0</sub><sup>${Y - 1}</sup> (T<sub>e</sub>\u1D40 \xB7 k<sub>e</sub> \xB7 T<sub>e</sub>)</div>`, T += "<p>Each element contributes its K<sub>global</sub> to the positions determined by its node DOF indices.</p>", T += gl(P, g.length), T += "<h2>5. Boundary Conditions</h2>";
    const Z = [
      "u<sub>x</sub>",
      "u<sub>y</sub>",
      "u<sub>z</sub>",
    ...
    if (T += `<div class="rpt-eq-small">Fixed DOFs: [${ye.join(", ")}] \u2192 ${ye.length} constraints<br/>`, T += `Free DOFs: ${X} \u2212 ${ye.length} = <b>${X - ye.length}</b></div>`, T += "<h3>5.2 Applied Loads</h3>", J.loads && J.loads.size > 0) {
      T += '<table class="rpt-data"><thead><tr><th>Node</th>';
      const re = [
        "F<sub>x</sub>",
        "F<sub>y</sub>",
    ...
    if (T += "<h2>6. Solution</h2>", T += "<p>After removing fixed DOFs, the reduced system is:</p>", T += '<div class="rpt-eq rpt-eq-highlight">K<sub>free</sub> \xB7 u<sub>free</sub> = F<sub>free</sub></div>', T += "<p>Solved using LU decomposition with partial pivoting (sparse matrix).</p>", T += "<h3>6.1 Nodal Displacements</h3>", H == null ? void 0 : H.deformations) {
      T += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const re of Z) T += `<th>${re}</th>`;
      T += "</tr></thead><tbody>", H.deformations.forEach((re, te) => {
    ...
    if (T += "<h3>6.2 Reactions</h3>", T += '<div class="rpt-eq">R = K<sub>total</sub> \xB7 u (extract at fixed DOFs)</div>', H == null ? void 0 : H.reactions) {
      T += '<table class="rpt-data"><thead><tr><th>Node</th>';
      for (const re of Z) T += `<th>${re}</th>`;
      T += "</tr></thead><tbody>", H.reactions.forEach((re, te) => {
        T += `<tr><td>${te}</td>`, re.forEach((q) => {
          const D = Math.abs(q) > 1e-10;
    ...
    if (T += "<h2>7. Internal Forces</h2>", T += "<p>For each element, transform global displacements to local and multiply by K<sub>local</sub>:</p>", T += '<div class="rpt-eq">u<sub>local</sub> = T \xB7 u<sub>global</sub></div>', T += '<div class="rpt-eq rpt-eq-highlight">f<sub>local</sub> = K<sub>local</sub> \xB7 u<sub>local</sub></div>', H == null ? void 0 : H.deformations) {
      ...
      for (let te = 0; te < Y; te++) {
        ...
        try {
          ...
          for (const ve of q) {
            const Be = ((_g = H.deformations) == null ? void 0 : _g.get(ve)) || [
              0,
              0,
              0,
              0,
  ...
  function Ko(e, g) {
    ...
    for (let J = 0; J < P; J++) {
      M += "<tr>";
      for (let H = 0; H < P; H++) {
        const X = ((_a = e[J]) == null ? void 0 : _a[H]) ?? 0, Y = Math.abs(X) < 1e-10;
        M += `<td class="${Y ? "z" : ""} ${J === H && !Y ? "diag" : ""}">${Y ? "0" : ml(X)}</td>`;
      }
      M += "</tr>";
    }
  ...
  function bl() {
    ...
    for (const j of X) {
      ...
      for (let ye = 0; ye <= 80; ye++) {
        const ie = ye / 80, re = 30 + ie * 540, te = 180 / 2 - j.fn(ie) * 60;
        W += (ye === 0 ? "M" : "L") + `${re.toFixed(1)},${te.toFixed(1)}`;
      }
      Y += `<path d="${W}" fill="none" stroke="${j.color}" stroke-width="2.5"/>`;
      const ge = 0.75, T = 30 + ge * 540 + 8, Z = 180 / 2 - j.fn(ge) * 60 - 6;
      Y += `<text x="${T}" y="${Z}" fill="${j.color}" font-size="11" font-weight="bold" font-family="sans-serif">${j.name}</text>`;
    }
    return Y += "</svg>", Y;
  }
  function gl(e, g) {
    ...
    for (let X = 0; X < M; X++) J += `<td style="color:#003366;font-weight:bold;font-size:7px">${X}</td>`;
    J += "</tr>";
    const H = Array.from({
      length: M
    }, () => Array(M).fill(0));
    for (let X = 0; X < e.length; X++) {
      const Y = e[X].map((j) => j * 6);
      for (const j of Y) for (const W of Y) for (let ge = 0; ge < 6; ge++) for (let T = 0; T < 6; T++) {
        const Z = j + ge, ye = W + T;
        Z < M && ye < M && H[Z][ye]++;
      }
    }
    for (let X = 0; X < M; X++) {
      J += `<tr><td style="color:#003366;font-weight:bold;font-size:7px">${X}</td>`;
      for (let Y = 0; Y < M; Y++) {
        const j = H[X][Y], W = j === 0 ? "#fff" : j === 1 ? "#e8f0fe" : j === 2 ? "#c6dcf5" : "#a0c4e8", ge = j === 0 ? "" : j.toString();
        J += `<td style="background:${W};color:#003366">${ge}</td>`;
      }
      J += "</tr>";
    }
  ...
  function xl(e, g, P, M, J, H) {
    var _a, _b, _c, _d, _e, _f;
    const X = P[e], Y = X.map((be) => g[be]), j = X.length === 2, W = j ? Rn(xn(Y[1], Y[0])) : 0, ge = ((_a = M.elasticities) == null ? void 0 : _a.get(e)) ?? 0, T = ((_b = M.areas) == null ? void 0 : _b.get(e)) ?? 0, Z = ((_c = M.momentsOfInertiaZ) == null ? void 0 : _c.get(e)) ?? 0, ye = ((_d = M.momentsOfInertiaY) == null ? void 0 : _d.get(e)) ?? 0, ie = ((_e = M.shearModuli) == null ? void 0 : _e.get(e)) ?? 0, re = ((_f = M.torsionalConstants) == null ? void 0 : _f.get(e)) ?? 0;
    let te = null, q = null, D = null;
    try {
      te = vo(Y, M, e), q = yo(Y), D = on(os(q), on(te, q));
    } catch {
    }
    const G = j ? xn(Y[1], Y[0]) : [
      0,
      0,
      0
    ], de = W > 0 ? G[0] / W : 0, $e = W > 0 ? G[1] / W : 0, Te = W > 0 ? G[2] / W : 0, Ve = Math.sqrt(de ** 2 + $e ** 2), ve = [];
    if ((J == null ? void 0 : J.deformations) && j) for (const be of X) {
  ...
  function vl(e, g, P, M, J, H) {
    var _a, _b;
    const X = xl(e, g, P, M, J, H), Y = document.createElement("div");
    return Y.className = "er-panel", Y.innerHTML = wl + `
    <div class="er-header">
      <span class="er-badge">Element ${e}</span>
      <span class="er-type">${X.isFrame ? "Frame" : "Shell"} \u2014 Nodes ${X.elem.join(" \u2192 ")} \u2014 L = ${Ye(X.L)}</span>
    ...
    <div class="er-body" id="er-body-tabla">${yl(X)}</div>
    <div class="er-body" id="er-body-math" style="display:none">${la(X)}</div>
    <div class="er-body" id="er-body-resumen" style="display:none">${$l(X)}</div>
  `, Y.querySelectorAll(".er-tab").forEach((j) => {
      j.addEventListener("click", () => {
        Y.querySelectorAll(".er-tab").forEach((ge) => ge.classList.remove("active")), j.classList.add("active");
        const W = j.dataset.tab;
        Y.querySelectorAll(".er-body").forEach((ge) => ge.style.display = "none"), Y.querySelector(`#er-body-${W}`).style.display = "";
      });
      ...
      j && (j.innerHTML = la(X)), setTimeout(() => {
        const W = Y.querySelector("#er-sf-canvas-math");
        W && Qo(W);
      }, 50), Y.querySelectorAll(".er-deriv-header").forEach((W) => {
        W.addEventListener("click", () => {
          const ge = W.dataset.toggle, T = Y.querySelector(`#er-${ge}`);
    ...
    if (g += '<div class="er-section-title">1. Propiedades</div>', g += '<table class="er-props">', g += `<tr><td>E</td><td>${Ye(e.E)}</td><td>A</td><td>${Ye(e.A)}</td></tr>`, g += `<tr><td>I<sub>z</sub></td><td>${Ye(e.Iz)}</td><td>I<sub>y</sub></td><td>${Ye(e.Iy)}</td></tr>`, g += `<tr><td>G</td><td>${Ye(e.G)}</td><td>J</td><td>${Ye(e.J)}</td></tr>`, g += "</table>", e.kLocal && (g += `<div class="er-section-title">2. K<sub>local</sub> (${e.kLocal.length}\xD7${e.kLocal.length})</div>`, g += ao(e.kLocal)), e.T && (g += '<div class="er-section-title">3. T \u2014 Transformaci\xF3n</div>', g += ao(e.T)), e.kGlobal && (g += '<div class="er-section-title">4. K<sub>global</sub> = T<sup>T</sup>\xB7K\xB7T</div>', g += ao(e.kGlobal)), g += '<div class="er-section-title">5. Desplazamientos</div>', e.uGlobal.length > 0) {
      ...
      for (let M = 0; M < e.elem.length; M++) {
        ...
        for (let J = 0; J < 6; J++) {
          ...
          g += `${P[J]}=<span class="${Math.abs(H) > 1e-10 ? "nz" : ""}">${Ye(H, 6)}</span> `;
        }
        g += "</div>";
      }
  ...
  function $l(e) {
    ...
    if (g += `<div class="er-section-title">Resumen \u2014 Elemento ${e.elemIdx}</div>`, g += '<table class="er-props">', g += `<tr><td>Tipo</td><td>${e.isFrame ? "Frame (Euler-Bernoulli)" : "Shell"}</td></tr>`, g += `<tr><td>Nodos</td><td>${e.elem.join(" \u2192 ")}</td></tr>`, g += `<tr><td>Longitud</td><td><b>${Ye(e.L)}</b></td></tr>`, g += `<tr><td>E</td><td>${Ye(e.E)}</td></tr>`, g += `<tr><td>A</td><td>${Ye(e.A)}</td></tr>`, g += "</table>", e.uGlobal.length > 0) {
      ...
      for (let M = 0; M < e.elem.length; M++) {
        ...
        for (let J = 0; J < 6; J++) {
          const H = e.uGlobal[M * 6 + J];
          g += `<td class="${Math.abs(H) > 1e-10 ? "nz" : ""}">${Ye(H, 6)}</td>`;
        }
        g += "</tr>";
      }
      g += "</table>";
  ...
  function Qo(e) {
    ...
    if (!g) return;
    const P = e.width, M = e.height, J = 30, H = P - 2 * J, X = (M - 3 * J) / 2;
    g.fillStyle = getComputedStyle(document.body).getPropertyValue("--fem-bg").trim() || "#111", g.fillRect(0, 0, P, M);
    const Y = (j, W, ge) => {
      g.strokeStyle = "#333", g.lineWidth = 1, g.strokeRect(J, j, H, X), g.strokeStyle = "#444", g.beginPath(), g.moveTo(J, j + X / 2), g.lineTo(J + H, j + X / 2), g.stroke(), g.fillStyle = "#888", g.font = "11px sans-serif", g.fillText(W, J + 4, j + 14);
      for (const Z of ge) {
        g.strokeStyle = Z.color, g.lineWidth = 2.5, g.beginPath();
        for (let ye = 0; ye <= 100; ye++) {
          const ie = ye / 100, re = J + ie * H, te = j + X / 2 - Z.fn(ie) * (X / 2 * 0.85);
          ye === 0 ? g.moveTo(re, te) : g.lineTo(re, te);
        }
        g.stroke();
      }
      let T = J + H - 90;
      for (const Z of ge) g.fillStyle = Z.color, g.font = "bold 10px sans-serif", g.fillText(Z.label, T, j + X - 6), T += 36;
      g.fillStyle = "#666", g.font = "9px monospace", g.fillText("0", J, j + X + 12), g.fillText("1", J + H - 6, j + X + 12), g.fillText("\u03BE", J + H / 2, j + X + 12);
    };
    Y(J, "Axial (lineal)", [
      {
        fn: (j) => 1 - j,
    ...
    ]), Y(J + X + J, "Flexi\xF3n (Hermite c\xFAbicos)", [
      {
  ...
  let Mo = false, Pn = null, ln = null, Bt = null, Tt = null;
  function El() {
  ...
  function Sl() {
    Mo = true, Tt && (Tt.innerHTML = "\u2715", Tt.style.background = "linear-gradient(135deg, #cc3333, #ff4444)", Tt.style.animation = "none"), Pn = document.createElement("div"), Pn.id = "tour-overlay", Pn.style.cssText = `
    position: fixed; inset: 0; z-index: 9999990;
    pointer-events: none;
  `, document.body.appendChild(Pn), Un(0);
  }
  function ss() {
    Mo = false, Tt && (Tt.innerHTML = "?", Tt.style.background = "linear-gradient(135deg, #0066cc, #0099ff)", Tt.style.animation = "helpPulse 2s infinite"), ln && (ln.remove(), ln = null), Bt && (Bt.remove(), Bt = null), Pn && (Pn.remove(), Pn = null);
  }
  function Un(e) {
    var _a, _b;
    if (e >= oo.length) {
    ...
    const g = oo[e], P = document.querySelector(g.selector);
    if (!P) {
      Un(e + 1);
      return;
    }
    ...
    const M = P.getBoundingClientRect(), J = window.innerWidth, H = window.innerHeight, X = 320, Y = 180;
  ...
  `, document.body.appendChild(ln);
    const j = J - M.right, W = M.left, ge = H - M.bottom, T = M.top;
    let Z = g.position || "bottom";
    Z === "bottom" && ge < Y + 20 && (Z = "top"), Z === "top" && T < Y + 20 && (Z = "right"), Z === "right" && j < X + 20 && (Z = "left"), Z === "left" && W < X + 20 && (Z = "bottom");
    let ye, ie, re = "";
    switch (Z) {
      case "bottom":
        ye = M.left + M.width / 2 - X / 2, ie = M.bottom + 14, re = "position:absolute;top:-8px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-bottom:8px solid #0099ff;";
        break;
      case "top":
        ye = M.left + M.width / 2 - X / 2, ie = M.top - Y - 14, re = "position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);width:0;height:0;border-left:8px solid transparent;border-right:8px solid transparent;border-top:8px solid #0099ff;";
        break;
      case "right":
        ...
        break;
      case "left":
        ye = M.left - X - 14, ie = M.top + M.height / 2 - Y / 2, re = "position:absolute;right:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-left:8px solid #0099ff;";
        break;
    }
    ye = Math.max(10, Math.min(ye, J - X - 10)), ie = Math.max(10, Math.min(ie, H - Y - 10)), Bt = document.createElement("div"), Bt.style.cssText = `
    position: fixed;
  ...
  function kl(e) {
    ...
    const g = e.split(/\r?\n/), P = {
      force: "TONF",
      length: "M"
    }, M = [], J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), Y = [], j = [], W = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), T = [], Z = [];
    let ye = "", ie = "";
    ...
    for (const Ce of g) {
      ...
      if (ie === "FRAME SECTIONS") {
        ...
        if (he) {
          ...
          const xe = H.get(Ae), ke = qe.match(/MATERIAL\s+"([^"]+)"/);
          ke && (xe.material = ke[1]);
    ...
    for (const [Ce] of W) $e.add(Ce);
    for (const Ce of $e) {
      const [qe, he] = Ce.split("@"), Ae = X.get(qe), xe = te.get(he);
  ...
  function zl(e) {
    ...
    for (const te of e) {
      const q = te.trimEnd();
      q.endsWith("_") ? P += q.slice(0, -1) + " " : (P += q, g.push(P), P = "");
    }
    P && g.push(P);
    const M = {
      force: "KN",
    ...
    const H = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), W = [], ge = [], T = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map(), ye = /* @__PURE__ */ new Map(), ie = [];
    let re = "";
    for (const te of g) {
      const q = te.trim();
      ...
      switch (re) {
        ...
        case "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES": {
          ...
          if (G) {
            ...
            de.E = tt(D.get("E1")), de.G = tt(D.get("G12")), de.nu = tt(D.get("U12")), de.density = tt(D.get("UnitMass")), H.set(G, de);
          }
          break;
        }
    ...
    return pa(M, J, H, X, Y, j, W, ge, T, Z, ye, ie);
  }
  function Al(e) {
    const g = {
      force: "KN",
    ...
    const M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), Y = [], j = [], W = /* @__PURE__ */ new Map(), ge = [];
    let T = "", Z = "";
    for (const re of e) {
      const te = re.trim();
    ...
    return pa(g, P, M, J, H, X, Y, j, W, /* @__PURE__ */ new Map(), /* @__PURE__ */ new Map(), ge);
  }
  function pa(e, g, P, M, J, H, X, Y, j, W, ge, T) {
    var _a;
    const Z = [], ye = /* @__PURE__ */ new Map(), ie = [];
    for (const [ve, Be] of H) ye.set(ve, ie.length), Z.push(ve), ie.push(Be);
  ...
  function Ll(e) {
    ...
    const { nodes: g, elements: P, nodeInputs: M, elementInputs: J } = e, H = e.units || {
      force: "KN",
      length: "m"
    }, X = e.title || "Awatif Model", Y = [], j = (q) => Y.push(q), W = () => Y.push(" ");
    ...
    if (P.forEach((q, D) => {
      ...
      for (const q of ge) {
        const D = P[q];
        j(`   Frame=${q + 1}   JointI=${D[0] + 1}   JointJ=${D[1] + 1}   IsCurved=No`);
      }
      W();
    }
    if (T.length > 0) {
      ...
      for (const q of T) {
        const D = P[q], G = D.map((de, $e) => `Joint${$e + 1}=${de + 1}`).join("   ");
        j(`   Area=${q + 1}   NumJoints=${D.length}   ${G}`);
      }
      W();
    }
    ...
    if (ge.length > 0) {
      ...
      for (const q of ge) {
        ...
        j(`   Frame=${q + 1}   AutoSelect=N.A.   AnalSect=${D}   MatProp=Default`);
      }
      W();
    }
    if (Z.size > 0) {
      ...
      for (const [, D] of Z) {
        q++;
        const G = D.A * 5 / 6;
        j(`   SectionName=SEC${q}   Material=${D.matKey}   Shape=Rectangular   t3=${It(D.h)}   t2=${It(D.b)}   Area=${It(D.A)}   TorsConst=${It(D.J)}   I33=${It(D.Iz)}   I22=${It(D.Iy)}   I23=0   AS2=${It(G)}   AS3=${It(G)} _`), j("        Color=Blue   FromFile=No   AMod=1   A2Mod=1   A3Mod=1   JMod=1   I2Mod=1   I3Mod=1   MMod=1   WMod=1");
      }
      W();
    }
    ...
    if (T.length > 0) {
      ...
      for (const D of T) {
        ...
        j(`   Area=${D + 1}   Section=${G}   MatProp=Default`);
      }
      W(), j('TABLE:  "AREA SECTION PROPERTIES"');
      let q = 0;
      for (const [, D] of ie) q++, j(`   Section=SSEC${q}   Material=${D.matKey}   MatAngle=0   AreaType=Shell   Type=ShellThin   DrillDOF=Yes   Thickness=${It(D.t)}   BendThick=${It(D.t)}   Color=Cyan`);
      W();
    }
    j('TABLE:  "JOINT COORDINATES"');
    for (let q = 0; q < g.length; q++) {
      const D = g[q];
      j(`   Joint=${q + 1}   CoordSys=GLOBAL   CoordType=Cartesian   XorR=${It(D[0])}   Y=${It(D[1])}   Z=${It(D[2])}   SpecialJt=No`);
    }
    if (W(), M.supports && M.supports.size > 0) {
      ...
      for (const [q, D] of M.supports) {
        ...
        j(`   Joint=${q + 1}   U1=${G(D[0])}   U2=${G(D[1])}   U3=${G(D[2])}   R1=${G(D[3])}   R2=${G(D[4])}   R3=${G(D[5])}`);
      }
      W();
    }
    if (j('TABLE:  "LOAD PATTERN DEFINITIONS"'), j("   LoadPat=DEAD   DesignType=Dead   SelfWtMult=0"), W(), j('TABLE:  "LOAD CASE DEFINITIONS"'), j('   Case=DEAD   Type=LinStatic   InitialCond=Zero   DesTypeOpt="Prog Det"   DesignType=Dead   DesActOpt="Prog Det"   DesignAct=Non-Composite   AutoType=None   RunCase=Yes'), W(), j('TABLE:  "CASE - STATIC 1 - LOAD ASSIGNMENTS"'), j('   Case=DEAD   LoadType="Load pattern"   LoadName=DEAD   LoadSF=1'), W(), M.forces && M.forces.size > 0) {
      ...
      for (const [q, D] of M.forces) D.some((G) => Math.abs(G) > 1e-12) && j(`   Joint=${q + 1}   LoadPat=DEAD   CoordSys=GLOBAL   F1=${It(D[0])}   F2=${It(D[1])}   F3=${It(D[2])}   M1=${It(D[3])}   M2=${It(D[4])}   M3=${It(D[5])}`);
      W();
    }
    const te = /* @__PURE__ */ new Map();
    ...
    for (const [q] of te) j(`   Material=${q}   Type=Concrete   SymType=Isotropic   TempDepend=No   Color=Green`);
    W(), j('TABLE:  "MATERIAL PROPERTIES 02 - BASIC MECHANICAL PROPERTIES"');
    for (const [q, D] of te) j(`   Material=${q}   UnitWeight=${It(D.rho * 9.81)}   UnitMass=${It(D.rho)}   E1=${It(D.E)}   G12=${It(D.G)}   U12=${It(D.nu)}   A1=9.9E-06`);
    W(), j('TABLE:  "MATERIAL PROPERTIES 03B - CONCRETE DATA"');
    for (const [q] of te) j(`   Material=${q}   Fc=27579   eFc=27579   LtWtConc=No   SSCurveOpt=Mander   SSHysType=Takeda   SFc=0.00222   SCap=0.005   FinalSlope=-0.1   FAngle=0   DAngle=0`);
    return W(), j('TABLE:  "PROGRAM CONTROL"'), j(`   ProgramName=SAP2000   Version=24.1.0   CurrUnits="${H.force}, ${H.length}, C"   SteelCode="AISC 360-16"   ConcCode="ACI 318-19"   AlumCode="AA 2015"   ColdCode=AISI-16   RegenHinge=Yes`), W(), j("END TABLE DATA"), j(""), Y.join(`\r
`);
  }
  function It(e) {
...
`);
  ...
  function Rl(e) {
    var _a, _b, _c, _d;
    const { nodes: g, elements: P, nodeInputs: M, elementInputs: J, title: H, units: X } = e, Y = (X == null ? void 0 : X.force) || "TONF", j = (X == null ? void 0 : X.length) || "M", W = [], ge = (be) => Math.round(be * 1e4) / 1e4;
    ...
    for (let be = Z.length - 1; be >= 1; be--) W.push(`  STORY "${ye[be]}"  HEIGHT ${ge(Z[be] - Z[be - 1])} MASTERSTORY "Yes"  `);
...
`);
  ...
  function Pl(e, g) {
    const P = e[g[0]], M = e[g[1]], J = Math.abs(M[1] - P[1]), H = Math.sqrt((M[0] - P[0]) ** 2 + (M[2] - P[2]) ** 2), X = J > H * 0.5;
    return X && H > 0.01 ? "BRACE" : X ? "COLUMN" : "BEAM";
  }
  function Ol(e) {
    var _a, _b;
    const { nodes: g, elements: P, nodeInputs: M, elementInputs: J } = e, H = [];
    return H.push("# OpenSeesPy model exported from Awatif FEM Studio"), H.push(`# ${g.length} nodes, ${P.length} elements`), H.push(""), H.push("import openseespy.opensees as ops"), H.push(""), H.push("ops.wipe()"), H.push("ops.model('basic', '-ndm', 3, '-ndf', 6)"), H.push(""), H.push("# --- Nodes ---"), g.forEach((X, Y) => {
      H.push(`ops.node(${Y + 1}, ${X[0]}, ${X[1]}, ${X[2]})`);
    }), H.push(""), H.push("# --- Boundary Conditions ---"), (_a = M.supports) == null ? void 0 : _a.forEach((X, Y) => {
      const j = X.map((W) => W ? 1 : 0).join(", ");
      H.push(`ops.fix(${Y + 1}, ${j})`);
    }), H.push(""), H.push("# --- Geometric Transformations ---"), H.push("ops.geomTransf('Linear', 1, 0.0, 0.0, 1.0)  # beams (vecxz = Z)"), H.push("ops.geomTransf('Linear', 2, -1.0, 0.0, 0.0)  # columns (vecxz = -X)"), H.push(""), H.push("# --- Elements (elasticBeamColumn) ---"), P.forEach((X, Y) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (X.length !== 2) return;
      const j = g[X[0]], W = g[X[1]], T = Math.abs(W[2] - j[2]) > Math.max(Math.abs(W[0] - j[0]), Math.abs(W[1] - j[1])) ? 2 : 1, Z = ((_a2 = J.areas) == null ? void 0 : _a2.get(Y)) ?? 1, ye = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(Y)) ?? 2e5, ie = ((_c = J.shearModuli) == null ? void 0 : _c.get(Y)) ?? 8e4, re = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(Y)) ?? 1, te = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(Y)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(Y)) ?? 1;
      H.push(`ops.element('elasticBeamColumn', ${Y + 1}, ${X[0] + 1}, ${X[1] + 1}, ${Z}, ${ye}, ${ie}, ${re}, ${te}, ${q}, ${T})`);
    }), H.push(""), M.loads && M.loads.size > 0 && (H.push("# --- Loads ---"), H.push("ops.timeSeries('Linear', 1)"), H.push("ops.pattern('Plain', 1, 1)"), M.loads.forEach((X, Y) => {
      const j = X.map((W) => W).join(", ");
      H.push(`ops.load(${Y + 1}, ${j})`);
    }), H.push("")), H.push("# --- Analysis ---"), H.push("ops.system('BandGeneral')"), H.push("ops.numberer('RCM')"), H.push("ops.constraints('Plain')"), H.push("ops.integrator('LoadControl', 1.0)"), H.push("ops.algorithm('Linear')"), H.push("ops.analysis('Static')"), H.push("ops.analyze(1)"), H.push(""), H.push("# --- Results ---"), H.push('print("\\n=== Displacements ===")'), g.forEach((X, Y) => {
      H.push(`print(f"Node {${Y + 1}}: {ops.nodeDisp(${Y + 1})}")`);
    }), H.push(""), H.push('print("\\n=== Reactions ===")'), H.push("ops.reactions()"), (_b = M.supports) == null ? void 0 : _b.forEach((X, Y) => {
...
`);
  ...
  function Nl(e) {
    var _a, _b;
    const { nodes: g, elements: P, nodeInputs: M, elementInputs: J } = e, H = [];
    return H.push("# OpenSees Tcl model exported from Awatif FEM Studio"), H.push(`# ${g.length} nodes, ${P.length} elements`), H.push(""), H.push("wipe"), H.push("model basic -ndm 3 -ndf 6"), H.push(""), H.push("# --- Nodes ---"), g.forEach((X, Y) => {
      H.push(`node ${Y + 1} ${X[0]} ${X[1]} ${X[2]}`);
    }), H.push(""), H.push("# --- Boundary Conditions ---"), (_a = M.supports) == null ? void 0 : _a.forEach((X, Y) => {
      const j = X.map((W) => W ? 1 : 0).join(" ");
      H.push(`fix ${Y + 1} ${j}`);
    }), H.push(""), H.push("# --- Geometric Transformations ---"), H.push("geomTransf Linear 1 0.0 0.0 1.0"), H.push("geomTransf Linear 2 -1.0 0.0 0.0"), H.push(""), H.push("# --- Elements ---"), P.forEach((X, Y) => {
      var _a2, _b2, _c, _d, _e, _f;
      if (X.length !== 2) return;
      const j = g[X[0]], W = g[X[1]], T = Math.abs(W[2] - j[2]) > Math.max(Math.abs(W[0] - j[0]), Math.abs(W[1] - j[1])) ? 2 : 1, Z = ((_a2 = J.areas) == null ? void 0 : _a2.get(Y)) ?? 1, ye = ((_b2 = J.elasticities) == null ? void 0 : _b2.get(Y)) ?? 2e5, ie = ((_c = J.shearModuli) == null ? void 0 : _c.get(Y)) ?? 8e4, re = ((_d = J.torsionalConstants) == null ? void 0 : _d.get(Y)) ?? 1, te = ((_e = J.momentsOfInertiaY) == null ? void 0 : _e.get(Y)) ?? 1, q = ((_f = J.momentsOfInertiaZ) == null ? void 0 : _f.get(Y)) ?? 1;
      H.push(`element elasticBeamColumn ${Y + 1} ${X[0] + 1} ${X[1] + 1} ${Z} ${ye} ${ie} ${re} ${te} ${q} ${T}`);
    }), H.push(""), M.loads && M.loads.size > 0 && (H.push("# --- Loads ---"), H.push("timeSeries Linear 1"), H.push("pattern Plain 1 1 {"), M.loads.forEach((X, Y) => {
      const j = X.map((W) => W).join(" ");
      H.push(`  load ${Y + 1} ${j}`);
    ...
    }), H.push('puts "\\n=== Reactions ==="'), H.push("reactions"), (_b = M.supports) == null ? void 0 : _b.forEach((X, Y) => {
      H.push(`puts "Node ${Y + 1}: [nodeReaction ${Y + 1}]"`);
    }), H.join(`
`);
  }
  function ql(e) {
    const g = [], P = [], M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map(), Z = /* @__PURE__ */ new Map();
      ...
      if (q) {
        ...
        if (de !== void 0 && $e !== void 0) {
          ...
          P.push([
            de,
            $e
          ]), Z.set(G, Te), Y.set(Te, parseFloat(q[4])), H.set(Te, parseFloat(q[5])), X.set(Te, parseFloat(q[6])), ge.set(Te, parseFloat(q[7])), j.set(Te, parseFloat(q[8])), W.set(Te, parseFloat(q[9]));
        }
        continue;
      }
  ...
  function _l(e) {
    const g = [], P = [], M = /* @__PURE__ */ new Map(), J = /* @__PURE__ */ new Map(), H = /* @__PURE__ */ new Map(), X = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), j = /* @__PURE__ */ new Map(), W = /* @__PURE__ */ new Map(), ge = /* @__PURE__ */ new Map(), T = /* @__PURE__ */ new Map();
      ...
      if (ie[0] === "element" && ie[1] === "elasticBeamColumn" && ie.length >= 12) {
        ...
        if (re !== void 0 && te !== void 0) {
          ...
          P.push([
            re,
            te
          ]), Y.set(q, parseFloat(ie[5])), H.set(q, parseFloat(ie[6])), X.set(q, parseFloat(ie[7])), ge.set(q, parseFloat(ie[8])), j.set(q, parseFloat(ie[9])), W.set(q, parseFloat(ie[10]));
        }
        continue;
      }
  ...
  function Dl(e) {
    ...
    let X;
    for (; (X = H.exec(e)) !== null; ) {
      const Y = parseInt(X[1]), j = X[2].toUpperCase();
      P[Y] = {
        id: Y,
        type: j,
        args: X[3]
  ...
  function ua(e, g) {
    const P = Ut(g.args), M = Wt(e, P[0]), J = M ? Bl(e, M) : [
      0,
      0,
      0
    ];
  ...
  function ma(e, g) {
    ...
    if (M && (J = ua(e, M)), P[0] && P[0] !== "$") {
      ...
      if (H && H.type === "IFCLOCALPLACEMENT") {
        const X = ma(e, H), Y = ts(J.origin, X.dirX, es(X.dirZ, X.dirX), X.dirZ);
        J.origin = [
          X.origin[0] + Y[0],
          X.origin[1] + Y[1],
          X.origin[2] + Y[2]
        ], J.dirZ = ts(J.dirZ, X.dirX, es(X.dirZ, X.dirX), X.dirZ), J.dirX = ts(J.dirX, X.dirX, es(X.dirZ, X.dirX), X.dirZ);
      }
    }
    return J;
  }
  ...
  const Hl = 0.01;
  function jl(e) {
    const g = Dl(e), { entities: P, typeIndex: M } = g, J = [], H = [], X = /* @__PURE__ */ new Map();
    X.set("Hormigon", {
      E: 2132888792e-2,
      nu: 0.2,
      rho: 2.4
    ...
    function T(te) {
      ...
      if (q === "IFCMATERIALPROFILE") {
        const D = Ut(te.args), G = D[2] || D[1] || "", de = Wt(P, G);
        if (de) return T(de);
      }
    ...
    function Z(te, q, D, G) {
      ...
      for (const $e of de) {
        const Te = P[$e];
        if (!Te) continue;
        const Ve = Ut(Te.args), ve = Ve[5] || Ve[4] || "", Be = Wt(P, ve);
        if (!Be) continue;
        const He = ma(P, Be), be = ge(Te);
  ...
  function $o(e, g) {
    ...
    for (const M of P) {
      const J = M.match(/#(\d+)/g) || [];
      for (const H of J) {
        const X = parseInt(H.replace("#", "")), Y = e[X];
        if (Y) {
          if (Y.type === "IFCEXTRUDEDAREASOLID") {
            const j = Ut(Y.args), W = parseFloat(j[3]) || 0, ge = Wt(e, j[1]);
            let T = [
              0,
              0,
              0
          ...
          if (Y.type === "IFCMAPPEDITEM") {
            const j = Ut(Y.args), W = Wt(e, j[0]);
            if (W && W.type === "IFCREPRESENTATIONMAP") {
              const ge = Ut(W.args), T = Wt(e, ge[1]);
              if (T) {
                const Z = $o(e, T);
                if (Z) return Z;
              }
  ...
  async function Yl(e, g) {
    ...
    const H = M.OpenModel(new Uint8Array(g)), X = /* @__PURE__ */ new Map(), Y = /* @__PURE__ */ new Map(), j = {
      843113511: "Columna",
      753842376: "Viga",
      1529196076: "Losa",
      900683007: "Zapata",
    ...
    for (const [ye] of ba) {
      const ie = Gl(ye);
      try {
        const re = M.GetLineIDsWithType(H, ye);
    ...
    for (const ye of ga) {
      const ie = new so();
      ie.name = `ifc-${ye}`, e.add(ie), W.set(ye, ie);
    }
    const ge = new Qa();
    let T = 0;
    const Z = new ta({
    ...
    return M.StreamAllMeshes(H, (ye) => {
      const ie = X.get(ye.expressID) ?? "other", re = W.get(ie), te = ye.geometries;
      for (let q = 0; q < te.size(); q++) {
        const D = te.get(q), G = M.GetGeometry(H, D.geometryExpressID), de = M.GetVertexArray(G.GetVertexData(), G.GetVertexDataSize()), $e = M.GetIndexArray(G.GetIndexData(), G.GetIndexDataSize()), Te = new tn(), Ve = new Float32Array(de.length / 2), ve = new Float32Array(de.length / 2);
  ...
  er = function(e) {
    ...
    let j = false;
    const W = /* @__PURE__ */ new Set(), ge = /* @__PURE__ */ new Map();
    ...
    function as(t, n, o, l, s) {
      ...
      for (const h of o) for (const I of t) for (const L of n) {
        const z = [
          new Oe(I - p, h, L),
          new Oe(I + p, h, L),
          new Oe(I, h, L - p),
          new Oe(I, h, L + p)
        ], O = new tn().setFromPoints(z), k = new Yn({
          color: 5596791,
          transparent: true,
          opacity: 0.4
        }), m = new Wn(O, k);
    ...
    function Xn(t, n, o, l, s) {
      ...
      const a = Math.min(...t), r = Math.max(...t), f = Math.min(...n), i = Math.max(...n), d = r - a || 1, b = i - f || 1, S = Math.max(d, b), $ = S * 0.08, y = l || t.map((m, u) => String.fromCharCode(65 + u)), p = s || n.map((m, u) => String(u + 1)), h = S * 0.018, I = n.length <= 1, L = 8947848;
      for (let m = 0; m < t.length; m++) {
        const u = t[m];
        if (I) {
          const E = -$ - h * 1.5;
          Io(u, 0, 0, u, 0, E + h, L, Pt), ko(y[m] || `${m}`, u, 0, E, h, Pt);
        } else {
          const E = f - $ - h * 1.5;
          Io(u, f, 0, u, E + h, 0, L, Pt), ko(y[m] || `${m}`, u, E, 0, h, Pt);
        }
      }
      if (!I) for (let m = 0; m < n.length; m++) {
        const u = n[m], E = a - $ - h * 1.5;
        Io(a, u, 0, E + h, u, 0, L, Pt), ko(p[m] || `${m}`, E, u, 0, h, Pt);
      }
      const z = h * 1.8, O = $ * 1.2, k = $ * 1.2;
      for (let m = 0; m < t.length - 1; m++) {
        const u = t[m], E = t[m + 1], A = Math.abs(E - u), R = (u + E) / 2, B = `${A.toFixed(2)} m`;
        I ? (Eo(B, R, 0, -O, z, Pt), So(u, 0, -O * 0.7, E, 0, -O * 0.7, 16763904, Pt)) : (Eo(B, R, f - k, 0, z, Pt), So(u, f - k * 0.7, 0, E, f - k * 0.7, 0, 16763904, Pt));
      }
      if (!I) for (let m = 0; m < n.length - 1; m++) {
        const u = n[m], E = n[m + 1], A = Math.abs(E - u), R = (u + E) / 2, B = `${A.toFixed(2)} m`;
        Eo(B, a - O, R, 0, z, Pt), So(a - O * 0.7, u, 0, a - O * 0.7, E, 0, 16763904, Pt);
      }
      Pt.traverse((m) => {
    ...
    const Ke = {
      ...
      build(t) {
            ...
            if (_) for (let fe = 0; fe < S; fe++) {
              ...
              for (let me = 0; me < b - 1; me++) {
                ...
                I.push([
                  h[`${me},${fe},${oe}`],
                  h[`${me + 1},${fe},${oe + 1}`]
                ]), m.add(we), O.set(we, oe);
                const ae = I.length;
                I.push([
                  h[`${me + 1},${fe},${oe}`],
                  h[`${me},${fe},${oe + 1}`]
                ]), m.add(ae), O.set(ae, oe);
              }
            }
        ...
        const u = 15100 * Math.sqrt(l) * 10, E = u / (2 * (1 + 0.2)), A = s * c, R = s * c ** 3 / 12, B = c * s ** 3 / 12, x = s * c * (s ** 2 + c ** 2) / 12, w = a * r, v = a * r ** 3 / 12, F = r * a ** 3 / 12, U = a * r * (a ** 2 + r ** 2) / 12, V = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map();
        for (let _ = 0; _ < I.length; _++) V.set(_, u), se.set(_, E), L.has(_) ? (Q.set(_, A), K.set(_, R), pe.set(_, B), ce.set(_, x), Ie.set(_, {
          type: "rect",
          b: s,
          h: c,
          name: `COL${n}`
        })) : m.has(_) ? (Q.set(_, A), K.set(_, R), pe.set(_, B), ce.set(_, x), Ie.set(_, {
          type: "rect",
          b: s,
          h: c,
          name: `BR${n}`
        })) : (Q.set(_, w), K.set(_, v), pe.set(_, F), ce.set(_, U), Ie.set(_, {
          type: "rect",
          b: a,
          h: r,
          name: `V${o}`
        ...
        return e.nodes.val = p, e.elements.val = I, e.nodeInputs.val = {
          ...
          sectionShapes: Ie
        }, D = L, G = z, $e = O, console.log(`Built: ${p.length} nodes, ${I.length} elements (${L.size} cols, ${z.size} beams, ${m.size} braces)`), console.log(`  Col: ${n}, Viga: ${o}, f'c=${l}${k ? `, braces=${k}` : ""}`), {
          nodes: p.length,
          elements: I.length
        };
      },
      ...
      addBeam(t, n, o, l, s) {
        var _a2;
        const c = We.findIndex((O) => O.label === t), a = Xe.findIndex((O) => O.label === n), r = We.findIndex((O) => O.label === o), f = Xe.findIndex((O) => O.label === l), i = lt.findIndex((O) => O.label === s);
        if (c < 0 || a < 0 || r < 0 || f < 0) {
          console.log("Axis not found");
          return;
        }
        if (i < 1) {
          console.log(`Story "${s}" not found. Available: ${lt.filter((O) => O.label !== "Base").map((O) => O.label)}`);
          return;
        }
        const d = We[c].coord, b = Xe[a].coord, S = We[r].coord, $ = Xe[f].coord, y = lt[i].elev, p = [
          ...
          const u = p.findIndex((E) => Math.abs(E[0] - O) < 1e-3 && Math.abs(E[1] - k) < 1e-3 && Math.abs(E[2] - m) < 1e-3);
          return u >= 0 ? u : (p.push([
            O,
            k,
            m
      ...
      addBrace(t, n, o, l, s, c) {
        ...
        const S = We[a].coord, $ = Xe[r].coord, y = lt[f].elev, p = We[i].coord, h = Xe[d].coord, I = lt[b].elev, L = [
          ...e.nodes.val
        ], z = [
          ...((_a2 = e.elements) == null ? void 0 : _a2.val) || []
        ], O = (u, E, A) => {
          const R = L.findIndex((B) => Math.abs(B[0] - u) < 1e-3 && Math.abs(B[1] - E) < 1e-3 && Math.abs(B[2] - A) < 1e-3);
          return R >= 0 ? R : (L.push([
            u,
            E,
            A
        ...
        return z.push([
          k,
          m
        ]), $e.set(z.length - 1, Math.min(f, b)), e.nodes.val = L, e.elements.val = z, console.log(`Added brace ${t}-${n}@${o} \u2192 ${l}-${s}@${c}`), z.length - 1;
      },
      help() {
        return `=== CLI Commands ===
MODEL:
...
`;
      ...
      model3d(t) {
        ...
        const y = b.length, p = S.length, h = $.length, I = l.length, L = [], z = {};
        for (let ee = 0; ee < h; ee++) for (let me = 0; me < p; me++) for (let we = 0; we < y; we++) z[`${we},${ee},${me}`] = L.length, L.push([
          b[we],
          $[ee],
          S[me]
        ]);
        ...
        const A = 15100 * Math.sqrt(a) * 10, R = A / (2 * (1 + 0.2)), B = r * f, x = r * f ** 3 / 12, w = f * r ** 3 / 12, v = r * f * (r ** 2 + f ** 2) / 12, F = i * d, U = i * d ** 3 / 12, V = d * i ** 3 / 12, se = i * d * (i ** 2 + d ** 2) / 12, Q = /* @__PURE__ */ new Map(), K = /* @__PURE__ */ new Map(), pe = /* @__PURE__ */ new Map(), ce = /* @__PURE__ */ new Map(), Ie = /* @__PURE__ */ new Map(), Pe = /* @__PURE__ */ new Map(), _ = /* @__PURE__ */ new Map();
        for (let ee = 0; ee < O.length; ee++) Q.set(ee, A), K.set(ee, R), k.has(ee) ? (pe.set(ee, B), ce.set(ee, x), Ie.set(ee, w), Pe.set(ee, v), _.set(ee, {
          type: "rect",
          b: r,
          h: f,
          name: `COL${s}`
        })) : (pe.set(ee, F), ce.set(ee, U), Ie.set(ee, V), Pe.set(ee, se), _.set(ee, {
          type: "rect",
          b: i,
          h: d,
          name: `V${c}`
        ...
        return as(b, S, $), Qe(), console.log(`Model3D: ${L.length}n ${O.length}e | ${y}x${p} grid, ${I} floors | COL${s} V${c} f'c=${a}`), {
          nodes: L.length,
          elements: O.length,
          columns: k.size,
          beams: m.size
      ...
      building(t, n, o, l = 3, s = 0, c = 0, a = 0, r = 0, f = 1) {
        ...
        for (let u = 0; u < b.length; u++) for (let E = 0; E < d.length; E++) for (let A = 0; A < i.length; A++) u === 0 && y(A, E) || (h[`${A},${E},${u}`] = p.length, p.push([
          i[A],
          d[E],
          b[u]
        ]));
        ...
        for (const { el: [u, E], floor: A } of z) {
          if (f <= 1) {
            D.add(L.length), $e.set(L.length, A), L.push([
              u,
              E
            ]);
            continue;
        ...
        for (const { el: [u, E], floor: A, dir: R, bay: B } of O) {
          const x = p[u], w = p[E];
          let v = u;
          for (let U = 1; U < l; U++) {
            const V = U / l, se = p.length;
            p.push([
              x[0] + (w[0] - x[0]) * V,
              x[1] + (w[1] - x[1]) * V,
              x[2] + (w[2] - x[2]) * V
        ...
        if (st = /* @__PURE__ */ new Set(), Ee && ze > 0) {
          const u = (E, A, R) => {
            for (let x = 0; x < p.length; x++) if (Math.abs(p[x][0] - E) < 1e-6 && Math.abs(p[x][1] - A) < 1e-6 && Math.abs(p[x][2] - R) < 1e-6) return x;
            const B = p.length;
            return p.push([
              E,
              A,
        ...
        for (const u of ve) {
          const E = b.length - 1, A = u.floors.includes(-1) ? Array.from({
            length: E
          }, (R, B) => B) : u.floors.filter((R) => R < E);
          for (const R of A) {
            let B, x, w, v;
            u.dir === "x" ? (B = u.bay, w = u.bay + 1, x = u.axisIdx, v = u.axisIdx) : (B = u.axisIdx, w = u.axisIdx, x = u.bay, v = u.bay + 1);
            const F = h[`${B},${x},${R}`], U = h[`${B},${x},${R + 1}`];
            let V, se;
            if (u.dir === "x" ? (V = h[`${w},${v},${R}`], se = h[`${w},${v},${R + 1}`]) : (V = h[`${w},${v},${R}`], se = h[`${w},${v},${R + 1}`]), F === void 0 || V === void 0 || U === void 0 || se === void 0) continue;
            const Q = be, K = He, pe = p[F], ce = p[V], Ie = p[U], Pe = p[se], _ = [];
            for (let ue = 0; ue <= K; ue++) {
              const oe = [], fe = ue / K;
              for (let ee = 0; ee <= Q; ee++) {
                const me = ee / Q, we = (1 - fe) * ((1 - me) * pe[0] + me * ce[0]) + fe * ((1 - me) * Ie[0] + me * Pe[0]), ae = (1 - fe) * ((1 - me) * pe[1] + me * ce[1]) + fe * ((1 - me) * Ie[1] + me * Pe[1]), Se = (1 - fe) * ((1 - me) * pe[2] + me * ce[2]) + fe * ((1 - me) * Ie[2] + me * Pe[2]);
                ue === 0 && ee === 0 ? oe.push(F) : ue === 0 && ee === Q ? oe.push(V) : ue === K && ee === 0 ? oe.push(U) : ue === K && ee === Q ? oe.push(se) : (oe.push(p.length), p.push([
                  we,
                  ae,
                  Se
                ]));
            ...
            for (let ue = 0; ue < K; ue++) for (let oe = 0; oe < Q; oe++) {
              const fe = _[ue][oe], ee = _[ue][oe + 1], me = _[ue + 1][oe + 1], we = _[ue + 1][oe], ae = L.length;
              de.add(ae), $e.set(ae, R), L.push([
                fe,
                ee,
                me,
                we
        ...
        if (zt = /* @__PURE__ */ new Set(), Ue) {
          const u = l, E = l, A = /* @__PURE__ */ new Map(), R = (B, x, w) => `${Math.round(B * 1e4)},${Math.round(x * 1e4)},${Math.round(w * 1e4)}`;
          for (let B = 0; B < p.length; B++) A.set(R(p[B][0], p[B][1], p[B][2]), B);
          for (let B = 1; B < b.length; B++) {
            const x = b[B];
            for (let w = 0; w < i.length - 1; w++) for (let v = 0; v < d.length - 1; v++) {
              const F = i[w], U = i[w + 1], V = d[v], se = d[v + 1], Q = [];
              for (let K = 0; K <= E; K++) {
                const pe = [];
                for (let ce = 0; ce <= u; ce++) {
                  const Ie = F + ce / u * (U - F), Pe = V + K / E * (se - V);
                  if (K === 0 && ce === 0) pe.push(h[`${w},${v},${B}`]);
                  else if (K === 0 && ce === u) pe.push(h[`${w + 1},${v},${B}`]);
                  else if (K === E && ce === 0) pe.push(h[`${w},${v + 1},${B}`]);
                  else if (K === E && ce === u) pe.push(h[`${w + 1},${v + 1},${B}`]);
                  else {
                    const _ = R(Ie, Pe, x), ue = A.get(_);
              ...
              for (let K = 0; K < E; K++) for (let pe = 0; pe < u; pe++) {
                const ce = Q[K][pe], Ie = Q[K][pe + 1], Pe = Q[K + 1][pe + 1], _ = Q[K + 1][pe], ue = L.length;
                zt.add(ue), $e.set(ue, B - 1), L.push([
                  ce,
                  Ie,
                  Pe,
                  _
        ...
        if (Ft && gt) {
          const u = gt === "all" || gt === "x" || gt === "perimeter", E = gt === "all" || gt === "y" || gt === "perimeter", A = b.length - 1;
          for (let R = 0; R < A; R++) {
            if (u) for (let B = 0; B < d.length; B++) {
              if (gt === "perimeter" && B !== 0 && B !== d.length - 1) continue;
              const x = Math.floor((i.length - 1) / 2);
              for (let w = 0; w < i.length - 1; w++) {
                if (gt === "perimeter" && w !== x || y(w, B) || y(w + 1, B)) continue;
                const v = h[`${w},${B},${R}`], F = h[`${w + 1},${B},${R + 1}`], U = h[`${w + 1},${B},${R}`], V = h[`${w},${B},${R + 1}`];
                v !== void 0 && F !== void 0 && (L.push([
                  v,
                  F
                ]), $e.set(L.length - 1, R)), U !== void 0 && V !== void 0 && (L.push([
                  U,
                  V
                ]), $e.set(L.length - 1, R));
              }
            }
            if (E) for (let B = 0; B < i.length; B++) {
              if (gt === "perimeter" && B !== 0 && B !== i.length - 1) continue;
              const x = Math.floor((d.length - 1) / 2);
              for (let w = 0; w < d.length - 1; w++) {
                if (gt === "perimeter" && w !== x || y(B, w) || y(B, w + 1)) continue;
                const v = h[`${B},${w},${R}`], F = h[`${B},${w + 1},${R + 1}`], U = h[`${B},${w + 1},${R}`], V = h[`${B},${w},${R + 1}`];
                v !== void 0 && F !== void 0 && (L.push([
                  v,
                  F
                ]), $e.set(L.length - 1, R)), U !== void 0 && V !== void 0 && (L.push([
      ...
      plateQ4(t = 10, n = 10, o = 16, l = 16, s = "simply-supported", c = -10, a = 0.2, r = 3e7, f = 0.3, i = 0) {
        ...
        S.nodeResults.forEach((z, O) => {
          (z.x < 1e-10 || z.x > t - 1e-10 || z.y < 1e-10 || z.y > n - 1e-10) && I.set(O, [
            true,
            true,
            true,
            true,
...
Util:     cad.info()  cad.clear()  cad.help()
      ...
      units(t, n) {
        t && (g = t), n && (P = n), M = Fn(g, P);
        const o = Le.querySelector("#cad3d-force-unit"), l = Le.querySelector("#cad3d-length-unit");
        return o && (o.textContent = g), l && (l.textContent = P), T && et(T), console.log(`Unidades: ${M.label} | E=${M.E.toExponential(3)} ${M.stress}`), M.id;
      },
      view(t) {
      },
      get mesh() {
    ...
    function Re() {
      ...
      if (![
        ...
        if (X.size > 0 || Y.size > 0 || j) {
          const n = e.elements.val, o = n.filter((l, s) => !(X.has(s) || Y.has(s) || j && !W.has(s)));
    ...
    function hs() {
      ...
      for (const x of i) for (const w of d) {
        ...
        for (let U = 0; U < p.length; U++) {
          const V = Math.hypot(p[U][0] - x, p[U][1] - w);
          V < v && (v = V, F = U);
        }
    ...
    function xs() {
      ...
      const L = a * r / Math.max(I.length, 1), z = /* @__PURE__ */ new Map();
      ...
      try {
        const m = kt(S, d, k, O), u = gn(S, d, O, m), E = S.map((R) => [
          R[0],
          0,
          R[1]
        ]);
        ...
        m && m.deformations && m.deformations.forEach((R) => {
          const B = Math.sqrt(R[0] * R[0] + R[1] * R[1] + R[2] * R[2]);
          A = Math.max(A, B);
        }), console.log(`Viga Alta: ${S.length} nodos, ${d.length} triangulos`), console.log(`  L=${t}, H=${n}, t=${o}, E=${l}, nu=${s}`), console.log(`  Carga: q=${a} kN/m sobre ${r}m central`), console.log(`  max|u| = ${A.toExponential(4)}`);
      } catch (m) {
    ...
    function vs() {
      const t = le("H") || 4, n = le("B") || 3, o = le("tw") || 0.3, l = le("tb") || 0.4, s = le("meshSize") || 0.2, c = le("E") || 25e6, a = le("nu") || 0.2, r = c / (2 * (1 + a)), f = le("gamma") || 18, i = le("Ka") || 0.33, d = le("Es") || 5e4, b = le("nus") || 0.3, S = d / (2 * (1 + b)), $ = le("kn") || 1e6, y = le("ks") || 1e4, p = le("gammaW") || 9.81, h = le("Hw") || 3.5, I = le("qs") || 0, L = xt, z = n * 0.3, O = n * 0.7, k = [
        [
          -z,
          0,
          0
      ...
      if (L === 0) {
        ...
        for (let F = 0; F < m.length; F++) {
          ...
          Math.abs(U - o) < s * 0.6 && V >= l - 1e-6 && v.push({
            idx: F,
            y: V
          });
        }
        ...
        if (L === 2) {
          ...
          ], U = Math.max(3, Math.ceil((v - l) / s)), V = [];
          ...
          for (let ae = 0; ae < m.length; ae++) {
            const Se = m[ae][0], je = m[ae][1];
            Math.abs(Se - o) < Q && je >= l - Q && K.push(ae);
          }
          K.sort((ae, Se) => m[ae][1] - m[Se][1]);
          const pe = [
            K[0]
          ];
          for (let ae = 1; ae < K.length; ae++) {
            const Se = m[K[ae]][1] - m[pe[pe.length - 1]][1];
            Math.abs(Se) > s * 0.05 && pe.push(K[ae]);
          }
          ...
          for (let ae = 0; ae < Ie; ae++) {
            const Se = u[ae], je = (m[Se[0]][0] + m[Se[1]][0] + m[Se[2]][0]) / 3, rt = (m[Se[0]][1] + m[Se[1]][1] + m[Se[2]][1]) / 3, ct = je >= -z && je <= O && rt >= 0 && rt <= l, Ct = je >= 0 && je <= o && rt >= l && rt <= l + t, _t = ct || Ct;
          ...
          for (let ae = 0; ae < K.length - 1; ae++) {
            const Se = K[ae], je = K[ae + 1], rt = ce.get(Se), ct = ce.get(je);
            u.push([
              je,
              Se,
              rt,
          ...
          for (let ae = 0; ae < Ie; ae++) Pe[ae] ? (oe.set(ae, d), fe.set(ae, d), me.set(ae, b), we.set(ae, S), ee.set(ae, 1)) : (oe.set(ae, c), fe.set(ae, c), me.set(ae, a), we.set(ae, r), ee.set(ae, 1));
          for (let ae = _; ae < u.length; ae++) oe.set(ae, $), fe.set(ae, 0), me.set(ae, 0), we.set(ae, y), ee.set(ae, 0);
          R = {
            elasticities: oe,
          ...
          for (let ae = 0; ae < Ie; ae++) {
            if (!Pe[ae]) continue;
            const Se = u[ae], je = m[Se[0]], rt = m[Se[1]], ct = m[Se[2]], Ct = Math.abs((rt[0] - je[0]) * (ct[1] - je[1]) - (ct[0] - je[0]) * (rt[1] - je[1])) / 2, _t = -f * Ct / 3;
            for (const Rt of Se) {
              const Dt = A.get(Rt) || [
                0,
                0,
          ...
          if (I > 0) {
            ...
            for (let Se = 0; Se < m.length; Se++) {
              ...
              Math.abs(rt - v) < s * 0.1 && je > o - 1e-6 && ae.push({
                idx: Se,
                x: je
              });
            }
            ae.sort((Se, je) => Se.x - je.x);
            for (let Se = 0; Se < ae.length; Se++) {
              let je = s;
              Se > 0 && Se < ae.length - 1 ? je = (ae[Se + 1].x - ae[Se - 1].x) / 2 : Se === 0 && ae.length > 1 ? je = (ae[1].x - ae[0].x) / 2 : Se === ae.length - 1 && ae.length > 1 && (je = (ae[Se].x - ae[Se - 1].x) / 2);
              const rt = -I * je, ct = A.get(ae[Se].idx) || [
                0,
                0,
                0,
                0,
          ...
          const se = (_) => {
            const ue = (m[_[0]][0] + m[_[1]][0] + m[_[2]][0]) / 3, oe = (m[_[0]][1] + m[_[1]][1] + m[_[2]][1]) / 3, fe = ue >= -z && ue <= O && oe >= 0 && oe <= l, ee = ue >= 0 && ue <= o && oe >= l && oe <= l + t;
            return fe || ee;
          ...
          if (I > 0) {
            ...
            for (let ue = 0; ue < _.length; ue++) {
              ...
              const fe = -I * oe, ee = A.get(_[ue].idx) || [
                0,
                0,
                0,
                0,
      ...
      if (L === 3) {
        ...
        for (let se = 0; se < m.length; se++) {
          const Q = m[se][0], K = m[se][1];
          Math.abs(Q - o) < s * 0.6 && K >= l - 1e-6 && V.push({
            idx: se,
            y: K
          });
        }
        V.sort((se, Q) => se.y - Q.y);
        for (let se = 0; se < V.length; se++) {
          const { idx: Q, y: K } = V[se], pe = Math.max(0, v - K);
          if (pe <= 0 || K < U - 1e-6) continue;
          const ce = Math.min(pe, F), Ie = p * ce;
          let Pe = s;
          se > 0 && se < V.length - 1 ? Pe = (V[se + 1].y - V[se - 1].y) / 2 : se === 0 && V.length > 1 ? Pe = (V[1].y - V[0].y) / 2 : se === V.length - 1 && V.length > 1 && (Pe = (V[se].y - V[se - 1].y) / 2);
          const _ = Ie * Pe;
        ...
        R = {
          elasticities: new Map(u.map((se, Q) => [
            Q,
            c
          ])),
          elasticitiesOrthogonal: new Map(u.map((se, Q) => [
            Q,
            c
          ])),
          thicknesses: new Map(u.map((se, Q) => [
            Q,
            o
          ])),
          poissonsRatios: new Map(u.map((se, Q) => [
            Q,
            a
          ])),
          shearModuli: new Map(u.map((se, Q) => [
            Q,
            r
          ]))
        };
      ...
      try {
        const w = kt(m, u, B, R), v = u.filter((pe) => pe.length === 3), F = {};
        for (const pe of Object.keys(R)) {
          const ce = R[pe];
        ...
        E.forEach((pe, ce) => se.set(ce, pe));
        const Q = /* @__PURE__ */ new Map();
        A.forEach((pe, ce) => Q.set(ce, [
          pe[0],
          pe[2],
          pe[1],
          pe[3],
        ...
        w && w.deformations && w.deformations.forEach((pe) => {
          ...
          K = Math.max(K, ce);
        }), console.log(`Muro Contencion [${x[L]}]: ${m.length} nodos, ${u.length} triangulos`), console.log(`  H=${t}, B=${n}, tw=${o}, tb=${l}, Ka=${i}, gamma=${f}, qs=${I}`), L === 1 && console.log(`  Es=${d}, nus=${b}`), L === 2 && console.log(`  Es=${d}, nus=${b}, kn=${$}, ks=${y}`), L === 3 && console.log(`  gammaW=${p}, Hw=${h}`), console.log(`  max|u| = ${K.toExponential(4)}`);
      } catch (w) {
    ...
    function ys() {
      const t = le("Lx") || 2, n = le("Ly") || 2, o = le("t") || 0.5, l = le("colA") || 0.4, s = le("colH") || 1.5, c = Math.round(le("nx") || 8), a = Math.round(le("ny") || 8), r = le("E") || 25e6, f = le("nu") || 0.2, i = le("P") || -500, d = le("Mx") || 0, b = le("My") || 0, S = le("ks") || 2e4, $ = t / c, y = n / a, p = t / 2, h = n / 2, I = l / 2, L = [];
      for (let E = 0; E <= a; E++) for (let A = 0; A <= c; A++) {
        const R = E * (c + 1) + A;
        let B = $, x = y;
        (A === 0 || A === c) && (B = $ / 2), (E === 0 || E === a) && (x = y / 2), L.push({
          node: R,
          dof: 0,
          k: S * B * x
        });
      ...
      let z = 0;
      for (let E = 0; E <= a; E++) for (let A = 0; A <= c; A++) Math.abs(A * $ - p) <= I + 1e-6 && Math.abs(E * y - h) <= I + 1e-6 && z++;
      const O = i / Math.max(z, 1), k = [];
      for (let E = 0; E <= a; E++) for (let A = 0; A <= c; A++) {
        const R = A * $, B = E * y;
        Math.abs(R - p) <= I + 1e-6 && Math.abs(B - h) <= I + 1e-6 && k.push({
          node: E * (c + 1) + A,
          dof: 0,
          value: O
        });
      ...
      if (Math.abs(d) > 1e-6) {
        ...
        for (let R = 0; R <= a; R++) for (let B = 0; B <= c; B++) {
          const x = B * $, w = R * y;
      ...
      try {
        ...
        const B = E.elementResults.map((U) => [
          ...U.nodes
        ]);
        B.push([
          R,
          ...
          R + 3,
          R
        ]), e.nodes.val = A, e.elements.val = B;
        ...
        E.nodeResults.forEach((U, V) => {
          const se = U.x, Q = U.y;
          (se < 1e-6 || se > t - 1e-6 || Q < 1e-6 || Q > n - 1e-6) && w.set(V, [
            false,
            false,
            true,
            false,
        ...
        if (v.set(R + 4, [
          ...
          const U = E.elementResults.length, V = /* @__PURE__ */ new Map(), se = /* @__PURE__ */ new Map(), Q = /* @__PURE__ */ new Map();
    ...
    function $s() {
      ...
      for (let x = 0; x < m.length; x++) {
        ...
        for (const [F, U] of I) {
          const V = Math.sqrt((w - F) * (w - F) + (v - U) * (v - U));
    ...
    function Ms() {
      const t = le("colB") || 0.3, n = le("colH") || 0.3, o = le("colT") || 8e-3, l = le("colLen") || 1.5, s = le("Lx") || 0.45, c = le("Ly") || 0.45, a = le("tPlaca") || 0.025, r = le("dBolt") || 0.022, f = le("sx") || 0.32, i = le("sy") || 0.32, d = Math.round(le("nSubColV") || 6), b = Math.round(le("nSubColH") || 4), S = Math.round(le("nSubPlaca") || 10), $ = le("E") || 2e8, y = le("nu") || 0.3, p = $ / (2 * (1 + y)), h = le("P") || -300, I = s / 2, L = c / 2, z = r / 2, O = t / 2, k = n / 2, m = [], u = [], E = S, A = s / E, R = c / E, B = (fe, ee) => ee * (E + 1) + fe;
      ...
      for (const [fe, ee] of K) {
        const [me, we] = se[fe], [ae, Se] = se[ee], je = [];
        for (let rt = 0; rt <= U; rt++) {
          const ct = [], Ct = rt / U * l;
          for (let _t = 0; _t <= V; _t++) {
            const Rt = _t / V, Dt = me + Rt * (ae - me), Sn = we + Rt * (Se - we);
            if (rt === 0) {
              const Vt = pe(Dt, Sn);
              if (Vt >= 0) {
                ct.push(Vt);
        ...
        for (let rt = 0; rt < U; rt++) for (let ct = 0; ct < V; ct++) u.push([
          je[rt][ct],
          je[rt][ct + 1],
          je[rt + 1][ct + 1],
          je[rt + 1][ct]
      ...
      for (let fe = 0; fe < (E + 1) * (E + 1); fe++) {
        ...
        for (const [we, ae] of x) {
          const Se = Math.sqrt((ee - we) * (ee - we) + (me - ae) * (me - ae));
      ...
      try {
        ...
        (fe == null ? void 0 : fe.deformations) && fe.deformations.forEach((we) => {
          const ae = Math.sqrt(we[0] * we[0] + we[1] * we[1] + we[2] * we[2]);
          me = Math.max(me, ae);
    ...
    function ws() {
      ...
      try {
        ...
        console.log(`Talud SRM: ${I.length} nodos, ${h.length} triangulos`), console.log(`  H=${t}, angulo=${n}\xB0, c=${f} kPa, \u03C6=${i}\xB0, \u03B3=${r}`), console.log("  \u2550\u2550\u2550 Strength Reduction Method (Mohr-Coulomb) \u2550\u2550\u2550"), console.log(`  FOS = ${u.fos.toFixed(3)}`), console.log(`  max|u| = ${B.toExponential(4)}`), console.log(`  max \u03B5_pl = ${x.toExponential(4)}`), u.fos < 1 ? console.warn("  \u26A0 TALUD INESTABLE (FOS < 1.0)") : u.fos < 1.5 && console.warn("  \u26A0 FOS < 1.5 \u2014 revisar estabilidad");
      } catch (k) {
    ...
    function On() {
      ...
      if (At.addBinding(i, "vigaMat", {
        ...
        At.addBinding(m, "vigaType", {
          ...
          Ne.steelVigaType = u.value, On(), Re();
        });
      }
      ...
      for (let m = 0; m < l; m++) {
        ...
        if (Ne.colMat === 0) if (Ne.colShape === 1) {
          ...
          const R = +Tn(1e8).toFixed(0), B = +Tn(3e8).toFixed(0), x = Math.max(1, Math.round((B - R) / 200));
          E.addBinding(A, "Es", {
            min: R,
            max: B,
            step: x,
      ...
      const L = re.length || 1, z = te.length || 1, O = L + 1, k = z + 1;
      if (L > 0) {
        const m = h.addFolder({
        ...
        for (let u = 0; u < L; u++) for (let E = 0; E < k; E++) {
          const A = `wx_${u}_${E}`, R = ve.some((w) => w.dir === "x" && w.bay === u && w.axisIdx === E), B = {};
          B[A] = R;
      ...
      if (z > 0) {
        ...
        for (let u = 0; u < z; u++) for (let E = 0; E < O; E++) {
          const A = `wy_${u}_${E}`, R = ve.some((w) => w.dir === "y" && w.bay === u && w.axisIdx === E), B = {};
          B[A] = R;
...
` + p;
  ...
  `;
    ...
    function Ea() {
      var _a2, _b, _c, _d, _e2, _f;
      const t = e.nodes.val, n = e.elements.val, o = (_a2 = e.nodeInputs) == null ? void 0 : _a2.val, l = (_b = e.elementInputs) == null ? void 0 : _b.val, s = P, c = g, a = [];
...
`);
    ...
    function Qn() {
      ...
      b.scene.traverse((z) => {
        var _a3;
        !S && z.isPoints && z.geometry && (S = z), !$ && z.isLineSegments && z.geometry && !z.name && ($ = z), z.isMesh && z.geometry && z.geometry.getAttribute("position") && !((_a3 = z.name) == null ? void 0 : _a3.includes("grid")) && y.push(z);
      });
      ...
      function L() {
        const z = (performance.now() - l) / 1e3, O = Math.sin(2 * Math.PI * o * z) * d;
        for (let k = 0; k < s; k++) {
          const m = ke[k];
          p[k * 3] = m[0] + (t[k * 6] || 0) * O, p[k * 3 + 1] = m[1] + (t[k * 6 + 1] || 0) * O, p[k * 3 + 2] = m[2] + (t[k * 6 + 2] || 0) * O;
        }
        if (S) {
          const k = S.geometry, m = k.getAttribute("position");
          m && m.array.length === p.length ? (m.array.set(p), m.needsUpdate = true) : k.setAttribute("position", new Gn(p.slice(), 3));
        }
        if ($) {
          for (let u = 0; u < h.length; u++) {
            const [E, A] = h[u];
            I[u * 6] = p[E * 3], I[u * 6 + 1] = p[E * 3 + 1], I[u * 6 + 2] = p[E * 3 + 2], I[u * 6 + 3] = p[A * 3], I[u * 6 + 4] = p[A * 3 + 1], I[u * 6 + 5] = p[A * 3 + 2];
          }
          const k = $.geometry, m = k.getAttribute("position");
          m && m.array.length === I.length ? (m.array.set(I), m.needsUpdate = true) : k.setAttribute("position", new Gn(I.slice(), 3));
        }
        if (y.length > 0) {
          const k = [];
          for (const m of c) if (m.length === 3) {
            const [u, E, A] = m;
            k.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), k.push(p[E * 3], p[E * 3 + 1], p[E * 3 + 2]), k.push(p[A * 3], p[A * 3 + 1], p[A * 3 + 2]);
          } else if (m.length === 4) {
            const [u, E, A, R] = m;
            k.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), k.push(p[E * 3], p[E * 3 + 1], p[E * 3 + 2]), k.push(p[A * 3], p[A * 3 + 1], p[A * 3 + 2]), k.push(p[u * 3], p[u * 3 + 1], p[u * 3 + 2]), k.push(p[A * 3], p[A * 3 + 1], p[A * 3 + 2]), k.push(p[R * 3], p[R * 3 + 1], p[R * 3 + 2]);
          }
          if (k.length > 0) {
            const m = new Float32Array(k);
            for (const u of y) {
              const A = u.geometry.getAttribute("position");
              if (A) if (A.array.length === m.length) A.array.set(m), A.needsUpdate = true;
              else {
                const R = A.array;
                R.length / 3;
                let B = 0;
                for (const x of c) if (x.length === 3) for (const w of x) B * 3 + 2 < R.length && (R[B * 3] = p[w * 3], R[B * 3 + 1] = p[w * 3 + 1], R[B * 3 + 2] = p[w * 3 + 2], B++);
                else if (x.length === 4) {
                  const [w, v, F, U] = x, V = [
                    w,
                    v,
                  ...
                  for (const se of V) B * 3 + 2 < R.length && (R[B * 3] = p[se * 3], R[B * 3 + 1] = p[se * 3 + 1], R[B * 3 + 2] = p[se * 3 + 2], B++);
                }
                A.needsUpdate = true;
              }
            }
    ...
    function Fo() {
      ...
      if (!o.loads || o.loads.size === 0) {
        const y = le("CM") ?? 0, p = le("CV") ?? 0, h = y + p, I = le("Ex") ?? 0, L = le("Ey") ?? 0;
        if (h === 0 && I === 0 && L === 0) return;
        const z = /* @__PURE__ */ new Map(), O = [];
        for (let w = 0; w < t.length; w++) o.supports.has(w) || O.push(w);
          ...
          F.forEach((U) => {
            ...
            if (L !== 0) {
              ...
              for (const se of U) if (k(t[se][1]) === v) {
                const Q = k(t[se][0]);
                V.has(Q) || (V.add(Q), A.add(se));
              }
            }
          });
        }
        const R = 9.81, B = /* @__PURE__ */ new Map();
        for (let w = 0; w < n.length; w++) {
          const v = n[w], F = ((_a2 = l.densities) == null ? void 0 : _a2.get(w)) ?? 0;
          if (!(Math.abs(F) < 1e-15)) {
            if (v.length === 2) {
              const U = ((_b = l.areas) == null ? void 0 : _b.get(w)) ?? 0, V = t[v[0]], se = t[v[1]], Q = Math.sqrt((se[0] - V[0]) ** 2 + (se[1] - V[1]) ** 2 + (se[2] - V[2]) ** 2), pe = -(F * U * Q * R) / 2;
              B.set(v[0], (B.get(v[0]) ?? 0) + pe), B.set(v[1], (B.get(v[1]) ?? 0) + pe);
            } else if (v.length >= 3) {
              ...
              for (const K of v) B.set(K, (B.get(K) ?? 0) + Q);
            }
          }
        }
        ...
        for (const w of O) {
          const v = E.has(w) ? I : 0, F = A.has(w) ? L : 0, U = B.get(w) ?? 0, V = x.has(w) ? h : 0, se = U + V;
          (v !== 0 || F !== 0 || Math.abs(se) > 1e-10) && z.set(w, [
            v,
            F,
            se,
            0,
      ...
      try {
        ...
        if (y) {
          ...
          const O = performance.now(), k = gn(t, n, l, y), m = performance.now() - O;
    ...
    function Ia(t, n, o, l, s, c, a) {
      const f = 4700 * Math.sqrt(c / 1e3) * 1e3 / l, i = t - 2 * o, d = n - 2 * o, b = t * n - i * d, S = (t * n * n * n - i * d * d * d) / 12, $ = (n * t * t * t - d * i * i * i) / 12, y = i * d, p = i * d * d * d / 12, h = d * i * i * i / 12, I = b + f * y, L = S + f * p, z = $ + f * h, O = l / (2 * (1 + s)), k = (t - o) * (n - o), m = 2 * ((t - o) / o + (n - o) / o), u = 4 * k * k / (m > 0 ? m : 1);
      return {
        A: I,
        Iz: L,
        Iy: z,
      ...
      if ((T === "edificio" || T === "frame") && D.size > 0) {
        ...
        for (let p = 0; p < t.length; p++) {
          if (de.has(p)) {
            ...
            o.elasticities.set(p, E), o.poissonsRatios.set(p, A), o.thicknesses.set(p, Be), o.shearModuli.set(p, E / (2 * (1 + A))), o.densities.set(p, 24 / 9.80665);
            continue;
          }
          if (zt.has(p)) {
            const E = 4700 * Math.sqrt(r / 1e3) * 1e3, A = 0.2;
            o.elasticities.set(p, E), o.poissonsRatios.set(p, A), o.thicknesses.set(p, at), o.shearModuli.set(p, E / (2 * (1 + A))), o.densities.set(p, 24 / 9.80665);
            continue;
          }
          const h = D.has(p), I = $e.get(p) ?? 0, L = f[I] ?? f[0] ?? {
            bCol: 0.4,
            hCol: 0.4,
            dCol: 0.4
          };
          let z, O, k, m;
          if (h) if (s === 0) O = i, k = d, m = b, z = a === 1 ? ks(L.dCol) : Ro(L.bCol, L.hCol), o.sectionShapes.set(p, a === 1 ? {
            type: "circ",
            d: L.dCol
          } : {
            type: "rect",
            ...
            if (E <= 1) {
              const A = kn[L.colProfileIdx] ?? kn[0];
              z = {
                A: A.A,
                Iz: A.Iz,
                Iy: A.Iy,
              ...
              const A = L.colBf ?? 0.3, R = L.colHf ?? 0.3, B = L.colTf ?? 0.02, x = L.colTw ?? 0.012;
              z = Po(A, R, B, x);
              ...
              const A = L.colBc ?? 0.3, R = L.colHc ?? 0.3, B = L.colT ?? 0.01;
              z = Oo(A, R, B);
              const x = `\u25A1${(R * 100).toFixed(0)}x${(A * 100).toFixed(0)}x${(B * 1e3).toFixed(0)}`;
              o.sectionShapes.set(p, {
                type: "HSS",
                b: A,
                h: R,
            ...
            const E = L.colBc ?? 0.3, A = L.colHc ?? 0.3, R = L.colT ?? 0.01, B = L.colFc ?? 28e3, x = L.colEs ?? 2e8, w = L.colNuS ?? 0.3;
            L.colNuC;
            const v = Ia(E, A, R, x, w, B);
            z = {
              A: v.A,
              Iz: v.Iz,
              Iy: v.Iy,
            ...
            const E = Te.get(p), A = E ? E.dir === "x" ? L.vigasX : L.vigasY : [], R = E ? A[E.bay] ?? A[0] ?? ut() : ut();
            if (c === 0) O = i, k = d, m = b, z = Ro(R.b, R.h), o.sectionShapes.set(p, {
              type: "rect",
              b: R.b,
              h: R.h
            });
              ...
              O = S, k = $, m = y;
              const B = Ne.steelVigaType;
              if (B <= 1) {
                const x = kn[R.profileIdx ?? 0] ?? kn[0];
                z = {
                  A: x.A,
                  Iz: x.Iz,
                  Iy: x.Iy,
          ...
          const u = ge.get(p);
          if (u) {
            if ((u.material ?? 1) === 0 ? (O = i, k = d, m = b) : (O = S, k = $, m = y), u.secType === "rect" && u.b && u.h) z = Ro(u.b, u.h), o.sectionShapes.set(p, {
              type: "rect",
              b: u.b,
              h: u.h
            });
          ...
          o.elasticities.set(p, O), o.shearModuli.set(p, k), o.areas.set(p, z.A), o.momentsOfInertiaZ.set(p, z.Iy), o.momentsOfInertiaY.set(p, z.Iz), o.torsionalConstants.set(p, z.J), o.densities.set(p, m), u && u.releases12 && u.releases12.some((E) => E) && (o.momentReleases || (o.momentReleases = /* @__PURE__ */ new Map()), o.momentReleases.set(p, u.releases12)), u && u.springs12 && u.springs12.some((E) => E > 0) && (o.partialFixitySprings || (o.partialFixitySprings = /* @__PURE__ */ new Map()), o.partialFixitySprings.set(p, u.springs12));
        }
    ...
    window.innerWidth <= 600 && Le.classList.add("collapsed"), setTimeout(() => {
      ...
      function n(x) {
        ...
        const F = 15e3 * Math.sqrt(210) * 10, U = 0.2, V = F / (2 * (1 + U)), se = 0.09, Q = 0.3 ** 4 / 12, K = 0.141 * 0.3 ** 4, pe = 0.25 * 0.4, ce = 0.25 * 0.4 ** 3 / 12, Ie = 0.4 * 0.25 ** 3 / 12, Pe = 1e-3, _ = 5 / 6 * se, ue = 5 / 6 * pe, oe = [];
        function fe(ee, me, we) {
          ...
          for (const Se of me) ae.elasticities.set(Se, F), ae.shearModuli.set(Se, V), ae.areas.set(Se, se), ae.momentsOfInertiaZ.set(Se, Q), ae.momentsOfInertiaY.set(Se, Q), ae.torsionalConstants.set(Se, K), ae.shearAreasY.set(Se, _), ae.shearAreasZ.set(Se, _);
          for (const Se of we) ae.elasticities.set(Se, F), ae.shearModuli.set(Se, V), ae.areas.set(Se, pe), ae.momentsOfInertiaZ.set(Se, Ie), ae.momentsOfInertiaY.set(Se, ce), ae.torsionalConstants.set(Se, Pe), ae.shearAreasY.set(Se, ue), ae.shearAreasZ.set(Se, ue);
          return ae;
        }
        if (x === "test-cantilever" || x === "test-all") {
          const we = 270 / (3 * F * Q), ae = [
            [
              0,
              0,
              0
          ...
          je.elasticities.set(0, F), je.shearModuli.set(0, V), je.areas.set(0, se), je.momentsOfInertiaZ.set(0, Q), je.momentsOfInertiaY.set(0, Q), je.torsionalConstants.set(0, K);
          const rt = kt(ae, Se, {
            supports: /* @__PURE__ */ new Map([
              [
                0,
        ...
        if (x === "test-scordelis" || x === "test-all") {
          ...
          for (const nt of Rt) {
            const St = _t[nt[0]], en = _t[nt[1]], Ot = _t[nt[2]], bn = _t[nt[3]], mn = [
              ...
              bn[1] - en[1],
              bn[2] - en[2]
            ], Us = mn[1] * Hn[2] - mn[2] * Hn[1], Xs = mn[2] * Hn[0] - mn[0] * Hn[2], Ks = mn[0] * Hn[1] - mn[1] * Hn[0], Ja = -90 * (0.5 * Math.sqrt(Us * Us + Xs * Xs + Ks * Ks)) / 4;
            for (const Zs of nt) {
              const Qs = Vt.get(Zs) || [
                0,
                0,
        ...
        if (c(oe), oe.length > 0) {
          ...
          const me = /* @__PURE__ */ new Map(), we = /* @__PURE__ */ new Map(), ae = Math.max(...ee.nodes.map((Se) => Se[2]));
          ee.nodes.forEach((Se, je) => {
            Math.abs(Se[2]) < 0.01 && me.set(je, [
              ...
              true,
              true
            ]), Math.abs(Se[2] - ae) < 0.01 && we.set(je, [
              10,
              0,
              0,
              0,
      ...
      function o(x) {
        ...
        const U = [
          ...F
        ].sort((_, ue) => _ - ue), V = U.map((_, ue) => ue === 0 ? "Base" : `Level_${ue}`), se = /* @__PURE__ */ new Map();
        U.forEach((_, ue) => se.set(_, V[ue])), v.push("$ STORIES - IN SEQUENCE FROM TOP");
        for (let _ = U.length - 1; _ >= 1; _--) v.push(`  STORY "${V[_]}"  HEIGHT ${U[_] - U[_ - 1]} MASTERSTORY "Yes"  `);
        ...
        const ce = (_) => {
          ...
          return {
            pt: K.get(oe) || "1",
            story: se.get(Math.round(ue[1] * 1e4) / 1e4) || "Base"
          };
        };
        ...
        if (x.elements.forEach((_, ue) => {
          ...
          const oe = x.nodes[_[0]], fe = x.nodes[_[1]], ee = Math.abs(fe[1] - oe[1]), me = Math.sqrt((fe[0] - oe[0]) ** 2 + (fe[2] - oe[2]) ** 2), we = ee > me * 0.5, ae = ce(_[0]), Se = ce(_[1]), je = we ? "COL30" : "VIGA";
          we ? (v.push(`  LINE  "E${ue + 1}"  COLUMN  "${ae.pt}"  "${ae.pt}"  1`), Ie.push(`  LINEASSIGN  "E${ue + 1}"  "${Se.story}"  SECTION "${je}"  `)) : (v.push(`  LINE  "E${ue + 1}"  BEAM  "${ae.pt}"  "${Se.pt}"  0`), Ie.push(`  LINEASSIGN  "E${ue + 1}"  "${ae.story}"  SECTION "${je}"  `));
        }), v.push(""), Q) {
      ...
      function l(x) {
        ...
        v.push(`"""ETABS API Validation: ${x.name}`), v.push('Generated by Awatif FEM Studio"""'), v.push("import comtypes.client, time, math"), v.push(""), v.push("helper = comtypes.client.CreateObject('ETABSv1.Helper')"), v.push("helper = helper.QueryInterface(comtypes.gen.ETABSv1.cHelper)"), v.push('myETABS = helper.CreateObjectProgID("CSI.ETABS.API.ETABSObject")'), v.push("myETABS.ApplicationStart()"), v.push("time.sleep(10)"), v.push("SapModel = myETABS.SapModel"), v.push("SapModel.InitializeNewModel()"), v.push("SapModel.File.NewBlank()"), v.push("SapModel.SetPresentUnits(12)  # tonf_m_C"), v.push(""), v.push(`E = ${w}`), v.push('SapModel.PropMaterial.SetMaterial("CONC", 2)'), v.push('SapModel.PropMaterial.SetMPIsotropic("CONC", E, 0.2, 5.5e-6)'), v.push('SapModel.PropFrame.SetRectangle("COL30", "CONC", 0.30, 0.30)'), v.push('SapModel.PropFrame.SetRectangle("VIGA", "CONC", 0.40, 0.25)'), x.elements.some((V) => V.length === 4) && v.push('SapModel.PropArea.SetWall("Muro20", 6, False, "CONC", 0.20)'), v.push(""), v.push("# Add elements"), v.push("FN = ' '"), x.elements.forEach((V, se) => {
          if (V.length === 2) {
            ...
            v.push(`[FN,r]=SapModel.FrameObj.AddByCoord(${Q[0]},${Q[2]},${Q[1]}, ${K[0]},${K[2]},${K[1]}, FN,"${Ie}","E${se + 1}","Global")`);
          } else if (V.length === 4) {
...
`);
      ...
      function s(x, w) {
        ...
        U.href = F, U.download = w, U.click(), URL.revokeObjectURL(F);
      }
      function c(x) {
        ...
        v += F ? '<div style="color:#4caf50;font-weight:bold;text-align:center;margin-top:8px">ALL TESTS PASSED (< 5% error vs ETABS)</div>' : '<div style="color:#f44336;font-weight:bold;text-align:center;margin-top:8px">Some tests exceeded 5% tolerance</div>', w.innerHTML = v, document.body.appendChild(w), window.__awatifDownloadE2k = (V) => {
          const se = window.__awatifTests[V], Q = se.name.replace(/[^a-zA-Z0-9]/g, "_").toLowerCase();
          s(o(se), `${Q}.e2k`);
        ...
        <div style="display:flex;gap:8px">
          ...
          for (const oe of w.elements) if (K.has(oe.category) && oe.type === "frame" && oe.nodeIds.length >= 2) {
            ...
            const fe = ((_a4 = w.materials) == null ? void 0 : _a4.get(oe.material)) || {
              ...
              nu: 0.2,
              rho: 2.4
            }, ee = oe.b || 0.3, me = oe.h || 0.3, we = ee * me, ae = ee * me * me * me / 12, Se = me * ee * ee * ee / 12, je = ee * me * (ee * ee + me * me) / 12, rt = fe.E / (2 * (1 + fe.nu));
            Ie.elasticities.set(_, fe.E), Ie.shearModuli.set(_, rt), Ie.areas.set(_, we), Ie.momentsOfInertiaZ.set(_, Se), Ie.momentsOfInertiaY.set(_, ae), Ie.torsionalConstants.set(_, je), Ie.densities.set(_, fe.rho), Ie.sectionShapes.set(_, {
              type: "rect",
              b: ee,
              h: me,
              name: oe.sectionName
      ...
      function b(x) {
        ...
        let U, V, se, Q;
        if (v.length > 0 || F.length > 0) U = v.map((ce) => ce.coord), V = F.map((ce) => ce.coord), se = v.map((ce) => ce.label), Q = F.map((ce) => ce.label);
        else {
          ...
          U = [
            ...
            ...Ie
          ].sort((Pe, _) => Pe - _), se = U.map((Pe, _) => String(_ + 1)), Q = V.map((Pe, _) => String.fromCharCode(65 + _));
        }
        const K = x.stories.length > 0 ? Math.max(...x.stories.map((ce) => ce.elev)) : Math.max(...x.nodes.map((ce) => ce[2]));
        We = U, Xe = V, pt = K, setTimeout(() => {
          wt(), Xn(U, V, K, se, Q), wo(x.stories, U, V), No(), qo();
        }, 100);
        const pe = {
          COLUMN: 0,
          BEAM: 0,
        ...
        console.log(`E2K grids: X=[${se.join(",")}] Y=[${Q.join(",")}]`), console.log(`E2K stories: ${x.stories.map((ce) => `${ce.name}@${ce.elev.toFixed(2)}`).join(", ")}`), console.log(`E2K elements: ${pe.COLUMN} columns, ${pe.BEAM} beams, ${pe.BRACE} braces`), Qe();
      }
      function S(x, w) {
        const v = new Blob([
          x
        ...
        if (a === "import-ifc") {
          ...
          v.onload = async () => {
            ...
            try {
              ...
              U.controls.target.copy(se), U.camera.position.set(se.x + K, se.y + K * 0.5, se.z + K), U.camera.lookAt(se), U.controls.maxDistance = K * 5, U.controls.update(), U.render(), window.__ifcLoadResult = V, window.__ifcArrayBuffer = F;
      ...
      $ && ($.value = g, $.addEventListener("change", (x) => {
        x.stopPropagation(), g = $.value, M = Fn(g, P), T && et(T);
      }));
      const y = Le.querySelector("#cad3d-length-unit");
      y && (y.value = P, y.addEventListener("change", (x) => {
        x.stopPropagation(), P = y.value, M = Fn(g, P), T && et(T);
      })), Le.querySelectorAll("[data-preset]").forEach((x) => {
        x.addEventListener("click", (w) => {
          w.stopPropagation();
          const v = x.dataset.preset, F = H[v];
          F && (g = F.force, P = F.length, J = F.stress, M = Fn(g, P), $ && ($.value = g), y && (y.value = P), Le.querySelectorAll("[data-preset]").forEach((U) => {
            U.classList.toggle("active", U.dataset.preset === v);
          }), T && et(T), console.log(`Preset: ${v} \u2192 ${g}+${P}, stress: ${J.label}`));
        });
        ...
        const v = Le.querySelector("#cad3d-mode-prev"), F = Le.querySelector("#cad3d-mode-next"), U = Le.querySelector("#cad3d-mode-label"), V = Le.querySelector("#cad3d-modal-scale");
        if (qe) {
          const se = Ze();
          ((_b2 = se == null ? void 0 : se.settings) == null ? void 0 : _b2.loads) && (_e = se.settings.loads.rawVal, se.settings.loads.val = false), Lo(), v.style.display = "", F.style.display = "", U.style.display = "", V && (V.style.display = ""), p();
        } else Co(), v.style.display = "none", F.style.display = "none", U.style.display = "none", V && (V.style.display = "none"), T && T !== "placa-q4" && T !== "placa-3q" && Re(), setTimeout(() => {
          var _a4;
          const se = Ze();
          ((_a4 = se == null ? void 0 : se.settings) == null ? void 0 : _a4.loads) && _e && (se.settings.loads.val = true);
        }, 600);
      });
      function p() {
        var _a3;
        ...
        for (let U = 0; U <= he; U++) {
          const V = (_a3 = Ae.massParticipation) == null ? void 0 : _a3[U];
          if (V) for (let se = 0; se < 6; se++) F[se] += V[se];
        }
      ...
      (_o2 = Le.querySelector("#cad3d-mode-prev")) == null ? void 0 : _o2.addEventListener("click", (x) => {
        ...
        for (let U = 0; U < ke.length; U++) {
          const V = w[U * 6] || 0, se = w[U * 6 + 1] || 0, Q = w[U * 6 + 2] || 0;
          F = Math.max(F, Math.sqrt(V * V + se * se + Q * Q));
        }
      ...
      const I = Le.querySelector("#cad3d-cli-toggle"), L = Le.querySelector("#cad3d-cli-panel"), z = Le.querySelector("#cad3d-cli-output"), O = Le.querySelector("#cad3d-cmd"), k = [];
      let m = -1;
...
`));
        ...
        if ((x.key === "Delete" || x.key === "Backspace") && ht.size > 0) {
          x.preventDefault(), ht.forEach((w) => X.add(w)), ht.clear(), fn && (fn.remove(), fn = null), Re();
          return;
        }
        if (x.key === "Escape") {
          if (pn) if (vt !== null) {
...
`);
      ...
      Le.addEventListener("mousedown", (x) => {
        ...
        Le.style.bottom = "unset", E = x.clientX, A = x.clientY, R = v.left, B = v.top, x.preventDefault();
      }), window.addEventListener("mousemove", (x) => {
        u && (x.preventDefault(), Le.style.left = R + (x.clientX - E) + "px", Le.style.top = B + (x.clientY - A) + "px");
    ...
    let nn = false, Yt = false, pn = false, jt = "line", Xt = [], vt = null, Nt = null, qt = null, Nn = null, Zt = null;
    const Et = {
      node: true,
      grid: true,
      midpoint: true,
    ...
    function _n() {
      ...
      }), qn.length > Ta && qn.shift(), lo.length = 0;
    }
    function As() {
    ...
    function Bo(t) {
      ...
      const c = new tn();
      c.setAttribute("position", new Gn(s, 3));
      const a = new Wn(c, new Yn({
        color: 16711935,
        linewidth: 3,
        depthTest: false
      }));
...
${i.join(", ")}`);
        ...
        j = true, W.clear(), ht.forEach((o) => W.add(o)), ht.clear(), wn(), En(), Re();
      }), n.querySelector("#sel-showall").addEventListener("click", () => {
        Y.clear(), j = false, W.clear(), En(), Re();
      }), n.querySelector("#sel-delete").addEventListener("click", () => {
        _n(), ht.forEach((o) => X.add(o)), ht.clear(), wn(), En(), Re();
    ...
    function io() {
      var _a2;
      const t = Ze();
      Nt && t && (t.scene.remove(Nt), Nt.geometry.dispose(), Nt.material.dispose(), Nt = null), qt && t && (t.scene.remove(qt), qt.geometry.dispose(), qt.material.dispose(), qt = null), Ho(), vt = null, Zt = null, pn = false, Nn && (Nn.remove(), Nn = null), (_a2 = Le.querySelector("#cad3d-draw")) == null ? void 0 : _a2.classList.remove("inspect-active"), t == null ? void 0 : t.render();
    }
    function La() {
      Nn && Nn.remove();
    ...
    `, document.body.appendChild(t), Nn = t;
      const l = () => {
    ...
    function Fs(t, n, o, l) {
      ...
      if (Et.node) {
        ...
        for (let p = 0; p < f.length; p++) {
          const h = f[p], I = new Oe(h[0], h[1], h[2]).project(o), L = Math.sqrt((I.x - c) ** 2 + (I.y - a) ** 2);
          L < d && L < y && (y = L, $ = p);
        }
        if ($ >= 0) return {
          nodeIdx: $,
      ...
      if (Et.midpoint) {
        ...
        for (const p of i) {
          ...
          const h = f[p[0]], I = f[p[1]], L = new Oe((h[0] + I[0]) / 2, (h[1] + I[1]) / 2, (h[2] + I[2]) / 2), z = L.clone().project(o), O = Math.sqrt((z.x - c) ** 2 + (z.y - a) ** 2);
          O < d * 0.8 && O < $ && ($ = O, y = L);
        }
        if (y) return {
          nodeIdx: null,
    ...
    function Rs(t) {
      ...
      if (Nt && (n.scene.remove(Nt), Nt.geometry.dispose(), Nt.material.dispose(), Nt = null), vt !== null && t.worldPos) {
        ...
        if (jt === "arc" && Zt !== null) {
          ...
          s.setAttribute("position", new Gn(f, 3));
        } else s.setAttribute("position", new Gn([
          l[0],
          l[1],
          l[2],
          t.worldPos.x,
        ...
        Nt = new Ln(s, c), jt === "arc" && Zt !== null && (Nt = new Wn(s, c)), Nt.renderOrder = 9999, n.scene.add(Nt);
      }
      n.render();
    }
    function Ps(t, n, o, l) {
    ...
    function Os(t) {
      ...
      const c = new tn();
      c.setAttribute("position", new Gn(s, 3)), Jt = new Wn(c, new Yn({
        color: 16776960,
        linewidth: 3,
        depthTest: false
      ...
      for (let b = 0; b < a.length; b++) {
        ...
        if (S.length === 2) {
          ...
          if (i.intersectTriangle($, y, p, false, h)) {
            ...
            const L = $.add(y).add(p).divideScalar(3), z = new Oe();
          ...
          let L = i.intersectTriangle($, y, p, false, I);
          if (L) {
            const z = i.origin.distanceTo(I);
            z < r && (r = z, f = b);
          }
          if (L = i.intersectTriangle($, p, h, false, I), L) {
            const z = i.origin.distanceTo(I);
            z < r && (r = z, f = b);
          }
        }
    ...
    function Ra(t) {
      if (t.length === 2) {
        const o = xn(t[1], t[0]), l = Rn(o), s = o[0] / l, c = o[1] / l, a = o[2] / l;
        return `<div class="fem-eq eq-box">
    ...
    function qs(t) {
      ...
      if (c) {
        const m = Rn(xn(s[1], s[0])), u = ((_d = a.elasticities) == null ? void 0 : _d.get(t)) ?? 0, E = ((_e2 = a.areas) == null ? void 0 : _e2.get(t)) ?? 0, A = ((_f = a.momentsOfInertiaZ) == null ? void 0 : _f.get(t)) ?? 0, R = ((_g = a.momentsOfInertiaY) == null ? void 0 : _g.get(t)) ?? 0, B = ((_h = a.shearModuli) == null ? void 0 : _h.get(t)) ?? 0, x = ((_i = a.torsionalConstants) == null ? void 0 : _i.get(t)) ?? 0, w = ((_j = a.momentReleases) == null ? void 0 : _j.get(t)) || [], v = ((_k = a.partialFixitySprings) == null ? void 0 : _k.get(t)) || [], F = [
          "P (Axial)",
          "V2 (Corte)",
          "V3 (Corte)",
          "T (Torsi\xF3n)",
        ...
        let U = "";
        for (let V = 0; V < 6; V++) {
          const se = V, Q = V + 6, K = (w.length >= 12 ? w[se] : V >= 3 && w.length >= 6 && w[V - 3]) ? "checked" : "", pe = (w.length >= 12 ? w[Q] : V >= 3 && w.length >= 6 && w[V]) ? "checked" : "", ce = v.length >= 12 && v[se] > 0 ? v[se].toFixed(1) : "", Ie = v.length >= 12 && v[Q] > 0 ? v[Q].toFixed(1) : "";
          U += `<tr>
          <td style="text-align:left;color:var(--fem-key)">${F[V]}</td>
          <td style="text-align:center"><input type="checkbox" data-rel="${se}" ${K}></td>
          <td style="text-align:center"><input type="checkbox" data-rel="${Q}" ${pe}></td>
          <td><input type="number" data-spr="${se}" value="${ce}" placeholder="0" style="width:50px;background:var(--fem-bg);color:var(--fem-val);border:1px solid var(--fem-border);font-size:10px;text-align:right"></td>
        ...
        `${l[0]}${l[1]}${Me(m)}${Me(u)}${Me(E)}${Me(A)}${Me(R)}${Me(B)}${Me(x)}${U}`;
      } else {
        const m = ((_l2 = a.elasticities) == null ? void 0 : _l2.get(t)) ?? 0, u = ((_m = a.thicknesses) == null ? void 0 : _m.get(t)) ?? 0, E = ((_n2 = a.poissonsRatios) == null ? void 0 : _n2.get(t)) ?? 0, A = m / (2 * (1 + E)), R = l.length === 4, B = m / (1 - E * E);
        `${l.length}${l.join(", ")}${Me(m)}${Me(A)}${Me(u)}${Me(E)}`, R && (S = `<div class="fem-eq eq-box">
          ...
          </span> = ${De(Me(m), "1\u2212" + Me(E) + "\xB2")} = <span class="highlight">${Me(B)}</span></div>
          ...
          <div>${C("D", "s")} = \u03BA\xB7${C("G")}\xB7${C("t")} = <span class="highlight">${Me(5 / 6 * A * u)}</span> &nbsp; <sub style="color:var(--fem-label)">\u03BA = 5/6</sub></div>
      ...
      let i = "", d = "", b = "", S = "", $ = "", y = "", p = "", h = "", I = null, L = null, z = null, O = [];
      try {
        if (I = vo(s, a, t), L = yo(s), z = on(os(L), on(I, L)), O = c ? [
          "ux\u2080",
          "uy\u2080",
          "uz\u2080",
          "\u03B8x\u2080",
          ...
          const A = Rn(xn(s[1], s[0])), R = ((_o2 = a.elasticities) == null ? void 0 : _o2.get(t)) ?? 0, B = ((_p = a.areas) == null ? void 0 : _p.get(t)) ?? 0, x = ((_q = a.momentsOfInertiaZ) == null ? void 0 : _q.get(t)) ?? 0, w = ((_r = a.momentsOfInertiaY) == null ? void 0 : _r.get(t)) ?? 0, v = ((_s2 = a.shearModuli) == null ? void 0 : _s2.get(t)) ?? 0, F = ((_t = a.torsionalConstants) == null ? void 0 : _t.get(t)) ?? 0;
          S = Fa(R, B, x, w, v, F, A);
        }
        ...
        i = `<div class="matrix-label">k_local (${I.length}\xD7${I.length}) ${m}</div>${Go(I, O)}`, d = `<div class="matrix-label">T \u2014 Transformaci\xF3n (${L.length}\xD7${L.length}) ${u}</div>${Go(L, O)}`, b = `<div class="matrix-label">K_global = T^T \xB7 k \xB7 T ${E}</div>${Go(z, O)}`;
      } catch (m) {
      ...
      if (r == null ? void 0 : r.deformations) {
        ...
        l.map((u, E) => {
          ...
          const A = ((_a3 = r.deformations) == null ? void 0 : _a3.get(u)) || [
            ...
            0,
            0
          ], R = m.map((B, x) => `<span class="prop-key">${B}</span>: <span class="${Math.abs(A[x]) > 1e-10 ? "result-val" : ""}">${Me(A[x])}</span>`).join(" &nbsp;");
          return `<div style="margin-bottom:2px"><strong>Nodo ${u}:</strong> ${R}</div>`;
        }).join("");
      }
      if (f && c && (r == null ? void 0 : r.deformations) && I && L) {
        const m = (_u = f.normals) == null ? void 0 : _u.get(t), u = (_v = f.shearsY) == null ? void 0 : _v.get(t), E = (_w = f.shearsZ) == null ? void 0 : _w.get(t), A = (_x = f.torsions) == null ? void 0 : _x.get(t), R = (_y = f.bendingsY) == null ? void 0 : _y.get(t), B = (_z = f.bendingsZ) == null ? void 0 : _z.get(t), x = [
          "ux",
          "uy",
          "uz",
          "\u03B8x",
        ...
        const U = (Q, K) => Q.map((pe, ce) => `<span style="color:${Math.abs(pe) > 1e-10 ? "var(--fem-nonzero)" : "var(--fem-eq-dots)"}">${K[ce % 6]}=${Me(pe)}</span>`).join(", "), se = [
          "N",
          "Vy",
          "Vz",
          "Mx",
        ...
        `${C("u", "global")}${l.map((Q, K) => `<span style="color:var(--fem-label)">nodo ${Q}:</span> ${x.map((pe, ce) => `<span style="color:${Math.abs(w[K * 6 + ce]) > 1e-10 ? "var(--fem-eq-var)" : "var(--fem-eq-dots)"}">${Me(w[K * 6 + ce])}</span>`).join(", ")}`).join(" | ")}${C("u", "local")}${C("T")}${C("u", "global")}${C("u", "local")}${U(v, [
          ...x,
          ...x
        ])}${C("f", "local")}${C("k", "local")}${C("u", "local")}${C("f", "local")}${F.map((Q, K) => `<span style="color:${Math.abs(Q) > 1e-10 ? "var(--fem-nonzero)" : "var(--fem-eq-dots)"}">${se[K]}=${Me(Q)}</span>`).join(", ")}${C("P", "1")}${C("N", "i")}${Me(F[0])}${C("P", "7")}${C("N", "j")}${Me(F[6])}${C("P", "2")}${C("V", "y,i")}${Me(F[1])}${C("P", "8")}${C("V", "y,j")}${Me(F[7])}${C("P", "3")}${C("V", "z,i")}${Me(F[2])}${C("P", "9")}${C("V", "z,j")}${Me(F[8])}${C("P", "4")}${C("M", "x,i")}${Me(F[3])}${C("P", "10")}${C("M", "x,j")}${Me(F[9])}${C("P", "5")}${C("M", "y,i")}${Me(F[4])}${C("P", "11")}${C("M", "y,j")}${Me(F[10])}${C("P", "6")}${C("M", "z,i")}${Me(F[5])}${C("P", "12")}${C("M", "z,j")}${Me(F[11])}${m ? m.map((Q) => Me(Q)).join(", ") : "\u2014"}${u ? u.map((Q) => Me(Q)).join(", ") : "\u2014"}${E ? E.map((Q) => Me(Q)).join(", ") : "\u2014"}${A ? A.map((Q) => Me(Q)).join(", ") : "\u2014"}${R ? R.map((Q) => Me(Q)).join(", ") : "\u2014"}${B ? B.map((Q) => Me(Q)).join(", ") : "\u2014"}`;
      } else if (f && c) {
        const m = (_B = f.normals) == null ? void 0 : _B.get(t), u = (_C = f.shearsY) == null ? void 0 : _C.get(t), E = (_D = f.shearsZ) == null ? void 0 : _D.get(t), A = (_E = f.torsions) == null ? void 0 : _E.get(t), R = (_F = f.bendingsY) == null ? void 0 : _F.get(t), B = (_G = f.bendingsZ) == null ? void 0 : _G.get(t);
        `${m ? m.map((x) => Me(x)).join(", ") : "\u2014"}${u ? u.map((x) => Me(x)).join(", ") : "\u2014"}${E ? E.map((x) => Me(x)).join(", ") : "\u2014"}${A ? A.map((x) => Me(x)).join(", ") : "\u2014"}${R ? R.map((x) => Me(x)).join(", ") : "\u2014"}${B ? B.map((x) => Me(x)).join(", ") : "\u2014"}`;
      } else if (f && !c) {
        const m = (_H = f.bendingXX) == null ? void 0 : _H.get(t), u = (_I = f.bendingYY) == null ? void 0 : _I.get(t), E = (_J = f.bendingXY) == null ? void 0 : _J.get(t), A = (_K = f.membraneXX) == null ? void 0 : _K.get(t), R = (_L = f.membraneYY) == null ? void 0 : _L.get(t), B = (_M = f.membraneXY) == null ? void 0 : _M.get(t);
        `${m ? m.map((x) => Me(x)).join(", ") : "\u2014"}${u ? u.map((x) => Me(x)).join(", ") : "\u2014"}${E ? E.map((x) => Me(x)).join(", ") : "\u2014"}${A ? A.map((x) => Me(x)).join(", ") : "\u2014"}${R ? R.map((x) => Me(x)).join(", ") : "\u2014"}${B ? B.map((x) => Me(x)).join(", ") : "\u2014"}`;
      }
      `${l[0]}`, 6 * l[0], 6 * l[0] + 5, `${l[1]}`, 6 * l[1], 6 * l[1] + 5, l.length === 3 && (`${l[2]}`, 6 * l[2], 6 * l[2] + 5), n.length * 6, n.length * 6, Kt = vl(t, n, o, a, r, f), Kt.id = "fem-inspect-panel", document.body.appendChild(Kt), (_N = Kt.querySelector("#er-close")) == null ? void 0 : _N.addEventListener("click", () => An()), (_O = Kt.querySelector("#rel-apply")) == null ? void 0 : _O.addEventListener("click", () => {
        ...
        m.forEach((B) => {
          ...
          x > 0 && (A[parseInt(B.dataset.spr)] = x);
        }), a.momentReleases || (a.momentReleases = /* @__PURE__ */ new Map()), a.partialFixitySprings || (a.partialFixitySprings = /* @__PURE__ */ new Map()), E.some((B) => B) ? a.momentReleases.set(t, E) : a.momentReleases.delete(t), A.some((B) => B > 0) ? a.partialFixitySprings.set(t, A) : a.partialFixitySprings.delete(t), console.log(`Releases elem ${t}:`, E.map((B, x) => B ? relIds[x] : "").filter(Boolean).join(" ") || "none"), console.log(`Springs elem ${t}:`, A);
      ...
      const k = c ? (() => {
        var _a3, _b2, _c2, _d2, _e3, _f2;
        const m = Rn(xn(s[1], s[0])), u = ((_a3 = a.elasticities) == null ? void 0 : _a3.get(t)) ?? 0, E = ((_b2 = a.areas) == null ? void 0 : _b2.get(t)) ?? 0, A = ((_c2 = a.momentsOfInertiaZ) == null ? void 0 : _c2.get(t)) ?? 0, R = ((_d2 = a.momentsOfInertiaY) == null ? void 0 : _d2.get(t)) ?? 0, B = ((_e3 = a.shearModuli) == null ? void 0 : _e3.get(t)) ?? 0, x = ((_f2 = a.torsionalConstants) == null ? void 0 : _f2.get(t)) ?? 0;
        return qa(u, E, A, R, B, x, m);
      })() : void 0;
      Kt.querySelectorAll("[data-full]").forEach((m) => {
        m.addEventListener("click", (u) => {
          ...
          if (E === "kLocal" && I) {
            ...
            Jo(`Elemento ${t} \u2014 Rigidez Local k_local`, A, Yo(I, O), k);
          } else if (E === "T" && L) Jo(`Elemento ${t} \u2014 Transformaci\xF3n T`, $, Yo(L, O));
          else if (E === "kGlobal" && z) {
            const A = c ? Ns() : "<em>Shell 18\xD718</em>";
            Jo(`Elemento ${t} \u2014 Rigidez Global K = T^T \xB7 k \xB7 T`, A, Yo(z, O), k);
          }
        });
      });
    }
    function _s() {
      const l = [], s = [];
      for (let y = 0; y <= 8; y++) {
        const p = y / 8, h = 30 * p, L = 12 * (1 - p) * (1 - p * 0.3) / 2, z = l.length;
        if (l.push([
          -L,
          -L,
          h
    ...
    function js() {
      ...
      for (let p = 0; p <= 20; p++) {
        ...
        for (let O = 0; O < 3; O++) {
          const k = O * 2 * Math.PI / 3 - Math.PI / 2, m = L * Math.cos(k), u = L * Math.sin(k), E = s.length;
          s.push([
            m,
            u,
            I
    ...
    function Ws() {
      ...
      for (let b = 0; b < 3; b++) {
        ...
        for (let L = 0; L <= 4; L++) {
          const z = L / 4, O = -p / 2 + p * z, k = y * (1 - z * z * 0.3);
          for (let m = 0; m <= 12; m++) {
    ...
    function Vo() {
      ...
      try {
        ...
        if (k && e.deformOutputs) {
          ...
          const u = c * h + Math.floor(s / 2), E = k.deformations.get(u), A = E ? E[0] : 0;
      ...
      const O = Ze();
      O && (O.settings.shellResults.val = "displacementX"), setTimeout(() => wt(), 50), Qe();
    }
    function Ys() {
      var _a2, _b, _c, _d, _e2, _f, _g, _h;
      ...
      for (let O = 0; O <= c; O++) for (let k = 0; k <= s; k++) S.push([
        k * d,
        O * b,
        0
      ]);
      const h = s + 1;
      for (let O = 0; O < c; O++) for (let k = 0; k < s; k++) $.push([
        O * h + k,
        O * h + k + 1,
        (O + 1) * h + k + 1,
        (O + 1) * h + k
      ...
      try {
        ...
        if (O && e.deformOutputs) {
          ...
          const m = O.deformations.get(I), u = m ? m[2] : 0, E = l * o * o * o / 12, A = f * n * n * n / (3 * a * E);
    ...
    function Js() {
      ...
      try {
        ...
        if (k && e.deformOutputs) {
          ...
          const m = gn(S, $, z, k);
          e.analyzeOutputs && (e.analyzeOutputs.val = m);
          const u = (c / 2 | 0) * h + s, E = k.deformations.get(u), A = E ? E[2] : 0;
      ...
      const O = Ze();
      O && (O.settings.shellResults.val = "displacementZ"), setTimeout(() => wt(), 50), Qe();
    }
    function _a() {
      var _a2, _b;
    ...
    `, document.body.appendChild(t), (_b = t.querySelector("#pushover-close")) == null ? void 0 : _b.addEventListener("click", () => t.remove()), (_c = t.querySelector("#pushover-run")) == null ? void 0 : _c.addEventListener("click", async () => {
        ...
        for (let h = 1; h <= b; h++) {
          const I = y * h / b;
          for (let L = 0; L <= p; L++) $.push(I * Math.sin(2 * Math.PI * L / p));
        }
    ...
    function Ba(t, n, o, l) {
      ...
      const y = b - d, p = $ - S, h = (O) => r.left + (O - d) / y * f, I = (O) => r.top + i - (O - S) / p * i;
      ...
      for (let O = 1; O < n.length; O++) s.lineTo(h(n[O]), I(o[O]));
    ...
    function ja(t) {
      ...
      for (let V = 0; V < a; V++) {
        ...
        for (let Q = 0; Q < r; Q++) {
          ...
          f.push(se * Math.sin(K));
        }
      }
      const i = n / o, d = l * o;
      let b = 0, S = 0, $ = -i, y = i, p = 0, h = 0, I = 0, L = 0, z = 0, O = 0;
      const k = [];
      for (const V of f) {
        let se = $, Q = y, K = p, pe = h, ce = I, Ie = L, Pe = z, _ = O, ue;
        const oe = V - b;
        ...
        if ((_ === 0 || _ === 3) && (oe < 0 ? (_ = 2, pe = -i, ce = -n, K = pe, Ie = 0, Pe = 0) : (_ = 1, pe = i, ce = n, K = pe, Ie = 0, Pe = 0)), _ === 2 && oe > 0) {
          _ = 1, Ie = b, Pe = S, b < se && (se = b);
          const Se = (Q - se) / (2 * 1 * i), je = 1 + 0 * Math.pow(Se, 0.8);
          pe = (n * je - d * i * je - Pe + o * Ie) / (o - d), ce = n * je + d * (pe - i * je), K = Q;
        } else if (_ === 1 && oe < 0) {
          _ = 2, Ie = b, Pe = S, b > Q && (Q = b);
          const Se = (Q - se) / (2 * 1 * i), je = 1 + 0 * Math.pow(Se, 0.8);
          pe = (-n * je + d * i * je - Pe + o * Ie) / (o - d), ce = -n * je + d * (pe + i * je), K = se;
        }
        const fe = Math.abs((K - pe) / i);
        let ee = s - 0.925 * fe / (0.15 + fe);
        ee < 0.1 && (ee = 0.1);
        const me = (V - Ie) / (pe - Ie), we = 1 + Math.pow(Math.abs(me), ee), ae = Math.pow(we, 1 / ee);
        ue = l * me + (1 - l) * me / ae, ue = ue * (ce - Pe) + Pe, k.push(ue), b = V, S = ue, $ = se, y = Q, p = K, h = pe, I = ce, L = Ie, z = Pe, O = _;
      }
      const m = t.querySelector("#nl-canvas"), u = m.getContext("2d"), E = m.width, A = m.height;
      u.clearRect(0, 0, E, A);
      const R = Math.max(...f.map(Math.abs)), B = Math.max(...k.map(Math.abs)), x = (E - 40) / (2 * R), w = (A - 40) / (2 * B), v = E / 2, F = A / 2;
      u.strokeStyle = "#444", u.lineWidth = 1, u.beginPath(), u.moveTo(20, F), u.lineTo(E - 20, F), u.stroke(), u.beginPath(), u.moveTo(v, 20), u.lineTo(v, A - 20), u.stroke(), u.fillStyle = "#888", u.font = "10px monospace", u.textAlign = "center", u.fillText("\u03B5 (strain)", E - 40, F - 5), u.fillText("\u03C3 (stress)", v + 30, 15), u.fillText(`\xB1${(R * 100).toFixed(1)}%`, E - 30, F + 12), u.fillText(`\xB1${(B / 1e3).toFixed(0)} MPa`, v + 40, 30), u.strokeStyle = "#00ccff", u.lineWidth = 1.5, u.beginPath();
      for (let V = 0; V < f.length; V++) {
        const se = v + f[V] * x, Q = F - k[V] * w;
        V === 0 ? u.moveTo(se, Q) : u.lineTo(se, Q);
      }
      ...
      U.textContent = `Steel02: Fy=${(n / 1e3).toFixed(0)} MPa, E\u2080=${(o / 1e6).toFixed(0)} GPa, b=${l}, R\u2080=${s} \u2014 ${a} ciclos, amp=${(c * 100).toFixed(1)}%`;
    }
    function Wa() {
      var _a2, _b, _c, _d;
    ...
    `, document.body.appendChild($), Bn = $, $.querySelector("#ep-close").addEventListener("click", () => {
        $.remove(), Bn = null, An();
      }), $.querySelector("#ep-delete").addEventListener("click", () => {
        X.add(t), $.remove(), Bn = null, An(), Re();
    ...
    setTimeout(() => {
      ...
      function a(f, i, d, b, S) {
        const $ = Math.min(f, d), y = Math.max(f, d), p = Math.min(i, b), h = Math.max(i, b), I = e.nodes.val, L = e.elements.val, z = [];
        for (let O = 0; O < L.length; O++) {
          const k = L[O], m = k.map((u) => c(I[u])).filter(Boolean);
          if (m.length !== 0) if (S) m.every((E) => E.x >= $ && E.x <= y && E.y >= p && E.y <= h) && z.push(O);
          else {
            ...
            if (k.length === 2) {
              const E = m[0], A = m[1];
              r(E.x, E.y, A.x, A.y, $, p, y, h) && z.push(O);
            }
          }
        }
        return z;
