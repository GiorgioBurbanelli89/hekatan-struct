...
export const edificioAporticado: ExampleDef = {
  ...
  params: {
    ...
    // ── Secciones ──
    matCol:   PE("Secciones", "Material columna", 0, { "Hormigón": 0, "Acero W": 1 }),
    matViga:  PE("Secciones", "Material viga",    0, { "Hormigón": 0, "Acero W": 1 }),
    fcConcr:  P("Secciones", "f'c hormigón (kg/cm²)", 240, 140, 420, 10),
    fyAcero:  P("Secciones", "fy acero (kg/cm²)", 2530, 1800, 4200, 100),
    ...
    vSecOn:   P("Avanzado", "Vigas secundarias (0/1)", 0, 0, 1, 1),
    nVSec:    P("Avanzado", "N° vigas sec. por vano", 2, 1, 5, 1),
    vSecDir:  P("Avanzado", "Dir secundarias (0=X,1=Y)", 0, 0, 1, 1),
    bracesMode: PE("Avanzado", "Diagonales", 0, { "ninguna": 0, "perimetrales": 1, "todas": 2, "solo X": 3, "solo Y": 4 }),
    slabOn:   P("Avanzado", "Losa (0/1)", 0, 0, 1, 1),
  ...
  build(p, states) {
    ...
    // Propiedades por tipo de elemento + material
    const colB = p.colSize, colH = p.colSize;
    const colA = colB * colH;
    const colIz = (colB * colH**3) / 12;
    const colIy = (colH * colB**3) / 12;
    const colJ  = 0.14 * Math.pow(Math.min(colB, colH), 4);
    const vB = p.vigaB, vH = p.vigaH;
    const vA = vB * vH;
    const vIz = (vB * vH**3) / 12;
    const vIy = (vH * vB**3) / 12;
    const vJ  = 0.21 * Math.pow(Math.min(vB, vH), 3) * Math.max(vB, vH);
