...
export function getLoadParams(u: UnitSystem) {
  ...
  return {
    ...
    "placa-xy": [],
    "pergola": [
      { key: "Lx",    val: 5.50,  min: 2, max: 20,  step: 0.5,   label: `Lx ancho (${u.length})` },
      { key: "Ly",    val: 8,     min: 2, max: 30,  step: 0.5,   label: `Ly largo (${u.length})` },
      { key: "H1",    val: 3.00,  min: 1, max: 8,   step: 0.25,  label: `H1 izq (${u.length})` },
      { key: "H2",    val: 4.00,  min: 1, max: 8,   step: 0.25,  label: `H2 der (${u.length})` },
      { key: "nCol",  val: 4,     min: 2, max: 8,   step: 1,     label: "N columnas" },
      { key: "nCorr", val: 8,     min: 3, max: 16,  step: 1,     label: "N correas" },
      { key: "E",     val: u.E,   min: 1e6, max: 1e12, step: 1e6, label: `E (${u.stress})` },
      { key: "t",     val: 0.0005, min: 0.0001, max: 0.01, step: 0.0001, label: `t panel (${u.length})` },
      { key: "q",     val: 1.0,   min: 0.1, max: 20, step: 0.1,  label: `q carga (${u.force}/${u.length}²)` },
      { key: "colD",  val: 0.16,  min: 0.05, max: 0.5, step: 0.01, label: `Col d (${u.length})` },
      { key: "colBf", val: 0.16,  min: 0.05, max: 0.5, step: 0.01, label: `Col bf (${u.length})` },
      { key: "colTf", val: 0.013, min: 0.003, max: 0.05, step: 0.001, label: `Col tf (${u.length})` },
      { key: "colTw", val: 0.008, min: 0.003, max: 0.05, step: 0.001, label: `Col tw (${u.length})` },
      { key: "vigD",  val: 0.20,  min: 0.05, max: 0.5, step: 0.01, label: `Vig d (${u.length})` },
      { key: "vigBf", val: 0.10,  min: 0.05, max: 0.5, step: 0.01, label: `Vig bf (${u.length})` },
      { key: "vigTf", val: 0.0085, min: 0.003, max: 0.05, step: 0.001, label: `Vig tf (${u.length})` },
      { key: "vigTw", val: 0.0056, min: 0.003, max: 0.05, step: 0.001, label: `Vig tw (${u.length})` },
      { key: "corrB", val: 0.06,  min: 0.02, max: 0.2, step: 0.01, label: `Corr b (${u.length})` },
      { key: "corrT", val: 0.004, min: 0.001, max: 0.02, step: 0.001, label: `Corr t (${u.length})` },
      { key: "supUx", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Ux" },
      { key: "supUy", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Uy" },
      { key: "supUz", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Uz" },
      { key: "supRx", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Rx" },
      { key: "supRy", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Ry" },
      { key: "supRz", val: 1, min: 0, max: 1, step: 1, label: "Apoyo Rz" },
    ],
    "eiffel": [], "arco": [], "puente": [], "twisted": [],
    "burj": [], "opera": [], "diagrid": [],
  };
