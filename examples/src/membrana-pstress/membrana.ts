...
export const membrana: ExampleDef = {
  ...
  params: {
    ...
    q:  { default: 10, min: 1, max: 30, step: 1, label: "q tracción (kN/m²)" },
  ...
  build(p, states) {
    ...
    // Para membrana: poblar membraneXX/YY/XY (no bending, porque no hay flexión)
    ...
    out.elementResults.forEach((er, i) => {
      membraneXX.set(i, [er.Mxx, er.Mxx, er.Mxx, er.Mxx]);
      membraneYY.set(i, [er.Myy, er.Myy, er.Myy, er.Myy]);
      membraneXY.set(i, [er.Mxy, er.Mxy, er.Mxy, er.Mxy]);
    });
    states.analyzeOutputs.val = { membraneXX, membraneYY, membraneXY };
