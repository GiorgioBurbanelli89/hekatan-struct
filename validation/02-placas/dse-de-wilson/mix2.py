...
print("")
print(" barrido fino de la restriccion (modo=lado, pt=1):")
mejor=(1e9,None,0)
for al in list(np.linspace(0,1,11))+list(np.linspace(1.2,6,25)):
    ...
    if abs(al-round(al,1))<1e-9 and (al*10)%5==0:
        print("  a=%5.2f  %s  ||dK||=%6.2f%%  <1%%: %d/9"
              % (al," ".join("%8.3f"%z for z in w), d, ok))
print("")
print("  mejor ||dK|| = %.2f %% con alpha=%.3f  (%d de 9 modos)"
