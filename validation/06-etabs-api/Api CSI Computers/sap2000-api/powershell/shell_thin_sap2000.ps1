# Shell Thin Cantilever — SAP2000 API (PowerShell) — FE05
...
$ModelPath = Join-Path $ModelDirectory "shell_thin.sdb"
...
[void] $SapModel.PropArea.SetShell_1("SHELL_THIN", 1, $false, "SHELL_MAT", 0, $t, $t)
...
Write-Output ("═" * 60)
Write-Output "  SHELL THIN (ShellType=1) — SAP2000 vs HekatanLab vs Membrana"
Write-Output ("═" * 60)
...
"`nSapResult_st = {0:e6}" -f $u_max_sap
"IndResult_st = {0:e6}" -f $delta_mem
"PercentDiff_st = {0:f4}" -f (($u_max_sap/$delta_mem)-1)
