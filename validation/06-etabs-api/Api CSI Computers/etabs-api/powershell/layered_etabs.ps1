# Layered Plate — ETABS API (PowerShell) — FE04 [0/90/90/0]
...
$ModelPath = Join-Path $ModelDirectory "plate_layered.edb"
...
# eShellType.ShellLayered (suele ser 4)
[void] $SapModel.PropArea.SetSlab("PLATE_LAYERED", 1, 4, "LAYER_MAT", $tTotal)

# Definir capas
$LayerName   = [string[]] @("L1","L2","L3","L4")
$Dist        = [double[]] $layerDistance
...
$S11Type     = [int[]] @(1,1,1,1); $S22Type = [int[]] @(1,1,1,1); $S12Type = [int[]] @(1,1,1,1)
[void] $SapModel.PropArea.SetSlabLayer("PLATE_LAYERED", $nLayers,
    [ref]$LayerName, [ref]$Dist, [ref]$ThickArr, [ref]$NumIntegPts,
    [ref]$MatPropArr, [ref]$MatAngleArr, [ref]$S11Type, [ref]$S22Type, [ref]$S12Type)
...
foreach ($a in $AreaNames) { [void] $SapModel.AreaObj.SetLoadUniform($a, "Q", -$q, 10, $true, "Global") }
...
foreach ($p in $PointNames) {
    ...
    if ($NumRes -gt 0 -and [math]::Abs($U3[0]) -gt $w_max) { $w_max = [math]::Abs($U3[0]) }
}
...
Write-Output ("═" * 60)
Write-Output "  LAYERED [0/90/90/0] — ETABS vs Navier isotrópica"
Write-Output ("═" * 60)
"  ETABS API     w_max = {0:e6}" -f $w_max
"  Navier iso    w_max = {0:e6}" -f $w_navier
"  Diff = {0:f2}%" -f ((($w_max/$w_navier)-1)*100)
"`nETABSResult_lay = {0:e6}" -f $w_max
"IndResult_lay   = {0:e6}" -f $w_navier
"PercentDiff_lay = {0:f4}" -f (($w_max/$w_navier)-1)
